...
#-------------------------------------------------------------------------------
#
#  $Header: //components/rel/scons.tz/1.0.7.1/build/scons/gensecimage/gensecimage.py#1 $
...
#
#See wiki for mbn builder which contains some historical info:
#http://qwiki.qualcomm.com/qct-drvsw/MbnBuilder
#Contains most update mbn tools released by boot:
#//source/qcom/qct/core/boot/scons/main/latest/shared/src/bsp/build/scripts/
#
import stat
import csv 
import itertools
...
signattrs_mandatory = ['msm_part', 'sw_id', 'oem_id', 'model_id', 'debug']
...
def create_file_to_sign(source_file_name, target_file_name, mbn_header_size, size_of_preamble, cert_chain_size_in = CERT_CHAIN_ONEROOT_MAXSIZE):
  ...
  if (source_header.sig_size != 0 or source_header.cert_chain_size != 0):
     raise RuntimeError, "Error: input file %s is signed. Please use unsigned image for input source file." % source_file_name
...
def sign_with_qpsa(image_to_sign, cert_dir):
  ...
  if os.path.exists(QPSA_PY) is True:
    if os.path.exists(QPSA_ZIP) is True:
      # Issue QPSA command
      ...
      log("QPSA zip file not found at location %s" % QPSA_ZIP)
      raise RuntimeError, "QPSA zip not found"
  else:
...
def read_ini_file(section_name):
  ...
  if not ((image_format == 'rawelf') or (image_format == 'htelf') or (image_format == 'mbn')):    
  ...
  if (stage == "qpsa:sign"):  
      ...
      for mandatory_option in signattrs_mandatory:
...
def copy_to_final_output ():
    ...
    if ((stage == "postsign") or (stage== "qpsa:sign")):    
...
def process_section (section_name, stage):
    ...
    if (image_format == 'mbn'):
      ...
      if ((stage == "presign") or (stage== "qpsa:sign")):
        #import pdb; pdb.set_trace()
        if has_magic_num is True:
...
def parse_args_and_validate(args):
    ...
    try:
      opts, remainder = getopt.getopt(args, "hv", ["help", "stage=", "section=", "config=", "signconfig=", "postsignconfig=", "encrypt_keys="])
    ...
    if not((stage == "presign") or (stage == "qpsa:sign") or (stage == "postsign")  \
        or (stage == "genhashtable") or (stage =="encryptonly")):   
      print "stage = " + stage    
      log("Missing stage to process\n")
      usage() 
      raise RuntimeError, "Missing stage to process"
    if (section_name == ''):
  ...
  while num_retry_remaining > 0:  
      ...
      if filelock_enable is False or lock.acquire():
          try:             
            ...
            print e #"Exception was triggered -- exiting"            
            sys.exit(2)
