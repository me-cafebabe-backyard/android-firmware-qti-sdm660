...
#-------------------------------------------------------------------------------
#
#  $Header: //components/rel/scons.tz/1.0.7.1/build/scons/build/build.py#1 $
...
def build (*args):
   ...
   try:
      ...
      lg.log("Running $Header: //components/rel/scons.tz/1.0.7.1/build/scons/build/build.py#1 $")
      ...
      if os.path.exists(setenv):
         ...
         new_env = lg.log_exec([setenv, compiler_id], capture_output=True)[1]
         new_env = [x.strip() for x in new_env.split('\n')]
         setenv_dict = {}
         for setting in new_env:
            if '=' in setting:
               var, value = setting.split('=', 1)
               setenv_dict[var] = value
      else:
