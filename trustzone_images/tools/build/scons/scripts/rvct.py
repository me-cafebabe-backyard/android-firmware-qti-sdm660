...
#-------------------------------------------------------------------------------
#
#  $Header: //components/rel/scons.tz/1.0.7.1/build/scons/scripts/rvct.py#1 $
...
def generate(env):
   ...
   #  Stack Protection
   #----------------------------------------------------------------------------
   if rvct_ver >= 4 and 'USES_STACK_PROTECTOR' in env:
   ...
   env.Replace(LINKFLAGS = "$ARMLD_ELF_CMD $ARMLD_MAP_CMD $ARMLD_INFO_CMD sizes,totals $ARMLD_LIST_CMD " \
      ...
      "${TARGET.base}.scl $ARMLD_NOREMOVE_CMD --any_placement=best_fit $LFLAGS")
