...
#-------------------------------------------------------------------------------
#
#  $Header: //components/rel/scons.tz/1.0.7.1/build/scons/scripts/doc_builder.py#1 $
...
#------------------------------------------------------------------------------
# Relpath support for Python 2.5.2 (used by Linux)
#  -- Copyright R.Barran 30/08/2004 - Distributed under PSF License
#------------------------------------------------------------------------------
...
def doxyfile_builder(target, source, env):
   ...
   if os.path.exists(template_doxyfile):
       ...
       for sourcepath in source:
          relativepath = relpath(str(env.subst(sourcepath)),doxyfile_dir)
...
def pdftemplate_builder(target, source, env):
    ...
    for sourcepath in source[1:]:
       ...
       if os.path.isdir(str(sourcepath)):
          copy_tree(str(sourcepath),latex_abs_dir,preserve_mode=0 )
