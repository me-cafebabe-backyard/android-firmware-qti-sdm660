...
def get_report(o_filelist, lib_filelist):
    ...
    for filename in o_filelist:
        try:
            ...
            o_table.append([r.get_code_size(), r.get_rodata_size(),  r.get_rwdata_size(), r.get_zidata_size(), os.path.basename(filename)])
    ...
    for filename in lib_filelist:
        try:
            ...
            lib_table.append([lib_code_size, lib_rodata_size,  lib_rwdata_size, lib_zidata_size, os.path.basename(filename)])
...
def create_report_for_mapfile(mapfile):
    ...
    for line in fp:
        ...
        if line[:4] == 'LOAD':
            if line[-4:] == '.lib':
                lib_filelist.append(line.split(' ')[1])
            elif line[-2:] == '.o':
                o_filelist.append(line.split(' ')[1])

    print '\n\n***** Processing Report: ' + mapfile + '.linkreport'
    report_text = get_report(o_filelist, lib_filelist)
