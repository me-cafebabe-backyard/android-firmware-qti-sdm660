...
/*===========================================================================
  ...
  $Header: //components/rel/core.tz/1.0.7.1/securemsm/sse/qsee/SecureDisplay/libgd/include/gd.h#1 $
  $DateTime: 2020/01/14 12:35:09 $
  $Author: pwbldsvc $

when       who      what, where, why
--------   ---      ------------------------------------
06/20/16   dr       Restructure and fix bugs in RenderImage + alpha blending fix
04/11/16   sn       Added gdGetUTF8StringLen
01/25/16   dr       Add gdImageSetRowPixels
04/26/15   ng       Disable unused functions
...
/* If 'truecolor' is set true, the image is truecolor;
   ...
   24/05/2013 - In order to make the alpha channel
   truly opaque on ION 'A' must occupy 8 bits, i.e.
   alpha max must be FF rather than 7F.
   This means that this version is NOT backwards
   compatible with gd 1.x.
   Also in ION alpha opaque is considered to be FF
   and transparent as 0, so these have been switched.
   The byte order for colours is ABGR so the GetColor
   functions have been adjusted to match this.
   The mask for red/green/blue have also been adjusted
   to always include alpha. */

#define gdAlphaMax 255
#define gdAlphaOpaque 255
#define gdAlphaTransparent 0
#define gdRedMax 255
#define gdGreenMax 255
#define gdBlueMax 255
#define gdTrueColorGetAlpha(c) (((c) & 0xFF000000) >> 24)
#define gdTrueColorGetBlue(c) (((c) & 0x00FF0000) >> 16)
#define gdTrueColorGetGreen(c) (((c) & 0x0000FF00) >> 8)
#define gdTrueColorGetRed(c) ((c) & 0x000000FF)
#define gdEffectReplace 0
#define gdEffectAlphaBlend 1
#define gdEffectNormal 2
#define gdEffectOverlay 3

#define gdPixelTypeUndefined -1
#define gdPixelTypeRGBA 0
#define gdPixelTypeRGB  1
...
typedef struct {
	...
	int w;
	int h;
	/* Font data; array of integers.
	   Run length encoded font representation. */
	int *data;
}
gdFontExt;

/* Text functions take these. */
typedef gdFontExt *gdFontPtrExt;
...
BGD_DECLARE(gdImagePtr) gdImageCreateExt (int sx, int sy, unsigned char **buf_ptr);

/* Creates a truecolor image (millions of colors). */
BGD_DECLARE(gdImagePtr) gdImageCreateTrueColor (int sx, int sy);

BGD_DECLARE(gdImagePtr) gdImageCreateTrueColorExt (int sx, int sy, int **buf_ptr);
...
BGD_DECLARE(void) gdImageDestroyEx (gdImagePtr im);
...
/* Optimized version of gdImageSetPixel which operates on an image row instead of a single pixel
 * Parameters:
*     gdImagePtr im                  - pointer to gdImage (the destination image)
*     unsigned int imX               - X coordinate of the destination image (X offset)
*     unsigned int imY               - Y coordinate of the destination image (destination row)
*     unsigned char* rowData         - a row of pixel data (the source row)
*     unsigned int rowFirstPixelIndex- first pixel in rowData to start copying
*     unsigned int rowLastPixelIndex - last pixel in the rowData that will be copied
*     int rowPixelType               - rowData pixel format (RGB, RGBA)
*/
BGD_DECLARE(int) gdImageSetRowPixels (gdImagePtr im, unsigned int imX, unsigned int imY,
		unsigned char* rowData, unsigned int rowFirstPixelIndex, unsigned int rowLastPixelIndex, int rowPixelType);

BGD_DECLARE(void) gdImageSetLine (gdImagePtr im, int x, int y, int length, int color);
...
BGD_DECLARE(void) gdImageFilledRectangle (gdImagePtr im, int x1, int y1, int x2, int y2,
					  int color);
BGD_DECLARE(void) gdImageQuickFilledRectangle (gdImagePtr im, int x1, int y1, int x2, int y2,
					  int color);
BGD_DECLARE(void) gdImageFilledRoundedRectangle (gdImagePtr im, int x1, int y1, int x2, int y2,
					  int color);
BGD_DECLARE(void) gdImageGradientFilledRectangle (gdImagePtr im, int x1, int y1, int x2, int y2,
		int color1, int color2, int direction);
...
#if 0
/* These font functions are not in use.
 * Please note that the implementation of these functions makes an assumption
 * that characters in the font are sequential.
...
BGD_DECLARE(int) gdFontFindIndex(int *data, int nchars, int c);
BGD_DECLARE(unsigned int) gdImageCharVarWidth (gdImagePtr im, gdFontPtrExt f, unsigned int x, unsigned int y, int c, unsigned int color);
BGD_DECLARE(unsigned int) gdImageStringVarWidth (gdImagePtr im, gdFontPtrExt f,
		unsigned int x, unsigned int y, unsigned char *s, unsigned int len, unsigned int color);
BGD_DECLARE(unsigned int) gdGetMaxLenForWidth (gdFontPtrExt f, unsigned char *s, unsigned int len, unsigned int max_width, unsigned int* missing_char);
BGD_DECLARE(unsigned int) gdGetUTF8StringLen (const unsigned char *s, unsigned int byte_length);
...
BGD_DECLARE(void) gdImageStringExt (gdImagePtr im, gdFontPtrExt f, int x, int y,
                                     unsigned char *s, int color);
BGD_DECLARE(int) gdIsValidUTF8String (unsigned char *s, unsigned int len);
BGD_DECLARE(int) getUnicode(const unsigned char *s);
...
/* A simpler way to obtain an opaque truecolor value for drawing on a
   ...
   For ION the color is expected as ABGR so the bit
   shifting has been changed to reflect this.
   Default (opaque) alpha is also 255 rather
   than 0 as in original representation.*/

#define gdTrueColor(b, g, r) (((255U) << 24) + \
				      ((b) << 16) + \
				      ((g) << 8) +  \
				      (r))

/* Returns a truecolor value with an alpha channel component.
   gdAlphaMax (255) is opaque, 0 is transparent.
   Bit shifting is adjusted to have the final value
   in the expected AABBGGRR byte order. */

#define gdTrueColorAlpha(a, b, g, r) (((a) << 24) + \
				      ((b) << 16) + \
				      ((g) << 8) +  \
				      (r))
...
BGD_DECLARE(void) gdImageFilledEllipse (gdImagePtr im, int cx, int cy, int w, int h,
                                        int color);
BGD_DECLARE(void) gdImageGradientFilledEllipse (gdImagePtr im, int mx, int my, int w,
                                                 int h, int colour1, int colour2, int direction);
...
/* Copy part of the srcImage to the same location on the dstImage.
 * Assumes images of the same size, 32bit and do not require alpha blending. */
BGD_DECLARE(int) gdImageCompatibleCopy(gdImagePtr dstImage, gdImagePtr srcImage,  int x1, int y1, int x2, int y2);
...
} gdDisplayInfo, *gdDisplayInfoPtr;
...
typedef struct{
    ...
    int colorType;
    int interlaceType;
} gdImageData, *gdImageDataPtr;

BGD_DECLARE(int) gdGetImageData (int l_fd, gdImageDataPtr imgData);

BGD_DECLARE(int) gdImageDrawFromPng (int l_fd, gdImagePtr img_ptr, gdDisplayInfoPtr dispInfo);
