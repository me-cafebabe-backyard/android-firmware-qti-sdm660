...
static object_ctx_t me = {CGuestVM_UID};
static uint32 FIQ_count = 0;
...
static int32_t CGuestVM_handleCommand(object_ctx_t *me, const void *req_ptr, size_t req_len, const void *rsp_ptr, size_t rsp_len, size_t *rsp_lenout)
{
  ...
  switch (*command)
  {
    ...
    case TEE_TEST_FIQ_TIMER_TEE:
    {
      ...
      /* Kick off the interrupt */
      TEST_OK(IPVC_startTimer(TEE_Outbound_Object, 10, 0, &registeredIntnum))
      while (!interrupt_exit) {
        ;
      }
      ret = Object_OK;
...
static int32_t CGuestVM_handleInterrupt(object_ctx_t *me, int32_t bpos_val)
{
  FIQ_count++;
  return Object_OK;
}

static IGuestVM_DEFINE_INVOKE(CGuestVM_invoke, CGuestVM_, object_ctx_t *)
...
{
  return (Object) { CGuestVM_invoke, &me };
}
