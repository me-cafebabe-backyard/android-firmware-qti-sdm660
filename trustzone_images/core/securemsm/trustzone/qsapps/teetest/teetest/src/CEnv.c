...
// Implementation
//------------------------------------------------------------------------
static object_ctx_t me;

#define CEnv_release(ctx)   Object_OK
#define CEnv_retain(ctx)    Object_OK

static int32_t
CEnv_open(object_ctx_t *me, uint32_t id_val, Object *obj_ptr)
{
...
static int32_t
CEnv_log(object_ctx_t *me, const void *text_ptr, size_t text_len)
{
  return Object_OK;
}

static int32_t
CEnv_exit(object_ctx_t *me, int32_t code_val)
{
  return Object_OK;
}

static IEnv_DEFINE_INVOKE(CEnv_invoke, CEnv_, object_ctx_t *)
