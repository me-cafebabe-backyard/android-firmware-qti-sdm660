...
/*===========================================================================
  ...
  $Header: //components/rel/core.tz/1.0.7.1/securemsm/trustzone/qsapps/sampleapp/src/qsee_api_tests.c#1 $
  $DateTime: 2020/01/14 12:35:09 $
  $Author: pwbldsvc $

  # when       who     what, where, why
  # --------   ---     ---------------------------------------------------------
  # 03/17/17   vg      Added qsee i2c platform structures in qsee_i2c_test
...
int run_buses_test(uint32 I2cDeviceId,uint32 slaveaddr)
{
   int ret = 0;
   int retClose = 0;
...
int run_debugpolicy_test()
{
  ...
  qsee_log(QSEE_LOG_MSG_DEBUG, "debugpolicy test: posi addr = %x , length = %d", debugPolicyBufPtr, TEST_DEBUGPOLICY_BUF_SIZE);
  ret = read_debug_policy(debugPolicyBufPtr, TEST_DEBUGPOLICY_BUF_SIZE, &debugPolicyOutSize);
  TEST_ASSERT(ret, Object_OK, "debug policy should be loaded");
