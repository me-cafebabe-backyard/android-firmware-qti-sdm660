...
/*===========================================================================
  ...
  $Header: //components/rel/core.tz/1.0.7.1/securemsm/trustzone/qsapps/sampleapp/src/app_main.c#1 $
  $DateTime: 2020/01/14 12:35:09 $
  $Author: pwbldsvc $

  # when       who     what, where, why
  # --------   ---     ---------------------------------------------------------
  # 03/17/17   vg      Added QSEE logs to print DeviceId & Slaveaddr
  # 01/24/17   vg      Added i2ccore & slaveid as inputparam to i2c test
...
#define CLIENT_CMD80_RUN_SMCI_TIME_LISTENER_TEST              80
#define CLIENT_CMD81_RUN_SMCI_NAME_LISTENER_TEST              81
#define CLIENT_CMD82_RUN_HLOS_TIME                            82
...
extern int run_buses_test(uint32 I2cDeviceId,uint32 slaveaddr);
...
extern int run_smci_time_test(void);
extern int run_smci_name_test(void);
extern int run_hlos_time(void);

#define SYSCALL_mink_server_exit        0xE  /** Must match internal value */
typedef struct UThreadSyscallResult UThreadSyscallResult;
struct UThreadSyscallResult {
   int32_t result;
   uint32_t aux;
};
extern UThreadSyscallResult mink_generic_syscall(uint32_t num, intptr_t r0, intptr_t r1, intptr_t r2, intptr_t r3);
                        ...
                        void* rsp, uint32 rsplen)
{
   ...
   else if (cmd_id == CLIENT_CMD103_RUN_WLIST_TEST)
   {
   ...
   switch (cmd_id) {
      ...
      case CLIENT_CMD16_RUN_BUSES_TEST:
		 ...
		 qsee_log(QSEE_LOG_MSG_DEBUG, "DeviceId: %u,  SlaveAddr:%u", (uint32)cmd_ptr->data, (uint32)cmd_ptr->data2);
		 
         retval = run_buses_test((uint32)cmd_ptr->data, (uint32)cmd_ptr->data2);
         if (retval) {
            rsp_ptr->status = FAILED;
			qsee_log(QSEE_LOG_MSG_ERROR, "DeviceId: %u,  SlaveAddr:%u", (uint32)cmd_ptr->data, (uint32)cmd_ptr->data2);
      ...
      case CLIENT_CMD23_RUN_SECURE_UI_TOUCH_TEST:
         ...
         break;
      case CLIENT_CMD24_RUN_CP_UTILS_TAG_MEM:
#ifdef USES_NO_CP
         retval = cp_utils_tag_mem((uint32)(((tz_cp_tag_mem_req_t*)cmd)->tag),
                                   (tz_mem_array_s_t*)(&(((tz_cp_tag_mem_req_t*)cmd)->secBufferHandle)),
                                   (uint32)(((tz_cp_tag_mem_req_t*)cmd)->secBuffLength));
#else
         retval = FAILED;
         qsee_log(QSEE_LOG_MSG_ERROR, "This feature is not supported on external builds");
#endif
         ((tz_cp_tag_mem_rsp_t *)rsp)->ret = retval;
         break;
  ...
  case CLIENT_CMD56_RUN_SP_KEYMASTER_TEST:
    ...
    case CLIENT_CMD54_MINK_SERVER_EXIT:
      mink_generic_syscall(SYSCALL_mink_server_exit, 0, 0, 0, 0);
      /* Invoking mink_server_exit should trigger an application fault, if
         this execution chain continues/returns success then the test case is
         at fault.

         We need to return success if this is the case to differentiate the
         fault scenario from the application fault return status.
      */
      qsee_log(QSEE_LOG_MSG_ERROR, "****FAILED : mink_server_exit");
      rsp_ptr->status = SUCCESS;
      break;
     ...
     case CLIENT_CMD60_RUN_HDCP_KEY_WRITE_TEST:
      ...
      case CLIENT_CMD80_RUN_SMCI_TIME_LISTENER_TEST:
         retval = run_smci_time_test();
         if (retval) {
            rsp_ptr->status = FAILED;
            qsee_log(QSEE_LOG_MSG_ERROR, "****FAILED : SMC Invoke Time Test");
         } else {
            rsp_ptr->status = SUCCESS;
            qsee_log(QSEE_LOG_MSG_DEBUG, "****PASSED : SMC Invoke Time Test");
         }
         break;

      case CLIENT_CMD81_RUN_SMCI_NAME_LISTENER_TEST:
         retval = run_smci_name_test();
         if (retval) {
            rsp_ptr->status = FAILED;
            qsee_log(QSEE_LOG_MSG_ERROR, "****FAILED : SMC Invoke Name Test");
         } else {
            rsp_ptr->status = SUCCESS;
            qsee_log(QSEE_LOG_MSG_DEBUG, "****PASSED : SMC Invoke Name Test");
         }
         break;

      case CLIENT_CMD82_RUN_HLOS_TIME:
         retval = run_hlos_time();
         if (retval) {
            rsp_ptr->status = FAILED;
            qsee_log(QSEE_LOG_MSG_ERROR, "****FAILED : Get HLOS time (via listener)");
         } else {
            rsp_ptr->status = SUCCESS;
            qsee_log(QSEE_LOG_MSG_DEBUG, "****PASSED : Get HLOS time (via listener)");
         }
         break;
