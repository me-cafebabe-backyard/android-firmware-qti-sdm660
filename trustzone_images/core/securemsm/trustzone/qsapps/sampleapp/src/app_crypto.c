...
/*===========================================================================
  ...
  $Header: //components/rel/core.tz/1.0.7.1/securemsm/trustzone/qsapps/sampleapp/src/app_crypto.c#1 $
                                ...
                                uint8 *ct2)
{
   ...
   /*hash_ctx validation is alredy taken are in qsee_hash_init 
    this check is for KW errors*/
  if(hash_ctx == NULL)
  {
    qsee_log(QSEE_LOG_MSG_ERROR,"qsee_hash_init context creation  failed");
    status = -E_FAILURE;
  }
   /*-----------------------------------------------------------------------
     Set hash mode
                             ...
                             uint8 verify)
{
   ...
   if ((E_SUCCESS == status)&& (verify == 1)&& pt_tmp)
   {
      memset(pt_tmp, 0, pt_len);
   }
