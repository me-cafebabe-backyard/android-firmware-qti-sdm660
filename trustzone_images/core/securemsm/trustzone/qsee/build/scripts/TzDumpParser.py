...
def print_out_str(out_file, in_string) :
  if out_file is None :
    print (in_string)
  else :
...
elif sys.argv[1] not in (a_family_chipsets + b_family_chipsets):
  print "\nUnknown chipset %s.  Supported values:\n" % (sys.argv[1])
  for chip in a_family_chipsets + b_family_chipsets:
    print "%s" % (chip)
  sys.exit(1)
...
try:
  ShMemDumpFile = open(sys.argv[2], "rb")
  if sys.argv[1] in a_family_chipsets:
    # If only one binary is specified, it contains all of the information
    if(len(sys.argv) > 4):
      # Multiple binaries
      LogDumpFile = open(sys.argv[3], "rb")
      ShMemDumpFile.seek(SHARED_IMEM_TZ_BASE_OFF_2,0)
      TzOutPutFile = open(sys.argv[4], "wb")
    else:
      # Single Binary
      LogDumpFile = open(sys.argv[2], "rb")
      ShMemDumpFile.seek(SHARED_IMEM_TZ_BASE_OFF,0)
      TzOutPutFile = open(sys.argv[3], "wb")
    TZDiagAddr = (struct.unpack ('<I',ShMemDumpFile.read(4)))[0] & 0x000FFFFF
  elif sys.argv[1] in b_family_chipsets:
    LogDumpFile = open(sys.argv[3], "rb")
    TzOutPutFile = open(sys.argv[4], "wb")
    ShMemDumpFile.seek(MSGRAM_TZ_BASE_OFF,0)
...
if (TZDiagAddr & 0xFFF): # memory offset not aligned one
  ...
  print_out_str(TzOutPutFile, " SHARED IMEM TZ Not Pointing to TZ Log! ")
  sys.exit(0)
...
while True:
  try:
    ...
    if (g_tzbsp_diag[0] != 0x747A6461):  #0x747A6461 == tzda
      ...
      print_out_str(TzOutPutFile, " TZBSP Log Not available or corrupted! ")
      sys.exit(0)
    
    # -----------------------------------------------
    # Prepare the Log File Now
    # ----------------------------------------------
    print_out_str(TzOutPutFile, "===================================================== ")
    print_out_str(TzOutPutFile, "Extracted TZBSP Log ")
    print_out_str(TzOutPutFile, "===================================================== \n")
     
    # print the No of CPU's
    print_out_str(TzOutPutFile, ("CPU Count -> %d " % g_tzbsp_diag[2]))
    
    # print the VMID's 
    print_out_str(TzOutPutFile, "\n-------- VMID Status --------- ")
    print_out_str(TzOutPutFile, "VMID : VMID Desc")
    ...
    for i in range (0, TZBSP_DIAG_NUM_OF_VMID) : 
      ...
      if g_tzbsp_diag_vmid[0] != 0xff:
        print_out_str(TzOutPutFile, ("%4x : %s " % (g_tzbsp_diag_vmid[0], printable_string (g_tzbsp_diag_vmid[1:7]))))
    
    # print the Boot status 
    print_out_str(TzOutPutFile, "\n---------------------------------- Boot Status -------------------------- ")
    print_out_str(TzOutPutFile, "CPU : WarmEntry  : WarmExit   : PCEntry   : PCExit    : Warm JumpAddr: JumpInstr ")
    ...
    for i in range(0,g_tzbsp_diag[2]) :                 # g_tzbsp_diag[2] == CPU Count
      ...
      print_out_str(TzOutPutFile, ("%3x : 0x%08x : 0x%08x : 0x%08x: 0x%08x: 0x%08x   : 0x%08x  " % (i, 
    ...
    print_out_str(TzOutPutFile, "\n-------- Reset Status --------- ")
    print_out_str(TzOutPutFile, "CPU : Reset Reason/Type : Reset Count ")
    ...
    for i in range (0, g_tzbsp_diag[2]) :  # g_tzbsp_diag[2] == CPU Count
      ...
      print_out_str(TzOutPutFile, ("%3x : 0x%08x (%s) : 0x%08x" % (i, g_diag_reset_info[0], Error_Fatal_table[g_diag_reset_info[0]].ljust(30), g_diag_reset_info[1])))
    ...
    print_out_str(TzOutPutFile, "Index: IntInfo : Avail : IntType: IntDesc         : IntrCnts => CPU0              : CPU1              : CPU2              : CPU3 ")
    ...
    for i in range (0, num_interrupts) :  
      ...
      if 1==g_tzbsp_diag[2]:  # for CPU Count 1 , interupt counter will be 4
        ...
        print_out_str(TzOutPutFile, ("%5d: 0x%04x  : 0x%03x : 0x%03x  : %15s : IntrCnts => 0x%-16x" 
      ...
      if 2==g_tzbsp_diag[2]:  # for CPU Count 2 , interupt counter will be 4
        ...
        print_out_str(TzOutPutFile, ("%5d: 0x%04x  : 0x%03x : 0x%03x  : %15s : IntrCnts => 0x%-16x: 0x%-16x" 
                                           % (i, g_tzbsp_diag_int[0], g_tzbsp_diag_int[1], g_tzbsp_diag_int[3],
                                           printable_string(g_tzbsp_diag_int[4:3+TZBSP_MAX_INT_DESC]),
      ...
      if 4==g_tzbsp_diag[2]:  # for CPU Count 4, interupt counter will be 4 
        ...
        print_out_str(TzOutPutFile, ("%5d: 0x%04x  : 0x%03x : 0x%03x  : %15s : IntrCnts => 0x%-16x: 0x%-16x: 0x%-16x: 0x%-16x" 
                                           % (i, g_tzbsp_diag_int[0], g_tzbsp_diag_int[1], g_tzbsp_diag_int[3],
                                           printable_string(g_tzbsp_diag_int[4:3+TZBSP_MAX_INT_DESC]),
    ...
    print_out_str(TzOutPutFile, "\n------------------- ******************************* ----------------- ")
    print_out_str(TzOutPutFile, "------ TZBSP DIAG RING BUFF, each line for an individual log ---- ")
    print_out_str(TzOutPutFile, "------------------- ******************************* ----------------- ")
    LogDumpFile.close()
    ...
    while line :
      ...
      if "Error Fatal" not in line:
        print_out_str(TzOutPutFile, line)
      line = g_tzbsp_ringbuff.readline()
