...
#-------------------------------------------------------------------------------
#
#  $Header: //components/rel/core.tz/1.0.7.1/bsp/build/scripts/defaults.py#1 $
...
def generate(env):
   ...
   if 'BUILD_SCRIPTS_ROOT' not in env:
      ...
      if build_scripts_root is None:
         build_scripts_root = '${BUILD_ROOT}/core/bsp/build/scripts'
   ...
   if 'BUILD_SCRIPTS_OEM_ROOT' not in env:
      ...
      if build_scripts_oem_root is None:
         build_scripts_oem_root = '${BUILD_ROOT}/core/bsp/build/scripts'
...
def set_db_name(env):
   # create file name
   bid = env.get('OEM_BUILD_ID')
   db_ext = ".dblite"
   
   if bid is None:
      bid = env.get('BUILD_ID')
