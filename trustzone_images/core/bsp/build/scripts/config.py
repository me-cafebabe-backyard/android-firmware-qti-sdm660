...
#-------------------------------------------------------------------------------
#
#  $Header: //components/rel/core.tz/1.0.7.1/bsp/build/scripts/config.py#1 $
...
def generate(env):
   env.Replace(BUILDDATA_ROOT = "${BUILD_ROOT}/core/bsp/build/data")
...
def init_build_config(env, target_files=True):
   ...
   if len(image_tools) > 0:
      ...
      env.LoadToolScript("${BUILD_SCRIPTS_ROOT}/mbn_builder.py")
      env.LoadToolScript("${BUILD_SCRIPTS_ROOT}/bin_builder.py")
      env.LoadToolScript("${BUILD_SCRIPTS_ROOT}/hex_builder.py")
      env.LoadToolScript("${BUILD_SCRIPTS_ROOT}/kernel_builders.py")
      env.LoadToolScript("${BUILD_SCRIPTS_ROOT}/scl_builder.py")
      env.LoadToolScript("${BUILD_SCRIPTS_ROOT}/doc_builder.py")
      env.LoadToolScript("${BUILD_SCRIPTS_ROOT}/buildspec_builder.py")
   return None
...
def init_incpaths(env):
   ...
   #toolpath = ['${BUILD_ROOT}/core/bsp/build/scripts']
...
def init_au_incpaths(env):
   ...
   if not env.PathExists("".join([externalpath_file_path, "/", externalpath_file_name, externalpath_file_ext])):
      #externalpath_file_path = "${BUILD_ROOT}/core/bsp/build/scripts"
...
#===============================================================================
def init_image_vars (env, alias_list, proc, config, plat='l4', 
   ...
   # take into account possible additional build path form OEM
   if 'OEM_BUILDPATH' in env:
      env.Replace(BUILDPATH = env.subst("${QC_BUILDPATH}/${OEM_BUILDPATH}"))
      env.Replace(SHORT_BUILDPATH = env.subst("${QC_SHORT_BUILDPATH}/${OEM_BUILDPATH}"))
   else:
      env.Replace(BUILDPATH = qc_buildpath)
      env.Replace(SHORT_BUILDPATH = qc_short_buildpath)
   
   #----------------------------------------------------------------------------   
   # init some vars based on configuration
   if config == 'none': 
      # NO PROC defined configuration (experimental, bringup)
      # set some flags, to get pass errors for scripts that are expecting 
      # something to be set all the time, this config 'none' it is used 
      # as a dummy to build things like documents were modem/apps, etc 
      # don't make any sense, but some scripts still look for the 
      # definition.
      env.Replace(BUILD_ID = env.subst('${BUILD_ID}'))
      env.Replace(CUST_H = "none")
