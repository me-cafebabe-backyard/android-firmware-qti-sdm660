...
#-------------------------------------------------------------------------------
#
#  $Header: //components/rel/core.tz/1.0.7.1/bsp/build/scripts/platform_l4.py#1 $
...
def generate(env):
   ...
   if do_local_files:
      ...
      quartz_machine_xml = env.QuartzXMLBuilder('quartz_cfg_machine_${TARGET_NAME}',
      ...
      #----------------------------------------------------------------------------
      l4_scl_hdr_file = env.L4SclHeaderBuilder("cust_l4_scl_${TARGET_NAME}",
         quartz_xml)
   ...
   if do_link:
      ...
      #-------------------------------------------------------------------------
      '''
      image_elf = env.Program("${TARGET_NAME}", image_objs, LIBS=image_libs)
      env.Depends(image_elf, target_scl)
      
      image_map = env.subst('${TARGET_NAME}.map')
      image_sym = env.subst('${TARGET_NAME}.sym')
      
      env.Clean(image_elf, image_map)
      env.Clean(image_elf, image_sym)
      '''
      #'''
      image_elf = env.AddProgram(source=image_objs, LIBS=image_libs)
      env.Depends(image_elf, target_scl)
      #'''
   ...
   if do_post_link:      
      ...
      # DL segments, etc.
      #-------------------------------------------------------------------------
      target_elf = env.ModifyElfFlagsBuilder('${TARGET_NAME}_modifyelf', 
      ...
      # Build CMM scripts
      #-------------------------------------------------------------------------
      quartz_constant_cmm = env.QuartzConstBuilder('quartz_const_${TARGET_NAME}', [
