...
#-------------------------------------------------------------------------------
#
#  $Header: //components/rel/core.tz/1.0.7.1/bsp/build/scripts/rvct.py#1 $
...
def generate(env):
   ...
   armToolsVersion = re.compile('[rR][vV][cC][tT]([0-9])').search(armtools)
   
   if armToolsVersion is not None:
      env.Replace(ARMTOOLS = armtools)
      rvct_ver = int(armToolsVersion.group(1))
   else:
      env.Replace(ARMTOOLS = "RVCT2")
      armtools = 'Assuming RVCT2xx'
      rvct_ver = 2      

   # Pass along the RVCT major version number to other interested parties
   env.Append(ARMTOOLSVERSION = rvct_ver)
   ...
   # Init FLAGS      
   env.Replace(CCFLAGS = "$ARMCC_CPU $ARMCC_APCS $ARMCC_CODE $ARM_DBG $ARMCC_OPT $BUILD_FLAGS $CC_DFLAGS $CFLAGS ")
   env.Replace(CXXFLAGS = "$ARMCC_CPU $ARMCXX_APCS $ARMCXX_CODE $ARM_DBG $ARMCXX_OPT $BUILD_FLAGS $CC_DFLAGS $CFLAGS ")
