...
#-------------------------------------------------------------------------------
#
#  $Header: //components/rel/core.tz/1.0.7.1/bsp/build/scripts/hexagon.py#1 $
...
def generate(env):
   ...
   if env['PLATFORM'] in ['windows', 'win32', 'cygwin'] and hexagon_rtos_release == '1.1':
      env.LoadToolScript('cygwin', toolpath = ['${BUILD_ROOT}/core/bsp/build/scripts'])
   ...
   # since we may have change to cygwin re init formatting
      env.LoadToolScript('formatting', toolpath = ['${BUILD_ROOT}/core/bsp/build/scripts'])       
   ...
   if 'USES_BLAST' in env:
      ...
      env.Replace(QDSP6_LIBDIR = "${L4Q6_ROOT}/lib/qube/qdsp6${Q6VERSION}")
   ...
   env.Replace(QDSP6_CPPPATH = [
      "${QDSP6_RELEASE_DIR}/dinkumware/include/common",
      "${QDSP6_RELEASE_DIR}/dinkumware/include/common/c++",
      "${QDSP6_RELEASE_DIR}/qc/include/common",
      "${QDSP6_RELEASE_DIR}/qc/include/qube",
   ])
   ...
   #import pdb; pdb.set_trace()
   #env.Replace(CC = env.RealPath("${QDSP6BIN}/qdsp6-gcc"))
   env.Replace(CC = QDSP6BIN + "qdsp6-gcc${EXE_EXT}")

   # HEXAGON set ANSI C++ compiler
   #env.Replace(CXX = env.RealPath("${QDSP6BIN}/qdsp6-g++"))
   env.Replace(CXX = QDSP6BIN + "qdsp6-g++${EXE_EXT}")

   # HEXAGON assembler
   #env.Replace(AS = env.RealPath("${QDSP6BIN}/qdsp6-as"))
   env.Replace(AS = QDSP6BIN + "qdsp6-as${EXE_EXT}")

   # HEXAGON library archiver
   #env.Replace(AR = env.RealPath("${QDSP6BIN}/qdsp6-ar"))
   env.Replace(AR = QDSP6BIN + "qdsp6-ar${EXE_EXT}")

   # HEXAGON linker
   #env.Replace(LINK = env.RealPath("${QDSP6BIN}/qdsp6-ld"))
   env.Replace(LINK = QDSP6BIN + "qdsp6-ld${EXE_EXT}")
   
   # HEXAGON library archiver
   #env.Replace(RANLIB = env.RealPath("${QDSP6BIN}/qdsp6-ranlib"))
   ...
   env.Replace(HEXAGONLD_MAP_CMD = "-M")
   ...
   #-------------------------------------------------------------------------------
   env.Replace(HEXAGON_MACHINE = "-m${Q6VERSION} -mv1-v2-uncached-data")
   ...
   if hexagon_ver >= 2:
      env.Replace(HEXAGON_LANIND = "-fdata-sections -ffunction-sections " \
         "-mno-literal-pool-addresses -nostdlib -nostdinc -fno-exceptions " \
   ...
   # Init FLAGS   
   env.Replace(CCFLAGS   = "$HEXAGONCC_WARN $HEXAGON_MACHINE $HEXAGON_UNDEF $HEXAGONCC_OPT $HEXAGON_DBG $HEXAGON_LANIND $BUILD_FLAGS $CC_DFLAGS $CFLAGS ")
   env.Replace(CXXFLAGS  = "$HEXAGONCXX_WARN $HEXAGON_MACHINE $HEXAGON_UNDEF $HEXAGONCC_OPT $HEXAGON_DBG $HEXAGON_LANIND $BUILD_FLAGS $CC_DFLAGS $CFLAGS ")
   env.Replace(ASMFLAGS  = "$HEXAGONCC_WARN $HEXAGON_MACHINE $HEXAGON_UNDEF $HEXAGONCC_OPT $HEXAGON_DBG $HEXAGON_LANIND $BUILD_FLAGS $CC_DFLAGS ")
   env.Replace(ASFLAGS   = "$HEXAGONCC_WARN $HEXAGON_MACHINE $HEXAGON_UNDEF $HEXAGONCC_OPT $HEXAGON_DBG $HEXAGON_LANIND $BUILD_FLAGS $CC_DFLAGS ")
   ...
   CCCOM = "${TEMPFILE('$CC $CCFLAGS $_CCCOMCOM $INCPATHS $_QDSP6_CPPINCFLAGS $HEXAGON_INHIBIT_WARN_CMD $HEXAGON_OUTPUT_CMD $TARGET.posix $HEXAGON_COMPILE_CMD $SOURCES.posix')}"
   ...
   CXXCOM = "${TEMPFILE('$CXX $CXXFLAGS $_CCCOMCOM $INCPATHS $_QDSP6_CPPINCFLAGS $HEXAGON_INHIBIT_WARN_CMD $HEXAGON_OUTPUT_CMD $TARGET.posix $HEXAGON_COMPILE_CMD $SOURCES.posix')}"
   ...
   env.Replace(ASASCOM   =  "${TEMPFILE('$CC $CCFLAGS $_CCCOMCOM $INCPATHS $_QDSP6_CPPINCFLAGS $HEXAGON_INHIBIT_WARN_CMD $HEXAGON_OUTPUT_CMD $TARGET.posix $HEXAGON_COMPILE_CMD $SOURCES.posix')}")
   ...
   if 'USES_BLAST' in env:
      ...
      env.Replace(LINKFLAGS = "--defsym COMPILE_FOR_RTOS=3 -lstdc++ -g -r -m${Q6VERSION} -nostdlib ${LFLAGS}")
   ...
   if 'USES_BLAST' in env:
      ...
      env.Replace(QDSP6OBJS1  = [
         '${QDSP6_RELEASE_LIB_DIR}/crt0.o',
         '${QDSP6_RELEASE_LIB_DIR}/init.o',
      ])

      env.Replace(QDSP6OBJS3  = [
         '${QDSP6_LIBDIR}/libqube.a',
         '${QDSP6_RELEASE_LIB_DIR}/libc.a',
         '${QDSP6_RELEASE_LIB_DIR}/libqcc.a',
         '${QDSP6_RELEASE_LIB_DIR}/libstdc++.a',
         ])
