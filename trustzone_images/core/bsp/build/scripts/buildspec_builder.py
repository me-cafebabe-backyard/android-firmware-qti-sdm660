...
def buildspec_generate(env):
   ...
   if not SCons.Script.COMMAND_LINE_TARGETS[0]:
      env.PrintError("cannot execute --buildspec option without a build Target!! ")
      raise RuntimeError, 'invoking --buildspec with a build Target'
...
def buildspec_print_cmd_line(s, target, src, env):
   ...
   if len(strlist) > 1:
      ...
      currentLogFile = str(env['CURRENTLOGFILE'])
      if os.environ.get('BUILDSPEC') == 'CACHE':
         listOfTags.append(logstr)
      else:
         buildspec_writeline(currentLogFile, logstr, env)
