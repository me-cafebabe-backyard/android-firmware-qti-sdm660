...
@file DALSys.h
        ...
        Copyright (c) 2008,2010-2015 Qualcomm Technologies Inc.
        All Rights Reserved.
        Qualcomm Confidential and Proprietary

$Header: //components/rel/core.tz/1.0.7.1/api/dal/DALSys.h#1 $
=============================================================================*/
...
/*
  ...
  and will have garbage values.  If heap is not initialized when realloc is 
  invoked it will ASSERT. If the block cannot be resized, ptr is not NULL 
  and size is not 0, then DAL_ERROR is returned and the original block 
  is left untouched. If the ptr block is successfully resized and the 
  returned value is different from the ptr value passed in, the old block 
  passed in must be considered deallocated and no longer usable.
  (i.e. do NOT try to mem_free it!)  

  @param dwSize   : Requested size of memory to be alloced
         ppMem    : Return parameter, callee will populate this to point to the
                    new block of allocated memory, valid only upon DAL_SUCCESS 
                    return code else pointing to old block.
  @see DALSYS_Free
