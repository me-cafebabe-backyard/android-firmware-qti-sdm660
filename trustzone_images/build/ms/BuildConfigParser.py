...
def parseVariable(varXml, getDependenciesFunc, nodeMap = None):
    ...
    ret = {
        "export" : not parseBooleanProperty(varXml, "intermediate", \
                default = False),
    }
    if method == "declare":
        ret["value"] = value
...
def checkProperty(xmlNode, propertyName, default = None, allowed = None, \
    ...
    """Check that the provided XML node has something defined for the given
       ...
       ensure that the value is NOT contained within 'forbidden'. Error message
       is generic unless explicitly specified via 'errorMsg'."""
