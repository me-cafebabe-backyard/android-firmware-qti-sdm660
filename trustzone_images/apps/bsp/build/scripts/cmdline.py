...
#-------------------------------------------------------------------------------
#
#  $Header: //components/rel/apps.tz/1.0.7.1/bsp/build/scripts/cmdline.py#1 $
...
def generate(env):
   ...
   # select correct python command if path is defined in environment
   pythoncmd = "python"
   ...
   if 'BUILD_SCRIPTS_ROOT' not in env:
      BUILD_SCRIPTS_ROOT = env.RealPath('${BUILD_ROOT}/core/bsp/build/scripts', posix=True)
      env.Replace(BUILD_SCRIPTS_ROOT = BUILD_SCRIPTS_ROOT)

   if 'BUILD_SCRIPTS_OEM_ROOT' not in env:
      BUILD_SCRIPTS_OEM_ROOT = env.RealPath('${BUILD_ROOT}/core/bsp/build/scripts', posix=True)
   ...
   print "COREBSP_ROOT  = %s " % (COREBSP_ROOT)   
   print "L4_ROOT       = %s " % (L4_ROOT)
   ...
   vars.AddVariables(
      ...
      ('USES_FLAGS', 'Adds USES_FLAGS to the environment USES_FLAGS=USEX_X,USES_Y,USES_Z', None, validator),
      ('BUILD_ID', 'Target Build ID from AMSS world i.e. BUILD_ID=SKCAAAF', None, validator),
      ('BUILD_VER', 'Target Build Version i.e. BUILD_VER=1109', None, validator),
   ...
   # Need to be after initial variables becuase of dependencies
   vars.AddVariables(
      SCons.Script.PathVariable(
         'LIB_ROOT',
         'The root path where libaries will be installed',
         BUILD_ROOT +'/build/ms/corelibs/'+ env['BUILD_ID'], PathVariable.PathIsDirCreate),

      SCons.Script.PathVariable(
         'MBN_ROOT',
         'The root path where MBNs will be installed',
         BUILD_ROOT +'/build/ms/bin/'+ env['BUILD_ID'], PathVariable.PathIsDirCreate),

         # Disable this to allow other scripts to add their own variables
         #SCons.Script.PathVariable(
         #   'OBJ_ROOT',
         #   'The root path where objs will be installed',
         #   BUILD_ROOT +'build/ms/coreobjs', PathVariable.PathExists),
   )
   ...
   print "PLAT_ROOT     = %s" % env['PLAT_ROOT']   
