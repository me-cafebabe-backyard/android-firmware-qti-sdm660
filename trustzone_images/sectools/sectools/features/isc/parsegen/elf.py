...
from sectools.features.isc.defines import SECBOOT_VERSION_2_0, AUTHORITY_QTI
...
class SecParseGenElf(SecParseGenMbn):
    ...
    def __init__(self,
        ...
        num_program_header_entries = len(self._elf_parsegen.phdrs)
        if hash_segment:
            extracted_hash_table_size = SecParseGenMbn(data=hash_segment,
                                                       ...
                                                       validating=validating).code_size
        else:
        ...
        self._extract_hash_segment_algorithm(num_program_header_entries, extracted_hash_table_size)
    ...
    def _extract_hash_segment_algorithm(self, num_program_header_entries, hash_table_size=0):
        if hash_table_size > 0:
            # Hash Entry size equals size of Hash Table divided by (number of hashed segments + 2)
            # + 1 for Hash Table Entry 0 (ELF + Program Header)
            # + 1 for Hash Table Entry 1 (Dummy Entry)
            hash_size = hash_table_size / (num_program_header_entries + 2)

            logger.debug("Number of segments hashed: " + str(num_program_header_entries))
            logger.debug("Hash segment size: " + str(hash_table_size))
            logger.debug("Hash segment entry size: " + str(hash_size))
