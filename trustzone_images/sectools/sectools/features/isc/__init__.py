...
class SecImageCore(object):
    ...
    def process(self,
        ...
        try:
            ...
            version = 'v' + crypto.openssl.OPENSSL_VERSION_MIN + ' or greater '
    ...
    def _process_secure_operation(self, image, progress, security_policy_list,
                                  i_integrity_check, i_sign, i_encrypt, i_decrypt, idx=0, prefix_override=None):
        from imageinfo import ImageInfo, StatusInfo
        # Check bounds
    ...
    def _create_parsegen_obj(self, image, validating=False, prefix_override=None):
        c_path.create_debug_dir(image.dest_image.debug_dir_parsegen)
        from sectools.features.isc.parsegen import get_parser
        parsegen = get_parser(image, validating, prefix_override)
