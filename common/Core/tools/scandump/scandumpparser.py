...
-------------------------------------------------------------------------------
  ...
  $Header: //components/rel/core.glue/1.2.1/tools/scandump/scandumpparser.py#2 $
...
def parseoptions():
    ...
    argparser.add_option('-c', '--bitfields', dest='bitfields', help='Bitfield count per slice', metavar='bitfield_count', default=16)
    argparser.add_option('-s', '--slices', dest='slices', help='Total number of slices (each of bitfield_count length)', metavar='slices', default=4)
    ...
    global mappingdata
    mappingdata = __import__(options.mapping)
...
def remap(parsed_registers, mapping):
    ...
    for sw_register, reg_decoding in mapping.iteritems():
        ...
        if re.match('val(.*?)', reg_decoding['val']):
            ...
            pointer_val = parsed_registers[reg_decoding['val']]
            for reg in reg_decoding['regs']:
                reg_name = reg
                try:
...
def xmlparse(inputfile, register_info):
    ...
    for register, signal in register_info.iteritems():
        ...
        for element in root.findall(".//bitfield-entry[register='%s']" % (register)):
            ...
            if (len(registers_val[register]) == 0):
                registers_val[register]= ['0'] * len(shift_array)
...
def gen_output(outputfile, registers):
    if outputfile is not None:
        ...
        if extension in ['.cmm']:
            ofptr.write('\n ON ERROR CONTINUE')
        for name in sorted(registers, key=registers.get):
            if extension in ['.cmm']:
                ofptr.write('\nREGISTER.SET ' + name + ' ' + registers[name] + '\n')
            else:
                ofptr.write('\n' + name + ' : ' + registers[name])
        ofptr.close()
    else:
