...
def CreateDictionary(targetconfigs):
    ...
    CMMBuilderDict.update( 
            {"Dump Bus Bandwidth Requests" :
                     {'dropdown menu name'  : "Dump Bus Bandwidth Requests",
                      'description'         : "Dump Bus Bandwidth Requests",
                      'scriptname'          : "ICBDump.cmm", #Make sure to include the extension here
                      'arguments'           : [],
                      'abspath'             : os.path.join(targetconfigs['RootDir'],"systemdrivers","icb"),
                      'submenu'             : 'ICB',
                      'user-interactive?'   : False,             #Mainly for cmmbuilder to know if this can be used without user prompt
                      'subsystems'          : ["mpss","adsp","slpi","wlan"],
                      #'prerequisites'       : ['attach',"Load HWIO Menus"],       #Prerequisites. attach=> proc will attach first. Also can put Tags from other scripts
                      'prerequisites'       : ["break"],       #Prerequisites. attach=> proc will attach first. Also can put Tags from other scripts
