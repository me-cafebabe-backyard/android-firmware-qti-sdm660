...
#  $Header: //components/rel/config.glue/94.0/storage/checksparse.py#2 $
#  $DateTime: 2020/03/10 23:51:57 $
#  $Author: pwbldsvc $

# when          who     what, where, why
# --------      ---     -------------------------------------------------------
# 2019-05-13    dng     Add fill chunk support.
...
Examples:
  ...
  -b, --image=IMAGE          optional; update only for image named IMAGE
  -v, --verbose              optional; verbose log when unsparsing image
  -e, --expand               optional; Expand the Sparse image to a disk image (same as simg2img)
...
def fillout_raw_chunk(fp_out, blocks, blk_sz, fill_value):
    length = (1L * blocks) * blk_sz
    chunk = None

    if length > COPY_BUF_SIZE:
        chunk = COPY_BUF_SIZE
        fillbuf_sz = COPY_BUF_SIZE / 4
    else:
        chunk = length
        fillbuf_sz = (length / 4) + 1

    fill_data = bytearray(fill_value * fillbuf_sz)
    ...
    while length!=0:
        if length > COPY_BUF_SIZE:
            ...
            chunk = length
        fp_out.write(fill_data[0:chunk])
        length -= chunk

def zeroout_raw_chunk(fp_out, blocks, blk_sz):
    fillout_raw_chunk(fp_out, blocks, blk_sz, b'\x00\x00\x00\x00')
...
def main():
    if len(sys.argv) < 2:
        ...
        sys.exit(-1)
    try:
        opts, args = getopt.getopt(sys.argv[1:], "i:o:?s:t:x:b:ve", ["input=", "outputpath=", "help", "search_path=", "location=", "xmlout=", "image=", "verbose", "expand"])
...
#    import pdb; pdb.set_trace()
    ...
    for xml_element in xml_iter:
        if xml_element.tag == 'program' and xml_element.attrib['filename'] != '' and 'sparse' in xml_element.attrib and xml_element.attrib['sparse'].lower() == 'true':
            ...
            if (not image_only is None) and xml_element.attrib['filename'] != image_only:
                print "\n\t  skip processing image due to explicit -b/--image flag\n"
                continue
            ...
            results = genfile(filename, int(xml_element.attrib['start_sector']), output_path, int(xml_element.attrib['SECTOR_SIZE_IN_BYTES']), verbose_log, always_zero_fill)
            if results == -1:
                #sys.exit(-5)
                continue
...
def genfile(filename, current_start_sector, output_path, sector_size_in_bytes, verbose, zero_fill):
    ...
    for i in range(sparse_header['total_chunks']):
        ...
        if chunk_header['chunk_type'] == CHUNK_TYPE_RAW:
            ...
            if verbose == 1:
                print "Handled RAW chunk %d chunk size %d" % (i, chunk_header['chunk_sz'])
            ...
            if (not out_fp is None) and not out_fp.closed:
                ...
                if (chunk_header['chunk_sz'] <= 2 and sparse_header['chunk_hdr_sz'] <= 4096) or zero_fill == 1:
                    results[-1]['num_partition_sectors'] = str(int(results[-1]['num_partition_sectors']) + file_size_sectors)
                ...
                # output is not open but requested to always zero-fill; create a new file
                print "\tcreating %s" % os.path.join(output_path, "%s_%d%s" % (file_name, chunk_number, file_ext))
                try:
                    out_fp = open(os.path.join(output_path, "%s_%d%s" % (file_name, chunk_number, file_ext)), "wb")
                except:
            ...
            if chunk_header['total_sz'] != sparse_header['chunk_hdr_sz']:
                ...
                return -1
            if verbose == 1:
                print "Handled DONTCARE chunk %d chunk size %d" % (i, chunk_header['chunk_sz'])
            current_start_sector += file_size_sectors
            total_blocks = total_blocks + chunk_header['chunk_sz']
        elif chunk_header['chunk_type'] == CHUNK_TYPE_FILL:
            temp = (1L * chunk_header['chunk_sz']) * sparse_header['blk_sz']
            if temp % sector_size_in_bytes == 0:
                file_size_sectors = temp/sector_size_in_bytes
            else:
                print "Fill chunk size %d is not a sector-multiple" % temp
                return -1
            temp = chunk_header['total_sz'] - CHUNK_HEADER_LEN
            if temp != FILL_VALUE_LEN:
                print "Fill value is not 4 bytes in length (reported %d)" % temp
                return -1
            if out_fp is None or out_fp.closed:
                print "\tcreating %s" % os.path.join(output_path, "%s_%d%s" % (file_name, chunk_number, file_ext))
                try:
                ...
                results[-1]['num_partition_sectors'] = str(int(results[-1]['num_partition_sectors']) + file_size_sectors)
            fill_value = in_fp.read(FILL_VALUE_LEN)
            fillout_raw_chunk(out_fp, chunk_header['chunk_sz'], sparse_header['blk_sz'], fill_value)
            current_start_sector += file_size_sectors
            total_blocks = total_blocks + chunk_header['chunk_sz']
            if verbose == 1:
                print "Handled FILL chunk %d chunk size %d" % (i, chunk_header['chunk_sz'])
        else:
            if (not out_fp is None) and not out_fp.closed:
                out_fp.close()
