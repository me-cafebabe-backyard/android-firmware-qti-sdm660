...
$Header: //components/rel/core.qdsp6/1.0.c2/services/utimers/src/utimer.c#1 $
...
static utimer_trace_type utimer_trace;
...
inline uint32 utimer_index_to_handle(uint32 timer_idx)
{
   uint32 handle;
...
inline boolean utimer_handle_to_index(uint32 handle, uint32 *timer_idx)
{
   uint16 temp, tid;
...
{                               
  ...
  if ((force == TRUE || ( utimers.defer_cnt == 0 && match_count != utimers.match_value)))
  {
    ...
    #ifndef FEATURE_UTIMER_DEFER_SUPPORT
       /* Doing notification here since only non-deferrable timers are currently supported in utimers. 
          If we reach here, it means there is a change in first non-deferrable timer
		  and we will signal registered clients about change. 
		  This is done to save space required for checking both deferrable, non-deferrable timers. */
       if(caller != UTIMER_MVS_DEFER_MATCH_INT && caller != UTIMER_MVS_UNDEFER_MATCH_INT)
          utimer_notify_first_non_defer_utimer_expiry_change();
    #else
       /* Knowingly removed ; at end to get compiler error if deferrable utimers are
       suported. In such case, need to check both deferrable, non-deferrable timers
	   when ever there is a change and signal then. Best place would be in
	   UTIMER_UPDATE_TIMER_INTERRUPT, but better check all functions and then
       do the changes */
       ERR_FATAL("non_defer_expiry_change notification should be handled differently if deferrable timers are allowed", 0, 0, 0)
    #endif
  }
...
utimer_timetick_type utimer_drv_get_first_non_deferrable_timer_expiry( void )
{
...
utimer_timetick_type utimer_drv_defer_match_interrupt_64( void )
{
  ...
  future_expiry = utimer_drv_get_first_non_deferrable_timer_expiry();
