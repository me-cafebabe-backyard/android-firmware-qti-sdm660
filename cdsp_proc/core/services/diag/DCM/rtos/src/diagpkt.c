...
$Header: //components/rel/core.qdsp6/1.0.c2/services/diag/DCM/rtos/src/diagpkt.c#1 $
  ...
  /* For multi-pd */
   ...
   if(DCM_table_Initialized  )
   {
     ...
     while ( q_item != NULL )
     {
       ...
       {
         local_count = 0;
         if ( q_item->master_tbl_entry[0].client_id > 0 ) //MPD ToDo: was client_id != NULL
  ...
  while( (msg_pkt_ptr != NULL) && (data_pkt_len > 0) )
  {
    ...
    case DIAG_CTRL_MSG_DIAGMODE:
    {
      ...
      case DIAG_CTRL_MSG_FEATURE:
      {
        ...
        ULogFront_RealTimePrintf(diag_err_log_handle, 1 , "diagpkt_process_ctrl_msg: Received APPS Feature mask = 0x%X", msg_pkt_ptr->data.ctrl_msg_feature.mask[0]);
    ...
    case DIAG_CTRL_MSG_BUF_SET_WM_PER:
      ...
      case DIAG_CTRL_MSG_DIAGID:
      {
        if( msg_pkt_ptr->data.ctrl_msg_diagID.version<= DIAG_CTRL_MSG_DIAGID_VER)
        {
          ...
          /* If diagID received is a valid ID, set signal to notify Diag that there's reg tables to be fwded */
          if(diag_get_diagID() != DIAGID_UNKNOWN_ID)
          {
            diagpkt_ready_to_fwd();
          }
        }
