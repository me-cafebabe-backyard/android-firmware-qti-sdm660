...
Copyright (c) 1990-2012,2016 by Qualcomm Technologies Incorporated.  All Rights Reserved.
*====*====*====*====*====*====*====*====*====*====*====*====*====*====*====*/
...
$Header: //components/rel/core.qdsp6/1.0.c2/services/utils/src/queue.c#1 $ $DateTime: 2020/02/10 01:57:30 $ $Author: pwbldsvc $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
05/25/16   nk      Fixed KW warings
...
static uint32 utils_global_queue_counter = 0;
...
void q_lock_init( q_type *q_ptr )
{
  int ret_value;
  ASSERT( q_ptr != NULL );
  q_ptr->mutex = (void*)malloc(sizeof(osal_crit_sect_t));
  if(q_ptr->mutex == NULL)
  {     
     ASSERT(q_ptr->mutex != NULL );
     return;
  }
  ret_value = osal_init_crit_sect(q_ptr->mutex);
  ASSERT( ret_value == OSAL_SUCCESS ); 
  utils_global_queue_counter++;
...
void q_lock_delete( q_type *q_ptr )
{
  int ret_value;
  ASSERT( q_ptr != NULL );
  ret_value = osal_delete_crit_sect(q_ptr->mutex);
  ASSERT( ret_value == OSAL_SUCCESS ); 
  free(q_ptr->mutex);
...
static void q_lock( q_type *q_ptr )
{
  ...
  ret_value = osal_enter_crit_sect((osal_crit_sect_t*)q_ptr->mutex);
  ASSERT( ret_value == OSAL_SUCCESS ); 
  return;
...
static void q_unlock( q_type *q_ptr )
{
  int ret_value;
  ASSERT( q_ptr != NULL );
  ret_value = osal_exit_crit_sect((osal_crit_sect_t*)q_ptr->mutex);
  ASSERT( ret_value == OSAL_SUCCESS ); 
  return;
} /* END q_unlock */
...
#endif /* FEATURE_QUEUE_USES_MUTEX */
   ...
   */
   ...
   while( item_ptr != NULL )
   {
      if( compare_func( item_ptr, compare_val ) != 0 )
      {
	    ...
	    /* If the Tail points to Deleted Node, make Tail points to previous
            */
            if (q_ptr->link.prev_ptr == (q_link_type *) item_ptr)
            {
               q_ptr->link.prev_ptr = (q_link_type *)prev_ptr;
	    }
         }
         else
         {
      ...
      if( compare_func( item_ptr, NULL ) != 0 )
      {
         ...
         */
         if( prev_ptr != NULL )
         {
            /* Remove from the middle or tail
            */
            /* Make Previous Node->Next point to Deleted Node ->Next
            */
            prev_ptr->link.next_ptr = item_ptr->link.next_ptr;

            /* Deleted Node->Next point to NULL */
            item_ptr->link.next_ptr = NULL;
