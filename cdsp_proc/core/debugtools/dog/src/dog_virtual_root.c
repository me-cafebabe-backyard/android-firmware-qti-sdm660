...
#define VIR_DOG_GRACE_TIMER_PERIOD_SEC      40
...
static void dog_virtual_start_grace_timer(int client_handle, void *data_ptr)
{
   ...
   {
      if (-1 < qurt_qdi_handle_invoke(client_handle, QDI_OS_SYSENV, QDI_HANDLE_LOCAL_CLIENT, QURT_SYSENV_PROCNAME_TYPE, &pn))
      {
         ...
         if(NULL != dog_virtual_grace_timer_curr)
         {
            ...
            if(TE_SUCCESS != (t_err = timer_set_64(&dog_virtual_grace_timer_curr->dog_pd_grace_timer, 
                                         VIR_DOG_GRACE_TIMER_PERIOD_SEC, 
                                         0, T_SEC )))
            {
            ...
            TMS_MSG_ERROR_2( "DOG_VIR : Started grace timer with asid = %u, name '%s'\n", pn.asid, pn.name);
         }
         else
         {
            TMS_MSG_ERROR( "DOG_VIR : Malloc failed");
         }
      }
      else
      {
         TMS_MSG_HIGH( "Cannot obtain qdi client info");
      }

   }
   else
   {
      TMS_MSG_HIGH_2( "DOG_VIR : User PD not started by TMS owned pd-mon with asid = %u, name '%s'\n", pn.asid, pn.name);
   }
   ...
   while(NULL != dog_vir_grace_curr)
   {
      if(asid == dog_vir_grace_curr->pn.asid)
      {
         if (dog_vir_grace_curr->dog_pd_grace_timer_set == TRUE)
         {
            ...
            TMS_MSG_HIGH_2( "DOG_VIR : Stopped and undef the grace timer with asid = %u, name '%s'\n", dog_vir_grace_curr->pn.asid, dog_vir_grace_curr->pn.name);
         }
...
static int dog_qdi_open(int client_handle, char* name, uint32_t name_len)
{
   ...
   if(-1 != ret_handle)
   {
      ...
      if(-1 != ret)
      {
         TMS_MSG_HIGH( "DOG_QDI: User PD virtual dog recorded with Root PD \n");
      }
      else
      {
         ERR_FATAL("DOG_QDI: qurt_qdi_copy_from_user() failed ", ret, 0, 0);
      }
   }
   else
   {
      ERR_FATAL("DOG_QDI: qurt_qdi_handle_create_from_obj_t() failed ", ret_handle, 0, 0);
   }
...
static int dog_qdi_close(dog_qdi_device_p clientobj)
{
   ...
   if(dog_virtual_disable != TRUE)
   {
      ...
      TMS_MSG_LOW( "DOG_QDI: User PD virtual dog de-registerd with Root PD hb dog \n");
   }
...
static int dog_qdi_hb_register(dog_qdi_device_p clientobj)
{
   ...
   if(dog_virtual_disable != TRUE)
   {
      ...
      {
         ERR_FATAL( "DOG_QDI: dog_hb_register_qurt failed", dataobj->dog_vir_hb_handle, 0, 0);
      }

      TMS_MSG_LOW( "DOG_QDI: User PD virtual dog registerd with Root PD hb dog \n");
   }
...
static int dog_qdi_worker_wait(int client_handle, dog_qdi_device_p clientobj, uint32_t * remote_mask)
{
   ...
   if(-1 == ret)
   {
      TMS_MSG_ERROR( "DOG_QDI: in dog_qdi_worker_wait() qurt_qdi_copy_to_user() failed \n");
   }
...
static int dog_qdi_pong(dog_qdi_device_p clientobj)
{
   ...
   dataobj = &(clientobj->dog_user_pd_data);
      ...
      TMS_MSG_LOW( "DOG_QDI: User PD virtual dog pinged Root PD \n");
   }
                      ...
                      )
{
   ...
   switch (dog_qdi_method)
   {
      ...
      case QDI_OPEN:
      {
         ...
         TMS_MSG_HIGH_2( "DOG_QDI: device open %s == %d", task_name, ret);
         break;
      }
      ...
      case QDI_CLOSE:
      {
         ...
         {
            TMS_MSG_ERROR( "DOG_QDI: dog_qdi_close() failed");
         }

         ret = qurt_qdi_method_default(client_handle, obj, dog_qdi_method, a1, a2, a3, a4, a5, a6, a7, a8, a9);
         TMS_MSG_HIGH_2( "DOG_QDI: device close %s == %d", task_name, ret);
         break;
      }
...
void dog_qdi_init(void)
{
   ...
   if(ret == 0)
   {
      TMS_MSG_HIGH( "DOG_QDI: device = /dev/dog registered successfully");
   }
   else
   {
      ERR_FATAL( "DOG_QDI: qurt_qdi_devname_register failed", ret, 0, 0);
   }
   ...
   if(dog_disable_pd_grace_timer != TRUE)
   {
      ...
      {
         ERR_FATAL("Vir dog Could not register onspawn callback with QURT",0,0,0);
      }
   }
