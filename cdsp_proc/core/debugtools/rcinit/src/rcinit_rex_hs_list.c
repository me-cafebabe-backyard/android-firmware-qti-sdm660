...
/*=============================================================================
Edit History
$Header: //components/rel/core.qdsp6/1.0.c2/debugtools/rcinit/src/rcinit_rex_hs_list.c#1 $
   ...
   if (RCINIT_GROUP_7 >= group)
   {
      ...
      else
      {
         ...
         if (cnt < RCINIT_TASK_MAX)                                              // update empty
         {
            ...
            array[cnt].tid = (rcinit_tid_p)rex_self();
            array[cnt].hs = 0;
         }
         else
         {
...
unsigned long rcinit_internal_hs_list_hs(rcinit_internal_hs_p array, RCINIT_GROUP group, RCINIT_NAME task_name)
{
   ...
   if (RCINIT_GROUP_7 >= group)
   {
      for (cnt = 0; cnt < RCINIT_TASK_MAX; cnt++)                                // find existing
      {
         if ((rcinit_tid_p)rex_self() == array[cnt].tid)                         // check matching tcb pointer
         {
