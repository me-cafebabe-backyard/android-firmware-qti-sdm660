...
$Header: //components/rel/core.qdsp6/1.0.c2/debugtools/eic/src/err_inject_crash.c#1 $
...
err_inject_crash_diag (void * req_ptr, uint16 pkt_len)
{
  ...
  if (crash_pending)
  {
    TMS_MSG_HIGH("err_inject_crash: crash pending, ignoring subsequent request");
...
)
{
  ...
  if (req_crash >= SIZEOF_ERR_INJECT_CRASH_TYPE) 
  {
    ...
    TMS_MSG_ERROR_1("err_inject_crash: invalid crash ID %#x", req_crash);
    return;
  }
...
void err_inject_crash_execute(timer_cb_data_type data)
{
  ...
  if ( temp_crash_info->image_mode == ERR_MAINIMAGE_MODE )
  {
    ...
    default:
      TMS_MSG_ERROR_1("err_inject_crash: unhandled crash ID %#x", temp_crash_info->crash);
    }
  }
  else
  {
...
static void err_inject_crash_devcfg_check(void)
{
  ...
  if (DAL_SUCCESS == DALSYS_GetPropertyValue(pHandle,"eic_crash_delay", 0, &propValue))
  {
    ...
    {
      TMS_MSG_LOW_1("Small eic_crash_delay dalconfig, using %d", ERR_INJECT_CRASH_DELAY_DEVCFG_MIN);
