...
$Header: //components/rel/core.qdsp6/1.0.c2/debugtools/err/src/err_exception_handler.c#1 $
...
void err_exception_handler(void)
{
  ...
  for (;;)
  {
    ...
    if (PROCINFO_QDI_FAILURE == procinfo_get_name_from_pid(PROCINFO_OEM_NAME_TYPE, pid, pname, sizeof(pname)))
    {
         /* If PID not found: pname will revert to qurt provided name */
         procinfo_get_name_from_pid(PROCINFO_INTERNAL_NAME_TYPE, pid, pname, sizeof(pname));
    }

    /* Executes Pre STM Callback */
    err_execute_external_cb_pre_STM(tid);
    err_invoke_user_process_cbs(ERRCB_BUCKET_PRE_STM);
    ...
    if ( !err_has_err_fatal_occured(sys_err.badva, sys_err.cause) )
    {
      ...
      err_fatal_jettison_core ( pid, NULL, exception_msg, 0, 0, 0);
    }
    else 
    {
      /* Update tid and is_exception to coredump */
