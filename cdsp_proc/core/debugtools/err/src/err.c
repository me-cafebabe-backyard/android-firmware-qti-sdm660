...
$Header: //components/rel/core.qdsp6/1.0.c2/debugtools/err/src/err.c#1 $
...
void err_execute_external_cb_post_STM_helper(void)
{
  ...
  err_execute_external_cb(ERRCB_BUCKET_POST_STM, 0);
}
...
void err_execute_external_cb_normal_helper(void)
{
  ...
  err_execute_external_cb(ERRCB_BUCKET_NORMAL, 0);
}
...
void err_execute_external_cb_late_helper(void)
{
  ...
  err_execute_external_cb(ERRCB_BUCKET_LATE, 0);
}
...
void err_execute_external_cb_last_helper(void)
{
  ...
  err_execute_external_cb(ERRCB_BUCKET_LAST, 0);
}
...
void err_fatal_handler ( void )
{
  ...
  while(err_preflush_internal[fptr_index] != NULL)
  {
    ...
    ERR_SET_AND_FLUSH_PTR(coredump.err.err_cb_current, err_preflush_internal[fptr_index]); 
