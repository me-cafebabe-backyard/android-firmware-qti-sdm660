...
uint32_t servreg_service_count = 1;
char *servreg_local_domain = "msm/modem/wlan_pd";
char *servreg_soc_name = "msm";
char *servreg_domain_name = "modem";
char *servreg_subdomain_name = "wlan_pd";
uint32_t servreg_qmi_instance_id = 180 ;
...
SERVREG_NAME servreg_create_rcesn_name(SERVREG_NAME name)
{
   ...
   if(SERVREG_NULL != name)
   {
      ...
      rcesn_name = (SERVREG_NAME)servreg_malloc(len * sizeof(char));
      ...
      {
         TMS_MSG_ERROR("servreg_create_rcesn_name() malloc failed");
      }
   }
   else
   {
      TMS_MSG_ERROR("in servreg_create_rcesn_name() name is NULL ");
   }
...
SERVREG_NAME servreg_create_sync_name(SERVREG_NAME name, SERVREG_SERVICE_STATE state)
{
   ...
   if(SERVREG_NULL != name)
   {
      ...
      sync_name = (SERVREG_NAME)servreg_malloc(len * sizeof(char));
      ...
      if(SERVREG_NULL != sync_name)
      {
         ...
         else
         {
            ...
            TMS_MSG_ERROR("servreg_create_sync_name() invalid state");
         }
      }
      else
      {
         TMS_MSG_ERROR("servreg_create_sync_name() malloc failed");
      }
   }
   else
   {
      TMS_MSG_ERROR("in servreg_create_sync_name() name is NULL ");
   }
...
SERVREG_NAME servreg_create_ssr_name(SERVREG_NAME domain, SERVREG_SERVICE_STATE state)
{
   ...
   if(NULL != sub_name)
   {
      ...
      if(SERVREG_NULL != sub_name2)
      {
         ...
         if(SERVREG_SERVICE_STATE_DOWN == state)
         {
            ...
            {
               TMS_MSG_ERROR("servreg_create_ssr_name() malloc failed");
            }
         }
   ...
   {
      TMS_MSG_ERROR("servreg_create_ssr_name() strstr failed");
   }
   return ssr_name;
...
SERVREG_NAME servreg_concat(SERVREG_NAME domain, SERVREG_NAME service)
{
   ...
   if(SERVREG_NULL != domain)
   {
      if(SERVREG_NULL != service)
      {
         ...
         {
            TMS_MSG_ERROR("servreg_concat() malloc failed");
         }
      }
      else
      {
   ...
   {
      TMS_MSG_ERROR("servreg_concat() domain is NULL ");
   }
