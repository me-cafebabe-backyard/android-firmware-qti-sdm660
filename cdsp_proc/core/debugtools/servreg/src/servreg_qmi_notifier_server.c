...
static sr_qmi_s_proxy_p sr_qmi_s_proxy_pool_alloc(void)
{
   ...
   if ( SERVREG_NULL != ret ) {
     ...
     ret->curr_state = SERVREG_SERVICE_STATE_UNINIT;
      ...
      ERR_FATAL( "sr_qmi_s_proxy_p malloc failed", 0, 0, 0);
   }
   return ret;
}
...
static sr_qmi_s_proxy_p sr_qmi_s_listener_attach(SERVREG_MON_HANDLE sr_mon_handle, qmi_client_handle client_handle)
{
   ...
   if(SERVREG_NULL == sr_qmi_s_proxy)
   {
      ...
      if(SERVREG_NULL != sr_qmi_s_proxy)
      {
         ...
         {
            ERR_FATAL( "sr_qmi_s_client malloc failed", 0, 0, 0);
         }
      ...
      {
         ERR_FATAL( "sr_qmi_s_proxy malloc failed", 0, 0, 0);
      }
   }
   else
   {
...
static SERVREG_RESULT sr_qmi_s_listener_detach(sr_qmi_s_proxy_p sr_qmi_s_proxy, qmi_client_handle client_handle_del)
{
   ...
   {
      TMS_MSG_ERROR("delete: sr_qmi_s_proxy is NULL \n");
      ret = SERVREG_INVALID_HANDLE;
   }
...
static SERVREG_BOOL servreg_qmi_validate_connection_handle(void *connection_handle)
{
   ...
   if(connection_handle != NULL)
   {
      while(SERVREG_NULL != client)
      {
         ...
         {
            ret = SERVREG_TRUE;
            TMS_MSG_HIGH("in servreg_qmi_validate_connection_handle() connection_handle MATCHES \n");
            break;
         }
         client = client->next;
         TMS_MSG_ERROR("in servreg_qmi_validate_connection_handle() connection_handle does not MATCH \n");
      }
   }
   else
   {
      TMS_MSG_ERROR("in servreg_qmi_validate_connection_handle() connection_handle is NULL \n");
   }
...
int servreg_get_asid_from_proc_spc(char* proc_spc, int *asid)
{
    ...
    int len = servreg_nmelen(proc_spc);
    int i;

    TMS_MSG_SPRINTF_HIGH_2("process specifier = %s, len = %d", proc_spc, len);

    for (i=0; i< PD_MON_MAX_PD; i++)
    {
        if (pd_mon_internal_get_asid(i) != -1)
        {
            TMS_MSG_SPRINTF_HIGH_1("process specifier = %s", ((servreg_mon_node_p)pd_mon_internal_get_servreg_h(i))->service_name);

            if (0 == servreg_nmecmp(((servreg_mon_node_p)pd_mon_internal_get_servreg_h(i))->service_name, proc_spc, len))
            {
               *asid = pd_mon_internal_get_asid(i);
               
               TMS_MSG_HIGH_1("servreg_get_asid_from_proc_spc: asid = %d", *asid);
      ...
      if(sr_qmi_s_proxy->curr_state != new_curr_state)
      {
         ...
         if(SERVREG_NULL != service_name)
         {
            ...
            while(SERVREG_NULL != sr_qmi_s_client)
            {
               ...
               TMS_MSG_HIGH_1("in servreg_send_qmi_ind_msg_state_change() qmi_csi_send_ind err= %d" ,err);
                                          ...
                                          void           *service_cookie)
{
   ...
   if(NULL != req_handle)
   {
      if(SERVREG_TRUE == servreg_qmi_validate_connection_handle(connection_handle) && SERVREG_TRUE == client->connected)
      {
         ...
         switch(msg_id)
         {
                  ...
                  if(SERVREG_TRUE == request->enable)
                  {
                     ...
                     if(SERVREG_SUCCESS == ret)
                     {
                           ...
                           if(SERVREG_NULL != sr_qmi_s_proxy)
                        {
                           ...
                           resp_send.resp.result = QMI_RESULT_SUCCESS_V01;
                           resp_send.resp.error  = QMI_ERR_NONE_V01;
                           TMS_MSG_HIGH_1("servreg_notif_server_process_req() REGISTER_LISTENER_REQ success, state = %d \n", resp_send.curr_state);
                        }
                        else
                        {
                           ERR_FATAL( "servreg_create_qmi_notif_server_node() failed", 0, 0, 0);
                        }
                     }
                        else //failed to register with servreg framework
                     {
                        ...
                        TMS_MSG_ERROR("in servreg_notif_server_process_req() REGISTER_LISTENER_REQ fail \n");

                        ret = servreg_free_monitor_handle(sr_mon_handle);

                        if(SERVREG_SUCCESS != ret)
                        {
                           TMS_MSG_ERROR("in servreg_notif_server_process_req() could not free the monitor handle \n");
                        }
            ...
            case SERVREG_NOTIF_QUERY_STATE_REQ:
            {
               ...
               {
                  TMS_MSG_HIGH("in servreg_notif_server_process_req() STATE_REQ get success\n");
                  resp_send.curr_state_valid = 1;
                  resp_send.curr_state = (qmi_servreg_notif_service_state_enum_type_v01)servreg_get_service_curr_state(sr_mon_handle);
                  resp_send.resp.result = QMI_RESULT_SUCCESS_V01;
                  resp_send.resp.error  = QMI_ERR_NONE_V01;
            ...
            case SERVREG_NOTIF_STATE_UPDATED_IND_ACK_REQ:
            {
               ...
               {
                  SERVREG_TRACE_0(QMI_ACK_RESP_SUCCESS);
                  resp_send.resp.result = QMI_RESULT_SUCCESS_V01;
                  resp_send.resp.error  = QMI_ERR_NONE_V01;
               }
               else
               {
                  SERVREG_TRACE_0(QMI_ACK_RESP_FAIL);
                  resp_send.resp.result = QMI_RESULT_FAILURE_V01;
                  resp_send.resp.error  = QMI_ERR_INVALID_HANDLE_V01;
               }
               //send response
            ...
            default:
            {
               ...
               TMS_MSG_ERROR_1("in servreg_notif_server_process_req() Unhandled QMI request id: 0x%x", msg_id);
      ...
      {
         TMS_MSG_ERROR("in servreg_notif_server_process_req() connection_handle is invalid");
         retval = QMI_CSI_CB_INTERNAL_ERR;
      }
  }
  else
  {
      TMS_MSG_ERROR("in servreg_notif_server_process_req() req_handle is NULL");
      retval = QMI_CSI_CB_INTERNAL_ERR;
  }
...
void servreg_notif_server_disconnect(void *connection_handle, void *service_cookie)
{
   ...
   if(SERVREG_NULL != connection_handle)
   {
      while(SERVREG_NULL != client_curr)
      {
         if(client_curr == (sr_client_type_p)connection_handle && (SERVREG_FALSE != client_curr->connected))
         {
            ...
            TMS_MSG_HIGH("in servreg_notif_server_disconnect() client released connection");
      ...
      while(SERVREG_NULL != sr_qmi_s_proxy)
      {
        ...
        while(SERVREG_NULL != sr_qmi_s_client)
        {
          if(client_h == sr_qmi_s_client->client_handle)
          {
            ...
            {
              ERR_FATAL("SERVREG_QMI: QMI client disconnected with outstanding ack, 0x%x, 0x%x ",client_h,sr_qmi_s_proxy,0);
            }
   ...
   {
     TMS_MSG_ERROR("in servreg_notif_server_disconnect() connection handle is NULL");
   }
...
qmi_csi_cb_error servreg_notif_server_connect(qmi_client_handle client_handle, void *service_cookie, void **connection_handle)
{
   ...
   client = (sr_client_type_p)servreg_malloc(sizeof(sr_client_type_t));
   ...
   {
      TMS_MSG_ERROR("in servreg_notif_server_connect() malloc failed and connection failed with server");
   }
...
qmi_csi_service_handle servreg_qmi_server_init(qmi_csi_os_params *os_params)
{
   ...
   {
      TMS_MSG_ERROR("in servreg_qmi_server_init() QMI notif server init failed");
   }
   else
   {
      TMS_MSG_HIGH("in servreg_qmi_server_init() QMI notif server init success");
   }
...
static SERVREG_RESULT servreg_qmi_handle_false_ack_req(sr_qmi_s_proxy_p sr_qmi_s_proxy, int32_t transaction_id)
{
   ...
   if(SERVREG_NULL == sr_qmi_s_proxy)
   {
     TMS_MSG_ERROR("in servreg_qmi_handle_false_ack_req() sr_qmi_s_proxy is null");
     return SERVREG_FAILURE;
   }
   ...
   {
     TMS_MSG_ERROR("in servreg_qmi_handle_false_ack_req() sr_qmi_s_client_last is null");
     return SERVREG_FAILURE;
   }
                                                   ...
                                                   qmi_client_handle client_h)
{
   ...
   if(SERVREG_NULL != sr_qmi_s_proxy)
   {
      ...
      if (sr_qmi_s_client->last_ack_id < sr_qmi_s_client->transaction_id)
      {
         ...
         TMS_MSG_ERROR( "SERVREG_QMI: dereg with outstanding ack");
      ...
      while(SERVREG_NULL != curr_client_p)
      {
         if(client_h == curr_client_p->client_handle)
         {
            ...
            if (curr_client_p->last_ack_id == transaction_id)
            {
               ...
               TMS_MSG_ERROR( "duplicate qmi_servreg ack rx'd");
               servreg_mutex_unlock_dal(&servreg_qmi_server_internal.mutex);
               return SERVREG_FAILURE;
            }
            else
