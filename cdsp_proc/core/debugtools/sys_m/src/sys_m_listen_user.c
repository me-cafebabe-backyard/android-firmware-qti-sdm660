...
/*=============================================================================
Edit History
$Header: //components/rel/core.qdsp6/1.0.c2/debugtools/sys_m/src/sys_m_listen_user.c#1 $
...
static void SYS_M_LISTEN_WORKER(void *argv __attribute__((unused)))
{
   ...
   if (0 <= (handle = sys_m_listen_init("USER")))
   {
      do
      {
         ...
         {
            TMS_MSG_SPRINTF_HIGH_1("sys_m_listen_read evt =  %s", out);
...
DLL_API_GLOBAL void sys_m_listen_user_init(void)
{
