...
/*=============================================================================
Edit History
$Header: //components/rel/core.qdsp6/1.0.c2/debugtools/rcevt/src/rcecb.c#1 $
...
int rcecb_getregistrants_handle(RCECB_HANDLE const handle)
{
   ...
   if (RCE_NULL != nde_p)
   {
      ...
      while (RCE_NULL != nfy_p)                                                  // loop for check
      {
         if (RCECB_SIGEX_TYPE_CALLBACK_P0 <= (RCECB_SIGEX_TYPE)nfy_p->sigex_type && RCECB_SIGEX_TYPE_CALLBACK_P2 >= (RCECB_SIGEX_TYPE)nfy_p->sigex_type)
         {
            rc++;                                                                // return result
         }
...
{
   ERR_FATAL("RCECB client callback runtime may not exceed %d", RCECB_CALLBACK_MAX_TIME, 0, 0);
...
rce_nde_p rcecb_signal_common(rce_nde_p nde_p)
{
   ...
   while (RCE_NULL != nfy_p)                                                     // iterate
   {
      ...
      if (RCECB_SIGEX_TYPE_CALLBACK_P0 == (RCECB_SIGEX_TYPE)nfy_p->sigex_type)
      {
         ...
         timer_set_64(&nde_p->expiry, RCECB_CALLBACK_MAX_TIME, RCECB_CALLBACK_MAX_TIME, T_MSEC);
