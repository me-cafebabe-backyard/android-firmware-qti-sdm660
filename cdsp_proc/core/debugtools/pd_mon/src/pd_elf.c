...
/*=============================================================================
Edit History
$Header: //components/rel/core.qdsp6/1.0.c2/debugtools/pd_mon/src/pd_elf.c#1 $
...
PD_ELF_STATUS pd_elf_prgtab_alloc_list(Elf32_Ehdr* header_p, pd_elf_prgtab_p* prgtab_p, uint32 list_sz, void* head_ptr, uint32 data_sz, void* virt_addr, uint32 virt_sz)
{
   ...
   if (virt_sz < list_sz)
   {
      TMS_MSG_ERROR_2( "prgtab_alloc_virtual insufficient vmem 0x%x/0x%x", list_sz, virt_sz);
   }
   else
   {
      ...
      if (NULL != pt_p)
      {
         TMS_MSG_ERROR_2("prgtab_alloc_virtual malloc success 0x%x 0x%x", virt_addr, list_sz);
      ...
      {
         TMS_MSG_ERROR_2( "prgtab_alloc_virtual malloc failure 0x%x 0x%x", virt_addr, list_sz);
      }
   }
...
PD_ELF_STATUS pd_elf_write(int fd, Elf32_Ehdr* header_p, pd_elf_prgtab_p* prgtab_p, pd_elf_sectab_p* sectab_p, void ** prgtab_phdr)
{
   ...
   if (PD_ELF_STATUS_SUCCESS != pd_elf_header_write(fd, header_p))
   {
      ...
      TMS_MSG_ERROR( "device dump write header error");
      return rc;
   }
   ...
   if (PD_ELF_STATUS_SUCCESS != pd_elf_prgtab_write(fd, header_p, prgtab_p, prgtab_phdr))
   {
      ...
      TMS_MSG_ERROR( "device dump write prgtab error");
      return rc;
   }
   ...
   if (PD_ELF_STATUS_SUCCESS != pd_elf_sectab_write(fd, header_p, sectab_p))
   {
      ...
      TMS_MSG_ERROR( "device dump write sectab error");
      return rc;
   }
