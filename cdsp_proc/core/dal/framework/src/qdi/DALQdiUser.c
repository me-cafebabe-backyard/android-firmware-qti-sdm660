...
$Header: //components/rel/core.qdsp6/1.0.c2/dal/framework/src/qdi/DALQdiUser.c#1 $
...
_DoRemoteCreateProxy(DalDeviceHandle *hdalDevice)
{
    ...
    pid_str = servreg_get_local_domain(); //get PD/Domain name string
    if(pid_str != NULL && hdalDevice != NULL)
    {
      return qurt_qdi_handle_invoke(gRemoteDALInitHandle, 
                                    DALREMOTE_MSG_CREATE_PROXY, 
                                    pid_str, 
                                    strlen(pid_str), 
                                    ((DALDeviceFWDCtx *) hdalDevice)-> remoteQDIhandle);
