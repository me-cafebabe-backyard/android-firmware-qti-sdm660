...
$Header: //components/rel/core.qdsp6/1.0.c2/dal/framework/src/qdi/DALQdiQurtOS.c#1 $
...
/*
 * This function adds a proxy reference to the object referenced by user process.
 */
static int
DALRemote_CreateProxy(int client_handle, const char * pPidStr, int nPidLen , int qdiHandle){


    if(qurt_client_proxy_ref_add(qdiHandle) == QURT_EOK) return DAL_SUCCESS;
    else return DAL_ERROR;
}


/*
 * This function removes a proxy reference to the object referenced by user process.
 */
static int
DALRemote_ReleaseProxy(int client_handle, int qdiHandle){
    
    if(qurt_client_proxy_ref_remove(qdiHandle) == QURT_EOK) return DAL_SUCCESS;
    else return DAL_ERROR;
                         ...
                         qurt_qdi_arg_t a9 )
{
    ...
    case DALREMOTE_MSG_CREATE_PROXY:
        return DALRemote_CreateProxy(client_handle, a1.ptr, a2.num, a3.num);
        
    case DALREMOTE_MSG_REMOVE_PROXY:
        return DALRemote_ReleaseProxy(client_handle, a1.num);
