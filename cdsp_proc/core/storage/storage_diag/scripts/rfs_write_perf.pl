...
############################ rfs_put-cmd-q #################################
sub storage_rfs_write_perf_cmd_q
{
  my $RequestString;
  my $variant;
  my @Rsp;
  ...
  printf "\tSending rfs-write-perf-cmd-q Request: %s, %d, %d\n", $file_name, $offset, $write_size;
  my $Response = $port_selected->SendCommand($variant, 20000);
  printf "\tReceived response for rfs-write-perf-cmd-q!\n";
  @Rsp = unpack "CCSIIIIII", $Response;

  if ($Rsp[3] != 0)
  {
    printf "rfs-write-perf-cmd-q FAILED : %d\n", $Rsp[3];
    $result = $Rsp[3];
    die;
  }
  else
...
sub storage_rfs_write_perf_cmd_q_get_results
{
  my $RequestString;
  my $variant;
  my @Rsp;
...
sub rfs_write_perf_test
{
  ...
  {
    if ($remaining_bytes < $write_size)
    {
      $write_size = $remaining_bytes;
    }

    $cmd_hdl = storage_rfs_write_perf_cmd_q($file_name, $written_bytes, $write_size, $seed++);
    sleep (1);
    $state = storage_rfs_write_perf_cmd_q_get_results($cmd_hdl);
    $state == 2 or die "\n\nInvalid state : \n\n", $state;
...
sub rfs_write_perf
{
  ...
  my $write_size = (16*1024);

  printf ($out_fd "%05.3f,",($file_size/1024.0 /1024));
  $key = "$file_size";
  rfs_write_perf_test ($file_size, $write_size);
  sleep (2);
...
sub rfs_write_perf_setup_output_file
{
  my $time_str = strftime('%Y-%m-%d-%H-%M-%S',localtime);
  print $time_str;

  my $out_dir = 'results/'.$time_str;
  my $out_file= $out_dir."/rfs_write_partial_stats_".$time_str.".csv";
  my $out_file_detail = $out_dir."/rfs_write_partial_stat_details".$time_str.".csv";
...
#################################### Main Script ##############################
get_phone($gui);
rfs_write_perf_setup_output_file ();
rfs_write_perf ();
rfs_write_perf_cleanup ();
