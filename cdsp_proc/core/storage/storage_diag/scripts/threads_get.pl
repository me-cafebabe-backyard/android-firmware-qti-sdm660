...
sub storage_rfs_get_cmd_q
{
  ...
  my $file_size = $_[2];
  my $seed_val = $_[3];
  my $verify_after_each_write = $_[4];
  my $file_name = $_[5];

  my $reserved = 0;
  my $sub_cmd_id = 4; # rfs_get cmd
  my $sub_cmd_req_pkt_len = 4;
  my $cmd_q_hdl = 0;
  

  $RequestString = pack ("CCSIIIIIIIIa256",
                         ...
                         $file_size, #I
                         $seed_val, #I
                         $verify_after_each_write, #I
                         $file_name #a256
                         );
                         
  $variant = Variant(VT_ARRAY | VT_UI1, 288);
  $variant->Put($RequestString);

  printf "\tSending rfs-get-cmd-q Request: %s : %d\n", $file_name, $file_size;
  my $Response = $port_selected->SendCommand($variant, 20000);
  printf "\tReceived response for rfs_get-cmd-q!\n";
  @Rsp = unpack "CCSIIIIII", $Response;

  if ($Rsp[3] != 0)
  {
    printf "rfs_get-cmd-q FAILED : %d\n", $Rsp[3];
    $result = $Rsp[3];
    die;
  }
  else
...
sub storage_rfs_get_cmd_q_get_results
{
  my $RequestString;
  my $variant;
  my @Rsp;
...
sub rfs_get_cmd_q_run_later_in_threads
{
  my $file_size = $_[0];
  my $num_threads = $_[1];
  ...
  for (my $i = 1; $i <= $loop_count; $i++)
  {
    ...
    #Q as many get requests as threads requested   
    for ($j = 1; $j <= $num_threads; $j++)
    {
        printf "Queueing thread $j\'s req\n";
        
        $cmd_hdl[$j] = storage_rfs_get_cmd_q(1, 1, $file_size,
                                             $j, 0, $file_name);
        $state = storage_rfs_get_cmd_q_get_results($cmd_hdl[$j]);
        $state == 0 or die "\n\nInvalid state : \n\n", $state;        
    }
...
#    printf ("File size %d\n", $i);
    ...
    {
      $key = "$j"."_"."$file_size";
      rfs_get_cmd_q_run_later_in_threads ($file_size, $j);
      sleep (2);
    }
    printf($out_fd "\n");
  }
...
sub rfs_get_setup_output_file
{
  my $time_str = strftime('%Y-%m-%d-%H-%M-%S',localtime);
  print $time_str;
  
  my $out_dir = 'results/'.$time_str;
  my $out_file= $out_dir."/tftp_get_stats_".$time_str.".csv";
  my $out_file_detail = $out_dir."/tftp_get_stat_details".$time_str.".csv";
...
#################################### Main Script ##############################
get_phone($gui);
rfs_get_setup_output_file ();
rfs_get_cmd_q_run_threads ();
rfs_get_cleanup ();