...
/*===========================================================================
  ...
  $Header: //components/rel/core.qdsp6/1.0.c2/storage/tftp/os/src/tftp_os_modem.c#1 $ $DateTime: 2020/02/10 01:57:30 $ $Author: pwbldsvc $
...
volatile int tftp_assert_run_infinite_loop = 0;
  ...
  tftp_os_file_open_helper(const char *path, int32 oflags, uint16 mode_bits,
                           int32 group, tftp_file_handle *handle,
                           const char *stream_mode)
  {
    int32 open_res, fd;

    open_res = tftp_os_open_file (path, oflags, mode_bits, group);
    if (open_res < 0)
    {
      return open_res;
    }
