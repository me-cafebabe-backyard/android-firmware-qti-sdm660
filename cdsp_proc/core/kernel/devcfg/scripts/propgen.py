...
def GenerateArray(DriverName, node, GlobalDef, GlobalOffset, remove_dup_names, memory_optimised_props):
   ...
   if node.getAttribute("id")[:2] =="0x":
      ...
      #It is a String based Device ID
      
      #Is it an empty string name ?
      if len(node.getAttribute("id"))==0:
         print "ERROR *** Empty Device Name"
         raise ValueError("Empty Device Name in Driver : {}".format(DriverName))
      ...
      if StringDeviceNames.has_key(node.getAttribute("id")):
         if remove_dup_names is False:
            ...
            print "ERROR *** Duplicate Device Name : {}".format(node.getAttribute("id"))
      ...
      for L in node.getAttribute("id"):
          if L not in Supported_char:
              print "ERROR *** Device Name {} has NON-Supported Character : {}".format(node.getAttribute("id"),L)
      ...
      # DALPROP_ATTR_TYPE_BYTE_SEQ
         ...
         for index in range(len(value) - 1):
            ...
            try:
                ...
                print "ERROR *** Invalid (or empty sequence) value for property type DALPROP_ATTR_TYPE_BYTE_SEQ in Device: {} for Driver {}".format(node.getAttribute("id"),DriverName)
                raise ValueError("Invalid property value for type DALPROP_ATTR_TYPE_BYTE_SEQ")         #Insert length of the BYTE-SEQ as the first element.
...
def main():
   ...
   parser.add_option("-o", "--MemoryOptimiseProps", dest="memory_optimised_properties", default='false', help="Storage space per property(2/3 words)., INPUT")
   parser.add_option("-s", "--structSize", dest="size_of_struct", default='false', help="Return sizeof(struct) for STRUCT properties ?, INPUT")
   ...
   if options.memory_optimised_properties is not None:
      memory_optimised_props =  options.memory_optimised_properties
      if ((re.sub('\s','',memory_optimised_props)).lower() == 'true'):
          memory_optimised_props = 1
      elif ((re.sub('\s','',memory_optimised_props)).lower() == 'false'):
          memory_optimised_props = 0
      else:
          raise ValueError("Invalid value for <props_memory_optimisation> tag. Valid value : true/false")
   if options.size_of_struct is not None:
      structSize =  options.size_of_struct
      if ((re.sub('\s','',structSize)).lower() == 'true'):
          structSize = 1
      elif ((re.sub('\s','',structSize)).lower() == 'false'):
          structSize = 0
      else:
          raise ValueError("Invalid value for <size_of_struct> tag. Valid value : true/false")
   ...
   for Mod in ModList.getElementsByTagName("module"):
      ...
      for Driver in Mod.getElementsByTagName("driver"):
         ...
         for Device in Driver.getElementsByTagName("device"):
            GenerateArray(Driver.getAttribute("name"),Device, GlobalDef, GlobalOffset, remove_dup_names, memory_optimised_props)
   ...
   if ConfigFileName != None:
      PrintConfig(ModName, CONFIG, DevcfgDataFileName, ConfigType, structSize)
      CONFIG.close()
