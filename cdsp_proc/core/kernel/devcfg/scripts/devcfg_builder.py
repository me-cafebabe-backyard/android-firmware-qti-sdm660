...
class Config:
    ...
    def parseImageCfgXML(self,env):
        ...
        srcOnly                      = image_cfg_dom.getElementsByTagName("src_only")
        configOnly                    = image_cfg_dom.getElementsByTagName("config_only")
        ...
        #Flag to compile source only
        try:
            srcOnlyFlag = re.sub('\s','',srcOnly[0].firstChild.data.encode("ascii")).lower()
        except:
            srcOnlyFlag = 'false'
            
        #Flag to compile config only
        try:
            configOnlyFlag = re.sub('\s','',configOnly[0].firstChild.data.encode("ascii")).lower()
        except:
            configOnlyFlag = 'false'
        ...
        self.setFeature('srcOnly',srcOnlyFlag)
        self.setFeature('configOnly',configOnlyFlag)
...
def generate(env):
    ...
    if 'DAL_DEVCFG_OEM_QSEE_IMG' in env.get('BUILD_TAGS'):
        env.get('DEVCFG').setFeature('devcfgImgBuildTag','DAL_DEVCFG_OEM_QSEE_IMG')
        env.get('DEVCFG').setFeature('targetImg','oem_tz')
    elif 'DAL_DEVCFG_OEM_HYP_IMG' in env.get('BUILD_TAGS'):
        env.get('DEVCFG').setFeature('devcfgImgBuildTag','DAL_DEVCFG_OEM_HYP_IMG')
        env.get('DEVCFG').setFeature('targetImg','oem_hyp')
    ...
    env.Replace(DEVCFG_XML_TAGS = env.get('DEVCFG').getFeature('devcfgXMLTagList'))
        ...
        for each_xml_tag in tag_list:     
            ...
            if each_xml_tag in thread_input_dict:            
                ...
                for file in c_file_list:
                    ...
                    if env.get('DEVCFG').getFeature('srcOnly') != 'true':
                        ...
                        env.get('DEVCFG').getFeature('DEVCFG_INFO_COLLECTOR').setdefault(target_image,{}).setdefault(cc_xml_tag,[]).append(config_c_file[0])
