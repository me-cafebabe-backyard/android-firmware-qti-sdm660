...
===============================================================================
 ...
 $Header: //components/rel/core.qdsp6/1.0.c2/hwengines/bam/core4/bamcore.c#1 $

when       who     what, where, why
--------   ---     ------------------------------------------------------------
03/10/15   SS      Added pipe interrupt mapping for slimbus bam at bam Init/Reset.
01/12/15   SS      Added Istari support
...
#ifdef HWIO_CORE5
#include <core5/bamhwio.h>
#else
#include <core4/bamhwio.h>
#endif
...
#define BAM_TESTBUS_MAX_INDEX             0x30
...
/*
   ...
   Shere       0xB    0x18   0x25
   Hawker      0xB    0x18   0x26
   Tesla       0xB    0x18   0x26
   Istari      0xB    0x19   0x27
   ...
   Istari          1.7.4   1.7.4  1.7.4 
*/


//Core specific values

#define BAM_MIN_VERSION                   6    // Aragorn
#define BAM_MAX_VERSION                   0xB  // Tesla, Istari, Nazgul

#define BAMLITE_MIN_VERSION               0x13 // Aragorn
#define BAMLITE_MAX_VERSION               0x19 // Istari, Nazgul

#define BAMNDP_MIN_VERSION                0x20 // Aragorn
#define BAMNDP_MAX_VERSION                0x27 // Istari, Nazgul
...
static void bamdev_write_reg(bam_vaddr base,uint32 offset,uint32 val)
{
...
static void bamdev_write_reg_field(bam_vaddr base,uint32 offset,uint32 mask, uint32 shift, uint32 field_val)
{
    bam_vaddr addr = base + offset;
...
static uint32 bamdev_read_reg(bam_vaddr base,uint32 offset)
{
    bam_vaddr addr = base + offset;
...
/* ============================================================================
**  Function : bamcore_sw_ver
** ============================================================================
*/
/**
  Return the bam sw version

  @return bam sw version
*/
uint32 bamcore_sw_ver(bam_vaddr base )
{
    return (bamdev_read_reg(base,BAM_SW_VERSION));
}
...
void bamcore_init(bam_vaddr base, uint32 irq, uint32 irq_mask, uint32 sum_thresh, const bam_target_config_type* tgt_cfg)
{
    ...
    //Config Bits2 for BAM
    /*if ( bamcore_sw_ver(base) >= BAM_SW_VER(1,7,3))
    {
        bamdev_write_reg(base,BAM_CNFG_BITS2,tgt_cfg->cfg_bits2);
    }
    */
...
void bamcore_deinit(bam_vaddr base)
{
    ...
    bamdev_write_reg_field(base, BAM_CTRL, BAM_MNS(BAM_CTRL,BAM_SW_RST), 1);
    //Disable the bam
    bamdev_write_reg_field(base,BAM_CTRL, BAM_MNS(BAM_CTRL,BAM_EN),0x0);
...
bam_paddr bamcore_get_error_address(bam_vaddr base)
{
#ifdef BAM_LPAE_SUPPORTED
    uint64 eaddress = 0;
    eaddress =  (uint64)bamdev_read_reg(base, BAM_AHB_MASTER_ERR_ADDR_LSB);
    ...
    return eaddress;
#else
    return bamdev_read_reg(base, BAM_AHB_MASTER_ERR_ADDR);
#endif
}
...
bam_status_type bamcore_pipe_init(bam_vaddr base, uint32 pipe_num,bam_pipe_config_type *pipe_cfg,uint32 irq_en, uint32 ee)
{
...
#ifdef BAM_LPAE_SUPPORTED
    uint32 lpae_supported =  bamcore_lpae_supported(base);
#endif
    // Reset the BAM pipe 
    ...
    if ( lpae_supported )
    {
        bamdev_write_reg(base, BAM_P_DESC_FIFO_ADDR_LSBn(pipe_num), ADDR_LPAE_LSB(pipe_cfg->desc_base_pa));
    ...
    if (pipe_cfg->mode == BAM_MODE_BAM2BAM) 
    {
        ...
        if ( lpae_supported )
        {
            ...
            bamdev_write_reg(base, BAM_P_DATA_FIFO_ADDR_LSBn(pipe_num), ADDR_LPAE_LSB(pipe_cfg->data_base_pa));
        } 
#else
        ...
        if ( lpae_supported )
        {
            ...
            bamdev_write_reg(base, BAM_P_EVNT_DEST_ADDR_LSBn(pipe_num), ADDR_LPAE_LSB(peer_dest_addr));
        }
#else
...
void bamcore_bamregdump(bam_vaddr base, uint32 *buf, bam_paddr base_pa, uint32 ee)
{
    ...
    if(0!= base)
    {
        ...
        if(ee != BAM_MAX_EES)
        {
            ...
            bambuf->bam_pipe_attr_een = bamdev_read_reg(base,BAM_PIPE_ATTR_EEn(ee));
        }
    }
    return;
}
...
/* ============================================================================
**  Function : bamcore_pipe_get_toggle
** ============================================================================
*/
/**
...
/* ============================================================================
**  Function : bamcore_secure_cfg
** ============================================================================
*/
/**
  configure BAM pipe trust register for pipe interrupt

  @return operation success or failure
*/
uint32 bamcore_secure_cfg(bam_vaddr base, uint32 numpipes, const bam_target_config_type *tgt_cfg)
{
    uint32 pipemask = 0, ee = 0;
    uint32 pipe = 0;

    // Assign top-level interrupt to EE 
    // Disable global reset 
    bamcore_set_ee(base, 0, 0);
    // Configure bam to consider EE of the pipe when locking
    bamdev_write_reg_field(base, BAM_TRUST_REG, BAM_MNS(BAM_TRUST_REG,LOCK_EE_CTRL), 1);
    
    for (ee = 0; ee <BAM_MAX_EES; ee++) 
    {
        pipemask = tgt_cfg->sec_config->ee[ee].pipe_mask;
        for (pipe = 0; (pipe <= numpipes && (pipemask != 0)); pipe++, pipemask >>=1) 
        {
            if((1 & pipemask) != 0) 
            {
                // Sets the Execution Environment for the pipe 
