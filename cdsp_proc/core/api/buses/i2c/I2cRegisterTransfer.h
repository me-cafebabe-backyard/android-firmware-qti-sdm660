...
$Header: //components/rel/core.qdsp6/1.0.c2/api/buses/i2c/I2cRegisterTransfer.h#1 $

When     Who    What, where, why
-------- ---    -----------------------------------------------------------
12/19/16 VG     I2C 2-byte&4-byte register addressing endianness correction
03/12/14 LK     Created
...
typedef enum I2cRegOffsetType 
{
   ...
   I2cRegOffset_32Bit   /**< 32 bit offset. */
} I2cRegOffsetType;
...
)
{
   ...
   // zero out the transfer command
   pTr->tranCfg.cmd         = I2cTrCfgCmd_Null;
}

/** @brief Initialize a transfer command

    @param[in]     pTr             Pointer to transfer.
    @param[in]     eTranCmd        Transfer command.

    @return             Nothing.
  */
static __inline void I2c_TrObj_Init_Cmd
(
   I2cTransfer          *pTr,            
   I2cTrCfgCmd           eTranCmd
)
{
   pTr->tranCfg.cmd = eTranCmd;
}
...
)
{
   ...
   if (offsetType == I2cRegOffset_8Bit)
   {
     uSize = 1;
   ...
   {
     uSize = 2;
     pRegObj->offset[1] = (uint8)((offset >> 0) & 0xFF);     
     pRegObj->offset[0] = (uint8)((offset >> 8) & 0xFF);
   }

   if (offsetType == I2cRegOffset_32Bit)
   {
     uSize = 4;
     pRegObj->offset[3] = (uint8)((offset >> 0)  & 0xFF);     
     pRegObj->offset[2] = (uint8)((offset >> 8)  & 0xFF);
     pRegObj->offset[1] = (uint8)((offset >> 16) & 0xFF);
     pRegObj->offset[0] = (uint8)((offset >> 24) & 0xFF);
   }
