...
/*
  ...
  $Header: //components/rel/core.qdsp6/1.0.c2/api/systemdrivers/DDIClock.h#1 $
  $DateTime: 2020/02/10 01:57:30 $
  $Author: pwbldsvc $

  when       who     what, where, why
  --------   ---     ---------------------------------------------------------- 
  05/24/17   shm     Added Clock_SetOverride API
...
struct DalClock
{
   ...
   DALResult (*AdjustSourceFrequency)(DalDeviceHandle * _h,  ClockSourceType eSource,  int32 nDeltaLAlpha);
   DALResult (*SelectClockSource)(DalDeviceHandle * _h,  ClockIdType  nClock, ClockSourceType eSource);
...
/* =========================================================================
**  Function : DalClock_AdjustSourceFrequency
** =========================================================================*/
/**
  Allows minor runtime tweeks to a particular source, where supported.

  This function adjusts a source based on the deltaL.alpha value supplied
  by the user.

  @param _h           [in] Handle to the DAL device.
  @param eSource      [in] The source to adjust the frequency on.
  @param nDeltaLAlpha [in] The deltaL.alpha value to use to modify the source
                           frequency.

  @return
  DAL_SUCCESS  - The frequency adjustment was made. \n
  DAL_ERROR    - An error occurred attempting the source adjustment.

*/

static __inline DALResult
DalClock_AdjustSourceFrequency(DalDeviceHandle * _h,  ClockSourceType eSource,  int32 nDeltaLAlpha)
{
   return((DalClockHandle *)_h)->pVtbl->AdjustSourceFrequency( _h, eSource, nDeltaLAlpha);
}


/* =========================================================================
**  Function : DalClock_SelectSource
** =========================================================================*/
/**
  Allows specification (where available) of a clock source.

  This function allows a user to select a clock source for a particular clock.
  Frequency will not be adjusted with this API so the this API should be called
  when the clock is not on.

  @param _h      [in] Handle to the DAL device.
  @param nClock  [in] The clock to switch the source on.
  @param eSource [in] The source to switch the clock to.
...
static __inline DALResult
DalClock_SelectClockSource(DalDeviceHandle * _h,  ClockIdType nClock, ClockSourceType eSource)
{
   return((DalClockHandle *)_h)->pVtbl->SelectClockSource( _h, nClock, eSource);
}
...
** =========================================================================*/
/**
  Applies the PLL overrides in SPM_EVENT_CTL register.

  This function is invoked by sleep driver just before entering sleep.
  This function is not actually a DAL function and does not require
  the DAL framework.
