...
/*=============================================================================
                             Edit History
$Header: //components/rel/core.qdsp6/1.0.c2/api/systemdrivers/DDIPlatformInfo.h#1 $
...
DalPlatformInfo_Platform (void)
{
  ...
  DalDeviceHandle             *phPlatformInfoTemp;
  DALResult                    eResult;
  DalPlatformInfoPlatformType  platform;

  if (phPlatformInfo == NULL)
  {
    eResult = DAL_DeviceAttach(DALDEVICEID_PLATFORMINFO, &phPlatformInfoTemp);
    if (eResult != DAL_SUCCESS)
    {
      return DALPLATFORMINFO_TYPE_UNKNOWN;
    }

    phPlatformInfo = phPlatformInfoTemp;
  }
