...
/**@ingroup func_qurt_mem_cache_phys_clean
  ...
  A cache line will be operated on if (LINE.PhysicalPageNumber & MASK) == ADDRMATCH

  @note1hang addrmatch value should be the Upper 24 bit Physical address to be matched against

  @datatypes
  #qurt_mem_cache_op_t \n

  @param[in] mask      24 bit address mask
  @param[in] addrmatch Physical Page number (24 bits) of memory to be used as address match.
  @param[in] opcode    Type of cache clean operation. Values:\n  #QURT_MEM_CACHE_FLUSH\n  #QURT_MEM_CACHE_FLUSH_INVALIDATE\n 
