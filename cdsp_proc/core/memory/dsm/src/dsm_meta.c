...
$Header: //components/rel/core.qdsp6/1.0.c2/memory/dsm/src/dsm_meta.c#1 $
...
)
{
  ...
  DSM_ASSERT( item_ptr != NULL && meta_ptr != NULL );
  DSM_ASSERT( DSM_POOL_ITEM_SIZE(item_ptr->pool_id) == 0 );
  ...
  DSM_ASSERT( table->pad_size >= size && size > 0 );
