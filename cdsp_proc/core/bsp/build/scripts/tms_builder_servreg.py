...
#===============================================================================
# Edit History
# $Header: //components/rel/core.qdsp6/1.0.c2/bsp/build/scripts/tms_builder_servreg.py#1 $
...
TOOL_KEY_SUBDOMAIN_LIST     = ['root_pd','modem_pd','audio_pd','sensor_pd','wlan_pd']
...
def tool_action(target, source, env):
   ...
   Playbook_C_Output.append('servreg_entry_t servreg_local_services[]={\n')
   ...
   # Final Output
      ...
      Playbook_C_Output.append('unsigned int servreg_service_count = ' + str(json_service_count) + ';\n')
      Playbook_C_Output.append('char *servreg_local_domain = "' + json_domain_dict_item[TOOL_KEY_SOC_NAME] + '/' + json_domain_dict_item[TOOL_KEY_DOMAIN_NAME] + '/' + json_domain_dict_item[TOOL_KEY_SUBDOMAIN_NAME] + '";\n')
      Playbook_C_Output.append('char *servreg_soc_name = "' + json_domain_dict_item[TOOL_KEY_SOC_NAME] + '";\n')
      Playbook_C_Output.append('char *servreg_domain_name = "' + json_domain_dict_item[TOOL_KEY_DOMAIN_NAME] + '";\n')
      Playbook_C_Output.append('char *servreg_subdomain_name = "' + json_domain_dict_item[TOOL_KEY_SUBDOMAIN_NAME] + '";\n')
      Playbook_C_Output.append('unsigned int servreg_qmi_instance_id = {:d} '.format(json_domain_dict_item[TOOL_KEY_QMI_INSTANCE_ID]) + ';\n')
...
def tool_method_add_domain(env, build_ids, input_dict):
   ...
   if not input_dict[TOOL_KEY_SUBDOMAIN_NAME] in TOOL_KEY_SUBDOMAIN_LIST:
      tool_error(env, input_dict[TOOL_KEY_SUBDOMAIN_NAME] + ' is not a valid ' + TOOL_KEY_SUBDOMAIN_NAME + ' name. Get name from list = [modem_pd, audio_pd, sensor_pd, , wlan_pd, root_pd ] ')
