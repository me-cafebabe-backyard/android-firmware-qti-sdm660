...
void sio_set_driver_init_list(void)
{
  ...
  CORE_VERIFY(index <= SIO_MAX_DRIVERS);
...
void sio_init_rdm_sdm()
{
  ...
  /* Initialize SIO Critical Section */
  CORE_VERIFY(qmutex_create(&sio_crit_sect, &qmutexattr) == EOK);  
}
...
{
   CORE_VERIFY(qmutex_lock(sio_crit_sect) == EOK);
}
...
{
    CORE_VERIFY(qmutex_unlock(sio_crit_sect) == EOK);
}
