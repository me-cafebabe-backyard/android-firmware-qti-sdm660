...
$Header: //components/rel/core.qdsp6/1.0.c2/buses/slimbus/src/dal/SlimBusTarget.c#1 $
...
SBResult SlimBus_ClockOn(SlimBusDevCtxt *pDevCtxt)
{
  ...
  if ( pDevCtxt->bIsHwOwner )
  {
    ...
    if (NULL != pDevCtxt->Plat.hMMPM)
    {
      MMPM_STATUS status = MMPM_Request_Ext(pDevCtxt->Plat.hMMPM, &RscExtParam);
...
SBResult SlimBus_ClockOff(SlimBusDevCtxt *pDevCtxt)
{
  ...
  if ( pDevCtxt->bIsHwOwner )
  {
    if (NULL != pDevCtxt->Plat.hMMPM)
    {
      MMPM_Release_Ext(pDevCtxt->Plat.hMMPM, &RscExtParam);
    }
