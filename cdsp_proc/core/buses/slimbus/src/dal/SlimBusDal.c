...
$Header: //components/rel/core.qdsp6/1.0.c2/buses/slimbus/src/dal/SlimBusDal.c#1 $

when       who     what, where, why
--------   ---     --------------------------------------------------------
10/27/17   DPK     Added a flag check whether to use GPIO interrupt or not
04/04/17   RC      Addiotnal check as part of inactive configuaration.
03/06/17   MJS     Use DALSYS timer APIs rather than DAL Timer driver.
03/01/17   RC	   Add the additionals check to configure the SLIMBUS 
                   uTLMM GPIOs  
11/22/16   MJS     Porting cross-platform compatibility updates.
10/25/16   RC	   To use the right  TLMM API to configure the SLIMBUS GPIO 
...
SBResult SlimBus_InitServices(SlimBusDevCtxt *pDevCtxt)
{
  ...
  {
    uint32 uGpioNum = 0;
	uCfg = pDevCtxt->pBSP->uaGpioConfig[uIdx];
