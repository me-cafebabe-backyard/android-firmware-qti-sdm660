...
$Header: //components/rel/core.qdsp6/1.0.c2/buses/slimbus/src/common/SlimBus.c#1 $

when       who     what, where, why
--------   ---     --------------------------------------------------------
06/19/17   RC      Added logic to clear the datashift bit while slimbus core
                   coming out power collapse for HWBUG QCTDD03351991.
...
void SlimBusEnumFailHardwareReset(SlimBusDevCtxt *pDevCtxt)
{
  ...
  /* As Bam reset is happening in the  SlimBus_DeviceInitHardware, mark the bamhandles NULL*/
  for(i = 0;i < pDevCtxt->numPorts ;i++)
  {
    SlimBusMasterPortType *pPort = &pDevCtxt->Ports[i];
...
/**
 * @brief Clears the Datashift
 *
 * This function clears the boolean variable for the 
 * data shift when the core got  HW reset and
 * when it come out of power collapse.
  * 
 * @param[in] pDevCtxt  Pointer to the device context
 */
void SlimBus_ClearbDataShift(SlimBusDevCtxt *pDevCtxt)
{
  uint32 i;
  /*32bit  Datashift at the port level due to HWBUG QCTDD03351991 will be get cleared
    after power collapse so we need to clear the corresponding flag after the core reset*/
  for ( i = 0; i < pDevCtxt->numPorts; i++ )
  {
    if(pDevCtxt->Ports[i].bDataShiftRemoval)
    {
      pDevCtxt->Ports[i].bDataShiftRemoval = FALSE ;
      SB_LOG_INFO_0("clearing the datashift flag");
    }
  }
}
...
void SlimBus_CheckAndRestoreNonRetentionRegisters(SlimBusDevCtxt *pDevCtxt)
{
       ...
       0 == HAL_sb_DeviceIntGetMask(pDevCtxt->hSB, pDevCtxt->eMsgDev) )
  {
    ...
    {
      SlimBus_ClearbDataShift(pDevCtxt);
    }
  }
}
...
/**
  ...
  */
SBResult
SlimBus_GetMaxEndPointsSupported(SlimBusHandle h, uint32 *puMaxEP)
{
  SlimBusClientCtxt *pClientCtxt = SB_GET_PCLIENTCTXT(h);
  SlimBusDevCtxt *pDevCtxt = SB_GET_PDEVCTXT(pClientCtxt->hDevCtxt);
  uint32 i;
...
) 
{
  ...
  /* Scan for the first port available for allocation, and for multi-channel
     audio ensure that a contiguous group of ports is available */
  for (i = 0; i < pDevCtxt->numPorts; i++)
  {
    if (pDevCtxt->Ports[i].eState == SB_HANDLE_AVAILABLE)
    {
      /* Half-duplex channel must start on even port */
      if ( ( eReqs & SLIMBUS_PORT_REQ_O_HALF_DUPLEX ) &&
           ( i % 2 == 1 ) )
      {
        continue;
      ...
      /* For multi-channel audio, validate that we can allocate a contiguous
         block of ports */
      if (eReqs & SLIMBUS_PORT_REQ_O_MULTI_CHAN_GROUP)
      {
        SBBOOL bMultiOK = TRUE;
        uint32 numPhysPorts = numPorts;
        
        if ( eReqs & SLIMBUS_PORT_REQ_O_HALF_DUPLEX )
        {
          numPhysPorts *= 2;
        }
        if ( i+numPhysPorts > pDevCtxt->numPorts )
        {
          i = pDevCtxt->numPorts;
          break;
        }
        for (j = i+1; j < i+numPhysPorts; j++)
        {
          if ( (pDevCtxt->Ports[j].eState != SB_HANDLE_AVAILABLE) ||
               (HAL_sb_PgdPortGetFifoSize(pDevCtxt->hSB, 0, pDevCtxt->Ports[j].portNum) < uMinFifoBytes) ||
               (uMinNumEndPoints > 1 && 
                pDevCtxt->pBSP->uaNumEndPoints[i + pDevCtxt->uBasePortNum] < uMinNumEndPoints) )
          {
            result = SLIMBUS_ERR_CONSTRAINTS_NOT_SATISFIED;
          ...
          if (uMinNumEndPoints > pDevCtxt->Ports[j].numEndPoints)
          {
            if (SB_SUCCESS != SlimBus_Realloc(sizeof(SlimBusEndPointType)*uMinNumEndPoints,
                                              sizeof(SlimBusEndPointType)*pDevCtxt->Ports[j].numEndPoints,
                                              (void**)&pDevCtxt->Ports[j].aEndPoints,
                                              SB_MEMTAG_25))
            {
              result = SB_ERROR;
              bMultiOK = FALSE;
            ...
            pDevCtxt->Ports[j].numEndPoints = uMinNumEndPoints;
            for ( k = 0; k < pDevCtxt->Ports[j].numEndPoints; k++ )
            {
              pDevCtxt->Ports[j].aEndPoints[k].endPointNum = k;
              pDevCtxt->Ports[j].aEndPoints[k].pPort = &pDevCtxt->Ports[j];
            }
          }
        }
        if (!bMultiOK)
  ...
  /* If we couldn't find a port available for allocation, error */
  if ( i >= pDevCtxt->numPorts )
  {
    SB_LOG_ERR_0("No master ports available during alloc request");
  ...
  /* reset error code to success if we are able to allocate a port */
  result = SB_SUCCESS;
  
  /* Now that we found port(s) available for allocation, do the allocation */  
  for (portIdx = 0; portIdx < numPorts && i < pDevCtxt->numPorts; i++)
  {
    if (pDevCtxt->Ports[i].eState == SB_HANDLE_AVAILABLE)
    {
  ...
  if (portIdx < numPorts)
  {
    SB_LOG_ERR_0("Unable to alloc all requested master ports");

    SlimBus_UnlockDevice(pDevCtxt);
    SlimBus_DeallocMasterPorts(h, phPortBuf, uPortBufSize);
    return SB_ERROR;
  }
...
{
  return SlimBus_AllocMasterPortsInternal(h, eReqs, 1, uMinFifoBytes, phPortBuf, uPortBufSize);
}
...
/**
  ...
  @return  SB_SUCCESS on success, an error code on error
  */
SBResult SlimBus_AllocMasterPortWithEndPoints(SlimBusHandle h, uint32 uMinNumEndPoints, uint32  uMinFifoBytes, SlimBusResourceHandle *phPort)
{
  return SlimBus_AllocMasterPortsInternal(h, 0, uMinNumEndPoints, uMinFifoBytes, phPort, sizeof(*phPort));
}
...
/**
  ...
  @return SB_SUCCESS on success, an error code on error 
  */
SBResult SlimBus_SetBatchBufferSize(SlimBusHandle h, uint32 uMaxBufferSize)
{
  SlimBusClientCtxt *pClientCtxt = SB_GET_PCLIENTCTXT(h);
...
/**
  ...
  */
SBResult
SlimBus_SendBatchedMessages(SlimBusHandle h)
{
  SlimBusClientCtxt *pClientCtxt = SB_GET_PCLIENTCTXT(h);
  SlimBusDevCtxt *pDevCtxt = (SB_GET_PDEVCTXT(pClientCtxt->hDevCtxt));
  SBResult result = SB_SUCCESS;
...
/**
  ...
  */
static SBResult 
SlimBus_DeallocDataChannelInternal(SlimBusClientCtxt *pClientCtxt, SlimBusDevCtxt *pDevCtxt, SlimBusChannelType *pChannel) 
{
  SBResult result = SB_SUCCESS;
...
SlimBus_DeallocDataChannel(SlimBusHandle h, SlimBusResourceHandle  hChannel) 
{
  ...
  {
    result = SlimBus_DeallocDataChannelInternal(pClientCtxt, pDevCtxt, pChannel);
  }
 ...
 */
uint32 SlimBus_DeviceDetach(SlimBusHandle h)
{
  SlimBusClientCtxt *pClientCtxt = SB_GET_PCLIENTCTXT(h);
  SlimBusDevCtxt *pDevCtxt = SB_GET_PDEVCTXT(pClientCtxt->hDevCtxt);
  uint32 dwref = 0;
  ...
  for (uIdx = 0; uIdx < pDevCtxt->numChannelSlots; uIdx++)
  {
         ...
         pClientCtxt == pChannel->pClientCtxt )
    {
      ...
      {
        SlimBus_DeallocDataChannelInternal(pClientCtxt, pDevCtxt, pChannel);
      }
