...
$Header: //components/rel/core.qdsp6/1.0.c2/buses/slimbus/src/common/SlimBusMaster.c#1 $
...
Helper functions
------------------------------------------------------------------------------*/
static void pushSlimBusSinkNode(struct SlimBusSinkNode **headRef, struct SlimBusDevCtxt *pDevCtxt,
                      SlimBusLogicalAddrType LA, uint32 portNum)
{
  struct SlimBusSinkNode *newSinkNode = NULL;
  struct SlimBusSinkNode *temp = *headRef;

  while (temp != NULL)
  {
    if ((temp->SinkPortNum == portNum)&&(temp->LA == LA))
    {
      break;
    }
    temp = temp->next;
  ...
  if(temp == NULL)  
  {
    SlimBus_Malloc(sizeof(struct SlimBusSinkNode), (void **)&newSinkNode, SB_MEMTAG_26);
    newSinkNode->LA = LA;
    newSinkNode->SinkPortNum = portNum;
    newSinkNode->next = *headRef; 
    *headRef = newSinkNode;
  }
}

static void pushChanLAddAssociation(SlimBusChanLAddAssociation **headRef, SlimBusDevCtxt *pDevCtxt,
              uint32 chanNum, uint32 portNum, SlimBusPortFlowType eFlow)
{
  SlimBusChanLAddAssociation *pNewAssociationNode = NULL;
  SlimBusChanLAddAssociation *temp = *headRef;

  while (temp != NULL)
  {
    if ((temp->portNum == portNum)&&(temp->channelNum == chanNum))
    {
      temp->eFlow = eFlow;
      break;
    }
  ...
  if(temp == NULL)  
  {
    SlimBus_Malloc(sizeof(SlimBusChanLAddAssociation), (void **)&pNewAssociationNode, SB_MEMTAG_27);

    pNewAssociationNode->channelNum = chanNum;
    pNewAssociationNode->portNum = portNum;
    pNewAssociationNode->eFlow = eFlow;
    pNewAssociationNode->pNext = *headRef;

    *headRef = pNewAssociationNode;
  }
}


static void getChanLAddAssociation(SlimBusChanLAddAssociation *head, SlimBusDevCtxt *pDevCtxt,
            uint32 portNum, SlimBusChanLAddAssociation **resultRef)
{
  SlimBusChanLAddAssociation *temp = head;
  
  while (temp != NULL)
  {
    if (temp->portNum == portNum)
    {
      *resultRef = temp;
      break;
    }
...
static void deleteChanLAddAssociation(SlimBusChanLAddAssociation **headRef, 
                    SlimBusDevCtxt *pDevCtxt, uint32 portNum)
{
  SlimBusChanLAddAssociation *temp = *headRef;
  SlimBusChanLAddAssociation *prev = *headRef;
...
static SBResult removeSlimBusSinkNode(struct SlimBusSinkNode **headRef,
                SlimBusDevCtxt *pDevCtxt, SlimBusLogicalAddrType LA,
                uint32 portNum)
{
  struct SlimBusSinkNode *temp = *headRef;
  struct SlimBusSinkNode *prev = *headRef;

  while (temp != NULL)
  {
    if ((temp->LA == LA) && (temp->SinkPortNum == portNum))
    {
      if (temp == *headRef)   //First node
      {
        *headRef = temp->next;
      }
      else
         ...
         SB_QC_MASTER_PROTOCOL_V1 == pDevice->pSat->uSatProtoVer )
    {
                ...
                SB_MSG_MC_USR_REQ_CONNECT_SINK == eUsrMC )
      {
        ...
        if (eUsrMC == SB_MSG_MC_USR_REQ_CONNECT_SOURCE)
        {
          ...
          SB_LOG_VERB_4("Got satellite connect source port request "
                      "(satLA: 0x%x) (channel: %d) (destLA: 0x%x) (port: %d) ",
                      LA, chanNum, destLA, portNum);
        }
        else
        {
          eMC = SB_MSG_MC_CONNECT_SINK;
          SB_LOG_VERB_4("Got satellite connect sink port request "
                      "(satLA: 0x%x) (channel: %d) (destLA: 0x%x) (port: %d) ",
                      LA, chanNum, destLA, portNum);
        }
        if (eMC == SB_MSG_MC_CONNECT_SOURCE)
        {
          if (pChanParam->bIsSrcConnected == TRUE)
          {
            if((pChanParam->SrcLA != destLA)|| 
               (pChanParam->SrcPortNum != portNum))
            {
              SB_LOG_ERR_2("ProcessMasterRxMsg: A source is already connected (LA: 0x%x) (port: %ld)",
                           pChanParam->SrcLA, pChanParam->SrcPortNum);
              return SB_ERROR;
            }
          }
          else
          {
            pChanParam->bIsSrcConnected = TRUE;
            pChanParam->SrcLA = destLA;
            pChanParam->SrcPortNum = portNum;
            pushChanLAddAssociation(&(pDevice->pConnectedChannels), pDevCtxt,
                                    chanNum, portNum, SLIMBUS_PORT_SOURCE_FLOW);
          }
        }
        else if (eMC == SB_MSG_MC_CONNECT_SINK)
        {
          pushSlimBusSinkNode(&(pChanParam->pSinkList), pDevCtxt, LA,
                              portNum);
          pushChanLAddAssociation(&(pDevice->pConnectedChannels), pDevCtxt,
                                  chanNum, portNum, SLIMBUS_PORT_SINK_FLOW);
        }
      ...
      else if ( SB_MSG_MC_USR_REQ_DISCONNECT_PORT == eUsrMC )
      {
        ...
        uint8 portNum, TID;
        uint32 *pBuf;
        SlimBusChanLAddAssociation *pAssociationNode;
        ...
        *pBuf = SB_MSG_SET(TRUE,USR_GENERIC_ACK_1,RESP);
        pDevCtxt->TxMsgFifo.sendBufFcn(pDevCtxt, &pDevCtxt->TxMsgFifo);
        pAssociationNode = NULL;
        getChanLAddAssociation(pDevice->pConnectedChannels, pDevCtxt, portNum, &(pAssociationNode)); 
        
        if (pAssociationNode != NULL)
        {
          if (pAssociationNode->eFlow == SLIMBUS_PORT_SOURCE_FLOW)
          {
            pDevCtxt->pSched->apChanParams[pAssociationNode->channelNum]->SrcLA = 0;
            pDevCtxt->pSched->apChanParams[pAssociationNode->channelNum]->SrcPortNum = 0;
            pDevCtxt->pSched->apChanParams[pAssociationNode->channelNum]->bIsSrcConnected = FALSE;
          }
          else
          {
            removeSlimBusSinkNode(&(pDevCtxt->pSched->apChanParams[pAssociationNode->channelNum]->pSinkList),
                                  pDevCtxt, LA, portNum);
          }
          deleteChanLAddAssociation(&(pDevice->pConnectedChannels), pDevCtxt, portNum);
        }   
      }
      else
      {
...
{
  SlimBusEventNodeType *pEvent = &pDevCtxt->pSched->SchedEventNode;
...
)
{
       ...
       ( bReadyForPause = (pDevCtxt->bActiveFramer && SlimBus_ReadyForPause(pDevCtxt)) ) )
  {
    ...
    if (SB_SUCCESS == result)
    {
      do
      {
        ...
        {	
          if(pDevCtxt->Ports[i].eState == SB_HANDLE_ALLOCATED)
          {	
            if(pDevCtxt->Ports[i].eFlow == SLIMBUS_PORT_SINK_FLOW)
            {
         ...
         !bAllowFailure )
    {
      ...
      for ( ; pList != NULL; pList = pNextList, pNextList = NULL )
      {
        ...
        while ( NULL != pParam )
        {
          ...
          if (pParam->uNewRefCnt == 0)
          {
            SlimBusSinkNode *pPrevSinkNode;

            /* Source clean up first */
            if (pParam->SrcLA != 0)
            {
              SlimBusDeviceType *pTempSrcDevice = SlimBus_GetDevicePtr(pDevCtxt, pParam->SrcLA);

              if ((pTempSrcDevice != NULL) && (pTempSrcDevice->pConnectedChannels != NULL))
              {
                deleteChanLAddAssociation(&(pTempSrcDevice->pConnectedChannels), pDevCtxt, pParam->SrcPortNum);
              } 
              pParam->bIsSrcConnected = FALSE;
              pParam->SrcPortNum = 0;
              pParam->SrcLA = 0;
            }
            ...
            {
              pPrevSinkNode = pParam->pSinkList;
              pParam->pSinkList = pParam->pSinkList->next;

              if (pPrevSinkNode->LA != 0)
              {
                SlimBusDeviceType *pTempSinkDevice = SlimBus_GetDevicePtr(pDevCtxt, pPrevSinkNode->LA);

                if ((pTempSinkDevice != NULL) && (pTempSinkDevice->pConnectedChannels != NULL))
                {
                  deleteChanLAddAssociation(&(pTempSinkDevice->pConnectedChannels), pDevCtxt, pPrevSinkNode->SinkPortNum);
                }
                pPrevSinkNode->LA = 0;
                pPrevSinkNode->SinkPortNum = 0;
                pPrevSinkNode->next = NULL;

                SlimBus_Free(pPrevSinkNode);
              }
            } 
          }
          ...
          else
          {
            ...
            /*Logic added to set the dirty bit for sync port, when channel 
 			...
 			if((pParam->uCurOffset+pParam->ucSegLen) ==  pParam->uCurInterval)
			{
			  uint32 portnum,idx;
     		  SlimBusMasterPortType *pPort = NULL;			  
			  SlimBusSinkNode *pTmpSinkList;
			  pTmpSinkList = pParam->pSinkList;
			  while (pTmpSinkList != NULL)
			  {
		        if (pTmpSinkList->LA != NULL)
			    {
      			  if((pTmpSinkList->LA) == (pDevCtxt->pgdLA))
				  {
	  			    portnum = pTmpSinkList->SinkPortNum;
					idx = portnum - (pDevCtxt->uBasePortNum);
					if(idx <  pDevCtxt->numPorts)
					{
		              pPort = &pDevCtxt->Ports[idx];
				      pPort->bDataShiftRemoval = TRUE;
					  SB_LOG_INFO_0("update the datashift variable for PGD");
					}
				  }
			    }
			    pTmpSinkList = pTmpSinkList->next;  
			  } 
     		}
  ...
  if (pDevCtxt->bPendingSSR)
  {
    ...
    SlimBus_EventTrigger(pDevCtxt->pSched->hSchedEvent);
  }
...
)
{
  ...
  *(pBuf++) = SB_MSG_SET(uChanNum,CONNECT_SOURCE_1,CN);
  ...
  if (eFlow == SLIMBUS_PORT_SOURCE_FLOW)
  {
    if (pChanParam->bIsSrcConnected == TRUE)
    {
      SB_LOG_ERR_2("MasterConnectPort: A source is already connected (LA: 0x%x) (port: %ld)",
                   pChanParam->SrcLA, pChanParam->SrcPortNum);
      return SB_ERROR;
    }
    else
    {
      pChanParam->bIsSrcConnected = TRUE;
      pChanParam->SrcLA = pDevice->LA;
      pChanParam->SrcPortNum = uPortNum;
    } 
  }
  else if (eFlow == SLIMBUS_PORT_SINK_FLOW)
  {
    pushSlimBusSinkNode(&(pChanParam->pSinkList), pDevCtxt, pDevice->LA, uPortNum);
  }
  
  pushChanLAddAssociation(&(pDevice->pConnectedChannels), pDevCtxt,
            uChanNum, uPortNum, eFlow);
...
{
  SlimBusChanLAddAssociation *pAssociationNode = NULL;
  uint32 *pBuf;
  ...
  *(pBuf++) = SB_MSG_ASM_FIRST_WORD(SB_MSG_MC_DISCONNECT_PORT_LEN,
  ...
  getChanLAddAssociation(pDevice->pConnectedChannels, pDevCtxt, uPortNum, &(pAssociationNode)); 
  if (pAssociationNode != NULL)
  {  
    if (pAssociationNode->eFlow == SLIMBUS_PORT_SOURCE_FLOW)
    {
    ...
    {
      removeSlimBusSinkNode(&(pDevCtxt->pSched->apChanParams[pAssociationNode->channelNum]->pSinkList),
                            pDevCtxt, pDevice->LA, uPortNum);
    }
    deleteChanLAddAssociation(&(pDevice->pConnectedChannels), pDevCtxt, uPortNum);
  }
...
SBResult SlimBus_DeviceInitMaster(SlimBusDevCtxt *pDevCtxt)
{
  ...
  pDevCtxt->pSched->ePause = SB_PAUSE_PAUSED;
  pDevCtxt->pSched->bEnablePause = TRUE;
...
void SlimBus_SSReset(SlimBusDevCtxt *pDevCtxt)
{
  ...
  if (SB_MASTER_DRIVER == pDevCtxt->eDrvType)
  {
    ...
    if (pDevCtxt->pSched->bReconfigBusy)
    {
      ...
      SlimBus_UnlockDevice(pDevCtxt);
      SlimBus_EventWait(pDevCtxt->pSched->hSchedEvent);
      SlimBus_LockDevice(pDevCtxt);
    }
