...
$Header: //components/rel/core.qdsp6/1.0.c2/buses/slimbus/src/common/SlimBus.h#1 $
...
10/21/16   RC      Support to know which tlmm hold the slimbus gpio lines  
...
typedef enum
{
  ...
  SB_WRKRND_O_DATASHIFT_INVALIDATE = 0x2000,
...
{
  /* Channel number with which the device is connected */
  uint32 channelNum;
  /* Port number with which the device is connected to the above channel */
  uint32 portNum;
  /* Is the device connected as a source or sink */
  SlimBusPortFlowType eFlow;
  /* Pointer to the next association structure in the list */
...
struct SlimBusSinkNode
{
  ...
  /* Port number with which the device is connected to the channel */
  uint32 SinkPortNum;
...
typedef struct
{
  ...
  /* To know the SLIMBUS GPIO lines which part of TLMM */
  SBBOOL bIsLpiTlmm;  
} SlimBusBSPType;
...
struct SlimBusMasterPortType
{
  ...
  /* Memory handle created for the dummy 4-byte shift descriptor */
...
typedef struct
{
  ...
  /* Is a source connected to the channel */
  SBBOOL bIsSrcConnected;
  /* Logical address of the source connected to the channel */
  SlimBusLogicalAddrType SrcLA;
  /* Port number with which the source device is connected to the channel */
  uint32 SrcPortNum;
...
/**
  ...
  */
extern SBResult
SlimBus_AllocMasterPortWithEndPoints(SlimBusHandle h, uint32 uMinNumEndPoints, uint32  uMinFifoBytes, SlimBusResourceHandle *phPort);
...
/**
  ...
  */
extern SBResult
SlimBus_SetBatchBufferSize(SlimBusHandle h, uint32 uMaxBufferSize);
