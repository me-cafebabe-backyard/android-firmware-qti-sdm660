...
================================================================================
$Header: //components/rel/core.qdsp6/1.0.c2/power/sleep2.0/src/statistics/sleep_qr.c#1 $
                                           ...
                                           int32            delta)
{
  ...
  synth_LPRM_dyn_data     *synthLPRMDynData   = &dynData->synth.LPRM[synthDynNum];
  ...
  if(0 == delta)
  {
    ...
    /* Loop through uncacheable component modes to compute new synthmode backoff */
    numMode         = synthMode->uncacheable.num_component_modes;
    component_modes = synthMode->uncacheable.component_modes;
    for(modeIdx = 0; modeIdx < numMode; modeIdx++)
    {
      dynNum            = component_modes[modeIdx]->dynamic_num;
                                   ...
                                   sleep_fLUT_node *fLUT)
{
  ...
  for(synthIdx=0; synthIdx < synthLPR->mode_count; synthIdx++) 
  {
    ...
    synthLPRMDynData    = &dynData->synth.LPRM[synthDynNum];
                                  ...
                                  int32             delta)
{
  ...
  for(modeIdx = 0; modeIdx < numMode; modeIdx++)
  {
    sleepLPRM         = component_modes[modeIdx];
                     ...
                     int32            wakeupDelta)
{
  ...
  synthQRData     = &dynData->synth.LPRM[synthDynNum].QR_data;
                             ...
                             sleep_fLUT_node   *fLUT)
{
  ...
  for(modeIdx=0; modeIdx < numMode; modeIdx++) 
  {
    componentLPRM   = component_modes[modeIdx];
...
{
  uint32              nMode;
  synth_LPRM_dyn_data *synthLPRMDynData = dynData->synth.LPRM;
