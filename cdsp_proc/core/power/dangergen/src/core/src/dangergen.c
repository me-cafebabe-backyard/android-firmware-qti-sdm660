    ...
    dangergen_watchdog_params_type* params)
{
    ...
    // Automatically set threshold2, threshold3, and window if they were not
    // set by the user.  Default window will be four times the duration of
    // threshold 1.
    if(generator->threshold2_ns == DANGERGEN_UNSPECIFIED_TIME)
        generator->threshold2_ns = generator->threshold1_ns;
    if(generator->threshold3_ns == DANGERGEN_UNSPECIFIED_TIME)
        generator->threshold3_ns = generator->threshold1_ns;
    if(generator->window_ns == DANGERGEN_UNSPECIFIED_TIME)
        generator->window_ns = 4 * generator->threshold1_ns;
...
static void dangergen_qos_enable_vote(dangergen_driver_type* self, bool addVote)
{
    if(addVote)
    {
        ...
        if(self->qos_enable_count == 1)
        {
			...
			if(sysmon_qos_clock_enable(SYSMON_QOS_CLIENT_ID_QDSPPM, 1) != 0)
    ...
    else // remove vote
    {
        if(self->qos_enable_count > 0)
        {
            ...
            if(self->qos_enable_count == 0)
            {
				...
				if(sysmon_qos_clock_enable(SYSMON_QOS_CLIENT_ID_QDSPPM, 0) != 0)
