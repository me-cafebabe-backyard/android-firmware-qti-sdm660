...
$Header: //components/rel/core.qdsp6/1.0.c2/power/qdsp_pm/src/driver/qdsp_pm_mgr/request_mgr/asic.c#1 $
*/
            ...
            )
        {
            for(index = 0; index < table->numElements; index++)
            {
                if(vote <= table->pRows[index].bwThreshold)
                {
                    ...
                    {
                        pTableEntry->bwVote = table->pRows[index].bwVote;
                    }
            ...
            ADSPPM_LOG_PRINTF_6(ADSPPM_LOG_LEVEL_INFO,
                ...
                (uint32) pTableEntry->bwVote, // two params for %llu
                (uint32) (pTableEntry->bwVote >> 32));
...
Adsppm_Status ACM_ApplyAhbBwCompensation(uint64 bwVoteIn, uint64* pBwVoteOut)
{
    ...
    uint32 index;
    AsicCompensatedAhbBwTableType *table = g_AsicConfig.compensatedAhbBwTable;
                ...
                if(lookupResult != -1)
                {
                    *pBwVoteOut = lookupResult;
                    ADSPPM_LOG_PRINTF_4(ADSPPM_LOG_LEVEL_INFO,
                        "AHB BW compensation: aggregated vote %llu, "
                        "compensated %llu",
...
Adsppm_Status ACM_GetAhbeFromAdspFreq(uint32 adspClockFreq, uint32* ahbeClockFreq)
{
    Adsppm_Status result = Adsppm_Status_Failed;
    AsicAdspToAhbeFreqTableType* table = g_AsicConfig.adspToAhbeFreqTable;
    if((ahbeClockFreq != NULL) && (table != NULL) && (table->pRows != NULL))
    {
        uint32 index = 0;
        for(index = 0; index < table->numElements; index++)
        {
        ...
        // Use the highest AHBE freq if ADSP freq is larger than any table entry
        if(index >= table->numElements)
            index = table->numElements - 1;
        *ahbeClockFreq = table->pRows[index].ahbeFreqHz;
        result = Adsppm_Status_Success;
    }
    return result;
}
...
Adsppm_Status asicGetAsicConfig(AsicConfigType *pConfig)
{
    ...
    if(result == Adsppm_Status_Success)
    {
        ...
        for(i = 0; i < pConfig->cores->numElements; i++)
        {
            ...
            {
                ADSPPM_LOG_PRINTF_1(ADSPPM_LOG_LEVEL_ERROR,
                    "iinvalid array size of resource instances, coreId=%u", i);
                result = Adsppm_Status_Failed;
                break;
            }
        }
    ...
    DAL_GET_PROPERTY_VALUE_WITH_CHECK("COMPENSATED_AHB_BW_TABLE")
    if(result == Adsppm_Status_Success)
    {
        pConfig->compensatedAhbBwTable = (AsicCompensatedAhbBwTableType *)system_property_var.Val.pStruct;
    }
    ...
    DAL_GET_PROPERTY_VALUE_WITH_CHECK("ADSP_TO_AHBE_FREQ_TABLE") 
	
    if(result == Adsppm_Status_Success)
    {
        pConfig->adspToAhbeFreqTable =
            (AsicAdspToAhbeFreqTableType *)system_property_var.Val.pStruct;
    }
