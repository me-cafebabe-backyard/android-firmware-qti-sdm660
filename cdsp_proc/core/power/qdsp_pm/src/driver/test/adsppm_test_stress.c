...
static MMPM_STATUS TestStressMultiClientMultiBwReqRel(void)
{
    ...
    if(clientId1 && clientId2)
    {
        ...
        for(loop = 0; (loop < STRESS_TEST_LOOP) && (MMPM_STATUS_SUCCESS == retSts); loop++)
        {
            sts = Test_RequestBandwidth(clientId1, &bw1, 1);
            if(MMPM_STATUS_SUCCESS != sts)
            {
                ADSPPMTEST_LOG_PRINTF_1(
                    ADSPPMTEST_LOG_LEVEL_ERROR,
            ...
            sts = Test_RequestBandwidth(clientId2, &bw2, 1);
            if(MMPM_STATUS_SUCCESS != sts)
            {
                ADSPPMTEST_LOG_PRINTF_1(
                    ADSPPMTEST_LOG_LEVEL_ERROR,
...
#define STRESS_20_NAME    "stress_20"
#define STRESS_20_DETAILS "Bandwidth Req-Rel, 2 clients, different bus route, multiple times"

static MMPM_STATUS TestStressMultiClientMultiBwReqRel_20(void)
{
    MMPM_STATUS sts = MMPM_STATUS_FAILED, retSts = MMPM_STATUS_SUCCESS;
    uint32 clientId1 = 0, clientId2 = 0, loop;
    MmpmCoreIdType coreId = MMPM_CORE_ID_LPASS_ADSP;
    ...
    clientId1 = Test_Register(coreId, instanceId, STRESS_20_NAME"_1");
    clientId2 = Test_Register(coreId, instanceId, STRESS_20_NAME"_2");

    if(clientId1 && clientId2)
    {
        bw1.masterPort = MMPM_BW_PORT_ID_ADSP_MASTER;
        bw1.slavePort = MMPM_BW_PORT_ID_SRAM_SLAVE;
        bw1.bwVal = 100000000;
        bw1.usagePercent = 100;
        bw1.usageType = MMPM_BW_USAGE_LPASS_DSP;

        bw2.masterPort = MMPM_BW_PORT_ID_ADSP_MASTER;
        bw2.slavePort = MMPM_BW_PORT_ID_DDR_SLAVE;
        bw2.bwVal = 100000000;
        bw2.usagePercent = 100;
        bw2.usageType = MMPM_BW_USAGE_LPASS_DMA;
 ...
 * @brief Bandwidth Req-Rel, 2 clients, different bus route, multiple times
 */
static MMPM_STATUS Test_Stress_20_Bandwidth(void)
{
    MMPM_STATUS sts = MMPM_STATUS_FAILED;
    ADSPPMTEST_LOG_TEST_HEADER(STRESS_20_NAME, STRESS_20_DETAILS);

    sts = TestStressMultiClientMultiBwReqRel_20();

    ADSPPMTEST_LOG_TEST_RESULT(AdsppmTestId_Stress, STRESS_20_NAME, sts);
    return sts;
}


#define STRESS_21_NAME    "stress_21"
#define STRESS_21_DETAILS "Bandwidth Req-Rel, single client, multiple times"

static MMPM_STATUS TestStressSingleClientMultiBwReqRel_21(void)
{
    MMPM_STATUS sts = MMPM_STATUS_FAILED, retSts = MMPM_STATUS_SUCCESS;
    uint32 clientId = 0, loop;
    MmpmCoreIdType coreId = MMPM_CORE_ID_LPASS_ADSP;
    ...
    clientId = Test_Register(coreId, instanceId, STRESS_21_NAME);
 ...
 * @brief Bandwidth Req-Rel, single client, multiple times
 */
static MMPM_STATUS Test_Stress_21_Bandwidth(void)
{
    MMPM_STATUS sts = MMPM_STATUS_FAILED;
    ADSPPMTEST_LOG_TEST_HEADER(STRESS_21_NAME, STRESS_21_DETAILS);

    sts = TestStressSingleClientMultiBwReqRel_21();

    ADSPPMTEST_LOG_TEST_RESULT(AdsppmTestId_Stress, STRESS_21_NAME, sts);
    return sts;
}
...
void Test_Stress(void)
{
    ...
    Test_Stress_20_Bandwidth();

    Test_Stress_21_Bandwidth();
