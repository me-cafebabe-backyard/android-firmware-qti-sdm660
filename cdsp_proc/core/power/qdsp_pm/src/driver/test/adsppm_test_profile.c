...
#define PROFILE_10_NAME    "profile_10"
#define PROFILE_10_DETAILS "Profile BW request duration"

static MMPM_STATUS TestProfileBwReq_10(void)
{
    MMPM_STATUS sts = MMPM_STATUS_FAILED, retSts = MMPM_STATUS_SUCCESS;
    uint32 clientId = 0;
    MmpmCoreIdType coreId = MMPM_CORE_ID_LPASS_ADSP;
    ...
    clientId = Test_Register(coreId, instanceId, PROFILE_10_NAME);
    ...
    if(clientId)
    {
        ...
        // AHB transaction within LPASS core
        bandWidthArray.busRoute.masterPort = MMPM_BW_PORT_ID_ADSP_MASTER;
        bandWidthArray.busRoute.slavePort = MMPM_BW_PORT_ID_SRAM_SLAVE;
        bandWidthArray.bwValue.busBwValue.usagePercentage = 100;
        bandWidthArray.bwValue.busBwValue.usageType = MMPM_BW_USAGE_LPASS_DMA;
 ...
 * @brief Profile BW request duration
 */
static MMPM_STATUS Test_Profile_10(void)
{
    MMPM_STATUS sts = MMPM_STATUS_FAILED;
    ADSPPMTEST_LOG_TEST_HEADER(PROFILE_10_NAME, PROFILE_10_DETAILS);

    sts = TestProfileBwReq_10();

    ADSPPMTEST_LOG_TEST_RESULT(AdsppmTestId_Profile, PROFILE_10_NAME, sts);
    return sts;
}
