    ...
    for(coreId = MMPM_CORE_ID_LPASS_ADSP; coreId < MMPM_CORE_ID_LPASS_END; coreId++)
    {
        for(instanceId = MMPM_CORE_INSTANCE_0; instanceId < MMPM_CORE_INSTANCE_MAX; instanceId++)
        {
            if(AsicInfo_isCoreInstanceAvailable(coreId, instanceId))
            {
                ...
                if(clientId)
                {
                    for(master = MMPM_BW_PORT_ID_ADSP_MASTER; master < MMPM_BW_PORT_ID_DML_SLAVE; master++)
                    {
                        if(AsicInfo_isBusPortAvailable(master))
                        {
                            for(slave = MMPM_BW_PORT_ID_DML_SLAVE; slave < MMPM_BW_PORT_ID_CORE; slave++)
                            {
                                if(AsicInfo_isBusPortAvailable(slave))
                                {
                                    ...
                                    if(validRoute)
                                    {
                                        for(usage = MMPM_BW_USAGE_LPASS_DSP; usage < MMPM_BW_USAGE_LPASS_ENUM_MAX; usage++)
                                        {
                                            ...
                                            sts = Test_RequestBandwidth(clientId, &bwReq, 1);
                    ...
                    for(master = MMPM_BW_PORT_ID_ADSP_MASTER; master < MMPM_BW_PORT_ID_DML_SLAVE; master++)
                    {
                        if(AsicInfo_isBusPortAvailable(master))
                        {
                            for(usage = MMPM_BW_USAGE_LPASS_DSP; usage < MMPM_BW_USAGE_LPASS_ENUM_MAX; usage++)
                            {
                                ...
                                sts = Test_RequestBandwidth(clientId, &bw, 1);
                                if(MMPM_STATUS_SUCCESS != sts)
                                {
                                    retSts = MMPM_STATUS_FAILED;
                                }
...
static MMPM_STATUS TestBandwidthCl1ReqCl2ReqRel(void)
{
    ...
    if(clientId1 && clientId2)
    {
        ...
        sts = Test_RequestBandwidth(clientId1, &bw, 1);
        if(MMPM_STATUS_SUCCESS != sts)
        {
            retSts = MMPM_STATUS_FAILED;
        }
        ...
        sts = Test_RequestBandwidth(clientId2, &bw, 1);
        if(MMPM_STATUS_SUCCESS != sts)
        {
            retSts = MMPM_STATUS_FAILED;
        }
        ...
        for(instanceId = MMPM_CORE_INSTANCE_0; instanceId < MMPM_CORE_INSTANCE_MAX; instanceId++)
        {
            if(AsicInfo_isCoreInstanceAvailable(coreId, instanceId))
            {
                ...
                if(clientId[numClient])
                {
                    ...
                    sts = Test_RequestBandwidth(clientId[numClient], &bw[0], numBw);
                    if(MMPM_STATUS_SUCCESS != sts)
                    {
                        retSts = MMPM_STATUS_FAILED;
                    }
...
#define BANDWIDTH_21_NAME    "bandwidth_21"
#define BANDWIDTH_21_DETAILS "Client 1 Req, client 2 Req-Rel; client 2 Rel should not affect client 1 Req"

static MMPM_STATUS TestBandwidthCl1ReqCl2ReqRel_21(void)
{
    MMPM_STATUS sts = MMPM_STATUS_FAILED, retSts = MMPM_STATUS_SUCCESS;
    BwReqTestType bw;
    uint32 clientId1 = 0, clientId2 = 0, setFreq, readFreq;
    ...
    clientId1 = Test_Register(coreId, instanceId, BANDWIDTH_21_NAME"_1");
    clientId2 = Test_Register(coreId, instanceId, BANDWIDTH_21_NAME"_2");

    if(clientId1 && clientId2)
    {
        bw.masterPort = MMPM_BW_PORT_ID_ADSP_MASTER;
        bw.slavePort = MMPM_BW_PORT_ID_SRAM_SLAVE;
        bw.bwVal = bwVal;
        bw.usagePercent = 100;
        bw.usageType = MMPM_BW_USAGE_LPASS_DMA;
 ...
 * @brief Client 1 Req, client 2 Req-Rel; client 2 Rel should not affect client 1 Req
 */
static MMPM_STATUS Test_Bandwidth_21(void)
{
    MMPM_STATUS sts = MMPM_STATUS_FAILED;
    ADSPPMTEST_LOG_TEST_HEADER(BANDWIDTH_21_NAME, BANDWIDTH_21_DETAILS);

    sts = TestBandwidthCl1ReqCl2ReqRel_21();

    ADSPPMTEST_LOG_TEST_RESULT(AdsppmTestId_Bandwidth, BANDWIDTH_21_NAME, sts);
    return sts;
}


#define BANDWIDTH_22_NAME    "bandwidth_22"
#define BANDWIDTH_22_DETAILS "2 clients Req-Rel"

static MMPM_STATUS TestBandwidthCl1ReqRelCl2ReqRel_22(void)
{
    MMPM_STATUS sts = MMPM_STATUS_FAILED, retSts = MMPM_STATUS_SUCCESS;
    BwReqTestType bw;
    uint32 clientId1 = 0, clientId2 = 0, setFreq, readFreq;
    ...
    clientId1 = Test_Register(coreId, instanceId, BANDWIDTH_22_NAME"_1");
    clientId2 = Test_Register(coreId, instanceId, BANDWIDTH_22_NAME"_2");
 ...
 * @brief 2 clients Req-Rel
 */
static MMPM_STATUS Test_Bandwidth_22(void)
{
    MMPM_STATUS sts = MMPM_STATUS_FAILED;
    ADSPPMTEST_LOG_TEST_HEADER(BANDWIDTH_22_NAME, BANDWIDTH_22_DETAILS);

    sts = TestBandwidthCl1ReqRelCl2ReqRel_22();

    ADSPPMTEST_LOG_TEST_RESULT(AdsppmTestId_Bandwidth, BANDWIDTH_22_NAME, sts);
    return sts;
}


#define BANDWIDTH_23_NAME    "bandwidth_23"
#define BANDWIDTH_23_DETAILS "Multiple clients Req-Rel"

static MMPM_STATUS TestBandwidthMultiClientReqRel_23(void)
{
    MMPM_STATUS sts = MMPM_STATUS_FAILED, retSts = MMPM_STATUS_SUCCESS;
    uint32 numBw = 0, numClient = 0, setFreq, readFreq, i;
    BwReqTestType bw[MAX_BW_REQUEST] = {};
    ...
    for(coreId = MMPM_CORE_ID_LPASS_ADSP; coreId < MMPM_CORE_ID_LPASS_END; coreId++)
    {
        for(instanceId = MMPM_CORE_INSTANCE_0; instanceId < MMPM_CORE_INSTANCE_MAX; instanceId++)
        {
            if(AsicInfo_isCoreInstanceAvailable(coreId, instanceId))
            {
                clientId[numClient] = Test_Register(coreId, instanceId, BANDWIDTH_23_NAME);
                if(clientId[numClient])
                {
                    GetReqBandwidthParam(coreId, bw, &numBw);
 ...
 * @brief Multiple clients Req-Rel
 */
MMPM_STATUS Test_Bandwidth_23(void)
{
    MMPM_STATUS sts = MMPM_STATUS_FAILED;
    ADSPPMTEST_LOG_TEST_HEADER(BANDWIDTH_23_NAME, BANDWIDTH_23_DETAILS);

    sts = TestBandwidthMultiClientReqRel_23();

    ADSPPMTEST_LOG_TEST_RESULT(AdsppmTestId_Bandwidth, BANDWIDTH_23_NAME, sts);
    return sts;
}


#define BANDWIDTH_24_NAME    "bandwidth_24"
#define BANDWIDTH_24_DETAILS "Multiple same client Reqs are updated"

static MMPM_STATUS TestBandwidthSameClientMultiReq_24(void)
{
    MMPM_STATUS sts = MMPM_STATUS_FAILED, retSts = MMPM_STATUS_SUCCESS;
    uint32 clientId = 0, setFreq, readFreq, i;
    MmpmCoreIdType coreId = MMPM_CORE_ID_LPASS_ADSP;
    ...
    clientId = Test_Register(coreId, instanceId, BANDWIDTH_24_NAME);
 ...
 * @brief Multiple same client Reqs are updated
 */
static MMPM_STATUS Test_Bandwidth_24(void)
{
    MMPM_STATUS sts = MMPM_STATUS_FAILED;
    ADSPPMTEST_LOG_TEST_HEADER(BANDWIDTH_24_NAME, BANDWIDTH_24_DETAILS);

    sts = TestBandwidthSameClientMultiReq_24();

    ADSPPMTEST_LOG_TEST_RESULT(AdsppmTestId_Bandwidth, BANDWIDTH_24_NAME, sts);
    return sts;
}


#define BANDWIDTH_25_NAME    "bandwidth_25"
#define BANDWIDTH_25_DETAILS "Invalid client ID Req, expect failure Req"

static MMPM_STATUS TestBandwidthInvalidClientId_25(void)
{
    MMPM_STATUS sts = MMPM_STATUS_FAILED, retSts = MMPM_STATUS_SUCCESS;
    uint32 clientId = 0;
    MmpmCoreIdType coreId = MMPM_CORE_ID_LPASS_ADSP;
    ...
    clientId = Test_Register(coreId, instanceId, BANDWIDTH_25_NAME);
 ...
 * @brief Invalid client ID Req, expect failure Req
 */
static MMPM_STATUS Test_Bandwidth_25(void)
{
    MMPM_STATUS sts = MMPM_STATUS_FAILED;
    ADSPPMTEST_LOG_TEST_HEADER(BANDWIDTH_25_NAME, BANDWIDTH_25_DETAILS);

    sts = TestBandwidthInvalidClientId_25();

    ADSPPMTEST_LOG_TEST_RESULT(AdsppmTestId_Bandwidth, BANDWIDTH_25_NAME, sts);
    return sts;
}


#define BANDWIDTH_26_NAME    "bandwidth_26"
#define BANDWIDTH_26_DETAILS "Invalid parameters Req, expect failure Req"

static MMPM_STATUS TestBandwidthInvalidBwParam_26(void)
{
    MMPM_STATUS sts = MMPM_STATUS_FAILED, retSts = MMPM_STATUS_SUCCESS;
    BwReqTestType bw;
    uint32 clientId = 0;
    ...
    clientId = Test_Register(coreId, instanceId, BANDWIDTH_26_NAME);
 ...
 * @brief Invalid parameters Req, expect failure Req
 */
static MMPM_STATUS Test_Bandwidth_26(void)
{
    MMPM_STATUS sts = MMPM_STATUS_FAILED;
    ADSPPMTEST_LOG_TEST_HEADER(BANDWIDTH_12_NAME, BANDWIDTH_26_DETAILS);

    sts = TestBandwidthInvalidBwParam_26();
...
#define BANDWIDTH_27_NAME    "bandwidth_27"
#define BANDWIDTH_27_DETAILS "LPASS AHB clock frequency plan"

static MMPM_STATUS TestBandwidthLpassAhbFreqPlan_27(void)
{
    MMPM_STATUS sts = MMPM_STATUS_FAILED, retSts = MMPM_STATUS_SUCCESS;
    uint32 clientId = 0, setFreq, readFreq, i;
    const AsicInfo_ClockFreqVoltPlanType *pFreqPlan = NULL;
    ...
    clientId = Test_Register(coreId, instanceId, BANDWIDTH_27_NAME);
 ...
 * @brief LPASS AHB clock frequency plan
 */
static MMPM_STATUS Test_Bandwidth_27(void)
{
    MMPM_STATUS sts = MMPM_STATUS_FAILED;
    ADSPPMTEST_LOG_TEST_HEADER(BANDWIDTH_27_NAME, BANDWIDTH_27_DETAILS);

    sts = TestBandwidthLpassAhbFreqPlan_27();

    ADSPPMTEST_LOG_TEST_RESULT(AdsppmTestId_Bandwidth, BANDWIDTH_27_NAME, sts);
    return sts;
}


#define BANDWIDTH_28_NAME    "bandwidth_28"
#define BANDWIDTH_28_DETAILS "Zero BW request"

static MMPM_STATUS TestBandwidthZeroBwReq_28(void)
{
    MMPM_STATUS sts = MMPM_STATUS_FAILED, retSts = MMPM_STATUS_SUCCESS;
    uint32 clientId = 0, setFreq, readFreq;
    const AsicInfo_ClockFreqVoltPlanType *pFreqPlan = NULL;
    ...
    clientId = Test_Register(coreId, instanceId, BANDWIDTH_28_NAME);
 ...
 * @brief Zero BW request
 */
static MMPM_STATUS Test_Bandwidth_28(void)
{
    MMPM_STATUS sts = MMPM_STATUS_FAILED;
    ADSPPMTEST_LOG_TEST_HEADER(BANDWIDTH_28_NAME, BANDWIDTH_28_DETAILS);

    sts = TestBandwidthZeroBwReq_28();

    ADSPPMTEST_LOG_TEST_RESULT(AdsppmTestId_Bandwidth, BANDWIDTH_28_NAME, sts);
    return sts;
}
