...
/**
  ...
  DCVS NPA node definitions for MSM8996 SSC clock driver.
*/
/*
  ====================================================================
  ...
  ==================================================================== 
  $Header: //components/rel/core.qdsp6/1.0.c2/systemdrivers/clock/hw/msm8998/adsp/src/ClockSSCDCVS.c#1 $
...
/*
 * NPA remote resources used on SSC
 */
...
)
{
  ...
  if( eVRegLevel < VCS_CORNER_NUM_OF_CORNERS )
  {
     ...
     */
    nWarmUpTime = 
      CLOCK_GET_WARMUP_DELTA(Clock_VRegWarmupTable[eVRegLevel].nVoltageUS,
                             Clock_VRegWarmupTable[VCS_CORNER_RETENTION].nVoltageUS);
    
    HWIO_OUTF(SSC_SSCAON_SWITCHER_DLY, SSC_SSCAON_SWITCHER_DLY, nWarmUpTime);
...
)
{
  ...
  if (!(pImageCtxt->CPUCtxt.nDisableDCS & CLOCK_FLAG_DISABLED_HW_LOGIC))
  {
    ...
    {
      HWIO_OUT(SSC_QDSP6SS_STRAP_ACC, pNewConfig->nStrapACCVal);
    }
    ...
    /*
     * Need to set the PLL override so that the SPM does not try to freeze
     * a PLL and wait for a lock when unfreezing if the PLL was already in
     * a freeze state prior to operation.  This would result in an infinite
     * loop waiting for a PLL lock.  We must ensure SPM freeze is sent 
     * to the proper PLL that is sourcing the Q6.
     */
    if(Clock_nHWIOBaseSSC != NULL)
    {
      if (pNewConfig->Mux.HALConfig.eSource == HAL_CLK_SOURCE_SCCPLL0)
      {
        HWIO_OUTF(SSC_QDSP6SS_SPM_EVENT_CTL, PLL_OVRRD, 0x1);
        HWIO_OUTF(SSC_QDSP6SS_SPM_EVENT_CTL, EXT_PLL_OVRRD, 0x0);
      }
      else
      {
        HWIO_OUTF(SSC_QDSP6SS_SPM_EVENT_CTL, PLL_OVRRD, 0x0);
        HWIO_OUTF(SSC_QDSP6SS_SPM_EVENT_CTL, EXT_PLL_OVRRD, 0x1);
      }
    }
        ...
        pPreviousConfig->Mux.HALConfig.eSource)
    {
      if (pNewConfig->Mux.nFreqHz < pPreviousConfig->Mux.nFreqHz)
      {
        /*  Program the CDIV to the GFMUX. */
        HAL_clk_ConfigClockDivider(pClock->HALHandle, (pNewConfig->Mux.HALConfig.nDiv2x/2)-1);
      }
      ...
      /*-----------------------------------------------------------------------*/
      /* We need to manually configure the source here because the source      */
      /* frequency configuration for SVS2 and SVS Fmax is the same.  This      */
      /* avoids a potential issue switching from SVS frequency to Turbo.       */
      /*-----------------------------------------------------------------------*/
    ...
    {
      if ((pNewConfig->Mux.HALConfig.eSource == pPreviousConfig->Mux.HALConfig.eSource) && 
          (pNewConfig->Mux.HALConfig.eSource == HAL_CLK_SOURCE_SCCPLL0))
      {
        /*  Program the CDIV to the GFMUX. */
        HAL_clk_ConfigClockDivider(pClock->HALHandle, (pNewConfig->Mux.HALConfig.nDiv2x/2)-1);
      }
...
DALResult Clock_TestCPU(ClockDrvCtxt* pDrvCtxt)
{
  if (DAL_SUCCESS == Clock_SetCPUFrequency(pDrvCtxt, 100000000, FALSE))
  {
    if (DAL_SUCCESS == Clock_SetCPUFrequency(pDrvCtxt, 500000000, FALSE))
    {
      if (DAL_SUCCESS == Clock_SetCPUFrequency(pDrvCtxt, 110000000, FALSE))
      {
        if (DAL_SUCCESS == Clock_SetCPUFrequency(pDrvCtxt, 100000000, FALSE))
        {
          if (DAL_SUCCESS == Clock_SetCPUFrequency(pDrvCtxt, 200000000, FALSE))
          {
            if (DAL_SUCCESS == Clock_SetCPUFrequency(pDrvCtxt, 700000000, FALSE))
            {
              return Clock_SetCPUFrequency(pDrvCtxt, 200000000, FALSE);
            }
            else {
              return DAL_ERROR;
            }
