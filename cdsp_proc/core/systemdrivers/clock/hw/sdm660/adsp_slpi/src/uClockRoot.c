...
/*
  ...
  $Header: //components/rel/core.qdsp6/1.0.c2/systemdrivers/clock/hw/sdm660/adsp_slpi/src/uClockRoot.c#1 $
...
/*
 * Use value 2 for dual edge.
 */
#define CLOCK_SCC_RCG_MODE_VAL   0x2

/*
 * SCC_PLL input is SRC_SEL 0x2 for 100MHz domain.
 */
#define CLOCK_SCC_PLL_OUT_VAL    0x5
  ...
  if (eClockId < CLOCK_TOTAL_CLOCK_ENUMS)
  {
    ...
    if (uClockUARTFreqConfig[nCfgIdx].nFreqHz != 0)
    {
      if (gnClockRegs[eClockId].pRCG != NULL)
      {
        ...
        nNewCfgVal |=
          (CLOCK_SCC_PLL_OUT_VAL << HWIO_LPASS_SCC_UART1_CFG_RCGR_SRC_SEL_SHFT);

        nNewCfgVal |=
          (CLOCK_SCC_RCG_MODE_VAL << HWIO_LPASS_SCC_UART1_CFG_RCGR_MODE_SHFT);
