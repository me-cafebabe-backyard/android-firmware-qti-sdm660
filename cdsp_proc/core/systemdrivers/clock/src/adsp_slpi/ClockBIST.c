...
/*
  ...
  $Header: //components/rel/core.qdsp6/1.0.c2/systemdrivers/clock/src/adsp_slpi/ClockBIST.c#1 $
...
)
{
  ...
  eResult = Clock_EnableClock (Clock_GetInitialClientCtxt(), ClockId);
  ...
  eResult = Clock_DisableClock (Clock_GetInitialClientCtxt(), ClockId);
...
)
{
  ...
  else
  {
    if (CLOCK_FLAG_IS_SET(
         (ClockNodeType*)ClockId, BIST_FAILURE_ANTICIPATED))
    {
      ULOG_RT_PRINTF_1 (pDrvCtxt->hClockBISTLog,
                        "   Clock %s BIST expectedly failed!",
                        ((ClockNodeType*)ClockId)->szName);
    }
    else
    {
