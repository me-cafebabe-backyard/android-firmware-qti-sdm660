...
$Header: //components/rel/core.qdsp6/1.0.c2/systemdrivers/GPIOInt/src/kernel/GPIOInt_DirConn.c#1 $
                                        ...
                                        uint8 uImage )
{
  ...
  /*
   * If uimage is present , regardless of the interrupt being used in DDR the uimage driver
   * still needs to be able to trigger it for sleep driver.
   */
  if(uImage != 0)
  {
    uGPIOInt_SetDirectConnectGPIOMapping(gpio,interrupt_id);
  }
