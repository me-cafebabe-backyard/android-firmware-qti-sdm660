...
$Header: //components/rel/core.qdsp6/1.0.c2/systemdrivers/GPIOInt/src/kernel/uimage/uGPIOIntKernelIsland.c#1 $
...
//  uint32 nReg;
  ...
  /* No summary interrupt support in uImage. */

    /* If the GPIO is a summary interrupt we need to just write to the status register to force trigger it */
//    nReg = 
//      UGPIOINT_GET_ADDRESS(HWIO_TLMM_GPIO_INTR_STATUSn_ADDR(0),gpio);
//    outpdw(nReg,0x1);
//  }
//
  qurt_mutex_unlock(&uGPIOIntKernelData.uGPIOIntKernelLock); 
