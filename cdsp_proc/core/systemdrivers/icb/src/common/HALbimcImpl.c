...
$Header: //components/rel/core.qdsp6/1.0.c2/systemdrivers/icb/src/common/HALbimcImpl.c#1 $

when         who     what, where, why
----------   ---     --------------------------------------------------------
2015/02/03   sds     Add device parameter for number of decode segments.
2015/01/15   sds     Disable segments before reprogramming them
2014/12/23   sds     Properly scale BKE thresholds when necessary
2014/12/04   sds     Add support for BIMC 3.2.1 BKE scaling factor
2014/11/07   sds     Implement fixed mode via BKE, rather than overrides.
...
#define BIMC_HW_REV_BGE(major,minor,step) \
   (pInfo->devParams.uHWRevision >= \
    (uint32_t)(BIMC_OUTFC((major),MISC_HW_VERSION,MAJOR) | \
              BIMC_OUTFC((minor),MISC_HW_VERSION,MINOR) | \
              BIMC_OUTFC((step), MISC_HW_VERSION,STEP)))

#define BIMC_VERIFY_SEGMENT_IDX(info, idx) if(idx >= (info)->devParams.uNumSegments) return
...
)
{
    ...
    */
    ...
    uint32_t uGrantCount = (uint32_t) CEILING_DIV(pBandwidth->uBandwidth * uGrantPeriod,
                                                  pInfo->uQosFreq * 1000);

    /* If we exceed the maximum grant count without scaling:
     *  If the hardware supports scaling, enable it, and ceiling to the max scaled value.
     *  If not, ceiling to max unscaled value. */
    if( uGrantCount > MAX_GRANT_COUNT )
    {
      if( BIMC_HW_REV_BGE(3,2,1) )
      {
        /* Scaling supported.
         * Shift by the scaling value and make sure we still fit in the register. */
        pBKEParams->uScalingFactor = 1;
        uGrantCount = MIN(MAX_GRANT_COUNT, uGrantCount >> 4);
      }
      else
      {
        /* Scaling not supported. Ceiling to maximum unscaled grant count. */
        pBKEParams->uScalingFactor = 0;
        uGrantCount = MAX_GRANT_COUNT;
      }
    }
    /* No scaling required. */
    else
    ...
    if( 1 == pBKEParams->uScalingFactor )
    {
      uThresholdHighOffs   >>= 4;
      uThresholdMediumOffs >>= 4;
      uThresholdLowOffs    >>= 4;
    }
...
** ----------------------------------------------------------------------- */
/**
@copydoc HAL_bimc_EnableSlaveSegment
*/
void bimc_EnableSlaveSegment
(
  HAL_bimc_InfoType *pInfo,
  uint32_t           uSlaveIdx,
...
{
  uint32_t enable = true==bEnable?1:0;

  /* Verify the segment is in range for this core. */
  BIMC_VERIFY_SEGMENT_IDX(pInfo, uSegmentIdx);

  /* Change the segment enable to the requested value. */
  BIMC_OUTFI2(pInfo->pBaseAddr, SEGMENTn_ADDR_BASEm_LOWER, uSlaveIdx, uSegmentIdx, ENABLE, enable);
}
...
)
{
  ...
  /* Preemptively disable the segment, since we can't do one-shot updates. */
  BIMC_OUTFI2(pInfo->pBaseAddr, SEGMENTn_ADDR_BASEm_LOWER, uSlaveIdx, uSegmentIdx, ENABLE, 0);
...
)
{
  switch(eMode)
  {
    case BIMC_QOS_MODE_FIXED:
    {
      HAL_bimc_QosBandwidthType bandwidth = { 0, 0, 0, 0, 0 };
      uint32_t uHealthVal = BKE_HEALTH_VAL( 0, pPriority->fixed.uAReqPriorityRead, pPriority->fixed.uPriorityLvl );

      /* Set all health levels to the same AREQ and PL */
      BIMC_OUTMI( pInfo->pBaseAddr, M_BKE_HEALTH_3_CONFIG,  uMasterIdx, BKE_HEALTH_MASK, uHealthVal );
      BIMC_OUTMI( pInfo->pBaseAddr, M_BKE_HEALTH_2_CONFIG,  uMasterIdx, BKE_HEALTH_MASK, uHealthVal );
      BIMC_OUTMI( pInfo->pBaseAddr, M_BKE_HEALTH_1_CONFIG,  uMasterIdx, BKE_HEALTH_MASK, uHealthVal );
      BIMC_OUTMI( pInfo->pBaseAddr, M_BKE_HEALTH_0_CONFIG,  uMasterIdx, BKE_HEALTH_MASK, uHealthVal );
      
      /* Set a bandwidth and window of zero. We want the registers cleared. */
      bimc_SetQosBandwidth( pInfo, uMasterIdx, &bandwidth );
      break;
    }
    case BIMC_QOS_MODE_REGULATOR:
    case BIMC_QOS_MODE_LIMITER:
...
)
{
  ...
  if( pBandwidth->uBandwidth && pBandwidth->uWindowSize )
  {
    ...
    if( BIMC_HW_REV_BGE(3,2,1) )
    {
      BIMC_OUTFI( pInfo->pBaseAddr, M_BKE_ENABLE, uMasterIdx, SCALING_FACTOR, bkeParams.uScalingFactor );
    }
  ...
  else
  {
    ...
    if( BIMC_HW_REV_BGE(3,2,1) )
    {
      BIMC_OUTFI( pInfo->pBaseAddr, M_BKE_ENABLE, uMasterIdx, SCALING_FACTOR, 0 );
    }
...
)
{
  ...
  if( pBandwidth->uBandwidth && pBandwidth->uWindowSize )
  {
    ...
    if( BIMC_HW_REV_BGE(3,2,1) )
    {
      BIMC_OUTFI2( pInfo->pBaseAddr, M_BKEn_ENABLE, uMasterIdx, uHwContext, SCALING_FACTOR, bkeParams.uScalingFactor );
    }
  ...
  else
  {
    ...
    if( BIMC_HW_REV_BGE(3,2,1) )
    {
      BIMC_OUTFI2( pInfo->pBaseAddr, M_BKEn_ENABLE, uMasterIdx, uHwContext, SCALING_FACTOR, 0 );
    }
