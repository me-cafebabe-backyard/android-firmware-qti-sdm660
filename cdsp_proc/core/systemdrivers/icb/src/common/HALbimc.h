...
$Header: //components/rel/core.qdsp6/1.0.c2/systemdrivers/icb/src/common/HALbimc.h#1 $
...
typedef struct
{
  ...
  uint32_t uNumSegments; /**< Number of segments for address decode. */
...
/**
@brief Enable or disable a slave segment without modifying other settings.

@param[in] pInfo -       A pointer to the BIMC info structure
@param[in] uSlaveIdx -   The slave index
@param[in] uSegmentIdx - The segment index
@param[in] bEnable -     True to enable the segment, false to disable

@pre HAL_bimc_Init() must have been called.

@sa HAL_bimc_Init()
*/
void HAL_bimc_EnableSlaveSegment( HAL_bimc_InfoType *pInfo,
                                  uint32_t           uSlaveIdx,
