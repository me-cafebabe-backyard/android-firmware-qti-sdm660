...
$Header: //components/rel/core.qdsp6/1.0.c2/systemdrivers/hal/tlmmtile/src/HALtlmmtile.c#1 $
...
uint32 HAL_tlmmtile_SetDirConn ( uint32 nDirConn, HAL_tlmmtile_FieldType nField, uint32 nVal )
{
  ...
  for ( nTile = 0; nTile < gHalTlmm.nNumTiles; nTile++ )
  {
    HALTLMMTILE_OUT( gHalTlmm.DirConnIntrCfg.nBase[nTile],
                     Args.nOffset,
                     Args.nShift,
                     Args.nBmsk,
                     nVal );
...
uint32 HAL_tlmmtile_GetDirConn ( uint32 nDirConn, HAL_tlmmtile_FieldType nField )
{
  ...
  return HALTLMMTILE_IN( gHalTlmm.DirConnIntrCfg.nBase[0],
                         Args.nOffset,
                         Args.nShift,
                         Args.nBmsk );
