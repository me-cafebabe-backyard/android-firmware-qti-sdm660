...
$Header: //components/rel/core.qdsp6/1.0.c2/systemdrivers/hal/clk/hw/v1/HALclkSparkPLL.c#1 $
...
)
{
  ...
  /*
   * Wait for the PLL to go active during the 200 us timeout period.
   */
  bResult =
...
)
{
  ...
  /*
   * Wait for the PLL to lock during the 200 us timeout period.
   */
  bResult =
    HAL_clk_ValidateRegSetTimeout(
      HAL_CLK_PLL_HWIO_ADDR(PLL_MODE, pCtxt),
...
)
{
  ...
  /*
   * Wait for PLL_UPDATE to clear during the 200 us timeout period.
   */
  bResult =
...
)
{
  ...
  /*
   * Program the post divider.
   */
  nUserVal = HAL_CLK_PLL_HWIO_IN(PLL_USER_CTL, pCtxt);
  nUserVal &= ~HWIO_PLL_USER_CTL_POST_DIV_RATIO_BMSK;
  nPostDiv = pConfig->nPostDiv;
  ...
  nUserVal |=
    (nPostDiv << HWIO_SHFT(PLL_USER_CTL, POST_DIV_RATIO));
  ...
  /*
   * Wait for PLL_UPDATE to be cleared during the 200 us timeout period.
   */
  bResult =
    HAL_clk_ValidateRegClearTimeout(
        HAL_CLK_PLL_HWIO_ADDR(PLL_MODE, pCtxt),
