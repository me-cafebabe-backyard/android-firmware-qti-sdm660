...
$Header: //components/rel/core.qdsp6/1.0.c2/systemdrivers/hal/clk/hw/sdm660/adsp_slpi/src/HALclkTest.c#1 $
...
)
{
  ...
  *pnFreqHz = 0;
  ...
  switch (nTestClockType)
  {
    ...
    case HAL_CLK_LPASS_Q6_TEST:
      HWIO_OUTF(LPASS_QDSP6SS_TEST_CLK_CTL, DBG_CLK_SEL, nTestClockSel);
      HWIO_OUTF(LPASS_QDSP6SS_TEST_CLK_CTL, Q6_DBG_CLK_EN, 1);
      HWIO_OUT(LPASS_DBG_CLK, HAL_CLK_LPASS_TEST_Q6SS_CLK_SRC & HAL_CLK_TEST_SEL_MASK );
      HWIO_OUTF(LPASS_DBG_CLK, DBG_CLK_ENABLE, 0x1);
      HWIO_OUTF(GCC_GCC_DEBUG_CLK_CTL, MUX_SEL, 
                HAL_CLK_GCC_TEST_LPASS_GCC_DBG_CLK & HAL_CLK_TEST_SEL_MASK); 
      break;
