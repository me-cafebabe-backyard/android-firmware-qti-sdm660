...
#                                                                              #
# $Header: //components/rel/dspcore.adsp/2.9/scripts/crashman/memheap2_heapwalkparser.pl#4 $
# $Author: pwbldsvc $
...
sub processHeapTable {
   ...
   my ($blockno, $address,$caller_ptr,$caller_symbol,$tcb_ptr,$tcb_symbol, $allocated_bytes,$extra_bytes,$freeflag); 
   my ($lastflag,$padbyte);  
...
#   1    0x01F48A30           1127       4304    0x0  0x00   
      ...
      if ($debugCheck == 1) {
        ...
        $tcb_ptr = $values[$index]; $index++;
        $tcb_symbol = $values[$index]; $index++;
      }
		...
		if($freeflag ne "0x1")
        {
            ...
            if ($debugCheck == 1) {
               ...
               $usedHeapSummaryHeapWalk{$TotalNoOfBlocksUsed}{TCB_PTR} = $tcb_ptr; 
               $usedHeapSummaryHeapWalk{$TotalNoOfBlocksUsed}{TCB_SYMBOL} = $tcb_symbol; 
            }
...
sub printBlockSummary {
   ...
   if ($debugCheck == 1){
      print $OUTPUTFILEHANDLE "S.NO, BlockNumber, Caller Pointer, Caller Symbol, TCB Pointer, TCB Symbol, AllocatedBytes, ExtraBytes\n";
   }else{
   ...
   foreach  (sort {$a <=> $b} keys %usedHeapSummaryHeapWalk) {
       ...
       if ($debugCheck == 1){
          ...
          $tcb_ptr = $usedHeapSummaryHeapWalk{$_}{TCB_PTR};
          $tcb_symbol = $usedHeapSummaryHeapWalk{$_}{TCB_SYMBOL};
       }
       $AlctdBytes=$usedHeapSummaryHeapWalk{$_}{ALLOCBYTES};
       $ExtraBytes=$usedHeapSummaryHeapWalk{$_}{EXTRABYTES};
       if ($debugCheck == 1){
           print $OUTPUTFILEHANDLE "$sno, $startblk,$debugInfo,$debugInfoSymbol,$tcb_ptr,$tcb_symbol,$AlctdBytes, $ExtraBytes\n";
       }else{
...
sub processUsedFragmentsTcb {
   ...
   foreach  (sort keys %usedHeapSum) {
      ...
      $TotalAllocatedBytesMemDbg += $usedHeapSum{$_}{ALLOCBYTES};
	   $hashCostSummarytcb{$keyCost}{TCB_SYMBOL}=$usedHeapSum{$_}{TCB_SYMBOL};
   }
}
...
sub processMemDebugTable {
   ...
   while(<MEMDBG>)
   {
	  ...
	  if(scalar @values eq 6)
	  {
		...
		$usedHeapSummaryMemDbg{$TotalNoOfBlocksUsedMemDbg}{TCB_SYMBOL} = $tcb_symbol;
...
sub printCostSummaryTcb {
   ...
   print $OUTPUTFILEHANDLE "TCB Pointer, TCB Symbol, Num of Allocations, Max Allocated,Min Allocated, Total Allocated, Percent Allocated\n";
   $~ = SummaryFormat;
   foreach (sort { $hashCostSummarytcb{$b}{ALLOCBYTES} <=> $hashCostSummarytcb{$a}{ALLOCBYTES}  } keys %hashCostSummarytcb) {
      ...
      print $OUTPUTFILEHANDLE "$tcb,$tcb_symbol, $NumAllctns,$MaxAllctd,$MinAllctd,$TotAllctd,$PercentAllocated\n";
   }
   print $OUTPUTFILEHANDLE "\n\n";
}
