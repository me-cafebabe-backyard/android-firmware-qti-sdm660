...
Crashman_Version = "External_Release_1.41"
print "************************************"
...
if etm_flag == True and (smmu_32bt!=False or smmu_64bt!=False):
    ...
    if os.path.exists(Path):
        ...
        command = '''start %s\\t32mqdsp6.exe -c %s, %s %s %s %s'''%(T32_path,OUTDumpPath+"\\temp\\config_sim_usb.t32",CurrDirectory+"\\load_adsp_etm_dump.cmm",elfname,dumpfile,targetid,cdsp_flag)
        os.system(command)
    sys.exit(0)
        
        
flag = 0
if smmu_32bt!=False and smmu_output== False:
    if not os.path.exists(ramparse_path):
        print "**************************************************************"
        print "Linux ram dump parser path is not accessible/available. Please Check the Path "
        print "Expecting aarch64-linux-gnu-gdb.exe/aarch64-linux-gnu-nm.exe in linux ram dump parser path location!!!"
...
if smmu_64bt!=False and smmu_output== False:
    if not os.path.exists(ramparse_path):
        print "**************************************************************"
        print "Linux ram dump parser path is not accessible/available. Please Check the Path "
        print "Expecting aarch64-linux-gnu-gdb.exe/aarch64-linux-gnu-nm.exe in linux ram dump parser path location!!!"
...
if smmu_64bt!=False or smmu_32bt!=False:
    if opts.LINUX_SMMU:
...
cmd = '''%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n'''%(targetid,UniqueNumber,INDumpPath,OUTDumpPath,CRM_buildpath,CurrDirectory,CustomerPath,crashman_lite,slpi_flag,asha_flag,SOFILE,load_dump_full,quit_flag,smmu_enable,frpcshell,frpcshell_name,wp_smmu_enable,INDumpPath.split('\\')[-1],Enable_6GB_dumps,cdsp_flag,smmu_64bt)
f_arg.write(cmd)
f_arg.close()
