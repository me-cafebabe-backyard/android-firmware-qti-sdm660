...
from sectools.features.isc.signer.signerutils.attributes import SigningAttributes, AttestationCertificate
...
class BaseSigner(SecPluginIntf_Abs):
    ...
    def sign(self, data_to_sign, imageinfo, debug_dir=None, is_hash=False):
        ...
        if (hmac_from_config.hmac_type == hmac_from_config.HMAC_TYPE_QTI and
    ...
    @classmethod
    def get_hash_algorithm_from_cert(cls, certificate):
        ...
        if sha_algo_field is None:
            raise RuntimeError(
                "Error in certificate subject. SHA algorithm (OU 7) field not found. Not a valid certificate. Exiting")
        else:
            sha_algo_field = sha_algo_field[0]
            logger.debug('SHA Algorithm = ' + sha_algo_field)
            if sha_algo_field in cls.OU_SHA_MAP.keys():
                return cls.OU_SHA_MAP[sha_algo_field]
            else:
                raise RuntimeError("Unknown SHA algorithm string: " + sha_algo_field)

    @classmethod
    def get_hmac_params_from_cert(cls, certificate):
    ...
    def validate_sig_using_hash(self, image_hash, signature, cert_chain_der, signed_authority=None):
        ...
        cert_text = crypto.cert.get_text(cert_chain_der[0])
        ...
        hash_algo = self.get_hash_algorithm_from_cert(cert_text)
    ...
    def validate_sig(self, to_sign, signature, cert_chain_der, signed_authority=None):
        ...
        hmac_params = self.get_hmac_params_from_cert(cert_text)
        hash_algo = self.get_hash_algorithm_from_cert(cert_text)
        image_hash = Hasher().get_hash(to_sign, hmac_params=hmac_params if not use_pss else None, sha_algo=hash_algo)

        # Validate the hash
        return self.validate_sig_using_hash(image_hash, signature, cert_chain_der, signed_authority=signed_authority)
    ...
    def validate_root_cert_hash(self, cert_chain_der, expected_root_cert_hash):
        ...
        attest_cert = AttestationCertificate(cert_chain_der[0])
        attest_cert_hash_algo = None
        if "hash_algorithm" in attest_cert.subject_dict.keys():
            attest_cert_hash_algo = attest_cert.subject_dict["hash_algorithm"]
        root_cert_hash = HMAC.HASH_ALGO_MAP[attest_cert_hash_algo](root_cert).hexdigest()
    ...
    def validate(self, image, root_cert_hash=None, imageinfo=None):
        if image.is_signed():
            ...
            for signed_authority, data_to_sign, data_signature, cert_chain in image.get_signing_assets():
                # Check if empty
                ...
                if not self.validate_sig(data_to_sign, data_signature, cert_chain_der, signed_authority=signed_authority):
            ...
            if imageinfo is not None:
                if not cert_chain_der:
                    ...
                    mismatches = self.validate_signing_attributes(cert_chain_der, imageinfo)
                    create_mismatch_table(mismatches, errstr, operation="signing", data_type_to_compare="Attribute", image_region="Attestation Cert")
            if errstr:
