...
$Header: //components/rel/core.qdsp6/1.0.c2/services/time/src/timer_qdi.c#1 $ 
...
static inline timer_error_type timer_qdi_get_first_non_deferrable_timer_expiry(int client_handle, time_timetick_type *ptick)
{
   time_timetick_type expiry_time;
   int                result;
   
   expiry_time = timer_drv_get_first_non_deferrable_timer_expiry();
                          ...
                          qurt_qdi_arg_t a9)
{
   ...
   switch(method)
   {
      ...
      case TIMER_QDI_GET_FIRST_NON_DEFERRABLE_TIMER_EXPIRY:  
         return timer_qdi_get_first_non_deferrable_timer_expiry(client_handle, a1.ptr); 		 
