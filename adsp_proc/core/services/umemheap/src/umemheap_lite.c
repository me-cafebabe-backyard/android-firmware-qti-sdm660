...
Copyright (c) 1997-2014,2017 by Qualcomm Technologies Incorporated.  All Rights Reserved.
============================================================================ */
...
$Header: //components/rel/core.qdsp6/1.0.c2/services/umemheap/src/umemheap_lite.c#1 $ $DateTime: 2020/02/10 01:57:30 $ $Author: pwbldsvc $
...
#define MEMHEAP_ASSERT( xx_exp ) \
       if( !(xx_exp) ) \
       { \
            ERR_FATAL( "Memheap Assertion", 0, 0, 0 );\
       }
...
mem_heap_enter_crit_sect( void * ptr)
{
  ...
  qurt_pimutex_lock((qurt_mutex_t*) &(((mem_heap_type *)ptr)->memheap_crit_sect));
...
mem_heap_leave_crit_sect( void * ptr)
{
  ...
  qurt_pimutex_unlock((qurt_mutex_t*) &(((mem_heap_type *)ptr)->memheap_crit_sect));
...
)
{
  ...
  qurt_pimutex_init ( (qurt_mutex_t *)&(heap_ptr->memheap_crit_sect) );
                              ...
                              a multiple free of the same block is detected */
  {
	...
	if(theBlock != heap_ptr->first_block)
    {
      ...
      if(!((temp->freeBlock_frdOff)%kMinChunkSize)){
        ...
        else if(temp->pad  == umagic_num[heap_ptr->magic_num_index]){
          validBlock = TRUE;
        }
