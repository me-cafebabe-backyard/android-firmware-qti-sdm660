...
$Header: //components/rel/core.qdsp6/1.0.c2/services/diag/DCM/common/src/diag_diagID.c#1 $
...
boolean diag_diagID_set_id(char *name, uint8 id)
{
  ...
  if(ptr != NULL)
  {
    ...
    ULogFront_RealTimePrintf(diag_err_log_handle, 1, "diag_diagID_set_id: diagID : %d", id);
    ULogFront_RealTimeString(diag_err_log_handle, name);	
