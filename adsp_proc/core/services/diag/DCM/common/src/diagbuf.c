...
Copyright (c) 1999-2018 by QUALCOMM Technologies, Incorporated.
All Rights Reserved.
...
/*===========================================================================
 ...
 $Header: //components/rel/core.qdsp6/1.0.c2/services/diag/DCM/common/src/diagbuf.c#1 $
...
extern uint32 diag_buffered_bytes(uint8 stream_index, uint8 diag_id); /*From diagcomm_smd.c */
...
============================================================================*/
void* diagbuf_alloc( uint8 code, uint32 length, int nice, uint8 diagbuf_alloc_request )
{
  void *ptr = NULL; /* Pointer to return.        */

  //TODO: Move this function here.
  ptr = diagbuf_alloc_internal(length, nice, diagbuf_alloc_request);
  ...
  else if (ch_type == DIAGCOMM_IO_DATA)
  {
    if(port_num == DIAGCOMM_PORT_1)
    {
      ...
      else
      {
        ...
        if((buf_header->pattern != 0xDEAD) || ((buf_header->length + tx_mode_ptr->buf_tail) > tx_mode_ptr->buf_size))
        {
      ...
      if(tx_mode_ptr->buf_tail > tx_mode_ptr->buf_size)
      {
        tx_mode_ptr->buf_tail = 0;
      }
     }
...
void diag_add_block_header(diag_send_buf_type* send_buf, uint16 length, uint8 diag_id)
{
  ...
  ASSERT(l_tx_mode != NULL);
    ...
    while(free_size < length)
    {
        ...
        if((send_buf->tail >send_buf->size) || (tail_data_block_header->pattern != 0xDEAD) || ((tail_data_block_header->length + send_buf->tail ) >= send_buf->size))
        {
            tail_data_block_header->pattern=0;
            tail_data_block_header->length=0;
            send_buf->tail=0;
        ...
        {
            free_size = send_buf->size - (new_head + sizeof(diag_send_buf_header_type));
        }
        else
        {
            free_size = send_buf->tail - (new_head + sizeof(diag_send_buf_header_type));
        }
    }
...
/*Logic to check if we have to flush bufferred data*/
         ...
         /*Process incoming desc*/
            ...
            /*Add a header here to mark the start of a data block;
              ...
              diag_add_block_header(&send_buf, (pkt_length + diag_send_buf_pad ), diag_id);
               ...
               diag_get_sendbuf_params(&send_buf, desc->is_cmd_rsp_type, diag_id);
         }
      }
   }
   ...
   if ((!desc->is_cmd_rsp_type) && (FALSE == diagcomm_io_conn[DIAGCOMM_PORT_REMOTE_PROC][DIAGCOMM_PORT_1].allow_flow)) {
   ...
   if ((send_buf.head >= (send_buf.size - 1)) && (!desc->is_cmd_rsp_type)) {
...
void diag_send_buf_retry(diagcomm_io_channel_type ch_type, diagcomm_enum_port_type port_type, diag_tx_mode_type *tx_mode_ptr)
{
   ...
   else if (ch_type == DIAGCOMM_IO_CMD)
   {
     ...
     diagcomm_send((void *)diagbuf_send_cmd[port_type], diag_send_buf_cmd_index[port_type], TRUE, port_type, NULL);
   }
