...
Copyright (c) 2012-2016, 2018 by QUALCOMM Technologies, Incorporated.  All Rights Reserved.
...
$Header: //components/rel/core.qdsp6/1.0.c2/services/diag/LSM/qurt/src/Diag_LSM_Msg.c#1 $
...
void qsr_msg_send_var(const msg_qsr_const_type *const_blk, uint32 num_args, ...)
{
   if (diag_qdi_handle >= DIAG_FD_MIN_VALUE)
   {
      ...
      pMsg = qsr_msg_send_prep(const_blk, num_args, &nLength, TRUE, stm_enabled);
      if (pMsg)
      {
         msg_qsr_type *pTemp = (msg_qsr_type *)(pMsg);
      ...
      msg_save_trace((const void *)const_blk, f3_args, MIN(num_args, MAX_VAR_ARGS), (void *)pMsg, DIAG_F3_TRACE_QSHRINK);
   }
} /* qsr_msg_send_var */
...
msg_v2_send_var(const msg_v2_const_type *const_blk, uint32 num_args, ...)
{
   if (diag_qdi_handle >= DIAG_FD_MIN_VALUE)
   {
      ...
      pMsg = qsr_msg_send_prep((msg_qsr_const_type *)const_blk, num_args, &nLength,
                               MSG_V2_CHECK_IS_HASH(const_blk->msg), stm_enabled);
      if (pMsg)
      {
         if (QSR_BITFLAG_IS_HASH & MSG_V2_CHECK_IS_HASH(const_blk->msg))
         {
...
#endif
         {
            diagbuf_mpd_commit(pMsg); // VinDbg to skip 00 00 00 01 marker that LSM added for a packet to be identified in DCM.
         }
      }
 ...
 unsigned long long timestamp, boolean ts_valid)
{
   ...
   if (valid_ssid && (const_blk->desc.ss_mask & rt_mask))
   {
      ...
      pMsg = (byte *)diagbuf_mpd_alloc(0, alloc_len, 0, DIAGBUF_ALLOC_REQUEST_MSG);
