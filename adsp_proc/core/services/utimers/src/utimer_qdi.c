...
$Header: //components/rel/core.qdsp6/1.0.c2/services/utimers/src/utimer_qdi.c#1 $ 
...
static inline utimer_error_type utimer_qdi_get_first_non_deferrable_timer_expiry(int client_handle, utimer_timetick_type *ptick)
{
   utimer_timetick_type expiry_time;
   int                  result;
   
   expiry_time = utimer_drv_get_first_non_deferrable_timer_expiry();
                          ...
                          qurt_qdi_arg_t a9)
{
   ...
   switch(method)
   {
      ...
      case UTIMER_QDI_GET_FIRST_NON_DEFERRABLE_TIMER_EXPIRY:  
         return utimer_qdi_get_first_non_deferrable_timer_expiry(client_handle, a1.ptr); 
