 ...
 * the SCons USES_FLAG parameter in the build command
 */
#ifdef FEATURE_DISABLE_RPM_COMMUNICATION
static volatile bool that_rpm_guy_is_crazy = true;
#else
static volatile bool that_rpm_guy_is_crazy = false;
#endif /* FEATURE_DISABLE_RPM_COMMUNICATION */
...
static unsigned _rpm_mq_send(rpm_mq_t *self, rpm_msg_t *msg)
{
...
unsigned rpm_mq_put(rpm_mq_t *self, rpm_set_type set, rpm_resource_type resource, unsigned id, rpm_resource_t *resource_data)
{
    ...
    {
        if(!_rpm_mq_send(self, msg))
        {
            // Msg is not in flight so we can try to send other msgs
            rpm_mq_flush_limit(self);
        }
    }
...
static unsigned oldest_known_msg_age(rpm_mq_t *self)
{
    rpm_msg_t *msg;

    // Try each queue.
    msg = (rpm_msg_t *)inq_head(self->completed);
    if(msg) return msg_age(self, msg);

    msg = (rpm_msg_t *)inq_head(self->in_flight);
    if(msg) return msg_age(self, msg);

    msg = (rpm_msg_t *)inq_head(self->enqueued);
    if(msg) return msg_age(self, msg);
...
void rpm_mq_wait(rpm_mq_t *self, unsigned msg_id)
{
    ...
    // If it's older than the oldest message in our queues, the message is definitely done already.
    if((self->next_msg_id - msg_id) > oldest_known_msg_age(self))
    {
        ULOG_RT_PRINTF_1(rpm_log, "rpm_msg_wait_complete (msg_id: 0x%0.8x) (event: 0x0)", msg_id);
        return;
    }
...
static void rpm_mq_send_msg_at_head(rpm_mq_t *self)
{
    ...
    CORE_VERIFY_PTR(outgoing);
    _rpm_mq_send(self, outgoing);
}

void rpm_mq_retire_acks(rpm_mq_t *self)
{
    while(inq_head(self->completed))
    {
        retire_msg(self, (rpm_msg_t *)inq_pop(self->completed));
    }

    while(inq_head(self->in_flight))
    {
        retire_msg(self, (rpm_msg_t *)inq_pop(self->in_flight));
    }
}
...
void rpm_mq_flush_limit(rpm_mq_t *self)
{
    // Send out a new message, if we have them waiting to go out.
