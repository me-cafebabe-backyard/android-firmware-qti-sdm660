/*==============================================================================
  FILE:         q6_lpm_config_v62.c

  OVERVIEW:     This file implements the APIs from q6_lpm_config header file
                on SSC subsystems for Q6 v62 version. It may be shared for
                targets with same Q6 version.

  NOTE:         This file depends on how Q6 core is integrated within a 
                subystem. So SSC implementation may vary from LPASS/MSS.
                
                Based on HPG_QDSP6SS_V62_1.2+ (rev 3.3 - Feb 11, 2016)
...
================================================================================
$Header: //components/rel/core.qdsp6/1.0.c2/power/sleep2.0/src/target/q6_lpm_config_v62.c#1 $
...
void q6LPMConfig_setupBase(void)
{
  ...
  SLEEP_HWIO_OUT( SPMCTL_EN_EXT,
          ...
          SLEEP_HWIO_FVAL( SPMCTL_EN_EXT, SHUTDOWN_REQ, 0x0 ) |
          SLEEP_HWIO_FVAL( SPMCTL_EN_EXT, XO_ENABLE,    0x0 ));

  /* Due to HW bug (HW CR QCTDD03560301), SparkPLL_14LPP PLL fails to lock after FREEZE/UNFREEZE operation for some 
   * corner cases, LPASS Q6 PLL cannot be configured for FREEZE MODE. It should be turned off during long APCR/full PC.
   */   
  ...
  SLEEP_HWIO_OUT( SLPC_CFG, 
            SLEEP_HWIO_FVAL( SLPC_CFG, EXTHW_WAKE_EN,       0x1 ) |
  ...
  SLEEP_HWIO_OUTF( SPM_EVENT_CTL, PLL_FINE_OVRRD, 0x1);
  SLEEP_HWIO_OUTF( SPM_EVENT_CTL, SMMU_STATUS_OVRRD, 0x0);
                                  ...
                                  boolean                 flag )
{
  ...
  /* SMMU_STATUS_OVRRD will be set to 1 in island. So, need to reset it here back to 0 during
   * normal mode PC.
   */
  SLEEP_HWIO_OUTF( SPM_EVENT_CTL, SMMU_STATUS_OVRRD,  0x0 );
...
void q6LPMConfig_setupL2Config(mem_state state)
{
  ...
  else
  {
    ...
    memRetValue = 
                  ...
                  SLEEP_HWIO_FVAL(SPMCTL_EN_MEM_RET, L2TAG,   0xF)  |
                  SLEEP_HWIO_FVAL(SPMCTL_EN_MEM_RET, L2DATA,  l2data);
  }
