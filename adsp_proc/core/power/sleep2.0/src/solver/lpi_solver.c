...
================================================================================
$Header: //components/rel/core.qdsp6/1.0.c2/power/sleep2.0/src/solver/lpi_solver.c#1 $
                                          ...
                                          sleep_solver_output *output)
{
  ...
  {
    /* Get current time to be used for determination of soft deadline and hard duration */
    ...
    normalSoftDeadline = currentTime + input->soft_duration;
    
    /* Calculate hard duration from first hard deadline */
    normalHardDuration  = input->hard_deadlines.normal_min - currentTime;
    islandHardDuration  = input->hard_deadlines.island_timer - currentTime;
      ...
      for(i = 0; i < SLEEP_LPI_NUM_SYNTHESIZED_MODES; i++)
      {
        ...
        output->threshold_deadline = 
          sleepLPISolver_setThresholdDeadline(lookupElement->duration_thresh, 
                                              ...
                                              input->hard_deadlines.island_timer,
                                              &output->ref_deadline);
