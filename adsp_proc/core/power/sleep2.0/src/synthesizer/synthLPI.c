...
================================================================================
$Header: //components/rel/core.qdsp6/1.0.c2/power/sleep2.0/src/synthesizer/synthLPI.c#1 $
...
void synthLPI_updateLatency(int32 enterAdjustment, int32 exitAdjustment)
{
  ...
  for(nFreq=0; nFreq < SLEEP_NUM_STATIC_FREQ; nFreq++)
  {
    ...
    adjustedLat = exitLat + exitAdjustment;
    CORE_VERIFY((adjustedLat > 0) && ((exitLat * 8) > adjustedLat));
    
    /* Verify the enter latency adjustment */
    adjustedLat = seedLatency->enter[nFreq] + enterAdjustment;
    CORE_VERIFY((adjustedLat > 0) && ((seedLatency->enter[nFreq] * 8) > adjustedLat));
    ...
    CORE_VERIFY(seedLatency->enter[nFreq] < seedLatency->enter_exit[nFreq]);
