...
$Header: //components/rel/core.qdsp6/1.0.c2/power/qdsp_pm/src/driver/mpd/adsppmcb.c#1 $
*/
...
{
    qurt_pimutex_init(&Adsppmcb_mutex_Srv);
    pfnlist = NULL;
}
...
void Adsppmcb_register_callback_Srv( QDI_Adsppm_cbinfo_t *arg)
{
    ...
    if(p)
    {
        ...
        qurt_pimutex_lock(&Adsppmcb_mutex_Srv);
        p->next = pfnlist;
        pfnlist = p;
        qurt_pimutex_unlock(&Adsppmcb_mutex_Srv);
    }
}
...
void Adsppmcb_Deinit_Srv(void)
{
    ...
    qurt_pimutex_destroy(&Adsppmcb_mutex_Srv);
}
