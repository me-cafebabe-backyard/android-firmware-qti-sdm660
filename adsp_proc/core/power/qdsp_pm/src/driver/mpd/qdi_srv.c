/*
* Copyright (c) 2015-17 Qualcomm Technologies, Inc.
* All Rights Reserved.
...
$Header: //components/rel/core.qdsp6/1.0.c2/power/qdsp_pm/src/driver/mpd/qdi_srv.c#1 $
*/
...
void QDI_Adsppm_do_callback(QDI_Adsppm_cbinfo_t *ptr, MmpmCallbackParamType *pValue, uint32 pd_num)
{
    ...
    if(cbq)
    {
        ...
        qurt_pimutex_lock(&(ptr->object->mtx));
        cbq->next = ptr->object->cbqueue;
        ptr->object->cbqueue = cbq;
        qurt_pimutex_unlock(&(ptr->object->mtx));
        qurt_signal_set(&ptr->object->sigLocal[pd_num], 1);
    }
}


int QDI_Adsppm_get_callback(
    int client_handle,
    ...
    unsigned int pd_num)
{
    ...
    for(;;)
    {
		...
		if(ptrPdMatched)
        {
            ...
            qurt_pimutex_unlock(&pCbDrv->mtx);
            break;
        }
        else
        {
            ...
            result = qurt_signal_wait_cancellable(
                &pCbDrv->sigLocal[pd_num], 1, QURT_SIGNAL_ATTR_WAIT_ALL, &signalRead);
