...
$Header: //components/rel/core.qdsp6/1.0.c2/power/qdsp_pm/src/driver/qdsp_pm_mgr/resource_mgr/adspclkmgr.c#1 $
*/
...
#define QDSPPM_ISLAND_Q6_MAX_CLK 249593856
    ...
    if(status == Adsppm_Status_Success)
     {
		...
		if (gAdsppmAdspClkManager.uSleepHandle)
		{
			...
			ADSPPM_LOG_PRINTF_2(ADSPPM_LOG_LEVEL_INFO_EXT,"NPA request to %s usleep for %d Hz", (uSleep_vote ? "Allow" :"DisAllow"), freqHz);
    ...
    uint32 newClientsFinalVote)
{
    ...
    sd_adsppm_vote_t adspClk;
    adspClk.coreVoteinHz = newClientsFinalVote;

   sysmon_adsppm_static_vote_update(
        SYSMON_DCVS_RSC_ID_ADSP_CLK, &adspClk);
