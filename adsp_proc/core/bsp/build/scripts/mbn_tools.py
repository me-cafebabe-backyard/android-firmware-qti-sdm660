...
#
# Copyright 2010-2013 by Qualcomm Technologies, Incorporated.
# All Rights Reserved.
# QUALCOMM Proprietary/GTDR
#
#-------------------------------------------------------------------------------
#
#  $Header: //components/rel/core.qdsp6/1.0.c2/bsp/build/scripts/mbn_tools.py#1 $
...
CERT_CHAIN_MAXSIZE        = 6*1024          # Default Cert Chain Max Size
...
def pboot_gen_elf(env, elf_in_file_name,
   ...
   if elf_out_file_name is not None:
      ...
      # Increment program header count
   ...
   if elf_out_file_name is not None:
     ...
     # Initialize the hash segment program header
     ...
     if curr_phdr.p_offset < hash_seg_end:
        hashtable_shift = hash_seg_end - curr_phdr.p_offset

     # Move program headers to after ELF header
     phdr_start = ELF_HDR_SIZE
     ...
     elf_out_fp.seek(0)
     # Read the elf header
     elfhdr_buff = elf_out_fp.read(ELF_HDR_SIZE)
...
def pboot_add_hash(env, elf_in_file_name, 
   ...
   if hash_segment_found is True: 
      ...
      raise RuntimeError, "Hash segment program header not found!"
...
def generate_code_hash(env, elf_in_file_name):
   ...
   # Finally, mark the hash segment read-only
   phdr_flag_off = 24
   file_offset = elf_header.e_phoff + (elf_header.e_phentsize * hash_seg_idx) + phdr_flag_off
   elf_in_fp.seek(file_offset)

   new_flag = (hash_phdr.p_flags & ~PH_PERM_MASK) | PH_PERM_RO
   s = struct.Struct('I') 
   new_flag_bytes = s.pack(new_flag)
...
def init_build_vars(env):
   ...
   global CERT_CHAIN_MAXSIZE
   CERT_CHAIN_MAXSIZE = get_dict_value(env['GLOBAL_DICT'], 'CERT_CHAIN_MAXSIZE', (6*1024))
