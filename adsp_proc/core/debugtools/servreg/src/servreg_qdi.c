...
static int servreg_qdi_update_handle(int client_handle, servnotif_qdi_device_p clientobj, uint32_t state, int32_t transaction_id);
...
static void servreg_qdi_release(qurt_qdi_obj_t* qdiobj)
{
   ...
   if(SERVREG_SUCCESS == ret)
   {
      ...
      servnotif_cache_pd_data_node = (servnotif_cache_pd_data_p)servreg_malloc(sizeof(servnotif_cache_pd_data_t));
   ...
   {
      TMS_MSG_ERROR("servreg_delete_qdi_notif_list() failed ");
   }
...
static int servreg_qdi_open(int client_handle, SERVREG_NAME name, uint32_t name_len)
{
   ...
   clientobj = (servnotif_qdi_device_p)servreg_malloc(sizeof(servnotif_qdi_device_t));
      ...
      if (SERVREG_NULL != domain)
      {
         ...
         if(-1 != ret)
         {
            ...
            if(SERVREG_NULL != dataobj->sr_mon_handle)
            {
               ...
               {
                  TMS_MSG_HIGH_1(" User PD = %s recorded in Root-PD as UP \n", domain);
...
static int servreg_qdi_close(servnotif_qdi_device_p clientobj)
{
   ...
   {
      TMS_MSG_HIGH(" in servreg_qdi_close() User-PD state is set to DOWN");

      ret = servreg_free_monitor_handle(dataobj->sr_mon_handle);
      if(SERVREG_SUCCESS != ret)
      {
         TMS_MSG_ERROR("in servreg_qdi_close() could not free the monitor handle");
      }
   }
   else
   {
...
static int servreg_qdi_worker_wait(int client_handle, servnotif_qdi_device_p clientobj, uint32_t * remote_mask)
{
   ...
   if(-1 == ret)
   {
      TMS_MSG_ERROR("in servreg_qdi_worker_wait() qurt_qdi_copy_to_user() failed \n");
   }
      ...
      if (-1 != ret)
      {
         ...
         if(SERVREG_NULL != sr_mon_handle)
         {
            ...
            if(SERVREG_SUCCESS == result)
            {
               ...
               {
                  SERVREG_TRACE_0(FAIL);
                  TMS_MSG_ERROR("sr_qdi_notif_handle is null\n");
                  ret = -1;
               }
            }
            else
   ...
   if(SERVREG_NULL != service_name)
   {
      ...
      if (-1 != ret)
      {
         ...
         if(SERVREG_NULL != sr_mon_handle)
         {
            ...
            if(SERVREG_SUCCESS != result)
            {
               TMS_MSG_ERROR("servreg_deregister_listener_qurt() failed");
               ret = -1;
            }
         }
         else
 ...
 *     clientobj        : QDI object which represents the client
 *                        which made this QDI request.
 *     state            : address of the monitor handle address who's state change
 *     transaction_id   : state of the monitor handle who's state changed
 *
 * Returns:
 *     SERVREG_SUCCESS (0) or SERVREG_FAILURE(-1)
 * =====================================================================  */
                      ...
                      )
{
   ...
   switch (servnotif_qdi_method)
   {
      ...
      case QDI_OPEN:
      {
         ...
         TMS_MSG_HIGH_2("device open %s == %d", task_name, ret);
         break;
      }
      ...
      case SERVREG_QDI_WORKER_WAIT:
      {
         ...
         TMS_MSG_HIGH_2("device servreg_qdi_worker_wait() method %s == %d", task_name, ret);
         break;
      }
      ...
      case SERVREG_QDI_REG_REMOTE_LISTENER:
      {
         ...
         TMS_MSG_HIGH_2("device servreg_qdi_register_remote_listener() method %s == %d", task_name, ret);
         break;
      }
      ...
      case SERVREG_QDI_DEREG_REMOTE_LISTENER:
      {
         ...
         TMS_MSG_HIGH_2("device servreg_qdi_deregister_remote_listener() method %s == %d", task_name, ret);
         break;
      }
      ...
      case SERVREG_QDI_FREE_REMOTE_HANDLE:
      {
         ...
         TMS_MSG_HIGH_2("device servreg_qdi_free_remote_handle() method %s == %d", task_name, ret);
         break;
      }
      ...
      case SERVREG_QDI_SET_REMOTE_ACK:
      {
         ...
         TMS_MSG_HIGH_2("device servreg_qdi_set_remote_ack() method %s == %d", task_name, ret);
         break;
      }
      ...
      case SERVREG_QDI_GET_REMOTE_HANDLE:
      {
         ...
         TMS_MSG_HIGH_2("device servreg_qdi_get_remote_handle() method %s == %d", task_name, ret);
         break;
      }
      ...
      case SERVREG_QDI_CREATE_QMI_ENTRY:
      {
         ...
         TMS_MSG_HIGH_2("device servreg_qdi_create_qmi_entry() method %s == %d", task_name, ret);
         break;
      }
      ...
      case SERVREG_QDI_REGISTER_NEW_HANDLE:
      {
         ...
         TMS_MSG_HIGH_2("device servreg_qdi_register_handle() method %s == %d", task_name, ret);
         break;
      }
      ...
      case SERVREG_QDI_SET_STATE:
      {
         ...
         TMS_MSG_HIGH_2("device servreg_qdi_set_state() method %s == %d", task_name, ret);
         break;
      }
      ...
      {
         ret = servreg_qdi_update_handle(client_handle, clientobj, a1.num, a2.num);
         TMS_MSG_HIGH_2("device servreg_qdi_update_handle() method %s == %d", task_name, ret);
         break;
      }
      ...
      case SERVREG_QDI_REGISTER_ROOT_HANDLES:
      {
         ...
         TMS_MSG_HIGH_2("device servreg_qdi_register_root_handles() method %s == %d", task_name, ret);
         break;
      }
      ...
      case SERVREG_QDI_GET_ROOT_ACK_HANDLE:
      {
         ...
         TMS_MSG_HIGH_2("device servreg_qdi_get_ack_handle() method %s == %d", task_name, ret);
         break;
      }
...
void servreg_qdi_init(void)
{
   ...
   if(ret == 0)
   {
      TMS_MSG_HIGH("device = /dev/servnotif registered successfully");
   }
   else
   {
