...
static SERVREG_DL_HANDLE servreg_get_sr_dl_handle(SERVREG_NAME const name)
{
   ...
   while(SERVREG_NULL != sr_dl_table)
   {
      ...
      {
         len = servreg_nmelen(sr_dl_table->service_name);
         if(servreg_nmecmp(sr_dl_table->service_name, name, len) == 0 )
         {
            sr_dl_handle = sr_dl_table2sr_dl_handle(sr_dl_table);
            TMS_MSG_HIGH_1("handle = 0x%x ", sr_dl_handle);
            break;
         }
      }
      else
   ...
   if(SERVREG_NULL == sr_dl_handle)
   {
      ...
      if(SERVREG_NULL != sr_dl_table)
      {
         ...
         else
         {
            ...
            ERR_FATAL( "SERVREG_LOC_S: buf_p malloc failed", 0, 0, 0);
         }
      ...
      {
         TMS_MSG_ERROR("sr_dl_table is NULL\n");
      }
   }
   else
   {
...
static servreg_qmi_locator_server_node_p servreg_qmi_locator_server_node_pool_init(void)
{
   ...
   {
      next_pool = (servreg_qmi_locator_server_node_pool_p)servreg_malloc(sizeof(servreg_qmi_locator_server_node_pool_t));
   }
...
static SERVREG_QMI_LOC_SERVER_HANDLE servreg_create_qmi_locator_server_node(qmi_client_handle client_handle)
{
   ...
   if(SERVREG_NULL == sr_qmi_locator_server_handle)
   {
      ...
      {
         ERR_FATAL( "SERVREG_QMI_LOC: sr_qmi_locator_server_node malloc failed", 0, 0, 0);
      }
   }
...
static SERVREG_RESULT servreg_delete_qmi_locator_server_node(SERVREG_QMI_LOC_SERVER_HANDLE sr_qmi_locator_server_handle)
{
   ...
   {
      TMS_MSG_ERROR("sr_qmi_locator_server_handle is NULL\n");
      ret = SERVREG_INVALID_HANDLE;
   }
...
static SERVREG_BOOL servreg_qmi_validate_connection_handle(void *connection_handle)
{
   ...
   if(connection_handle != NULL)
   {
      while(SERVREG_NULL != client)
      {
         ...
         {
            ret = SERVREG_TRUE;
            TMS_MSG_ERROR("connection_handle is MATCHES \n");
            break;
         }
         client = client->next;
         TMS_MSG_ERROR("connection_handle does not MATCH \n");
      }
   }
   else
   {
      TMS_MSG_ERROR("connection_handle is NULL \n");
   }
...
void servreg_send_qmi_ind_msg_db_updated(void)
{
   ...
   while(SERVREG_NULL != sr_qmi_locator_server_node)
   {
      ...
      TMS_MSG_HIGH_1("qmi_csi_send_ind err= %d" ,err);
                                          ...
                                          void           *service_cookie)
{
   ...
   if(NULL != req_handle)
   {
         ...
         switch(msg_id)
         {
            case SERVREG_LOC_INDICATION_REGISTER_REQ:
            {
               ...
               if(SERVREG_TRUE == request->enable_database_updated_indication)
               {
                  ...
                  {
                     ERR_FATAL( "SERVREG_QMI_LOC: servreg_create_qmi_locator_server_node() failed", 0, 0, 0);
                  }
               }
               else /* LOC Deregister Listener */
               {
                  ...
                  if(SERVREG_NULL != sr_qmi_locator_server_handle)
                  {
                     ...
                     {
                        ERR_FATAL( "SERVREG_QMI_LOC: servreg_delete_qmi_locator_server_node() failed", 0, 0, 0);
                     }
                  }
                  else
                  {
            ...
            case SERVREG_LOC_GET_DOMAIN_LIST_REQ:
            {
               ...
               resp_send = (qmi_servreg_loc_get_domain_list_resp_msg_v01*)servreg_malloc(sizeof(qmi_servreg_loc_get_domain_list_resp_msg_v01));
               ...
               {
                  TMS_MSG_ERROR("SERVREG_LOC_GET_DOMAIN_LIST_REQ or malloc failed \n");
            ...
            case SERVREG_LOC_REGISTER_SERVICE_LIST_REQ:
            {
               ...
               for(i = 0; i < request->service_list_len; i++)
               {
                  ...
                  if(SERVREG_NULL != sr_dl_table && SERVREG_NULL != srdl_entry)
                  {
                     ...
                     srdl_entry->domain_name = (SERVREG_NAME)servreg_malloc(sizeof(char) * len1);
                     ...
                     {
                        servreg_dl_table_pool_free(sr_dl_table, &servreg_dl_internal_server);
                        ERR_FATAL( "SERVREG_LOC_S: malloc failed", 0, 0, 0);
                     }
                  ...
                  {
                     ERR_FATAL( "SERVREG_LOC_S: srdl_entry or sr_dl_table malloc failed", 0, 0, 0);
                  }
               } /* for () */
      ...
      {
         TMS_MSG_ERROR("connection_handle is invalid");
         retval = QMI_CSI_CB_INTERNAL_ERR;
      }
  }
  else
  {
      TMS_MSG_ERROR("req_handle is NULL");
      retval = QMI_CSI_CB_INTERNAL_ERR;
  }
...
void servreg_locator_server_disconnect(void *connection_handle, void *service_cookie)
{
   ...
   {
      TMS_MSG_ERROR("connection handle is NULL");
   }
   return;
}
...
qmi_csi_cb_error servreg_locator_server_connect(qmi_client_handle client_handle, void *service_cookie, void **connection_handle)
{
   ...
   client = (servreg_locator_client_type_p)servreg_malloc(sizeof(servreg_locator_client_type_t));
   ...
   {
      TMS_MSG_ERROR("malloc failed and connection failed with server");
   }
