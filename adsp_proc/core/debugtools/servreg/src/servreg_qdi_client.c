 ...
 *    SERVREG_SUCCESS (0) or SERVREG_FAILURE(-1)
 * =====================================================================  */
SERVREG_RESULT servreg_qdi_invoke_update_remote_handle(SERVREG_SERVICE_STATE sr_state, int32_t sr_transaction_id)
{
   int ret = -1;

   ret = qurt_qdi_handle_invoke(servnotif_qdi_client_handle, SERVREG_QDI_UPDATE_HANDLE, (uint32_t)sr_state, sr_transaction_id);
...
void servreg_qdi_init(void)
{
   ...
   if (0 <= servnotif_qdi_client_handle)
   {
      TMS_MSG_HIGH_1("user-pd servreg_qdi_init() PASS, handle = %d\n", servnotif_qdi_client_handle);
   }
   else
   {
