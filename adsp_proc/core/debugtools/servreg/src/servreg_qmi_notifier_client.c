...
static servreg_qmi_notif_client_node_p servreg_qmi_notif_client_node_pool_free(servreg_qmi_notif_client_node_p sr_qmi_notif_client_node)
{
   ...
   {
      TMS_MSG_ERROR("sr_qmi_notif_client_node is NULL and cannot be freed");
   }
...
SERVREG_RESULT servreg_delete_qmi_notif_client_node(SERVREG_QMI_NOTIF_CLIENT_HANDLE sr_qmi_notif_client_handle)
{
   ...
   {
      TMS_MSG_ERROR("sr_qmi_notif_client_handle is NULL \n");
      ret = SERVREG_INVALID_HANDLE;
   }
...
static void servreg_ssr_down_event_cb(uint32_t sr_qmi_notif_handle_addr)
{
   ...
   if(SERVREG_NULL != sr_qmi_notif_client_node)
   {
      ...
      {
         TMS_MSG_HIGH("servreg_ssr_down_event_cb() User-PD DOWN due to SSR");
      ...
      {
         TMS_MSG_ERROR("servreg_ssr_down_event_cb() servreg_set_state failed");
      }
     ...
     {
       TMS_MSG_ERROR("SERVREG_QMI_NOTIF: servreg_ssr_down_event_cb() servreg_get_service_name failed");
      ...
      {
        TMS_MSG_ERROR("SERVREG_QMI_NOTIF: servreg_ssr_down_event_cb() servreg_get_qmi_table_entry failed");
      ...
      if(SERVREG_NULL != sr_qmi_entry->servreg_qmi_notifier_client_handle)
      {
        ...
        {
          TMS_MSG_ERROR("SERVREG_QMI_NOTIF: servreg_ssr_down_event_cb() qmi_client_release failed");
...
static SERVREG_RESULT servreg_reconnect_remote_listener(SERVREG_QMI_NOTIF_CLIENT_HANDLE sr_qmi_notif_client_handle)
{
      ...
      if(SERVREG_NULL != service_name)
      {
        ...
        if(SERVREG_SUCCESS == ret)
        {
           ...
           if(QMI_NO_ERR == qmi_client_return)
           {
              ...
              {
                 TMS_MSG_ERROR_1("ERROR qmi response = %d\n", servreg_notif_resp.resp.result);
              }
           }
           else
           {
              TMS_MSG_ERROR_1(" ERROR qmi_client_return= %d\n", qmi_client_return);
           }

        }
        else
        {
           TMS_MSG_ERROR("cannot connect to the QMI server");
        }
      }
      else
      {
         TMS_MSG_ERROR("service name is NULL");
      }
   ...
   {
      TMS_MSG_HIGH("sr_qmi_notif_client_handle is null \n");
   }
...
void servreg_notif_retry_timout_cb(void* sr_qmi_notif_handle_addr)
{
   ...
   if(SERVREG_NULL != sr_qmi_notif_client_node)
   {
      if(sr_qmi_notif_client_node->server_retry_count != 0)
      {
         ...
         TMS_MSG_ERROR_1("server connect retry attempt = %d ", (SERVREG_QMI_NOTIF_MAX_RETRY_COUNT - sr_qmi_notif_client_node->server_retry_count));
      ...
      else
      {
         ...
         TMS_MSG_ERROR_1("Could not connect to Subsys Server after %d re-try attempts", SERVREG_QMI_NOTIF_MAX_RETRY_COUNT);
      }
   }
}
...
static void servreg_ssr_up_event_cb(uint32_t sr_qmi_notif_handle_addr)
{
   ...
   if(SERVREG_NULL != sr_qmi_notif_client_node)
   {
      TMS_MSG_HIGH("subsys ssr up. Starting retry timer");
...
static void servreg_register_ssr_event(SERVREG_NAME service_name, SERVREG_QMI_NOTIF_CLIENT_HANDLE sr_qmi_notif_client_handle)
{
   ...
   if(SERVREG_NULL != service_name)
   {
      ...
      {
         TMS_MSG_ERROR("in servreg_register_ssr_event() root-pd could not register with SSR \n");
      }
   }
   else
   {
      TMS_MSG_HIGH("in servreg_register_ssr_event() user-pd need not register for ssr");
   }
}
...
void servreg_create_qmi_table_entry(SERVREG_NAME domain_name, uint32 qmi_instance_id)
{
   ...
   {
      TMS_MSG_ERROR("don't need to record qmi_instance_id in user-pd");
   }
}
   ...
   if(SERVREG_NULL == sr_qmi_notif_client_handle)
   {
      ...
      if(SERVREG_NULL != sr_qmi_notif_client_handle)
      {
        ...
        if(SERVREG_FAILURE == ret)
        {
           ...
           {
              TMS_MSG_ERROR("servreg_delete_qmi_notif_client_node() failed\n");
           }
        }
      }
      else
   ...
   if(SERVREG_NULL != sr_qmi_notif_client_handle && sr_listener_count == 0)
   {
      ...
      if(SERVREG_NULL != sr_qmi_entry)
      {
         ...
         if(QMI_NO_ERR == qmi_client_return)
         {
            ...
            {
               TMS_MSG_ERROR_1("in servreg_deregister_remote_listener() ERROR qmi response = %d\n", servreg_notif_resp.resp.result);
               ret = SERVREG_FAILURE;
            }
         }
         else
         {
            TMS_MSG_ERROR_1("in servreg_deregister_remote_listener() ERROR qmi_client_return= %d\n", qmi_client_return);
            ret = SERVREG_FAILURE;
         }
      }
      else
      {
         TMS_MSG_ERROR("in servreg_deregister_remote_listener() sr_qmi_entry NULL \n");
         ret = SERVREG_FAILURE;
      }
...
SERVREG_RESULT servreg_set_remote_ack(SERVREG_NAME service_name, SERVREG_MON_HANDLE sr_mon_handle, SERVREG_SERVICE_STATE curr_state, int32_t sr_transaction_id)
{
   ...
   sr_remote_ack_req_entry = (servreg_remote_ack_req_p)servreg_malloc(sizeof(servreg_remote_ack_req_t));
   ...
   {
      ERR_FATAL("servreg_set_remote_ack() malloc failed \n", 0, 0, 0);
   }
   ...
   TMS_MSG_HIGH("servreg_set_remote_ack: signal sent to sr_remote_req task\n");   
...
static SERVREG_RESULT servreg_remote_queue_action(SERVREG_NAME service_name, SERVREG_MON_HANDLE sr_mon_handle, int32_t sr_transaction_id)
{
   ...
   if(SERVREG_NULL != sr_qmi_notif_client_handle)
   {
      if(0 < sr_transaction_id)
      {
         ...
         if(SERVREG_NULL != sr_qmi_entry)
         {
            ...
            if(QMI_NO_ERR == qmi_client_return)
            {
               ...
               {
                  ret = SERVREG_SUCCESS;
                  TMS_MSG_HIGH("in servreg_set_remote_ack() success \n");
                  SERVREG_TRACE_0(REMOTE_ACK_QMI_SUCCESS);
               }
               else
               {
                  TMS_MSG_ERROR_1("in servreg_set_remote_ack() ERROR qmi response = %d\n", servreg_notif_resp.resp.result);
            ...
            {
               TMS_MSG_ERROR_1("in servreg_set_remote_ack() ERROR qmi_client_return= %d\n", qmi_client_return);
         ...
         {
            TMS_MSG_ERROR("in servreg_set_remote_ack() sr_qmi_entry NULL \n");
            ret = SERVREG_FAILURE;
         }
      ...
      {
         TMS_MSG_HIGH_1("SUCCESS sr_transaction_id: %d\n", sr_transaction_id);
         ret = SERVREG_SUCCESS;
      }
   }
...
static void servreg_notif_client_ind_cb(qmi_client_type client_handle, uint32_t msg_id, void *ind_buf, uint32_t ind_buf_len, void *ind_cb_data)
{
   ...
   TMS_MSG_HIGH_2("in servreg_notif_client_ind_cb() request: client_handle = %d, msg_id =%d", client_handle, msg_id);
   ...
   if(!ind_msg) 
   {
      TMS_MSG_ERROR("in servreg_notif_client_ind_cb() indication buff allocation failed");
      return;
   }
   ...
   {
      TMS_MSG_ERROR_1("in servreg_notif_client_ind_cb() Received error from QMI framework call %d" , qmi_err);
   ...
   sr_remote_qmi_ind_entry = (servreg_remote_qmi_ind_p)servreg_malloc(sizeof(servreg_remote_qmi_ind_t));
   if(SERVREG_NULL == sr_remote_qmi_ind_entry)
   {
     TMS_MSG_ERROR("in servreg_notif_client_ind_cb() could not get the notif node \n");
     servreg_free(ind_msg);
     return;
   }
...
SERVREG_RESULT servreg_qmi_connect_to_server(SERVREG_NAME domain_name)
{
   ...
   if(SERVREG_NULL != sr_qmi_entry)
   {
      ...
      if(QMI_NO_ERR == qmi_client_return)
      {
         ...
         if(QMI_NO_ERR == qmi_client_return)
         {
            ...
            ret = SERVREG_SUCCESS;
            TMS_MSG_ERROR_1("in servreg_qmi_connect_to_server() qmi_client_init SUCCESS with qmi_intance_id = %d \n", sr_qmi_entry->qmi_instance_id);
         }
         else
         {
            TMS_MSG_ERROR_1("SERVREG_QMI_NOTIF: in servreg_qmi_connect_to_server() qmi_client_init returned failure qmi_client_return = %d\n", qmi_client_return);
      ...
      {
         TMS_MSG_ERROR_1("in servreg_qmi_connect_to_server() qmi_client_get_service_instance returned failure qmi_client_return = %d", qmi_client_return);
         //qmi_client_release(servreg_qmi_client_notifier);
         ret = SERVREG_FAILURE;
      }
   }
   ...
   {
      TMS_MSG_ERROR("in servreg_qmi_connect_to_server() qmi_instance_id not found \n");
      ret = SERVREG_INVALID_HANDLE;
   }
...
static SERVREG_RESULT sr_ind_process(qmi_servreg_notif_state_updated_ind_msg_v01* indication)
{
   ...
   if(SERVREG_NULL == sr_qmi_notif_client_node)
   {
      TMS_MSG_ERROR("could not get the notif node for qmi ind process");
      return ret;
   }
   ...
   TMS_MSG_HIGH("set state success in qmi ind sr process");
...
void sr_remote_req_task( unsigned long int unused_param )
{
   ...
   for (;;)
   {
      ...
      if (q_mask & SERVREG_QMI_REMOTE_ACK_REQ_SIG)
      {
         ...
         TMS_MSG_HIGH( "sr_remote_req_task: signal rcvd & cleared\n");
      ...
      if (q_mask & SERVREG_QMI_REMOTE_QMI_IND_SIG)
      {
         ...
         TMS_MSG_HIGH("SERVREG_QMI_REMOTE_QMI_IND_SIG: signal rcvd & cleared\n");
...
void servreg_qmi_notif_client_init(void)
{
   ...
   else
   {
      ...
      if (servreg_notif_service_object)
      {
         ...
         {
            TMS_MSG_ERROR_1("in servreg_qmi_notif_client_init() qmi_client_notifier_init qmi_client_return = %d ",qmi_client_return);
         }
      }
      else
      {
         TMS_MSG_ERROR(" in servreg_qmi_notif_client_init() servreg_notif_service_object in NULL \n");
      }
   }
