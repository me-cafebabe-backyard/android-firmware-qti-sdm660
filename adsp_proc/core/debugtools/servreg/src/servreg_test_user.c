...
#include "servreg_test_user.h"
#include "gen_cb_ctxt.h"

#define SERVREG_TEST_U_TASK_NAME                    "servreg_test"
#define SERVREG_TEST_U_TASK_STACK                   4096
#define SERVREG_TEST_U_SIG_LOC_AVAILABLE            0x1
#define SERVREG_TEST_U_SIG_LOCAL_LIST_MASK          0x2
#define SERVREG_TEST_U_SIG_REMOTE_QDI_LIST_MASK     0x4
#define SERVREG_TEST_U_SIG_REMOTE_QMI_LIST_MASK     0x8

#define SERVREG_TEST_U_SERVICE_NAME             "tms/servreg_test"
...
void servreg_state_change_cb(void* unused);
...
SERVREG_MON_HANDLE sr_mon_remote_list_qdi_handle = SERVREG_NULL;
SERVREG_MON_HANDLE sr_mon_remote_list_qmi_handle = SERVREG_NULL;
SERVREG_SERVICE_STATE sr_curr_state = SERVREG_SERVICE_STATE_UNINIT;
...
void servreg_test_case_service_state_init(void)
{
   ...
   if(SERVREG_NULL != domain)
   {
      ...
      sr_mon_service_handle = servreg_alloc_monitor_handle(domain, SERVREG_TEST_U_SERVICE_NAME);
      ...
      if(SERVREG_NULL != sr_mon_service_handle)
      {
         ...
         {
            TMS_MSG_HIGH("Service registered successfully with SR \n");
         }
         else
         {
            TMS_MSG_ERROR("Service did not register with SR \n");
...
void servreg_test_case_register_listener_local_service(void)
{
   ...
   if(SERVREG_NULL != domain)
   {
      sr_mon_local_list_handle = servreg_alloc_monitor_handle(domain, SERVREG_TEST_U_SERVICE_NAME);

      if(SERVREG_NULL != sr_mon_local_list_handle)
      {
         ret = servreg_register_listener_qurt(sr_mon_local_list_handle, &servreg_test_signal, SERVREG_TEST_U_SIG_LOCAL_LIST_MASK);

         if(SERVREG_SUCCESS == ret)
         {
            TMS_MSG_HIGH("Client registered successfully with servreg_register_listener_qurt() \n");
         }
         else
         {
            TMS_MSG_ERROR("Client did not register with  servreg_register_listener_qurt() \n");
...
void servreg_test_case_register_listener_remote_qdi_service(void)
{
   ...
   {
      ret = servreg_get_domainlist(SERVREG_TEST_U_SERVICE_NAME, dlhandle);
   ...
   if(SERVREG_NULL == dlhandle || SERVREG_SUCCESS != ret)
   {
      ...
      {
         if(strcmp(domain_scope, "modem") == 0)
         {
         ...
         else if(strcmp(domain_scope, "slpi") == 0)
         {
      ...
      {
         TMS_MSG_ERROR("servreg_get_local_domain_scope is NULL \n");
         domain = SERVREG_NULL;
      }
   }
   ...
   if(SERVREG_NULL != domain)
   {
      sr_mon_remote_list_qdi_handle = servreg_alloc_monitor_handle(domain, SERVREG_NULL);

      if(SERVREG_NULL != sr_mon_remote_list_qdi_handle)
      {
         ret = servreg_register_listener_qurt(sr_mon_remote_list_qdi_handle, &servreg_test_signal, SERVREG_TEST_U_SIG_REMOTE_QDI_LIST_MASK);

         if(SERVREG_SUCCESS == ret)
         {
            TMS_MSG_HIGH("Client registered successfully with servreg_register_listener_qurt() via QDI \n");
         }
         else
         {
            TMS_MSG_ERROR("Client did not register with  servreg_register_listener_qurt() \n");

            ret = servreg_free_monitor_handle(sr_mon_remote_list_qdi_handle);
            if(SERVREG_SUCCESS != ret)
            {
               TMS_MSG_ERROR(" could not free the monitor handle \n");
               TMS_MSG_ERROR(" could not free the monitor handle");
            ...
            TMS_MSG_ERROR("service() failed ");
         }
      }
      else
      {
         TMS_MSG_ERROR(" sr_mon_remote_list_qdi_handle is null");
      }
   }
   else
   {
   ...
   {
      TMS_MSG_ERROR("dlhandle free failed");
   }
...
void servreg_test_case_register_listener_remote_qmi_service(void)
{
   ...
   if(SERVREG_NULL == dlhandle || SERVREG_SUCCESS != ret)
   {
      ...
      if(SERVREG_NULL != domain_scope)
      {
         ...
         {
            domain = "msm/adsp/root_pd";
            /* 4 is the qmi_instance_id for adsp */
            servreg_create_qmi_table_entry(domain, 4);
            TMS_MSG_ERROR("Could not get the domain info from locator, so assigning one = msm/adsp/root_pd \n");
         }
         else if(strcmp(domain_scope, "adsp") == 0)
         {
            domain = "msm/slpi/root_pd";
            /* 20 is the qmi_instance_id for slpi */
            servreg_create_qmi_table_entry(domain, 20);
            TMS_MSG_ERROR("Could not get the domain info from locator, so assigning one = msm/slpi/root_pd \n");
         }
         else if(strcmp(domain_scope, "slpi") == 0)
         {
            domain = "msm/modem/root_pd";
            /* 100 is the qmi_instance_id for modem */
            servreg_create_qmi_table_entry(domain, 100);
            TMS_MSG_ERROR("Could not get the domain info from locator, so assigning one = msm/modem/root_pd \n");
         }
         else
         {
   ...
   if(SERVREG_NULL != domain)
   {
      ...
      {
         ret = servreg_register_listener_qurt(sr_mon_remote_list_qmi_handle, &servreg_test_signal, SERVREG_TEST_U_SIG_REMOTE_QMI_LIST_MASK);
         ...
         else
         {
            ...
            TMS_MSG_ERROR("servreg_test_case_register_listener_remote_qmi_service() failed ");
         }
      }
      else
      {
...
void servreg_test_case_register_sync_notif(void)
{
   ...
   /* In user pd you can get sync complete notification for local handles only */
   if(SERVREG_NULL != sr_mon_local_list_handle)
   {
...
void servreg_test_task(dword param)
{
   ...
   qurt_anysignal_init (&servreg_test_signal);
   ...
   {
      ret = servreg_register_servloc_availability_signal_qurt(&servreg_test_signal, SERVREG_TEST_U_SIG_LOC_AVAILABLE);
   ...
   for(;;)
   {
      sigs = qurt_anysignal_wait (&servreg_test_signal, SERVREG_TEST_U_SIG_LOC_AVAILABLE | SERVREG_TEST_U_SIG_LOCAL_LIST_MASK | SERVREG_TEST_U_SIG_REMOTE_QDI_LIST_MASK | SERVREG_TEST_U_SIG_REMOTE_QMI_LIST_MASK);

      if(sigs & SERVREG_TEST_U_SIG_LOC_AVAILABLE)
      {
         /* Clear the signal */
         qurt_anysignal_clear (&servreg_test_signal, SERVREG_TEST_U_SIG_LOC_AVAILABLE);
      ...
      if (sigs & SERVREG_TEST_U_SIG_LOCAL_LIST_MASK) 
      {
         /* Clear the signal */
         qurt_anysignal_clear (&servreg_test_signal, SERVREG_TEST_U_SIG_LOCAL_LIST_MASK);
         ...
         if(SERVREG_SUCCESS != ret)
         {
            TMS_MSG_ERROR("servreg_set_ack() local failed");
         }

         TMS_MSG_HIGH_1("Local state change notification received new state = 0x%x \n", sr_new_state);
      }

      if (sigs & SERVREG_TEST_U_SIG_REMOTE_QDI_LIST_MASK) 
      {
         /* Clear the signal */
         qurt_anysignal_clear (&servreg_test_signal, SERVREG_TEST_U_SIG_REMOTE_QDI_LIST_MASK);

         sr_new_state = servreg_get_service_curr_state(sr_mon_remote_list_qdi_handle);

         transaction_id = servreg_get_transaction_id(sr_mon_remote_list_qdi_handle);
         ret = servreg_set_ack(sr_mon_remote_list_qdi_handle, transaction_id);

         if(SERVREG_SUCCESS != ret)
         {
            TMS_MSG_ERROR("servreg_set_ack() remote failed");
         }

         TMS_MSG_HIGH_1("Remote QDI state change notification received new state = 0x%x \n", sr_new_state);
      }

      if (sigs & SERVREG_TEST_U_SIG_REMOTE_QMI_LIST_MASK) 
      {
         /* Clear the signal */
         qurt_anysignal_clear (&servreg_test_signal, SERVREG_TEST_U_SIG_REMOTE_QMI_LIST_MASK);
