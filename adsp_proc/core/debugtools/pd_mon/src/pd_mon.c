...
/*=============================================================================
Edit History
$Header: //components/rel/core.qdsp6/1.0.c2/debugtools/pd_mon/src/pd_mon.c#1 $
...
static void PROGRAM_MONITOR(void* argv)
{
   ...
   if (sizeof(pd_mon_internal.status)/sizeof(PD_MON_STATUS_T) > i)
   {
      ...
      stat_p->asid = PD_MON_STATUS_TEMP_ASID;
   ...
   else
   {
      ...
      TMS_MSG_ERROR("PD_MON FAILURE NO SPACE");                       // Log Failure of PD to Start (NEVER HOST FATAL)
   }
...
void pd_mon_qurt_onspawn_cb(int client_handle, void __attribute__((unused)) *data_ptr)
{
   ...
   if (NULL == stat_p)
   {
      TMS_MSG_MED( "Spawn by non-TMS caller, no proxy attached");
