...
$Header: //components/rel/core.qdsp6/1.0.c2/api/dal/DalDevice.h#1 $
=============================================================================*/
...
struct DalDevice
{
   union
   {
      ...
      DALResult (*AttachEx)(const char*,DALDEVICEID,DalDeviceHandle **, int);
   };
...
/*
  @brief Establish communication link with the desired device
   It is directly call DAL_StringDeviceAttach API 
*/
...
/*
  @brief Add a proxy reference count for a given DAL user handle.

  User(s)/client(s) of DAL drivers must use this API to register a proxy for given DAL-user handle.
  Upon successfull execution, if the Dal Device handle is aquired from a driver from root PD, then
  a proxy reference is registered in root on behalf of the User/Client. 
      
  @param phDalDevice   : DAL Device Handle
...
/*
  @brief Remove proxy reference count for a given DAL user handle.

  User(s)/client(s) of DAL drivers must use this API to remove proxy for given DAL-user handle.
  Upon successfull execution, if the Dal Device handle has a registered proxy, the
  proxy reference is removed.
