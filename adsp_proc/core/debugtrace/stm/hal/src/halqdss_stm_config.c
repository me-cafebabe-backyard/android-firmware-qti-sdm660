...
/*========================================================================== 
 $Header: //components/rel/core.qdsp6/1.0.c2/debugtrace/stm/hal/src/halqdss_stm_config.c#1 $
==========================================================================*/  
...
int qdss_stm_busy_count;
/*FUNCTION HAL_qdss_stm_Disable*/
void HAL_qdss_stm_Disable(void)
{
   HWIO_OUTF(QDSS_STMTCSR,EN,LOW);
   qdss_stm_busy_count = 0;
   //wait 10ms atmost
   while ( HAL_qdss_stm_isBusy() && (qdss_stm_busy_count < 10000 )) {
      qdss_stm_busy_count++;
      DALSYS_BusyWait(1);
   }
}
...
/*FUNCTION HAL_qdss_stm_HWEventEnableMask*/
void  HAL_qdss_stm_HWEventEnableMask(uint32 uBitMask)
{
   HWIO_OUT(QDSS_STMHEER,uBitMask);
}
...
/*FUNCTION HAL_qdss_stm_HWEventTriggerSetMask*/
void  HAL_qdss_stm_HWEventTriggerSetMask(uint32 uBitMask)
{
   HWIO_OUT(QDSS_STMHETER,uBitMask);
}
...
void  HAL_qdss_stm_SetAuxCR(void)
{
   HWIO_OUTF(QDSS_STMAUXCR,FIFOAF,1);
}
