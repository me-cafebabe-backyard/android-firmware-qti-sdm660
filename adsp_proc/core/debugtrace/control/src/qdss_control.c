...
int qdss_control_on_bootup(void)
{
   ...
   //Used for standalone testing on Virtio
...
uint32 qdss_get_ssc_tsctr_freq(void)
{
   ...
   if (qdss.ctxt.clk.hClock) {
      DalClock_GetClockFrequency(qdss.ctxt.clk.hClock,
                                 qdss.ctxt.clk.ssc_dbg_tsctr_clk_id,
                                 &freq);
   }
...
int QDSSLocalClkOn(void)
{
   ...
   if (NULL==qdss.ctxt.clk.hClock) {
      ...
      _TRY(nErr,DalClock_GetClockId(qdss.ctxt.clk.hClock,
                                   "island_scc_at_clk",
                                   &qdss.ctxt.clk.ssc_at_clk_id));

      _TRY(nErr,DalClock_GetClockId(qdss.ctxt.clk.hClock,
                                   "island_scc_q6_atbm_clk",
                                    &qdss.ctxt.clk.ssc_q6_atbm_clk_id));

      _TRY(nErr,DalClock_GetClockId(qdss.ctxt.clk.hClock,
                                   "island_scc_pclkdbg_clk",
                                    &qdss.ctxt.clk.ssc_pclkdbg_clk_id));

      _TRY(nErr,DalClock_GetClockId(qdss.ctxt.clk.hClock,
                                   "island_scc_dbg_tsctr_clk",
                                   &qdss.ctxt.clk.ssc_dbg_tsctr_clk_id));
   }

   TRY(nErr,DalClock_SetClockFrequency(qdss.ctxt.clk.hClock,qdss.ctxt.clk.ssc_at_clk_id, 1, CLOCK_FREQUENCY_HZ_AT_LEAST, NULL));
   TRY(nErr,DalClock_EnableClock(qdss.ctxt.clk.hClock,qdss.ctxt.clk.ssc_at_clk_id));

   TRY(nErr,DalClock_SetClockFrequency(qdss.ctxt.clk.hClock,qdss.ctxt.clk.ssc_q6_atbm_clk_id, 1, CLOCK_FREQUENCY_HZ_AT_LEAST, NULL));
   TRY(nErr,DalClock_EnableClock(qdss.ctxt.clk.hClock,qdss.ctxt.clk.ssc_q6_atbm_clk_id));

   TRY(nErr,DalClock_SetClockFrequency(qdss.ctxt.clk.hClock,qdss.ctxt.clk.ssc_pclkdbg_clk_id, 1, CLOCK_FREQUENCY_HZ_AT_LEAST, NULL));
   TRY(nErr,DalClock_EnableClock(qdss.ctxt.clk.hClock,qdss.ctxt.clk.ssc_pclkdbg_clk_id));

   TRY(nErr,DalClock_SetClockFrequency(qdss.ctxt.clk.hClock,qdss.ctxt.clk.ssc_dbg_tsctr_clk_id, 1, CLOCK_FREQUENCY_HZ_AT_LEAST, NULL));
   TRY(nErr,DalClock_EnableClock(qdss.ctxt.clk.hClock,qdss.ctxt.clk.ssc_dbg_tsctr_clk_id));
...
int _QDSSLocalClkOff(void)
{
   ...
   TRY(nErr,DalClock_DisableClock(qdss.ctxt.clk.hClock,qdss.ctxt.clk.ssc_at_clk_id));

   TRY(nErr,DalClock_DisableClock(qdss.ctxt.clk.hClock,qdss.ctxt.clk.ssc_q6_atbm_clk_id));

   TRY(nErr,DalClock_DisableClock(qdss.ctxt.clk.hClock,qdss.ctxt.clk.ssc_pclkdbg_clk_id));

   TRY(nErr,DalClock_DisableClock(qdss.ctxt.clk.hClock,qdss.ctxt.clk.ssc_dbg_tsctr_clk_id));
   CATCH(nErr) {}
   return nErr;
}
