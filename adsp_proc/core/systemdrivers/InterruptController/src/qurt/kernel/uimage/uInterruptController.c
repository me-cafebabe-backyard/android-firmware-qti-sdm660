...
$Header: //components/rel/core.qdsp6/1.0.c2/systemdrivers/InterruptController/src/qurt/kernel/uimage/uInterruptController.c#1 $
...
int32 uInterruptController_Init(void)
{
  ...
  for(index =0;index < MAX_NUMBER_OF_UINTS; index++)
  {
    ...
    uIntData.state[index].nAonID = UINT_NOAONID;
  }
                            ...
                            uIRQ isr,uIRQCtx param, uint32 nFlags, uint16 nAonID)
{
   ...
   * Set Always on ID
   */
  uIntData.state[UINT_IDX(nInterruptID)].nAonID = nAonID;
...
uInterruptController_UnRegister(uint32 nInterruptID)
{
  ...
  {
    /* Deregister */
    HAL_aonint_Disable ( uIntData.state[UINT_IDX(nInterruptID)].nAonID );
    uIntData.state[UINT_IDX(nInterruptID)].nAonID = UINT_NOAONID;
  }
