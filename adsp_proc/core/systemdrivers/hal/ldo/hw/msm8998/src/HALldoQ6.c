...
$Header: //components/rel/core.qdsp6/1.0.c2/systemdrivers/hal/ldo/hw/msm8998/src/HALldoQ6.c#1 $
...
// FV - HW programming requires investigation and verification effort.
#if 0
  HWIO_OUT(LPASS_QDSP6SS_LDO_CFG0, 0x1004000);
  HWIO_OUT(LPASS_QDSP6SS_LDO_CFG1, 0x50);
  HWIO_OUT(LPASS_QDSP6SS_LDO_CFG2, 0xB068);
#endif
...
// FV - HW programming requires investigation and verification effort.
  ...
  if(nQ6PowerCtl & HWIO_FMSK(LPASS_QDSP6SS_PWR_CTL, BHS_ON))
  {
    /*
     * The mode bit inside LDO_CTL1 needs to be set during the switch.
     */
    nLDOCTL1Val = HWIO_INF(LPASS_QDSP6SS_LDO_CFG0, LDO_CTL1);
    HWIO_OUTF(
     ...
     */
    nQ6PowerCtl &= ~HWIO_FMSK(LPASS_QDSP6SS_PWR_CTL, BHS_ON);
    nQ6PowerCtl &= ~HWIO_FMSK(LPASS_QDSP6SS_PWR_CTL, LDO_BYP);
...
// FV - HW programming requires investigation and verification effort.
  ...
  if ((nQ6PowerCtl & HWIO_FMSK(LPASS_QDSP6SS_PWR_CTL, BHS_ON)) == 0)
  {
    /*
     ...
     * Turn off LDO.
     */
    nQ6PowerCtl &= ~HWIO_FMSK(LPASS_QDSP6SS_PWR_CTL, LDO_PWR_UP);
    HWIO_OUT(LPASS_QDSP6SS_PWR_CTL, nQ6PowerCtl);
  }
#endif
...
** =========================================================================*/
/**
  Set the LPASS Q6 LDO to required voltage.
...
// FV - HW programming requires investigation and verification effort.
  ...
  HWIO_OUTF(LPASS_QDSP6SS_LDO_VREF_SET, VREF_LDO, nLDOVoltageData);
#endif
...
** =========================================================================*/
/**
  Set the LPASS Q6 LDO retention voltage.
...
// FV - HW programming requires investigation and verification effort.
   ...
   */
  nLDOVoltageData = HAL_LDO_UV_MAP_TO_HW(nLDOVoltageUV);
  HWIO_OUTF(LPASS_QDSP6SS_LDO_VREF_SET, VREF_RET, nLDOVoltageData);
#endif
...
)
{
  ...
  /*
   * Install all LPASS LDOs.
   */
  HAL_ldo_InstallLDO(HAL_LDO_CPU_0, &HAL_ldo_mQ6LDOControl);

} /* END HAL_ldo_PlatformInitLPASSMain */
