...
static uint32
GPIOMgr_DeviceDetach(DalDeviceHandle* h)
{
...
static DALResult
GPIOMgr_DeviceInit(DalDeviceHandle *h)
{
...
static DALResult
GPIOMgr_DeviceDeInit(DalDeviceHandle *h)
{
   return DAL_SUCCESS;
}
...
static DALResult
GPIOMgr_DevicePowerEvent(DalDeviceHandle *h, DalPowerCmd PowerCmd,
...
static DALResult
GPIOMgr_DeviceOpen(DalDeviceHandle* h, uint32 mode)
{
  return DAL_SUCCESS;
}
...
static DALResult
GPIOMgr_DeviceClose(DalDeviceHandle* h)
{
   return DAL_SUCCESS;
}
...
static DALResult
GPIOMgr_DeviceInfo(DalDeviceHandle* h, DalDeviceInfo* info, uint32 infoSize)
{
   info->Version = GPIOMGR_INTERFACE_VERSION;
   return DAL_SUCCESS;
}
...
static DALResult
GPIOMgr_DeviceSysRequest(DalDeviceHandle* h,DalSysReq ReqIdx,
...
GPIOMgr_InitInterface(GPIOMgrClientCtxt* pclientCtxt)
{
   ...
   pclientCtxt->GPIOMgrIFHandle.dwVtblen = 15;
...
DALResult GPIOMgr_LockGPIO(DalDeviceHandle * h, uint32 nGPIO, uint32 nPD)
{
   GPIOMgrDevCtxt *dCtxt = NULL;
   DALResult eResult = DAL_ERROR;
...
DALResult GPIOMgr_ReleaseGPIO(DalDeviceHandle * h, uint32 nGPIO)
{
   GPIOMgrDevCtxt *dCtxt = NULL;
   DALResult eResult = DAL_ERROR;
...
DALResult GPIOMgr_GetGPIOCurrentPD(DalDeviceHandle * h, uint32 nGPIO, uint32* pnPD)
{
   GPIOMgrDevCtxt *dCtxt = NULL;
   DALResult eResult = DAL_ERROR;
        ...
        ( dCtxt->pState[nGPIO].nFlags & GPIOMGR_FLAG_INUSE ) )
   {
     ...
     *pnPD = (uint32)dCtxt->pState[nGPIO].nPD;
     eResult = DAL_SUCCESS;
   }
...
DALResult GPIOMgr_GetDirectConnectGPIO(DalDeviceHandle * h, uint32 nGPIO, uint32* pDirConnID)
{
    GPIOMgrDevCtxt *dCtxt = NULL;
    DALResult eResult = DAL_ERROR;
    uint32 nDirConn = 0;
    ...
    if ( ( nGPIO < dCtxt->nMaxGpios ) )
    {
        for ( nDirConn = 0; nDirConn < dCtxt->nDirConnMapSize; nDirConn++ )
        {
                 ...
                 ( dCtxt->pDirConnMap[nDirConn].nReservedPD == nCurrPD ) )
            {
                ...
                *pDirConnID = (uint32)dCtxt->pDirConnMap[nDirConn].nDirConnID;
                eResult = DAL_SUCCESS;
                break;
            }
        }
...
DALResult GPIOMgr_ReleaseDirectConnectGPIO(DalDeviceHandle * h, uint32 nGPIO, uint32 nDirConnID)
{
    GPIOMgrDevCtxt *dCtxt = NULL;
    DALResult eResult = DAL_ERROR;
    uint32 nDirConn = 0;
...
DALResult GPIOMgr_GetDirectConnectInterruptID(DalDeviceHandle * h, uint32 nGPIO, uint32* pInterruptID)
{
    GPIOMgrDevCtxt *dCtxt = NULL;
    DALResult eResult = DAL_ERROR;
    uint32 nDirConn = 0;
         ...
         ( dCtxt->pState != NULL ) )
    {
        for ( nDirConn = 0; nDirConn < dCtxt->nDirConnMapSize; nDirConn++ )
        {
            if ( dCtxt->pDirConnMap[nDirConn].nGPIO == nGPIO )
            {
                *pInterruptID = (uint32)dCtxt->pDirConnMap[nDirConn].nInterruptID;
                eResult = DAL_SUCCESS;
                break;
            }
        }
