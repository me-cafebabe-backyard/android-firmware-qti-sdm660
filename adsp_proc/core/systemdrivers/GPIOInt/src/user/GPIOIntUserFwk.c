...
$Header: //components/rel/core.qdsp6/1.0.c2/systemdrivers/GPIOInt/src/user/GPIOIntUserFwk.c#1 $
...
static DALResult 
GPIOInt_GPIOInt_RegisterIsrEx( DalDeviceHandle * h, uint32  gpio, GPIOIntTriggerType trigger, 
                             GPIOINTISR isr,GPIOINTISRCtx param, uint32 nFlags) 
{
  return GPIOIntr_RegisterIsrEx(((GPIOIntHandle *)h)->pClientCtxt, gpio, trigger, isr, param, nFlags);
}
...
static DALResult 
GPIOInt_GPIOInt_IsInterruptRegistered( DalDeviceHandle * h, uint32  gpio, void *  state, uint32  size) 
{
  return FALSE;
}
static DALResult 
...
static DALResult 
GPIOInt_GPIOInt_DisableGPIOInterrupt( DalDeviceHandle * h, uint32 gpio) 
{
    return DAL_ERROR;
}

static DALResult 
GPIOInt_GPIOInt_EnableGPIOInterrupt( DalDeviceHandle * h, uint32 gpio) 
{
	return DAL_ERROR;
}
