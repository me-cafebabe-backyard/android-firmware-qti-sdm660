...
$Header: //components/rel/core.qdsp6/1.0.c2/systemdrivers/GPIOInt/src/kernel/GPIOInt_SummaryKernel.c#1 $
...
)
{
    ...
    for(gpio = 0; gpio < device->gpioint_cntrl->gpio_number; gpio++)
    {
          ...
          nPD != 0 )
      {
        if(HAL_gpioint_IsPending(gpio) == TRUE)
        {
          ...
          /* 
           * Clear the interrupt at PMU level
           */
		  DalInterruptController_InterruptClear(device->intr_handle,
		        (DALInterruptID)device->gpioint_cntrl->summary_intr_id);
		  /* 
  ...
  if(retry_count >= GPIOINT_SUMMARY_INTR_NO_OF_TRIALS)
  {
    ...
    DALSYS_LogEvent(device->DevId, DALSYS_LOGEVENT_FATAL_ERROR, 
          "GPIOIntr_Isr: GPIO IRQ is wrongly targeted to SLPI QDSP by another Subsystem"); 
  }
