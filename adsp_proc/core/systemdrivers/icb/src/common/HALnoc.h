...
$Header: //components/rel/core.qdsp6/1.0.c2/systemdrivers/icb/src/common/HALnoc.h#1 $
...
typedef enum
{
  NOC_EXT_SOCKETQOSEN = (1 << 0),
  NOC_EXT_EXTTHREN    = (1 << 1),
  NOC_EXT_INTCLKEN    = (1 << 2),
  NOC_EXT_EXTLIMITEN  = (1 << 3),
} HAL_noc_ExtControlFlagsType;

/* Qos external flags structure */
typedef struct
{
  uint32_t uMask;  /**< Bitmask of the flags to change. */
  uint32_t uFlags; /**< Bitmask of the flag values to use. */
}
HAL_noc_QosExtControlType;
...
/**
@brief Set the Qos bandwidth level for the given QoS port number.
...
/**
@brief Set the Qos external control flags for the given QoS port number.

@param[in] pInfo -       A pointer to the NOC info structure
@param[in] uMasterPort - The port number of the master Qos to modify
@param[in] pExtControl - The external control flags to modify

@pre HAL_noc_Init() must have been called.

@sa HAL_noc_Init()
*/
void HAL_noc_SetQosExtControl( HAL_noc_InfoType *         pInfo,
                               uint32_t                   uMasterPort,
...
/**
@brief Get the Qos external control flags for the given QoS port number.

@param[in] pInfo -       A pointer to the NOC info structure
@param[in] uMasterPort - The port number of the master Qos to view
@param[in] pExtControl - A location to store the flags

@pre HAL_noc_Init() must have been called.

@sa HAL_noc_Init()
*/
void HAL_noc_GetQosExtControl( HAL_noc_InfoType *         pInfo,
                               uint32_t                   uMasterPort,
                               HAL_noc_QosExtControlType *pExtControl );
