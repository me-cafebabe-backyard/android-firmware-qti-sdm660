...
$Header: //components/rel/core.qdsp6/1.0.c2/systemdrivers/icb/src/common/ABT_dal.c#1 $ 
...
ABT_error_type ABT_Platform_Init( ABT_propdata_type * PropData )
{
  ...
  ClockIdType nId;
  uint32 i, result_freq;
  ClockPowerDomainIdType hPwr;
  DALResult eResult;
  ...
  {
    if(PropData->slave_info[i].pwr_domain_name != NULL)
    {
       //Get power domain to enable GDS
       if(DAL_SUCCESS != DalClock_GetPowerDomainId(pHandle, PropData->slave_info[i].pwr_domain_name, &hPwr))
        {
       ...
       //Enable GDS
      if(DAL_SUCCESS != DalClock_EnablePowerDomain(pHandle, hPwr))
      {
  ...
  for(i = 0; i < PropData->len; i++)
  {     
      if(PropData->slave_info[i].clk_name == NULL)
      {
          continue;
      }
      ...
      eResult  = DalClock_SetClockFrequency(pHandle, nId, 0x1, CLOCK_FREQUENCY_HZ_AT_LEAST, &result_freq);
      if((DAL_SUCCESS != eResult) && (DAL_ERROR_NOT_SUPPORTED != eResult))
      {
          ERR_FATAL("ABTimeout: Could not set clock frequency.", 0, 0, 0);
          return ABT_ERROR_INIT_FAILURE;
      }
...
ABT_error_type ABT_Platform_Deinit( ABT_propdata_type * PropData )
{
  ...
  {     
      ...
      if(DAL_SUCCESS != DalClock_DisableClock(pHandle, nId))
      {
  ...
  for(i = 0; i < PropData->len; i++)
  {
     if(PropData->slave_info[i].pwr_domain_name != NULL)
     {
        ...
        //Disable GDS
        if(DAL_SUCCESS != DalClock_DisablePowerDomain(pHandle, hPwr))
        {
