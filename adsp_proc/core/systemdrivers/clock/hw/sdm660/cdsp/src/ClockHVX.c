...
/*  
  ...
  ==================================================================== 
  $Header: //components/rel/core.qdsp6/1.0.c2/systemdrivers/clock/hw/sdm660/cdsp/src/ClockHVX.c#1 $
  $DateTime: 2020/02/10 01:57:30 $
  $Author: pwbldsvc $

  when       who     what, where, why
  --------   ---     -------------------------------------------------
  12/09/16   spl     Created for TURING on SDM660.
...
)
{
  ...
  if (nState == CLOCK_SOURCE_REQUIRED)
  {
     ...
     */
    HWIO_OUT(TURING_QDSP6SS_CP_CLK_CTL, 0x0);

    /*
     * Apply coprocessor reset.
     */
    HWIO_OUT(TURING_QDSP6SS_CP_RESET, 0x1);

    /*
     * Poll QDSP6SS_CP_PWR_STATUS bit ENR_ACK are both zero.
     */
    while(0 != (HWIO_INM(TURING_QDSP6SS_CP_PWR_STATUS,
                   HWIO_TURING_QDSP6SS_CP_PWR_STATUS_ENR_ACK_BMSK)))
    {
      DALSYS_BusyWait(2);
    }
     ...
     */
    HWIO_OUT(TURING_QDSP6SS_CP_PWR_CTL, 0x3);

    while(HWIO_INF(TURING_QDSP6SS_CP_PWR_STATUS, ENR_ACK) == 0)
    {
      DALSYS_BusyWait(2);
    }
     ...
     */
    HWIO_OUT(TURING_QDSP6SS_CP_PWR_CTL, 0x2);

    /*
     * Release the coprocessor out of reset.
     */
    HWIO_OUT(TURING_QDSP6SS_CP_RESET, 0x0);

    /*
     * Enable coprocessor clock.
     */
    HWIO_OUT(TURING_QDSP6SS_CP_CLK_CTL, 0x1);
  ...
  else
  {
     ...
     */
    HWIO_OUT(TURING_QDSP6SS_CP_PWR_CTL, 0x1);
