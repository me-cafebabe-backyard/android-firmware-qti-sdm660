...
*/
/**
  @file ClockTURINGDCVS.c 
  
  DCVS NPA node definitions for SDM660 TURING clock driver.
*/
/*
  ====================================================================
  ...
  ==================================================================== 
  $Header: //components/rel/core.qdsp6/1.0.c2/systemdrivers/clock/hw/sdm660/cdsp/src/ClockTURINGDCVS.c#1 $
 ...
 * Clock ID's
 */
#define CLOCK_ID_CPU   "turing_q6core"
...
/*
 * NPA remote resources used on TURING
 */
...
)
{
  ...
  {
    /*
    * CX-iPeak Mitigation Vote
    */
    if (pImageCtxt->pBSPConfig->bEnableCXiPeakLM && 
	     pNewConfig->Mux.eVRegLevel == VCS_CORNER_TURBO)
    {
  ...
  if (!(pImageCtxt->CPUCtxt.nDisableDCS & CLOCK_FLAG_DISABLED_HW_LOGIC))
  {
    ...
    {
      HWIO_OUT(TURING_QDSP6SS_STRAP_ACC, pNewConfig->nStrapACCVal);
    }
  ...
  if (pNewConfig->Mux.eVRegLevel < pPreviousConfig->Mux.eVRegLevel)
  {
     ...
     */
    if (pImageCtxt->pBSPConfig->bEnableCXiPeakLM &&
	     pPreviousConfig->Mux.eVRegLevel == VCS_CORNER_TURBO)
    {
...
)
{
  ...
  for ( ; nPL < pCPUPerfConfig->nMaxPerfLevel; nPL++)
  {
    ...
    if ((pCPUConfig[nConfig].Mux.nFreqHz >= nFreqHz) && 
        Clock_IsBSPSupported(&pCPUConfig[nConfig].Mux.HWVersion))
    {
      break;
    }
  }
