...
/**
  ...
  Main entry point for the SDM660 LPASS clock driver.
*/
/*
  ====================================================================
  ...
  ==================================================================== 
  $Header: //components/rel/core.qdsp6/1.0.c2/systemdrivers/clock/hw/sdm660/adsp_slpi/src/ClockLPASS.c#1 $
  $DateTime: 2020/02/10 01:57:30 $
  $Author: pwbldsvc $
 
  when       who     what, where, why
  --------   ---     -------------------------------------------------
  05/24/17   shm     Added Clock_SetOverride for WA to set proper
                     PLL override bits
  11/11/16   shm     Remove LPAPLL2 active config assignemnt. It would
                     be set by Clock_InitSource from ClockSourcesToInit.
                     NPA BIST framework is added.
  10/25/16   shm     Enabling uClock driver initialization
...
)
{
  ...
  if (DAL_SUCCESS == Clock_GetClockId(
                       pDrvCtxt, "audio_wrapper_aon_clk", &nAONBusClkId))
  {
    ...
    if (DAL_SUCCESS != eRes)
    {
      DALSYS_LogEvent(
        DALDEVICEID_CLOCK, DALSYS_LOGEVENT_FATAL_ERROR,
        "Clock_InitImage: Failed to enable the audio_wrapper_aon_clk clock.\
         Error code: %x", eRes);
      return eRes;
    }
  }
   ...
   */
  if (DAL_SUCCESS == Clock_GetClockId(
                       pDrvCtxt, "audio_core_core_clk",
                       &nCoreClkId))
  {
    Clock_EnableClock(Clock_GetInitialClientCtxt(), nCoreClkId);

    if (DAL_SUCCESS == Clock_GetClockId(
                         pDrvCtxt, "audio_core_sysnoc_mport_core_clk",
                         &nMportClkId))
    {
      Clock_EnableClock(Clock_GetInitialClientCtxt(), nMportClkId);

      if (DAL_SUCCESS == Clock_GetPowerDomainId(
                           pDrvCtxt, "VDD_AUDIO_CORE", &nPowerDomain))
      {
  ...
  /*-----------------------------------------------------------------------*/
  /* Set a indefinite vote for the digital PLL. This PLL will remain on    */
  /* until the SPM collapses it for power collapse in order to mitigate    */
  /* extra delays caused by enabling the PLL via vote.                     */
  /*-----------------------------------------------------------------------*/

  HWIO_OUTF(LPASS_LPA_PLL_VOTE_LPASS_HW, LPAAUDIO_DIG_PLL, 0x1);

  /*-----------------------------------------------------------------------*/
  /* Run the NPA BIST (if enabled) for Q6 switching                        */
  /*-----------------------------------------------------------------------*/
...
{
  /* Q6 is running on Spark PLL */
  if (HWIO_INF(LPASS_QDSP6SS_CORE_CFG_RCGR, SRC_SEL) == 1)
  {
    ...
    if ((HWIO_INF(LPASS_QDSP6SS_SPM_EVENT_CTL, PLL_OVRRD) == 1) ||
        (HWIO_INF(LPASS_QDSP6SS_SPM_EVENT_CTL, EXT_PLL_OVRRD) == 0))
    {
      DALSYS_LogEvent(
        DALDEVICEID_CLOCK, DALSYS_LOGEVENT_FATAL_ERROR,
        "Wrong SPM_EVENT_CTL setting when Q6 is on Spark: 0x%x",
        HWIO_IN(LPASS_QDSP6SS_SPM_EVENT_CTL));
    }
  }
  
  /* Q6 is running on Peel PLL */
  else if (HWIO_INF(LPASS_QDSP6SS_CORE_CFG_RCGR, SRC_SEL) == 4)
  {
    ...
    if ((HWIO_INF(LPASS_QDSP6SS_SPM_EVENT_CTL, PLL_OVRRD) == 0) ||
        (HWIO_INF(LPASS_QDSP6SS_SPM_EVENT_CTL, EXT_PLL_OVRRD) == 1))
    {
      DALSYS_LogEvent(
        DALDEVICEID_CLOCK, DALSYS_LOGEVENT_FATAL_ERROR,
        "Wrong SPM_EVENT_CTL setting when Q6 is on Peel: 0x%x",
        HWIO_IN(LPASS_QDSP6SS_SPM_EVENT_CTL));
    }
  }
