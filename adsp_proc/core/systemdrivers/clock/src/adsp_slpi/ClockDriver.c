...
/*
  ...
  $Header: //components/rel/core.qdsp6/1.0.c2/systemdrivers/clock/src/adsp_slpi/ClockDriver.c#1 $
...
)
{
  ...
  uint32                         nFreqHz;
  uint32                         nSrcIdx;
  ClockMuxConfigType            *pMatchedConfig;
         ...
         (nClockDomainIndex < pDrvCtxt->nNumClockDomains))
  {
        ...
        (pDrvCtxt->anSourceIndex[ClockMuxConfig.eSource] != 0xFF))
    {
      ...
      /* Calculate clocks frequency in this domain */
      nSrcIdx = pDrvCtxt->anSourceIndex[ClockMuxConfig.eSource];
      nFreqHz = pDrvCtxt->aSources[nSrcIdx].pActiveFreqConfig->nFreqHz;
      if (ClockMuxConfig.nDiv2x > 2)
      {
        nFreqHz = 2 * (nFreqHz / ClockMuxConfig.nDiv2x);
      }

      if (ClockMuxConfig.nM > 1)
      {
        nFreqHz *= ClockMuxConfig.nM;
      }

      if (ClockMuxConfig.nN > 1)
      {
        nFreqHz /= ClockMuxConfig.nN;
      }
    }
    ...
    /*
     * Initialize clock ActiveMuxConfig if its source is configured and a matching
     * configuration is found in the BSP.
     */
    if ((pDrvCtxt->aClockDomains[nClockDomainIndex].pBSPConfig != NULL) &&
        (nFreqHz != 0))
    {
      eResult = Clock_FindClockConfig(pDrvCtxt->aClockDomains[nClockDomainIndex].pBSPConfig,
                                      nFreqHz,
                                      CLOCK_FREQUENCY_HZ_EXACT,
                                      &pMatchedConfig);
      if (eResult == DAL_SUCCESS)
      {
        pDrvCtxt->aClockDomains[nClockDomainIndex].pActiveMuxConfig = pMatchedConfig;
      }
    }
...
)
{
  ...
  /* Error if clock's has a BSP config but its ActiveMuxConfig not set */
  if ((pClockDomain->pBSPConfig != NULL) && (pClockDomain->pActiveMuxConfig == NULL))
  {
...
)
{
  ...
  if ((pClockDomain->nReferenceCount + pClockDomain->nReferenceCountSuppressible) == 1)
  {
    ...
    {
      if (HAL_clk_IsClockDomainSupported(pClockDomain->HALHandle, HAL_CLK_CLOCK_DOMAIN_API_ISON))
      {
...
)
{
  ...
  if (pNewSource != NULL && pNewConfig->pSourceFreqConfig != NULL)
  {
   ...
   if(CLOCK_FLAG_IS_SET(pDomain,DOMAIN_NON_MND_SWITCH))
   {
     /*
      * Enable the current source and new source before switch
      */
	 if(pDomain->pSource != NULL)
	 {
       eResult = Clock_EnableSourceInternal(pDrvCtxt, pDomain->pSource, FALSE);
  ...
  {
    /*
     * Remove the votes put before switch
     */
	if(pDomain->pSource != NULL)
	{
      eResult = Clock_DisableSourceInternal(pDrvCtxt, pDomain->pSource, FALSE, FALSE);
...
)
{
  ...
  if (TRUE == HAL_clk_IsClockSupported(pClock->HALHandle, HAL_CLK_CLOCK_API_INVERT))
  {
    DALCLOCK_LOCK(pDrvCtxt);

    HAL_clk_InvertClock(pClock->HALHandle, bInvert);
...
)
{
  ...
  *pnFrequencyHz = 0;
  ...
  if (pClock->pDomain->pBSPConfig != NULL)
  {
    ...
    {
      eResult = DAL_ERROR_NOT_INITIALIZED;
    }
  }
  else
  {
...
)
{
  ...
  /*-----------------------------------------------------------------------*/
  /* Make sure the clock has a valid test clock ID before calculating.     */
  /*-----------------------------------------------------------------------*/
...
)
{
  ...
  if (TRUE == HAL_clk_IsClockSupported(pClock->HALHandle, HAL_CLK_CLOCK_API_CONFIG))
  {
    /*-----------------------------------------------------------------------*/
...
)
{
  ...
  for (i = 0; i < pDrvCtxt->nNumClocks; i++)
  {
    ...
    if (bCalcSupported)
    {
       ...
       */
      Clock_SWEvent(
        CLOCK_EVENT_CLOCK_LOG_STATE,
        4,
        HAL_clk_GetTestClockId(pClock->HALHandle),
        nIsOn, nReset, nFreqHz);
    ...
    else
    {
       ...
       */
      Clock_SWEvent(
        CLOCK_EVENT_CLOCK_LOG_STATE_NF,
        3,
        HAL_clk_GetTestClockId(pClock->HALHandle),
        nIsOn, nReset);
