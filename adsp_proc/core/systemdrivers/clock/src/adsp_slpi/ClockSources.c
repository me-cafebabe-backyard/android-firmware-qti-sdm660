...
/*
  ...
  $Header: //components/rel/core.qdsp6/1.0.c2/systemdrivers/clock/src/adsp_slpi/ClockSources.c#1 $
...
)
{
  ...
  {
    HAL_clk_ConfigSource(eSource, HAL_CLK_CONFIG_XO_DOUBLER_DCC);
  }
  ...
  for (nSourceIndex = 0; nSourceIndex < pDrvCtxt->nNumSources; nSourceIndex++)
  {
    ...
    if (DetectedConfig.eSource != HAL_CLK_SOURCE_NULL)
    {
      ...
      while(pSourceFreqConfig->nFreqHz != 0)
      {
         ...
         */
        if (/* To detect PLL if it runs on different sources(X0,Sleep).
            DetectedConfig.eSource  == pSourceFreqConfig->HALConfig.eSource  &&             */
    ...
    else
    {
      DALSYS_LogEvent(
        ...
        "Unable to match source[%s] detected HW state with BSP frequency config.",
        pSource->szName);
    }
     ...
     */
    pSourceFreqConfig = pSource->pBSPConfig->pCalibrationFreqConfig;
    if (pSourceFreqConfig)
    {
      ...
      if (pSource->pCalibrationFreqConfig == NULL)
      {
        DALSYS_LogEvent(
          ...
          "Unable to detect calibration frequency config for source[%s].",
        pSource->szName);
