...
$Header: //components/rel/core.qdsp6/1.0.c2/mproc/ipc_router/src/ipc_router_qdi_driver.c#1 $
...
/*===========================================================================
FUNCTION      ipc_router_qdi_poll_check_events

DESCRIPTION   Checks if there are events available
              descriptor (Similar to the standard UNIX poll() method.

ARGUMENTS     h - The calling client's handle
              pfd - pollfd structure (See ipc_router_qdi_poll_fd)

RETURN VALUE  +Ve number if events are pending otherwise zero. Negative error
              code on failure.

SIDE EFFECTS  None
===========================================================================*/
static int ipc_router_qdi_poll_check_events
(
 ipc_router_qdi_handle_t *h,
 ipc_router_qdi_poll_fd *pfd
)
{
  ipc_router_os_sem_lock(&h->clients_lock);

  if(h->active_receivers <= 0 || LIST_CNT(h->clients) == 0)
  {
    pfd->revents = IPC_QDI_POLLNVAL;
    ipc_router_os_sem_unlock(&h->clients_lock);
    return IPC_ROUTER_QDI_BADF_ERR;
  }
  ...
  {
    pfd->revents |= IPC_QDI_POLLHUP;
  }

  ipc_router_os_sem_lock(&h->rx_q_lock);
  if(pfd->events & IPC_QDI_POLLIN)
  {
    if(LIST_CNT(h->rx_q) > 0)
    {
      pfd->revents |= IPC_QDI_POLLIN;
    }
  }
  ipc_router_os_sem_unlock(&h->rx_q_lock);

  if(pfd->events & IPC_QDI_POLLOUT)
  {
    pfd->revents |= IPC_QDI_POLLOUT;
  }
  return pfd->revents == 0 ? 0 : 1;
}
...
(
 int client_handle,
 ipc_router_qdi_handle_t *handle,
 ipc_router_qdi_poll_fd *poll_fds,
 uint32_t len,
 int timeout_ms
)
{
  ...
  int ret = 0, i;
  ipc_router_qdi_waiter_t waiter;
  ipc_router_qdi_handle_t **handles;
  ipc_router_qdi_poll_fd *pfd;
  ...
  /* Allocate memory for handles and polling fds */
  pfd = ipc_router_os_calloc(1, sizeof(ipc_router_qdi_poll_fd) * len + sizeof(ipc_router_qdi_handle_t*) * len);
  if (pfd == NULL)
  {
    return IPC_ROUTER_QDI_NO_MEM_ERR;
  }
  
  handles = (ipc_router_qdi_handle_t **)(&pfd[len]);

  /* Copy fds to local buffer */  
  rc = qurt_qdi_copy_from_user(client_handle, pfd, poll_fds, sizeof(ipc_router_qdi_poll_fd) * len);
  if(rc)
  {
    ipc_router_os_free(pfd);
    return IPC_ROUTER_QDI_PERM_ERR;
  }
    
  /* Take ref to all poll fds */
    for(i = 0; i < len; i++)
    {
      pfd[i].revents = 0;
      if(pfd[i].fd < 0)
    ...
    handles[i] = (ipc_router_qdi_handle_t *)qurt_qdi_objref_get_from_handle(QDI_HANDLE_LOCAL_CLIENT, pfd[i].fd);
    if(!handles[i] || handles[i]->obj.invoke != ipc_router_qdi_main_invocation)
      {
    ...
    rc = ipc_router_qdi_poll_check_events(handles[i], &pfd[i]);
    if (rc < 0)
      {
      ret = rc;
      goto REFREL_RETURN;
...
REFREL_RETURN:
  for(i = 0; i < len; i++)
  {
    ...
    if (handles[i])
    {
      qurt_qdi_objref_release(&handles[i]->obj);
    }
  }

  rc = qurt_qdi_copy_to_user(client_handle, poll_fds, pfd, sizeof(ipc_router_qdi_poll_fd) * len);
  if (rc)
  {
    ret = rc;
  }
