...
#define COPY_BYTE_OFF(dest, src, off) \
  *(((uint8_t *)dest) + off) = *(((uint8_t *)src) + off)

#define COPY_BYTE(dest, src) COPY_BYTE_OFF(dest, src, 0)

#define COPY_SHORT(dest, src) do { \
  COPY_BYTE_OFF(dest, src, 0); \
  COPY_BYTE_OFF(dest, src, 1); \
} while(0)

#define COPY_WORD(dest, src) do { \
  COPY_BYTE_OFF(dest, src, 0); \
  COPY_BYTE_OFF(dest, src, 1); \
  COPY_BYTE_OFF(dest, src, 2); \
  COPY_BYTE_OFF(dest, src, 3); \
} while(0)
  ...
  *pp_element = p_element;
  ...
  switch( flags & QMI_IDL_FLAGS_TYPE ) 
  {
    ...
    case QMI_IDL_GENERIC_8_BYTE:
      ...
      QMI_IDL_MEM_COPY( p_dst, length, *pp_src, length );
      /* Range Checking */
      if (flags & (QMI_IDL_FLAGS_RANGE_CHECKED << 16))
      {
        int8_t ret_val = 0;
        ...
        for(i=0; i<length; i++)
        {
          ...
          QMI_IDL_MEM_COPY( &tmp_val, sizeof(tmp_val), *pp_src, (1 << (flags & QMI_IDL_FLAGS_TYPE)));
          ...
          }else if(ret_val == QMI_IDL_RANGE_RESPONSE_DEFAULT)
          {
            ...
            QMI_IDL_MEM_COPY(p_dst+offset,(1 << (flags & QMI_IDL_FLAGS_TYPE)),&tmp_val,
                             (1 << (flags & QMI_IDL_FLAGS_TYPE)));
          }
    ...
    case QMI_IDL_1_BYTE_ENUM:
      ...
      if (flags & (QMI_IDL_FLAGS_RANGE_CHECKED << 16))
      {
        ...
        for(i=0; i<length; i++)
        {
          ...
          {
            uint64_t offset = i*4;
            COPY_WORD(p_dst+offset, &tmp_val);
          }        
...
)
{
  ...
  if( (flags & QMI_IDL_FLAGS_TYPE) == QMI_IDL_STRING ) {
    ...
    QMI_IDL_MEM_COPY( *pp_dst, dst_bytes, p_src, length );
  ...
  switch( flags & QMI_IDL_FLAGS_TYPE ) {
    ...
    case QMI_IDL_GENERIC_8_BYTE:
      ...
      if (flags & (QMI_IDL_FLAGS_RANGE_CHECKED << 16))
      {
        ...
        {
          tmp_val = 0;
          QMI_IDL_MEM_COPY( &tmp_val, sizeof(tmp_val), p_src, (1 << (flags & QMI_IDL_FLAGS_TYPE)));
          ret_val = p_type_table->p_ranges[range_index].range_check(&tmp_val);
...
)
{
  ...
  if (src_len < p_msg->c_struct_sz) {
    QMI_IDL_HANDLE_ERROR( exc, QMI_IDL_LIB_BUFFER_TOO_SMALL,
                          1, dst_len, p_msg->c_struct_sz );
  }else if(p_msg->c_struct_sz == 0 || dst_len == 0 || p_dst == NULL || p_element == NULL)
  {
...
***********************************************************************/
ENCDEC_EXPORT int32_t qmi_idl_message_decode
(
  /* Pointer to service object, from auto-generated header from IDL */
  const qmi_idl_service_object_type p_service,
...
***********************************************************************/
ENCDEC_EXPORT int32_t qmi_idl_message_encode
(
  /* Pointer to service object, from auto-generated header from IDL */
  const qmi_idl_service_object_type p_service,
...
***********************************************************************/
ENCDEC_EXPORT int32_t qmi_idl_encode_resp_tlv
(
...
)
{
  ...
  COPY_SHORT(p_dst2, &result);
  COPY_SHORT(p_dst2+2, &error);
...
*/
/*=========================================================================*/
ENCDEC_EXPORT uint32_t qmi_idl_get_std_resp_tlv_len(void)
{
