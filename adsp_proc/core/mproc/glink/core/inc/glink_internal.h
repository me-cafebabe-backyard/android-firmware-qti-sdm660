...
$Header: //components/rel/core.qdsp6/1.0.c2/mproc/glink/core/inc/glink_internal.h#1 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
09/01/16   na      Updated definition of structure 'glink_link_notif_data_type'
...
08/20/15   pp      Added APIs to acquire/release per edge transport_q_cs locks
07/07/15   bc      Remove unnecessary variable in channel ctx
08/20/15   pp      Change tx_cs type to os_mutex_type
                   Fix for potential channel migration deadlock 
...
#define GLINK_XPORT_NAME_LEN 32
#define GLINK_REMOTE_SS_NAME_LEN 32
...
#define GLINK_LOG_EVENT_NO_FILTER(type, ch_name, xport, remote_ss, param) \
  glink_mem_log(type, ch_name, xport, remote_ss, (uint32)param);
...
/* Used for xport logging where channel name information is unavaliable */
#define GLINK_LOG_EVENT_NO_FILTER(type, ch_name, xport, remote_ss, param) \
  OS_LOG_MSG(5, "[%s, %s, %s, %s, %x]" , #type, ch_name,                  \
...
#define GLINK_LOG_ERROR_EVENT(type, ch_name, xport, remote_ss, param)	 \
  OS_LOG_ERROR(5, "[%s, %s, %s, %s, %x]" , #type, ch_name,               \
               xport, remote_ss, param);
#else
...
/* Magic marker expected to be present in the channel context */
#define GLINK_CHANNEL_CTX_MAGIC 0x6e496c47
...
struct glink_channel_ctx {
  ...
  *   or local side closed the port. Optional */
  ...
  /* save timeout value for remote intent wait */
  uint32                              rm_intent_timeout;

  /* Reference count for this channel */
  uint32                              ref_count;

  /* Optional client intent data memory allocator */
  glink_notify_allocate_cb            notify_allocate;

  /* Optional client intent data memory free */
  glink_notify_deallocate_cb          notify_deallocate;

  /* Magic value to verify channel validity */
  uint32                              magic;
};
...
typedef struct {
  ...
  char xport_store[GLINK_XPORT_NAME_LEN];   /* To store xport name */     
  char remote_ss_store[GLINK_REMOTE_SS_NAME_LEN];  /* To store remote_ss name */
...
===========================================================================*/
/** 
 * Update link status and scan registered link notification list to 
 * notify about xport link state change
 *
...
===========================================================================*/
/** 
 * Takes a reference on a fully opened channel context. If the channel 
 * is not fully opened, then the function returns FALSE without taking
 * a reference
 *
 * @param[in]  handle        glink channel handle
...
/*===========================================================================
  FUNCTION      glink_default_notify_rx_sigs
===========================================================================*/
/** 
 *  Default implementation of rx sigs callback in case client doesnt provide
 * 
 * @param[in]    remote_ss  Name of the remote sub system on the edge
 *
 * @return       None.
