...
class Elf_File:
    ...
    def read_elf_info(self):
        ...
        self.phdr = [p for p in self.rawphdr if p.p_type == p.PT_LOAD and p.p_memsz > 0]
        ...
        for s in salloc:
            ...
            for p in self.phdr:
                if p.p_vaddr >= s.sh_addr + s.sh_size:
                    ...
                    over.append(p)
            if len(over) == 0:
                if "clade" in s.name.lower():
                    nullphdr = Elf32ProgramHeader(None,
                                    first_alloc = QurtInfinity,
                                    first_progbits = QurtInfinity,
                                    furthest_alloc = -QurtInfinity,
                                    furthest_progbits = -QurtInfinity,
                                    section_list=[],
                                    p_align=s.sh_addralign,
                                    p_filesz=alignup(s.sh_size,0x1000),
                                    p_flags=s.sh_flags,
                                    p_memsz=alignup(s.sh_size,0x1000),
                                    p_offset=0,
                                    p_paddr=0x0,
                                    p_type=Elf32ProgramHeader.PT_NULL,
                                    p_vaddr=s.sh_addr,
                                    f="",
                                    temp=True)
                    self.phdr.append(nullphdr)
                    over.append(nullphdr)
                    s.temp=True
                else:
...
class QurtElfPatch:
    ...
    def read_pool_configs(self):
        ...
        remove_pages = 0
        for island_region in self.phys_island_regions:
            if island_region[2] & 0x10:
                # This is an island region that borrows from the end
                #  of DEFAULT_PHYSPOOL.
                remove_pages += (island_region[1] - island_region[0]) + 1
        while True:
    ...
    def move_pool_mappings(self):
        ...
        for t in w:
            ...
            self.island_moves.append([orgaddr, newpage << 12, t.size << 12, t.copybytes])

    def clade_uncomp_offset(self):
        clade_region_paddr = self.clade_base_paddr.value
        for i in self.infiles:
            for p in i.phdr:
                if p.temp and len(p.section_list):
                    p.p_paddr += clade_region_paddr
                    clade_region_paddr += 0x04000000
    ...
    def split_island_pool_wa(self):
        self.split_island_pool = []
        island_start = 0
        island_end = 0
        num_pages = 0    #number of 4k pages
        self.way_size = 0
        self.max_4k_pages = 0
        i = 0
        for m in self.phys_island_regions:
            ...
            island_start = m[0]
            island_end = m[1]
            self.way_size = (m[1] - m[0] + 1) >> 3  #size/ways 
            while(i <8):
                pool_start = (m[0]+(self.way_size*i)+ 1)  # +1 at the end skips the fist 4k page of every 64k boundary
                pool_end = (m[0]+(self.way_size*i)+self.way_size)
                num_pages = (pool_end - pool_start) #num of 4k pages
                self.max_4k_pages = num_pages
                self.split_island_pool.append([pool_start,pool_end,num_pages,m[2]]) #we store the region start, end, number of pages remaining in region and cache attributes
                i += 1
    def adjust_island_opt_mappings_wa(self):
        from copy import copy
      	w = []
        page_size_list = []
        ...
        for t in self.opt_mappings:
            if t.island:
                if (t.ASID == 0 or t.W == 0 or t.infile.restart == 0):
                    w.append(t)
                    page_size_list.append(t.size) #create list of page sizes
        for t in w:
            tmp = (t.vpage | t.size)
            tmp = (tmp & -tmp)
            t.sortkey = tmp
        ...
        key_list = list(set(page_size_list)) #create a list of keys based on page sizes

        for key in key_list:   #create a key:value pair
                               #where key is the page size. Value is a tuple
                               #first element of tuple is max allowables pages of size key
                               #second element number of pages, of size key, that are currently present
            page_list[key] = [self.max_4k_pages/key*8,page_size_list.count(key)]
        page_list = dict(sorted(page_list.items(),key=lambda x:x[0],reverse=True)) #sort based on highest sized key

        for t in w:
            if page_list[t.size][1] > page_list[t.size][0]: #if current pages exceed max allowed , break them 
                tmp = 0
                while tmp < 4:
                    tmp_t = copy(t)
                    tmp_t.vpage = t.vpage + ((t.size/4)*tmp)
                    tmp_t.ppage = t.ppage + ((t.size/4)*tmp)
                    tmp_t.size = t.size/4
                    self.opt_mappings.append(tmp_t)  #add the new broken mappings
                    tmp += 1
                self.opt_mappings.remove(t) #remove the original mapping
                if t.size > 1: #we do this only for page sizes bigger than 4k. If page size 
                               #is 4k and number of pages exceed max allowable, we will anyways
                               #fail later
                    self.adjust_island_opt_mappings_wa() #do this till we have good mappings
                break
       
    def move_island_mappings_wa(self):
        ...
        for t in w:
            moved = False
            for m in self.split_island_pool:
                if m[2] / t.size:   #see if page fits in this section of island pool
                    orgaddr = self.lookup_vma_to_lma(t.vpage << 12) + (self.virt_base - self.phys_base)
                    off_pg = m[2]/t.size
                    t.ppage = m[0] + (off_pg*t.size)-1
                    ...
                    m[2] -= t.size
                    moved = True
                    self.island_moves.append([orgaddr, use_dest, t.size << 12, t.copybytes])
                    break
            if not moved:
    ...
    def move_island_mappings(self):
        ...
        for t in w:
            moved = False
            for m in self.phys_island_regions:
                pages_avail = (m[1]-m[0]+1)
                if not moved:
                    if (m[0] & (t.sortkey-1)) == 0:
                        # Alignment checks out
                        if t.size <= pages_avail:
                            orgaddr = self.lookup_vma_to_lma(t.vpage << 12) + (self.virt_base - self.phys_base)
                            # Size checks out
                            t.ppage = m[0]
                            t.C = m[2] & 0xF
    ...
    def phys_island_report(self):
        ...
        if self.l2_cache_v61_v1.value == 'true':
            for x in self.split_island_pool:
                total_remaining_region += x[2]
        else:
    ...
    def ProcessFile(self):
        ...
        if self.CONFIG_ADVANCED_MEMORY:
            ...
            if self.phys_island_regions:
                if self.l2_cache_v61_v1.value == 'true':
                    self.split_island_pool_wa()   #split the island physpool to remove bad addresses
                    self.adjust_island_opt_mappings_wa()  #split the mappings (if required) such that they fit into the pool
                    self.move_island_mappings_wa()   #move the island mappings
                else:
        ...
        self.eip.writeback(None, infiles, self.island_moves, self.clade_regs, self.opt_mappings)
