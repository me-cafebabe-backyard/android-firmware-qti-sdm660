...
def hex_cfg_builder(target, source, env):
    ...
    for s in source[1:]:
        if ('nmi_status_sim' in s.name):
            tempstr = tempstr.replace('nmi_status_sim.so', s.get_abspath())
        else:
            tgtfile.write(s.get_abspath()+'\n')
...
def test_lowfootprint_analysis_builder(target, source, env):
    ...
    fail = False
    size = 0
    size_threshold = env.get('FOOTPRINT_THRESHOLD')
    q6_tools_root = env.get('Q6_TOOLS_ROOT')
    q6_tools_root_split = os.path.split(q6_tools_root)
    q6_tools_root_split = q6_tools_root_split[1].split(".")
    if q6_tools_root_split[0] >= 7:
        hexagon_size_exe =  os.path.join(q6_tools_root ,"Tools/bin/hexagon-size")
    else:
        hexagon_size_exe =  os.path.join(q6_tools_root ,"gnu/bin/hexagon-size")
     
    command=[hexagon_size_exe, "-A", str(source[0])]
    ...
    file = open(str(target[1]), 'w+')
    file.write("\n\nLow Footprint kernel size is "+str(size)+" KB\n")
    if fail == True:
        file.write("***EXCEEDS SIZE THRESHOLD***\n")
    file.close
