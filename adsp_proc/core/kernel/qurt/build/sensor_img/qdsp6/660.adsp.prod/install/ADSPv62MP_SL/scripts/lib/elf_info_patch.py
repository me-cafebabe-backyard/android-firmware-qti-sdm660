...
def _build_eip_info(physaddr, f, infiles, island_moves, clade_registers, opt_mappings, mappings=None):
    import struct
    ...
    for i in infiles[1:]:
        ...
        if mappings != None:
            ...
            for m in this_maps:
                ...
                if m.W.value():
                    perms += 2
                eip_data += encode_eip_record(USER_APP_MAPPING, '<LLLL', m.sh_addr, int(m.physaddr), m.sh_size, perms)
        ...
        for s in segs:
            ...
            eip_data += encode_eip_record(USER_APP_MAPPING, '<LLLL', s[0], (s[0]+s[2]-physaddr) & 0xFFFFFFFF, s[1]-s[0], perms)
        for rcopy in i.restart_copies:
            print 'USER_APP_INIT copy %X bytes from %X to %X' % (rcopy[2], rcopy[1], rcopy[0])
            eip_data += encode_eip_record(USER_APP_INIT, '<LLL', *rcopy)
...
class ElfInfoPatch:
    ...
    def writeback(self, *args):
        data = _build_eip_info(self.cfg.physaddr, *args)
