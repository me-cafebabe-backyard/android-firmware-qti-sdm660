...
class QIBSection(Elf32SectionHeader):
    ...
    def set_mapping_attributes(self):
        ...
        self.APPNAME = ConfigField(getattr(self,'appname','__tmp__'))
        print '... %s' % self.name
...
class Elf_File(object):
    def __init__(self, cfg, appname, filename, restartable):
        ...
        f.seek(eh.e_shoff)
        sections = [QIBSection(f, f=f, cfg=cfg, appname=appname, restartable=restartable, rc=self.restart_copies) for _ in range(eh.e_shnum)]
...
class QurtElfPatch:
    ...
    def eip_build_get_xml(self):
        import xml.dom.minidom
        for length, data in self.eip_build_records(2, 'L'):
            if length > len(data):
            ...
            if doc.documentElement.tagName == 'build_vars':
                ...
                for el in doc.documentElement.childNodes:
                    setattr(self, el.tagName, el.getAttribute('value'))
