...
$Header: //components/rel/core.qdsp6/1.0.c2/buses/uart_lite/src/Uart.c#1 $
...
static const CLOCK_SETTINGS baud_table[] =
{
   ...
   {     115200,    14745600,      0xAA },
   {    1000000,    48000000,      0xDD },
   {    1500000,    48000000,      0xEE },
   {    3000000,    48000000,      0xFF },
   {0}
};
...
static uint32 process_tx_data(UART_CONTEXT* uart_ctxt,char* buf, uint32 num_bytes)
{
   ...
   while (bytes_to_tx > 0)
   {
      ...
      if (bytes_to_tx == 0)
      {
         // On SLPI if we don't do busywait till all data goes out, it will power collapse and
         // and the clocks will go to sclk and data will be corrupted. To avoid that do a flush.
         flush_tx(uart_ctxt);
         return bytes_txed;
      }
...
static uint32 receive_data(UART_CONTEXT* uart_ctxt, char* buf, uint32 bytes_to_rx)
{
   ...
   if (uart_ctxt->read_buf == NULL)
   {
      ...
      {
         snap_bytes =  HALuart_GetDmaRxSnapChars(base) - uart_ctxt->rx_client_read;
         if(bytes_to_rx >= snap_bytes)
         {
         HALuart_ResetStaleInterrupt(base); // Clear the stale interrupt.
            bytes_to_rx = snap_bytes;
         is_stale_intr = TRUE;
      }
      }
      bytes_read = HALuart_ReadRxFifo(base, buf, bytes_to_rx);
      uart_ctxt->rx_bytes_left -= bytes_read;
      uart_ctxt->rx_client_read += bytes_read;
...
UartResult Uart_deinit(UartHandle h)
{
   ...
   UART_QDSS_LOG_0(UART_DEINIT_DONE);
   UART_LOG_0(INFO,"-Uart_deinit");
   return result;
}
...
UartResult Uart_init(UartHandle* h,UartPortID id)
{
   ...
   UART_QDSS_LOG_0(UART_INIT);

   if (qurt_island_get_status() == 1)
   {
      UART_LOG_0(ERROR, "Uart_init in island mode is not supported.");
      return UART_ERROR;
   }
   ...
   *h = (UartHandle)uart_ctxt;
   ...
   UART_QDSS_LOG_0(UART_INIT_DONE);
   UART_LOG_0(INFO,"-Uart_init");
   return UART_SUCCESS;
}
   ...
   if (misr)
   {
      if (uart_ctxt->client_callback)
      {
         if ((misr & UARTDM_ISR_RXSTALE) || (misr & UARTDM_ISR_RXLEV))
         {
            ...
            {
               num_bytes_available = HALuart_GetDmaRxSnapChars(base) - uart_ctxt->rx_client_read;
            }
            else
            {
            ...
            uart_ctxt->client_callback(UART_EVENT_RX, num_bytes_available, uart_ctxt->cb_data);
         }
      }
      else
      {
...
uint32 Uart_receive(UartHandle h, char* buf,uint32 bytes_to_rx)
{
   ...
   UART_QDSS_LOG_1(UART_RECEIVE_DONE, bytes_read);
   UART_LOG_0(INFO,"-Uart_receive");
...
UartResult Uart_register_event_callback(UartHandle h, UART_CALLBACK client_callback, void* cb_data)
{
   ...
   ((UART_CONTEXT*)h)->cb_data         = cb_data;
   return UART_SUCCESS;
}
...
uint32 Uart_transmit(UartHandle h, char* buf, uint32 bytes_to_tx)
{
   ...
   UART_QDSS_LOG_1(UART_TRANSMIT, bytes_to_tx);
   ...
   UART_QDSS_LOG_0(UART_TRANSMIT_DONE);
   UART_LOG_0(INFO,"-Uart_transmit");
