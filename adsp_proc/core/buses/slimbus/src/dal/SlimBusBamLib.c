...
$Header: //components/rel/core.qdsp6/1.0.c2/buses/slimbus/src/dal/SlimBusBamLib.c#1 $
...
SBBOOL SlimBus_GetOnePipeDesc(SlimBusMasterPortType *pPort)
{
  ...
  if ((NULL != pCtxt) && (NULL != pCtxt->hBAM))
  {
    if ( pCtxt->bPoll )
    {
      ...
      {
	    uint32 uUserData;
	    uUserData = (uint32 ) result.data.xfer.xfer_cb_data;
        if(uUserData == SLIMBUS_DATASHIFT_INVALIDATE)
	    {
		  pPort->bDataShiftRemoval = FALSE;
		  SlimBus_DestroyObject(pPort->hPhysMem);
		  SB_LOG_INFO_0("got he IO vector for the submitted for HW bug workaround");
    	  return FALSE;
		}
...
SlimBus_ConfigMasterPipe( DalDeviceHandle * h, SlimBusResourceHandle  hPort, SlimBusPortFlowType  eFlow, const SlimBusPipeConfigType * pConfig) 
{
  	   ...
  	   && (pPort->bDataShiftRemoval))
  {
	...
	do
    {
      result = SlimBus_PhysMemAlloc((BITS_PER_UINT32/8), &hPhysMem);
      if(SB_SUCCESS != result)
      {
        hPhysMem = NULL;
		break; 
      ...
      if (NULL != hPhysMem)
      {
        result = SlimBus_PhysMemInfo(hPhysMem, &uVirtAddr, &uPhysAddr);
        if (SB_SUCCESS != result)
        {
          SlimBus_DestroyObject(hPhysMem);
          hPhysMem = NULL;
		  break;
        }
      }
      IOVec.uAddr = uPhysAddr;
      IOVec.uSize = BITS_PER_UINT32/8;
      IOVec.uFlags = BAM_IOVEC_FLAG_INT;
	  uint32 *pUserData;
      pUserData = (void *)SLIMBUS_DATASHIFT_INVALIDATE;
	  pPort->hPhysMem = hPhysMem;

	  
      bam_result = bam_pipe_transfer(pCtxt->hBAM, IOVec.uAddr, IOVec.uSize,
                                     IOVec.uFlags,pUserData);
      if (BAM_SUCCESS != bam_result)
      {
	      SB_LOG_ERR_0("Error returned from BAM pipe transfer to remove the invalid data shift");
        result = SB_ERROR;
        break;
      }
      SB_LOG_INFO_0("Sumbitted the IO vector for the HW bug workaround");
    }while (FALSE);
  }
...
{
  return SlimBus_GetBamIOVecEx(h, hPort, eTransferDir, pIOVec, NULL);
}
