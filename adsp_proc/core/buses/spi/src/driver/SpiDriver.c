...
/*===========================================================================
  ...
  $Header: //components/rel/core.qdsp6/1.0.c2/buses/spi/src/driver/SpiDriver.c#1 $
  $DateTime: 2020/02/10 01:57:30 $
  $Author: pwbldsvc $

when         who      what, where, why
----------   ---      ------------------------------------
07/16/2015   vmk      Added clk control
...
int32 spi_open(spi_device_id_t device_id)
{
   ...
   do
   {
      ...
      if (spiDevices[device_idx].status & SPI_DEVICE_OPEN)
      {
         ...
         result = SPI_ERROR_DEVICE_IN_USE;
         break;
      }
...
int32 spi_full_duplex (spi_device_id_t device_id, const spi_config_t *p_config, spi_transaction_t *p_write_info, spi_transaction_t *p_read_info)
{
   ...
   do
   {
	  ...
	  UTRACER_EVENT(SPI_FULL_DUPLEX, p_write_info->buf_len, p_read_info->buf_len);	  
   ...
   while (0);
   UTRACER_EVENT(SPI_FULL_DUPLEX_DONE);
   return result;
}
...
int32 spi_close(spi_device_id_t device_id)
{
   ...
   while (0);
   UTRACER_EVENT(SPI_CLOSE_DONE);
   return result;
}

int32 spi_clk_ctrl (spi_device_id_t device_id, boolean ctrl, uint32 freq_in_hz)
{
   int32 result = SPI_SUCCESS, device_idx;
   ...
   do
   {
      ...
      if (device_id >= SPI_MAX_NUM_DEVICES)
      {
         SPIDEVICE_LOG(ERROR, "spi_clk_ctrl() invalid device_id: %08x", device_id);
         result = SPI_ERROR_INVALID_PARAM;
         break;
      }
      ...
      if (!(spiDevices[device_idx].status & SPI_DEVICE_OPEN))
      {
         SPIDEVICE_LOG(ERROR, "spi_clk_ctrl() SPI device not open");
         result = SPI_ERROR_DEVICE_NOT_OPEN;
         break;
      }
	  ...
	  hSpi = spiDevices[device_idx].hSpiDev;
	  //device_id is qup_core_num
	  result = SpiDevice_ClkCtrl(hSpi, ctrl, freq_in_hz);
   }
   while(0);
   return result;
}
