...
/*===========================================================================
  ...
  $Header: //components/rel/core.qdsp6/1.0.c2/storage/tftp/common/src/tftp_pkt.c#1 $ $DateTime: 2020/02/10 01:57:30 $ $Author: pwbldsvc $

when         who   what, where, why
----------   ---   ---------------------------------------------------------
2018-03-29   rp    validate oack options arrary size and then add items.
              ...
              tftp_pkt_option_id_name_map[] =
{
  ...
  { TFTP_PKT_OPTION_ID_RSIZE,             "rsize"},
...
tftp_pkt_get_option_id_from_name (const char *option_name)
{
  ...
  len1 = tftp_strlen (option_name);

  for ( i = 0; i < tftp_pkt_option_id_map_count; ++i )
  {
    len2 = tftp_strlen (tftp_pkt_option_id_name_map[i].option_name);
                               ...
                               uint32 start_idx)
{
  ...
  while (idx < tftp_pkt->raw_pkt_buff_data_size)
  {
    ...
    str_len = tftp_strlen (option_name);
    ...
    str_len = tftp_strlen (option_val_str);
    idx += str_len;
    TFTP_DEBUG_ASSERT (tftp_pkt->raw_pkt_buff[idx] == 0);
    ++idx;
    ...
    TFTP_ASSERT (option_idx < TFTP_PKT_OPTION_ID_MAX);
    if (option_idx >= TFTP_PKT_OPTION_ID_MAX)
    {
      result = -1;
      break;
    }
...
tftp_pkt_parse_req_pkt (struct tftp_pkt_type *tftp_pkt)
{
  ...
  idx += tftp_strlen (tftp_pkt->pkt_fields.req_pkt.filename);
  TFTP_DEBUG_ASSERT (tftp_pkt->raw_pkt_buff[idx] == 0);
  ++idx;
 ...
 /* The idx should still be within the buffer. */
  ...
  idx += tftp_strlen (tftp_pkt->pkt_fields.req_pkt.mode);
  TFTP_DEBUG_ASSERT (tftp_pkt->raw_pkt_buff[idx] == 0);
  ++idx;
...
tftp_pkt_parse_error_pkt (struct tftp_pkt_type *tftp_pkt)
{
  ...
  tftp_pkt->pkt_fields.error_pkt.error_string =
                   (const char *)&tftp_pkt->raw_pkt_buff[idx];
  idx += tftp_strlen (tftp_pkt->pkt_fields.error_pkt.error_string);
  TFTP_DEBUG_ASSERT (tftp_pkt->raw_pkt_buff[idx] == 0);
  ++idx;
                        ...
                        uint16 error_code_h, const char *error_msg)
{
  ...
  uint32 idx, error_msg_len, pkt_len;
  ...
  /* store error-message */
  error_msg_len = tftp_strlen (error_msg);
                              ...
                              struct tftp_pkt_options_type *options)
{
  ...
  uint32 idx, str_len, remaining_buff_size;
