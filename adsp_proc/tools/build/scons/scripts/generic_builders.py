...
#------------------------------------------------------------------------------
#
#  $Header: //components/rel/scons.qc/1.1/scripts/generic_builders.py#10 $
#  $DateTime: 2017/01/04 23:53:03 $
#  $Author: pwbldsvc $
#  $Change: 12147151 $
...
def generate(env):
    ...
    env.AddMethod(list_file_create, "ListFileCreate")
...
# get_builder_action
#===============================================================================
def get_builder_action(env, action, action_text='Generating ', \
                        action_target="TARGET", action_source="SOURCE"):
    '''
       get_builder_action
    '''
    ...
    if verbose == 3:
        verbose_str = "%s ARGS: [T:%s, S:%s]" % (str(action), \
                      action_target_str, action_source_name_str)
    elif verbose == 2:
        verbose_str = "<== %s\n" \
            "Source: %s\n" \
            "Target: %s\n" \
            "Command: %s\n==>" % (action_text, action_source_name_str, \
                                  action_target_str, str(action))
    elif verbose == 1:
        verbose_str = "=== %s %s ==> %s" % (action_text, \
                      action_source_name_str, action_target_str)
    else:
        verbose_str = \
            "=== %s ${BUILDPATH}/%s" % (action_text, action_target_name_str)
...
# get_builder
#==============================================================================
def get_builder(env, builder_name, action, action_text='Generating ', \
...
def list_file_builder(target, source, env):
    ...
    list_file_create(env, list_fname, source, env.get('add_header', False), \
       env.get('relative_path'), env.get('posix', False),  \
       env.get('sort', False))

def list_file_create(env, list_fname, file_list, add_header=False, \
                     rel_path_target=None, posix=False, sort=False):
    '''
       list_file_create
    '''
    banner = ""
    ...
    if rel_path_target is None:
        ...
        for fname in file_list:
            list_file.write(env.RelPath(os.path.realpath(fname), \
                rel_path_target, posix) + '\n')
...
def list_file_extractor_get_nodes(env, lf_list):
    '''
       ...
       reads list files and extracts the list of files, uses list_file_builder
       to create the listfiles consiladating all of the files extracted...
    '''
    ...
    for list_file in lf_list:
        lf_fname = env.RealPath(str(list_file))
        ...
        if env.PathExists(lf_fname):
            ...
            # after getting the list of files, we need to get their relative
            # path with respect to where linker will execute. The files in the
            # list could already be a relative to where the original lf was
            # created. Trying to find relative paths is difficult, it is easier
            # to detect non relative paths as these start with "/" or
            # second char is ":" (windows).
...
#
#===============================================================================
def replace_text_file(env, target, source, replace_dict=None, \
                      header_style=None):
    '''
       replace_text_file
    '''
...
#===============================================================================
def preprocess_replace_text_file(env, target, source, replace_dict=None, \
                                 header_style=None):
    '''
       preprocess_replace_text_file
...
def replace_text_file_builder_generate(env):
    '''
       replace_text_file_builder_generate
    ...
    env.AddMethod \
        (preprocess_replace_text_file, "PreprocessAndReplaceTextInFile")
...
def replace_text_file_builder(target, source, env):
    ...
    if header_style is not None:
        banner = env.CreateFileBanner \
                           (os.path.split(replace_fname)[1], style=header_style)
...
def install_files_func(env, target, source):
    ...
    if type(target) is list:
        env.PrintError \
            ("InstallFiles can only have single target a file or folder ")
    ...
    if do_multi_source:
        ...
        for source_file in source:
            target = env.RealPath \
                (target_path + "/" + os.path.split(str(source_file))[1])
...
def install_files_builder_generate(env):
    '''
       install_files_builder_generate
    '''
    env.AddMethod(install_files_func, "InstallFiles")

    install_files_act = \
        env.GetBuilderAction(install_files_builder, 'Install    ')
...
def install_files_builder(target, source, env):
    '''
       ...
       - Target can only be a single file or a directory, not multiple targets.
       - To copy from many sources to a directory use a directory as target.
       - Using directory as a target from a single source will cause it to use
...
#    list file.
#
# - NOTE: The list file is used for both linking into an image
#         as well as to clean
#
#===============================================================================
def ext_bldsys_get_lf_nodes(env, list_file):
    '''
       ext_bldsys_get_lf_nodes
    '''
    #import pdb; pdb.set_trace()
...
def ext_bldsys_builder_generate(env):
    '''
       ext_bldsys_builder_generate
    '''
    env.AddMethod(ext_bldsys_func, "ExBuildSystem")
    env.AddMethod(ext_bldsys_get_lf_nodes, "ExBuildSystemGetListFileNodes")

    ext_bldsys_act = env.GetBuilderAction \
                         (ext_bldsys_builder, 'Ext BLDSYS ', action_source=None)
...
def ext_bldsys_func(env, target, **kwargs):
    ...
    # since we don't know anything about the dependecies in the external build
    # system always call the build system and let it decide if tinsg are up to
    # date or not.
...
def ext_bldsys_emitter(target, source, env):
    ...
    if env.GetOption('clean'):
        ...
        if clean_cmd is not None:
            ...
            data, err, rv = \
                env.ExecCmds(clean_cmd, dir=clean_dir, shell=True, buffer=False)
...
def ext_bldsys_builder(target, source, env):
    '''
       ...
       - pass in clean list with CLEAN_LIST; these are the files that will
         removed during clean
       - pass in clean list with CLEAN_LF; this is a file contaning a list of
         files which will removed during clean. Files listed must be relative to
         the list file.
    ...
    if build_cmd is None:
        env.PrintError \
            ("Missing build command [EXBLDSYS_BLDCMD] don't know what to do!!")
        raise RuntimeError('External Build System, called with no command')

    build_dir = env.get('BUILD_DIR')

    data, err, rv = \
        env.ExecCmds(build_cmd, dir=build_dir, shell=True, buffer=False)
...
def build_products_file_generate(env):
    ...
    build_products = \
        ARGUMENTS.get('BUILD_PRODUCTS', '${TARGET_ROOT}/BuildProducts.txt')
    env.Replace(BUILD_PRODUCT_FILE=build_products)

def build_producst_file_emitter(target, source, env):
    '''
       build_producst_file_emitter
    '''
    ...
    image_file = "./" + env.RelPath \
        (source_full_str, os.path.dirname(build_products_file_path), posix=True)
    ...
    if env.PathExists(build_products_file_path):
        ...
        build_prod_data = [x.strip() for x in build_prod_file.readlines()]
    ...
    if not env.GetOption('no_exec'):
        if env.GetOption('clean'):
            ...
            #import pdb; pdb.set_trace()
            if image_file in build_prod_data:
                build_prod_data.remove(image_file)

                if len(build_prod_data) == 0:
                    env.Clean(target, build_products_file_path)
                else:
                    build_prod_file = open(build_products_file_path, "w")
                    build_prod_file.write('\n'.join(build_prod_data))
                    build_prod_file.close()
        else:
            # add lines for this image.
            if image_file not in build_prod_data:
                build_prod_file = open(build_products_file_path, "w")
                build_prod_data.append(image_file)
                build_prod_file.write('\n'.join(build_prod_data) + '\n')
                build_prod_file.close()
...
def build_producst_file_builder(target, source, env):
    ...
    image_file = "./" + env.RelPath \
                                 (source_full_str, "${TARGET_ROOT}", posix=True)
