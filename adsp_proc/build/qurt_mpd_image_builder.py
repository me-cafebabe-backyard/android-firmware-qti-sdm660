...
def imagebuild_generator(source, target, env, for_signature):
   ...
   if 'USES_PD_RESTARTABLE' in env:
	return ' '.join(Flatten([interp,
                                ...
                                [str(w) for w in source if ( str(w).endswith('.elf') and not str(w).startswith('AUDIO_'))],
                                '-R',
                                [str(w) for w in source if ( str(w).startswith('AUDIO_') and str(w).endswith('.elf'))],
                                '-o',
                                [str(w) for w in target if str(w).endswith('.pbn')],
                                '-p',
                                env['PHYS_ADDR'],'--assert_free_microimage=%s' % env['MICROIMAGE_RESERVED']]))
   else:
	return ' '.join(Flatten([interp,
                                [str(w) for w in source if str(w).endswith('.py')],
...
def generate(env):
   env['BUILDERS']['ImageBuild'] = Builder(generator = imagebuild_generator, emitter = imagebuild_emitter, PHYS_ADDR='0x1e000000', MICROIMAGE_RESERVED='0K')
