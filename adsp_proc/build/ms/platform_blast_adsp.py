...
#-------------------------------------------------------------------------------
#
#  $Header: //components/rel/dspbuild.adsp/3.0/ms/platform_blast_adsp.py#5 $
...
def generate(env):
   ...
   # For the Sensors IMG, we need to specify a listing file.  I've removed the assignment for the image_objs and img_libs.
   image_listfile = env.get('QCOREIMG_LISTFILE')
   image_libs = []
   image_objs = []
   ...
   if do_link:
      ...
      # Build env QCOREIMG
      #-------------------------------------------------------------------------
      # We are passing in the listing file from the SENSOR IMG.
      image_elf = env.AddProgram("${SHORT_BUILDPATH}/${TARGET_NAME}", image_objs, LIBS=image_libs, LISTFILES=image_listfile)
      
      # The image linking depends on whether the LCS file was generated.  
      if 'USES_ISLAND' in env:
         rootenv = env.get('IMAGE_ENV')
         env.Depends(image_elf, rootenv['lcs_file'])
   ...
   if do_post_link:
      ...
      #      [image_elf, "${BUILD_ROOT}\${CHIPSET}\qdsp6.xml"])
         
      #-------------------------------------------------------------------------
      # Install ELF, reloc files
      #-------------------------------------------------------------------------
      if 'USES_FEATURE_QSHRINK_ENABLED' in env:
         env.LoadToolScript("${BUILD_SCRIPTS_ROOT}/../../../../qdsp6/qshrink/src/Qshrink20.py")

         qshrink20_elf = env.Qshrink20(
            "${SHORT_BUILDPATH}/${TARGET_NAME}_qsr2",
            [image_elf, "${BUILD_SCRIPTS_ROOT}/../../../../qdsp6/qshrink/src/msg_hash.txt"])

         # copy elf and reloc to needed locations for AMSS tools to load on target
         install_target_elf = env.InstallAs(
            "${BUILD_MS_ROOT}/${BUILD_ELF_EXTN}_${BUILD_ID}.elf", qshrink20_elf)
      else:
         # copy elf and reloc to needed locations for AMSS tools to load on target
         install_target_elf = env.InstallAs(
            "${BUILD_MS_ROOT}/${BUILD_ELF_EXTN}_${BUILD_ID}.elf", image_elf)

      #-------------------------------------------------------------------------
      # Build env mbn files
      #-------------------------------------------------------------------------
      #target_mbn = env.MbnBuilder("${SHORT_BUILDPATH}/${TARGET_NAME}", install_target_elf, IMAGE_TYPE='qdsp6sw', FLASH_TYPE=env['TARGET_FLASH_TYPE'])
