...
from sectools.features.isc.cfgparser.defines import JTAGID_SIGN_IDS, MRC_1_0_CHIPSETS, MRC_2_0_CHIPSETS, SECBOOT_2_0_SINGLE_SIGN_CHIPSETS, SECBOOT_2_0_DOUBLE_SIGN_CHIPSETS, ECDSA_CHIPSETS, SECBOOT_1_IMAGE_TYPES
...
class _ImageList(object):
    ...
    def validate(self, images, *args):
        ...
        def validate_serial_bound_value(value, value_name, sign_id, chipset=None, allowed_chipsets=None, num_root_certs=None, ignore_num_root_certs=False):
            if value is not None:
                if chipset is not None and allowed_chipsets is not None and chipset not in allowed_chipsets:
                    ...
                    if lower_8_bytes not in [0, 1, 2]:
                        add_error("sign_id={0}: {1} must end with 0, 1, or 2. {1} = {2}".format(sign_id, value_name, value))
                    ...
                    elif upper_8_bytes == 0 and lower_8_bytes == 2:
        ...
        for image in image_list:
            ...
            if num_root_certs > 1 and secboot_version == SECBOOT_VERSION_2_0 and chipset not in (SECBOOT_2_0_SINGLE_SIGN_CHIPSETS + SECBOOT_2_0_DOUBLE_SIGN_CHIPSETS):
                # Multiple root certs not allowed with secboot 2.0 chipsets multi-party signing
                add_error("Multiple root certs are not supported for secboot 2.0 chipset {0}".format(chipset))
            ...
            if rot_en is not None and (int(rot_en, 16) & 0xFFFFFFFF) > 1:
                add_error("rot_en 32bit LSB must be 0 or 1: sign_id={0}, rot_en={1}".format(sign_id, rot_en))
            ...
            if soc_vers is not None and in_use_soc_hw_version == 1:
                ...
                validate_hex_list(soc_vers_list, MAX_NUM_SOC_VERS, "soc_vers", "soc_ver", 6)
            ...
            if chipset in MRC_2_0_CHIPSETS and chipset in SECBOOT_2_0_DOUBLE_SIGN_CHIPSETS and max_num_root_certs is None:
            ...
            if chipset in MRC_1_0_CHIPSETS or chipset in MRC_2_0_CHIPSETS:
                ...
                if chipset in MRC_2_0_CHIPSETS and num_root_certs not in [1, 4]:
                    add_error("num_root_certs must be either 1 or 4")
            ...
            if len(multi_serial_numbers) > 0:
                if chipset not in (SECBOOT_2_0_SINGLE_SIGN_CHIPSETS + SECBOOT_2_0_DOUBLE_SIGN_CHIPSETS):
                    if debug is None:
                ...
                validate_hex_list(multi_serial_numbers, MAX_NUM_SERIALS, "multi_serial_numbers", "serial", 10, False)
