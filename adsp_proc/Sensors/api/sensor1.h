...
$Header: //components/rel/ssc.slpi/2.2/api/sensor1.h#1 $ 
$DateTime: 2017/04/17 22:20:39 $

when       who    what, where, why 
---------- --- -----------------------------------------------------------
2015-26-03 PS  Updated license header
2011-07-27 sc  Removed legacy sensor1_attach & sensor1_detach
2010-10-06 jtl Added sensor1_attach & sensor1_detach & BROKEN_PIPE msg type
2010-06-30 jtl Initial revision
...
/**
   ...
   one callback per client can be registered.

   Linux Android Implementation notes:  In the sensor1 remoting API (accessed
   via libsensor1.so) this can be called only once.
...
/**
   ...
   Before calling {@link sensor1_close} a client should deregister from any
   ...
   not deregister from services, additional resources may not be freed.

   @note Registered callback functions will not be called after this function
   has returned. However, the callbacks may be called while the client is in the
   process of closing. Thus, clients should take care to insure their registered
   callback function will execute properly until after this function has
   returned.
