...
/*==============================================================================
  ...
  2016-05-04 jtl  Re-arrange port_write API implementation
  2016-02-26 jtl  Move vector transfer command to uImage
  2015-10-21 jtl  Added timed write and vector transfer commands.
    ...
    uint32_t*               read_count)
{
  ...
  {
    UMSG(MSG_SSID_SNS,DBG_ERROR_PRIO, "fun arg is NULL");
...
/**
 * @brief Performs a bus write operation with timing information.
 *
 ...
 *         SNS_DDF_EFAIL to indicate an error has occurred.
 */
SNS_DDF_UIMAGE_CODE sns_ddf_status_e sns_ddf_write_port_timed(
    const sns_ddf_handle_t  handle,
    uint32_t                reg_addr,
...
//                  reg_addr, bytes );
  ...
  {
    case SNS_DDF_BUS_I2C:
      status = sns_ddf_comm_bus_i2c_write( handle, reg_addr, buffer, bytes, &bytes_written, write_time );
      break;
    case SNS_DDF_BUS_SPI:
#if SNS_DDF_COMM_BUS_SPI_ENABLE_DRIVER
      status = sns_ddf_comm_bus_spi_write( handle, reg_addr, buffer, bytes, &bytes_written, write_time );
      if ( status == SNS_DDF_SUCCESS )
      {
        bytes_written = bytes;
      } else
 ...
 * @param[in]  num_vectors   Number of elements in the vectors array
 * @param[out] total_bytes   Total number of bytes read & written for all registers
 * @param[out] write_time    Time (in SSC clock ticks*32) of the bus transaction
 * 
 ...
 *  	   SNS_DDF_EFAIL to indicate an error has occurred.
 */
SNS_DDF_UIMAGE_CODE sns_ddf_status_e sns_ddf_rw_port_v(
    const sns_ddf_handle_t      handle,
    ...
    uint64_t                   *write_time )
{
  ...
  if( num_vectors == 0 || handle == NULL || vectors == NULL )
  {
    return status;
  }

  switch( ddf_handle->bus )
  {
    case SNS_DDF_BUS_I2C:
      status = sns_ddf_comm_bus_i2c_rw_v( handle, vectors, num_vectors, total_bytes, write_time );
      break;
    case SNS_DDF_BUS_SPI:
      status = sns_ddf_comm_bus_spi_rw_v( handle, vectors, num_vectors, total_bytes, write_time );
      break;
    default:
      break;
  }
...
SNS_DDF_UIMAGE_CODE sns_ddf_status_e sns_ddf_write_port(
    ...
    uint8_t*                write_count)
{
  return sns_ddf_write_port_timed( handle, reg_addr, buffer, bytes, write_count, NULL );
}
