...
/*==============================================================================
  ...
  $Header: //components/rel/ssc.slpi/2.2/ddf/src/sns_ddf_uimg_signal.c#2 $
  $DateTime: 2017/05/19 04:36:09 $
  $Author: pwbldsvc $

  when       who  what, where, why
  ---------- ---  -----------------------------------------------------------
  2017-05-08 bd   Removed signal timestamp check to avoid infinite loop
  2016-04-12 tc   Add support for SNS_DDF_SIGNAL_POST QDSS event
...
{
  sns_ddf_time_t sig_timestamp = sns_ddf_get_timestamp();
#if !defined(SNS_PCSIM) && !defined(SNS_QDSP_SIM)
  SNS_ASSERT(sig_num < SNS_DDF_SIGNAL_MAX_CNT);
  sns_profiling_log_qdss(SNS_DDF_SIGNAL_POST, 2,
      (uint32_t)sns_ddf_sig_tb.sig_info[sig_num].gpio_num, (uint32_t)sig_timestamp);
  qurt_atomic_set_bit((unsigned int *)&sns_ddf_sig_tb.pending_flags, sig_num);
  sns_ddf_sig_tb.sig_info[sig_num].timestamp = sig_timestamp;
