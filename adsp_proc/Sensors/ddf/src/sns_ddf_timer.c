...
/*==============================================================================
  ...
  $Header: //components/rel/ssc.slpi/2.2/ddf/src/sns_ddf_timer.c#1 $
  $DateTime: 2017/04/17 22:20:39 $

  when       who  what, where, why
  ---------- ---  -----------------------------------------------------------
  2016-04-21  sd   fixed timer release bug
...
sns_ddf_status_e sns_ddf_timer_start(sns_ddf_timer_s timer, uint32_t usec)
{
  ...
  if ( timer_ptr != NULL )
  {
    ...
    UMSG_3(MSG_SSID_SNS, DBG_LOW_PRIO, "start - idx=%u offset=%u per=%u", 
           timer_ptr->q_idx, offset, timer_ptr->is_periodic);
#endif
  }
  else
  {
    UMSG_1(MSG_SSID_SNS, DBG_ERROR_PRIO, "start - timer 0x%x not found", timer);
...
#ifdef SNS_DDF_TIMER_DEBUG
    UMSG_2(MSG_SSID_SNS, DBG_LOW_PRIO, "cancel - idx=%u per=%u", 
           timer_ptr->q_idx, timer_ptr->is_periodic);
#endif
  }
  else
  {
    UMSG_1(MSG_SSID_SNS, DBG_ERROR_PRIO, "cancel - timer 0x%x not found", timer);
    status = SNS_ERR_BAD_PARM;
  }
#endif
...
SNS_DDF_UIMAGE_CODE sns_ddf_status_e sns_ddf_timer_release(sns_ddf_timer_s timer)
{
  sns_err_code_e status = SNS_ERR_NOTSUPPORTED;
#ifndef SNS_QDSP_SIM
  sns_ddf_timer_obj_s* timer_ptr;

  sns_smgr_enter_bigimage();
  timer_ptr =  sns_ddf_timer_queue_del(timer);
  if ( timer_ptr != NULL )
  {
    status = SNS_SUCCESS;
    qurt_atomic_clear_bit(&sns_ddf_timer_q.pending_flags, timer_ptr->q_idx);
  ...
  {
    UMSG_1(MSG_SSID_SNS, DBG_ERROR_PRIO, "release - timer 0x%x not found", timer);
    status = SNS_ERR_BAD_PARM;
  }
  sns_smgr_exit_bigimage();
#endif
...
SNS_DDF_UIMAGE_CODE void sns_ddf_timer_post(void* timer_id)
{
       ...
       sns_ddf_timer_q.timer_ptr[idx] != NULL )
  {
    ...
    {
      UMSG_3(MSG_SSID_SNS, DBG_ERROR_PRIO, "idx=%u sched=%u expired=%u",
  ...
  while (0 != (flags = sns_ddf_timer_q.pending_flags))
  {
         ...
         sns_ddf_timer_q.timer_ptr[idx] != NULL )
    {
...
#ifdef SNS_DDF_TIMER_DEBUG
      {
        ...
        {
          UMSG_3(MSG_SSID_SNS, DBG_ERROR_PRIO, "idx=%u expired=%u dispatched=%u",
                 idx, timer_ptr->expired, timer_ptr->dispatched);
        }
        if ( took > 15 )
        {
          UMSG_2(MSG_SSID_SNS, DBG_ERROR_PRIO, "idx=%u handle_timer took %u ticks",
                 idx, took);
        }
      }
#endif
