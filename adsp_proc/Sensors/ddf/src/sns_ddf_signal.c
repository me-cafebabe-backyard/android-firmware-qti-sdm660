...
/*==============================================================================
  ...
  $Header: //components/rel/ssc.slpi/2.2/ddf/src/sns_ddf_signal.c#1 $
  $DateTime: 2017/04/17 22:20:39 $

  when       who  what, where, why
  ---------- ---  -----------------------------------------------------------
  2016-01-07 JTL  Using uGPIOInt for deregistering GPIO interrupts
    ...
    sns_ddf_signal_irq_trigger_e  trigger)
{
    ...
    if(!sns_ddf_sig_tb.sig_info[sig_idx].is_registered)
    {
      err = uGPIOInt_RegisterInterrupt(
                                       ...
                                       (uGPIOINTISR)sns_ddf_smgr_notify_irq,
                                       (uGPIOINTISRCtx)sig_idx,
                                       0);
      if (err != -1)
      {
        sns_ddf_sig_tb.sig_info[sig_idx].is_registered = true;
        DDF_MSG_2(MED,"Micro GPIO Int Reg Done [idx(%d) gpio(%d)]",sig_idx,gpio_num);
      }
      else
      {
        MSG_3(MSG_SSID_SNS, DBG_ERROR_PRIO, "sig_reg FAILED - idx(%d) gpio(%d) reg(%d)", 
              sig_idx, gpio_num, sns_ddf_sig_tb.sig_info[sig_idx].is_registered);
      }
    }
    DALSYS_SyncLeave(sns_ddf_sig_tb.sync_handle);
...
sns_ddf_status_e sns_ddf_signal_deregister (uint32_t gpio_num)
{
    ...
    if (sns_ddf_sig_tb.sig_info[sig_idx].is_registered)
    {
      err = uGPIOInt_DeregisterInterrupt(gpio_num);
      if (err != -1)
      {
