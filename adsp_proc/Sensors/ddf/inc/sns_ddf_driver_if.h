...
/*==============================================================================
  ...
  ---------- ---  -----------------------------------------------------------
  2015-10-27 jtl  Incremented SNS_DDF_DRIVER_VTBL_REVISION for S4S
  2015-05-28 pn   Bumped up SNS_DDF_DRIVER_VTBL_REVISION for FILTER_DELAY change
...
/**<  Revision number of the VTBL 
      @note This is expected to be incremented whenever the VTBL structure
      changes
*/
#define SNS_DDF_DRIVER_VTBL_REVISION 0x4

typedef struct
{
  /**<  Revision number of the VTBL, this must always be initialized to
        SNS_DDF_DRIVER_VTBL_REVISION */
  uint32_t            vtbl_revision;
  /**<  The size of sns_ddf_driver_if_s structure.
        This must always be initialized to sizeof(sns_ddf_driver_if_s) */
  uint32_t            vtbl_size;
  /**<  The address of the SNS DDF driver VTBL */
  sns_ddf_driver_if_s *vtbl;
} sns_ddf_driver_vtbl_info;

/**
  @brief Function prototype to be implemented by dynamic sensor drivers.
  The function is expected to return valid pointer to the 
  sns_ddf_driver_vtbl_info structure containing the DDF VTBL
  interface implementations, the size of the VTBL and the revision number
  associated with the VTBL.
*/
typedef sns_ddf_driver_vtbl_info* (*sns_ddf_driver_get_vtbl_info)(void);
