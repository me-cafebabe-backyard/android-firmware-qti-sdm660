...
#include "comdef.h" //PENDING: investigate how scons implicit declaration works for "PACK" w/o this header

/*============================================================================
  Preprocessor Definitions and Constants
  ===========================================================================*/

/** @addtogroup sa_constants
@{ */
...
*/
typedef uint32_t sns_sam_timestamp;
#define SNS_SAM_TIMESTAMP_MAX UINT32_MAX
 ...
 *
 * @note1hang Sensor UIDs are referred to by reference in all function 
 *            parameters of sns_sam_algo_callback. A pointer to the SUID
 *            must be saved from 
 *            sns_sam_algo_msg_api::sns_sam_algo_dep_sensors_met().
*/
...
/** An instance of algorithm input or output data. */
struct sns_sam_algo_io
{
...
/** An instance of algorithm input data. */
typedef struct sns_sam_algo_io sns_sam_algo_input;

/** An instance of algorithm output data. */
typedef struct sns_sam_algo_io sns_sam_algo_output;
...
/** QMI decoded response message generated by some service. */
...
enum sns_sam_report_mode
{
  ...
  /**< One shot operation is not supported in SAM.*/
...
enum sns_sam_algo_attr_type{
  ...
  SAM_ALGO_ATTR_RESERVED_FIFO_SIZE,
  /**< Minimum number of reports reserved for a sensor. */
  SAM_ALGO_ATTR_CNT
...
{
  uint64_t attrValue;
  /**< Value associated with the attribute; need not use the entire 64 bits. */
...
@{ */
   ...
   * instance input FIFO, and copies the supplies contents onto the buffer.
   *
   ...
   * @param[in] input Data to be added to the input FIFO.
   *
   * @return
   * SAM_ENONE -- Operation was successful. \n
  ...
  sns_sam_err (*sns_sam_algo_add_input)(
      struct sns_sam_algo_handle const *algoHndl,
      sns_sam_sensor_uid const *sensorUID,
      sns_sam_algo_input const *input );

  /**
   * Allocates memory for a single output data, adds that item to the algorithm
   * instance output FIFO, and copies the supplies contents onto the buffer.
   *
   * @note1hang outputData must have the length as supplied in
   *            sns_sam_algo_api::sns_sam_algo_mem_req().
   *
   * @param[in] algoHndl Opaque handle of this algorithm instance for SAM.
   * @param[out] outputData Data to be added to the output FIFO.
   *
   * @return
   * SAM_ENONE -- Operation was successful. \n
   * SAM_EFAILED -- No more memory is presently available on the system. \n
   ...
   * @newpage
   */
  sns_sam_err (*sns_sam_algo_log_qdss)(
      struct sns_sam_algo_handle const *algoHndl,
      sns_tracer_event qdssEventId,
      uint32_t inputType );

  /**
   * Obtains the number of remaining input samples for the specified sensor type.
   *
   * @note1hang Within a single call to sns_sam_algo_update(), this value only
   *            decreases as a result of calling sns_sam_algo_get_input().
   *
   * @param[in] algoHndl Opaque handle of this algorithm instance for SAM.
   * @param[in] sensorUID Sensor type to query; NULL for all input
   *
   * @return
   * The number of remaining input samples.
   *
   * @dependencies
   * None.
   *
   * @newpage
   */
  uint32_t (*sns_sam_algo_input_cnt)(
      struct sns_sam_algo_handle const *algoHndl,
      sns_sam_sensor_uid const *sensorUID );

  /**
   * Finds and removes the next input object from the input FIFO of a specified
   * sensor type. If sensorUID is NULL, the function chooses the input with the
   * earliest timestamp among the heads of all input FIFOs.
   *
   * @param[in] algoHndl Opaque handle of this algorithm instance for SAM.
   * @param[in] sensorUID Restrict samples from this input type only;
   *                      can be NULL.
   *
   * @return
   * Acquired input data; otherwise, NULL if none is available.
   *
   * @dependencies
   * This function can only be called within sns_sam_algo_update().
   *
   * @newpage
   */
  sns_sam_algo_input (*sns_sam_algo_get_input)(
      struct sns_sam_algo_handle const *algoHndl,
      sns_sam_sensor_uid const *sensorUID );

  /**
   * Finds and removes the next output object from the output FIFO.
   *
   * @param[in] algoHndl Opaque handle of this algorithm instance for SAM.
   *
   * @return
   * Acquired output data; otherwise, NULL if none is available.
   *
   * @dependencies
   * This function can only be called within sns_sam_algo_gen_batch_ind().
   *
   * @newpage
   */
  sns_sam_algo_output (*sns_sam_algo_get_output)(
      struct sns_sam_algo_handle const *algoHndl );

  /**
   * Sends an UPDATE_REQ to a dependent algorithm.
   *
   * @param[in] algoHndl Opaque handle of this algorithm instance for SAM.
   * @param[in] sensorUID Intended recipient of the update request message.
   * @param[in] updateReq Update request message to be sent.
   *
   * @return
   * SAM_ENONE -- Operation was successful. \n
   * SAM_ETYPE -- No active stream is found with this sensor. \n
   * SAM_EFAILED -- Unable to send the request.
   *
   * @dependencies
   * None
   *
   * @newpage
   */
  sns_sam_err (*sns_sam_algo_send_update)(
      struct sns_sam_algo_handle const *algoHndl,
      sns_sam_sensor_uid const *sensorUID,
...
struct sns_sam_algo_const_buf_size
{
  ...
  uint32_t inputDataSize;  /**< Input data size. */
};
...
@{ */
  ...
  /**
   * Obtains all attributes of an algorithm. These are used by the SAM
   * Framework and sent to clients via sns_sam_get_algo_attrib_resp_msg_v01.
   *
   * @datatypes
   * #sns_sam_algo_persist \n
   * #sns_sam_algo_attr
  ...
  /**
   * The algorithm must determine whether its registry dependencies have been
   * met. If a requested registry group is unavailable, regData->groupData
   * is set to NULL.
   * 
   * @note1hang This function does not require implementation for algorithms
  ...
  /**
   * Gets the list of an algorithm's registry groups. Registry groups are not
   * the exclusive property of any single algorithm; multiple algorithms can
   * request the same registry group to be read.
   *
  ...
  /**
   * Processes/parses an incoming indication from one of this algorithm's
   * sensor dependencies.
   *
   * @datatypes
   * #sns_sam_algo_callback \n
   ...
   *
   * @param[in] cbFunc Provides additional functionality to the algorithms.
   * @param[in] outputData First item on the algorithm output data FIFO.
   ...
   *
   * @param[in] cbFunc Provides additional functionality to the algorithms.
   * @param[in] extInstanceID ID assigned by SAM to refer to this client request.
   ...
   *
   * @dependencies
   * sns_sam_algo_api::sns_sam_algo_reset() must be called prior to this
   * function.
   *
   * @newpage
   */
...
/** @ingroup sa_datatypes
 * Contains the algorithm registration APIs.
*/
typedef struct
{
  uint32_t               structSize;
  /**< Size of this structure. This parameter must always be initialized to
       sizeof(sns_sam_algo_config_container). */
  uint32_t               numRegistrations;
  /**< Number of algorithm registration functions in the array. */
  sns_sam_algo_register  *registrationList;
  /**< List (array) of SNS algorithm registration functions. */
} sns_sam_algo_config_container;

/** @ingroup sns_sam_algo_get_registration_container
 * Function for dynamically loaded SNS algorithms to provide a
 * pointer to a container containing the algorithm registration APIs.
 *
 * @datatypes
 * #sns_sam_algo_config_container
 *
 * @return
 * A valid pointer to a container describing a set of algorithms to be
 * registered with the SAM framework.
 */
typedef sns_sam_algo_config_container*
   (*sns_sam_algo_get_registration_container)(void);
