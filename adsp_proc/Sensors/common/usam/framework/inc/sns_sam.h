...
/**
 * check if ts1 is equal or advanced than ts2 within delta range. Take into account possible clock rollover.
 *
 * @param[i] ts1 Timestamp 1
 * @param[i] ts2 Timestamp 2
 * @param[i] delta delta range
 *
 * @return true if ts1 is equal or advanced than ts2  within delta range
 *         false else
 */
bool sns_sam_compare_ts_delta( sns_sam_timestamp const ts1,
                              sns_sam_timestamp const ts2,
