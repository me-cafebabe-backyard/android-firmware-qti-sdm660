    ...
    smr_err transpErr )
{
  ...
  else
  {
    ...
    if(NULL != respMsg)
    {
      SNS_OS_FREE((void *)(respMsg->msg.msg.buf));
      SNS_OS_FREE(respMsg);
    }
  }

  SNS_OS_FREE( (void*)respCBData );
}
...
sns_sam_handle_release(sns_sam_sensor_req *sensorReq)
{
  ...
  while( NULL != qCurr )
  {
    ...
    if( sensorReq == message->sensorReq )
    {
      ...
      SNS_OS_FREE( (void*)message->msg.msg.buf );
      SNS_OS_FREE( (void*)message );
    }
  }
  sns_os_mutex_post( samClntMsgQMutex );
  ...
  {
    SNS_OS_FREE( (void*)sensorReq->enableReq->msg.buf );
    SNS_OS_FREE( (void*)sensorReq->enableReq );
  }
...
sns_sam_process_clnt_msg_q( void )
{
  ...
  /* only process up-to a fixed number of items on the queue, to make sure
     other signals are also handled and not starved under heavy load */
  uint32_t remaining_count = SNS_SAM_CLNT_MSG_Q_MAX;
  ...
  } while( NULL != qCurr && remaining_count > 0 );

  /* if there are remaining items in queue, signal SAM to process them later */
  if ( qCurr != NULL )
  {
    SNS_PRINTF_STRING_LOW_1( samModule,
      "defer processing of samClntMsgQ, remaining=%d", samClntMsgQ.cnt );
    sns_os_sigs_post( sns_sam_sig_event, SNS_SAM_CLNT_MSG_SIG, OS_FLAG_SET,
      &errOS );
  }
#ifdef FAST_PLAYBACK
