    ...
    unsigned int reqBufLen, void *serviceCookie )
{
  ...
  {
    SNS_OS_FREE( (void*)reqMsg->msg.msg.buf );
    SNS_OS_FREE( (void*)reqMsg );
...
sns_sam_process_svc_msg_q( void )
{
  ...
  // Adjust the SAM MPPS vote based on Queue count
  ...
  do
  {
    ...
    if( NULL != reqMsg )
    {
      ...
      SNS_OS_FREE( (void*)reqMsg->msg.msg.buf );
      SNS_OS_FREE( (void*)reqMsg->respMsg.msg.buf );
      SNS_OS_FREE( reqMsg );
    }
  } while( NULL != reqMsg );
}
