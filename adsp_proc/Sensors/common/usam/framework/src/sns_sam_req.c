    ...
    struct sns_sam_msg *respMsg )
{
  ...
  if( NULL != algo )
  {
    ...
    if( SAM_ENOMEM != err )
    {
      ...
      SNS_OS_FREE( (void*)configData.buf );
    }
    else
    {
    ...
    struct sns_sam_msg *respMsg )
{
  ...
  if( NULL != algo )
  {
    ...
    else if ( reportResp->max_batch_size > SNS_SAM_DEFAULT_RESERVED_FIFO_SIZE )
    {
