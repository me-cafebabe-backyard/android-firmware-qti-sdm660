...
#define SNS_SAM_TS_10_MIN DSPS_SLEEP_CLK*60*10
/*============================================================================
  Static Data
  ===========================================================================*/
  ...
  if( SNS_SAM_ALGOINST_NOMEM_UIMAGE == algoInst->imageMode )
  {
         ...
         qCurr = sns_q_next( &algoInst->algoInputQ, qCurr ) )
    {
           ...
           qCurrBin = sns_q_next( &inputQ->inputDataQ, qCurrBin ) )
      {
        ...
        if( !SNS_OS_IS_UIMG_ADDR(inputBin) )
        {
          return ;
        }
      }
    ...
    bool releaseEmpty )
{
  ...
  while( NULL != ioBin && ioBin->readOffset == ioBin->writeOffset )
  {
    if( releaseEmpty )
    {
      bool isUBin = SNS_OS_IS_UIMG_ADDR( ioBin );
      ...
      if( SNS_SAM_ALGOINST_NOMEM_UIMAGE == algoInst->imageMode && !isUBin )
      {
        sns_sam_update_uimage_status( algoInst );
      }
...
sns_sam_output_release( sam_client_req *clientReq, uint32_t cnt )
{
  ...
  while( NULL != ioBin && ioBin->readOffset == ioBin->writeOffset )
  {
    isBBin |= !SNS_OS_IS_UIMG_ADDR( ioBin );
  ...
  if( SNS_SAM_ALGOINST_NOMEM_UIMAGE == clientReq->algoInstance->imageMode && isBBin )
  {
    ...
    sns_sam_sensor_uid const *sensorUID, sns_sam_algo_input const *input )
{
  ...
  {
    //mark sample invalid if it's equal or older than lastInputTs within 10min
    if ( sns_sam_compare_ts_delta(inputQ->lastInputTs, input->timestamp, SNS_SAM_TS_10_MIN) )
    {
      SNS_UMSG_MEDIUM_3( samModule,
        "Warning: Invalid input detected: inputSUID=0x%x, algoSUID=0x%x, ts=%lu",
        *sensorUID, *algoInst->algorithm->sensor.sensorUID, inputQ->lastInputTs);
      inputValid = false;
    }
  }
...
sns_sam_move_output( sam_client_req *clientReq )
{
  ...
  while( NULL != qCurr )
  {
    ...
    if( SNS_OS_IS_UIMG_ADDR(outputBin) && NULL != qCurr )
    {
