    ...
    sns_sam_algo_output const *outputData )
{
  ...
  if( SAM_ENONE == err )
  {
    ...
    SNS_OS_FREE( (void*)indMsg.msg.buf );
  }
  else
  {
...
/**
 * Send output data to a client or store for future use.
 *
 * @param[i] clientReq  Client request that the output is intended for
 * @param[i] outputData An algorithm's output that should be sent or saved.
 *
 * @return SAM_ENOT_AVAILABLE if SMR connection is busy
 *         SAM_ENOMEM if there is insufficient memory to send an indication
 *         SAM_EFAILED if error in sending indication
 *         SAM_ENONE otherwise
 */
STATIC SNS_SAM_UIMAGE_CODE sns_sam_err
sns_sam_process_output( sam_client_req * clientReq,
  sns_sam_algo_output const *outputData)
{
  sns_sam_err err = SAM_ENONE;

  if( 0 != clientReq->batchPeriod || clientReq->clientBusy ||
    0 != sns_sam_output_cnt(clientReq) )
  {
...
sns_sam_handle_report( sam_client_req *clientReq )
{
  ...
  {
    sns_sam_process_output(clientReq, &outputData);
    SNS_OS_FREE((void *)outputData.data);
  }
}

SNS_SAM_UIMAGE_CODE sns_sam_err
sns_sam_handle_output( sns_sam_algo_inst *algoInst,
  sns_sam_algo_output const *outputData )
{
  sns_sam_err err = SAM_ENONE;
  sns_q_link_s *qCurr;
...
sns_sam_handle_ind( sns_sam_ind *indMsg )
{
  ...
  SNS_OS_FREE( (void*)indMsg->msg.msg.buf );
  SNS_OS_FREE( (void*)indMsg );

  sns_sam_create_batch_int_ind( sensorReq );

  /* If the sensor request is temporary, it needs to be merged after first
     indication */
  if (sensorReq->isTemporary)
  {
    sns_sam_uimage_exit();
    sns_sam_merge_sensor_req(sensorReq);
  }
}
...
sns_sam_client_timer_cb( sam_client_req *clientReq )
{
  ...
  sns_sam_client_event *clientEvent;
  sns_q_link_s *qCurr;
  bool cltReqInQ = false;

  SNS_UMSG_LOW_0( samModule, "sns_sam_client_timer_cb" );

  //DO NOT queue again if same clientReq is already in queue
  sns_os_mutex_pend( samBatchQMutex, 0, &errOS );
  for( qCurr = sns_q_check( &samBatchQ ); NULL != qCurr;
      qCurr = sns_q_next( &samBatchQ, qCurr ) )
  {
    if ((sam_client_req *)qCurr == clientReq)
    {
      cltReqInQ = true;
      break;
    }
