...
#define FLUSH_REQ_TIMEOUT_US 1500000
  ...
  sns_sam_algo_inst const *algoInst )
{
       ...
       qCurr = sns_q_next( &sensorReqQ, qCurr ) )
  {
    ...
    if( sensorUID == sensorReq->sensor->sensorUID &&
        sensorReq->isTemporary == false &&
...
sns_sam_shrink_enable_req( sns_sam_enable_req *enableReqMsg )
{
  ...
  SNS_OS_FREE( (void*)enableReqMsg->msg.buf );
...
/**
 * Merge the given temporary sensor request with an existing
 * sensor request if possible. If not, promote the temporary
 * sensor request to be permenant.
 *
 * @param[i] tempReq Temporary sensor request to be merged
 *
 **/
void
sns_sam_merge_sensor_req(sns_sam_sensor_req* tempReq)
{
  /* temporary request would have only one algo instance associated and
     it will be at index 0 */
  sns_sam_algo_inst* algoInst = tempReq->algoInst[0];
  q16_t minBatch = 0;

  SNS_PRINTF_STRING_MEDIUM_1( samModule, "merging temp request, sensorUID=%x",
    *tempReq->sensorUID );

  /* if the algoInst is NULL, this request is already merged. */
  if( algoInst != NULL )
  {
    sns_sam_sensor_req* exReq =
      sns_sam_find_existing_sensor_req( tempReq->sensorUID, algoInst );
    if (exReq != NULL)
    {
      minBatch = sns_sam_get_min_batch_period(exReq);
    }
    if ( exReq != NULL && sns_sam_mark_sensor_req( exReq, algoInst ) )
    {
      /* remove temporary request */
      tempReq->flushInProgress = false;
      sns_sam_handle_flush_response( tempReq );
      sns_sam_remove_ai_ref( tempReq, algoInst );
      sns_sam_remove_sensor_req( tempReq );
      sns_sam_set_batch_period( algoInst, exReq->sensor->sensorUID, minBatch );
      SNS_PRINTF_STRING_MEDIUM_0( samModule,
        "merged with existing sensor request" );
    }
    else
    {
      /* promote the temporary request */
      tempReq->isTemporary = false;
      sns_sam_distribute_sensor_req( tempReq );
      SNS_PRINTF_STRING_MEDIUM_0( samModule,
        "existing request not found, temporary request promoted" );
    }
  }
}
...
sns_sam_create_sensor_req( sns_sam_algo_inst *algoInst,
  sns_sam_enable_req *enableReqMsg )
{
  sns_sam_sensor_req *newReq, *reqToDelete, *exReq;

  SNS_PRINTF_STRING_MEDIUM_2( samModule, "algoInst=%p, UID=%lx", algoInst,
   *enableReqMsg->sensorUID );

  reqToDelete = sns_sam_find_sensor_req( algoInst, enableReqMsg->sensorUID );

  exReq = sns_sam_find_existing_sensor_req( enableReqMsg->sensorUID, algoInst );
  SNS_PRINTF_STRING_MEDIUM_2( samModule, "exReq=%p, UID=%lx", exReq,
    *enableReqMsg->sensorUID );

  SNS_PRINTF_STRING_MEDIUM_1( samModule, "creating new request, UID=%lx",
    *enableReqMsg->sensorUID );

  newReq = sns_sam_create_new_sensor_req( enableReqMsg );
  
  if ( NULL == newReq )
  {
    SNS_PRINTF_STRING_ERROR_1( samModule,
      "Failure generating new sensor request: %x", *enableReqMsg->sensorUID );
    return SAM_EFAILED;
  }

  newReq->algoInst[0] = algoInst;

  /* If existing request is available for sharing, mark the new request as
    temporary. After the first indication, this request will be merged with
    existing request.*/
  newReq->isTemporary = ( NULL == exReq ) ? false : true;

  SNS_PRINTF_STRING_MEDIUM_1( samModule, "isTemporary = %d",
    newReq->isTemporary );
  ...
  if ( newReq->isTemporary == false )
  {
    sns_sam_distribute_sensor_req( newReq );
  }
  return SAM_ENONE;
}
...
sns_sam_initiate_flush( sam_client_req *clientReq, smr_req_handle reqHndl )
{
  ...
  if( 0 == sns_q_cnt( &clientReq->flushReqQ ) )
  {
    ...
    if( SAM_ENONE != err )
    {
      ...
      clientReq->last_flush_ts = sns_em_get_system_time();
    }
  }
      ...
      ( NULL != flushReq && NULL != reqHndl && SAM_ENONE != err ) )
  {
    ...
    if ( clientReq->flushReqQ.cnt > 0)
    {
      uint32_t delta_us;
      sns_ddf_time_t cur_ts = sns_em_get_system_time();
      if ( cur_ts < clientReq->last_flush_ts )
      {
        delta_us = sns_em_convert_dspstick_to_usec(
          cur_ts - (int64_t)clientReq->last_flush_ts);
      }
      else
      {
        delta_us = sns_em_convert_dspstick_to_usec(
          cur_ts - clientReq->last_flush_ts);
      }
      SNS_ASSERT(delta_us < FLUSH_REQ_TIMEOUT_US);
    }
