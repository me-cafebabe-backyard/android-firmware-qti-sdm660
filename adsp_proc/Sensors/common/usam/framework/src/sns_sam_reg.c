...
sns_sam_reg_req( sns_sam_reg_data const *regData )
{
  ...
  sns_reg_group_read_req_msg_v02 readRegReqMsg;
  sns_reg_group_write_req_msg_v02 writeRegReqMsg;
  ...
  else
  {
    ...
    reqMsg.msg.buf = (intptr_t)&writeRegReqMsg;
    SNS_OS_MEMZERO( (void*)reqMsg.msg.buf, reqMsg.msg.bufSize );
    reqMsg.msgID = SNS_REG_GROUP_WRITE_REQ_V02;

    writeRegReqMsg.group_id = regData->groupID;
    writeRegReqMsg.data_len = regData->groupDataLen;
    SNS_OS_MEMSCPY( writeRegReqMsg.data, SNS_REG_MAX_GROUP_BYTE_COUNT_V02,
