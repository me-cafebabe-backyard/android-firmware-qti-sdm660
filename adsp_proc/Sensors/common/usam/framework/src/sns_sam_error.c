...
//error_ind_msg struct size vary due to inconsistent def(i.e PED(Most Common)8 bytes,CMC 12 bytes)
//here we define MAX error msg size to 12
//NOTE: sns_sam_cmc_error_ind_msg_v02 will be garbage due to different struct def
#define SNS_SAM_MAX_ERROR_IND_MSG_SIZE 12
/*============================================================================
  Static Data
  ===========================================================================*/
...
sns_sam_send_error_ind( sensor1_error_e sensor1Err, sam_client_req const *clientReq )
{
  ...
  sns_sam_ped_error_ind_msg_v01 * errorIndPtr;
  uint8_t errorIndBuf[SNS_SAM_MAX_ERROR_IND_MSG_SIZE]={ 0 };
  int32_t errQMI;

  errorIndPtr = (sns_sam_ped_error_ind_msg_v01 *)errorIndBuf;
  ...
  {
    SNS_ASSERT(indMsg.msg.bufSize <= SNS_SAM_MAX_ERROR_IND_MSG_SIZE);
    indMsg.msg.buf = (intptr_t)errorIndPtr;
    ...
    errorIndPtr->instance_id = clientReq->extInstanceID;
    errorIndPtr->error = sensor1Err;
