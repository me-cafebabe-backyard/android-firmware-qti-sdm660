...
sns_usam_task( void *argPtr )
{
  ...
  for( ;; )
  {
    ...
    //Reset the SAM MPPS vote based on Queue count after processing all signals
...
sns_sam_determine_uimage( sns_sam_sensor_algo const *algo )
{
  ...
  {
    if( SNS_OS_IS_UIMG_ADDR(algo->algoAPI->sns_sam_algo_update) )
    {
      uImage = true;
    }
  }
...
/* check if ts1 is equal or advanced than ts2 within delta range */
SNS_SAM_UIMAGE_CODE bool
sns_sam_compare_ts_delta( sns_sam_timestamp const ts1,
                          sns_sam_timestamp const ts2,
                          sns_sam_timestamp const delta )
{
  if(( (ts1>ts2) && ((ts1-ts2) < delta))
           || ((ts1<ts2) && ( (ts2-ts1) > (SNS_SAM_TIMESTAMP_MAX - delta) ) )
           || (ts1 == ts2) )
  {
    return true;
  }
