...
$Header: //components/rel/ssc.slpi/2.2/osa/src/sns_osa_dsps.c#1 $
$DateTime: 2017/04/17 22:20:39 $
$Author: pwbldsvc $

when       who     what, where, why
---------- --- ---------------------------------------------------------
2016-12-20 bd  Moved sns_os_sigs_accept() to uimage
...
2016-02-22 qm  add semaphore wrapper
                                            ...
                                            uint8_t          *name)
{
  ...
  if (!pbos)
  {
    ...
    if(NULL == (stack_ptr = (char *) SNS_OS_MALLOC(SNS_OSA, new_stk_size)))
    {
...
OS_EVENT     *sns_os_mutex_create          (uint8_t          prio,
                                            uint8_t         *perr)
{
  OS_EVENT *mutex_event = SNS_OS_MALLOC(SNS_OSA, sizeof(OS_EVENT));
...
void sns_os_sem_init_val(OS_SEM *sem, uint16_t val){
  qurt_sem_init_val(sem, val);
}

int32_t sns_os_sem_up(OS_SEM *sem){
  return qurt_sem_up(sem);
}

int32_t sns_os_sem_down(OS_SEM *sem){
  return qurt_sem_down(sem);
}
