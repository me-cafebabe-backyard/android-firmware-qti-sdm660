...
$Header: //components/rel/ssc.slpi/2.2/pm/src/sns_pm_unit_test.c#1 $


when         who     what, where, why
----------   ---     ---------------------------------------------------------
08-30-2016   pc      Fix compiler errors/warnings in ADSP_STANDALONE mode
...
/*=====================================================================
                         INTERNAL DATA TYPES
=======================================================================*/
#define NUM_SUPPORTED_IMAGE_MODE 4
...
void sns_pm_ut_log_status(char *msg)
{
  ...
  sns_pm_ut_status[ut_status_index].status.core_handle.pmic_client_vdd1 = sns_pm.core_handle.pmic_client_vdd1;
  sns_pm_ut_status[ut_status_index].status.core_handle.pmic_client_vddio1 = sns_pm.core_handle.pmic_client_vddio1;
  sns_pm_ut_status[ut_status_index].status.core_handle.pmic_client_vdd2 = sns_pm.core_handle.pmic_client_vdd2;
  sns_pm_ut_status[ut_status_index].status.core_handle.pmic_client_vddio2 = sns_pm.core_handle.pmic_client_vddio2;
  ...
  for(i = 0; i < SNS_PM_CLIENT_ID_MAX; i++)
  {
    ...
    if(sns_pm.status.client_handle[i] != NULL)
    {
      ...
      sns_pm_ut_status[ut_status_index].clients[i].mpps = sns_pm.status.client_handle[i]->mpps;
    }
  }
  ...
  sns_pm_ut_status[ut_status_index].status.status.total_mcps = sns_pm.status.total_mcps;
...
void sns_pm_perform_ut_thread_1(void *p_arg)
{
  ...
  /*Update rail value as needed and/or add to test_package */
  uint8_t rail = 0xff;
  ...
  for(mode = 0; mode < NUM_SUPPORTED_IMAGE_MODE; mode++)
  {
    ...
    for(test_itr = 0; test_itr < test_package_t1.num_test_cases; test_itr++)
    {
      ...
      {
      case CLIENT_INIT :
        rv_err = sns_pm_client_init(&test_package_t1.test_clients[test_package_t1.num_test_clients],
                                    test_package_t1.testcases[test_itr].fn_param.callback, "TEST1", SNS_PM_CLIENT_ID_DEBUG);
        ...
        break;
      case VOTE_PWR_RAIL :
        rv_err = sns_pm_vote_power_rail(ut_client_handle, rail, test_package_t1.testcases[test_itr].fn_param.pwr_rail);
        PM_MSG_1(LOW, "[T1] VOTE_PWR_RAIL     returned [%d]", (int)rv_err);
        break;
      case VOTE_MIPS :
        rv_err = sns_pm_vote_mpps(ut_client_handle, test_package_t1.testcases[test_itr].fn_param.mips>>2); //convert MIPS to MPPS
...
void sns_pm_perform_ut_thread_2(void *p_arg)
{
  ...
  for(mode = 0; mode < NUM_SUPPORTED_IMAGE_MODE; mode++)
  {
    ...
    for(test_itr = 0; test_itr < test_package_t2.num_test_cases; test_itr++)
    {
      ...
      {
      case CLIENT_INIT :
        rv_err = sns_pm_client_init(&test_package_t2.test_clients[test_package_t2.num_test_clients],
                                     test_package_t2.testcases[test_itr].fn_param.callback, "TEST2", SNS_PM_CLIENT_ID_DEBUG);
        if(rv_err == SNS_PM_SUCCESS)
        {
        ...
        break;
      case VOTE_PWR_RAIL :
        rv_err = sns_pm_vote_power_rail(ut_client_handle, rail, test_package_t2.testcases[test_itr].fn_param.pwr_rail);
        PM_MSG_1(LOW, "[T2] VOTE_PWR_RAIL     returned [%d]", (int)rv_err);
        break;
      case VOTE_MIPS :
        rv_err = sns_pm_vote_mpps(ut_client_handle, test_package_t2.testcases[test_itr].fn_param.mips>>2); //Conver MIPS to MPPS
