...
$Header: //components/rel/ssc.slpi/2.2/pm/src/sns_pm.c#1 $


when         who     what, where, why
----------   ---     ---------------------------------------------------------
03-27-2017   sd      Include uSleep_mode_trans.h for 8998
20-03-2017   kn      DDR BW voting api converted to async type to reduce processing latency
09-03-2017   kn      vddio1 rail needs to keep always ON to fix high rbsc observed on SDM660 RCM
02-01-2017   kn      Turn off vddio1 rail when sensor use case is not running on Starlord
10-26-2016   rt      Do not turn off LVS2(SSC_PX) rail
08-17-2016   qm      change MIPS term to MCPS/MPPS
06-28-2016   pn      Changed frequently logged messages to LOW
05-25-2016   rt      Fix SLPI deadlock issue on island entry
03-14-2016   gju     Use consolidated SNS_OS_FREE to free any memory block
01-08-2016   ps      Fix compiler errors/warnings in ADSP_STANDALONE mode
...
/* Init variables */
bool sns_pm_mcps_initial_vote = true;
...
extern sns_em_timer_obj_t uimg_enter_tmr_obj;
                                          ...
                                          sns_pm_client_id_e client_id)
{
  ...
  if(sns_pm.status.num_clients >= SNS_PM_CLIENT_ID_MAX || sns_pm_initialized == false || client_id >= SNS_PM_CLIENT_ID_MAX ||
                                          ...
                                          sns_pm_pwr_rail_e state )
{
  ...
  {
    /* Do not turn off LVS2 (SSC_PX) rail */
    /* This keeps the SSC_PX rails always ON and prevents crash during SSR */
    if ((state == SNS_PWR_RAIL_OFF) && (rail == SNS_PM_SENSOR_VDDIO1))
    {
      return SNS_PM_SUCCESS;
    }
...
FUNCTION sns_pm_vote_mpps

DESCRIPTION
  This function sets the clocks on Q6 for sensors based on client
  MPPS. MPPS from all clients will be aggregated and it will scale to the
  appropriate value to meet SVS2 / SVS / NOMINAL / TURBO clks.

===========================================================================*/
sns_pm_err_code_e sns_pm_vote_mpps( sns_pm_handle_t client_handle,
                                    uint32 mpps)
{
  ...
  if(sns_pm.status.current_mode == SNS_IMG_MODE_MICRO)
  {
    //PM_MSG_0(MED, "Vote MPPs : not allowed in Micro Image");
    return SNS_PM_ERR_NOT_ALLOWED;
  }
  else
  {
    ...
    /* Due to initial clk required for init, if it's the very fist vote from client,
       directly assign it to total mpps to remove initial value offset */
    if(sns_pm_mcps_initial_vote == true)
    {
      sns_pm.status.total_mcps = mpps*SNS_PM_AVERAGE_CYCLES_PER_PACKET;
      sns_pm_mcps_initial_vote = false;
    }
    else
    {
      /* Only need to update the difference of the requested client */
      sns_pm.status.total_mcps += mpps*SNS_PM_AVERAGE_CYCLES_PER_PACKET;
      sns_pm.status.total_mcps -= client_info->mpps * SNS_PM_AVERAGE_CYCLES_PER_PACKET;
    }

    PM_MSG_3(MED, "Client[%d] Vote MCPS: %d, total MCPS:%d", client_info->client_id, mpps* SNS_PM_AVERAGE_CYCLES_PER_PACKET, sns_pm.status.total_mcps);
    /* Update the client info */
    client_info->mpps = mpps;

    /* Send vote to ADSP PM */
    sns_pm_internal_vote_mcps(sns_pm.status.total_mcps);
...
FUNCTION sns_pm_internal_vote_mcps

DESCRIPTION
This function convert client's MPPS to MCPS and aggregated and
it will be forwarded to ADSP PM which sets the clocks appropriately.
...
void sns_pm_internal_vote_mcps(uint32_t mcps)
{
    /*
      Caution:
      In SNS_PM we are using MCPS/MPPS
      However MMPM api/struct using MIPS
      So keep in mind MIPS and MCPS mean the same in this func
    */
    ...
    {
        PM_MSG_1(MED, "MCPS voting failed %d", result_status);
    }
}
/*===========================================================================
...
void sns_pm_internal_vote_ddr_bw ( uint32_t bw , int percentage )
{
  ...
  PM_MSG_1(LOW, "Voting DDR BW bytesperSec %d", bw);
                                              ...
                                              uint32 wakeup_interval )
{
  ...
  else
  {
    ...
    //PM_MSG_2(HIGH, "Set Wakeup Interval : client 0x%x voted wakeup_interval[%d]", client_handle, wakeup_interval);
    ...
    else
    {
      ...
      /* Save the current wakeup interval for the Max Duration client */
      sns_pm.status.wakeup_interval = wakeup_interval;
    }
...
void sns_pm_uSleep_notification_cb(uSleep_state_notification state)
{
  ...
  if(state == USLEEP_STATE_ENTER)
  {
    ...
    /* Allow/disallow SLPI Retention based on Max Sleep duration*/
    if(sns_pm.status.wakeup_interval > 2000 )
    {
...
void sns_pm_uimg_enter_timer_cb(void *arg)
{
  ...
  {
    PM_MSG_0(LOW, "timer expired : vote for uimg");
    sns_profiling_log_qdss(SNS_PM_VOTE_IMG_MODE, 1, SNS_IMG_MODE_MICRO);
    /* Remove Max Sleep duration required on voting for island mode
        so that Sleep driver allows island entry */
    sns_pm_set_wakeup_interval(NULL, 0);
