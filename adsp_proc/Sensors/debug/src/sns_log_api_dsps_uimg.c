...
/*==============================================================================
  ...
  ---------- ---  -----------------------------------------------------------
  2017-01-13 qm   Fix log pack logging incorrect issue
  2016-10-26 bd   Do not send log packet when debug thread is not up
  2016-02-29 bd   Added HD22 support
  2016-02-04 qmao enable query log packet bit mask from diag directly
...
#define NUM_MASK_MOD_SIZE(x)  (x % (sizeof(sns_log_mask_t) * 8 ))
                                     ...
                                     void     *log_pkt)
{
  ...
  {
    SNS_OS_FREE( (uint8_t *)log_pkt - sizeof(log_pkt_size) );
                                 ...
                                 void** log_pkt_ptr)
{
...
#endif /* UIMG_LOG */
#ifdef UIMG_LOG 
  if( sns_debug_pm_client_handle == NULL )
  {
    return SNS_ERR_NOTALLOWED;
  }
#endif
  ...
  *log_pkt_ptr = NULL;
  ...
  if (((sns_log_mask_t)1 << log_pkt_type) & sns_diag_dsps_log_mask_arry[NUM_MASK_ARRAY_IDX(log_pkt_type)])
  {
    return SNS_ERR_NOTALLOWED;
  }
                                 ...
                                 void      *log_pkt)
{
  ...
  //return sns_logpkt_submit(log_pkt_id, log_pkt_size, log_pkt);
...
//  }
  ...
  /* Filter log packets based on log mask */
  if (((sns_log_mask_t)1 << NUM_MASK_MOD_SIZE(log_pkt_type)) &
      sns_diag_dsps_log_mask_arry[NUM_MASK_ARRAY_IDX(log_pkt_type)])
  {
    return SNS_ERR_NOTALLOWED;
  }
                                         ...
                                         uint32_t pkt_size)
{
  ...
  {
    SNS_OS_FREE((uint8_t *)qmi_log_pkt_msg - sizeof(uint32_t));
    return SNS_ERR_NOMEM;
  }
}
...
void sns_debug_logpkt_drop(void)
{
  ...
  while (sns_q_cnt(&sns_debug_logpkt_buff.hold_queue) > 0)
  {
    ...
    if (logpkt_msg_ptr != NULL)
    {
      ...
      SNS_OS_FREE((uint8_t *)log_pkt_ptr - sizeof(logpkt_msg_ptr->pkt_size));
      SNS_OS_FREE(logpkt_msg_ptr);
    }
    else
      break;
  }
...
void sns_debug_logpkt_commit_send(void)
{
  ...
  while (sns_q_cnt(&sns_debug_logpkt_buff.hold_queue) > 0)
  {
    ...
    if (logpkt_msg_ptr != NULL)
    {
      ...
      if (ret_val == 0)
        SNS_SMGR_PRINTF1(HIGH, "Fail to submit logpkt %u", logpkt_msg_ptr->pkt_type);
