...
/* $Header: //components/rel/ssc.slpi/2.2/smgr/src/sns_smgr_reg.c#1 $ */
...
/*============================================================================
  ...
  2016-06-15  BD   Added LPS22HB
  2016-04-22  MW   Added CM3526, TMG490x
  2015-04-12  SD   Added LSM6DSM
  2016-03-17  BD   Added LIS3MDL
  2015-10-05  TC   Enable GPIO configuration with registry
...
extern sns_hw_gpio_config sns_smgr_hw_ssc_gpio_config[SNS_REG_DDF_SLPI_GPIO_CONFIG_MAX];
...
#endif
#ifdef CONFIG_SUPPORT_LPS22HB
	{ SNS_REG_UUID_LPS22HB, &sns_dd_press_lps22hb_if},	
#endif
#ifdef CONFIG_SUPPORT_LSM6DS3
{ SNS_REG_UUID_LSM6DS3, &sns_dd_lsm6ds3_if},
{ SNS_REG_UUID_LSM6DS3_DB, &sns_dd_lsm6ds3_if},
#endif
...
#endif
#ifdef CONFIG_SUPPORT_CM3526
    { SNS_REG_UUID_CM3526, &sns_dd_cm3526_if},
#endif
#ifdef CONFIG_SUPPORT_TMG490x
    { SNS_REG_UUID_TMG490x, &sns_dd_tmg490x_if},
#endif
  ...
  sns_common_resp_s_v01  sns_resp)
{
  ...
  else
  {
                  ...
                  ix < ARR_SIZE(cfg_group_ptr->drv_cfg); ix++ )
    {
      ...
      {
        sensor_cfg_ptr->vdd_rail   = 0x93; // Match SNS_PM_SENSOR_VDD1 in sns_pm_priv.h
        sensor_cfg_ptr->vddio_rail = 0x2;
        SNS_SMGR_PRINTF2(HIGH, "ssi: pwrrails not specified using defaults vdd:%u vddio:%u",
  ...
  sns_common_resp_s_v01   sns_resp)
{
            ...
            (item_type == SNS_SMGR_REG_ITEM_TYPE_GROUP) )
  {
    ...
    {
      bool is_slpi_gpio = (bool) (reg_gpio_conf->gpio_num & 0xFF00);
      ...
      while ( (!found_empty_slot) && (i < SNS_REG_DDF_SLPI_GPIO_CONFIG_MAX) )
      {
        if ( SNS_SMGR_HW_INVALID_GPIO == sns_smgr_hw_ssc_gpio_config[i].gpio_num ||
             ( (gpio_num == sns_smgr_hw_ssc_gpio_config[i].gpio_num) &&
               (is_slpi_gpio == sns_smgr_hw_ssc_gpio_config[i].is_slpi_gpio) ) )
        {
          // Found an appropriate gpio config slot, filling it in now
          sns_smgr_hw_ssc_gpio_config[i].is_slpi_gpio      = is_slpi_gpio;
  ...
  else if ( (item_id == SNS_REG_SCM_GROUP_MAG_DYN_CAL_PARAMS_V02) &&
            (item_type == SNS_SMGR_REG_ITEM_TYPE_GROUP) )
  {
    sns_sam_cal_report_ind_msg_v01 mag_cal_ind_msg;
    mag_dyn_cal_reg_data = *(sns_reg_mag_cal_params_data_group_s*)data_ptr;
        ...
        else
        {
          ...
          SNS_SMGR_PRINTF2(
            HIGH, "ix=%d reg_id=%u", ix, (unsigned)sensor_cfg_ptr->driver_reg_id);
                ...
                (sensor_ptr->ddf_sensor_ptr[SNS_SMGR_DATA_TYPE_PRIMARY_V01]->all_cal_ptr != NULL) )
      {
        ...
        {
          mag_fac_cal_reg_data = *(sns_reg_mag_cal_params_data_group_s*)data_ptr;
          ...
          cal_db_ptr->zero_bias[0] = 0;
          cal_db_ptr->zero_bias[1] = 0;
          cal_db_ptr->zero_bias[2] = 0;

          //load initial active mode for SI param
          for(iz = 0; iz < PHONE_MODE_NUM; iz++)
          {
            if(true == mag_fac_cal_reg_data.mode[iz].active)
          {
              break;
            }
          }
          if(PHONE_MODE_NUM == iz)
          {
            //default to mode 0 if registry mode setting invalid
            iz = 0;
          }
          sns_smgr.cal_phone_mode = iz;
          for(iz = 0; iz < PHONE_MODE_NUM; iz++)
          {
            mag_fac_cal_reg_data.mode[iz].active = 0;
          }
          mag_fac_cal_reg_data.mode[sns_smgr.cal_phone_mode].active = 1;
          //clean up registry mode active info
          ...
          SNS_SMGR_PRINTF1(HIGH,
                  "proc_reg_data - loading initial active mag cal SI param for phone mode:%u",
                   sns_smgr.cal_phone_mode);
          ...
          //we have mag fac_cal/SI, now let's get auto_cal/HI as well
          //request for mag HI param, get it at smgr init rather than wait for qmagcal to send us
          //this is needed to meet CTS spec
          sns_smgr_req_reg_data(SNS_REG_SCM_GROUP_MAG_DYN_CAL_PARAMS_V02, SNS_SMGR_REG_ITEM_TYPE_GROUP);
        }
        else if ( smgr_is_valid_fac_cal(reg_data_ptr) ) //this part handle Gyro fac cal?
        {
          uint32_t i;
