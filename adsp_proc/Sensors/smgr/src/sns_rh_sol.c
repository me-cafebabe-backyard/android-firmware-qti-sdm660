...
/* $Header: //components/rel/ssc.slpi/2.2/smgr/src/sns_rh_sol.c#3 $ */
/* $DateTime: 2017/09/11 01:26:56 $ */
/* $Author: pwbldsvc $ */
...
/*============================================================================
  ...
  ----------  ---  -----------------------------------------------------------
  2017-05-18  bd   Use min_sample_interval when setting last_sent_ts to avoid timestamp rollback 
  2017-03-21  bd   Signal Sample_ready when updating item info
  2017-02-17  sd   Move sample ready signal for the first valid sample after cofig 
                   event done to avoid sample not saved
  2017-02-08  sd   Fixed no indication for streaming 2 event sensors in one report
  2017-01-16  sd   when replacing exiting report, reply status should be success
  2016-12-07  bd   Freed pending reports before deleting reports
  2016-12-07  bd   Updated associated reports in RH context
  2016-12-01  bd   Fixed possible mem leak
  2016-11-16  sd   added query timestamp roll over check  
  2016-11-15  bd   - Save samples when processing new buffering request
                   - Do not allocate pending report for periodic_like reports
  2016-11-04  bd   - Sent first report indication sooner in concurrent usecases.
                   - Updated item info in RH context
  2016-10-21  bd   Fixed mutex dead lock issue
  2016-09-30  sd   Added sns_rh_sol_link_lastest_sample_ts 
  2016-08-26  np   Added playback support.
  2016-08-25  sd   reset interpolator interval to 0 if no interpolation required.
  2016-07-22  pn   Uses depot interval as report's min_sampling_interval
  ...
  2016-06-28  pn   - Fixed interpolation init bug causing initial samples to be discarded
                   - Fixed missing Periodic/Periodic-like indications
                   - Changed some debug messages
  2016-06-10  jtl  Fix interpolation_required.
  2016-06-08  jtl  Don't interpolate if the actual rate is within 5% of requested.
  2016-06-03  jtl  Force interpolation S4S sensor timestamps if S4S is not synced
  2016-05-18  pn   Periodic-like Buffering report are treated the same as Batching reports
  2016-05-05  jtl  Updated compute_sampling_factor for S4S
  2016-04-13  ks   Dynamic calibration reset api support  
  2016-04-05  jtl  Initial S4S implementation
...
/* Don't interpolate if the actual ODR is within this % of the requested ODR */
#define RH_SOL_INTERP_THRESH_PCT 5
 ...
 * Local Functions
 * -------------------------------------------------------------------------*/
SMGR_STATIC void sns_rh_sol_reschedule_report(sns_rh_rpt_spec_s* rpt_ptr);
...
SMGR_STATIC void sns_rh_sol_compute_cic_factor(sns_rh_rpt_item_s* item_ptr)
{
  ...
  {
    q16_t min_rate = item_ptr->sampling_rate_hz * (100-RH_SOL_INTERP_THRESH_PCT) / 100;
    q16_t max_rate = item_ptr->sampling_rate_hz * (100+RH_SOL_INTERP_THRESH_PCT) / 100;
    q16_t acc_ts_rate = item_ptr->ddf_sensor_ptr->depot_data_rate/item_ptr->sampling_factor;

    cic_factor = item_ptr->sampling_factor;
    
    if ( SMGR_BIT_TEST(item_ptr->flags, RH_RPT_ITEM_FLAGS_ACCURATE_TS_B) ||
         ( min_rate < acc_ts_rate &&
           max_rate > acc_ts_rate ) )
    {
  ...
  /* Re init the CIC filter if the CIC factor has changed, or if the CIC frequency
   * has changed by more than the threshold percent */
  if ( (cic_factor != item_ptr->cic_obj.factor) ||
       ( (cic_frequency <
          (item_ptr->cic_obj.input_frequency * (100-SMGR_ODR_THRESHOLD_DELTA_PCT) )) &&
         (cic_frequency >
          (item_ptr->cic_obj.input_frequency * (100+SMGR_ODR_THRESHOLD_DELTA_PCT) )) ) )
  {
...
/*!
  @Detail Interpolation is done only if
          ...
          - OR
          - sensor is S4S sensor, but not yet synced
  ...
  const sns_rh_rpt_item_s* item_ptr)
{
  ...
  /* Minimum & maximum allowed rates for no interpolation. If effective rate is outside
   * of this range, interpolate the data if it's not an accurate TS client */
  q16_t resulting_rate = item_ptr->ddf_sensor_ptr->depot_data_rate / item_ptr->sampling_factor;
  q16_t min_rate = item_ptr->sampling_rate_hz * (100-RH_SOL_INTERP_THRESH_PCT) / 100;
  q16_t max_rate = item_ptr->sampling_rate_hz * (100+RH_SOL_INTERP_THRESH_PCT) / 100;
  ...
  if ( !sns_smgr_ddf_sensor_is_event_sensor(item_ptr->ddf_sensor_ptr) &&
       ...
       ( min_rate > resulting_rate ||
         max_rate < resulting_rate ) )
  {
    interpolation_required = true;
  } 
  else if( SMGR_SENSOR_IS_S4S(item_ptr->ddf_sensor_ptr->sensor_ptr) &&
           item_ptr->ddf_sensor_ptr->sensor_ptr->curr_s4s_sched &&
           !item_ptr->ddf_sensor_ptr->first_synced_s4s_sample_received )
  {
    interpolation_required = true;
    SNS_SMGR_PRINTF0(MED, "interp_req: forcing interpolation for non-synced S4S");
  }
             
  SNS_SMGR_PRINTF5(LOW, "interp_req: req=%u min=0x%x max=0x%x "
                   "sampling_hz=0x%x resulting_rate=0x%x",
                   interpolation_required, min_rate, max_rate,
                   item_ptr->sampling_rate_hz, resulting_rate );
...
{
  uint32_t orig_interval_ticks = item_ptr->interpolator_obj.interval_ticks;
  ...
  if ( sns_rh_sol_interpolation_required(item_ptr) )
  {
    ...
    if ( item_ptr->samples_sent )
    {
      if ( item_ptr->ts_last_saved != 0 )
      {
        ...
        sns_smgr_s4s_get_next_sample_ts(item_ptr->ddf_sensor_ptr, item_ptr->ts_last_saved);
      }
      else
      {
        item_ptr->interpolator_obj.desired_timestamp =
          item_ptr->ts_last_sent + item_ptr->sampling_interval;
        sns_smgr_s4s_get_next_sample_ts(item_ptr->ddf_sensor_ptr, item_ptr->ts_last_sent);
      }
    }

    SNS_SMGR_PRINTF4(
      MED, "init_interp - intvl(old/new)=%u/%u desired_ts=%u now=%u",
      orig_interval_ticks, item_ptr->interpolator_obj.interval_ticks, 
      item_ptr->interpolator_obj.desired_timestamp, sns_ddf_get_timestamp());
  }
  else
  {
    if ( item_ptr->cic_obj.factor > 1 )
...
SMGR_STATIC void sns_rh_sol_compute_effective_rate(sns_rh_rpt_item_s* item_ptr)
{
  ...
  q16_t min_rate = item_ptr->sampling_rate_hz * (100-RH_SOL_INTERP_THRESH_PCT) / 100;
  q16_t max_rate = item_ptr->sampling_rate_hz * (100+RH_SOL_INTERP_THRESH_PCT) / 100;
  q16_t acc_ts_rate = ddf_sensor_ptr->depot_data_rate/item_ptr->sampling_factor;
...
SMGR_STATIC bool sns_rh_flush_report(sns_rh_rpt_spec_s *rpt_ptr)
{
  ...
  if ( rpt_ptr->state == RH_RPT_STATE_FLUSHING )
  {
    ...
    {
      SNS_SMGR_PRINTF4(
        MED, "flush_report - rpt_id=%u/0x%x state=0x%02x #inds=%u",
        rpt_ptr->rpt_id, rpt_ptr->msg_header.connection_handle, 
...
{
  uint32_t i = 0;
  while ( i < SNS_SMGR_MAX_ITEMS_PER_REPORT_V01 && ddf_sensor_ptrs[i] != NULL )
  {
  ...
  uint8_t report_id)
{
  ...
  {
    SNS_SMGR_PRINTF4(
      MED, "find_report - rpt_id=%u/0x%x suspended=%u #inds=%u",
  ...
  sns_smgr_ddf_sensor_s* ddf_sensor_ptr)
{
        ...
        ddf_sensor_ptr->depot_data_rate) )
  {
    ...
    while ( item_ptr != NULL )
    {
      ...
      {
        sns_rh_prepare_report_for_flushing(item_ptr->parent_report_ptr);
      }
      item_ptr = item_ptr->next_item_ptr;
    }
  }
  ...
  sns_smgr_reason_pair_s_v01*                 reason_pairs)
{
  ...
  SNS_SMGR_PRINTF2(MED, "parse_periodic_item - sensor=%u dtype=%u",
  ...
  sns_smgr_reason_pair_s_v01*              reason_pairs)
{
  ...
  SNS_SMGR_PRINTF3(MED, "parse_buffering_item - sensor=%u dtype=%u rate=%u",
                   in_item_ptr->SensorId, in_item_ptr->DataType,
            ...
            (SENSOR_STATE_PRE_INIT == ddf_sensor_ptr->sensor_ptr->sensor_state) )
  {
    ...
    sns_rh_sol_add_reason_code(
      ...
      num_pairs_ptr, reason_pairs);
    SNS_SMGR_PRINTF2(
      ERROR, "parse_buffering_item - sensor=%u state=%u",
             ...
             (in_item_ptr->SamplingRate <= SNS_SMGR_SAMPLING_RATE_INVERSION_POINT_V01)) )
  {
    sns_rh_sol_add_reason_code(
      ...
      num_pairs_ptr, reason_pairs);
    SNS_SMGR_PRINTF2(
      ERROR, "parse_buffering_item - req_rate=%u max=0x%X",
  ...
  sns_rh_rpt_spec_s*                    rpt_ptr)
{
  ...
  SNS_SMGR_PRINTF3(
    HIGH, "parse_notify_suspend - src=%u proc=%u send=%u",
    src_module, rpt_ptr->proc_type, rpt_ptr->send_ind_during_suspend);
}
  ...
  sns_rh_rpt_spec_s*                     rpt_ptr)
{
  ...
  SNS_SMGR_PRINTF4(
    LOW, "parse_periodic_report - rpt_id=%u/0x%x #items=%d ackNak=%u",
    rpt_ptr->rpt_id, rpt_ptr->msg_header.connection_handle, rpt_ptr->num_items, ackNak);
  return ackNak;
}
  ...
  sns_rh_rpt_spec_s*               rpt_ptr)
{
       ...
       (ackNak == SNS_SMGR_RESPONSE_ACK_MODIFIED_V01) )
  {
    if ( rpt_ptr->num_items > 0 )
    {
      ...
      SNS_SMGR_PRINTF3(
        LOW, "parse_buffering_report - q16_rr=0x%X rpt_intvl=%u per_like=%u",
  ...
  SNS_SMGR_PRINTF4(
    LOW, "parse_buffering_report - rpt_id=%u/0x%x #items=%d ackNak=%u",
    rpt_ptr->rpt_id, rpt_ptr->msg_header.connection_handle, rpt_ptr->num_items, ackNak);
  return ackNak;
}
  ...
  bool*               pending_resp_ptr )
{
  ...
  if ( rpt_pending != NULL )
  {
    ...
    rpt_pending->resp_ptr->sns_result_t = SNS_RESULT_SUCCESS_V01;
    rpt_pending->resp_ptr->sns_err_t    = SENSOR1_SUCCESS;
    sns_rh_mr_send_resp(&rpt_pending->msg_header, (void**)&rpt_pending->resp_ptr);
    ...
    if ( q_ptr == rpt_queue_ptr && rpt_ptr->rpt_tick != 0 )
    {
      ...
      if ( pending_rpt_ptr != NULL )
      {
        ((sns_smgr_buffering_resp_msg_v01*)pending_rpt_ptr->resp_ptr)->AckNak = 
          SNS_SMGR_RESPONSE_ACK_SUCCESS_V01;
        pending_rpt_ptr->resp_ptr->sns_result_t = SNS_RESULT_SUCCESS_V01;
        pending_rpt_ptr->resp_ptr->sns_err_t    = SENSOR1_SUCCESS;
        sns_rh_mr_send_resp(&pending_rpt_ptr->msg_header, (void**)&pending_rpt_ptr->resp_ptr);
...
SMGR_STATIC void sns_rh_sol_activate_report(sns_rh_rpt_spec_s* rpt_ptr)
{
  ...
  SNS_SMGR_PRINTF5(
    HIGH, "activate_report - rpt_id=%u/0x%x type=%u/0x%x #rpts=%u", 
    rpt_ptr->rpt_id, rpt_ptr->msg_header.connection_handle, 
    rpt_ptr->msg_header.svc_num, rpt_ptr->msg_header.msg_id, sns_q_cnt(rpt_queue_ptr));
  SNS_SMGR_PRINTF4(
    HIGH, "activate_report - addr=0x%x proc=%u state=0x%x per/ev=0x%x", 
    rpt_ptr, rpt_ptr->proc_type, rpt_ptr->state << 8 | rpt_ptr->state_extended, 
    rpt_ptr->periodic_like << 1 | rpt_ptr->event_sensors_only);
  ...
  sns_smgr_buffering_resp_msg_v01** resp_ptr_ptr)
{
  ...
  else
  {
         ...
         ack_nak == SNS_SMGR_RESPONSE_ACK_MODIFIED_V01 )
    {
      ...
      {
        uint8_t i;
        for (i=0; i<req_ptr->Item_len &&
                  rpt_ptr->num_items < ARR_SIZE(rpt_ptr->item_list); i++)
        {
          if(req_ptr->Item[i].Calibration == SNS_SMGR_CAL_SEL_FULL_CAL_V01 )
          {
            SNS_SMGR_PRINTF2(HIGH,"Starting calibration for sensor %d Cal %d",
                        req_ptr->Item[i].SensorId,req_ptr->Item[i].Calibration );
            sns_rh_cal_scheduler(req_ptr->Item[i].SensorId,true);
          }
        }
      }
    }
...
/*===========================================================================
  ...
  FUNCTION:  sns_rh_sol_update_cal_scheduler

===========================================================================*/
/*!
  @brief   Updates the calibration algorithms for the affected sensors.

  @details After a report is deleted its associated sensors might need
           to disable the associated calibration algorithms .
 ...
 */
/*=========================================================================*/
SMGR_STATIC void sns_rh_sol_update_cal_scheduler(
  sns_smgr_ddf_sensor_s*   ddf_sensor_ptrs[SNS_SMGR_MAX_ITEMS_PER_REPORT_V01])
{
  uint32_t i = 0;
  ...
  while ( i < SNS_SMGR_MAX_ITEMS_PER_REPORT_V01 && ddf_sensor_ptrs[i] != NULL )
  {
    sensor_id = ddf_sensor_ptrs[i]->sensor_ptr->const_ptr->sensor_id;
    i++;
    SNS_SMGR_PRINTF1(HIGH,"sns_rh_sol_update_cal_scheduler sensor id %i",sensor_id);

    sns_rh_cal_scheduler(sensor_id,false);
  }
}
...
void sns_rh_sol_delete_report(sns_rh_rpt_spec_s* rpt_ptr)
{
  ...
  sns_rh_sol_update_cal_scheduler(ddf_sensor_ptrs);  
}
/*===========================================================================
...
void sns_rh_sol_deactivate_report(sns_rh_rpt_spec_s* rpt_ptr)
{
  ...
  {
    SNS_OS_FREE(rpt_ptr->ind_buffer);
    rpt_ptr->ind_buffer = NULL;
  }
  if ( rpt_ptr->pending_rpt_ptr != NULL  )
  {
    SNS_SMGR_PRINTF4(
    HIGH, "deactivate_report - addr=0x%x pending ptr=0x%x msg_header=0x%x resp=0x%x", 
    rpt_ptr, rpt_ptr->pending_rpt_ptr, &rpt_ptr->pending_rpt_ptr->msg_header, rpt_ptr->pending_rpt_ptr->resp_ptr);
    
    sns_rh_mr_send_resp(&rpt_ptr->pending_rpt_ptr->msg_header, 
                        (void**)&rpt_ptr->pending_rpt_ptr->resp_ptr);
    sns_rh_sol_delete_report(rpt_ptr->pending_rpt_ptr);
    rpt_ptr->pending_rpt_ptr = NULL;
  }
  if ( rpt_ptr->resp_ptr != NULL  )
  {
    SNS_SMGR_PRINTF4(
    HIGH, "deactivate_report - addr=0x%x pending ptr=0x%x msg_header=0x%x resp=0x%x", 
    rpt_ptr, rpt_ptr->pending_rpt_ptr, &rpt_ptr->msg_header, rpt_ptr->resp_ptr);
    sns_rh_mr_send_resp(&rpt_ptr->msg_header, 
                        (void**)&rpt_ptr->resp_ptr);
  }
      ...
      if(ack_nak == SNS_SMGR_RESPONSE_ACK_SUCCESS_V01)
       {
         ...
         for (i=0; i<req_ptr->Item_len &&
                   rpt_ptr->num_items < ARR_SIZE(rpt_ptr->item_list); i++)
         {
           if(req_ptr->cal_sel[i] == SNS_SMGR_CAL_SEL_FULL_CAL_V01 )
           {
              SNS_SMGR_PRINTF2(HIGH,"Starting calibration for sensor %d Cal %d",
                         req_ptr->Item[i].SensorId,req_ptr->cal_sel[i] );
              sns_rh_cal_scheduler(req_ptr->Item[i].SensorId,true);
           }
  ...
  sns_rh_mr_req_q_item_s* msg_ptr)
{
  ...
  if ( resp_ptr != NULL )
  {
    ...
    sns_rh_mr_send_resp(&msg_ptr->header, (void**)&resp_ptr);
  }
  ...
  sns_rh_mr_req_q_item_s*  msg_ptr)
{
  ...
  SNS_SMGR_PRINTF5(
    MED, "buffering_req - rpt_id=%u/0x%x action=%u #items=%u rr=0x%x",
    req_ptr->ReportId, msg_ptr->header.connection_handle, 
    req_ptr->Action, req_ptr->Item_len, req_ptr->ReportRate);
...
/*===========================================================================
  ...
  FUNCTION:   sns_rh_sol_update_ts

===========================================================================*/
/*! 
  @brief expand 32 bit time to 64 bit acording to current time and roll over status
 
  @details
  

  @param[i] t         32 bit time to be expanded
            time_now  64 bit current time

  @return 64 bit expanded time for t
*/
/*=========================================================================*/
static sns_ddf_time_t sns_rh_sol_update_ts(uint32_t t,sns_ddf_time_t time_now)
{
  uint32_t time_now_32bit= (uint32_t)time_now;
  if ( SNS_RH_32BIT_SAME_SIGNBIT(t,time_now_32bit ) )
  {
    return (t + sns_em_system_time_offset) | ((uint64_t)time_now & 0xFFFFFF00000000);
  }
  else if ( SNS_RH_32BIT_GREATER (t,time_now_32bit) &&
            SNS_RH_32BIT_ROLLOVER_CHECK(time_now_32bit, t) )
  {
    return (t + sns_em_system_time_offset) | ((uint64_t)time_now & 0xFFFFFF00000000)
            + 0x100000000;
  }
  else if ( SNS_RH_32BIT_GREATER ( time_now_32bit, t ) &&
            SNS_RH_32BIT_ROLLOVER_CHECK( t, time_now_32bit ) )
  ...
  sns_rh_mr_req_q_item_s* msg_ptr)
{
  ...
  t0 = sns_rh_sol_update_ts(req_ptr->TimePeriod[0], time_now );
  t1 = sns_rh_sol_update_ts(req_ptr->TimePeriod[1], time_now );
...
void sns_rh_sol_compute_sample_ratio(sns_rh_rpt_spec_s* rpt_ptr)
{
  ...
  RH_FOR_EACH_REPORT_ITEM( rpt_ptr, i )
  {
    ...
    SNS_SMGR_PRINTF5(
      LOW, "sample_ratio - rpt_id=%u/0x%x sensor=%u spr=%u ratio*100=%u",
      rpt_ptr->rpt_id, rpt_ptr->msg_header.connection_handle,
      SMGR_SENSOR_ID(item_ptr->ddf_sensor_ptr->sensor_ptr),
      item_ptr->samples_per_report, item_ptr->sample_cnt_ratio*100);
  }
}
...
===========================================================================*/
/*!
  @brief Updates the given report item after sensor is reconfigured.
 
  @note Executes in RH context.

  @param[i] items_ptr - the report item to update

  @return none
*/
/*=========================================================================*/
SMGR_STATIC void sns_rh_sol_update_items_info(sns_rh_rpt_item_s* item_ptr)
{
  ...
  SNS_SMGR_PRINTF5(
    HIGH, "items_info - sensor=%u rpt_id=%u/0x%x updated(item/sensor)=%u/%u",
    SMGR_SENSOR_TYPE(ddf_sensor_ptr->sensor_ptr, ddf_sensor_ptr->data_type),
    rpt_ptr->rpt_id, rpt_ptr->msg_header.connection_handle, item_ptr->ts_last_updated, 
    ddf_sensor_ptr->client_stat.client_modifier.last_updated);
  ...
  if ( item_ptr->ts_last_sent == 0 )
  {
    ...
    item_ptr->ts_last_sent = 
      rpt_ptr->msg_header.timestamp -
      item_ptr->ddf_sensor_ptr->current_filter_delay -
      item_ptr->parent_report_ptr->min_sampling_interval;
#else
    item_ptr->ts_last_sent = 
  ...
  if ( item_ptr->state < RH_RPT_ITEM_STATE_IDLE )
  {
         ...
         ddf_sensor_ptr->latest_sample_ts != 0 )
    {
      ...
      if ( rpt_ptr->event_sensors_only && rpt_ptr->rpt_interval == 0 )
      {
  ...
  item_ptr->ts_last_updated = time_now;
  sns_rh_signal_me(SNS_RH_SAMPLE_READY_SIG);

  SNS_SMGR_PRINTF5(
    MED, "items_info - rpt_id=%u/0x%x sensor=%u ind_cnt=%u quality=%d",
    rpt_ptr->rpt_id, rpt_ptr->msg_header.connection_handle,
    SMGR_SENSOR_TYPE(ddf_sensor_ptr->sensor_ptr, ddf_sensor_ptr->data_type),
    rpt_ptr->ind_count, item_ptr->quality);
  SNS_SMGR_PRINTF5(
    MED, "items_info - state=%u rate=0x%x samp_intvl=%u factor(smpl/cic)=%u/%u",
    item_ptr->state, item_ptr->effective_rate_hz, item_ptr->sampling_interval,
    item_ptr->sampling_factor, item_ptr->cic_obj.factor);
  SNS_SMGR_PRINTF4(
    MED, "items_info - updated=%u last(proc/saved/sent)=%u / %u / %u",
    item_ptr->ts_last_updated, item_ptr->last_processed_sample_timestamp, 
    item_ptr->ts_last_saved, item_ptr->ts_last_sent);
}

#ifdef FAST_PLAYBACK
/*===========================================================================

  FUNCTION:   sns_rh_sol_playback_update_item
...
void sns_rh_sol_playback_update_item(
  sns_smgr_ddf_sensor_s *ddf_sensor_ptr,
  bool                  new_item_only)
{
  struct sns_rh_rpt_item_s*  item_ptr;
  struct sns_rh_rpt_item_s** item_ptr_ptr;
  /* ---------------------------------------------------------------------- */

  SNS_SMGR_PRINTF3(
    HIGH, "items_info - ddf_sensor=%d new_only=%u depot_intvl=%u",
    SMGR_SENSOR_TYPE(ddf_sensor_ptr->sensor_ptr, ddf_sensor_ptr->data_type), 
    new_item_only, ddf_sensor_ptr->depot_data_interval);

  SMGR_FOR_EACH_ASSOC_ITEM( ddf_sensor_ptr, item_ptr, item_ptr_ptr )
  {
    if ( new_item_only && (item_ptr->state != RH_RPT_ITEM_STATE_LINKING) )
    {
      continue;
    }
    sns_rh_sol_update_items_info(item_ptr);
  }
}
#endif
...
void sns_rh_update_periodic_like_flag(sns_rh_rpt_spec_s* rpt_ptr)
{
  ...
  else if ( rpt_ptr->q16_rpt_rate != 0 )
  {
    ...
    RH_FOR_EACH_REPORT_ITEM( rpt_ptr, i )
    {
      ...
      rpt_ptr->item_list[i]->sample_cnt_ratio = 1.0 / rpt_ptr->num_items;
    }
...
/*===========================================================================
  ...
  FUNCTION:   sns_rh_sol_update_assoc_reports

===========================================================================*/
/*!
  @brief Prepares reports associated with the given sensor for rescheduling
 
  @details Executes in RH context when the given sensor completes (re)configuration.
 ...
 */
/*=========================================================================*/
void sns_rh_sol_update_assoc_reports(sns_smgr_ddf_sensor_s* ddf_sensor_ptr)
{
  sns_rh_rpt_item_s*  item_ptr;
  sns_rh_rpt_item_s** item_ptr_ptr;
  sns_ddf_time_t      ts_sensor_config_change = 
    ddf_sensor_ptr->client_stat.client_modifier.last_updated;
  /* ---------------------------------------------------------------------- */
  ...
  SMGR_FOR_EACH_ASSOC_ITEM( ddf_sensor_ptr, item_ptr, item_ptr_ptr )
  {
    ...
    if ( rpt_ptr->state != RH_RPT_STATE_DELETING && 
         item_ptr->ts_last_updated < ts_sensor_config_change )
    {
      sns_rh_sol_update_items_info(item_ptr);
    }
    ...
    if ( SNS_RH_REPORT_IS_PERIODIC(rpt_ptr) || rpt_ptr->periodic_like ||
         ...
         rpt_ptr->state == RH_RPT_STATE_DELETING ||
         rpt_ptr->ts_last_updated >= ts_sensor_config_change ||
         item_ptr->effective_rate_hz == 0 )
    {
    ...
    if ( sns_rh_sol_compute_min_sampling_interval(rpt_ptr) )
    {
      sns_rh_sol_compute_sample_ratio(rpt_ptr);
      sns_rh_sol_reschedule_report(rpt_ptr);
    }
  }
}
...
/*===========================================================================
  ...
  FUNCTION:   sns_rh_sol_reschedule_report

===========================================================================*/
/*!
  @brief Reschedules the given report if necessary
      ...
      uint8_t i;
       RH_FOR_EACH_REPORT_ITEM( rpt_ptr, i )
       {
         sns_ddf_time_t latest_sample_ts = 
           rpt_ptr->item_list[i]->ddf_sensor_ptr->latest_sample_ts;
...
/*!
  ...
  @detail  Report in extended state == RH_RPT_STATE_EXT_ACTIVATE_PENDING 
           gets activated, sends response, and gets deleted.
           Also pending report attached to rpt_ptr will send response and 
           be deleted.
