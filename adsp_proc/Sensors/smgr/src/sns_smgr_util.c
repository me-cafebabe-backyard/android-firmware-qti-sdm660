...
/* $Header: //components/rel/ssc.slpi/2.2/smgr/src/sns_smgr_util.c#1 $ */
...
/*============================================================================
  ...
  2016-05-27  jtl  Update compute_depot_data_rate to support drivers disabling S4S
  2016-05-23  jtl  Update get_sched_intval_wm to round to nearest tick
  2016-04-13  ks   Dynamic calibration reset api support
  2016-03-30  jtl  Updated s4s. Added get_sched_intval_wm
  2016-03-08  pn   Added sns_smgr_set_md_state()
  2016-02-22  tc   Added is_daf_active function
  2015-10-12  jtl  Intial S4S implementation
...
{
  uint32  i, num_axes = ARR_SIZE(cal_ptr->zero_bias);
  ...
  cal_ptr->compensation_matrix_valid = false;

  for ( i = 0; i < ARR_SIZE(cal_ptr->compensation_matrix); i++ )
  {
    if (i % (num_axes+1) == 0)
    {
      cal_ptr->compensation_matrix[i] = FX_FLTTOFIX_Q16(1.0);
    }
    else
    {
      cal_ptr->compensation_matrix[i] = 0;
    }
  }
}
...
{
  return sns_smgr_get_sched_intval_wm( req_freq, 1 );
}

/*===========================================================================

  FUNCTION:   sns_smgr_get_sched_intval_w_wm

===========================================================================*/
/*!
  @brief Calculates an interval from requested freq and watermark

  @param[i] req_freq - The freq in Hz, in Q16 format
  @param[i] watermark - the FIFO watermark. If wm=0, behaves as if wm=1.

  @return
   The timetick of the scheduling period
*/
/*=========================================================================*/
uint32_t sns_smgr_get_sched_intval_wm(q16_t req_freq, int32_t watermark)
{
  uint32_t interval_tick;
  float wm = watermark?watermark:1.0;
  interval_tick = roundf((float)DSPS_SLEEP_CLK*wm/FX_FIXTOFLT_Q16_SP(req_freq));
  SNS_SMGR_PRINTF4(
    LOW, "freq_to_intvl_w_wm - wm=%d freq=0x%x/%u intvl=%u ",
    watermark, req_freq, req_freq>>16, interval_tick);
  ...
  const sns_smgr_ddf_sensor_s* ddf_sensor_ptr)
{
       ...
       SMGR_SENSOR_IS_S4S(ddf_sensor_ptr->sensor_ptr) )
  {
    if( SMGR_SENSOR_IS_S4S(ddf_sensor_ptr->sensor_ptr) &&
        ddf_sensor_ptr->sensor_ptr->curr_s4s_sched )
    {
      if( ddf_sensor_ptr->sensor_ptr->curr_s4s_sched->ideal_sync_interval == 0 ||
          ddf_sensor_ptr->current_s4s.t_ph == 0 )
      {
        /* Driver has set the isi or the t_ph to 0, trying to disable S4S.
         * Set the depot_data_rate according to the reported ODR */
        depot_data_rate =
          DDF_ODRTOFIX_Q16(ddf_sensor_ptr->current_odr) / 
          ddf_sensor_ptr->device_sampling_factor;
        where = 1;
      }
      else if( ddf_sensor_ptr->sensor_ptr->curr_s4s_sched->synced )
      {
        /* If the S4S schedule is synced, use the current tph & sync interval
           to compute the depot data rate */
        float isi = ddf_sensor_ptr->sensor_ptr->curr_s4s_sched->ideal_sync_interval;
        float tph = ddf_sensor_ptr->current_s4s.t_ph;
        
        SNS_SMGR_PRINTF2(MED, "compute_depot_rate - isi=%u tph=%u", isi, tph );
        depot_data_rate = FX_FLTTOFIX_Q16( (float)DSPS_SLEEP_CLK * tph / isi );
        where = 2;
      }
      else
      {
        /* If this is a yet-to-be-synced S4S sensor, adjust the ODR by
         * the nominal S4S interval */
        const q16_t s4s_ratio =
          FX_FLTTOFIX_Q16( (float)DSPS_SLEEP_CLK /(float)NOMINAL_TPH_PERIOD_TICKS );

        depot_data_rate =
          DDF_ODRTOFIX_Q16(ddf_sensor_ptr->current_odr) / 
          ddf_sensor_ptr->device_sampling_factor;
        depot_data_rate = FX_MUL_Q16( depot_data_rate, s4s_ratio );
        where = 3;
      }
    }
    else
  ...
  SNS_SMGR_PRINTF4(
    MED, "compute_depot_rate - ddf_sensor=%d depot_rate=0x%X/%u case=%u",
    SMGR_SENSOR_TYPE(ddf_sensor_ptr->sensor_ptr, ddf_sensor_ptr->data_type),
    depot_data_rate, depot_data_rate >> 16, where);
...
/*===========================================================================
  ...
  FUNCTION:   smgr_sensor_type_max_depot_data_rate

===========================================================================*/
/*!
  @brief Returns the max depot data rate value of all sensor types
...
*/
/*=========================================================================*/
q16_t smgr_sensor_type_max_depot_data_rate(const sns_smgr_sensor_s* sensor_ptr)
{
  q16_t max_depot_data_rate = 0;
  uint32_t i;
  ...
  for ( i = 0; i < ARR_SIZE(sensor_ptr->ddf_sensor_ptr); i++ )
  {
    if ( sensor_ptr->ddf_sensor_ptr[i] != NULL )
    {
      max_depot_data_rate = MAX(max_depot_data_rate,
                                sensor_ptr->ddf_sensor_ptr[i]->depot_data_rate);
    }
  }
  return max_depot_data_rate;
}
...
/*===========================================================================
  ...
  FUNCTION:   sns_smgr_is_daf_active

===========================================================================*/
/*!
  @brief Checks if DAF is active for any sensor

  @Detail

  @return
    TRUE if DAF is active for any sensor
    FALSE if there are no active DAF requests
*/
/*=========================================================================*/
bool sns_smgr_is_daf_active(void)
{
  int i;

  /* Go through all of the sensors to see if any of them have an active DAF
     request */
  for ( i = 0; i < ARR_SIZE(sns_smgr.sensor); i++ )
  {
    if ( sns_smgr.sensor[i].daf_active )
    {
      return true;
    }
  }
