...
/* $Header: //components/rel/ssc.slpi/2.2/smgr/src/sns_smgr_fifo.c#2 $ */
/* $DateTime: 2017/05/29 00:36:25 $ */
/* $Author: pwbldsvc $ */
...
/*============================================================================
  ...
  ----------  ---  -----------------------------------------------------------
  2016-12-14  sd   do not duplicate wakeup interval vote with sibling sensors
  2016-06-28  pn   - Fixed FIFO WM calculation for S4S sensors
                   - Prevented attempt to flush empty FIFOs
  ...
  2016-02-22  bd   Updated handling of FIFO_AVAILABLE and FIFO_UNAVAILABLE events
  2016-02-26 jtl   Still calculate ewur for S4S sensors
...
)
{
                       ...
                       shared_sensor_ptr, sibling_link)
  {
    ...
    if ( sibling_sensor_ptr->sensor_state > SENSOR_STATE_IDLE )
    {
      ...
      sibling_sensor_ptr->fifo_cfg.is_fifo_enabled = available;
    }
  }
...
)
{
  ...
  SNS_SMGR_PRINTF1( MED, "FIFO SNS_DDF_EVENT_FIFO_AVAILABLE ddf_sensor=%d", sensor );
...
{
  sns_smgr_ddf_sensor_s* ddf_sensor_ptr = sns_smgr_match_ddf_sensor( sensor_ptr, sensor );
  SNS_SMGR_PRINTF1( MED, "FIFO SNS_DDF_EVENT_FIFO_UNAVAILABLE ddf_sensor=%d", sensor );
  if ( ddf_sensor_ptr == NULL )
  {
    return SNS_DDF_EINVALID_PARAM;
  }
...
)
{
      ...
      sensor_ptr->fifo_cfg.can_fifo_be_used)
  {
    ...
    if( SMGR_SENSOR_IS_S4S(sensor_ptr) && desired_wm != 0)
    {
      SNS_SMGR_PRINTF0(MED, "set_watermark - S4S enabled. Setting max WM");
      fifo_attr.watermark = sensor_ptr->fifo_cfg.max_watermark;
    }
    else
    {
      fifo_attr.watermark = desired_wm;
    }
    SNS_SMGR_PRINTF2(MED, "set_watermark - desired_wm=%u fifo_attr.wm=%u",
                     desired_wm, fifo_attr.watermark);
        ...
        ( curr_wm != 0 && desired_wm == 0 ) )
    {
                       ...
                       shared_sens_ptr, sibling_link)
      {
        ...
        if ( sibling_ptr->fifo_cfg.voted_wkup_intvl ) {
          vote_wkupintvl = false;
          break;
        }
        if (vote_wkupintvl) {
          sns_smgr_update_ewur(new_intr_rate, old_intr_rate,sensor_ptr->is_uimg_refac, true );
          sensor_ptr->fifo_cfg.voted_wkup_intvl = true;
        }
      }
    }
  }
...
sns_err_code_e sns_smgr_fifo_set_wm_and_intr(sns_smgr_sensor_s* sensor_ptr)
{
  ...
  sns_smgr_ddf_sensor_s* ddf_sensor_ptr = SMGR_PRIMARY_SENSOR(sensor_ptr);
  bool enable_intr = (sensor_ptr->fifo_cfg.desired_watermark > 0) ? 
    (SMGR_SENSOR_IS_S4S(sensor_ptr) ? false : true) : false;
 ...
 */
/*=====================================================================*/
q16_t sns_smgr_fifo_get_max_rpt_rate(const sns_smgr_sensor_s* sensor_ptr)
{
  smgr_fifo_shared_sensor_s *shared_sensor_ptr;
  ...
  uint32_t*          sum_depot_ptr)
{
       ...
       SMGR_PRIMARY_SENSOR(sensor_ptr)->client_stat.num_rpt_items != 0 )
  {
    ...
    q16_t ddr = smgr_sensor_type_max_depot_data_rate(sensor_ptr);
    uint32_t ideal_wm = ddr / max_rpt_rate;

    SNS_SMGR_FIFO_DBG4(
      LOW, "ideal_wm - sensor=%u max_depot_data_rate=0x%x depot_max=%u wm=%u", 
      SMGR_SENSOR_ID(sensor_ptr), ddr, sensor_ptr->fifo_cfg.depot_max, ideal_wm);
...
SMGR_STATIC void sns_smgr_fifo_calculate_watermarks(sns_smgr_sensor_s* sensor_ptr)
{
  ...
  {
    sns_smgr_sensor_s* sibling_ptr = shared_sens_ptr->sensor_ptr;
	sibling_ptr->fifo_cfg.voted_wkup_intvl = false;
...
/*!
  ...
  @return True if the watermark has changed
 */
/*=========================================================================*/
bool sns_smgr_fifo_configure( sns_smgr_sensor_s* sensor_ptr, bool force_wm1 )
{
  smgr_fifo_shared_sensor_s* shared_sensor_ptr;
  bool rv = false;
...
{
  bool samples_in_fifo = false;
  if ( SMGR_SENSOR_FIFO_IS_SUPPORTED(sensor_ptr) )
  {
    sns_smgr_ddf_sensor_s* primary_sensor_ptr = SMGR_PRIMARY_SENSOR(sensor_ptr);
    if ( primary_sensor_ptr->latest_sample_ts + primary_sensor_ptr->depot_data_interval +
         primary_sensor_ptr->current_filter_delay <= sns_ddf_get_timestamp() )
    {
      samples_in_fifo = true;
    }
  }

  if ( samples_in_fifo && (force_flush || sensor_ptr->fifo_cfg.current_watermark > 1) )
  {
