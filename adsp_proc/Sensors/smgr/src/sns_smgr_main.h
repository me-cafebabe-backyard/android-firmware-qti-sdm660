...
/* $Header: //components/rel/ssc.slpi/2.2/smgr/src/sns_smgr_main.h#1 $ */
...
/*============================================================================
  ...
  ----------  ---  -----------------------------------------------------------
  2016-03-21  bd   Remove field added to sns_smgr_ddf_sensor_s to avoid conflict 
                   operation in RH and SMGR
  2016-02-17  sd   Added new field to sns_smgr_ddf_sensor_s
  2016-07-28  jtl  Add a nominal S4S second, for conversions between ODRs and S4S TPH.
  2016-06-03  jtl  Keep track of first S4S synced sample received
  2016-05-23  jtl  Update S4S nominal interval to be 32000 ticks (from 33000 ticks) 
  2016-04-04  pn   Distinguishes between requested ODR and current ODR
  2016-01-28 jtl   Updated S4S structure to be dynamic
...
/* Time in ticks for the S4S TPH period */
#define NOMINAL_S4S_1SEC_TICKS 32000LL
#define NOMINAL_TPH_PERIOD_TICKS (NOMINAL_S4S_1SEC_TICKS * 1LL)
...
/* S4S scheduler */
typedef struct smgr_s4s_sched_block_s
{
  /* All of the S4S schedules. One per DDF sensor ptr */
  sns_q_link_s                 sched_link;

  /* associated DDF sensor */
  sns_smgr_ddf_sensor_s*       ddf_sensor_ptr;

  /* Ideal time in one TPH interval */
  sns_ddf_time_t               ideal_sync_interval;

  /* Start time of the current TPH interval */
  sns_ddf_time_t               ideal_sync_offset;

  /* Time when the next ST/DT messages should be sent */
  sns_ddf_time_t               st_send_time;

  /* Max clock drift over one TPH interval */
  sns_ddf_time_t               max_clock_drift;

  /* true if clocks are synced */
  bool                         synced;

  /* Number of DTs missed in a row */
  int32_t                      missed_dts;
} smgr_s4s_sched_block_s;
...
typedef struct  smgr_sched_block_s
{
  ...
  /* The current scheduling interval by clock ticks. */
  ...
  /* Number of ticks the polling schedule has been temporarily delayed. The
   * schedule will make up for that delay on the next sampling interval 
   * by decrementing the new "next_tick" by this value. */
...
struct sns_smgr_ddf_sensor_s
{
  ...
  bool                        first_synced_s4s_sample_received;
  uint16_t                    flags;
  ...
  /* The S4S parameter configured in the device */
  sns_ddf_s4s_config_s        current_s4s;
...
struct sns_smgr_sensor_s 
{
  ...
  /* The S4S parameter configured in the device */
  smgr_s4s_sched_block_s         *curr_s4s_sched;
...
typedef struct  
{
  ...
  /*phone mode info to decide which cal param to load from registry*/
...
extern void sns_smgr_s4s_init_new_sched_block( sns_smgr_sensor_s* sensor_ptr );

extern void sns_smgr_s4s_config_s4s( sns_smgr_ddf_sensor_s* ddf_sensor_ptr,
                                     bool                   restart_schedule );

extern void sns_smgr_s4s_update_s4s( sns_smgr_ddf_sensor_s* ddf_sensor_ptr );

extern int32_t sns_smgr_s4s_proc_s4s_schedule( void );

extern void sns_smgr_s4s_reg_into_sched( sns_smgr_sensor_s *sensor_ptr );

extern void sns_smgr_s4s_dereg_from_sched( sns_smgr_sensor_s* sensor_ptr );

extern sns_ddf_time_t sns_smgr_s4s_get_next_sched_time( void );

extern bool sns_smgr_s4s_same_config(
  const sns_ddf_s4s_config_s * restrict config1,
  const sns_ddf_s4s_config_s * restrict config2 );

extern void sns_smgr_s4s_check_first_synced_sample( sns_smgr_ddf_sensor_s* ddf_sensor_ptr );

extern sns_ddf_time_t sns_smgr_s4s_get_next_sample_ts( sns_smgr_ddf_sensor_s const* ddf_sensor_ptr,
  sns_ddf_time_t timestamp );

extern void sns_smgr_handle_mode_switch_sig( void );
