...
/* $Header: //components/rel/ssc.slpi/2.2/smgr/src/sns_rh_batchbuff.c#1 $ */
...
/*============================================================================
  ...
  ----------  ---  -----------------------------------------------------------
  2016-11-15  bd   Set report to flushing only if not pending report exist
  2016-10-04  bd   Send 1 sample when sample_rate_ratio is very low, and reserve samples for subHz items
...
SMGR_STATIC void sns_rh_batchbuff_print(void)
{
  ...
  SMGR_FOR_EACH_Q_ITEM( rpt_queue_ptr, rpt_ptr, rpt_link )
  {
    if ( SNS_RH_REPORT_IS_BUFFERING(rpt_ptr) )
    {
      uint16_t i;
      SNS_SMGR_PRINTF3(
        HIGH, "print - rpt_id=%u/0x%x is_batching=%u", 
...
===========================================================================*/
/*!
  @brief Drops the batchbuff block with oldest samples

  @param[i] item_ptr - the report item on which to operate
...
===========================================================================*/
/*!
  @brief Removes samples exceeding reserved size for nonwakeup sensors
         ...
         sns_rh_ok_to_trim_nonwakeup_buffer(rpt_ptr) )
    {
      ...
      RH_FOR_EACH_REPORT_ITEM( rpt_ptr, i )
      {
        ...
        uint32_t reserved_fifo_size = 
          ...
          SNS_RH_BATCHBUFF_MAX_SAMPLE;
        while ( item_ptr->num_saved_samples > reserved_fifo_size )
        {
...
SNS_RH_UIMAGE_CODE SMGR_STATIC void sns_rh_batchbuff_flush(bool ssc_only)
{
  ...
  SMGR_FOR_EACH_Q_ITEM( rpt_queue_ptr, rpt_ptr, rpt_link )
  {
    if ( SNS_RH_REPORT_IS_BATCHING(rpt_ptr) )
    {
      ...
      if ( send_rpt && rpt_ptr->state_extended == RH_RPT_STATE_EXT_NONE )
      {
...
SMGR_STATIC void sns_rh_batchbuff_move_all(void)
{
  ...
  SMGR_FOR_EACH_Q_ITEM( rpt_queue_ptr, rpt_ptr, rpt_link )
  {
    if ( SNS_RH_REPORT_IS_BUFFERING(rpt_ptr) )
    {
      ...
      RH_FOR_EACH_REPORT_ITEM( rpt_ptr, i )
      {
        sns_rh_batchbuff_dump(rpt_ptr->item_list[i]);
      }
    }
  }
}
  ...
  uint16_t            subHz_item_cnt)
{
  ...
  RH_FOR_EACH_REPORT_ITEM( rpt_spec_ptr, i )
  {
    ...
    if ( item_spec_ptr->num_saved_samples > 0 )
    {
      ...
      if ( subHz_item_cnt > 0 && item_num_samples > 0 &&
        (num_samples + subHz_item_cnt) > SNS_SMGR_BUFFERING_REPORT_MAX_SAMPLES_V01 )
      {
        item_num_samples = (uint32_t)(item_spec_ptr->sample_cnt_ratio*(num_samples-subHz_item_cnt));
      }
      item_num_samples = MAX(item_num_samples, 1);
    }
    else
    {
      item_num_samples = 0;
...
SNS_RH_UIMAGE_CODE bool sns_rh_batchbuff_save_samples(sns_rh_rpt_item_s* item_ptr)
{
  ...
  uint8_t item_quality = item_ptr->quality;
  ...
  if ( item_ptr->state == RH_RPT_ITEM_STATE_LINKING ||
       ...
       (item_ptr->interpolator_obj.interval_ticks != 0 &&
        TICK1_GTR_TICK2(item_ptr->interpolator_obj.desired_timestamp,
                        latest_sample_ts))
       )
  {
          ...
          ((SMGR_SAMPLE_STATUS(sample_ptr) == SNS_DDF_SUCCESS) || item_ptr->samples_sent) )
  {
    ...
    sample_status = 
      sns_rh_process_sample(item_ptr, item_quality, sample_ptr, 
  ...
  SNS_SMGR_BATCH_DBG4(
    MED, "save_samples - #saved=%u last_saved=%u #uBB=%u #bBB=%u", 
    item_ptr->num_saved_samples, item_ptr->ts_last_saved,
    sns_q_cnt(&item_ptr->ubatch_buffer_queue),
    sns_q_cnt(&item_ptr->bbatch_buffer_queue));
