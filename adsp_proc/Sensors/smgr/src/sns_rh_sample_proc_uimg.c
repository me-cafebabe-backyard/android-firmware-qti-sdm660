...
/* $Header: //components/rel/ssc.slpi/2.2/smgr/src/sns_rh_sample_proc_uimg.c#1 $ */
...
/*============================================================================
  ...
  ----------  ---  -----------------------------------------------------------
  2016-10-04  np   Support for MAR reset added.
  2016-31-08  pc   SL bringup - Fixed Werror for unused rpt_ptr
  2016-08-17  bd   Fix infinite loop in RH during query test
  ...
  2016-06-28  pn   - Fixed interpolation init bug causing initial samples to be discarded
                   - Fixed timestamps jitter when interpolation stops
  2016-06-16  ks   Fix for exiting uImage before notifying mag anomaly  
  ...
  const sns_smgr_sample_s*    sample_ptr)
{
    ...
    if ( int_obj_ptr->sample1_ptr != NULL ) /* both s1 and s2 are present? */
    {
      ...
      if ( TICK1_GEQ_TICK2(ts2, ts1) ) /* s1 older than s2 - the normal case */
      {
        if ( TICK1_GTR_TICK2(ts1, int_obj_ptr->desired_timestamp) )
        {
          ...
          uint32_t num_intvls = 
            (ts1 - int_obj_ptr->desired_timestamp + int_obj_ptr->interval_ticks)/
            int_obj_ptr->interval_ticks;
          int_obj_ptr->desired_timestamp += (int_obj_ptr->interval_ticks * num_intvls);
#ifdef SNS_SMGR_INTERPOLATOR_DEBUG
          {
            SNS_SMGR_PRINTF5(
              ERROR, "interp_update - rpt_id=%u/0x%x ts1=%u ts=%u ts2=%u", 
              int_obj_ptr->item_ptr->parent_report_ptr->rpt_id, 
              int_obj_ptr->item_ptr->parent_report_ptr->msg_header.connection_handle,
              ts1, int_obj_ptr->desired_timestamp, ts2);
          }
#endif
        }
      }
  ...
  {
    SNS_SMGR_PRINTF5(
      ERROR, "interp_update - id=%u/0x%x ts1=%u desired=%u ts2=%u",
      int_obj_ptr->item_ptr->parent_report_ptr->rpt_id, 
      int_obj_ptr->item_ptr->parent_report_ptr->msg_header.connection_handle,
      int_obj_ptr->sample1_ptr ? SMGR_SAMPLE_TIMESTAMP(int_obj_ptr->sample1_ptr) : 0,
      int_obj_ptr->desired_timestamp,
    ...
    if ( SMGR_SENSOR_IS_MAG(ddf_sensor_ptr->sensor_ptr) )
    {
      if ( cal_data_ptr->calibration_accuracy != SNS_SMGR_CAL_ACCURACY_UNRELIABLE )
      {
        ...
        if (cal_mag_norm_sq > (SNS_SMGR_MAG_ANOMALY_DET_THRESH_GAUSS_SQ))
        {
          ...
          sns_rh_leave_island();
          reset_ret_code = sns_rh_cal_process_mar_request(
                           ddf_sensor_ptr->sensor_ptr->const_ptr->sensor_id);
          sns_rh_enter_island();
          if( reset_ret_code == SNS_SUCCESS )
          {
            SNS_SMGR_PRINTF0(HIGH, "smgr mag anomaly detected and algo resetted");
          }
          else
          {
            SNS_SMGR_PRINTF0(ERROR, "smgr mag anomaly detected but algo reset failed");
          }
        }
      }
  ...
  int32_t                   dest_data[SNS_SMGR_SENSOR_DIMENSION_V01])
{
  ...
  SNS_SMGR_SAMPLE_PROC_DBG3(
    ...
    item_ptr->parent_report_ptr->msg_header.connection_handle, item_quality);
  ...
  case SNS_SMGR_ITEM_QUALITY_CURRENT_SAMPLE_V01:
    if ( !same_sample )
    {
      ...
      {
        uint32_t delta = sample_timestamp -
          ((item_ptr->ts_last_saved != 0) ? item_ptr->ts_last_saved : item_ptr->ts_last_sent);
        uint32_t min_acceptable_gap =
          item_ptr->sampling_interval - (item_ptr->ddf_sensor_ptr->depot_data_interval >> 1);

        if ( delta >= min_acceptable_gap || 
        ...
        else /* sample too close to last sample sent */
        {
          ...
          item_ptr->sampling_count = item_ptr->sampling_factor - 1;
        }
      }
      item_ptr->sampling_count++;
      if ( item_ptr->sampling_count >= item_ptr->sampling_factor )
      {
  ...
  case SNS_SMGR_ITEM_QUALITY_INTERPOLATED_V01:
    ...
    SNS_SMGR_SAMPLE_PROC_DBG4(
      MED, "process_sample - rpt_id=%u desired=%u have=%u status=%u",
