...
/* $Header: //components/rel/ssc.slpi/2.2/smgr/src/sns_rh_internal_svc.c#1 $ */
...
/*============================================================================
  ...
  ----------  ---  -----------------------------------------------------------
  2017-01-17  bd   Schedule event gated reports when request is received 
  2016-12-01  bd   Fixed possible mem leak
  2016-07-13  np   Calibration algorithm support  
...
===========================================================================*/
/*!
  @brief Searches in given queue for a particular request on a given connection
         and returns it
  ...
  @param[i]   msg_id            - the request msg
  ...
  uint16_t    msg_id)
{
  ...
  SMGR_FOR_EACH_Q_ITEM( request_queue_ptr, q_item_ptr, q_link )
  {
    if ( q_item_ptr->header.connection_handle == conn_handle &&
         q_item_ptr->header.msg_id == msg_id )
    {
      found_q_item_ptr = q_item_ptr;
        sns_q_delete(&q_item_ptr->q_link);
      break;
  ...
  sns_rh_mr_req_q_item_s** msg_ptr_ptr)
{
  ...
  if ( ddf_sensor_ptr != NULL )
  {
    ...
    sns_rh_mr_req_q_item_s* existing_req_ptr = 
      sns_rh_find_request(q_ptr, msg_ptr->header.connection_handle,
                          msg_ptr->header.msg_id);
  ...
  sns_q_s*    q_ptr)
{
  ...
  SMGR_FOR_EACH_Q_ITEM(q_ptr, rpt_ptr, rpt_link)
  {
         ...
         (report_id == rpt_ptr->rpt_id) )
    {
      ...
      SNS_OS_FREE(rpt_ptr->item_list[0]);
      SNS_OS_FREE(rpt_ptr);
      found = true;
      break;
    }
  ...
  sns_smgr_event_gated_buffering_resp_msg_v02* resp_ptr )
{
  ...
  if ( ackNak == SNS_SMGR_RESPONSE_ACK_SUCCESS_V01 )
  {
    ...
    else /* event is disabled */
    {
      ...
      {
        d_rpt_ptr->holding_q = &se_ptr->disabled_state_rpt_q;     
        if ( se_ptr->status == SNS_SMGR_SENSOR_EVENT_STATUS_OCCURRED_V02 )
        {
          sns_rh_sol_schedule_report(hdr_ptr,d_rpt_ptr);
        }
        else
        {
          sns_q_put(&se_ptr->disabled_state_rpt_q, &d_rpt_ptr->rpt_link);
  ...
  while ( sns_rh.sensor_events[e].sensor_event != 0 )
  {
    ...
    for ( s=0; s<ARR_SIZE(sns_smgr.sensor); s++ )
    {
      ...
      for ( d=0; d<ARR_SIZE(sns_smgr.sensor[s].ddf_sensor_ptr); d++ )
      {
        ...
        if ( ddf_sensor_ptr != NULL )
        {
          sns_q_s* request_queue_ptr = &ddf_sensor_ptr->client_stat.sensor_status.request_queue;
          sns_rh_mr_req_q_item_s* q_item_ptr;
    ...
    if (num_reports > 0)
    {
      ...
      while ( num_reports-- > 0 )
      {
        ...
        SNS_OS_FREE(delete_rpt_ptr[num_reports]->item_list[0]);
        SNS_OS_FREE(delete_rpt_ptr[num_reports]);
      }
      sns_rh_disable_sensor_event(se_ptr);
    }
    e++;
