...
/* $Header: //components/rel/ssc.slpi/2.2/smgr/src/sns_smgr_ddf_if.c#2 $ */
/* $DateTime: 2017/05/19 04:21:44 $ */
/* $Author: pwbldsvc $ */
...
/*============================================================================
  ...
  2015-05-22  jtl   Added initial S4S implementation
  2016-05-17  qm   Uses SNS_OS_IS_UIMG_ADDR to check if addr is in uImage addr range
 ...
 * -------------------------------------------------------------------------*/
// The arbitrarily determined expected wakeup rate for an active DAF request
#define SNS_DAF_ACTIVE_EWUR_HZ         1000
#define SNS_DAF_ACTIVE_EWUR_HZ_Q16     FX_CONV_Q16(SNS_DAF_ACTIVE_EWUR_HZ, 0)
...
sns_ddf_status_e sns_ddf_smgr_notify_test_complete_with_data(
    sns_ddf_handle_t  smgr_handle,
    sns_ddf_sensor_e  sensor,
    sns_ddf_status_e  status,
    uint32_t          err,
    void * additional_info_ptr)
{
  sns_smgr_sensor_s* sensor_ptr = (sns_smgr_sensor_s*)smgr_handle;

  /* ---------------------------------------------------------------------- */

  SNS_SMGR_PRINTF3(HIGH, " notify_test_cmplt_with_data - sensor=%d status=%d err=%u",
                   sensor, status, err);
  ...
  {
    sns_smgr.self_test.result.additional_info_valid = true;
    /* Assumption that driver allocation for additional_info is 
       >= sizeof(sns_smgr.self_test.result.additional_info) */
    SNS_OS_MEMSCPY(&sns_smgr.self_test.result.additional_info, 
      sizeof(sns_smgr.self_test.result.additional_info),
      additional_info_ptr, sizeof(sns_smgr.self_test.result.additional_info));
  
    SNS_SMGR_PRINTF3(HIGH, " notify_test_cmplt_with_data - first=%d second=%d last=%u",
      sns_smgr.self_test.result.additional_info[0], 
      sns_smgr.self_test.result.additional_info[1], 
      sns_smgr.self_test.result.additional_info[ADDITIONAL_INFOLENGTH_V01-1]);
  }
  else
  {
    sns_smgr.self_test.result.additional_info_valid = false;
  ...
  void*                               conn_handle)
{
  ...
  // TODO: REMOVE THE "daf_active" FLAG, IF NECESSARY, AND PUT THE SMGR INTO THE
  //       RIGHT POWERSTATES AND MIPS+BW VOTES
...
void sns_ddf_smgr_set_uimg_refac(sns_ddf_handle_t smgr_handle)
{
  if ( SMGR_HANDLE_VALID((sns_smgr_sensor_s*)smgr_handle) )
  {
    ...
    if ( SNS_OS_IS_UIMG_ADDR(data_func) )
    {
  ...
  if ( SMGR_HANDLE_VALID(sensor_ptr) )
  {
    switch (event)
    {
      ...
      case SNS_DDF_EVENT_DAF_ACTIVE:
      {
        ...
        // Prevent the SMGR from entering into uImage mode
        sns_smgr_update_stay_in_bigimage();

        // Set the expected wakeup rate to some an appropriate value
        sns_smgr_update_ewur( SNS_DAF_ACTIVE_EWUR_HZ_Q16, 0, false, true );
      ...
      case SNS_DDF_EVENT_DAF_INACTIVE:
      {
        ...
        // Remove the expected DAF wakeup vote
        sns_smgr_update_ewur( 0, SNS_DAF_ACTIVE_EWUR_HZ_Q16, false, true );

        // Allow the SMGR to re-enter uImage mode
        sns_smgr_update_stay_in_bigimage();
        break;
      }
