...
/* $Header: //components/rel/ssc.slpi/2.2/smgr/src/sns_smgr_hw.h#1 $ */
...
/*============================================================================
  ...
  ----------  ---  -----------------------------------------------------------
  2016-04-21  jtl  Cleanup unused code. Set max supported I2C busses to 2 (to
                   save uImage memory)
...
#define SNS_SMGR_MAX_I2C_BUSSES 2
...
typedef struct
{
  ...
  uint32_t pull;
  uint32_t drive;
} sns_hw_gpio_state_setting;

typedef struct
{
  bool                      is_slpi_gpio;
  uint32_t                  gpio_num;
  sns_hw_gpio_state_setting active;
  sns_hw_gpio_state_setting inactive;
} sns_hw_gpio_config;
...
typedef struct
{
  ...
  sns_i2c_bus_s        i2c_bus_instances[SNS_SMGR_MAX_I2C_BUSSES];
  int                  i2c_bus_count;
} sns_hw_s;
