...
/* $Header: //components/rel/ssc.slpi/2.2/smgr/src/sns_rh_rpt_gen.c#1 $ */
...
/*============================================================================
  ...
  ----------  ---  -----------------------------------------------------------
  2017-01-25  db   fix compiling warning
  2016-12-22  sd   check timestamp order before packing query indication sample
  ...
  sns_rh_query_s* query_ptr)
{
  ...
  sns_ddf_time_t            previous_timestamp = 0;
  ...
  if ( item_quality != SNS_SMGR_ITEM_QUALITY_CURRENT_SAMPLE_V01 )
  {
    SNS_SMGR_QUERY_DBG3(
      MED, "query_ind - quality=%d interp_intvl=%d desired_ts=%u",
      item_quality, item_ptr->interpolator_obj.interval_ticks,
          ...
          TICK1_GEQ_TICK2(query_ptr->T1, SMGR_SAMPLE_TIMESTAMP(sample_ptr)) )
  {
    ...
    status = sns_rh_process_sample(item_ptr, item_quality, sample_ptr, &timestamp,
                                   ind_ptr->Samples[ind_ptr->Samples_len].Data);
    if ( (status == SNS_DDF_SUCCESS) && ((int64_t)(timestamp-previous_timestamp) >0) )
    {
      sns_rh_rpt_gen_pack_query_sample(
        query_ptr, ind_ptr, &timestamp, &previous_timestamp, item_quality);
    ...
    if ( ((item_quality != SNS_SMGR_ITEM_QUALITY_INTERPOLATED_V01) &&
          (item_quality != SNS_SMGR_ITEM_QUALITY_INTERPOLATED_FILTERED_V01) &&
       ...
       !sns_rh_mr_conn_busy(rpt_spec_ptr->msg_header.connection_handle) )
  {
    ...
    switch ( rpt_spec_ptr->msg_header.msg_id )
    {
      ...
      case SNS_SMGR_REPORT_REQ_V01:
        ...
        break;
      default:
        SNS_OS_FREE(rpt_spec_ptr->ind_buffer);
