...
/* $Header: //components/rel/ssc.slpi/2.2/smgr/inc/sns_smgr.h#2 $ */
...
#if defined(VIRTIO_BRINGUP) || defined(FAST_PLAYBACK_DEBUG)
...
typedef struct
{
  ...
  union
  {
    ...
    sns_smgr_sensor_cal_reset_req_msg_v01*   cal_reset_ptr;
    sns_smgr_phone_mode_switch_req_msg_v01*  mode_switch_ptr;
  } req;
  ...
  union
  {
    ...
    sns_smgr_sensor_cal_reset_resp_msg_v01*  cal_reset_ptr;
    sns_smgr_phone_mode_switch_resp_msg_v01*  mode_switch_ptr;
  } resp;
...
typedef struct
{
  ...
  uint8_t                     additional_info_valid;
  int32_t                     additional_info[ADDITIONAL_INFOLENGTH_V01]; //from driver
