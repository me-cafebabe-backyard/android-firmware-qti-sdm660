...
static inline void qurt_mem_region_attr_init(qurt_mem_region_attr_t *attr){
    ...
    attr->pga.pga_value = QURT_MEM_CACHE_WRITEBACK;
...
/**@ingroup func_qurt_mem_region_attr_get_bus_attr
  ...
  @param[in]  attr  Pointer to the memory region attribute structure.
  @param[out] pbits Pointer to an unsigned integer which will be filled in with
...
/**@ingroup func_qurt_mapping_reclaim
  ...
  - The associated TLB entries are deleted and made available for TLB
    management.\n
  - The virtual memory area itself is not freed - instead it is left in
    place as allocated but unmapped virtual memory. Any access to this
    memory area will generate an exception.\n
  ...
  NOTE - The virtual memory area is currently restricted to being
         completely filled with locked TLB entries which are contiguous
         within the memory area, and wholly contained by it.
  ...
  @param[in] vaddr   Virtual address of memory area to be freed.
  @param[in] vsize   Size (in bytes) of memory area to be freed.
  @param[in] pool    Handle to physical pool where freed physical memory is added.
                     If set to 0, freed physical memory is not added to any pool.

  @return
  0 -- Success\n
  Nonzero -- Failure.  This may indicate a partial success, or it may
       indicate that the request was malformed.  It is anticipated that
       QuRT will log some sort of messages related to the failure, and
       that callers will probably be free to ignore the return value.
...
/**@ingroup qurt_l2fetch_disable
  Disable L2FETCH activities on all hardware threads.
