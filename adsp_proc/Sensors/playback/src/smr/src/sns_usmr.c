...
smr_find_client(uint32_t client_id, bool *bigImage)
{
  ...
  {
    if (NULL != bigImage && !*bigImage && !SNS_OS_IS_UIMG_ADDR(curr))
    {
      SMR_LEAVE_ISLAND();
      *bigImage = true;
    }
...
smr_add_client(struct smr_client_hndl_s *client)
{
  ...
  if(SNS_OS_IS_UIMG_ADDR(client) || NULL == client_q)
  {
                                    ...
                                    void **connection_handle)
{
  ...
  if (NULL != service_data->connect_cb)
  {
    ...
    *  is not registered with the SMR. Allocate a wrapperfor the qmi_handle    */
    smr_qmi_client_handle  = SNS_OS_MALLOC(0, sizeof(struct smr_client_hndl_s));
...
smr_csi_disconnect_cb(void *connection_handle, void *service_cookie)
{
  ...
  if (NULL != connection_handle && NULL != service_cookie)
  {
    ...
    SNS_OS_FREE(smr_qmi_client_handle);
  }
}
                        ...
                        bool uImage)
{
  ...
  if (uImage && SNS_OS_IS_UIMG_ADDR(ind_cb) && service_index >= 0)
  {
    smr_client_p = SNS_OS_U_MALLOC(0, sizeof(struct smr_client_hndl_s));
  }
  if (NULL == smr_client_p)
  {
    smr_client_p = SNS_OS_MALLOC(0, sizeof(struct smr_client_hndl_s));
  }
  if (NULL == smr_client_p)
  {
  ...
  *client_handle = (smr_client_hndl)smr_client_p->client_id;
  ...
  SNS_COMMON_MSG_2(HIGH, "Client initiated : qmi_err %d, local idx",
      qmi_err, smr_client_p->service_index);
                             ...
                             smr_service_hndl *smr_handle)
{
  ...
  *smr_handle = smr_service;
  ...
  while( NULL != cb_data )
  {
    ...
    {
      SNS_ASSERT(NULL != cb_data->init_cb);
      ...
      cb_data->init_cb(cb_data->serviceObj, cb_data->instanceID, false, cb_data->smr_client_init_cb_param);
      SNS_OS_FREE(cb_data);
    }
    cb_data = (sns_smr_service_check_cb_data*)
                          ...
                          void *init_cb_param)
{
...
#if defined(FAST_PLAYBACK)
  SNS_ASSERT(NULL != init_cb);
  qmi_err = (smr_find_service(serviceObj, instanceID) >= 0) ? QMI_NO_ERR : QMI_SERVICE_ERR;
  ...
  {
    // TODO: MAKE SURE THAT IGNORING THE TIMEOUT DOES NOT HAVE ANY NEGATIVE CONSEQUENCES
    // If the service does not exist locally, then save the service object and
    // if the service registers, call the callback.
    ...
    sns_os_mutex_pend(smr_client_check_cb_data_q.mutex_ptr, 0, &os_err );
    /* Add the cb_data to the queue if its not already on it.*/
    if(!smr_cb_data->addedToQueue)
    {
      uint32_t svc_id = 0;

      // Set the flag so that the item does not get added to the queue again
      smr_cb_data->addedToQueue = true;

      qmi_idl_get_service_id(smr_cb_data->serviceObj, &svc_id);
      SNS_COMMON_MSG_1(MED, "client_check - Waiting for svc_id=%d", svc_id);

      sns_q_put(&smr_client_check_cb_data_q.protected_q, &smr_cb_data->q_link);
    }
    sns_os_mutex_post(smr_client_check_cb_data_q.mutex_ptr);

  }
#else /* defined(FAST_PLAYBACK) */
