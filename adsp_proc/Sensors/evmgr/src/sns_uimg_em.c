...
/* $Header: //components/rel/ssc.slpi/2.2/evmgr/src/sns_uimg_em.c#1 $  */
...
/*============================================================================
  ...
  2016-02-24  tc   Added functions to convert between DSPS and QTimer ticks
...
sns_err_code_e sns_em_cancel_utimer( sns_em_timer_obj_t timer_obj )
{
  ...
  utimer_clr_64(&temp->sTimer, UT_USEC );
  err = SNS_SUCCESS;
                                         ...
                                         uint64_t            delta_tick_time )
{
  ...
  {
    err = utimer_set_64(&temp->sTimer, (time_timetick_type)usec, 0, UT_USEC);
    if ( err != 0 )
    {
  ...
  {
    err = utimer_set_64(&temp->sTimer, (time_timetick_type)usec, (time_timetick_type)usec, UT_USEC);
    if ( err != 0 )
    {
...
#endif
  /* convert QTimer tick (19.2MHz) to DSPS tick (32768Hz) */
  return sns_em_convert_qtimertick_to_dspstick(raw_tick);
}
...
bool sns_em_is_utimer( const sns_em_timer_obj_t timer )
{
  ...
  return(FALSE);
#else
  return(SNS_OS_IS_UIMG_ADDR(timer));
#endif
}
...
=====================================================================================*/
inline uint64_t sns_em_convert_qtimertick_to_dspstick( uint64_t qtimer_tick )
{
  /* convert 19.2MHz tick to 32768Hz */
  return (((qtimer_tick << 4) / 9375) + sns_em_system_time_offset);
}
...
=====================================================================================*/
inline uint64_t sns_em_convert_dspstick_to_qtimertick( uint64_t dsps_tick )
{
  /* convert 32768Hz tick to 19.2MHz */
  return (((dsps_tick - sns_em_system_time_offset) * 9375) >> 4);
}
