...
/* $Header: //components/rel/ssc.slpi/2.2/evmgr/src/sns_em.c#1 $  */
...
/*============================================================================
  ...
  ----------  ---  -----------------------------------------------------------
  2017-03-02  bd   Free timer obj after mutex unlock
  2016-05-17  jtl  Added sensor event group to have non-deferrable timers
  2016-04-21  jtl  Cleanup unneeded fields from EM timer structure
  ...
  2015-08-24  tc   Allow non-one-shot timers to be cancelled as well
                                        ...
                                        sns_em_timer_obj_t* timer_obj_ptr )
{
  ...
  sns_em_tmr_s *new_tmr = SNS_OS_MALLOC(SNS_DBG_MOD_EM, sizeof(sns_em_tmr_s));
  ...
  *timer_obj_ptr = (sns_em_timer_obj_t)new_tmr;
  ...
  err = timer_def_osal(&(*timer_obj_ptr)->sTimer, &timer_non_defer_group, TIMER_FUNC1_CB_TYPE, (timer_t1_cb_type)timer_cb, (time_osal_notify_data)timer_cb_arg);
  if( err != TE_SUCCESS )
  {
                                         ...
                                         sns_em_timer_obj_t* timer_obj_ptr )
{
  ...
  *timer_obj_ptr = (sns_em_timer_obj_t)new_tmr;
  ...
  err = utimer_def_osal((utimer_ptr_type)(&(*timer_obj_ptr)->sTimer),
...
sns_err_code_e sns_em_cancel_timer( sns_em_timer_obj_t timer_obj )
{
  ...
  timer_clr_64(&temp->sTimer, T_USEC );
  err = SNS_SUCCESS;
                                        ...
                                        uint64_t            delta_tick_time )
{
  ...
  {
    err = timer_set_64(&temp->sTimer, (time_timetick_type)usec, 0, T_USEC);
    if ( err != 0 )
    {
  ...
  {
    err = timer_set_64(&temp->sTimer, (time_timetick_type)usec, (time_timetick_type)usec, T_USEC);
    if ( err != 0 )
    {
...
sns_err_code_e sns_em_delete_timer_obj( sns_em_timer_obj_t timer_obj )
{
  ...
  {
    temp = timer_obj;
    if(timer_undef(&temp->sTimer) != TE_SUCCESS)
    {
...
sns_err_code_e sns_em_delete_utimer_obj( sns_em_timer_obj_t timer_obj )
{
  ...
  {
    temp = timer_obj;
    if(utimer_undef(&temp->sTimer) != UTE_SUCCESS)
    {
      SNS_EM_PRINTF0(ERROR, "Timer_undef failed");
    }
    SNS_OS_FREE(temp);
   ...
   const sns_em_timer_obj_t timer, uint32_t *remaining_time)
{
   ...
   {
     temp = timer;
     *remaining_time = timer_get_64( &temp->sTimer, T_USEC );
   }
