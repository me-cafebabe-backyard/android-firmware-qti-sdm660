...
$Header: //components/rel/avs.adsp/2.8/api/audio/inc/adsp_asm_session_commands.h#6 $
  ...
  @detdesc
    ...
    purposes. It is supported only in the following modes:
     - On the loopback path, this command gives the delay experienced by the
       ...
       respect to video.
     - On the playback path (Pull mode, ULL, Gapless, compressed, and regular playback),
     ...
     - On the record path (Push mode record and compressed), this command gives
       the delay from the time the AFE gets the samples to the time the encoder
       writes to the shared client buffers.
    @par
