...
$Header: //components/rel/avs.adsp/2.8/api/avcs/inc/adsp_core_api.h#17 $
...
@{ */
/** ID of the audio DSP aDSP) processor. */
#define AUDIO_DSP_PROC_ID  0x0001

/** ID of the modem DSP (mDSP) processor. */
#define MODEM_DSP_PROC_ID  0x0002

/** ID of the sensor DSP (sDSP) processor. */
#define SENSOR_DSP_PROC_ID  0x0003

/** @} *//* end_addtogroup core_cmd_register_topos */


/** @ingroup avcs_offload_module
  ID of the Offload Module (OLM) module.

  The Multi-DSP Framework (MDF) allows the audio framework on the aDSP to
  offload some of its processing to other DSPs in the system.

  The OLM module abstracts the offloading-related framework on the aDSP. To
  communicate with remote services, this module publishes its requirements
  through CAPIv2 framework extensions for the dynamic service on the aDSP.
  
  For more information on CAPIv2, refer to
  @xrefcond{capiv2,80-N8098-1,80-N8098-2}.
*/
#define AVCS_ADSP_OLM_MODULE_ID 0x00012931
...
/** @ingroup core_cmd_register_topos
  ...
  @apr_msgpayload{avcs_cmd_register_topologies_t}
    @table{weak__avcs__cmd__register__topologies__t} @newpage
  ...
  @par Number of topologies (avcs_topology_buffer_t)
    ...
    @indent{12} (avcs_module_info_t (list of num_modules modules provided
    here)) -- for topology num_topologies @newpage
    @par
...
@endverbatim
  ...
  @par Module instance information (avcs_module_instance_info_t)
    @table{weak__avcs__module__instance__info__t}

  @par Module instance processing information (avcs_module_instance_proc_info_t)
    @table{weak__avcs__module__instance__proc__info__t}
  ...
  @detdesc
    ...
    bypassed. For more information, see amdb_cfg_flags_t::needed. @newpage
...
struct avcs_topology_definition_t
{
    ...
    /**< Versioning information.
         ...
         - 1 -- avcs_module_instance_info_t is used for module/instance
           ...
           topology.
         - 2 -- avcs_module_instance_proc_info_t is used for module/instance
           processing information. There should not be any repeating
           module/instances in the topology. @tablebulletend */
    ...
    /**< Bits 0 to 3 indicate the area to which this topology. Set bits 4 to
         ...
         @values{ for bits 0 to 3} @vertspace{-2}
...
typedef struct avcs_module_instance_proc_info_t avcs_module_instance_proc_info_t;

#include "adsp_begin_pack.h"

/** @weakgroup weak_avcs_module_instance_proc_info_t
@{ */
/* Defines a module-instance with the module ID-instance ID pair that are used in this topology.
 */
  struct avcs_module_instance_proc_info_t
  {
    uint32_t module_id;
    /**< ID of the module to be created.
    ...
    uint16_t client_proc_id;
    /**< ID of the processor.

        @values @vertspace{-2}
        - #AUDIO_DSP_PROC_ID
        - #MODEM_DSP_PROC_ID
        - #SENSOR_DSP_PROC_ID @tablebulletend */
  }
#include "adsp_end_pack.h"
;
/** @} *//* end_weakgroup weak_avcs_module_instance_proc_info_t */
...
/** Commands the AVCS to map multiple shared memory regions.
  ...
  @apr_msgpayload{avcs_cmd_shared_mem_map_regions_t}
    ...
    @par
    Immediately following this structure are num_regions instances of
    avs_shared_map_region_payload_t.
    @par
    For more information on these AVS structures, refer to
...
/** @ingroup core_cmd_map_mdf_sh_mem
    Commands the AVCS to map multiple shared memory regions to a remote
    processor ID. All mapped regions must be from the same memory pool.

  @apr_hdr_fields
    Opcode -- AVCS_CMD_MAP_MDF_SHARED_MEMORY
    @par
    Dst_port -- 0000

  @msgpayload{avcs_cmd_map_mdf_shared_memory_t}
    Immediately following this structure are num_regions instances of the map
    payload (avs_shared_map_region_payload_t).
    @tablens{weak__avcs__cmd__map__mdf__shared__memory__t}

  @detdesc
    The address fields in the array correspond to the remote processor
    indicated in proc_id.
    - For DSPs with SMMU enabled, the address is IOVA.
    - For DSPs without SMMU, the address is the physical address.
    @par
    For information on avs_shared_map_region_payload_t, refer to
  ...
  @dependencies
    The mem_map_handle must have been obtained using
    #AVCS_CMD_SHARED_MEM_MAP_REGIONS with the pool ID,
    ADSP_MEMORY_MAP_MDF_SHMEM_4K_POOL. (For information on the pool ID, refer
    to @xrefcond{Q3,80-NF774-1,80-NA610-1}.)
*/
#define AVCS_CMD_MAP_MDF_SHARED_MEMORY 0x00012930

typedef struct avcs_cmd_map_mdf_shared_memory_t avcs_cmd_map_mdf_shared_memory_t;

#include "adsp_begin_pack.h"

/** @weakgroup weak_avcs_cmd_map_mdf_shared_memory_t
@{ */
struct avcs_cmd_map_mdf_shared_memory_t
{
   uint32_t     mem_map_handle;
   ...
   /**< Number of regions to be mapped with the remote DSP processor specified
        in proc_id. */

}
#include "adsp_end_pack.h"
;
/** @} *//* end_weakgroup weak_avcs_cmd_map_mdf_shared_memory_t */
