...
typedef struct audproc_audiosphere_filt_coeffs_t
{
   uint32_t version;
   ...
   /**< @h2xmle_description  {Specifies the length of FFT.}
	...
	/**< @h2xmle_description  {...}
        ...
        @h2xmle_default      {0}
        @h2xmle_variableArraySize  {"4*(fft_size/2+1)"} */
}audproc_audiosphere_filt_coeffs_stereo_t
#include "adsp_end_pack.h"
;
...
typedef struct audproc_audiosphere_filter_design_t
{
   uint32_t version;
   /**< @h2xmle_description  {Specifies the version information of this parameter.(currently, only 0 is supported)}
...
}audproc_audiosphere_filter_design_stereo_t
#include "adsp_end_pack.h"
;
