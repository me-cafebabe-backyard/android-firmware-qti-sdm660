...
$Header: //components/rel/avs.adsp/2.8/api/afe/inc/adsp_afe_service_commands.h#36 $
...
/** Size of the range of port IDs for the PCIE interface. */
#define AFE_PORT_ID_PCIE_RANGE_SIZE                                 0x2
...
/** Start of the range of port IDs for PCIE ports. */
#define AFE_PORT_ID_PCIE_RANGE_START                               0xA000

/** End of the range of port IDs for USB audio ports. */
#define AFE_PORT_ID_PCIE_RANGE_END                                 (AFE_PORT_ID_PCIE_RANGE_START+AFE_PORT_ID_PCIE_RANGE_SIZE-1)
...
/** ID of PCIE Rx port 0. */
#define AFE_PORT_ID_PCIE_AUDIO_0_RX                                  0xA000

/** ID of PCIE Tx port 0. */
#define AFE_PORT_ID_PCIE_AUDIO_0_TX                                  0xA001
...
/** ID of the parameter used by #AFE_MODULE_AUDIO_DEV_INTERFACE to configure
    ...
    @apr_hdr_fields
    Opcode -- AFE_PARAM_ID_PCIE_AUDIO_CONFIG

    @apr_msgpayload{afe_param_id_pcie_audio_cfg_t}
    @table{weak__afe__param__id__pcie__audio__cfg__t} @newpage
 */
#define AFE_PARAM_ID_PCIE_AUDIO_CONFIG                                    0x000102b4

/** Version information used to handle future additions to
    AFE_PARAM_ID_PCIE_AUDIO_CONFIG processing (for backward compatibility).
 */
#define AFE_API_VERSION_PCIE_AUDIO_CONFIG                                 0x1

/** Enumeration for setting the data format to linear PCM when
    audio data is transmitted or received over a PCIE device. @newpage
 */
#define AFE_PCIE_AUDIO_DATA_FORMAT_LINEAR_PCM                             0x0

/** Enumeration for setting the data format to compressed format when
    audio data is transmitted or received over a PCIE device. 
    Here, UL schedule TS is inserted into DL packet sent to IPA layer.
    This data format is specific to Mav PL.@newpage
 */
#define AFE_PCIE_AUDIO_DATA_FORMAT_COMPRESSED_UL_SCHEDULE_IN_DL           0x1

/** Enumeration for setting the data format to compressed format when
    audio data is transmitted or received over a PCIE device. 
    Here, UL TS is inserted into UL packet sent by IPA layer.
    This TS value is supposed to be captured in logs 
    for debugging purpose incase of quality issues.
    This data format is specific to Mav PL.@newpage
 */
#define AFE_PCIE_AUDIO_DATA_FORMAT_COMPRESSED_TS_IN_UL           0x2

/**  Enumeration for setting the VFR timer source. This timer source is 
		 applicable for both SOFT and HARD VFR. */
#define AFE_PCIE_AUDIO_TIMER_SOURCE_VFR                                   0x0

typedef struct afe_param_id_pcie_audio_cfg_v1_t afe_param_id_pcie_audio_cfg_v1_t;

#include "adsp_begin_pack.h"

/** @weakgroup weak_afe_param_id_pcie_audio_cfg_t
@{ */
/* Payload of the AFE_PARAM_ID_PCIE_AUDIO_CONFIG parameter used by
    AFE_MODULE_AUDIO_DEV_INTERFACE.
 */
struct afe_param_id_pcie_audio_cfg_v1_t
{
   uint32_t                  minor_version;
   /**< Tracks the configuration of this parameter.

        @values #AFE_API_VERSION_PCIE_AUDIO_CONFIG */

    uint32_t                  sample_rate;
    /**< Sampling rate of the PCIE port.
    ...
    /**< Number of channels.
         ...
         @values 1 to #AFE_DRIVER_PCIE_MAX_CHANNEL_CNT. */		

    uint32_t                  data_format;
    /**< Data format supported by the PCIE device.

         @values #AFE_PCIE_AUDIO_DATA_FORMAT_LINEAR_PCM
                 #AFE_PCIE_AUDIO_DATA_FORMAT_COMPRESSED_UL_SCHEDULE_IN_DL */

   uint32_t                  frame_size;
   /**< This field must be set to 20000 us. */

   uint32_t                  timer_source;
   /**< This field must be set to one of following values
        @values #AFE_PCIE_AUDIO_TIMER_SOURCE_VFR. */
}
#include "adsp_end_pack.h"
;
/** @} */ /* end_weakgroup afe_param_id_pcie_audio_cfg_v1_t */
...
#define AFE_PORT_MAX_CHANNEL_CNT        (32) // For interfaces which support 32 channels
#define AFE_PORT_EIGHT_CHANNELS         (8)  // For interfaces which support only uptill 8 channels
...
#define AFE_DRIVER_HDMI_MAX_CHANNEL_CNT           (8)     // max number of channels for HDMI interface
#define AFE_DRIVER_I2S_MAX_CHANNEL_CNT            (8)     // max number of channels for I2S interface
#define AFE_DRIVER_PCM_MAX_CHANNEL_CNT            (8)     // max number of channels for PCM interface
#define AFE_DRIVER_PSEUDO_PORT_MAX_CHANNEL_CNT    (8)     // max number of channels for pseudo port interface
#define AFE_DRIVER_RIVA_MAX_CHANNEL_CNT           (8)     // max number of channels for RIVA interface
#define AFE_DRIVER_RT_PROXY_MAX_CHANNEL_CNT       (8)     // max number of channels for RT proxy interface
#define AFE_DRIVER_SLIMBUS_MAX_CHANNEL_CNT        (8)     // max number of channels for slimbus interface
#define AFE_DRIVER_TDM_MAX_CHANNEL_CNT            (32)    // max number of channels for slimbus interface
#define AFE_DRIVER_USBA_MAX_CHANNEL_CNT           (8)     // max number of channels for USB interface
#define AFE_DRIVER_PCIE_MAX_CHANNEL_CNT           (8)     // max number of channels for PCIE interface

#define AFE_DRIVER_TDM_MAX_SLOTS_PER_FRAME                  (32)  // max number of slots per frame supported by TDM interface
...
struct afe_param_id_slimbus_cfg_t
{
    ...
    /**< Number of channels.
         ...
         @values 1 to #AFE_DRIVER_SLIMBUS_MAX_CHANNEL_CNT */

    uint8_t                  shared_channel_mapping[AFE_DRIVER_SLIMBUS_MAX_CHANNEL_CNT];
...
struct afe_slimbus_slave_port_cfg_param_v1_t
{
   ...
   /**< Logical 8-bit address of the slave interface for an interface device.
    ...
    uint16_t slave_port_mapping[AFE_DRIVER_SLIMBUS_MAX_CHANNEL_CNT];
...
/** Version information used to handle future additions to
    ...
    compatibility).
*/
#define AFE_API_VERSION_SLOT_MAPPING_CONFIG                           0x2
...
struct afe_param_id_slot_mapping_cfg_t
{
    ...
    uint16_t                   offset[AFE_PORT_EIGHT_CHANNELS];
...
#include "adsp_end_pack.h"
;
/** @} */ /* end_weakgroup weak_afe_param_id_slot_mapping_cfg_v2_t */

/** @weakgroup weak_afe_param_id_slot_mapping_cfg_v2_t
@{ */
/* Payload of the AFE_PARAM_ID_PORT_SLOT_MAPPING_CONFIG command's TDM configuration
    parameter.
 */
struct afe_param_id_slot_mapping_cfg_v2_t
{
    uint32_t                  minor_version;
    /**< Tracks the configuration of this parameter.
...
/** Used by a client to request LPASS resources. The remote client will
  ...
  @note1hang Currently, this command supports the LPAIF DMA resources and Low Power Memory (LPM) resources.
  ...
  @par DMA resources payload (afe_cmd_request_lpass_dma_resources)
    @tablens{weak__afe__cmd__request__lpass__dma__resources}

  @par LPM resources payload (afe_cmd_request_lpm_resources)
    @tablens{weak__afe__cmd__request__lpm__resources}
...
@endcode
    ...
    When the request is for LPM resources:
@code
< afe_cmd_request_lpass_resources >
     resource_id = AFE_LPM_RESOURCE_ID
< afe_cmd_request_lpm_resources >
     size_in_bytes
@endcode
...
/** Macro for requesting LPM resources. */
#define AFE_LPM_RESOURCE_ID    0x00000002
...
#define AFE_LPM_INVALID_REQUEST    0xFFFFFFFF

typedef struct afe_cmd_request_lpm_resources afe_cmd_request_lpm_resources;

#include "adsp_begin_pack.h"

/** @weakgroup weak_afe_cmd_request_lpm_resources
@{ */
/** When the request is for LPM resources, this structure immediately
    follows %afe_cmd_request_lpass_resources.
*/
struct afe_cmd_request_lpm_resources
{
	uint32_t size_in_bytes;
	/**< Size of LPM memory requested. 

       @values @ge 0 */
}
#include "adsp_end_pack.h"
;
/** @} *//* end_weakgroup weak_afe_cmd_request_lpm_resources */
...
/** 
  ...
  @note1hang Currently, this command supports the LPAIF DMA resources and LPM resources.
  ...
  @par LPM resources payload (afe_cmdrsp_request_lpm_resources)
    @tablens{weak__afe__cmdrsp__request__lpm__resources}
    @newpage
...
< message payload >
  afe_cmdrsp_request_lpass_resources
     status
     resource_id = AFE_LPM_RESOURCE_ID
< LPM resources payload >
  afe_cmdrsp_request_lpm_resources
     size_in_bytes
     phy_addr_lsw
     phy_addr_msw
@endcode
...
typedef struct afe_cmdrsp_request_lpm_resources afe_cmdrsp_request_lpm_resources;

#include "adsp_begin_pack.h"

/** @weakgroup weak_afe_cmdrsp_request_lpm_resources
@{ */
/* This payload is for AFE_CMDRSP_REQUEST_LPASS_RESOURCES,
  when the LPM resources are requested.
*/
/** When the request is for LPM resources, this structure immediately
    follows %afe_cmdrsp_request_lpass_resources.
*/
struct afe_cmdrsp_request_lpm_resources
{
   uint32_t size_in_bytes;
   /**< Size of LPM memory allocated. 

        @values @ge 0 */

   uint32_t phy_addr_lsw;
   /**< Lower 32 bits of allocated LPM physical address. */

   uint32_t phy_addr_msw;
   /**< Higher 32 bits of allocated LPM physical address. */
}
#include "adsp_end_pack.h"
;
/** @} *//* end_weakgroup weak_afe_cmdrsp_request_lpm_resources */
...
/** Releases resources that were allocated through
  ...
  @par LPM resources payload (afe_cmd_release_lpm_resources)
    @tablens{weak__afe__cmd__release__lpm__resources}
...
@endcode
    ...
    When allocated LPM resources are to be released, the message payload
    contains the LPM resources payload.
    @par
    The payload contains the valid LPM size and physical address that was
    allocated as part of #AFE_CMD_REQUEST_LPASS_RESOURCES.
    @newpage
@code
< message payload >
...
@endcode
    @par
    The client must fill the same LPM physical address that was returned as
    part of #AFE_CMD_REQUEST_LPASS_RESOURCES. Otherwise, the aDSP returns an
    error.
    @par
    If the HLOS tries to release resources that are not allocated, the aDSP
...
typedef struct afe_cmd_release_lpm_resources afe_cmd_release_lpm_resources;

#include "adsp_begin_pack.h"
/** @weakgroup weak_afe_cmd_release_lpm_resources
@{ */
/** When the request is for LPM resources, this structure immediately
    follows %afe_cmd_release_lpass_resources.
*/
struct afe_cmd_release_lpm_resources
{
   uint32_t size_in_bytes;
   /**< Size of LPM memory to be released. 
...
#include "adsp_end_pack.h"
;
/** @} *//* end_weakgroup weak_afe_cmd_release_lpm_resources */
