...
$Header: //components/rel/avs.adsp/2.8/aud/services/dynamic_svcs/audio_dec_svc/src/AudioDecSvc.cpp#9 $
...
ADSPResult AudioDecSvc_Create (AudioDecSvcInitParams_t *pInitParams, void **handle)
{
   ...
   *handle = NULL;
   ...
   AudioDecSvc_t *pMe = (AudioDecSvc_t*) qurt_elite_memory_malloc( sizeof(AudioDecSvc_t), QURT_ELITE_HEAP_DEFAULT);
   if (!pMe)
   {
   ...
   if (ADSP_FAILED(result = qurt_elite_thread_launch(&(pMe->thread_id), aThreadname, NULL,
         threadStackSize, nPriority, AudioDecSvc_WorkLoop, (void*)pMe, QURT_ELITE_HEAP_DEFAULT)))
   {
