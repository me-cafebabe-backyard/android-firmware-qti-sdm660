...
$Header: //components/rel/avs.adsp/2.8/aud/services/dynamic_svcs/audio_dec_svc/src/AudioDecSvc_ChanMap.cpp#5 $
                                     ...
                                     dec_out_chan_map_t *out_chan_map)
{
   ...
   QURT_ELITE_ASSERT(out_chan_map->num_out_chan <= PCM_FORMAT_MAX_NUM_CHANNEL_V2);
                                 ...
                                 uint32_t *out_buf_size)
{
   *out_buf_size = in_chan_map->nChannels * samples_per_ch * bytes_per_samp;
   ...
   {
      chan_params_t in_chan_params[PCM_FORMAT_MAX_NUM_CHANNEL_V2];
...
/*
 *Function Name:AudioDecSvc_ProcessOutChanMapV2Cmd
 *
 *Parameters:  me: ptr to decoder service instance
 *             cmd: set param command pointer
 *
...
ADSPResult AudioDecSvc_ProcessOutChanMapV2Cmd(AudioDecSvc_t *me, AudioDecSvc_OutStream_t *pOutStrm, asm_dec_out_chan_map_param_v2_t *cmd)
{
   //check for valid parameters
   DecChannelMap_t chan_map; // TODO : check if this can be removed
   asm_dec_out_chan_map_t *ch_map = (asm_dec_out_chan_map_t *)(cmd+1);

   memscpy(chan_map.nChannelMap, sizeof(chan_map.nChannelMap),
       ch_map, cmd->num_channels * sizeof(asm_dec_out_chan_map_t) );
   chan_map.nChannels = cmd->num_channels;

   if(chan_map.nChannels > MAX_CHAN_MAP_CHANNELS_V2)
   {
      MSG_1(MSG_SSID_QDSP6, DBG_ERROR_PRIO,
         "Invalid # of channels (%d) in dec output chan map cmd", chan_map.nChannels);
   ...
   {
      if( (chan_map.nChannelMap[i] < PCM_CHANNEL_L)
          || (chan_map.nChannelMap[i] > PCM_MAX_CHANNEL_MAP) )
      {
         MSG_1(MSG_SSID_QDSP6, DBG_ERROR_PRIO,
         "Invalid channel position (%d) in dec output chan map cmd", chan_map.nChannelMap[i]);
