...
$Header: //components/rel/avs.adsp/2.8/aud/services/dynamic_svcs/audio_dec_svc/src/AudioDecSvc_Util.cpp#27 $
...
static ADSPResult AudioDecSvc_CopyPcmV5MediaFmt(AudioDecSvc_t* pMe, AudioDecSvc_InpStream_t *pInpStrm, void *pFmtBlk );
static void AudioDecSvc_PrintChannelType(uint8_t *ch_map, int32_t num_channels);
...
void AudioDecSvc_GetInitPCMDecBufSize(AudioDecSvc_t* pMe, AudioDecSvcIOStreamInitParams_t *pInitIOParams, uint32_t *outBufSize)
{
   ...
   *outBufSize = 0;
   ...
   {
     asm_multi_channel_pcm_fmt_blk_v5_dec_t *pcmFmtBlk = &(pInitIOParams->in.pull_mode.MediaFmt.pcm);
	   bytesPerSample = (pInitIOParams->bits_per_sample == 16) ? 2 : 4;
     sampleRate = pcmFmtBlk->fmt.sample_rate;
     numChannel = pcmFmtBlk->fmt.num_channels;
	   calBufSizeFromMediaFmt = true;
   }
...
static void AudioDecSvc_PrintChannelType(uint8_t *ch_map, int32_t num_channels)
{
  ...
  MSG_8(MSG_SSID_QDSP6, DBG_HIGH_PRIO,
        "PCM Dec channel mapping (4 channel maps per value) 0x%lx,0x%lx,0x%lx,0x%lx,0x%lx,0x%lx,0x%lx,0x%lx",ch_map_word[0],
...
static ADSPResult AudioDecSvc_CopyPcmV5MediaFmt(AudioDecSvc_t* pMe, AudioDecSvc_InpStream_t *pInpStrm, void *pFmtBlk )
{
   asm_multi_channel_pcm_fmt_blk_v5_t* pPcmFmtBlk_v5 = (asm_multi_channel_pcm_fmt_blk_v5_t*) ( pFmtBlk );
   asm_multi_channel_pcm_fmt_ch_map_t *ch_map;
   int32_t num_channels = (int32_t)pPcmFmtBlk_v5->num_channels;
   ch_map = (asm_multi_channel_pcm_fmt_ch_map_t *)(pPcmFmtBlk_v5 + 1);

   MSG_4(MSG_SSID_QDSP6, DBG_HIGH_PRIO,
         "PCM V5 Media Format 0x%lx. ch=%u,SR=%lu,bps=%u",pMe->capiContainer[0]->dec_fmt_id,
         pPcmFmtBlk_v5->num_channels,
         pPcmFmtBlk_v5->sample_rate,
         pPcmFmtBlk_v5->bits_per_sample);

   if(((!pPcmFmtBlk_v5->is_signed) || (pPcmFmtBlk_v5->num_channels == 0) || (pPcmFmtBlk_v5->num_channels > PCM_FORMAT_MAX_NUM_CHANNEL_V2)))
   {
   ...
   if ((0 == pPcmFmtBlk_v5->sample_rate) || (PCM_FORMAT_MAX_SAMPLING_RATE_HZ < pPcmFmtBlk_v5->sample_rate))
   {
   ...
   AudioDecSvc_PrintChannelType((uint8_t *)ch_map, num_channels);

   DecPcmFmt_t prev_fmt = pInpStrm->pcmFmt;

   pInpStrm->pcmFmt.endianness = DEC_LITTLE_ENDIAN;

   if(((16 == pPcmFmtBlk_v5->bits_per_sample) || (24 == pPcmFmtBlk_v5->bits_per_sample) || (32 == pPcmFmtBlk_v5->bits_per_sample)) &&
         (pPcmFmtBlk_v5->sample_word_size >= pPcmFmtBlk_v5->bits_per_sample))
   {
      uint32_t bytesPerSample = pPcmFmtBlk_v5->sample_word_size/8;
      if ( (bytesPerSample*8 != pPcmFmtBlk_v5->sample_word_size) || (bytesPerSample > 4))
      {
         MSG_1(MSG_SSID_QDSP6, DBG_ERROR_PRIO, "sample word size %u not byte aligned or is > 32.", pPcmFmtBlk_v5->sample_word_size);
                     return ADSP_EBADPARAM;
      }
      pInpStrm->pcmFmt.usBytesPerSample = bytesPerSample;
      pInpStrm->pcmFmt.usBitsPerSample = pPcmFmtBlk_v5->bits_per_sample;

      if((pPcmFmtBlk_v5->endianness != 0) && (pPcmFmtBlk_v5->endianness != 1))
      {
         MSG_1(MSG_SSID_QDSP6, DBG_ERROR_PRIO, "endianness type %u not supported.", pPcmFmtBlk_v5->endianness);
         return ADSP_EBADPARAM;
      }

      pInpStrm->pcmFmt.endianness = (pPcmFmtBlk_v5->endianness == 0) ? DEC_LITTLE_ENDIAN : DEC_BIG_ENDIAN;
      MSG_2(MSG_SSID_QDSP6, DBG_HIGH_PRIO, "Sample word size is %u, Endianness type is %u (0-little, 1-big)",
          pPcmFmtBlk_v5->sample_word_size, pPcmFmtBlk_v5->endianness);
   }
   else
   {
      MSG_2(MSG_SSID_QDSP6, DBG_ERROR_PRIO, "Error: Aud dec rcvd invalid bits per sample_[%lu], or sample word size [%lu].",
          pPcmFmtBlk_v5->bits_per_sample, pPcmFmtBlk_v5->sample_word_size);
      return ADSP_EBADPARAM;
   }

   //store PCM parameters
   pInpStrm->pcmFmt.chan_map.nChannels = pPcmFmtBlk_v5->num_channels;
   pInpStrm->pcmFmt.ulPCMSampleRate = pPcmFmtBlk_v5->sample_rate;
   pInpStrm->pcmFmt.usIsInterleaved = TRUE;

   memset(pInpStrm->pcmFmt.chan_map.nChannelMap, 0, sizeof(pInpStrm->pcmFmt.chan_map.nChannelMap));

   memscpy(pInpStrm->pcmFmt.chan_map.nChannelMap, sizeof(pInpStrm->pcmFmt.chan_map.nChannelMap),
       ch_map, pPcmFmtBlk_v5->num_channels * sizeof(asm_multi_channel_pcm_fmt_ch_map_t));
...
ADSPResult AudioDecSvc_CopyPcmOr61937MediaFmt(AudioDecSvc_t* pMe, AudioDecSvc_InpStream_t *pInpStrm, void *pFmtBlk )
{
   ...
   if(pMe->capiContainer[0]->dec_fmt_id == ASM_MEDIA_FMT_MULTI_CHANNEL_PCM_V5)
   {
       ADSPResult result = ADSP_EOK;
       result = AudioDecSvc_CopyPcmV5MediaFmt(pMe,pInpStrm,pFmtBlk );
       return result;
   }
   ...
   AudioDecSvc_PrintChannelType(&pPcmFmtBlk_v2->channel_mapping[0], pPcmFmtBlk_v2->num_channels);
...
ADSPResult AudioDecSvc_UpdateMediaFmt_PeerService(AudioDecSvc_t* pMe, AudioDecSvc_InpStream_t *pInpStrm)
{
   ...
   {
     void *pcmBlk;
     asm_multi_channel_pcm_fmt_blk_v5_dec_t pcmBlkV5;
     asm_multi_channel_pcm_fmt_blk_v3_t *pcmBlkV3;
     ...
     if(pMe->capiContainer[0]->dec_fmt_id == ASM_MEDIA_FMT_MULTI_CHANNEL_PCM_V5)
     {
         pcmBlkV5.fmt.bits_per_sample = pInPcmFmtBlk->bits_per_sample;
         pcmBlkV5.fmt.is_signed = pInPcmFmtBlk->is_signed;
         pcmBlkV5.fmt.num_channels = pInPcmFmtBlk->num_channels;
         pcmBlkV5.fmt.sample_rate = pInPcmFmtBlk->sample_rate;
         pcmBlkV5.fmt.sample_word_size = (pcmBlkV5.fmt.bits_per_sample == 16) ? 16 : 32; //internally it's always 16 or 32.

         memscpy(pcmBlkV5.channel_map, sizeof(pcmBlkV5.channel_map), pInPcmFmtBlk->channel_mapping,
             pInPcmFmtBlk->num_channels * sizeof(pInPcmFmtBlk->channel_mapping[0]));

         pcmBlk = (void *)(&pcmBlkV5);
     }
     else
     {
         // v3 payload requires lesser size than v5
         pcmBlkV3 = (asm_multi_channel_pcm_fmt_blk_v3_t *)(&pcmBlkV5);
         pcmBlkV3->bits_per_sample = pInPcmFmtBlk->bits_per_sample;
         pcmBlkV3->is_signed = pInPcmFmtBlk->is_signed;
         pcmBlkV3->num_channels = pInPcmFmtBlk->num_channels;
         pcmBlkV3->sample_rate = pInPcmFmtBlk->sample_rate;
         pcmBlkV3->sample_word_size = (pcmBlkV3->bits_per_sample == 16) ? 16 : 32; //internally it's always 16 or 32.

         memscpy(pcmBlkV3->channel_mapping, sizeof(pcmBlkV3->channel_mapping), pInPcmFmtBlk->channel_mapping,
             pInPcmFmtBlk->num_channels * sizeof(pInPcmFmtBlk->channel_mapping[0]));

         pcmBlk = (void *)pcmBlkV3;
     }

      result = AudioDecSvc_CopyPcmOr61937MediaFmt(pMe, pInpStrm, pcmBlk);
      ...
      DecChannelMap_t *chan_map)
{
   ...
   asm_data_event_sr_cm_change_notify_v2_t SrCmEvent;
   uint32_t eventopcode = ASM_DATA_EVENT_SR_CM_CHANGE_NOTIFY, payload_size = sizeof(asm_data_event_sr_cm_change_notify_t);
   ...
   if(pMe->capiContainer[0]->dec_fmt_id == ASM_MEDIA_FMT_MULTI_CHANNEL_PCM_V5)
   {
       eventopcode = ASM_DATA_EVENT_SR_CM_CHANGE_NOTIFY_V2;
