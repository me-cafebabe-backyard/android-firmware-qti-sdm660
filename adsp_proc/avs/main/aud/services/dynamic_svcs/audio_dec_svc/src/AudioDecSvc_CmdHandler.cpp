...
$Header: //components/rel/avs.adsp/2.8/aud/services/dynamic_svcs/audio_dec_svc/src/AudioDecSvc_CmdHandler.cpp#13 $
...
ADSPResult AudioDecSvc_SetParam(AudioDecSvc_t *pMe)
{
   ...
   case ELITEMSG_PARAM_ID_CAL:
   {
      ...
      case ASM_PARAM_ID_DEC_OUTPUT_CHAN_MAP_V2:
      {
         asm_dec_out_chan_map_param_v2_t *pDecOutChanMapMsg = (asm_dec_out_chan_map_param_v2_t *)pSetParamPayload;
         res = AudioDecSvc_ProcessOutChanMapV2Cmd(pMe, pOutStrm, pDecOutChanMapMsg);
         break;
      }
   ...
   // Pass on to let decoder CAPI decide if this is supported.
   default:
   {
      elite_msg_param_cal_t *pParamMsg = (elite_msg_param_cal_t *) pMe->cmdMsg.pPayload;
