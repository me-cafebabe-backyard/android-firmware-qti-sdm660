...
$Header: //components/rel/avs.adsp/2.8/aud/services/dynamic_svcs/audio_dec_svc/src/AudioDecSvc_DataHandler.cpp#20 $
      ...
      uint32_t *numBytesProcessed, bool_t isPackedChSpacing, uint32_t maxUsableOutBufSize, bool_t isInvalidateCache)
{
   ...
   int8_t *out_buf;
   int8_t *out_ptr[PCM_FORMAT_MAX_NUM_CHANNEL_V2];
   ...
   if(2 == bytes_per_sample_out)
   {
      ...
      if((num_in_channel > PCM_FORMAT_MAX_NUM_CHANNEL_V2) ||
          ((pMe->capiContainer[0]->dec_fmt_id != ASM_MEDIA_FMT_MULTI_CHANNEL_PCM_V5) && (num_in_channel > PCM_FORMAT_MAX_NUM_CHANNEL)))
      {
