...
$Header: //components/rel/avs.adsp/2.8/aud/services/dynamic_svcs/audio_dec_svc/src/AudioDecSvc_BufferUtil.cpp#12 $
...
ADSPResult AudioDecSvc_RecreatePcmBufsBasedOnMediaFmt(AudioDecSvc_t* pMe, AudioDecSvc_InpStream_t *pInpStrm)
{
   ...
   uint32_t pcm_fmt_max_num_channel;
   if (last_capi != NULL)
   {
   ...
   for (uint8_t capi_index = 0; capi_index < DEC_SVC_MAX_CAPI; capi_index++)
   {
      ...
      for(uint32_t out_capi_index = 0; out_capi_index < DEC_SVC_MAX_OUTPUT_STREAMS; out_capi_index++)
      {
         ...
         pcm_fmt_max_num_channel = (pMe->capiContainer[capi_index]->dec_fmt_id == ASM_MEDIA_FMT_MULTI_CHANNEL_PCM_V5)?PCM_FORMAT_MAX_NUM_CHANNEL_V2: PCM_FORMAT_MAX_NUM_CHANNEL;
         ...
         uint32_t num_ch = pMe->out_streams_ptr[out_capi_index]->out_chan_map.is_out_chan_map_received ? pcm_fmt_max_num_channel: pInpStrm->pcmFmt.chan_map.nChannels;
      ...
      uint32_t inBufSize, dec_capi_port_index_t in_capi_port_index)
{
   if (inBufSize && in_capi_port_index.valid)
   {
      ...
      uint8_t *int_buf = (uint8_t *) qurt_elite_memory_malloc(inBufSize, QURT_ELITE_HEAP_DEFAULT);
      if ( NULL==int_buf)
      {
