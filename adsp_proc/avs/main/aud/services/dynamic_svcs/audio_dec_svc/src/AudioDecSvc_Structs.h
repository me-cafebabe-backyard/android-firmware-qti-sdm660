...
$Header: //components/rel/avs.adsp/2.8/aud/services/dynamic_svcs/audio_dec_svc/src/AudioDecSvc_Structs.h#15 $
...
#define  PCM_FORMAT_MAX_NUM_CHANNEL_V2  (ELITE_PCM_MAX_NUM_CH)
...
#define MAX_SPKR_POS             (ELITE_PCM_MAX_NUM_CH)
...
#define STACK_SIZE_DEC_PROCESS (768)   //decoder stack size excluding core process function stack
...
struct DecChannelMap_t
{
   ...
   uint8_t   nChannelMap[CAPI_V2_MAX_CHANNELS_V2];          /** channel positions mapped */
};
...
struct dec_in_chan_map_t
{
   ...
   uint8_t                              in_chan[PCM_FORMAT_MAX_NUM_CHANNEL_V2];
} ;
...
struct dec_out_chan_map_t
{
   ...
   uint8_t                              out_chan[PCM_FORMAT_MAX_NUM_CHANNEL_V2];
   //details of input channels routed to each of the output channels above
   dec_in_chan_map_t                    in_chan_map[PCM_FORMAT_MAX_NUM_CHANNEL_V2];
...
static inline bool_t AudioDecSvc_IsPcmFmt(uint32_t fmt_id)
{
   return ( (ASM_MEDIA_FMT_MULTI_CHANNEL_PCM_V2 == fmt_id) ||
        ...
        (ASM_MEDIA_FMT_MULTI_CHANNEL_PCM_V5 == fmt_id));
}
