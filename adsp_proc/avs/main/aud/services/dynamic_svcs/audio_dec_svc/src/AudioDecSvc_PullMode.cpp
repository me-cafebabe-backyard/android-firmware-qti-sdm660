...
static ADSPResult AudioDecSvc_RegisterAvTimer(AudioDecSvc_t *pMe, AudioDecSvc_InpStream_t *pInpStrm)
{
   ...
   ADSPResult result;
   if (ADSP_EOK != (result = qurt_elite_timer_open(&(pInpStrm->pull_mode_ptr->avt_drv_handle), (char*)"ADec")))
   {
      MSG_1(MSG_SSID_QDSP6, DBG_ERROR_PRIO, "Dec Svc: Failed to Open timer Driver for input port with result: %d",result);
      return result;
   }
   return result;
}
...
static ADSPResult AudioDecSvc_DeregisterAvTimer(AudioDecSvc_t *pMe, AudioDecSvc_InpStream_t *pInpStrm)
{
   ...
   if (NULL != pInpStrm->pull_mode_ptr->avt_drv_handle)
   {
      if (ADSP_EOK != (result = qurt_elite_timer_close(pInpStrm->pull_mode_ptr->avt_drv_handle)))
      {
         MSG_1(MSG_SSID_QDSP6, DBG_ERROR_PRIO, "Dec Svc: Failed to Close timer Driver for input port with result: %d",result);
      }
...
ADSPResult AudioDecSvc_InitPullMode(AudioDecSvc_t *pMe, AudioDecSvc_InpStream_t *pInpStrm, AudioDecSvcIOStreamInitParams_t *pIOInitParams)
{
   ...
   asm_multi_channel_pcm_fmt_blk_v5_dec_t *pPcm = &pIOInitParams->in.pull_mode.MediaFmt.pcm;
   ...
   result = AudioDecSvc_CopyPcmOr61937MediaFmt(pMe, pInpStrm, pPcm_media_format);
   if(ADSP_FAILED(result))
   {
	   return result;
   }
   uint32_t unit_size = pPcm->fmt.sample_word_size/8 * pPcm->fmt.num_channels;
   ...
   if (pInpStrm->pull_mode_ptr->shared_circ_buf_size % unit_size)
   {
      MSG_3(MSG_SSID_QDSP6, DBG_HIGH_PRIO, "Circular buffer size %lu is not a multiple of (sample word size %u * channels %u) ",
            pInpStrm->pull_mode_ptr->shared_circ_buf_size, pPcm->fmt.sample_word_size, pPcm->fmt.num_channels);
      return ADSP_EFAILED;
   }
...
ADSPResult AudioDecSvc_SuspendPullMode(AudioDecSvc_t *pMe, AudioDecSvc_InpStream_t *pInpStrm)
{
   ...
   MSG(MSG_SSID_QDSP6, DBG_HIGH_PRIO,"AudioDecSvc :[SUSPEND] timer handle Close ");
...
ADSPResult AudioDecSvc_ResumePullMode(AudioDecSvc_t *pMe, AudioDecSvc_InpStream_t *pInpStrm)
{
   ...
   MSG(MSG_SSID_QDSP6, DBG_HIGH_PRIO,"AudioDecSvc :[RUN] timer handle resume ");
   return result;
}
...
ADSPResult AudioDecSvc_ProcessOutputDataQ_PCMPullMode(AudioDecSvc_t *pMe, AudioDecSvc_OutStream_t *pOutStrm)
{
   ...
   uint64_t time = qurt_elite_fwk_get_time(pInpStrm->pull_mode_ptr->avt_drv_handle);
