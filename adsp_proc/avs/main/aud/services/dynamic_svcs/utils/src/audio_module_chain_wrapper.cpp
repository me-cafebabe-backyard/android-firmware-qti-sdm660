...
$Header: //components/rel/avs.adsp/2.8/aud/services/dynamic_svcs/utils/src/audio_module_chain_wrapper.cpp#2 $
 ...
 ** ----------------------------------------------------------------------- */
static const aud_module_media_fmt_v1_t invalid_media_format_v1 = {
    {
        CAPI_V2_FIXED_POINT
    },
    {
...
static const aud_module_media_fmt_v2_t invalid_media_format_v2 = {
    {
        CAPI_V2_FIXED_POINT
    },
    {
...
static void audio_module_convert_v1_media_format_to_v2(aud_module_media_fmt_v1_t *format_ptr_v1, aud_module_media_fmt_v2_t *format_ptr_v2);
static void audio_module_convert_v2_media_format_to_v1(aud_module_media_fmt_v2_t *format_ptr_v2, aud_module_media_fmt_v1_t *format_ptr_v1);
...
static capi_v2_err_t audio_module_chain_wrapper_event_callback(void *context_ptr, capi_v2_event_id_t id, capi_v2_event_info_t *event_info_ptr)
{
   ...
   {
      if (payload->actual_data_len < sizeof(aud_module_media_fmt_v1_t))
      {
         MSG_3(MSG_SSID_QDSP6, DBG_ERROR_PRIO, " Error in audio_module_chain_wrapper_event_callback function. The actual size %lu is less than the required size %zu for id %lu.",
               payload->actual_data_len, sizeof(aud_module_media_fmt_v1_t), static_cast<uint32_t>(id));
         return CAPI_V2_ENEEDMORE;
      }

      aud_module_media_fmt_v1_t *pData = reinterpret_cast<aud_module_media_fmt_v1_t*>(payload->data_ptr);

      aud_module_media_fmt_v2_t format_v2 = invalid_media_format_v2;

      audio_module_convert_v1_media_format_to_v2(pData, &format_v2);
   ...
   {
      uint32_t req_size = sizeof(capi_v2_data_format_header_t) + sizeof(capi_v2_standard_data_format_v2_t);
      if (payload->actual_data_len < req_size)
      {
         MSG_3(MSG_SSID_QDSP6, DBG_ERROR_PRIO, " Error in audio_module_chain_wrapper_event_callback function. The actual size %lu is less than the required size %zu for id %lu.",
               payload->actual_data_len, req_size, static_cast<uint32_t>(id));
         return CAPI_V2_ENEEDMORE;
      }

      aud_module_media_fmt_v2_t *pData = reinterpret_cast<aud_module_media_fmt_v2_t*>(payload->data_ptr);

      req_size += (pData->f.num_channels * sizeof(pData->channel_type[0]));
      if (payload->actual_data_len < req_size)
      {
         MSG_3(MSG_SSID_QDSP6, DBG_ERROR_PRIO, " Error in audio_module_chain_wrapper_event_callback function. The actual size %lu is less than the required size %zu for id %lu.",
               payload->actual_data_len, req_size, static_cast<uint32_t>(id));
...
static void audio_module_chain_wrapper_port_initialize(module_port_t *port_info)
{
  port_info->media_event.event = invalid_media_format_v2;
  return;
}
    ...
    for(uint32_t i = 0; i < modules->num_modules; i++)
    {
	   ...
	   continue;
        ...
        for(uint32_t port_id = 0; port_id < pModule_info->port_info.num_input_ports; port_id++)
        {
            ...
            {
                input_ports_ptr->media_event.event.channel_type[ch] = (uint16_t)pPcmFormatBlock->channel_mapping[ch];
            }
            ...
            aud_module_media_fmt_v2_t *new_format_ptr = &input_ports_ptr->media_event.event;

            prop.id = CAPI_V2_INPUT_MEDIA_FORMAT_V2;
            prop.port_info.is_input_port = TRUE;
            prop.port_info.is_valid = TRUE;
            prop.port_info.port_index = port_id;
            prop.payload.actual_data_len = sizeof(new_format_ptr->h) + sizeof(new_format_ptr->f) + new_format_ptr->f.num_channels * sizeof(new_format_ptr->channel_type[0]);
            ...
            err_code1 = pModule_info->module_ptr->vtbl_ptr->set_properties(pModule_info->module_ptr, &proplist);
            if (CAPI_V2_FAILED(err_code1))
            {
               if(new_format_ptr->f.num_channels <= CAPI_V2_MAX_CHANNELS)
               {
                   // form V1 payload and send
                    err_code1 = CAPI_V2_EOK;
                    aud_module_media_fmt_v1_t new_format_ptr_v1 = invalid_media_format_v1;
                    audio_module_convert_v2_media_format_to_v1(new_format_ptr, &new_format_ptr_v1);
                    prop.id = CAPI_V2_INPUT_MEDIA_FORMAT;
                    prop.port_info.is_input_port = TRUE;
                    prop.port_info.is_valid = TRUE;
                    prop.port_info.port_index = i;
                    prop.payload.actual_data_len = sizeof(new_format_ptr_v1);
                    prop.payload.max_data_len = sizeof(new_format_ptr_v1);
                    prop.payload.data_ptr = reinterpret_cast<int8_t*>(&new_format_ptr_v1);
