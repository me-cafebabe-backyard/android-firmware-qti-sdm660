...
static void AudPP_topo_update_run_state(topo_event_client_t *obj_ptr, uint32_t run_command_response);
static void AudPP_topo_custom_event(topo_event_client_t *obj_ptr, void *payload, uint32_t payloadsize);
static ADSPResult AudPP_topo_process_respq(topo_event_client_t *obj_ptr);
static ADSPResult AudPP_populateCb(audproc_event_query_payload *clientStruct, audproc_event_node_t **event_root_node);
static void AudPP_PrintChannelType(const uint8_t *channel_type, int32_t num_channels, uint16_t obj_id );
/*---------------------------------------------------------------------------
 * Globals
 * -------------------------------------------------------------------------*/
const topo_event_vtable_t topo_event_vtable = {
	  ...
	  AudPP_topo_update_run_state,
	  AudPP_topo_custom_event,
	  AudPP_topo_process_respq
};
...
ADSPResult AudPP_setInputMediaInfo(ThisAudDynaPPSvc_t* me, const AdspAudioMediaFormatInfo_t *pInputMediaFormat,uint32_t port_id)
{
   ...
   if(topo_isBgThreadReq(me->pTopologyObj))
   {
     ...
     {
        result = topo_bg_thread_create(me->pTopologyObj, (void **)&me->bg_th_handle, QURT_ELITE_HEAP_DEFAULT,
...
static void AudPP_PrintChannelType(const uint8_t *ch_map, int32_t num_channels, uint16_t obj_id )
{
...
static void AudPP_PrintMediaFormat(const ThisAudDynaPPSvc_t* me, const AdspAudioMediaFormatInfo_t *pMediaFormat, bool_t is_input,uint32_t port_id)
{
   if(is_input)
   {
      ...
      MSG_3(MSG_SSID_QDSP6, DBG_HIGH_PRIO, "P%hX: Input MediaFmt in_port %x: Channel mapping for %lu channels is as follows", me->objId, port_id, pMediaFormat->channels);
      AudPP_PrintChannelType(&pMediaFormat->channelMapping[0], (int32_t)pMediaFormat->channels, me->objId);
   }
   else
   {
      ...
      MSG_2(MSG_SSID_QDSP6, DBG_HIGH_PRIO, "P%hX: Output MediaFmt Channel mapping for %lu channels is as follows", me->objId, pMediaFormat->channels);
      AudPP_PrintChannelType(&pMediaFormat->channelMapping[0], (int32_t)pMediaFormat->channels, me->objId);
   }
}
...
static ADSPResult AudPP_topo_process_respq(topo_event_client_t *obj_ptr)
{
	ADSPResult result = ADSP_EOK;
	ThisAudDynaPPSvc_t *me = AudPP_topo_cb_to_svc(obj_ptr);
	result = AudPP_ProcessRespQhandler(me);
	return result;
}
