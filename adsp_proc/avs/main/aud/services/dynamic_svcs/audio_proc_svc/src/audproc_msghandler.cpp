...
ADSPResult AudPP_DataProc_EosFormat(ThisAudDynaPPSvc_t* me,uint32_t port_id)
{
   ...
   if(AudPP_isPrimaryInputPort(port_id))
   {
     ...
     {
        topo_reset(me->pTopologyObj, NULL) ;
     }
...
void AudPP_DataProc_DataBufFormatProc(ThisAudDynaPPSvc_t* me)
{
   ...
   uint32_t unMyDataBitfield =  pMyStat->unCommandOnlyBitField | pMyStat->unCommandResponseOnlyBitField;;
   ...
   *pMyCurrentBitfield = pMyStat->unCommandOnlyBitField | pMyStat->unCommandResponseOnlyBitField;
   ...
   if (pMyStat->pOutDataMsgDataBuf != NULL)
   {
      ...
      if (pMyStat->m_fKeepProcessingonEos)
      {
         ...
         if (m_fdoneEmptyingBuf) //intermediate buffer is empty
         {
            ...
            *pMyCurrentBitfield = unMyDataBitfield |pMyStat->unCommandResponseOnlyBitField;//(unMyDataBitfield | pMyStat->unDataBitfield[AUDPP_PRIMARY_IN_PORT] | pMyStat->unCommandOnlyBitField);
      ...
      else
      {
         ...
         if(unMyDataBitfield)
         {
        	 *pMyCurrentBitfield = (unMyDataBitfield | pMyStat->unCommandOnlyBitField | pMyStat->unCommandResponseOnlyBitField);
         }
...
ADSPResult AudPP_Alloc_DataMsg_Cache(ThisAudDynaPPSvc_t *me)
{
   ...
   for(idx = 0; idx < me->numActiveInPorts; idx++)
   {
      ...
      if(NULL == pInPort->elite_data_msg_cache.pCachedInDataMsg)
      {
         ...
         pInPort->elite_data_msg_cache.pCachedInDataMsg = (elite_msg_any_t* )qurt_elite_memory_malloc(req_size, QURT_ELITE_HEAP_DEFAULT);
         if(NULL == pInPort->elite_data_msg_cache.pCachedInDataMsg)
         {
