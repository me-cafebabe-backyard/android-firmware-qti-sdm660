...
#define  NUM_CHANNELS_MAX    (ELITE_PCM_MAX_NUM_CH)
...
typedef struct _AdspAudioMediaFormatInfo_t
{
   ...
   uint8_t channelMapping[NUM_CHANNELS_MAX];
...
struct topo_event_vtable_t
{
   ...
   void (*update_run_state) (topo_event_client_t *obj_ptr, uint32_t olm_run_response_result);
   void (*custom_event) (topo_event_client_t *obj_ptr, void *payload, uint32_t payloadsize);
   ADSPResult (*process_respq) (topo_event_client_t *obj_ptr);
};
...
ADSPResult topo_init(const audproc_topology_definition_t *def_ptr,
	  ...
	  uint16_t is_input_output_realtime = 0,
...
ADSPResult common_topo_init(elite_cmn_topo_db_entry_t *def_ptr,
		...
		uint16_t is_input_output_realtime = 0,
		uint32_t num_active_input_ports = 1,
		qurt_elite_queue_t* dynasvc_cmdQ = NULL);
...
ADSPResult topo_update_run_state(void *context_ptr , uint32_t olm_run_response_result);
