...
Copyright (c) 2011-2017,2018 Qualcomm Technologies, Inc.  All Rights Reserved.
...
uint32_t AudPP_AggregateKpps(ThisAudDynaPPSvc_t *pMe)
{
 ...
 ADSPResult AudPP_register_avtimer(ThisAudDynaPPSvc_t* me)
 {
 	...
 	char client_name[5]; // 5 spaces for the unique id and one for the NULL terminator.
    snprintf(client_name, 5, "%hX", me->objId);

 	if (ADSP_EOK != (result = qurt_elite_timer_open(&(me->avt_drv_handle), (char*) &(client_name[0]))))
 	{
 		MSG_2(MSG_SSID_QDSP6,
 				DBG_ERROR_PRIO,
 				"P%hX : Failed to open AVtimer driver with result: %d",
 				me->objId,
 				result);
 		return result;
 	}
 	else
 	{
 		MSG_2(MSG_SSID_QDSP6,
 				DBG_HIGH_PRIO,
 				"P%hX : Sucessfully opened AVtimer driver with result: %d",
 				me->objId,
 				result);
 	}
 	return result;
 ...
 ADSPResult AudPP_deregister_avtimer(ThisAudDynaPPSvc_t* me)
 {
    ...
    if (NULL != me->avt_drv_handle)
    {
       if (ADSP_EOK != (result = qurt_elite_timer_close(me->avt_drv_handle)))
       {

  		MSG_2(MSG_SSID_QDSP6,
  				DBG_ERROR_PRIO,
  				"P%hX : Failed to Close AVtimer Driver with result: %d",
  				me->objId,
  				result);
       }
       me->avt_drv_handle = NULL;
    }
    return result;
 }
