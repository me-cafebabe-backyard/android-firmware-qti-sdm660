...
static const uint32_t module_using_hw_resampler[]=
{
 ...
 /**
 * This function is to send command to the Remote processor.
 * @param[in] 
 * me - Pointer to the current instance structure
 * olm_fwk_command_id - specifies the command ID
 * 
 * @return int
 * Returns an ADSP error code indicating status.
 */
ADSPResult AudPP_Send_Command_to_Remote (const ThisAudDynaPPSvc_t *me, uint32_t olm_fwk_command_id ,
		elite_msg_any_t* pMsg, uint32_t  *wait_for_send_ack)
{
	ADSPResult result = ADSP_EUNSUPPORTED;
	*wait_for_send_ack = 0;
	topo_olm_reset_state_for_remote_response(me->pTopologyObj);
	switch(olm_fwk_command_id)
	{
	case CAPI_V2_PARAM_ID_OLM_RUN:
		result = topo_set_run_to_olm(me->pTopologyObj, pMsg);
		break;
	case CAPI_V2_PARAM_ID_OLM_SUSPEND:
		result = topo_set_suspend_to_olm(me->pTopologyObj, pMsg);
		break;
	default :
		break;
	}
	...
	if(result==ADSP_EOK)
	{
		*wait_for_send_ack = topo_check_if_wait_for_remote_response(me->pTopologyObj);
	}
	return result;
}
