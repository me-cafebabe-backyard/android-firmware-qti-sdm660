...
#define TOPO_ALIGN_TO_4(x) (ELITE_ALIGN_BY_4(x))
...
struct topo_media_fmt_v2_t;
struct topo_media_fmt_v1_t;
struct topo_buf_t;
struct topo_graph_t;
...
// Define the  media format type supported.
struct topo_media_fmt_v2_t
{
...
static const topo_media_fmt_v2_t invalid_media_format_v2 = {
        {
                CAPI_V2_FIXED_POINT
        },
        {
                CAPI_V2_MEDIA_FORMAT_MINOR_VERSION,
...
struct topo_buf_t
{
    ...
    int64_t timestamp;
    int8_t *buf_ptrs[CAPI_V2_MAX_CHANNELS_V2];
    uint32_t num_bufs;
...
static ADSPResult common_topo_init_single_module(topo_struct *topo_ptr,
		const uint32_t index,
		avcs_module_instance_proc_info_t *module_info_ptr,
		const adsp_amdb_module_handle_info_t *handle_info_ptr,
		AudPP_AudProcType pp_type,
		uint32_t *stack_size_ptr,
		bool_t initialize_with_compressed_format,
		bool_t is_olm_module);
...
static ADSPResult common_topo_create_capi_v2_module(topo_struct *topo_ptr, adsp_amdb_capi_v2_t *handle, const uint32_t index, avcs_module_instance_proc_info_t *module_info_ptr, AudPP_AudProcType pp_type, uint32_t *stack_size_ptr, capi_v2_proplist_t *init_proplist_ptr);
static void topo_check_ec_module_presence(topo_struct *topo_ptr,adsp_amdb_module_handle_info_t *handle_info_ptr,
		uint32_t num_modules,uint32_t *ec_module_index, uint32_t *is_ec_offloaded);
...
ADSPResult topo_destroy_olm_modules(uint32_t num_modules, topo_struct *topo_ptr, uint32_t *resp_index);
...
static void topo_print_channel_type(const uint16_t *channel_type, int32_t num_channels, uint16_t obj_id );
...
struct topo_media_event_t
{
   ...
   topo_media_fmt_v2_t event;
};
static void topo_set_media_event(topo_media_event_t *downstream_event_ptr, const topo_media_fmt_v2_t *format_to_be_set);
...
typedef struct _module_mfc_intf_extn_info
{
  uint32_t        version;
  bool_t          changes_media_format;
}module_mfc_intf_extn_info;
...
typedef struct _module_info
{
   ...
   bool_t          is_enabled;
   module_mfc_intf_extn_info media_format_change_info;
   olm_info_t      *olm_info;
...
static ADSPResult topo_module_process_fwk_extn(topo_struct *topo_ptr, uint32_t num_modules);
...
ADSPResult topo_set_local_command_processing_complete(topo_struct *topo_ptr, uint32_t token, int32_t result);
...
struct topo_struct
{
   ...
   module_info     *olm_module; // temp array of OLM modules
   ...
   uint16_t is_input_output_realtime;
   ...
   bool_t  is_olm_present_in_topo;  // variable to indicate if OLM instance is present the topology.
   topo_olm_response_handler_t olm_response_handler; // OLM response handler
};
...
static ADSPResult topo_find_module_instance(const topo_struct *topo_ptr,
      ...
      uint32_t *index_ptr,
	  uint32_t *is_module_in_olm_topo);
...
static void topo_adsp_media_fmt_to_topo(const AdspAudioMediaFormatInfo_t *adsp_media_fmt_ptr, topo_media_fmt_v2_t *topo_media_fmt_ptr);
static void topo_topo_media_fmt_to_adsp(topo_media_fmt_v2_t *topo_media_fmt_ptr, AdspAudioMediaFormatInfo_t *adsp_media_fmt_ptr);
static void topo_print_media_format(uint16_t obj_id, const topo_media_fmt_v2_t *media_fmt_ptr);
static void topo_print_module_output_media_format(uint16_t obj_id, uint32_t module_id, uint32_t instance_id, const topo_media_fmt_v2_t *media_fmt_ptr);
...
static bool_t topo_is_equal_media_format(const topo_media_fmt_v2_t *f1, const topo_media_fmt_v2_t *f2);
...
static void topo_destroy_modules(uint32_t num_modules, topo_struct *topo_ptr, topo_buf_manager *buf_mgr_ptr);
...
ADSPResult topo_olm_set_config_param (topo_struct *topo_ptr, void *pPayload,
		uint32_t payloadSize, topo_set_get_param_mode_t mode,elite_msg_any_t* pMsg, uint32_t *cmd_token);
ADSPResult topo_olm_get_config_param (topo_struct *topo_ptr, void *pPayload, const uint16_t payloadSize,
		const uint32_t module_id, const uint32_t instance_id, const uint32_t paramId, uint32_t module_index, uint32_t *pParamSize,
		topo_set_get_param_mode_t mode, elite_msg_any_t* pMsg);
ADSPResult topo_olm_create_module (topo_struct *topo_ptr, elite_msg_any_t* pMsg);
ADSPResult topo_olm_set_flush_command (topo_struct *topo_ptr, elite_msg_any_t* pMsg);
// remove is and move it up
static void topo_is_convert_v1_media_format_to_v2(topo_media_fmt_v1_t *format_ptr_v1, topo_media_fmt_v2_t *format_ptr_v2);
static void topo_is_convert_v2_media_format_to_v1(topo_media_fmt_v2_t *format_ptr_v2, topo_media_fmt_v1_t *format_ptr_v1);
...
static ADSPResult topo_create_obj(const uint16_t obj_id, topo_struct **topo_obj_ptr, const uint32_t ouput_buffer_num_unit_frames, topo_event_client_t *event_client, uint16_t is_input_output_realtime)
{
   *topo_obj_ptr = NULL;
   ...
   topo_ptr->olm_module = NULL;
   topo_ptr->is_olm_present_in_topo = FALSE;
   memset((void *)&topo_ptr->olm_response_handler, 0, sizeof(topo_olm_response_handler_t));
...
{
    return qurt_elite_memory_malloc(malloc_size, QURT_ELITE_HEAP_DEFAULT);
}
      ...
      bool_t initialize_with_compressed_format)
{
   ...
   topo_ptr->module[index].media_format_change_info.changes_media_format = FALSE;
   topo_ptr->module[index].media_format_change_info.version = 1;
   topo_ptr->module[index].module_ptr = NULL;
   ...
   capi_v2_heap_id_t heap_id = { QURT_ELITE_HEAP_DEFAULT };
	  ...
	  bool_t is_olm_module)
{
	...
	if(!is_olm_module)
	{
		port_info = topo_graph_get_port_num_info(&topo_ptr->graph, index);
	}

   // Set up default values
   topo_ptr->module[index].module_id  = module_info_ptr->module_id;
   topo_ptr->module[index].instance_id  = (uint32_t)module_info_ptr->instance_id;
   topo_ptr->module[index].is_olm_module  = is_olm_module;
   topo_ptr->module[index].is_in_place = FALSE;
   topo_ptr->module[index].requires_data_buffering = TRUE;
   topo_ptr->module[index].is_enabled = TRUE;
   topo_ptr->module[index].media_format_change_info.changes_media_format = FALSE;
   ...
   if(is_olm_module)
   {
	   topo_ptr->module[index].is_olm_module = TRUE;
	   topo_ptr->module[index].olm_info = (olm_info_t *)qurt_elite_memory_malloc(sizeof(olm_info_t), QURT_ELITE_HEAP_DEFAULT);
	   if (NULL == topo_ptr->module[index].olm_info)
	   {
		   MSG_1(MSG_SSID_QDSP6, DBG_FATAL_PRIO, "P%hX audproc_svc: Failed to allocate memory for OLM module info", topo_ptr->obj_id);
		   ADSPResult result = ADSP_ENOMEMORY;
		   return result;
	   }
       
     memset(topo_ptr->module[index].olm_info, 0, sizeof(olm_info_t));
       
	   olm_resp_callback_info_t *olm_rsp_cb_info = (olm_resp_callback_info_t *)&topo_ptr->module[index].olm_info->olm_resp_cb_info;
	   olm_rsp_cb_info->cb_fn_ptr = topo_olm_response_callback;
	   olm_rsp_cb_info->context_ptr = topo_ptr;


	   topo_ptr->module[index].olm_info->frame_size = topo_ptr->ouput_buffer_num_unit_frames;
	   topo_ptr->module[index].olm_info->olm_iid = (uint16_t) topo_ptr->module[index].instance_id;
	   topo_ptr->module[index].olm_info->scale_factor = 1;
	   topo_ptr->module[index].olm_info->client_id  = topo_ptr->obj_id;
   ...
   *topo_obj = NULL;
   ...
   if (ADSP_FAILED(result))
   {
      if (NULL != topo_ptr)
      {
         if (NULL != topo_ptr->module)
         {
            topo_destroy_modules(num_created_modules, topo_ptr, &topo_ptr->buf_mgr);
...
adsp_amdb_module_handle_info_t *topo_amdb_get_modules_request(const uint16_t obj_id, uint32_t num_modules, elite_cmn_topo_db_entry_t *def_ptr)
{
	adsp_amdb_module_handle_info_t *handle_info_ptr = NULL;
	uint32_t num_modules_filled = 0;
    ...
    return handle_info_ptr;
}

ADSPResult topo_initialize_olm_in_topology(topo_struct *topo_ptr,
		AudPP_AudProcType pp_type,
		uint32_t *stack_size_ptr,
		bool_t initialize_with_compressed_format,
		uint32_t *num_created_modules,
		adsp_amdb_module_handle_info_t *handle_info_ptr,
		elite_cmn_topo_db_entry_t *def_ptr,
		qurt_elite_queue_t*  dynasvc_cmdQ)
{
	 ADSPResult result = ADSP_EOK;
	 //uint32_t stub_module_count = 0;
	 uint32_t num_olm_module =0;
	 uint32_t olm_index =0;
	 uint32_t num_modules =  def_ptr->topo_def.num_modules;
	 int32_t olm_module_list[MAX_OLM_MODULES];

	 for (uint32_t j = 0; j < MAX_OLM_MODULES; j++ )
	 {
		 olm_module_list[j] = 0xFFFF; // set to default value
	 }
	 ...
	 for (uint32_t j = 0; j < num_modules; j++ )
	 {
		 ...
		 // Check for OLM module presence in topology before creating
		 bool_t is_olm_module = FALSE;
		 is_olm_module = topo_check_if_olm_module(topo_ptr->obj_id,handle_info_ptr + j);
		 if(is_olm_module)
		 {
			 if(num_olm_module <MAX_OLM_MODULES)
			 {
			 olm_module_list[num_olm_module] = j;
			 num_olm_module++;
			 }
			 else
			 {
				 MSG_1(MSG_SSID_QDSP6, DBG_FATAL_PRIO, "P%hX audproc_svc: OLM module in the topology greater than supported value 16", topo_ptr->obj_id);
	 ...
	 if(num_olm_module > 0)
	 {
		 topo_ptr->is_olm_present_in_topo = TRUE;
		 topo_ptr->module = (module_info*)qurt_elite_memory_malloc(sizeof(module_info) * num_olm_module, QURT_ELITE_HEAP_DEFAULT);
		 if (NULL == topo_ptr->module)
		 {
			 MSG_1(MSG_SSID_QDSP6, DBG_FATAL_PRIO, "P%hX audproc_svc: Failed to allocate olm modules array", topo_ptr->obj_id);
			// topo_destroy(topo_ptr);
			 result = ADSP_ENOMEMORY;
			 return result;
		 }

		 memset(topo_ptr->module, 0, (sizeof(module_info) * num_olm_module));

		 avcs_module_instance_proc_info_t *module_info_ptr = def_ptr->module_info;
		 *num_created_modules = 0;
		 ...
		 for (uint32_t j = 0; j < num_modules; j++ )
		 {
			 ...
			 // Check for OLM module presence in topology before creating
			 bool_t is_olm_module = FALSE;
			 if(olm_module_list[olm_index]==j)
			 {
				 is_olm_module = TRUE;
			 }
			 if(is_olm_module)
			 {
				 result = common_topo_init_single_module(topo_ptr, olm_index,
						 &module_info_ptr[j],
						 handle_info_ptr + j,
						 pp_type,
						 stack_size_ptr,
						 initialize_with_compressed_format,
						 is_olm_module);
				 if (ADSP_FAILED(result))
				 {
					 MSG_3(MSG_SSID_QDSP6, DBG_FATAL_PRIO, "P%hX audproc_svc: OLM  Module-Instance with ID 0x%lX-0x%lX failed in init.",
							 topo_ptr->obj_id, module_info_ptr[j].module_id, module_info_ptr[j].instance_id);
					 break;
				 }
				 else
				 {
					 topo_ptr->module[olm_index].olm_info->cmd_respQ = dynasvc_cmdQ;
					 elite_cmn_topo_db_get_olm_topology(1, def_ptr->topo_def.topology_id, topo_ptr->module[olm_index].olm_info->olm_iid, &topo_ptr->module[olm_index].olm_info->olm_topo_info_ptr);
					 topo_ptr->module[olm_index].olm_info->module_index = j;
					 topo_ptr->module[olm_index].olm_info->is_input_output_realtime =  topo_ptr->is_input_output_realtime;
					 topo_ptr->olm_response_handler.num_olm_modules++;
					 olm_index++;
				 }
			 }
		 }
	 ...
	 *num_created_modules = olm_index;
	 if(olm_index!=num_olm_module)
	 {
		 MSG_3(MSG_SSID_QDSP6, DBG_FATAL_PRIO, "P%hX audproc_svc: Failed to Initilize %lu number of OLM module, "
				 "only %lu got initialized",topo_ptr->obj_id, num_olm_module, olm_index);
		 return result;
	 }
	 else
	 {
		 topo_ptr->graph.num_modules = num_olm_module;
	 }

	 if(result==ADSP_EOK)
	 {
		 result = topo_olm_create_module (topo_ptr, NULL);
		 if(result!=ADSP_EOK)
		 {
			 MSG_1(MSG_SSID_QDSP6, DBG_ERROR_PRIO,"Audproc obj %lu : Failed to create the Offloaded module "
					 "- topology initialization failed", topo_ptr->obj_id);
		 }
	 }


	 if((result==ADSP_EOK) && (topo_ptr->is_olm_present_in_topo))
	 {
		 MSG_1(MSG_SSID_QDSP6, DBG_ERROR_PRIO, "P%hX audproc_svc: Waiting for response from remote processing module : Open handling", topo_ptr->obj_id);
		 while(topo_check_if_wait_for_open_response(topo_ptr))
		 {
			 topo_ptr->event_client->vtbl->process_respq(topo_ptr->event_client);
		 }
		 result=topo_get_olm_open_response_result(topo_ptr);
	 }
...
ADSPResult topo_initialize_topology(topo_struct *topo_ptr,
		AudPP_AudProcType pp_type,
		uint32_t *stack_size_ptr,
		bool_t initialize_with_compressed_format,
		uint32_t *num_created_modules,
		adsp_amdb_module_handle_info_t *handle_info_ptr,
		elite_cmn_topo_db_entry_t *final_topo_def )
{
	 ADSPResult result = ADSP_EOK;
	 uint32_t stub_module_count = 0;
	 uint32_t num_modules = topo_graph_num_modules(&topo_ptr->graph);
	 uint32_t olm_module_index = 0;
	 uint32_t module_count =0;

	 if(topo_ptr->is_olm_present_in_topo && (topo_ptr->module!=NULL))
	 {
		 topo_ptr->olm_module = topo_ptr->module;
		 topo_ptr->module = NULL;
	 }
    ...
    avcs_module_instance_proc_info_t *module_info_ptr = final_topo_def->module_info;
    *num_created_modules = 0;
    ...
    for (uint32_t j = 0; j < num_modules; j++ )
    {
       ...
       // Check for OLM module presence in topology before creating
       bool_t is_olm_module = FALSE;
       is_olm_module = topo_check_if_olm_module(topo_ptr->obj_id,handle_info_ptr + stub_module_count + j);
       if(!is_olm_module)
       {
    	   result = common_topo_init_single_module(topo_ptr, j,
    			   &module_info_ptr[stub_module_count + j],
				   handle_info_ptr + stub_module_count + j,
				   pp_type,
				   stack_size_ptr,
				   initialize_with_compressed_format,
				   is_olm_module);
       ...
       {
    	   // copy the olm module
    	   if(olm_module_index < topo_ptr->olm_response_handler.num_olm_modules)
    	   {
    		   memscpy(&topo_ptr->module[j],
    				   sizeof(module_info),
					   &topo_ptr->olm_module[olm_module_index],
					   sizeof(module_info));

    		   {
    			   capi_v2_port_num_info_t port_info =  topo_graph_get_port_num_info(&topo_ptr->graph, j);
    			   topo_event_initialize_handle(topo_ptr, j);
    			   capi_v2_event_callback_info_t cb_info = { topo_event_callback, topo_event_get_handle(topo_ptr, j) };

    			   if(topo_ptr->module[j].num_input_ports!=topo_ptr->graph.nodes[j].num_inputs)
    			   {
    				   topo_ptr->module[j].num_input_ports=topo_ptr->graph.nodes[j].num_inputs;
					   topo_module_free_ports(&topo_ptr->module[j], &topo_ptr->buf_mgr);
    				   ADSPResult temp_result = topo_module_allocate_ports(&topo_ptr->module[j]);
    				   if (ADSP_FAILED(temp_result))
    				   {
    					   MSG_3(MSG_SSID_QDSP6, DBG_ERROR_PRIO, "P%hX audproc_svc: Failed to allocate memory for ports for module 0x%lX-0x%lX.",
    							   topo_ptr->obj_id, topo_ptr->module[j].module_id, topo_ptr->module[j].instance_id);
    			   ...
    			   capi_v2_proplist_t init_proplist = { SIZE_OF_ARRAY(init_props), init_props };
    			   capi_v2_err_t err_code = topo_ptr->module[j].module_ptr->vtbl_ptr->set_properties(topo_ptr->module[j].module_ptr, &init_proplist);
    			   if (CAPI_V2_FAILED(err_code))
    			   {
    				   result= ADSP_EUNEXPECTED;
    				   break;
    		   ...
    		   olm_module_index++;
    		   topo_ptr->module[j].olm_info->module_index = j;
    	   }
    	   else
    	   {
    		   result= ADSP_EUNEXPECTED;
    ...
    // If the local modules fail to initialize, copy the OLM modules to the in the new module info
    for(uint32_t j = olm_module_index; j < topo_ptr->olm_response_handler.num_olm_modules; j++ )
    {
    	 //topo_ptr->module[module_count] = topo_ptr->olm_module[j];
    	if(module_count < num_modules)
    	{
    		memscpy(&topo_ptr->module[module_count],
    				sizeof(module_info),
					&topo_ptr->olm_module[j],
					sizeof(module_info));
    		module_count++;
    	}
    	else
    ...
    *num_created_modules = module_count;

    if(topo_ptr->olm_module)
    {
    	qurt_elite_memory_free(topo_ptr->olm_module);
    }

    return result;
}

ADSPResult topo_update_topo_def_with_far_modules(topo_struct *topo_ptr, elite_cmn_topo_db_entry_t *def_ptr,
		adsp_amdb_module_handle_info_t **handle_info_dptr, elite_cmn_topo_db_entry_t **final_topo_def )
{
	...
	adsp_amdb_module_handle_info_t *handle_info_ptr = *handle_info_dptr;
	...
	final_ec_topo_ptr_size = sizeof(elite_cmn_topo_db_entry_t) + (num_modules + far_topo_module_count) * sizeof(avcs_module_instance_proc_info_t);
	...
	final_ec_topo_ptr->module_info = (avcs_module_instance_proc_info_t *)( (uint8_t* )final_ec_topo_ptr + sizeof(elite_cmn_topo_db_entry_t));
	...
	for(uint32_t module_num=0;module_num<far_topo_module_count;module_num++)
	{
		...
		final_ec_topo_ptr->module_info[module_num].client_proc_id = AUDIO_DSP_PROC_ID;
	...
	for(uint32_t module_num=far_topo_module_count;module_num<final_ec_topo_ptr->topo_def.num_modules;module_num++)
	{
		...
		final_ec_topo_ptr->module_info[module_num].client_proc_id = def_ptr->module_info[module_num - far_topo_module_count].client_proc_id;
	...
	*final_topo_def = final_ec_topo_ptr;
    ...
    num_modules = final_ec_topo_ptr->topo_def.num_modules;
    handle_info_ptr = topo_amdb_get_modules_request(topo_ptr->obj_id, num_modules, final_ec_topo_ptr);
    if (handle_info_ptr == NULL)
    {
    ...
    if(temp_handle_info_ptr!=NULL)
    {
       ...
       *handle_info_dptr = handle_info_ptr;
    }
	  ...
	  qurt_elite_queue_t*  dynasvc_cmdQ)
{
   ...
   *topo_obj = topo_ptr;
   ...
   if(0 != num_modules)
   {
       handle_info_ptr = topo_amdb_get_modules_request(obj_id, num_modules, def_ptr);
       if (handle_info_ptr == NULL)
       {
           MSG_1(MSG_SSID_QDSP6, DBG_FATAL_PRIO, "P%hX audproc_svc: "
                 "Failed to allocate memory for the module handler", obj_id);
       ...
       result = topo_initialize_olm_in_topology(topo_ptr,
    		   pp_type,
			   stack_size_ptr,
			   initialize_with_compressed_format,
			   &num_created_modules,
       ...
       {
    	   MSG_1(MSG_SSID_QDSP6, DBG_FATAL_PRIO, "P%hX audproc_svc: "
    			   "Failed to create OLM modules : Failed Topology Creation", obj_id);
    	   goto cleanup;
       }

       // Check for EC module presence in topo before creating topo
       uint32_t ec_module_num = TOPO_INVALID_MODULE_INDEX;
       uint32_t is_ec_offloaded = 0;
       topo_check_ec_module_presence(topo_ptr,handle_info_ptr,num_modules,&ec_module_num, &is_ec_offloaded);
       ...
       {
    	   result = topo_update_topo_def_with_far_modules(topo_ptr, def_ptr, &handle_info_ptr, &final_topo_def);

    	   if(result!=ADSP_EOK)
    	   {
    		   MSG_1(MSG_SSID_QDSP6, DBG_FATAL_PRIO, "P%hX audproc_svc: "
    				   "Failed to allocate memory for EC fwk Extension handling", obj_id);
    		   goto cleanup;
    	   }
    	   final_ec_topo_ptr = final_topo_def;
    	   far_topo_module_count = topo_ptr->ec_extn_info->num_far_modules;
   ...
   *stack_size_ptr = ( *stack_size_ptr > AUDPROC_TOPO_DEFAULT_STACK_SIZE) ? *stack_size_ptr : AUDPROC_TOPO_DEFAULT_STACK_SIZE;

   topo_lock_module_list(topo_ptr);
   if((0 != num_modules) && (handle_info_ptr!=NULL))
   {
	   result =  topo_initialize_topology(topo_ptr,
			   	   pp_type,
				   stack_size_ptr,
				   initialize_with_compressed_format,
				   &num_created_modules,
	   ...
	   result = topo_module_process_fwk_extn(topo_ptr, num_created_modules);
	   if (ADSP_FAILED(result))
	   {
		   goto cleanup;
	   }
...
MSG_5(MSG_SSID_QDSP6, DBG_HIGH_PRIO, "P%hX audproc_svc: Module-Instance with id 0x%lX-0x%lX "
        ...
        QURT_ELITE_ASSERT(in_buf_ptr->num_bufs <= CAPI_V2_MAX_CHANNELS_V2);
...
MSG_5(MSG_SSID_QDSP6, DBG_HIGH_PRIO, "P%hX audproc_svc: Module-Instance with id 0x%lX-0x%lX "
      ...
      topo_ptr->obj_id, topo_ptr->module[module_index].module_id, topo_ptr->module[module_index].instance_id,samples_in_output, i);
        ...
        QURT_ELITE_ASSERT(out_buf_ptr->num_bufs <= CAPI_V2_MAX_CHANNELS_V2);
...
static void topo_set_input_media_fmt_event (topo_struct *topo_ptr)
{
   ...
   {
      topo_media_fmt_v2_t out_format;
...
void topo_pause_reset (topo_struct *topo_ptr)
{
   ...
   topo_olm_reset_state_for_remote_response(topo_ptr);
   result = topo_olm_set_flush_command (topo_ptr, NULL);
   if(result!=ADSP_EOK)
   {
	   MSG_1(MSG_SSID_QDSP6, DBG_ERROR_PRIO, "P%hX audproc_svc: Failed to send Flush command to remote module ", topo_ptr->obj_id);
   }
 ...
 *   Error code returning status.
 */
ADSPResult topo_reset (topo_struct *topo_ptr, elite_msg_any_t* pMsg )
{
   MSG_1(MSG_SSID_QDSP6, DBG_HIGH_PRIO, "P%hX audproc_svc: Topology Flush Reset Begin", topo_ptr->obj_id);
   ADSPResult result;

   topo_olm_reset_state_for_remote_response(topo_ptr);
   result = topo_olm_set_flush_command (topo_ptr, pMsg);
   if(result!=ADSP_EOK)
   {
	   MSG_1(MSG_SSID_QDSP6, DBG_ERROR_PRIO, "P%hX audproc_svc: Failed to send Flush command to remote module ", topo_ptr->obj_id);
   }
   ...
   MSG_1(MSG_SSID_QDSP6, DBG_HIGH_PRIO, "P%hX audproc_svc: Topology Flush Reset Processing done ", topo_ptr->obj_id);
   return result;
}



void topo_reset_olm_module(void *context_ptr)
{
	...
	for (uint32_t i = 0; i < topo_graph_num_modules(&topo_ptr->graph); i++)
	{
		...
		if(topo_ptr->module[i].is_olm_module)
		{
 ...
 *   Error code returning status.
 */
ADSPResult topo_set_param (topo_struct *topo_ptr, void *pPayload, uint32_t payloadSize, topo_set_get_param_mode_t mode,elite_msg_any_t* pMsg)
{
#ifdef PP_TOPO_ENABLE_LOW_LEVEL_MSG
   ...
   void *olm_pPayload = pPayload;
   uint32_t olm_payloadSize = payloadSize;
   uint32_t cmd_token =0;

   if((topo_ptr->is_olm_present_in_topo) && (pMsg !=NULL))
   {
	   result = topo_olm_set_config_param (topo_ptr, olm_pPayload, olm_payloadSize,  mode, pMsg, &cmd_token);
   }
   ...
   while (payloadSize > 0)
   {
      ...
      uint32_t index=0;
      uint32_t is_module_in_olm_topo=0;
      ADSPResult errCode = topo_find_module_instance(topo_ptr, moduleID, instanceID, &index, &is_module_in_olm_topo);
      do
      {
    	  if(ADSP_SUCCEEDED(errCode))
    	  {
    		  if((!is_module_in_olm_topo))
    		  {
    			  capi_v2_err_t err_code = CAPI_V2_EOK;
    			  capi_v2_buf_t buf;
   ...
   if((is_ec_set_param) && (topo_ptr->ec_extn_info))
   {
     ...
     do
     {
         ...
         capi_v2_t *module_ptr = topo_ptr->module[ec_index].module_ptr;
          ...
          if(!topo_ptr->module[ec_index].is_olm_module)
          {
   ...
   if((topo_ptr->is_olm_present_in_topo) && (pMsg !=NULL))
   {
	   result |= topo_set_local_command_processing_complete(topo_ptr, cmd_token, result);
   }
...
ADSPResult topo_get_olm_mid_given_iid(topo_struct *topo_ptr, const uint32_t instance_id ,  uint32_t *module_id)
{
	ADSPResult result = ADSP_EFAILED;
	*module_id = 0;
	for(uint32_t index = 0; index < topo_graph_num_modules(&topo_ptr->graph);index++)
	{
		if(topo_ptr->module[index].is_olm_module)
		{
			if(topo_ptr->module[index].instance_id == instance_id )
			{
				*module_id = topo_ptr->module[index].module_id;
				result = ADSP_EOK;
			}
		}
	}
 ...
 *   Error code returning status.
 */
ADSPResult topo_get_param (topo_struct *topo_ptr, void *pPayload, const uint16_t payloadSize,
                           const uint32_t module_id, const uint32_t instance_id, const uint32_t paramId,
                           uint32_t *pParamSize, topo_set_get_param_mode_t mode, elite_msg_any_t* pMsg)
{
   ADSPResult result = ADSP_EFAILED;
   ...
   {
      case GIVEN_INSTANCE:
         result = topo_find_module_instance(topo_ptr, module_id, instance_id, &index, &is_module_in_olm_topo);
      break;
      ...
      case ZERO_INSTANCE:
         result = topo_find_module_instance(topo_ptr, module_id, 0, &index, &is_module_in_olm_topo);
      break;
   ...
   if (ADSP_SUCCEEDED(result))
   {
	   if(is_module_in_olm_topo)
	   {
		   if(NULL == pMsg)
		   {
			   MSG_1(MSG_SSID_QDSP6, DBG_HIGH_PRIO, "P%hX audproc_svc: Topology OLM Recevied NULL configuration packet", topo_ptr->obj_id);
			   result = ADSP_EBADPARAM;
			   return result;
		   }
		   result =  topo_olm_get_config_param (topo_ptr,
				   pPayload,
				   payloadSize,
				   module_id,
				   instance_id,
...
void topo_destroy (topo_struct *topo_ptr)
{
   if (NULL != topo_ptr)
   {
      ...
      if(NULL != topo_ptr->module)
      {
         topo_destroy_modules(topo_graph_num_modules(&topo_ptr->graph), topo_ptr, &topo_ptr->buf_mgr);
         qurt_elite_memory_free(topo_ptr->module);
      }
...
static ADSPResult find_module_instance_in_olm_topo(const elite_cmn_olm_topo_db_entry_t *olm_topo_info, const uint32_t module_id, int32_t *olm_index)
{

	for( int32_t index =0; index < olm_topo_info->num_modules; index++)
	{
		if(olm_topo_info->module_info[index].module_id == module_id )
		{
			*olm_index = index;
			 return ADSP_EOK;
		}
...
bool_t check_compare_instance_id_in_olm_topo(const elite_cmn_olm_topo_db_entry_t *olm_topo_info, uint32_t module_id, uint16_t instance_id )
{
	for( int32_t index =0; index < olm_topo_info->num_modules; index++)
	{
		if(olm_topo_info->module_info[index].module_id == module_id )
		{
			if(olm_topo_info->module_info[index].instance_id==instance_id)
			return 1;
		}
	}
	return 0;
...
bool_t check_compare_instance_id(const topo_struct *topo_ptr, uint32_t *index_ptr, uint32_t module_id, uint16_t instance_id, uint32_t is_module_in_olm_topo )
{
	uint32_t module_index = *index_ptr;

	if(!is_module_in_olm_topo)
	{
		if(topo_ptr->module[module_index].instance_id!=instance_id)
		{
			*index_ptr+=1;
			return 1;
		}
	...
	{
		if(!check_compare_instance_id_in_olm_topo(topo_ptr->module[module_index].olm_info->olm_topo_info_ptr, module_id, instance_id))
		{
			*index_ptr+=1;
			return 1;
		}
...
static ADSPResult find_first_module_instance_from_index(const topo_struct *topo_ptr, const uint32_t module_id, uint32_t *index_ptr, uint32_t *is_module_in_olm_topo)
{
    ...
    for (uint32_t i = *index_ptr; i < num_modules; i++)
    {
        ...
        {
            *index_ptr = i;
            *is_module_in_olm_topo = 0;
            return ADSP_EOK;
        }
        else if( topo_ptr->module[i].is_olm_module)
        {
        	int32_t olm_index = -1;
        	ADSPResult result = ADSP_EOK;
        	result = find_module_instance_in_olm_topo(topo_ptr->module[i].olm_info->olm_topo_info_ptr, search_module_id, &olm_index);
        	if((result == ADSP_EOK) && (olm_index>=0))
        	{
        		*index_ptr = i;
        		*is_module_in_olm_topo = 1;
        		return ADSP_EOK;
        	}
        }
    }
...
static ADSPResult topo_find_first_module_instance(const topo_struct *topo_ptr, const uint32_t module_id, uint32_t *index_ptr)
{
    ...
    result = find_first_module_instance_from_index(topo_ptr, secondary_module_id, &index, &is_module_in_olm_topo);
    if(ADSP_SUCCEEDED(result)){
        *index_ptr = index;
    }
    return result;
...
static ADSPResult topo_find_next_module_instance(const topo_struct *topo_ptr, const uint32_t module_id, uint32_t *index_ptr)
{
    ...
    uint32_t is_module_in_olm_topo = 0;
    ADSPResult result = find_first_module_instance_from_index(topo_ptr, module_id, &index, &is_module_in_olm_topo);
    if(ADSP_SUCCEEDED(result)){
        *index_ptr = index;
    }
    return result;
 ...
 *   ADSP_ENOTEXIST if the module was not found.
 */
static ADSPResult topo_find_module_instance(const topo_struct *topo_ptr, const uint32_t module_id, const uint32_t instance_id, uint32_t *index_ptr ,  uint32_t *is_module_in_olm_topo )
{
    ADSPResult result = ADSP_EFAILED;
    uint32_t index = 0;
    uint32_t secondary_module_id = module_id;
    ...
    result = find_first_module_instance_from_index(topo_ptr, secondary_module_id, (uint32_t*)&index, is_module_in_olm_topo);


    while(ADSP_SUCCEEDED(result) && (check_compare_instance_id(topo_ptr, &index , module_id, instance_id, *is_module_in_olm_topo)))
    {
        //index++;
		uint32_t check_if_module_in_olm_topo = 0;
        result = find_first_module_instance_from_index(topo_ptr, secondary_module_id, (uint32_t*)&index, &check_if_module_in_olm_topo);
        *is_module_in_olm_topo = check_if_module_in_olm_topo;
    }
      ...
      for(uint32_t i = 0; i < intf_ext_list.intf_extn_num.num_extensions; i++)
      {
         if(intf_ext_list.intf_extn_desc[i].is_supported)
         {
            ...
            switch(intf_ext_list.intf_extn_desc[i].id)
            {
               ...
               {
                  module_info_ptr->media_format_change_info.changes_media_format = payload.bits_per_sample || payload.channel_info || payload.sampling_rate;
                  module_info_ptr->media_format_change_info.version = payload.version;
                  break;
               }
...
static ADSPResult topo_module_process_fwk_extn(topo_struct *topo_ptr, uint32_t num_modules)
{
	ADSPResult result = ADSP_EOK;
	module_info *m=NULL;
	//uint32_t token = 0, num_olm_responses=0;
	//bool_t create_olm = FALSE;
		...
		for (uint32_t i=0;i < m->num_fwk_extns; i++)
		{
			...
			case FWK_EXTN_ECNS:
			{
				...
				if((TRUE ==  topo_ptr->ec_extn_info->ec_capabilities.ecns_monitoring_info.is_rtm_supported)
						&& (!topo_ptr->ec_extn_info->is_ec_offloaded))
				{
...
static void topo_module_alloc_and_query_fwk_extn(topo_struct *topo_ptr, adsp_amdb_capi_v2_t *handle,uint32_t module_index)
{
    ...
    for(uint32_t fwk_extn_index=0;fwk_extn_index<module_info_ptr->num_fwk_extns;fwk_extn_index++)
    {
        ...
        case FWK_EXTN_ECNS:
        {
            ...
            if(!topo_ptr->ec_extn_info->is_ec_offloaded)
            {
...
static void topo_adsp_media_fmt_to_topo(const AdspAudioMediaFormatInfo_t *adsp_media_fmt_ptr, topo_media_fmt_v2_t *topo_media_fmt_ptr)
{
   topo_media_fmt_ptr->f.minor_version = CAPI_V2_MEDIA_FORMAT_MINOR_VERSION;
   ...
   {
       topo_media_fmt_ptr->channel_type[i] = adsp_media_fmt_ptr->channelMapping[i]; // Conversion from 8 bit to 16 bit here.
   }
}
...
static void topo_topo_media_fmt_to_adsp(topo_media_fmt_v2_t *topo_media_fmt_ptr, AdspAudioMediaFormatInfo_t *adsp_media_fmt_ptr)
{
    ...
    case CAPI_V2_FIXED_POINT:
        ...
        if(topo_media_fmt_ptr->f.num_channels > CAPI_V2_MAX_CHANNELS_V2)
        {
          MSG_2(MSG_SSID_QDSP6, DBG_ERROR_PRIO, "audproc_svc: %lu channels at topo output, limiting to %d.", topo_media_fmt_ptr->f.num_channels, CAPI_V2_MAX_CHANNELS_V2);
          topo_media_fmt_ptr->f.num_channels = CAPI_V2_MAX_CHANNELS_V2;
        ...
        for (uint32_t i = 0; i < topo_media_fmt_ptr->f.num_channels; i++)
        {
            adsp_media_fmt_ptr->channelMapping[i] = topo_media_fmt_ptr->channel_type[i]; // Conversion from 16 bit to 8 bit here.
        }
...
static void topo_print_channel_type(const uint16_t *ch_map, int32_t num_channels, uint16_t obj_id )
{
  uint32_t *ch_map_word;
  QURT_ELITE_ALIGN(uint8_t ch_map_local[NUM_CHANNELS_MAX], 4); // align by 4
  ...
  MSG_9(MSG_SSID_QDSP6, DBG_HIGH_PRIO,
        "P%hX audproc_svc: channel mapping (4 channel maps per value) 0x%lx,0x%lx,0x%lx,0x%lx,0x%lx,0x%lx,0x%lx,0x%lx",obj_id,
        ch_map_word[0], ch_map_word[1],ch_map_word[2],
...
static void topo_print_media_format(uint16_t obj_id, const topo_media_fmt_v2_t *media_fmt_ptr)
{
    ...
    MSG_2(MSG_SSID_QDSP6, DBG_LOW_PRIO, "P%hX audproc_svc: channel maps for %lu channels is as follows : ", obj_id, media_fmt_ptr->f.num_channels);
    topo_print_channel_type(&media_fmt_ptr->channel_type[0], (int32_t)media_fmt_ptr->f.num_channels, obj_id);
}
...
static bool_t topo_is_media_format_valid(const topo_media_fmt_v2_t *f, uint16_t obj_id, uint32_t module_id, uint32_t instance_id)
{
    ...
    if (CAPI_V2_FIXED_POINT == f->h.data_format)
    {
        ...
        if ((f->f.num_channels < 1) || (f->f.num_channels > CAPI_V2_MAX_CHANNELS_V2))
        {
            MSG_6(MSG_SSID_QDSP6, DBG_ERROR_PRIO, "P%hX audproc_svc: Module-Instance with id 0x%lX-0x%lX"
                    ...
                    obj_id, module_id, instance_id, f->f.num_channels, 1, CAPI_V2_MAX_CHANNELS_V2);
            return FALSE;
        }
    }
...
void *topo_olm_response_get_struct_ptr(void *handle)
{
	//topo_struct *topo_ptr = topo_event_get_struct_ptr(handle);
	topo_struct *topo_ptr = (topo_struct *)handle;
    return (void *) (& topo_ptr->olm_response_handler);
}

void topo_olm_set_state_for_remote_response(topo_struct *topo_ptr)
{
	topo_ptr->olm_response_handler.wait_for_remote_response = 1;
}

void topo_olm_reset_state_for_remote_response(topo_struct *topo_ptr)
{
	topo_ptr->olm_response_handler.wait_for_remote_response = 0;
}


bool_t topo_check_if_wait_for_remote_response(topo_struct *topo_ptr)
{
	bool_t status = FALSE;
    status = (topo_ptr->olm_response_handler.wait_for_remote_response > 0) ? 1 : 0;
	return status;
}

ADSPResult topo_get_olm_open_response_result(topo_struct *topo_ptr)
{

    return capi_v2_err_to_adsp_result(topo_ptr->olm_response_handler.open_command_response_result);
...
static capi_v2_err_t topo_event_callback(void *context_ptr, capi_v2_event_id_t id, capi_v2_event_info_t *event_ptr)
{
    ...
    case CAPI_V2_EVENT_KPPS:
    {
        ...
        if (event_ptr->payload.actual_data_len < required_payload_size){
    ...
    {
        required_payload_size = sizeof(topo_media_fmt_v1_t);
        if (event_ptr->payload.actual_data_len < required_payload_size){
            CAPI_V2_SET_ERROR(result,CAPI_V2_ENEEDMORE);
            break;
        }

        topo_media_fmt_v1_t *format_ptr = reinterpret_cast<topo_media_fmt_v1_t*>(event_ptr->payload.data_ptr);
        topo_media_fmt_v2_t format_v2;
        memset(&format_v2, 0, sizeof(format_v2));

        topo_is_convert_v1_media_format_to_v2(format_ptr, &format_v2);
        ...
        if (!topo_is_media_format_valid(&format_v2, topo_ptr->obj_id, topo_ptr->module[module_index].module_id, topo_ptr->module[module_index].instance_id))
        {
            return CAPI_V2_EUNSUPPORTED;
        }
        ...
        topo_ptr->module[module_index].output_ports_ptr[event_ptr->port_info.port_index].media_event.event = format_v2;
        ...
        topo_print_module_output_media_format(topo_ptr->obj_id, topo_ptr->module[module_index].module_id,
                topo_ptr->module[module_index].instance_id, &format_v2);
        break;
    }
    case CAPI_V2_EVENT_OUTPUT_MEDIA_FORMAT_UPDATED_V2:
    {
        required_payload_size = sizeof(capi_v2_data_format_header_t) + sizeof(capi_v2_standard_data_format_v2_t);
        if (event_ptr->payload.actual_data_len < required_payload_size){
            CAPI_V2_SET_ERROR(result,CAPI_V2_ENEEDMORE);
            break;
        }

        topo_media_fmt_v2_t *format_ptr = reinterpret_cast<topo_media_fmt_v2_t*>(event_ptr->payload.data_ptr);
        ...
        required_payload_size += format_ptr->f.num_channels * sizeof(format_ptr->channel_type[0]);
        if (event_ptr->payload.actual_data_len < required_payload_size){
            CAPI_V2_SET_ERROR(result,CAPI_V2_ENEEDMORE); // put a msg
            break;
        }
        MSG_4(MSG_SSID_QDSP6, DBG_ERROR_PRIO, "P%hX audproc_svc: Module-Instance with id 0x%lX-0x%lX "
                            "provided out bits per sample %lu",
                            topo_ptr->obj_id, topo_ptr->module[module_index].module_id, topo_ptr->module[module_index].instance_id, format_ptr->f.bits_per_sample);



        topo_ptr->module[module_index].output_ports_ptr[event_ptr->port_info.port_index].media_event.event_is_pending = TRUE;

        // dont do memcpy directly for payload to internal struct as module may not be giving channel for all channels
        topo_ptr->module[module_index].output_ports_ptr[event_ptr->port_info.port_index].media_event.event.h = format_ptr->h;
        topo_ptr->module[module_index].output_ports_ptr[event_ptr->port_info.port_index].media_event.event.f = format_ptr->f;
        memscpy(topo_ptr->module[module_index].output_ports_ptr[event_ptr->port_info.port_index].media_event.event.channel_type,
            sizeof(topo_ptr->module[module_index].output_ports_ptr[event_ptr->port_info.port_index].media_event.event.channel_type),
            format_ptr->channel_type, format_ptr->f.num_channels * sizeof(capi_v2_channel_type_t));
        // Media format events raised by the module will be handled during the process
        topo_print_module_output_media_format(topo_ptr->obj_id, topo_ptr->module[module_index].module_id,
    ...
    {
        required_payload_size = sizeof(capi_v2_event_process_state_t);
        if (event_ptr->payload.actual_data_len < required_payload_size){
            CAPI_V2_SET_ERROR(result,CAPI_V2_ENEEDMORE);
            break;
        }
    ...
    {
        required_payload_size = sizeof(capi_v2_port_data_threshold_change_t);
        uint32_t port_id;
        if (event_ptr->payload.actual_data_len < required_payload_size)
        {
            CAPI_V2_SET_ERROR(result,CAPI_V2_ENEEDMORE);
    ...
    {
        required_payload_size = sizeof(capi_v2_event_headroom_t);
        if (event_ptr->payload.actual_data_len < required_payload_size){
            CAPI_V2_SET_ERROR(result,CAPI_V2_ENEEDMORE);
            break;
        }
    ...
    {
        required_payload_size = sizeof(capi_v2_event_get_library_instance_t);
        if (event_ptr->payload.actual_data_len < required_payload_size){
            CAPI_V2_SET_ERROR(result,CAPI_V2_ENEEDMORE);
            break;
        }
...
static void topo_is_convert_v1_media_format_to_v2(topo_media_fmt_v1_t *format_ptr_v1, topo_media_fmt_v2_t *format_ptr_v2)
{
  ...
  format_ptr_v2->h = format_ptr_v1->h;

  format_ptr_v2->f.minor_version = CAPI_V2_MEDIA_FORMAT_MINOR_VERSION;
  format_ptr_v2->f.bits_per_sample = format_ptr_v1->f.bits_per_sample;
  format_ptr_v2->f.bitstream_format = format_ptr_v1->f.bitstream_format;
  format_ptr_v2->f.data_interleaving = format_ptr_v1->f.data_interleaving;
  format_ptr_v2->f.data_is_signed = format_ptr_v1->f.data_is_signed;
  format_ptr_v2->f.num_channels = format_ptr_v1->f.num_channels;
  format_ptr_v2->f.q_factor = format_ptr_v1->f.q_factor;
  format_ptr_v2->f.sampling_rate = format_ptr_v1->f.sampling_rate;

  memscpy(format_ptr_v2->channel_type, sizeof(format_ptr_v2->channel_type), format_ptr_v1->f.channel_type, sizeof(format_ptr_v1->f.channel_type));
...
static void topo_is_convert_v2_media_format_to_v1(topo_media_fmt_v2_t *format_ptr_v2, topo_media_fmt_v1_t *format_ptr_v1)
{
  format_ptr_v1->h = format_ptr_v2->h;

  format_ptr_v1->f.bits_per_sample = format_ptr_v2->f.bits_per_sample;
  format_ptr_v1->f.bitstream_format = format_ptr_v2->f.bitstream_format;
  format_ptr_v1->f.data_interleaving = format_ptr_v2->f.data_interleaving;
  format_ptr_v1->f.data_is_signed = format_ptr_v2->f.data_is_signed;
  format_ptr_v1->f.num_channels = format_ptr_v2->f.num_channels;
  format_ptr_v1->f.q_factor = format_ptr_v2->f.q_factor;
  format_ptr_v1->f.sampling_rate = format_ptr_v2->f.sampling_rate;

  memscpy(format_ptr_v1->f.channel_type, sizeof(format_ptr_v1->f.channel_type), format_ptr_v2->channel_type, sizeof(format_ptr_v2->channel_type));
...
static bool_t topo_handle_pending_events(topo_struct *topo_ptr, uint32_t start_module_index,uint32_t port_id)
{
   ...
   memset(&output_media_event.event,0,sizeof(topo_media_fmt_v2_t));
   ...
   if (TOPO_INVALID_MODULE_INDEX == start_module_index)
   {
       ...
       topo_media_fmt_v2_t new_in_format;
   ...
   for (uint32_t i = start_module_index; i < topo_graph_num_modules(&topo_ptr->graph); i++)
    ...
    if (port_id == TOPO_PRIMARY_STRM) // TODO : Check if need to allocate buffer for far port and how?
    {
        if (
                ...
                (topo_ptr->is_input_output_realtime==1) && (!topo_isBgThreadReq(topo_ptr)))// || (topo_ptr->max_in_streams > 1)
                )
        {
...
static void topo_module_setup_output_buffers(topo_struct *topo_ptr, const topo_port_index_t *port_index_ptr)
{
   ...
   // Set up output buffers
   {
      ...
      topo_media_fmt_v2_t *out_format_ptr = &(topo_out_port_from_index(topo_ptr, port_index_ptr)->media_event.event);
...
static void topo_set_delay_ptr_virtualizer(topo_struct *topo_ptr)
{
            ...
            topo_get_virtualizer_delay_param_id(&virtualizer_delay_param_id) ) )
    {
        ...
        topo_set_param(topo_ptr, &delay_param, sizeof(delay_param), FIRST_INSTANCE, NULL);
    }
}
...
static void topo_set_ec_far_media_format(topo_struct *topo_ptr,const AdspAudioMediaFormatInfo_t *req_media_format)
{
    ...
    {
        asm_stream_param_data_v2_t param_payload;
        audproc_mfc_output_media_fmt_v2_t out_format;
        uint16_t channel_type[CAPI_V2_MAX_CHANNELS_V2];
    } ec_extn_mfc_param;
    uint32_t param_size = sizeof(audproc_mfc_output_media_fmt_v2_t) + req_media_format->channels * sizeof(uint16);
    param_size = TOPO_ALIGN_TO_4(param_size);

    ec_extn_mfc_param.param_payload.module_id = AUDPROC_MODULE_ID_MFC_FAR;
    ec_extn_mfc_param.param_payload.param_id = AUDPROC_PARAM_ID_MFC_OUTPUT_MEDIA_FORMAT_V2;
    ec_extn_mfc_param.param_payload.param_size = param_size;
    ...
    {
        ec_extn_mfc_param.channel_type[ch_index] = (uint16_t)req_media_format->channelMapping[ch_index];
    }
    ...
    // not sending V1 media format to MFC as it is internal module
}
...
ADSPResult topo_send_capiv2_output_media_format (topo_struct *topo_ptr, const AudPPSvcOutputMediaTypeParams_t *pParams, uint32_t port_index)
{
   ...
   capi_v2_err_t err_code = CAPI_V2_EOK;
   uint8_t propose_op_media_fmt[sizeof(intf_extn_param_id_chg_mf_propose_op_mf_v2_t) + CAPI_V2_MAX_CHANNELS_V2*sizeof(capi_v2_channel_type_t)] = {0};
   capi_v2_channel_type_t *channel_type;
   uint32_t payload_len = 0, id;

   intf_extn_param_id_chg_mf_propose_op_mf_v2_t *proposed_media_format = (intf_extn_param_id_chg_mf_propose_op_mf_v2_t *)propose_op_media_fmt;

   for(uint32_t i = 0; i < topo_ptr->graph.num_modules; i++)
   {
      if(topo_ptr->module[i].media_format_change_info.changes_media_format)
      {
        ...
        if(topo_ptr->module[i].media_format_change_info.version == 1)
        {
          // try with v1 payload, since v2 payload is bugger than v1
          intf_extn_param_id_chg_mf_propose_op_mf_t *proposed_media_format_v1 = (intf_extn_param_id_chg_mf_propose_op_mf_t *)propose_op_media_fmt;
          memset(proposed_media_format_v1, 0, sizeof(intf_extn_param_id_chg_mf_propose_op_mf_t));
          err_code = CAPI_V2_EOK;
          proposed_media_format_v1->default_bits_per_sample = pParams->useNativeBitsPerSample;
          proposed_media_format_v1->bits_per_sample = pParams->outputBitsPerSample;
          proposed_media_format_v1->default_sampling_rate = pParams->keepInputSamplingRate;
          proposed_media_format_v1->sampling_rate = pParams->outputSamplingRate;
          proposed_media_format_v1->default_channel_info = pParams->useNativeNumChannels;
          if(!pParams->useNativeNumChannels)
          {
             if(pParams->outputNumChannels == 0 || pParams->outputNumChannels > SIZE_OF_ARRAY(pParams->outChannelMap) ||
                 pParams->outputNumChannels > SIZE_OF_ARRAY(proposed_media_format_v1->channel_map) ||
                 pParams->outputNumChannels > CAPI_V2_MAX_CHANNELS)
             {
                MSG_1(MSG_SSID_QDSP6, DBG_ERROR_PRIO, "P%hX audproc_svc: Invalid number of channels %lu", pParams->outputNumChannels);
                return ADSP_EBADPARAM;
             }
             proposed_media_format_v1->num_channels = pParams->outputNumChannels;
             for(uint32_t i = 0; i < pParams->outputNumChannels; i++)
             {
                 proposed_media_format_v1->channel_map[i] = (capi_v2_channel_type_t)pParams->outChannelMap[i];
             }
          }
          payload_len = sizeof(intf_extn_param_id_chg_mf_propose_op_mf_t);
          id = INTF_EXTN_PARAM_ID_CHG_MF_PROPOSE_OP_MF;
        }
        else
        {
          ...
          if(!pParams->useNativeNumChannels)
          {
             if(pParams->outputNumChannels == 0 || pParams->outputNumChannels > SIZE_OF_ARRAY(pParams->outChannelMap) || pParams->outputNumChannels > CAPI_V2_MAX_CHANNELS_V2)
             {
                MSG_1(MSG_SSID_QDSP6, DBG_ERROR_PRIO, "P%hX audproc_svc: Invalid number of channels %lu", pParams->outputNumChannels);
                return ADSP_EBADPARAM;
             }
             proposed_media_format->num_channels = pParams->outputNumChannels;
             channel_type = (capi_v2_channel_type_t *)(proposed_media_format + 1);
             for(uint32_t i = 0; i < pParams->outputNumChannels; i++)
             {
                 channel_type[i] = (capi_v2_channel_type_t)pParams->outChannelMap[i];
             }
          }
          payload_len = sizeof(intf_extn_param_id_chg_mf_propose_op_mf_v2_t) + pParams->outputNumChannels * sizeof(capi_v2_channel_type_t);
          payload_len = TOPO_ALIGN_TO_4(payload_len);
          id = INTF_EXTN_PARAM_ID_CHG_MF_PROPOSE_OP_MF_V2;
        }
        ...
        if(!topo_ptr->module[i].is_olm_module)
        {
          capi_v2_buf_t proposed_media_format_payload = {
                reinterpret_cast<int8_t*>(propose_op_media_fmt),
                payload_len,
                sizeof(propose_op_media_fmt)
          };
          err_code = module_ptr->vtbl_ptr->set_param(module_ptr,
                                       id,
                                       &port_info,
                                       &proposed_media_format_payload);
          if (CAPI_V2_FAILED(err_code))
          {
    	  ...
    	  {
    		  capi_v2_olm_set_inf_extn_module_config_t  remote_module_pmf;
    		  remote_module_pmf.inf_extn_id = INTF_EXTN_CHG_MF;
    		  remote_module_pmf.payload_size = payload_len;
    		  remote_module_pmf.payload_ptr = proposed_media_format;

    		  capi_v2_buf_t proposed_media_format_payload_to_olm = {
    				  reinterpret_cast<int8_t*>(&remote_module_pmf),
					  sizeof(remote_module_pmf),
					  sizeof(remote_module_pmf)
    		  };
    		  ...
    		  err_code = module_ptr->vtbl_ptr->set_param(module_ptr,
						  ...
						  &port_info,
						  &proposed_media_format_payload_to_olm);
    		  if (CAPI_V2_FAILED(err_code))
    		  {
    			  MSG_3(MSG_SSID_QDSP6, DBG_ERROR_PRIO, "P%hX audproc_svc: Module-Instance with ID 0x%lX-0x%lX "
    					  "failed to set param for propose output media format.",
...
ADSPResult topo_get_module_list(const topo_struct *topo_obj, uint32_t* topo_module_list_ptr, uint32_t payload_size, uint32_t secOpCode)
{
   ...
   uint32_t num_modules,local_topo_num_module, per_instance_payload_size, req_payload_size,start_module_index=0;
   ...
   local_topo_num_module = topo_graph_num_modules(topo_graph_ptr);
   num_modules = ( local_topo_num_module - start_module_index );

   for(uint32_t i = start_module_index; i < local_topo_num_module; i++ )
   {
	   if(topo_obj->module[i].is_olm_module && topo_obj->module[i].olm_info)
	   {
		   num_modules += topo_obj->module[i].olm_info->num_active_topology_modules;
	   }
   }
         ...
         *topo_module_list_ptr = num_modules;//topo_graph_num_modules(topo_graph_ptr);
         ...
         if(secOpCode == ELITEMSG_CUSTOM_GET_PP_TOPOLOGY_MODULES)
         {
             for (uint32_t i = start_module_index; i < local_topo_num_module; i++)
             {
                 if(topo_obj->module[i].is_olm_module && topo_obj->module[i].olm_info)
                 {
                	 for(uint32_t j = 0; j<topo_obj->module[i].olm_info->num_active_topology_modules;j++)
                	 {
                		 *topo_module_list_ptr  = topo_obj->module[i].olm_info->active_topo_def[j].module_id;
                		 topo_module_list_ptr++;
                	 }
                 }
                 else
         ...
         else
         {
             ...
             for (uint32_t i = start_module_index; i < local_topo_num_module; i++)
             {
            	 if(topo_obj->module[i].is_olm_module && topo_obj->module[i].olm_info)
            	 {
            		 for(uint32_t j = 0; j<topo_obj->module[i].olm_info->num_active_topology_modules;j++)
            		 {
            			 module_info_ptr->module_id  = topo_obj->module[i].olm_info->active_topo_def[j].module_id;
            			 module_info_ptr->instance_id = topo_obj->module[i].olm_info->active_topo_def[j].instance_id;
            			 module_info_ptr++;
            		 }
            	 }
            	 else
...
static void topo_module_set_pending_input_media(module_info *module_ptr, uint16_t obj_id)
{
   ...
   capi_v2_err_t err_code = CAPI_V2_EOK;
   capi_v2_err_t err_code1 = CAPI_V2_EOK;
   ...
   for (uint32_t i = 0; i < module_ptr->num_input_ports; i++)
   {
      if (module_ptr->input_ports_ptr[i].media_event.event_is_pending)
      {
         ...
         /* Here first form CAPI_V2_INPUT_MEDIA_FORMAT_V2 payload and send it to the module, if module does not support
          * CAPI_V2_INPUT_MEDIA_FORMAT_V2 then it is expected to return error.
          * Upon receiving error form CAPI_V2_INPUT_MEDIA_FORMAT payload and send
          * If both fails disable the module*/

         topo_media_fmt_v2_t *new_format_ptr = &module_ptr->input_ports_ptr[i].media_event.event;
         actual_payload_len = sizeof(new_format_ptr->h) + sizeof(new_format_ptr->f) + new_format_ptr->f.num_channels * sizeof(capi_v2_channel_type_t);
         actual_payload_len = TOPO_ALIGN_TO_4(actual_payload_len);  // add more comment
         props[current_prop_index].id = CAPI_V2_INPUT_MEDIA_FORMAT_V2;
         ...
         props[current_prop_index].payload.actual_data_len = actual_payload_len;
         ...
         err_code1 = module_ptr->module_ptr->vtbl_ptr->set_properties(module_ptr->module_ptr, &proplist);
         current_prop_index = 0;
         if (CAPI_V2_FAILED(err_code1))
         {
             if(new_format_ptr->f.num_channels <= CAPI_V2_MAX_CHANNELS)
             {
                 // form V1 payload and send // add more comment
                  err_code1 = CAPI_V2_EOK;
                  topo_media_fmt_v1_t new_format_ptr_v1;
                  memset(&new_format_ptr_v1, 0, sizeof(new_format_ptr_v1));
                  topo_is_convert_v2_media_format_to_v1(new_format_ptr, &new_format_ptr_v1);
                  props[current_prop_index].id = CAPI_V2_INPUT_MEDIA_FORMAT;
                  props[current_prop_index].port_info.is_input_port = TRUE;
                  props[current_prop_index].port_info.is_valid = TRUE;
                  props[current_prop_index].port_info.port_index = i;
                  props[current_prop_index].payload.actual_data_len = sizeof(new_format_ptr_v1);
                  props[current_prop_index].payload.max_data_len = sizeof(new_format_ptr_v1);
                  props[current_prop_index].payload.data_ptr = reinterpret_cast<int8_t*>(&new_format_ptr_v1);
             ...
             CAPI_V2_SET_ERROR(err_code, err_code1);
             if (CAPI_V2_FAILED(err_code))
             {
...
static void topo_module_input_port_initialize(module_input_port_t *port_ptr)
{
   ...
   port_ptr->media_event.event = invalid_media_format_v2;
...
static void topo_destroy_modules(uint32_t num_modules, topo_struct *topo_ptr, topo_buf_manager *buf_mgr_ptr)
{
	...
	uint32_t resp_index;
	ADSPResult result = topo_destroy_olm_modules(num_modules, topo_ptr, &resp_index);
	if((result==ADSP_EOK) && (topo_ptr->is_olm_present_in_topo) && (topo_ptr->olm_response_handler.wait_for_remote_response))
	{
		MSG_1(MSG_SSID_QDSP6, DBG_ERROR_PRIO, "P%hX audproc_svc: Waiting for response from remote processing module : destroy handling", topo_ptr->obj_id);
		while(topo_check_if_wait_for_close_response(topo_ptr))
		{
			topo_ptr->event_client->vtbl->process_respq(topo_ptr->event_client);
		}
	}
	...
	for (uint32_t i = 0; i < num_modules; i++)
	{
		topo_module_destroy(&topo_ptr->module[i], buf_mgr_ptr);
	}
}
...
static void topo_module_destroy(module_info *module_struct_ptr, topo_buf_manager *buf_mgr_ptr)
{
   ...
   if(module_struct_ptr->olm_info!=NULL)
   {
	   if(module_struct_ptr->olm_info->fwk_extn_ptr)
	   {
		   qurt_elite_memory_free(module_struct_ptr->olm_info->fwk_extn_ptr);
		   module_struct_ptr->olm_info->fwk_extn_ptr = NULL;
	   }
	   if(module_struct_ptr->olm_info->active_topo_def)
	   {
		   qurt_elite_memory_free(module_struct_ptr->olm_info->active_topo_def);
		   module_struct_ptr->olm_info->active_topo_def = NULL;
	   }

	   qurt_elite_memory_free(module_struct_ptr->olm_info);
	   module_struct_ptr->olm_info=NULL;
   }
...
static ADSPResult topo_allocate_stream_structs(topo_struct *topo_ptr)
{
   ...
   uint32_t bufs_size = align_to_8_byte(sizeof(capi_v2_buf_t) * num_ports * CAPI_V2_MAX_CHANNELS_V2);
   ...
   for (uint32_t i = 0; i < topo_ptr->max_in_streams; i++)
   {
      ...
      for (uint32_t j = 0; j < CAPI_V2_MAX_CHANNELS_V2; j++)
      {
         topo_ptr->in_streams[i].buf_ptr[j] = empty_buf;
      }
      ptr += sizeof(capi_v2_buf_t) * CAPI_V2_MAX_CHANNELS_V2;
}
...
static void  topo_check_ec_module_presence(topo_struct *topo_ptr,adsp_amdb_module_handle_info_t *handle_info_ptr,
		uint32_t num_modules,
		uint32_t *ec_module_index,
		uint32_t *is_ec_offloaded)
{
      ...
      if(CAPI_V2 == handle_info_temp->interface_type && (TRUE == is_amdb_module_handle_valid(handle_info_temp)))
       ...
       if(topo_ptr->is_olm_present_in_topo)
       {
    	   uint32_t num_olm_module = topo_ptr->graph.num_modules;
    	   for(uint32_t olm_index = 0; olm_index < num_olm_module;olm_index++)
    	   {
    		   module_info  *olm_module = &topo_ptr->module[olm_index];

    		   if(olm_module->olm_info->num_fwk_extns>0)
    		   {
    			   for(uint32_t i=0; i < olm_module->olm_info->num_fwk_extns; i++ )
    			   {
    				   if(FWK_EXTN_ECNS == olm_module->olm_info->fwk_extn_ptr[i].id)
    				   {
    					   MSG_2(MSG_SSID_QDSP6, DBG_HIGH_PRIO," get_static_prop: OLM module_id(0x%lx) would "
    							   "need extension_id(0x%lx)", olm_module->module_id, olm_module->olm_info->fwk_extn_ptr[i].id);
    					   *ec_module_index = olm_module->olm_info->module_index;
    					   *is_ec_offloaded = 1;
    					   return;
    				   }
    			   }
...
bool_t is_amdb_module_handle_valid(const adsp_amdb_module_handle_info_t *amdb_module_handle_ptr)
{
...
static uint32_t topo_get_unit_frames_in_module_input_buffer(topo_struct *topo_ptr, uint32_t module_index, uint32_t port_id)
{
   ...
   capi_v2_standard_data_format_v2_t *format_ptr = &input_ports_ptr->media_event.event.f;
...
ADSPResult topo_module_register_events(topo_struct *topo_ptr, uint32_t event_id, uint32_t module_id,  uint16_t instance_id, uint32_t config_mask)
{
	...
	ADSPResult errCode = topo_find_module_instance(topo_ptr, module_id, instance_id, &index, &is_module_in_olm_topo);
	if(ADSP_SUCCEEDED(errCode))
	{
		...
		topo_set_get_param_mode_t mode, elite_msg_any_t* pMsg)
{
	...
	if(paramId != AUDPROC_PARAM_ID_SAMPLESLIP_SAMPLE_ADJUST)
	{
		MSG_1(MSG_SSID_QDSP6, DBG_HIGH_PRIO, "P%hX audproc_svc: Topology OLM Get Configuration Begin.", topo_ptr->obj_id);
	}
#endif

	uint32_t token = 0;
	uint32_t olm_module_index = 0;
	if(topo_ptr->module[module_index].is_olm_module)
	{
		result = topo_create_command_olm_response_handler( &topo_ptr->olm_response_handler , &token);
		if(result==ADSP_ENOMEMORY)
		{
			MSG_1(MSG_SSID_QDSP6, DBG_ERROR_PRIO, "P%hX audproc_svc: failed to allocate memory for response Handler "
					"in the Set Configuration Command to OLM Modules .",topo_ptr->obj_id);
			return result;
		}

		capi_v2_err_t err_code = CAPI_V2_EOK;
		capi_v2_buf_t buf;
		capi_v2_olm_getparam_t olm_get_config;
		elite_msg_param_cal_v2_t *pParamMsg = (elite_msg_param_cal_v2_t *)pMsg->pPayload;

		olm_get_config.token = token;
		olm_get_config.mode = mode;
		olm_get_config.module_id = module_id;
		olm_get_config.instance_id = (uint16_t)instance_id;
		olm_get_config.param_id = paramId;
		olm_get_config.param_size = pParamSize;
		olm_get_config.payload = (int8_t *) (pPayload);
		olm_get_config.max_data_size = payloadSize;
		olm_get_config.bufferMemNode = pParamMsg->bufferMemNode;

		buf.data_ptr = (int8_t *) (&olm_get_config);
		buf.max_data_len = sizeof(capi_v2_olm_getparam_t);
		buf.actual_data_len = sizeof(capi_v2_olm_getparam_t);

		capi_v2_port_info_t port_info;
		port_info.is_valid = FALSE;
		capi_v2_t *module_ptr = topo_ptr->module[module_index].module_ptr;

		topo_update_olm_response_handler_module_info(&topo_ptr->olm_response_handler,
							token,
							olm_module_index,
							topo_ptr->module[module_index].olm_info->olm_iid);

		err_code = module_ptr->vtbl_ptr->set_param(module_ptr, CAPI_V2_PARAM_ID_OLM_GETPARAM, &port_info, &buf);
		result = capi_v2_err_to_adsp_result(err_code);
		if (result!=ADSP_EOK)
		{
			MSG_3(MSG_SSID_QDSP6, DBG_ERROR_PRIO, "P%hX audproc_svc: "
					"OLM Module with ID 0x%lX-0x%lX failed getParam.", topo_ptr->obj_id,
					module_id, topo_ptr->module[module_index].instance_id);
			*pParamSize = 0;
		}
		else
		{
		...
		topo_update_olm_response_handler(&topo_ptr->olm_response_handler,
				token,
				olm_module_index,
				result,
				pMsg,
...
ADSPResult topo_olm_set_config_param(topo_struct *topo_ptr,
		void *pPayload,
...
{
#ifdef PP_TOPO_ENABLE_LOW_LEVEL_MSG
   MSG_1(MSG_SSID_QDSP6, DBG_HIGH_PRIO, "P%hX audproc_svc: Topology Set params to OLM module Begin.", topo_ptr->obj_id);
#endif
   ADSPResult result = ADSP_EOK, acc_result = ADSP_EOK;
   uint8_t *pPacket = (uint8_t *)pPayload;
   int32_t olm_module_index = 0;
   uint32_t token = 0;
   ...
   if(topo_ptr->is_olm_present_in_topo)
   	{
   		...
   		{
   			MSG_2(MSG_SSID_QDSP6, DBG_HIGH_PRIO, "P%hX audproc_svc: created set param OLM command response handler "
   			   					"with the token %lu .",topo_ptr->obj_id,token);
   		}

   		capi_v2_err_t err_code = CAPI_V2_EOK;
   		capi_v2_buf_t buf;
   		capi_v2_olm_setparam_t olm_set_config;
   		elite_msg_param_cal_v2_t *pParamMsg = (elite_msg_param_cal_v2_t *)pMsg->pPayload;

   		olm_set_config.token = token;
   		olm_set_config.mode = (int32_t) mode;
   		olm_set_config.payload_ptr = (int8_t *) (pPacket);
   		olm_set_config.payload_size = payloadSize;
   		olm_set_config.bufferMemNode = pParamMsg->bufferMemNode;

   		buf.data_ptr = (int8_t *) (&olm_set_config);
   		...
   		for(uint8_t i = 0; i < topo_ptr->graph.num_modules; i++)
   		{
   			if(topo_ptr->module[i].is_olm_module)
   			{
   				capi_v2_t *module_ptr = topo_ptr->module[i].module_ptr;
   				...
   				topo_update_olm_response_handler_module_info(&topo_ptr->olm_response_handler,
   						token,
						olm_module_index,
						topo_ptr->module[i].instance_id);

   				err_code = module_ptr->vtbl_ptr->set_param(module_ptr, CAPI_V2_PARAM_ID_OLM_SETPARAM, &port_info, &buf);
   				result |= capi_v2_err_to_adsp_result(err_code);
   				if (CAPI_V2_FAILED(err_code))
   				{
   					MSG_3(MSG_SSID_QDSP6, DBG_ERROR_PRIO, "P%hX audproc_svc: OLM Module-Instance with ID 0x%lX-0x%lX failed setParam.",
   							topo_ptr->obj_id, topo_ptr->module[i].module_id, topo_ptr->module[i].instance_id);
   				}
   				else
   				{
...
#ifdef PP_TOPO_ENABLE_LOW_LEVEL_MSG
   MSG_1(MSG_SSID_QDSP6, DBG_HIGH_PRIO, "P%hX audproc_svc: Topology OLM Set params End.", topo_ptr->obj_id);
#endif
   return result;
}
...
ADSPResult topo_olm_create_module (topo_struct *topo_ptr, elite_msg_any_t* pMsg)
{
	...
	if(topo_ptr->is_olm_present_in_topo)
	{
		...
		{
			MSG_1(MSG_SSID_QDSP6, DBG_ERROR_PRIO, "P%hX audproc_svc: failed to allocate memory for response Handler "
					"in the Open/Create Command to OLM Modules .",topo_ptr->obj_id);
			return result;
		}
		topo_set_wait_for_open_response(topo_ptr, TRUE);

		struct audproc_olm_open_t
		{
			capi_v2_olm_open_t open_cfg;
			olm_open_config_t  open_pp_cfg;
		}payload;
		module_info *m=NULL;


		for(uint8_t module_index = 0; module_index < topo_ptr->olm_response_handler.num_olm_modules; module_index++)
		{
			if(topo_ptr->module[module_index].is_olm_module)
			{
				m=&topo_ptr->module[module_index];

				payload.open_pp_cfg.olm_iid = m->olm_info->olm_iid;
				payload.open_pp_cfg.client_id = m->olm_info->client_id;
				payload.open_pp_cfg.scale_factor = 1;
				payload.open_pp_cfg.frame_size = m->olm_info->frame_size;
				payload.open_pp_cfg.framework_respQ = m->olm_info->cmd_respQ;
				payload.open_pp_cfg.olm_resp_cb_info.cb_fn_ptr = m->olm_info->olm_resp_cb_info.cb_fn_ptr;
				payload.open_pp_cfg.olm_resp_cb_info.context_ptr = m->olm_info->olm_resp_cb_info.context_ptr;
				payload.open_pp_cfg.olm_topo_info_ptr = m->olm_info->olm_topo_info_ptr;
				payload.open_cfg.token = token;
				payload.open_cfg.version = 0;
				payload.open_cfg.payload_size = sizeof(olm_open_config_t);

				switch(m->olm_info->is_input_output_realtime)
				{
					case 0 :
						payload.open_pp_cfg.is_input_output_realtime = 1;
						break;
					case 1 :
						payload.open_pp_cfg.is_input_output_realtime = 3;
						break;
					case 2 :
						payload.open_pp_cfg.is_input_output_realtime = 0;
						break;
					default: payload.open_pp_cfg.is_input_output_realtime = 3;
				}

				capi_v2_buf_t olm_open_config_payload =
				{
						reinterpret_cast<int8_t*>(&payload.open_cfg),
						sizeof(audproc_olm_open_t),
						sizeof(audproc_olm_open_t)
				};
				...
				err_code = m->module_ptr->vtbl_ptr->set_param(m->module_ptr, CAPI_V2_PARAM_ID_OLM_OPEN, &port_info, &olm_open_config_payload);
				result = capi_v2_err_to_adsp_result(err_code);
				acc_result |= result;
				if(result==ADSP_EOK)
				{
				...
				{
					MSG_3(MSG_SSID_QDSP6, DBG_ERROR_PRIO, "P%hX audproc_svc: failed to OPEN OLM Module-Instance with ID 0x%lX-0x%lX .",
							topo_ptr->obj_id, m->module_id, m->instance_id);
				}
			}
		}
...
ADSPResult topo_set_run_to_olm (topo_struct *topo_ptr, elite_msg_any_t* pMsg)
{
	ADSPResult result = ADSP_EOK, acc_result = ADSP_EOK;
	capi_v2_err_t err_code = CAPI_V2_EOK;
	int32_t olm_module_index = 0;
	...
	MSG_1(MSG_SSID_QDSP6, DBG_ERROR_PRIO, "P%hX audproc_svc: Issue the RUN Command to OLM Modules .",topo_ptr->obj_id);
	...
	if(topo_ptr->is_olm_present_in_topo)
	{
		...
		{
			MSG_1(MSG_SSID_QDSP6, DBG_ERROR_PRIO, "P%hX audproc_svc: failed to allocate memory for response Handler "
					"in the RUN Command to OLM Modules .",topo_ptr->obj_id);

			return result;
		}

		capi_v2_buf_t buf;
		capi_v2_olm_run_t run_cfg;

		run_cfg.token = token;
		run_cfg.version = 0;
		run_cfg.payload_size = 0;

		buf.data_ptr = (int8_t *) (&run_cfg);
		buf.max_data_len = sizeof(capi_v2_olm_run_t);
		buf.actual_data_len = sizeof(capi_v2_olm_run_t);
		...
		for(uint8_t i = 0; i < topo_ptr->graph.num_modules; i++)
		{
			if(topo_ptr->module[i].is_olm_module)
			{
				...
				err_code = topo_ptr->module[i].module_ptr->vtbl_ptr->set_param(topo_ptr->module[i].module_ptr,
				...
				{
					MSG_3(MSG_SSID_QDSP6, DBG_ERROR_PRIO, "P%hX audproc_svc: failed to Set RUN Command to OLM Module-Instance with ID 0x%lX-0x%lX .",
							topo_ptr->obj_id, topo_ptr->module[i].module_id, topo_ptr->module[i].instance_id);
				}
			}
		}
...
ADSPResult topo_set_suspend_to_olm (topo_struct *topo_ptr, elite_msg_any_t* pMsg)
{
	ADSPResult result = ADSP_EOK, acc_result = ADSP_EOK;
	int32_t olm_module_index = 0;
	uint32_t token =0;

	MSG_1(MSG_SSID_QDSP6, DBG_ERROR_PRIO, "P%hX audproc_svc: Issue the SUSPEND Command to OLM Modules .",topo_ptr->obj_id);
	...
	if(topo_ptr->is_olm_present_in_topo)
	{
		...
		{
			MSG_1(MSG_SSID_QDSP6, DBG_ERROR_PRIO, "P%hX audproc_svc: failed to allocate memory for response Handler "
					"in the Suspend Command to OLM Modules .",topo_ptr->obj_id);

			return result;
		}

		capi_v2_buf_t buf;
		capi_v2_olm_suspend_t suspend_cfg;

		suspend_cfg.token = token;
		suspend_cfg.version = 0;
		suspend_cfg.payload_size = 0;

		buf.data_ptr = (int8_t *) (&suspend_cfg);
		buf.max_data_len = sizeof(capi_v2_olm_suspend_t);
		buf.actual_data_len = sizeof(capi_v2_olm_suspend_t);
		...
		for(uint8_t i = 0; i < topo_ptr->graph.num_modules; i++)
		{
			if(topo_ptr->module[i].is_olm_module)
			{
				...
				topo_ptr->module[i].module_ptr->vtbl_ptr->set_param(topo_ptr->module[i].module_ptr,
				...
				{
					MSG_3(MSG_SSID_QDSP6, DBG_ERROR_PRIO, "P%hX audproc_svc: failed to Set SUSPEND Command to OLM Module-Instance with ID 0x%lX-0x%lX .",
							topo_ptr->obj_id, topo_ptr->module[i].module_id, topo_ptr->module[i].instance_id);
				}
			}
		}
...
ADSPResult topo_destroy_olm_modules(uint32_t num_modules, topo_struct *topo_ptr, uint32_t *resp_index)
{
	...
	{
		result = topo_create_command_olm_response_handler( &topo_ptr->olm_response_handler , &token);
		topo_set_wait_for_close_response(topo_ptr, TRUE);
		if(result==ADSP_ENOMEMORY)
		{
			MSG_1(MSG_SSID_QDSP6, DBG_ERROR_PRIO, "P%hX audproc_svc: failed to allocate memory for response Handler "
					"in the destroy command to OLM Modules .",topo_ptr->obj_id);
			return result;
		}
		struct audproc_olm_close_t
		{
			asm_stream_param_data_v3_t header;
			capi_v2_olm_close_t close_cfg;
		}payload;
		...
		for(uint8_t i = 0; i < num_modules; i++)
		{
			if(topo_ptr->module[i].is_olm_module)
			{
				payload.header.module_id   = topo_ptr->module[i].module_id;
				payload.header.instance_id = topo_ptr->module[i].instance_id;
				payload.header.param_id    = CAPI_V2_PARAM_ID_OLM_CLOSE;
				payload.header.param_size  = sizeof(capi_v2_olm_run_t);
				payload.header.reserved    = 0;
				payload.close_cfg.token      = token;
				...
				result = topo_set_param(topo_ptr, &payload, sizeof(payload), GIVEN_INSTANCE, NULL);
				acc_result |= result;
				if(result==ADSP_EOK)
				{
					olm_module_index++;
				...
				{
					MSG_3(MSG_SSID_QDSP6, DBG_ERROR_PRIO, "P%hX audproc_svc: failed to Set destroy command to OLM Module-Instance with ID 0x%lX-0x%lX .",
							topo_ptr->obj_id, topo_ptr->module[i].module_id, topo_ptr->module[i].instance_id);
				}
			}
		}
...
ADSPResult topo_olm_set_flush_command (topo_struct *topo_ptr, elite_msg_any_t* pMsg)
{
	...
	MSG_1(MSG_SSID_QDSP6, DBG_ERROR_PRIO, "P%hX audproc_svc: Issue the FLUSH/RESET Command to OLM Modules .",topo_ptr->obj_id);
	...
	if(topo_ptr->is_olm_present_in_topo)
	{
		...
		{
			MSG_1(MSG_SSID_QDSP6, DBG_ERROR_PRIO, "P%hX audproc_svc: failed to allocate memory for response Handler "
					"in the FLUSH/RESET Command to OLM Modules .",topo_ptr->obj_id);

			return result;
		}
		capi_v2_err_t err_code = CAPI_V2_EOK;
		capi_v2_buf_t buf;
		capi_v2_olm_flush_t olm_flush_config;
		olm_flush_config.token = token;
		buf.data_ptr = (int8_t *) (&olm_flush_config);
		buf.max_data_len = sizeof(capi_v2_olm_flush_t);
		buf.actual_data_len = sizeof(capi_v2_olm_flush_t);
		capi_v2_port_info_t port_info;
		port_info.is_valid = FALSE;
		...
		for(uint8_t i = 0; i < topo_ptr->graph.num_modules; i++)
		{
			if(topo_ptr->module[i].is_olm_module)
			{
				...
				err_code = module_ptr->vtbl_ptr->set_param(module_ptr, CAPI_V2_PARAM_ID_OLM_FLUSH, &port_info, &buf);
				result |= capi_v2_err_to_adsp_result(err_code);
				acc_result |= result;
				if(result==ADSP_EOK)
				{
				...
				{
					MSG_3(MSG_SSID_QDSP6, DBG_ERROR_PRIO, "P%hX audproc_svc: failed to Set FLUSH/RESET Command to OLM Module-Instance with ID 0x%lX-0x%lX .",
							topo_ptr->obj_id, topo_ptr->module[i].module_id, topo_ptr->module[i].instance_id);
				}
			}
		}
...
ADSPResult topo_handle_olm_inf_extension(module_info  *module_info_ptr, uint8_t *inf_extn_info_ptr , int32_t payload_size )
{
	ADSPResult result = ADSP_EOK;

	struct inf_extn_info_fmt
	{
		uint32_t inf_extn_id;
		uint32_t inf_extn_payload_size;
	};

	uint32_t inf_extn_id = 0;
	uint32_t inf_extn_payload_size = 0;
	uint8_t *payload_ptr = NULL;

	while(payload_size > 0)
	{
		inf_extn_info_fmt *data_ptr = (inf_extn_info_fmt *)inf_extn_info_ptr;
		inf_extn_id = data_ptr->inf_extn_id;
		inf_extn_payload_size = data_ptr->inf_extn_payload_size;
		if(inf_extn_payload_size!=0)
		{
			payload_ptr = (uint8_t *)(data_ptr + 1);
		}
		else
		{
			payload_ptr = NULL;
		...
		if((payload_size >= (inf_extn_payload_size +  sizeof(inf_extn_info_fmt))) && (payload_ptr != NULL))
		{
			switch(inf_extn_id)
			{
			case INTF_EXTN_CHG_MF:
			{
				intf_extn_chg_mf_t *payload = (intf_extn_chg_mf_t *)payload_ptr;
				if(inf_extn_payload_size <= sizeof(intf_extn_chg_mf_t))
				{
					module_info_ptr->media_format_change_info.changes_media_format = payload->bits_per_sample || payload->channel_info || payload->sampling_rate;
					module_info_ptr->media_format_change_info.version = payload->version;
				}
				...
				{
					MSG_2(MSG_SSID_QDSP6, DBG_ERROR_PRIO,"audproc_svc : Invalid Payload Size in OLM Interface extn handling"
							" intf_extn 0x%lx for module 0x%lx", inf_extn_id,  module_info_ptr->module_id);
				}
				break;
			}
			default:
			{
				MSG_2(MSG_SSID_QDSP6, DBG_ERROR_PRIO,"audproc_svc : unsupported intf_extn 0x%lx for module 0x%lx", inf_extn_id,  module_info_ptr->module_id);
				break;
			}
			}
			inf_extn_info_ptr = inf_extn_info_ptr + inf_extn_payload_size + sizeof(inf_extn_info_fmt);
			payload_size  -= (inf_extn_payload_size +  sizeof(inf_extn_info_fmt));
		}
		else
		{
			break;
...
ADSPResult topo_handle_olm_fwk_extension(topo_struct *topo_ptr, module_info  *module_info_ptr, uint8_t *fwk_extn_info_ptr , uint32_t num_fwk_extension, int32_t payloadsize)
{
	ADSPResult result = ADSP_EOK;

	struct fwk_extn_info_fmt
	{
	...
	uint32_t fwk_extn_id = 0;
	uint32_t fwk_extn_payload_size = 0;
	uint8_t *payload_ptr = NULL;
	uint32_t num_cnt_fwk_extn = 0;

	if(num_fwk_extension)
	{
		module_info_ptr->olm_info->fwk_extn_ptr =
				(capi_v2_framework_extension_id_t *)qurt_elite_memory_malloc(sizeof(capi_v2_framework_extension_id_t) * num_fwk_extension, QURT_ELITE_HEAP_DEFAULT);
		if (module_info_ptr->olm_info->fwk_extn_ptr == NULL)
		{
			MSG_1(MSG_SSID_QDSP6, DBG_FATAL_PRIO, "P%hX audproc_svc: "
					"Failed to allocate memory to store OLM supported FWK Extn IDs", topo_ptr->obj_id  );
			result = ADSP_ENOMEMORY;
			return result;
		}
		...
		while(payloadsize > 0)
		{
			fwk_extn_info_fmt *data_ptr = (fwk_extn_info_fmt *)fwk_extn_info_ptr;
			fwk_extn_id = data_ptr->fwk_extn_id;
			fwk_extn_payload_size = data_ptr->fwk_extn_payload_size;
			if(fwk_extn_payload_size)
			{
				payload_ptr = fwk_extn_info_ptr + sizeof(fwk_extn_info_fmt);
			}
			else
			{
				payload_ptr = NULL;
			...
			if((payloadsize >= (fwk_extn_payload_size + sizeof(fwk_extn_info_fmt))) && (payload_ptr != NULL))
			{
				switch(fwk_extn_id)
				{
				case FWK_EXTN_ECNS:
				{
					module_info_ptr->olm_info->fwk_extn_ptr[num_cnt_fwk_extn].id = fwk_extn_id;
					num_cnt_fwk_extn++;
					topo_ptr->ec_extn_info = (topo_ec_fwk_extn_t *)qurt_elite_memory_malloc(sizeof(topo_ec_fwk_extn_t),QURT_ELITE_HEAP_DEFAULT);
					if (topo_ptr->ec_extn_info == NULL)
					{
						MSG_1(MSG_SSID_QDSP6, DBG_FATAL_PRIO, "P%hX audproc_svc: "
								"Failed to allocate memory for the EC extension in the OLM FWK extn handling", topo_ptr->obj_id  );
						result = ADSP_ENOMEMORY;
						return result;
					}
					else
					{
						topo_ptr->ec_extn_info->instance_id = module_info_ptr->instance_id;
						topo_ptr->ec_extn_info->module_id = module_info_ptr->module_id;
						(void)memscpy(&topo_ptr->ec_extn_info->ec_capabilities,
								sizeof(ec_capabilities_t),
								payload_ptr,
								fwk_extn_payload_size);
					}
					//TODO : handle the EC extn payload
					break;
				}
				default:
				{
					MSG_2(MSG_SSID_QDSP6, DBG_ERROR_PRIO,"audproc_svc : unsupported fwk_extn 0x%lx for module 0x%lx", fwk_extn_id,  module_info_ptr->module_id);
					break;
				}
				}
				fwk_extn_info_ptr = fwk_extn_info_ptr + fwk_extn_payload_size  + sizeof(fwk_extn_info_fmt);
				payloadsize  -= (fwk_extn_payload_size + sizeof(fwk_extn_info_fmt));
			}
			else
			{
				break;
		...
		module_info_ptr->olm_info->num_fwk_extns = num_cnt_fwk_extn;
...
ADSPResult topo_update_olm_active_topo_definition(void *context_ptr , void *open_rsp_topo_def_info, uint16_t olm_iid)
{
	ADSPResult result = ADSP_EOK;
	topo_struct *topo_ptr = (topo_struct *)context_ptr;
	uint32_t num_modules = topo_graph_num_modules(&topo_ptr->graph);
	capi_v2_olm_active_topo_definition_t *olm_active_topo_def_ptr = (capi_v2_olm_active_topo_definition_t *)open_rsp_topo_def_info;
	for(uint32_t i = 0; i < num_modules; i++)
	{
		if(topo_ptr->module[i].is_olm_module)
		{
			if(topo_ptr->module[i].olm_info->olm_iid == olm_iid)
			{
				if(olm_active_topo_def_ptr->active_module_list!=NULL)
				{
					topo_ptr->module[i].olm_info->num_active_topology_modules = olm_active_topo_def_ptr->num_active_modules;
					uint32_t *ptr = (uint32_t*)qurt_elite_memory_malloc(olm_active_topo_def_ptr->payload_size, QURT_ELITE_HEAP_DEFAULT);
					if(ptr==NULL)
					{
						MSG_1(MSG_SSID_QDSP6, DBG_ERROR_PRIO, "audproc_svc: "
								"Failed to create memory to store the remote "
								"active topology definition for OLM IID %lu", olm_iid);
						result = ADSP_ENOMEMORY;
					}
					else
					{
						memscpy((uint8_t*)(ptr),
								olm_active_topo_def_ptr->payload_size,
								olm_active_topo_def_ptr->active_module_list,
								olm_active_topo_def_ptr->payload_size);
						topo_ptr->module[i].olm_info->active_topo_def = (avcs_module_instance_info_t*)ptr;
					}
				}
				else
				{
					topo_ptr->module[i].olm_info->num_active_topology_modules = 0;
				}
				break;
			}
		}
...
void topo_handle_olm_fwk_inf_extension(void *context_ptr )
{
	ADSPResult result = ADSP_EOK;
	topo_struct *topo_ptr = (topo_struct *)context_ptr;
	uint32_t num_modules = topo_graph_num_modules(&topo_ptr->graph);
	...
	for(uint32_t i = 0; i < num_modules; i++)
	{
		capi_v2_olm_get_fwk_inf_extn_info_t olm_fwk_inf_extn_info;

		memset((void*) &olm_fwk_inf_extn_info, 0, sizeof(capi_v2_olm_get_fwk_inf_extn_info_t) );

		if(topo_ptr->module[i].is_olm_module)
		{
			capi_v2_err_t err_code;
			capi_v2_buf_t buf;
			buf.data_ptr = (int8_t *) &olm_fwk_inf_extn_info;
			buf.max_data_len = sizeof(capi_v2_olm_get_fwk_inf_extn_info_t);
			capi_v2_t *module_ptr = topo_ptr->module[i].module_ptr;
			capi_v2_port_info_t port_info;
			port_info.is_valid = FALSE;
			err_code = module_ptr->vtbl_ptr->get_param(module_ptr, CAPI_V2_PARAM_ID_OLM_GET_FWK_INF_EXTN_INFO, &port_info, &buf);
			if (CAPI_V2_FAILED(err_code))
			{
				MSG_3(MSG_SSID_QDSP6, DBG_ERROR_PRIO, "P%hX audproc_svc: "
						"Module with ID 0x%lX-0x%lX failed to get OLM .", topo_ptr->obj_id,
						topo_ptr->module[i].module_id, topo_ptr->module[i].instance_id);
			}

			if((olm_fwk_inf_extn_info.data_ptr!=NULL) && (buf.actual_data_len > 0))
			{
				uint8_t *ptr = (uint8_t*)olm_fwk_inf_extn_info.data_ptr;
				topo_olm_extn_info_t *pextn_info = (topo_olm_extn_info_t*)ptr;
				uint8_t *inf_extn_info_ptr =  ptr + sizeof(topo_olm_extn_info_t);
				uint8_t *fwk_extn_info_ptr =  inf_extn_info_ptr + pextn_info->inf_extn_payload_size;
				result |= topo_handle_olm_inf_extension(&topo_ptr->module[i], inf_extn_info_ptr , pextn_info->inf_extn_payload_size );
				result |= topo_handle_olm_fwk_extension(topo_ptr, &topo_ptr->module[i], fwk_extn_info_ptr , pextn_info->num_fwk_extn, pextn_info->fwk_extn_payload_size );
			}
			else
			{
				MSG_3(MSG_SSID_QDSP6, DBG_ERROR_PRIO, "P%hX audproc_svc: "
										"OLM Module ID 0x%lX-0x%lX failed to get FWK/INF extension info .", topo_ptr->obj_id,
										topo_ptr->module[i].module_id, topo_ptr->module[i].instance_id);
			}
		}
	}
...
ADSPResult topo_update_run_state(void *topo_obj_ptr , uint32_t olm_run_response_result)
{
	ADSPResult error_code = ADSP_EOK;
	topo_struct *topo_ptr = (topo_struct *)topo_obj_ptr;
	topo_ptr->event_client->vtbl->update_run_state(topo_ptr->event_client, olm_run_response_result);
	return error_code;
}
...
{
	ADSPResult error_code = ADSP_EOK;
	error_code = topo_olm_set_client_command_handling_done(&topo_ptr->olm_response_handler, token, result);
	return error_code;
}

void topo_set_wait_for_open_response(topo_struct *topo_ptr, bool_t val)
{
	topo_ptr->olm_response_handler.wait_for_open_response = (uint32_t) val;
}

bool_t topo_get_wait_for_open_response(topo_struct *topo_ptr)
{
	bool_t val = (topo_ptr->olm_response_handler.wait_for_open_response >0 ) ? 1: 0;
	return val;
}

bool_t topo_check_if_wait_for_open_response(topo_struct *topo_ptr)
{
	bool_t status = FALSE;
    status = (topo_ptr->olm_response_handler.wait_for_open_response > 0) ? 1 : 0;
	return status;
}

bool_t topo_check_if_wait_for_close_response(topo_struct *topo_ptr)
{
	bool_t status = FALSE;
    status = (topo_ptr->olm_response_handler.wait_for_close_response > 0) ? 1 : 0;
	return status;
}

void topo_set_wait_for_close_response(topo_struct *topo_ptr, bool_t val)
{
	topo_ptr->olm_response_handler.wait_for_close_response = (uint32_t) val;
}

bool_t topo_get_wait_for_close_response(topo_struct *topo_ptr)
{
	bool_t val = (topo_ptr->olm_response_handler.wait_for_close_response >0 ) ? 1: 0;
	return val;
}
