...
Copyright (c) 2010-2017,2018 Qualcomm Technologies, Inc.  All Rights Reserved.
...
#define AUDDYNAPPSVC_CMDRSP_Q_NAME     "DPSr00"

static const uint32_t SVC_LAYER_STACK_SIZE = 1280;
...
static int AudPP_ProcessRemoteResponse(void* pInstance, elite_msg_any_t* pMsg);
//static ADSPResult AudPP_ProcessRespQhandler(void *pInstance);
...
static elite_svc_msg_handler_func AudPP_ProcessRespQHandler[]=
{
		AudPP_ProcessRemoteResponse
};
...
ADSPResult AudPP_CreateSvc (const AudPPSvcInitParams_t *pInitParams, void **handle, void **far_handle)
{
   ...
   result = AudPP_register_avtimer(me);
   if (ADSP_FAILED(result))
   {
      MSG_2(MSG_SSID_QDSP6, DBG_FATAL_PRIO, "P%hX audproc_svc: Fail to register timer %d!", me->objId, result);
      AudPP_DestroySvc(me);
      return result;
   }
...
void AudPP_DestroySvc (ThisAudDynaPPSvc_t* me)
{
   ...
   if (me)
   {
      ...
      qurt_elite_channel_destroy(&me->channel);
   	  ...
   	  ADSPResult result = ADSP_EOK;
   	  result |= AudPP_deregister_avtimer(me);
...
ADSPResult AudPP_ProcessRespQhandler(void *pInstance)
{
	...
	for (;;)
	{
		...
		result = qurt_elite_queue_pop_front(me->cmdRspQ, (uint64_t*) &msg);
		//const uint32_t resphandler_table_size = sizeof(AudPP_ProcessRespQHandler)/sizeof(AudPP_ProcessRespQHandler[0]);
		// Process the msg
		if (ADSP_EOK == result)
		{
			/*if (msg.unOpCode >= resphandler_table_size)
			{
			...
			if (ADSP_FAILED(result = AudPP_ProcessRespQHandler[0](pInstance, &msg)))
			{
				// in case of errors, return them
				return result;
			}
...
static int AudPP_WorkLoop(void* pInstance)
{
   ...
   pPPStat->unCommandResponseOnlyBitField = qurt_elite_queue_get_channel_bit(me->cmdRspQ);
   pPPStat->unBufBitfield = qurt_elite_queue_get_channel_bit(me->in_port[AUDPP_PRIMARY_IN_PORT]->handle.cmdQ) | qurt_elite_queue_get_channel_bit(me->bufQ)|qurt_elite_queue_get_channel_bit(me->cmdRspQ);
   ...
   for (;;)
   {
      ...
      if(topo_bg_is_bg_thread_processing(me->bg_th_handle))
      {
         ...
         pPPStat->unCurrentBitfield &= (~pPPStat->unCommandResponseOnlyBitField);
      ...
      //MSG_1(MSG_SSID_QDSP6, DBG_HIGH_PRIO, "P%hX audproc_svc: Background thread is not processing currently, POP the cmd ", me->objId);
	   if (unChannelStatus
            & qurt_elite_queue_get_channel_bit(me->cmdRspQ))
      {
         // call common utility function to process command queue in standard way.
         result = AudPP_ProcessRespQhandler(pInstance);
...
static int AudPP_Flush (void* pInstance, elite_msg_any_t* pMsg)
{
   ...
   if (ADSP_SUCCEEDED(result))
   {
	   result = topo_reset(me->pTopologyObj,pMsg);
   }
   //incr log-id after flush
   me->ulDataLogId++;

   // Ack back to sender
   if((!topo_check_if_wait_for_remote_response(me->pTopologyObj)))
   {
	   elite_svc_send_ack(pMsg, result);
   ...
   {
	   MSG_1(MSG_SSID_QDSP6, DBG_HIGH_PRIO, "P%hX audproc_svc: Flush End, pending ack", me->objId);
   }
...
static int AudPP_SetParam(void* pInstance, elite_msg_any_t* pMsg)
{
   ...
   case ELITEMSG_PARAM_ID_CAL_V2:
   {
      ...
      topo_olm_reset_state_for_remote_response(me->pTopologyObj);
      result = topo_set_param(me->pTopologyObj, pParamMsg->pnParamData, pParamMsg->unSize, mode,pMsg);
...
static int AudPP_GetParam(void* pInstance, elite_msg_any_t* pMsg)
{
   ADSPResult result;
   ThisAudDynaPPSvc_t *me = (ThisAudDynaPPSvc_t*)pInstance;
   topo_set_get_param_mode_t mode = GIVEN_INSTANCE;
   ...
   if((result!=ADSP_EOK) || (!topo_check_if_wait_for_remote_response(me->pTopologyObj)))
   {
	   elite_svc_send_ack(pMsg, result);
...
static int AudPP_Suspend(void* pInstance, elite_msg_any_t* pMsg)
{
   ...
   if (DYNA_SVC_PP_STATE_PAUSE == me->audPPStatus.dynaPPSvcState)
   {
	   ...
	   uint32_t wait_to_send_ack =0;
	   result = AudPP_Send_Command_to_Remote(me, CAPI_V2_PARAM_ID_OLM_SUSPEND, pMsg, &wait_to_send_ack );
	   if( result != ADSP_EOK)
	   {
		   MSG_1(MSG_SSID_QDSP6, DBG_HIGH_PRIO,"P%hX audproc_svc: Failed to send Suspend command to Remote",me->objId);
		   /** release, force_vote doesn't matter.*/
		   result |= AudPP_ProcessKppsBw(me, TRUE, FALSE);
		   elite_svc_send_ack(pMsg, result);
		   //return result;
	   }
	   else if(wait_to_send_ack==0)
	   {
		   /** release, force_vote doesn't matter.*/
		   result |= AudPP_ProcessKppsBw(me, TRUE, FALSE);
		   elite_svc_send_ack(pMsg, result);
...
static int AudPP_Run(void* pInstance, elite_msg_any_t* pMsg)
{
   ...
   if( result != ADSP_EOK)
   {
	   ...
	   goto __send_failure_ack;
   }

   result = AudPP_Send_Command_to_Remote(me, CAPI_V2_PARAM_ID_OLM_RUN, pMsg, &wait_to_send_ack);
   if( result != ADSP_EOK)
   {
	   MSG_1(MSG_SSID_QDSP6, DBG_HIGH_PRIO,"P%hX audproc_svc: Failed to send Run to Remote",me->objId);
	   goto __send_failure_ack;
   }
   ...
   if(wait_to_send_ack==0)
   {
	   AudPP_Set_Run_State(me ,TRUE);
	   elite_svc_send_ack(pMsg, result);
	   MSG_1(MSG_SSID_QDSP6, DBG_HIGH_PRIO, "P%hX audproc_svc: Run End, sending ack", me->objId);
   }
   topo_olm_reset_state_for_remote_response(me->pTopologyObj);
   return ADSP_EOK;

 __send_failure_ack :
 AudPP_Set_Run_State(me ,FALSE);
 elite_svc_send_ack(pMsg, result);
 MSG_1(MSG_SSID_QDSP6, DBG_HIGH_PRIO, "P%hX audproc_svc: Run End, sending failure ack", me->objId);
 return result;
...
static int AudPP_ProcessRemoteResponse(void* pInstance, elite_msg_any_t* pMsg)
{
	...
	if (AudPP_isInFatalState(me))
	{
		MSG_1( MSG_SSID_QDSP6, DBG_FATAL_PRIO,"P%hX audproc_svc: PP is in FATAL error state, cannot process responses.", me->objId);
		elite_svc_send_ack(pMsg, ADSP_EFAILED);
		return ADSP_EFAILED;
	}
	elite_apr_packet_t *pPkt = (elite_apr_packet_t *)pMsg->pPayload;
	uint16_t unPortId = ((uint16 ) elite_apr_if_get_dst_port(pPkt));
	payload_size = elite_apr_if_get_payload_size( pPkt);
	result = topo_get_olm_mid_given_iid(me->pTopologyObj, unPortId, &module_id);

	if(result == ADSP_EOK)
	{
		struct audproc_olm_remote_response_pkt_t
		{
		...
		payload.header.instance_id = unPortId;
		payload.header.param_id    = CAPI_V2_PARAM_ID_OLM_RESP_PKT;
		payload.header.param_size  = sizeof(payload)-sizeof(payload.header);
		payload.header.reserved    = 0;
		payload.olm_resp.pkt_ptr = pPkt;
		topo_set_get_param_mode_t mode =  GIVEN_INSTANCE;
		result = topo_set_param(me->pTopologyObj, &payload, sizeof(payload), mode,NULL);
	}
...
static ADSPResult AudPP_CreateSvcQueues(ThisAudDynaPPSvc_t* me, const int count)
{
   ...
   snprintf(me->cmdRspQName,  QURT_ELITE_DEFAULT_NAME_LEN, "%s%x%x", AUDDYNAPPSVC_CMDRSP_Q_NAME, (unsigned int)AUDPP_PRIMARY_IN_PORT, count);
   ...
   if (ADSP_FAILED(result = qurt_elite_queue_create(me->cmdQName, MAX_CMD_Q_ELEMENTS, &(me->in_port[AUDPP_PRIMARY_IN_PORT]->handle.cmdQ)))
		 || ADSP_FAILED(result = qurt_elite_queue_create(me->cmdRspQName, MAX_CMDRSP_Q_ELEMENTS, &(me->cmdRspQ)))
   ...
   if ( ADSP_FAILED(result = qurt_elite_channel_addq(&me->channel, me->in_port[AUDPP_PRIMARY_IN_PORT]->handle.cmdQ,0))
         ...
         || ADSP_FAILED(result = qurt_elite_channel_addq(&me->channel, me->cmdRspQ,0)))
   {
   ...
   me->audPPStatus.unCommandResponseOnlyBitField = qurt_elite_queue_get_channel_bit(me->cmdRspQ);
...
static void AudPP_DestroySvcQueues(ThisAudDynaPPSvc_t* me)
{
   ...
   // call utility function to destroy Response cmd Q
   elite_svc_destroy_cmd_queue(me->cmdRspQ);
...
static ADSPResult AudPP_InitializeCommonTopology(ThisAudDynaPPSvc_t* me, uint32_t* stack_size_ptr)
{
   ADSPResult result = ADSP_EOK,err_code = ADSP_EOK;
   uint32_t topology_id = me->ppCfgInfo.dynaPPSvcTopologyID;
   AudPP_AudProcType pp_type = me->ppCfgInfo.dynaPPSvcType;
   ...
   size_req = sizeof(elite_cmn_topo_db_entry_t) + ((num_modules)*sizeof(avcs_module_instance_proc_info_t));
   ...
   else if(topo_check_if_wait_for_open_response(me->pTopologyObj))
   {
	   me->audPPStatus.unCurrentBitfield = me->audPPStatus.unCommandResponseOnlyBitField;
	   topo_set_wait_for_open_response(me->pTopologyObj, TRUE);
	   while((me->audPPStatus.unCurrentBitfield) && (topo_get_wait_for_open_response(me->pTopologyObj)))
	   {
		   uint32_t unChannelStatus = qurt_elite_channel_wait(&(me->channel), me->audPPStatus.unCurrentBitfield);
		   if (unChannelStatus
				   & qurt_elite_queue_get_channel_bit(me->cmdRspQ))
		   {
			   // call common utility function to process Response queue in standard way.
			   err_code = AudPP_ProcessRespQhandler(me);
			   if (ADSP_FAILED(err_code))
			   {
				   MSG_2(MSG_SSID_QDSP6, DBG_ERROR_PRIO, "P%hX audproc_svc: Failed to send the OPEN RESPONSE of remote offloaded modules to OLM", me->objId, topology_id);
				   result|=err_code;
			   }
		   }
	   }
	   result|=topo_get_olm_open_response_result(me->pTopologyObj);
	   if (ADSP_FAILED(result))
	   {
		   MSG_2(MSG_SSID_QDSP6, DBG_ERROR_PRIO, "P%hX audproc_svc: Topology %lx, Creation of remote offloaded modules failed - PP open failed", me->objId, topology_id);
	   }
