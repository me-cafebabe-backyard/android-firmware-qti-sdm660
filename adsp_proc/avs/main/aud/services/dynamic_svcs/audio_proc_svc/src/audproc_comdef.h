...
static const uint32_t MAX_INPUT_CHANNELS = ELITE_PCM_MAX_NUM_CH;
static const uint32_t MAX_OUTPUT_CHANNELS = ELITE_PCM_MAX_NUM_CH;
...
//maximum number of commands responses expected ever in command queue.
static const uint32_t MAX_CMDRSP_Q_ELEMENTS = 32;
...
typedef struct _AudPPStatus_t
{
   ...
   uint32_t unCommandResponseOnlyBitField;
   // short-cut for setting pChannel to listen for Response only
...
typedef struct _ThisAudDynaPPSvc_t
{
   ...
   // command queue name
   char              cmdRspQName[QURT_ELITE_DEFAULT_NAME_LEN];
   // command queue name
   ...
   uint16_t	is_input_output_realtime;		/**< This variable is set by ASM/ADM to indicate if data
   ...
   void*   avt_drv_handle;           // handle of timer
} ThisAudDynaPPSvc_t;
...
ADSPResult AudPP_register_avtimer(ThisAudDynaPPSvc_t* me);
ADSPResult AudPP_deregister_avtimer(ThisAudDynaPPSvc_t* me);
...
ADSPResult AudPP_SuspendPrepare(ThisAudDynaPPSvc_t *me);
ADSPResult AudPP_RunPrepare(ThisAudDynaPPSvc_t *me);
...
ADSPResult AudPP_Send_Command_to_Remote (const ThisAudDynaPPSvc_t *me,
		uint32_t olm_fwk_command_id ,
		elite_msg_any_t* pMsg,
		uint32_t  *wait_for_send_ack);
/**
