...
$Header: //components/rel/avs.adsp/2.8/aud/services/dynamic_svcs/audio_enc_svc/inc/AudioEncSvc.h#11 $
...
struct asm_multi_channel_pcm_fmt_blk_v5_enc_t
{
   asm_multi_channel_pcm_fmt_blk_v5_t fmt;
   asm_multi_channel_pcm_fmt_ch_map_t channel_map[ELITE_PCM_MAX_NUM_CH];
};
...
struct AudioEncSvcPushModeParams_t
{
   union
   {
     asm_multi_channel_pcm_fmt_blk_v5_enc_t pcm;
   } EncCfg;
...
typedef struct elite_msg_custom_enc_set_out_media_fmt_t
{
    ...
    /** zero means no change is expected, input = output*/
    uint16_t channel_type[ELITE_PCM_MAX_NUM_CH];
    /** zero means no change is expected, input = output*/
