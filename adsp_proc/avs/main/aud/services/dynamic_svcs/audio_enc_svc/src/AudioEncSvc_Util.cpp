...
/*========================================================================
 ...
 $Header: //components/rel/avs.adsp/2.8/aud/services/dynamic_svcs/audio_enc_svc/src/AudioEncSvc_Util.cpp#26 $
...
static ADSPResult AudioEncSvc_UpdatePcmEncCfgv5(AudioEncSvc_t *pMe, void* pPcmConfig, bool_t isNativeModeAllowed);
static void AudioEncSvc_PrintChannelType(uint8_t *ch_map, int32_t num_channels);

void AudioEncSvc_InitMediaFmt(capi_v2_standard_data_format_v2_max_ch_t *std_media_fmt)
{
   std_media_fmt->fmt.minor_version = CAPI_V2_MEDIA_FORMAT_MINOR_VERSION;
   std_media_fmt->fmt.bits_per_sample = CAPI_V2_DATA_FORMAT_INVALID_VAL;
   std_media_fmt->fmt.bitstream_format = CAPI_V2_DATA_FORMAT_INVALID_VAL;
   std_media_fmt->fmt.data_interleaving = CAPI_V2_INVALID_INTERLEAVING;
   std_media_fmt->fmt.data_is_signed = CAPI_V2_DATA_FORMAT_INVALID_VAL;
   std_media_fmt->fmt.num_channels = CAPI_V2_DATA_FORMAT_INVALID_VAL;
   std_media_fmt->fmt.q_factor = CAPI_V2_DATA_FORMAT_INVALID_VAL;
   std_media_fmt->fmt.sampling_rate = CAPI_V2_DATA_FORMAT_INVALID_VAL;
...
void AudioEncSvc_InitMediaFmtv1(capi_v2_standard_data_format_t *std_media_fmt)
{
      ...
      AudioEncSvcInitParams_t *pInitParams, bool_t is_last_capi, enc_init_time_get_params_t *enc_params)
{
   ...
   {
       asm_multi_channel_pcm_fmt_blk_v4_t pPcmv4;
       void *pPcm_media_format;
       asm_multi_channel_pcm_fmt_blk_v5_enc_t *pPcm = &pInitParams->push_mode.EncCfg.pcm;
       if(pMe->mainCapiContainer->enc_cfg_id != ASM_MEDIA_FMT_MULTI_CHANNEL_PCM_V5)
       {
           pPcmv4.bits_per_sample = pPcm->fmt.bits_per_sample;
           pPcmv4.endianness = pPcm->fmt.endianness;
           pPcmv4.is_signed = pPcm->fmt.is_signed;
           pPcmv4.mode = pPcm->fmt.mode;
           pPcmv4.num_channels = pPcm->fmt.num_channels;
           pPcmv4.sample_rate = pPcm->fmt.sample_rate;
           pPcmv4.sample_word_size = pPcm->fmt.sample_word_size;

           memscpy(pPcmv4.channel_mapping, sizeof(pPcmv4.channel_mapping),
               pPcm->channel_map, pPcm->fmt.num_channels * sizeof(pPcm->channel_map[0]));
           pPcm_media_format = (void *)(&pPcmv4);
       }
       else
       {
           pPcm_media_format = (void *)(pPcm);
       }
      result =  AudioEncSvc_UpdatePcmEncCfg(pMe, pPcm_media_format, FALSE); //native mode not allowed
      ...
      unEncBufSizeBytes = AudioEncSvc_GetPcmOutBufSize(pInitParams->perf_mode,
            pInitParams->push_mode.EncCfg.pcm.fmt.sample_rate, output_bytes_per_sample, pInitParams->push_mode.EncCfg.pcm.fmt.num_channels);
   }
      ...
      AudioEncSvcInitParams_t* pInitParams, bool_t is_last_capi, enc_init_time_get_params_t *enc_params)
{
   ...
   case    ENC_CAPI_TYPE_PACKETIZER:
   {
      ...
      pMe->capiContainer[capi_index]->out_med_fmt.std.fmt.bitstream_format = capi_init_param.enc_cfg_id;
      pMe->capiContainer[capi_index]->out_med_fmt.std.fmt.bits_per_sample = pInitParams->bits_per_sample;
      break;
   }
   case  ENC_CAPI_TYPE_COP_PACKETIZER:
   {
      ...
      pMe->capiContainer[capi_index]->out_med_fmt.std.fmt.bitstream_format = capi_init_param.enc_cfg_id; //same bit stream as main CAPI
      break;
   }
   default:
   {
...
static void AudioEncSvc_PrintChannelType(uint8_t *ch_map, int32_t num_channels)
{
  ...
  QURT_ELITE_ALIGN(uint8_t ch_map_local[ELITE_PCM_MAX_NUM_CH], 4); // align by 4
  ...
  MSG_8(MSG_SSID_QDSP6, DBG_HIGH_PRIO,
        "PCM Enc cfg channel mapping (4 channel maps per value) 0x%lx,0x%lx,0x%lx,0x%lx,0x%lx,0x%lx,0x%lx,0x%lx",ch_map_word[0],
...
static ADSPResult AudioEncSvc_UpdatePcmEncCfgv5(AudioEncSvc_t *pMe, void* pPcmConfig, bool_t isNativeModeAllowed)
{
   asm_multi_channel_pcm_fmt_blk_v5_t* pPcmConfig_v5 = (asm_multi_channel_pcm_fmt_blk_v5_t*) ( pPcmConfig );
   asm_multi_channel_pcm_fmt_ch_map_t *ch_map;
   int32_t num_channels = (int32_t)pPcmConfig_v5->num_channels;

   ch_map = (asm_multi_channel_pcm_fmt_ch_map_t *)(pPcmConfig_v5 + 1);

   MSG_5(MSG_SSID_QDSP6, DBG_HIGH_PRIO, "PCM Enc Cfg 0x%lx. ch=%u,SR=%lu,bps=%u, native mode allowed %d", pMe->mainCapiContainer->enc_cfg_id,
       pPcmConfig_v5->num_channels, pPcmConfig_v5->sample_rate, pPcmConfig_v5->bits_per_sample, isNativeModeAllowed);

   if ((!pPcmConfig_v5->is_signed) || (pPcmConfig_v5->num_channels > 32)
         || ((!isNativeModeAllowed) && (pPcmConfig_v5->num_channels == 0)) || ((!isNativeModeAllowed) && (pPcmConfig_v5->sample_rate == 0)))
   {
      MSG(MSG_SSID_QDSP6, DBG_ERROR_PRIO, "Error:Received invalid parameters in PCM config block");
      return ADSP_EBADPARAM;
   }

   AudioEncSvc_PrintChannelType((uint8_t *)ch_map, num_channels);

   pMe->out_med_fmt_per_enc_cfg.endianness = ENC_LITTLE_ENDIAN;

   uint16_t pcmOutBytesPerSample;

   if(((16 == pPcmConfig_v5->bits_per_sample) || (24 == pPcmConfig_v5->bits_per_sample) || (32 == pPcmConfig_v5->bits_per_sample)) &&
         (pPcmConfig_v5->sample_word_size >= pPcmConfig_v5->bits_per_sample))
   {
      uint32_t bytesPerSample = pPcmConfig_v5->sample_word_size / 8;
      if ((bytesPerSample * 8 != pPcmConfig_v5->sample_word_size) || (bytesPerSample > 4))
      {
         MSG_1(MSG_SSID_QDSP6, DBG_ERROR_PRIO, "sample word size %u not byte aligned or is > 32.", pPcmConfig_v5->sample_word_size);
         return ADSP_EBADPARAM;
      }
      pcmOutBytesPerSample = bytesPerSample;

      if((pPcmConfig_v5->endianness != 0) && (pPcmConfig_v5->endianness != 1))
      {
         MSG_1(MSG_SSID_QDSP6, DBG_ERROR_PRIO, "endianness type %u not supported.", pPcmConfig_v5->endianness);
         return ADSP_EBADPARAM;
      }

      pMe->out_med_fmt_per_enc_cfg.endianness = (pPcmConfig_v5->endianness == 0) ? ENC_LITTLE_ENDIAN : ENC_BIG_ENDIAN;
      MSG_2(MSG_SSID_QDSP6, DBG_HIGH_PRIO, "Sample word size is %u, Endianness type is %u (0-little, 1-big)",
          pPcmConfig_v5->sample_word_size, pPcmConfig_v5->endianness);
   }
   else
   {
      MSG_2(MSG_SSID_QDSP6, DBG_ERROR_PRIO, "Error: received invalid bits per sample_[%lu], or sample word size [%lu].",
          pPcmConfig_v5->bits_per_sample, pPcmConfig_v5->sample_word_size);
      return ADSP_EBADPARAM;
   }

   pMe->out_med_fmt_per_enc_cfg.bits_per_sample = pPcmConfig_v5->bits_per_sample;
   pMe->out_med_fmt_per_enc_cfg.bytes_per_sample = pcmOutBytesPerSample;
   pMe->out_med_fmt_per_enc_cfg.num_channels = pPcmConfig_v5->num_channels;
   pMe->out_med_fmt_per_enc_cfg.sample_rate = pPcmConfig_v5->sample_rate;
   memscpy(pMe->out_med_fmt_per_enc_cfg.chan_map, sizeof(pMe->out_med_fmt_per_enc_cfg.chan_map),
       ch_map, pPcmConfig_v5->num_channels * sizeof(asm_multi_channel_pcm_fmt_ch_map_t));
...
ADSPResult AudioEncSvc_UpdatePcmEncCfg(AudioEncSvc_t *pMe, void* pPcmConfig, bool_t isNativeModeAllowed)
{
   ...
   if (ASM_MEDIA_FMT_MULTI_CHANNEL_PCM_V5 == main_capi->enc_cfg_id)
   {
       result = AudioEncSvc_UpdatePcmEncCfgv5(pMe, pPcmConfig, isNativeModeAllowed);
       return result;
   }
   ...
   AudioEncSvc_PrintChannelType(&pPcmConfig_v2->channel_mapping[0], pPcmConfig_v2->num_channels);
...
static void init_all_capi_out_media_fmt_at_enc_cfg(AudioEncSvc_t *pMe)
{
   for (uint8_t i = 0; i < ENC_SVC_MAX_CAPI; i++)
   {
      ...
      {
         if (0 == pMe->capiContainer[i]->out_med_fmt.std.fmt.bits_per_sample)
         {
            pMe->capiContainer[i]->out_med_fmt.std.fmt.bits_per_sample = pMe->out_med_fmt_per_enc_cfg.bits_per_sample;
         }

         if (0 == pMe->capiContainer[i]->out_med_fmt.std.fmt.num_channels)
         {
            pMe->capiContainer[i]->out_med_fmt.std.fmt.num_channels = pMe->out_med_fmt_per_enc_cfg.num_channels;
         ...
         if (0 == pMe->capiContainer[i]->out_med_fmt.std.fmt.sampling_rate)
         {
            pMe->capiContainer[i]->out_med_fmt.std.fmt.sampling_rate = pMe->out_med_fmt_per_enc_cfg.sample_rate;
         }

         pMe->capiContainer[i]->out_med_fmt.std.fmt.data_interleaving = CAPI_V2_DEINTERLEAVED_PACKED;
         pMe->capiContainer[i]->out_med_fmt.std.fmt.data_is_signed = 1;
         pMe->capiContainer[i]->out_med_fmt.std.fmt.q_factor = pMe->out_med_fmt_per_enc_cfg.bytes_per_sample == BYTES_PER_SAMPLE_TWO ? PCM_16BIT_Q_FORMAT: ELITE_32BIT_PCM_Q_FORMAT;
      }
   }
}
...
ADSPResult AudioEncSvc_UpdateEncConfig(AudioEncSvc_t *pMe, uint8_t *payload, uint32_t payload_size, uint32_t frames_per_buf)
{
   ...
   case ASM_MEDIA_FMT_MULTI_CHANNEL_PCM_V5:
   {
...
static void AudioEncSvc_CheckSendSrCmEvent(AudioEncSvc_t *pMe, enc_CAPI_container_t *capi_container, elite_multi_channel_pcm_fmt_blk_t *pInPcmFmtBlk)
{
   ...
   if (isNativeMode && (AudioEncSvc_IsPcmFmt(ulEncCfgId)) && pMe->CbData.pAsyncRespQ)
   {
      ...
      asm_data_event_enc_sr_cm_change_notify_v2_t SrCmEvent;
      uint32_t event_opcode = ASM_DATA_EVENT_ENC_SR_CM_CHANGE_NOTIFY;
      uint32_t payload_size = sizeof(asm_data_event_enc_sr_cm_change_notify_t);
      ...
      if(capi_container->enc_cfg_id == ASM_MEDIA_FMT_MULTI_CHANNEL_PCM_V5)
      {
          event_opcode = ASM_DATA_EVENT_ENC_SR_CM_CHANGE_NOTIFY_V2;
          payload_size = sizeof(SrCmEvent);
      }
      (void) AudioStreamMgr_GenerateClientEventFromCb(&pMe->CbData, event_opcode, pMe->CbData.unAsynClientToken, &SrCmEvent,
          payload_size);

      MSG(MSG_SSID_QDSP6, DBG_HIGH_PRIO, "Generated ASM_DATA_EVENT_ENC_SR_CM_CHANGE_NOTIFY/V2; \
...
ADSPResult AudioEncSvc_ProcessMediaTypeMsg(AudioEncSvc_t *pMe)
{
   ...
   audio_enc_svc_capi_v2_media_fmt_v1_t in_media_fmt; // TODO add v2 support for encoders
   ...
   if ( sr_cm_updated )
   {
      ...
      {
         //send media fmt to first CAPI // format v2 is supported only for PCM encoder as of now
   ...
   for (uint8_t i = 0; i < ENC_SVC_MAX_CAPI-1; i++)
   {
      if (pMe->capiContainer[i])
      {
         ...
         {
            aud_enc_svc_convert_v2_to_v1_media_type(&pMe->capiContainer[i]->out_med_fmt, &in_media_fmt);
            result = aud_enc_svc_set_media_fmt(pMe->capiContainer[i+1]->capi_ptr, &in_media_fmt, CAPI_V2_INPUT_MEDIA_FORMAT);
   ...
   {
      AudioEncSvc_NotifyPeerSvcWithMediaFmtUpdate(pMe, last_capi->out_med_fmt.std.fmt.sampling_rate, last_capi->out_med_fmt.std.fmt.num_channels,
            last_capi->out_med_fmt.std.channel_type, last_capi->out_med_fmt.std.fmt.bits_per_sample);
...
void aud_enc_svc_convert_v2_to_v1_media_type(audio_enc_svc_capi_v2_media_fmt_v2_t *v2, audio_enc_svc_capi_v2_media_fmt_v1_t *v1)
{
  v1->main.format_header.data_format = v2->main.format_header.data_format;
  v1->std.bits_per_sample = v2->std.fmt.bits_per_sample;
  v1->std.bitstream_format = v2->std.fmt.bitstream_format;
  v1->std.data_interleaving = v2->std.fmt.data_interleaving;
  v1->std.data_is_signed = v2->std.fmt.data_is_signed;
  v1->std.num_channels = v2->std.fmt.num_channels;
  v1->std.q_factor = v2->std.fmt.q_factor;
  v1->std.sampling_rate = v2->std.fmt.sampling_rate;

  memscpy(v1->std.channel_type, sizeof(v1->std.channel_type), v2->std.channel_type, v2->std.fmt.num_channels * sizeof(v2->std.channel_type[0]));
  return;
}
...
static void AudioEncSvc_UpdateInitialDelay(AudioEncSvc_t *pMe)
{
   ...
   if (pMe->delay_ptr)
   {
            ...
            (CAPI_V2_COMPR_OVER_PCM_PACKETIZED == last_capi->out_med_fmt.main.format_header.data_format))
      {
         ...
         uint32_t bytes_per_sample = (last_capi->out_med_fmt.std.fmt.bits_per_sample > 16) ? BYTES_PER_SAMPLE_FOUR : BYTES_PER_SAMPLE_TWO;
         uint32_t bytes_p_sec = (last_capi->out_med_fmt.std.fmt.sampling_rate * last_capi->out_med_fmt.std.fmt.num_channels * bytes_per_sample);
