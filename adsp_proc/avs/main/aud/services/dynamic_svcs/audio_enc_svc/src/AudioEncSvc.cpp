...
$Header: //components/rel/avs.adsp/2.8/aud/services/dynamic_svcs/audio_enc_svc/src/AudioEncSvc.cpp#14 $
...
static ADSPResult AudioEncSvc_Process(AudioEncSvc_t* pMe)
{
	...
	do
	{
		...
		if((pMe->endOfStream == TRUE) &&  (nReturnOutBuf == 0) && (!last_capi->in_buf_params.in_buf.actual_data_len))
		{
			MSG_1(MSG_SSID_QDSP6, DBG_LOW_PRIO, "nReturnOutBuf = %d",
					nReturnOutBuf);
...
static ADSPResult AudioEncSvc_SetOutMediaFmt(AudioEncSvc_t *pMe)
{
	...
	if (pPayload->num_channels)
	{
		...
		enc_CAPI_container_t *last_capi = AudioEncSvc_GetLastCapi(pMe);
		audio_enc_svc_capi_v2_media_fmt_v1_t media_fmt;  // TODO : check if this can come for PCM encoder as well
		media_fmt.main.format_header.data_format = last_capi->out_med_fmt.main.format_header.data_format;
		AudioEncSvc_InitMediaFmtv1(&media_fmt.std);
