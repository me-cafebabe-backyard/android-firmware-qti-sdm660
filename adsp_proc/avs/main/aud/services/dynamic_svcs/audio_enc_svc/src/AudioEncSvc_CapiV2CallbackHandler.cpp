...
$Header: //components/rel/avs.adsp/2.8/aud/services/dynamic_svcs/audio_enc_svc/src/AudioEncSvc_CapiV2CallbackHandler.cpp#8 $
...
static capi_v2_err_t audio_enc_svc_capi_v2_callback(void *context_ptr, capi_v2_event_id_t id, capi_v2_event_info_t *event_info_ptr)
{
   ...
   case CAPI_V2_EVENT_OUTPUT_MEDIA_FORMAT_UPDATED:
   {
      ...
      case CAPI_V2_COMPR_OVER_PCM_PACKETIZED:
      {
         ...
         if (capi_cont->out_med_fmt.std.fmt.bits_per_sample != pPcmData->bits_per_sample)
         {
            capi_cont->out_med_fmt.std.fmt.bits_per_sample = pPcmData->bits_per_sample;
            changed=true;
         }

         if (capi_cont->out_med_fmt.std.fmt.num_channels != pPcmData->num_channels)
         {
            capi_cont->out_med_fmt.std.fmt.num_channels = pPcmData->num_channels;
            changed=true;
         }

         if (capi_cont->out_med_fmt.std.fmt.sampling_rate != pPcmData->sampling_rate)
         {
            capi_cont->out_med_fmt.std.fmt.sampling_rate = pPcmData->sampling_rate;
            changed=true;
         }

         for (uint32_t i=0;((i < SIZEOFARRAY(capi_cont->out_med_fmt.std.channel_type) ) && (i < pPcmData->num_channels) && (i < SIZEOFARRAY(pPcmData->channel_type) )); i++)
         {
         ...
         if (capi_cont->out_med_fmt.std.fmt.data_interleaving != pPcmData->data_interleaving)
         {
            capi_cont->out_med_fmt.std.fmt.data_interleaving = pPcmData->data_interleaving;
            changed=true;
         }

         if (capi_cont->out_med_fmt.std.fmt.q_factor != pPcmData->q_factor)
         {
            capi_cont->out_med_fmt.std.fmt.q_factor = pPcmData->q_factor;
            changed=true;
         }

         if (capi_cont->out_med_fmt.std.fmt.data_is_signed != pPcmData->data_is_signed)
         {
            capi_cont->out_med_fmt.std.fmt.data_is_signed = pPcmData->data_is_signed;
            changed=true;
         }
