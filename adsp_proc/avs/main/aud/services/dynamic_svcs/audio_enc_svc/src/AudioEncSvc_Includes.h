...
$Header: //components/rel/avs.adsp/2.8/aud/services/dynamic_svcs/audio_enc_svc/src/AudioEncSvc_Includes.h#16 $
...
static const uint32_t MAX_NUM_CHANNELS = ELITE_PCM_MAX_NUM_CH;
...
typedef struct capi_v2_standard_data_format_v2_max_ch_t
{
  capi_v2_standard_data_format_v2_t fmt;
  capi_v2_channel_type_t channel_type[CAPI_V2_MAX_CHANNELS_V2];
}capi_v2_standard_data_format_v2_max_ch_t;

typedef struct audio_enc_svc_capi_v2_media_fmt_v2_t
{
   capi_v2_set_get_media_format_t main;
   union
   {
      capi_v2_standard_data_format_v2_max_ch_t std;
      capi_v2_raw_compressed_data_format_t raw_fmt;
   };
} audio_enc_svc_capi_v2_media_fmt_v2_t;

typedef struct audio_enc_svc_capi_v2_media_fmt_v1_t
{
   capi_v2_set_get_media_format_t main;
   union
   {
...
} audio_enc_svc_capi_v2_media_fmt_v1_t;
...
struct EncMediaFmt_t
{
   ...
   uint8_t        chan_map[CAPI_V2_MAX_CHANNELS_V2];
...
struct enc_CAPI_container_t
{
   ...
   audio_enc_svc_capi_v2_media_fmt_v2_t   out_med_fmt; //after CAPI raises out media fmt, it's stored here.
...
static inline bool_t AudioEncSvc_IsPcmFmt(uint32_t encCfgId)
{
   return ( (ASM_MEDIA_FMT_MULTI_CHANNEL_PCM_V2 == encCfgId) ||
        ...
        (ASM_MEDIA_FMT_MULTI_CHANNEL_PCM_V5 == encCfgId));
