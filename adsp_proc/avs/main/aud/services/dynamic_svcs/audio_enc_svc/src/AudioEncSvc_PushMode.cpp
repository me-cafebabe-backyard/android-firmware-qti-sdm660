...
ADSPResult AudioEncSvc_InitPushMode(AudioEncSvc_t *pMe, AudioEncSvcInitParams_t *pInitParams)
{
   ...
   if (pMe->push_mode_ptr->shared_circ_buf_size % unit_size)
   {
      MSG_3(MSG_SSID_QDSP6, DBG_HIGH_PRIO, "Circular buffer size %lu is not a multiple of (sample word size %u * channels %u) ",
            pMe->push_mode_ptr->shared_circ_buf_size, pPcm->fmt.sample_word_size, pPcm->fmt.num_channels);
      return ADSP_EFAILED;
   }
   ...
   if (ADSP_EOK != (result = qurt_elite_timer_open(&(pMe->push_mode_ptr->avt_drv_handle), (char*)"AEnc")))
   {
      MSG_1(MSG_SSID_QDSP6, DBG_ERROR_PRIO, "Enc Svc: Failed to Open timer Driver for input port with result: %d",result);
      return result;
   }
...
void AudioEncSvc_DeInitPushMode(AudioEncSvc_t *pMe)
{
   if (pMe->push_mode_ptr)
   {
      ...
      {
         ADSPResult result;
         if (ADSP_EOK != (result = qurt_elite_timer_close(pMe->push_mode_ptr->avt_drv_handle)))
         {
            MSG_1(MSG_SSID_QDSP6, DBG_ERROR_PRIO, "Enc Svc: Failed to Close timer Driver for input port with result: %d ",result);
         }
         pMe->push_mode_ptr->avt_drv_handle = NULL;
      }
...
ADSPResult AudioEncSvc_SuspendPushMode(AudioEncSvc_t *pMe)
{
       ...
       (NULL != pMe->push_mode_ptr->avt_drv_handle))
   {
      if (ADSP_EOK != (result = qurt_elite_timer_close(pMe->push_mode_ptr->avt_drv_handle)))
      {
         MSG_1(MSG_SSID_QDSP6, DBG_ERROR_PRIO, "AudioEncSvc :[SUSPEND] Failed to Close timer Driver with result: %d",result);
         return result;
      }
      else
      {
   ...
   {
      MSG(MSG_SSID_QDSP6, DBG_ERROR_PRIO, "AudioEncSvc :[SUSPEND] timer Driver handle is null");
   }
   return result;
}
...
ADSPResult AudioEncSvc_ResumePushMode(AudioEncSvc_t *pMe)
{
   ...
   if ((NULL != pMe->push_mode_ptr) &&
       (NULL == pMe->push_mode_ptr->avt_drv_handle))
   {
      /** Re-register timer handle after suspend */
      if (ADSP_EOK != (result = qurt_elite_timer_open(&(pMe->push_mode_ptr->avt_drv_handle), (char*)"ADec")))
      {
         MSG_1(MSG_SSID_QDSP6, DBG_ERROR_PRIO, "AudioEncSvc :[RESUME] Failed to Open timer Driver with result: %d",result);
         return result;
      }
   }
   return result;
...
ADSPResult AudioEncSvc_ProcessPCMPushMode(AudioEncSvc_t* pMe)
{
   ...
   uint64_t time = qurt_elite_fwk_get_time(pMe->push_mode_ptr->avt_drv_handle);
