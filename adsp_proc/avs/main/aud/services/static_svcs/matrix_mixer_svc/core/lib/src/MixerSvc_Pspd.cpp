...
$Header: //components/rel/avs.adsp/2.8/aud/services/static_svcs/matrix_mixer_svc/core/lib/src/MixerSvc_Pspd.cpp#9 $
...
static ADSPResult MtMx_PspdProcChMixerCfgMsg(MtMx_PspdStruct *pMePspd, pspd_chmixer_inout_t *param_ptr)
{
    ...
    capiv2_mediafmtv2_t outMediaFmt =
    {
            {
                    CAPI_V2_FIXED_POINT
            },
    ...
    {
        outMediaFmt.channel_type[i] = param_ptr->out_ch_map[i];
    }
    ...
    uint32_t actual_payload_len_in, actual_payload_len_out;
    actual_payload_len_in = sizeof(InpMediaFmt.h) + sizeof(InpMediaFmt.f) + InpMediaFmt.f.num_channels * sizeof(InpMediaFmt.channel_type[0]);
    actual_payload_len_in = ELITE_ALIGN_BY_4(actual_payload_len_in);
    actual_payload_len_out = sizeof(outMediaFmt.h) + sizeof(outMediaFmt.f) + outMediaFmt.f.num_channels * sizeof(outMediaFmt.channel_type[0]);
    actual_payload_len_out = ELITE_ALIGN_BY_4(actual_payload_len_out);

    for(uint16_t i = 0; i < param_ptr->num_in_ch; i++)
    {
        InpMediaFmt.channel_type[i] = param_ptr->in_ch_map[i];
    }
    ...
    {
            {
                ...
                { (int8_t*)&InpMediaFmt, actual_payload_len_in, sizeof(InpMediaFmt) },
                { true,true,0 }
            },
            {
                CAPI_V2_OUTPUT_MEDIA_FORMAT_V2,
                { (int8_t*)&outMediaFmt, actual_payload_len_out, sizeof(outMediaFmt) },
                { true,false,0 }
            }
    };
        ...
        case PSPD_CHMIXER_QCOM:
        {
            ...
            if(ADSP_EOK != result)
            {
                if(InpMediaFmt.f.num_channels <= CAPI_V2_MAX_CHANNELS)
                  {
                    // try with v1 payload
                     capiv2_mediafmt_t outMediaFmtv1 =
                     {
                             {
                                     CAPI_V2_FIXED_POINT
                             },
                     ...
                     for(uint16_t i = 0; i < param_ptr->num_out_ch; i++)
                     {
                         outMediaFmtv1.f.channel_type[i] = param_ptr->out_ch_map[i];
                     }

                     capiv2_mediafmt_t InpMediaFmtv1 =
                     {
                             {
                                     CAPI_V2_FIXED_POINT
                             },
                     ...
                     for(uint16_t i = 0; i < param_ptr->num_in_ch; i++)
                     {
                         InpMediaFmtv1.f.channel_type[i] = param_ptr->in_ch_map[i];
                     }
                     ...
                     {
                             {
                                 ...
                                 { (int8_t*)&InpMediaFmtv1, sizeof(InpMediaFmtv1), sizeof(InpMediaFmtv1) },
                                 { true,true,0 }
                             },
                             {
                                 CAPI_V2_OUTPUT_MEDIA_FORMAT,
                                 { (int8_t*)&outMediaFmtv1, sizeof(outMediaFmtv1), sizeof(outMediaFmtv1) },
                                 { true,false,0 }
                             }
                     };
...
static void MtMx_PspdProcUpdateKppsBw(MtMx_PspdStruct *pMePspd)
{
    ...
    if(PSPD_CHMIXER_NONE != pMePspd->eChMixerTypeVar && NULL != pMePspd->pChMixerCapiV2)
	{
		...
		else if (pMePspd->unNumInChannels <= 8)
		{
			unPspdKpps += PSPD_KPPS_CHMIXER_MODULE_ENABLED_8CH;		
			unPspdBw += PSPD_BW_CHMIXER_MODULE_ENABLED_8CH;
		}
		else if (pMePspd->unNumInChannels <= 16)
		{
                    unPspdKpps += PSPD_KPPS_CHMIXER_MODULE_ENABLED_16CH;
                    unPspdBw += PSPD_BW_CHMIXER_MODULE_ENABLED_16CH;
		}
		else
		{
                    unPspdKpps += PSPD_KPPS_CHMIXER_MODULE_ENABLED_32CH;
                    unPspdBw += PSPD_BW_CHMIXER_MODULE_ENABLED_32CH;
		}
	}	
