...
$Header: //components/rel/avs.adsp/2.8/aud/services/static_svcs/matrix_mixer_svc/core/lib/src/MixerSvc_MsgHandlers.cpp#21 $
		...
		case NEW_INPUT:
		{
            ...
            me->inPortParams[unInPortID] = (MatrixInPortInfoType*)qurt_elite_memory_malloc(sizeof(MatrixInPortInfoType), QURT_ELITE_HEAP_DEFAULT);
				...
				for (k = 0; k < MT_MX_MAX_OUTPUT_PORTS; k++)
				{
					...
					for (j = 0; j < MT_MX_NUM_CHANNELS_THIRTY_TWO; j++)
					{
						*pTargetGain++ = 0x2000;
						*pCurrentGain++ = 0x2000;
						*pCachedGain++ = 0x2000;
		...
		case NEW_OUTPUT:
		{
         ...
         me->outPortParams[unOutPortID] = (MatrixOutPortInfoType*)qurt_elite_memory_malloc(sizeof(MatrixOutPortInfoType), QURT_ELITE_HEAP_DEFAULT);
			...
			else
			{
				...
				MSG_2(MSG_SSID_QDSP6, DBG_HIGH_PRIO, "MtMx #%lu Opening timer Driver for o/p port %lu",me->mtMxID, unOutPortID);
				if (ADSP_EOK != (result = qurt_elite_timer_open(&(pCurrentOutPort->avt_drv_handle), (char*)"MtMxOp")))
				{
					MSG_2(MSG_SSID_QDSP6, DBG_ERROR_PRIO, "Failed to Open timer Driver for o/p port %lu with result: %d",unOutPortID, result);
					pCurrentOutPort->avt_drv_handle = NULL;
					goto __bailoutCmdCfgOutputPorts;
				}
			}
      ...
      // Native mode case
		...
		case CLOSE_OUTPUT:
		{
			...
			else
			{
				...
				{
					MSG_2(MSG_SSID_QDSP6, DBG_HIGH_PRIO, "MtMx #%lu Releasing timer Driver for o/p port %lu",me->mtMxID, unOutPortID);
					if (ADSP_EOK != (result = qurt_elite_timer_close(pCurrentOutPort->avt_drv_handle)))
					{
						MSG_2(MSG_SSID_QDSP6, DBG_ERROR_PRIO, "Failed to close timer Driver for o/p port %lu with result: %d",unOutPortID,result);
						pCurrentOutPort->avt_drv_handle = NULL;
						goto __bailoutCmdCfgOutputPorts;
					}
					pCurrentOutPort->avt_drv_handle = NULL;
...
ADSPResult MtMx_RampGains(This_t *me, elite_msg_any_t *pMsg)
{
	...
	if (ADM_MATRIX_ID_AUDIO_RX == pRampGainsPayload->matrix_id)
	{
		for (i = 0; i < unNumGains; i++)
		{
			...
			gain_params.channel_gain = &pSessionCOPPGain->target_gain_ch_1;
			gain_params.ramp_curve = pSessionCOPPGain->ramp_curve;
			gain_params.ramp_duration = pSessionCOPPGain->ramp_duration;
			...
			if (ADM_CMD_MATRIX_RAMP_GAINS_COPP_ID_ALL_CONNECTED_COPPS != pSessionCOPPGain->copp_id)
			{
				...
				{
					MtMx_SetupInputToOutputGainParams(me, unInPortID, unOutPortID, &gain_params);
				}
				else
				{
					result = ADSP_EUNEXPECTED;
...
ADSPResult MtMx_RampGainsv7(This_t *me, elite_msg_any_t *pMsg)
{
  ADSPResult                       result = ADSP_EOK;
  uint32_t                         unInPortID, unOutPortID;
  uint16_t                         unNumGains, i;
  MatrixInPortInfoType             *pCurrentInPort;
  adm_cmd_matrix_ramp_gains_v7_t      *pRampGainsPayload;
  ...
  EliteMsg_CustomMtMxRampGainsType *pPayload = (EliteMsg_CustomMtMxRampGainsType*)pMsg->pPayload;
  pRampGainsPayload = pPayload->pRampGainsPayloadv7;
  unNumGains = pRampGainsPayload->num_gains;

  MSG_3(MSG_SSID_QDSP6, DBG_HIGH_PRIO, "MtMx #%lu: Processing cmd %lu (RampGains) [num gains] = [%d]",
      me->mtMxID, pPayload->unSecOpCode, (int)pRampGainsPayload->num_gains);

  adm_session_copp_gain_v7_t *pTmp2 = (adm_session_copp_gain_v7_t*)(pRampGainsPayload + 1);
  ...
  if (ADM_MATRIX_ID_AUDIO_RX == pRampGainsPayload->matrix_id)
  {
    for (i = 0; i < unNumGains; i++)
    {
      ...
      gain_params.channel_gain = (uint16_t *)(pSessionCOPPGain + 1);
      gain_params.ramp_curve = pSessionCOPPGain->ramp_curve;
      gain_params.ramp_duration = pSessionCOPPGain->ramp_duration;
      ...
      gain_size = MTMX_ALIGNTO4(gain_size);
...
ADSPResult MtMx_Mute(This_t *me, elite_msg_any_t *pMsg)
{
	...
	MatrixInPortInfoType              *pCurrentInPort;
	uint32_t                         strMask;
        mtmx_mute_params_t             mute_params;

	EliteMsg_CustomMtMxMuteType *pPayload = (EliteMsg_CustomMtMxMuteType*)pMsg->pPayload;
	pMutePayload = pPayload->pMutePayloadv5;

	mute_params.channel_mute = &pMutePayload->mute_flag_ch_1;
	mute_params.ramp_duration = pMutePayload->ramp_duration;
	...
	if (ADM_MATRIX_ID_AUDIO_RX == pMutePayload->matrix_id)
	{
		...
		if (ADM_CMD_MATRIX_MUTE_COPP_ID_ALL_CONNECTED_COPPS != pMutePayload->copp_id)
		{
			...
			if (me->outPortParams[unOutPortID]->inPortsMask & (1 << unInPortID))
			{
				MtMx_SetupInputToOutputMuteParams(me, unInPortID, unOutPortID, &mute_params);
			}
			else
			{
				result = ADSP_EUNEXPECTED;
...
ADSPResult MtMx_Mutev7(This_t *me, elite_msg_any_t *pMsg)
{
  adm_cmd_matrix_mute_v7_t             *pMutePayload;
  ADSPResult                       result = ADSP_EOK;
  uint32_t                         unInPortID, unOutPortID;
  MatrixInPortInfoType              *pCurrentInPort;
  uint32_t                         strMask;
  ...
  EliteMsg_CustomMtMxMuteType *pPayload = (EliteMsg_CustomMtMxMuteType*)pMsg->pPayload;
  pMutePayload = pPayload->pMutePayloadv7;

  mute_params.channel_mute = (uint8_t *)(pMutePayload + 1);
  mute_params.ramp_duration = pMutePayload->ramp_duration;
		...
		uint32_t unOpCode)
{
	...
	if ((pMediaFormatBlk->num_channels > MT_MX_NUM_CHANNELS_THIRTY_TWO) || (pMediaFormatBlk->num_channels < MT_MX_NUM_CHANNELS_ONE))
	{
...
ADSPResult MtMx_SetPrimaryOutputPort(This_t *me, elite_msg_any_t *pMsg)
{
	...
	while(unNumSessions)
	{
		...
		if(!pCurrentInPort)
		{
		  MSG_2(MSG_SSID_QDSP6, DBG_HIGH_PRIO, "MtMx #%lu: Input Port [%u] alrady close",
		          me->mtMxID, pMapPayload->unInputPort);
		  pMapPayload++;
		  unNumSessions--;
		  continue;
		}
