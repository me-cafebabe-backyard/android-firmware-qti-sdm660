...
$Header: //components/rel/avs.adsp/2.8/aud/services/static_svcs/matrix_mixer_svc/avsync/lib/src/MixerSvc_AvSync.cpp#11 $
...
ADSPResult MtMx_InPortToCheckReInitSampleSlipLibrary(This_t *me, uint32_t unInPortID)
{
   ...
   if (NULL == pSampleSlip->pSampleSlipCapiv2)
   {
      ...
      capiv2_mediafmtv2_t InpMediaFmt =
      {
         {
            CAPI_V2_FIXED_POINT
         },
      ...
      uint32_t actual_payload_len = sizeof(InpMediaFmt.h) + sizeof(InpMediaFmt.f) + InpMediaFmt.f.num_channels * sizeof(InpMediaFmt.channel_type[0]);
      actual_payload_len = ELITE_ALIGN_BY_4(actual_payload_len);

      if(pCurrentInPort->unNumChannels == 0)
      {
         InpMediaFmt.channel_type[0] = PCM_CHANNEL_L;
         InpMediaFmt.channel_type[1] = PCM_CHANNEL_R;
      }
      else
      {
         for(uint16_t i = 0; i < pCurrentInPort->unNumChannels; i++)
         {
            InpMediaFmt.channel_type[i] = pCurrentInPort->unChannelMapping[i];
         }
      }
      capi_v2_prop_t setProp[] =   {
         {
            CAPI_V2_INPUT_MEDIA_FORMAT_V2,
            { (int8_t*)&InpMediaFmt, actual_payload_len, sizeof(InpMediaFmt) },
            { true,true,0 }
         }
      };
      ...
      {
          // send v1 format
          if(InpMediaFmt.f.num_channels <= CAPI_V2_MAX_CHANNELS)
            {
               // payload len of v1 is lesser than allocated payload len of v2
               capiv2_mediafmt_t *pInpMediaFmtv1 = (capiv2_mediafmt_t *)(&InpMediaFmt);
               pInpMediaFmtv1->h.data_format = CAPI_V2_FIXED_POINT;
               pInpMediaFmtv1->f.bitstream_format = CAPI_V2_DATA_FORMAT_INVALID_VAL;
               pInpMediaFmtv1->f.num_channels = (pCurrentInPort->unNumChannels == 0)? MT_MX_NUM_CHANNELS_TWO:pCurrentInPort->unNumChannels;
               pInpMediaFmtv1->f.bits_per_sample = (pCurrentInPort->unBitwidth == 0)? MT_MX_DEFAULT_BIT_WIDTH:pCurrentInPort->unBitwidth;
               pInpMediaFmtv1->f.q_factor = CAPI_V2_DATA_FORMAT_INVALID_VAL;
               pInpMediaFmtv1->f.sampling_rate = (pCurrentInPort->unSampleRate == 0)? MT_MX_SAMPLING_RATE_48000:pCurrentInPort->unSampleRate;
               pInpMediaFmtv1->f.data_is_signed = 1;
               pInpMediaFmtv1->f.data_interleaving = CAPI_V2_DEINTERLEAVED_UNPACKED;
               memset(pInpMediaFmtv1->f.channel_type, 0, sizeof(pInpMediaFmtv1->f.channel_type));

               if(pCurrentInPort->unNumChannels == 0)
               {
                  pInpMediaFmtv1->f.channel_type[0] = PCM_CHANNEL_L;
                  pInpMediaFmtv1->f.channel_type[1] = PCM_CHANNEL_R;
               }
               else
               {
                  for(uint16_t i = 0; i < pCurrentInPort->unNumChannels; i++)
                  {
                     pInpMediaFmtv1->f.channel_type[i] = pCurrentInPort->unChannelMapping[i];
                  }
               }

               setProp[0].id = CAPI_V2_INPUT_MEDIA_FORMAT;
               setProp[0].payload.data_ptr = (int8_t*)pInpMediaFmtv1;
               setProp[0].payload.actual_data_len = sizeof(*pInpMediaFmtv1);
               setProp[0].payload.max_data_len = sizeof(*pInpMediaFmtv1);
...
ADSPResult MxAt_Run(This_t *me, elite_msg_any_t *pMsg, EliteMsg_CustomMtMxRunType *pPayload)
{
	...
	else
	{
		...
		if(NULL == pCurrentOutPort->avt_drv_handle)
		{
			...
			ADSPResult result = ADSP_EOK;
			MSG_2(MSG_SSID_QDSP6, DBG_HIGH_PRIO, "MtMx #%lu Opening timer Driver for o/p port %lu",me->mtMxID, unPort2Run);
			if (ADSP_EOK != (result = qurt_elite_timer_open(&(pCurrentOutPort->avt_drv_handle), (char*)"MtMxOp")))
			{
				MSG_2(MSG_SSID_QDSP6, DBG_ERROR_PRIO, "Failed to Open timer Driver for o/p port %lu with result: %d",unPort2Run,result);
