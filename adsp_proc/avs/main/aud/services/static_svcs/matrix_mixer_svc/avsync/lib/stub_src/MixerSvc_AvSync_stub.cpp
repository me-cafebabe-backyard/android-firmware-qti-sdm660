...
$Header: //components/rel/avs.adsp/2.8/aud/services/static_svcs/matrix_mixer_svc/avsync/lib/stub_src/MixerSvc_AvSync_stub.cpp#6 $
...
ADSPResult MtMx_AvSyncReleaseHW(This_t *me, uint32_t unInPortID)
{
	...
	{
		MSG_2(MSG_SSID_QDSP6, DBG_HIGH_PRIO, "MtMx #%lu Releasing timer Driver for i/p port %lu",me->mtMxID, unInPortID);
		if (ADSP_EOK != (result = qurt_elite_timer_close(pCurrentInPort->avt_drv_handle)))
		{
			MSG_2(MSG_SSID_QDSP6, DBG_ERROR_PRIO, "Failed to close timer Driver for i/p port %lu with result: %d",unInPortID, result);
...
ADSPResult MtMx_AvSyncRequestHW(This_t *me, uint32_t unInPortID)
{
	...
	if(NULL == pCurrentInPort->avt_drv_handle)
	{
		MSG_2(MSG_SSID_QDSP6, DBG_HIGH_PRIO, "MtMx #%lu Opening timer Driver for i/p port %lu",me->mtMxID, unInPortID);
		if (ADSP_EOK != (result = qurt_elite_timer_open(&(pCurrentInPort->avt_drv_handle), (char*)"MtMxIp")))
		{
			MSG_2(MSG_SSID_QDSP6, DBG_ERROR_PRIO, "Failed to Open timer Driver for i/p port %lu with result: %d",unInPortID, result);
			pCurrentInPort->avt_drv_handle = NULL;
			return result;
		}
	}
		...
		MatrixOutPortInfoType *pCurrentOutPort, uint32_t ullCurrentDevicePathDelay)
{
	...
	pCurrentInPort->ullAbsoluteTime = (NULL != pCurrentInPort->avt_drv_handle) ? ((qurt_elite_fwk_get_time(pCurrentInPort->avt_drv_handle))+ullCurrentDevicePathDelay-ullStaleEosSamplesuSec):(0);
#ifdef MT_MX_EXTRA_DEBUG
