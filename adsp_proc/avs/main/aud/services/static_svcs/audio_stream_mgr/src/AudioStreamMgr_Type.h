...
$Header: //components/rel/avs.adsp/2.8/aud/services/static_svcs/audio_stream_mgr/src/AudioStreamMgr_Type.h#7 $
...
/*Generic function pointer to open a stream */
typedef ADSPResult (*AudioStreamMgr_OpenStreamCmdHandler_f) (AudioStreamMgr_t *pMe,
		uint8_t int_session_id, uint8_t stream_id);

typedef struct asm_stream_open_thread_obj
{
	   AudioStreamMgr_t *pMe;               // Instance  structure of ASM
	   uint8_t int_session_id;              // ID of the session to be opened
	   uint8_t stream_id;                   // ID of the stream to be opened
	   qurt_elite_thread_t threadId;        // reference to Helper thread ID created
	   AudioStreamMgr_OpenStreamCmdHandler_f fn_ptr; // function handler to open the specified session/stream ID
}asm_stream_open_thread_obj;
...
struct AudioStreamMgr_t
{
   ...
   qurt_elite_atomic_word_t unCurrentBitfield;      //contains the current channel mask or signals to act upon
   ...
   qurt_elite_atomic_word_t        unDataLogSeqNum;    //num for the PCM/bitstream logs: every session has new id.

   ASM_adsppm_t          adsppm;              //ADSPPM struct.

   asm_stream_open_thread_obj   *open_stream_obj[ASM_MAX_SUPPORTED_SESSION_ID]; // pointer to session object used while creating a stream.
   	   	   	   	   	   	   	   	   	   	   	   	   	   	   	   	   	   	   	   	// Valid only during creation

   qurt_elite_mutex_t  session_lock;  // mutex variable to lock the session
};
...
/* AudioStreamMgr EventQ handler type*/
typedef int (*AudioStreamMgr_EventHandler_f) (AudioStreamMgr_t* pMe);
