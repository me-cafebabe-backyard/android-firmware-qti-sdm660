...
/*========================================================================
$Header: //components/rel/avs.adsp/2.8/aud/services/static_svcs/audio_stream_mgr/src/AudioStreamMgr_StreamRespHandler.cpp#6 $
      ...
      AudioStreamMgr_SessionStateType *pSession )
{
   ...
   switch ( pPayload->unParamId )
   {
      ...
      case ELITEMSG_PARAM_ID_CAL_V2:
      {
         ...
         elite_msg_param_cal_v2_t *pCalPayload = (elite_msg_param_cal_v2_t*)(pPayload);
...
ADSPResult AudioStreamMgr_ProcessHelperThreadOpenResponseCustomEvent(AudioStreamMgr_t *pMe)
{

	ADSPResult result = ADSP_EOK;
	EliteEvent_CustomStreamOpen *pPayload = (EliteEvent_CustomStreamOpen *) pMe->msg.pPayload;

	uint8_t stream_id = pPayload->unStreamID;
	uint8_t unsessionID = pPayload->unsessionID;

	MSG_2(MSG_SSID_QDSP6, DBG_ERROR_PRIO, "AudioStreamMgr [%u %u]: helper thread instance ",
				unsessionID, stream_id );

	if(!(unsessionID < ASM_MAX_SUPPORTED_SESSION_ID))
	{
		return ADSP_EUNEXPECTED;
	}


	if(pMe->open_stream_obj[unsessionID])
	{
		qurt_elite_thread_t threadId = pMe->open_stream_obj[unsessionID]->threadId;
		ADSPResult nStatus = 0;
		qurt_elite_thread_join(threadId,  &nStatus);
		MSG_2(MSG_SSID_QDSP6, DBG_HIGH_PRIO,
				"AudioStreamMgr [%u,%u]: Open Stream : Free Helper thread",
				unsessionID,  stream_id);
		qurt_elite_memory_free(pMe->open_stream_obj[unsessionID]);
		pMe->open_stream_obj[unsessionID] = NULL;
	}
	result = pPayload->unResponseResult;
	elite_msg_release_msg(&pMe->msg);

	if(stream_id > ASM_MAX_SUPPORTED_STREAM_ID)
		return ADSP_EUNEXPECTED;

	AudioStreamMgr_StaticSessionStateType *pStSession = &pMe->aStSessions[unsessionID];
	AudioStreamMgr_SessionStateType *pSession = pMe->pSessions[unsessionID];
	...
	elite_apr_packet_t *pPkt = (elite_apr_packet_t*) (pStSession->cmdMsg.pPayload); // todo: Copy the cmdmsg in the open stream object
	qurt_elite_atomic_or(&pMe->unCurrentBitfield, (pStSession->unSessionCmdQMask << 1));
	if(result !=ADSP_EOK)
	{
		AudioStreamMgr_OpenStreamBailout(pMe, pSession, pStream, pPkt, result);
	}
   
   // Enabling the response & command Q
    //qurt_elite_atomic_or(&pMe->unCurrentBitfield, (pStSession->unSessionCmdQMask << 1));
   	if(pSession==NULL)
	{
		qurt_elite_atomic_or(&pMe->unCurrentBitfield, (pStSession->unSessionCmdQMask));
	}
	else
	{
		if (0 == pSession->unReqResps)
