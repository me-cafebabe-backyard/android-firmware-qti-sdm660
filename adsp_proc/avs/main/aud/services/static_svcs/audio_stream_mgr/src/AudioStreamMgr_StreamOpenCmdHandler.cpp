		...
		uint8_t int_session_id, uint8_t stream_id)
{
    ...
    qurt_elite_mutex_lock( &(pMe->session_lock) );
	if (NULL == pSession)
	{
		...
		if (ADSP_FAILED(result))
		{
			...
			qurt_elite_mutex_unlock( &(pMe->session_lock) );
			return result;
		}
	}
...
void AudioStreamMgr_OpenStreamBailout(AudioStreamMgr_t *pMe,
      ...
      elite_apr_packet_t *pPkt, ADSPResult result)
{
   if (pSession)
   {
       if (pSession->unReqResps == 0) //if no response expected
       {
           ...
           if (pStream && (pStream->state != ASM_STREAM_GAPLESS_CLOSE) && (result!=ADSP_EALREADY))
           {
               //destroy stream svc
...
/*
This function is to send the open response from the Helper thread to ASM.
*/

ADSPResult AudioStreamMgr_OpenStreamHelperThreadSendAckToAsm( AudioStreamMgr_t *pMe, uint8_t int_session_id,
		uint8_t stream_id , int result)
{
	uint32_t ulClientToken = 0;
	uint32_t unPayloadSize = sizeof(EliteEvent_CustomStreamOpen);

	MSG_2(MSG_SSID_QDSP6, DBG_HIGH_PRIO,"AudioStreamMgr [%u,%u]: ASM stream  sending Custom response for open command",
			int_session_id, stream_id);

	elite_msg_any_t   msg;

	ADSPResult result1 = elite_msg_create_msg( &msg, &unPayloadSize,
			ELITE_CUSTOM_EVENT,
			NULL,
			ulClientToken,
			0);
	...
	if(result1==ADSP_EOK)
	{
		MSG_2(MSG_SSID_QDSP6, DBG_HIGH_PRIO,"AudioStreamMgr [%u,%u]: ASM stream Sending Custom Open response from helper thread",
				int_session_id, stream_id);
		EliteEvent_CustomStreamOpen *stream_open_response_msg = (EliteEvent_CustomStreamOpen *) msg.pPayload;
		stream_open_response_msg->unSecOpCode = ELITE_EVENT_CUSTOM_FOR_STREAM_OPEN;
		stream_open_response_msg->unStreamID = stream_id;
		stream_open_response_msg->unsessionID = int_session_id;
		stream_open_response_msg->unResponseResult = result;
		qurt_elite_queue_push_back((qurt_elite_queue_t *)pMe->asmEvtQBuf, (uint64_t*)&msg);
	}

	return result;
}

ADSPResult AudioStreamMgr_OpenStreamCmdHelperThreadHandler(void * open_stream_obj)
{
	ADSPResult result=ADSP_EOK;

	asm_stream_open_thread_obj *asm_thread_obj = (asm_stream_open_thread_obj *)open_stream_obj;
	AudioStreamMgr_t *pMe = (AudioStreamMgr_t *)asm_thread_obj->pMe;
	uint8_t int_session_id = asm_thread_obj->int_session_id;
	uint8_t stream_id = asm_thread_obj->stream_id;
	AudioStreamMgr_OpenStreamCmdHandler_f fn_ptr = asm_thread_obj->fn_ptr;

	result = fn_ptr(pMe,int_session_id, stream_id );
		...
		uint8_t int_session_id, uint8_t stream_id)
{
	...
	AudioStreamMgr_OpenStreamHelperThreadSendAckToAsm(pMe, int_session_id, stream_id ,result);
		...
		uint8_t int_session_id, uint8_t stream_id)
{
	...
	asm_multi_channel_pcm_fmt_blk_v5_t *pMediaFmtv5;
	   ...
	   case ASM_MEDIA_FMT_MULTI_CHANNEL_PCM_V3:
	   {
	      ...
	      break;
           ...
           case ASM_MEDIA_FMT_MULTI_CHANNEL_PCM_V5:
           {
              ...
              pMediaFmtv5 = (asm_multi_channel_pcm_fmt_blk_v5_t*) pOpenCmdBytePtr;
              pOpenCmdBytePtr += sizeof(asm_multi_channel_pcm_fmt_blk_v5_t);   
              channel_map_size = pMediaFmtv5->num_channels * sizeof(asm_multi_channel_pcm_fmt_ch_map_t);
              channel_map_size = ASM_ALIGN_BY_4(channel_map_size);
              pOpenCmdBytePtr += channel_map_size;
	...
	/** create decoder service in pull mode */
	{
		...
		decInitParams.io_param.out.downstream_buf_delay_ptr = AudioStreamMgr_GetPostDecSvcVarBufDelay(pStream);
                ...
                switch(openStream.src_fmt_id)
                {
                   ...
                   case ASM_MEDIA_FMT_MULTI_CHANNEL_PCM_V4:
                   {
                      decInitParams.io_param.in.pull_mode.MediaFmt.pcm.fmt.bits_per_sample = pMediaFmt->bits_per_sample;
                      decInitParams.io_param.in.pull_mode.MediaFmt.pcm.fmt.is_signed = pMediaFmt->is_signed;
                      decInitParams.io_param.in.pull_mode.MediaFmt.pcm.fmt.endianness =
                             (openStream.src_fmt_id == ASM_MEDIA_FMT_MULTI_CHANNEL_PCM_V3)?0:((asm_multi_channel_pcm_fmt_blk_v4_t*)pMediaFmt)->endianness;
                      decInitParams.io_param.in.pull_mode.MediaFmt.pcm.fmt.num_channels = pMediaFmt->num_channels;
                      decInitParams.io_param.in.pull_mode.MediaFmt.pcm.fmt.sample_rate = pMediaFmt->sample_rate;
                      decInitParams.io_param.in.pull_mode.MediaFmt.pcm.fmt.sample_word_size = pMediaFmt->sample_word_size;
                      decInitParams.io_param.in.pull_mode.MediaFmt.pcm.fmt.mode = 0;

                      memscpy(decInitParams.io_param.in.pull_mode.MediaFmt.pcm.channel_map,
                             sizeof(decInitParams.io_param.in.pull_mode.MediaFmt.pcm.channel_map),
                             pMediaFmt->channel_mapping,
                             pMediaFmt->num_channels * sizeof(pMediaFmt->channel_mapping[0]));
                      break;
                   }
                   case ASM_MEDIA_FMT_MULTI_CHANNEL_PCM_V5:
                   {
                      asm_multi_channel_pcm_fmt_ch_map_t *ch_map;
                      decInitParams.io_param.in.pull_mode.MediaFmt.pcm.fmt.bits_per_sample = pMediaFmtv5->bits_per_sample;
                      decInitParams.io_param.in.pull_mode.MediaFmt.pcm.fmt.is_signed = pMediaFmtv5->is_signed;
                      decInitParams.io_param.in.pull_mode.MediaFmt.pcm.fmt.endianness = pMediaFmtv5->endianness;
                      decInitParams.io_param.in.pull_mode.MediaFmt.pcm.fmt.num_channels = pMediaFmtv5->num_channels;
                      decInitParams.io_param.in.pull_mode.MediaFmt.pcm.fmt.sample_rate = pMediaFmtv5->sample_rate;
                      decInitParams.io_param.in.pull_mode.MediaFmt.pcm.fmt.sample_word_size = pMediaFmtv5->sample_word_size;
                      decInitParams.io_param.in.pull_mode.MediaFmt.pcm.fmt.mode = pMediaFmtv5->mode;
                      ch_map = (asm_multi_channel_pcm_fmt_ch_map_t *)(pMediaFmtv5+1);
                      memscpy(decInitParams.io_param.in.pull_mode.MediaFmt.pcm.channel_map,
                              sizeof(decInitParams.io_param.in.pull_mode.MediaFmt.pcm.channel_map),
                              ch_map,
                              pMediaFmtv5->num_channels * sizeof(asm_multi_channel_pcm_fmt_ch_map_t));
                      break;
                   }
                   default:
                   {
		...
		uint8_t int_session_id, uint8_t stream_id)
{
	...
	asm_multi_channel_pcm_enc_cfg_v3_t *pEncCfgv3;
	asm_multi_channel_pcm_enc_cfg_v5_t *pEncCfgv5;
	avs_shared_map_region_payload_t *pRegion;
	...
	switch(openStream.sink_fmt_id)
	{
	   case ASM_MEDIA_FMT_MULTI_CHANNEL_PCM_V3:
	   {
              //map memory for the position buffer
	      pEncCfgv3 = (asm_multi_channel_pcm_enc_cfg_v3_t*) pOpenCmdBytePtr;
	   ...
	   {
              uint32_t channel_map_size = 0;
              pEncCfgv5 = (asm_multi_channel_pcm_enc_cfg_v5_t*) pOpenCmdBytePtr;
              pOpenCmdBytePtr += sizeof(asm_multi_channel_pcm_enc_cfg_v5_t);
              channel_map_size = pEncCfgv5->num_channels * sizeof(asm_multi_channel_pcm_fmt_ch_map_t);
              channel_map_size = ASM_ALIGN_BY_4(channel_map_size);
              pOpenCmdBytePtr += channel_map_size;
	      break;
	   }
...

	{
		...
		encInitParams.io_fmt_conv = ENC_SVC_IO_FORMAT_CONV_TYPE_AUTO;
                ...
                switch(openStream.sink_fmt_id)
                {
                   ...
                   case ASM_MEDIA_FMT_MULTI_CHANNEL_PCM_V4:
                   {
                      encInitParams.push_mode.EncCfg.pcm.fmt.bits_per_sample = pEncCfgv3->bits_per_sample;
                      encInitParams.push_mode.EncCfg.pcm.fmt.is_signed = pEncCfgv3->is_signed;
                      encInitParams.push_mode.EncCfg.pcm.fmt.endianness =
                             (openStream.src_fmt_id == ASM_MEDIA_FMT_MULTI_CHANNEL_PCM_V3)?0:((asm_multi_channel_pcm_fmt_blk_v4_t*)pEncCfgv3)->endianness;
                      encInitParams.push_mode.EncCfg.pcm.fmt.num_channels = pEncCfgv3->num_channels;
                      encInitParams.push_mode.EncCfg.pcm.fmt.sample_rate = pEncCfgv3->sample_rate;
                      encInitParams.push_mode.EncCfg.pcm.fmt.sample_word_size = pEncCfgv3->sample_word_size;
                      encInitParams.push_mode.EncCfg.pcm.fmt.mode = 0;

                      memscpy(encInitParams.push_mode.EncCfg.pcm.channel_map,
                             sizeof(encInitParams.push_mode.EncCfg.pcm.channel_map),
                             pEncCfgv3->channel_mapping,
                             pEncCfgv3->num_channels * sizeof(pEncCfgv3->channel_mapping[0]));
                      break;
                   }
                   case ASM_MEDIA_FMT_MULTI_CHANNEL_PCM_V5:
                   {
                      asm_multi_channel_pcm_fmt_ch_map_t *ch_map;
                      encInitParams.push_mode.EncCfg.pcm.fmt.bits_per_sample = pEncCfgv5->bits_per_sample;
                      encInitParams.push_mode.EncCfg.pcm.fmt.is_signed = pEncCfgv5->is_signed;
                      encInitParams.push_mode.EncCfg.pcm.fmt.endianness = pEncCfgv5->endianness;
                      encInitParams.push_mode.EncCfg.pcm.fmt.num_channels = pEncCfgv5->num_channels;
                      encInitParams.push_mode.EncCfg.pcm.fmt.sample_rate = pEncCfgv5->sample_rate;
                      encInitParams.push_mode.EncCfg.pcm.fmt.sample_word_size = pEncCfgv5->sample_word_size;
                      encInitParams.push_mode.EncCfg.pcm.fmt.mode = pEncCfgv5->mode;
                      ch_map = (asm_multi_channel_pcm_fmt_ch_map_t *)(pEncCfgv5+1);
                      memscpy(encInitParams.push_mode.EncCfg.pcm.channel_map,
                              sizeof(encInitParams.push_mode.EncCfg.pcm.channel_map),
                              ch_map,
                              pEncCfgv5->num_channels * sizeof(asm_multi_channel_pcm_fmt_ch_map_t));
                      break;
                   }
                   default:
                   {
