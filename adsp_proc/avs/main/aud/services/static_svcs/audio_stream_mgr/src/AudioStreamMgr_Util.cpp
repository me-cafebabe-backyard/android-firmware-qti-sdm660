...
/*========================================================================
$Header: //components/rel/avs.adsp/2.8/aud/services/static_svcs/audio_stream_mgr/src/AudioStreamMgr_Util.cpp#5 $
...
)
{
   ...
   {
      asm_multi_channel_pcm_enc_cfg_v5_t *pEncConfig = (asm_multi_channel_pcm_enc_cfg_v5_t *) pFrmtBlk ;
      asm_multi_channel_pcm_fmt_ch_map_t *ch_map;

      pParam->ulSampleRate = pEncConfig->sample_rate;
      pParam->ulNumChannels = pEncConfig->num_channels;
      ch_map = (asm_multi_channel_pcm_fmt_ch_map_t *)(pEncConfig+1);
      memscpy(pParam->channelMap, sizeof(pParam->channelMap), ch_map,
            sizeof(asm_multi_channel_pcm_fmt_ch_map_t) * pEncConfig->num_channels);
      return ADSP_EOK;
   }
   default:
   {
