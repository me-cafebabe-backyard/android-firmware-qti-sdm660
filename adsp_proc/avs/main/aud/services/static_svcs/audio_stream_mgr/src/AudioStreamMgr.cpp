...
/*========================================================================
$Header: //components/rel/avs.adsp/2.8/aud/services/static_svcs/audio_stream_mgr/src/AudioStreamMgr.cpp#10 $
...
/* Only handles one event currently in system EventQ, Remaining are dropped */
#define AUDIO_STREAM_MGR_MAX_OP_CODE_EVENT_CMD_Q 23
...
static const uint32_t THREAD_STACK_SIZE = 16640;
...
static ADSPResult  AudioStreamMgr_SysEventQCustomEventHandler(AudioStreamMgr_t *pMe);
static ADSPResult  AudioStreamMgr_SysEventQUnSupportedHandler(AudioStreamMgr_t *pMe);
...
/* Msg handler for system EventQ. Only support 1 messages currently. */
static AudioStreamMgr_EventHandler_f pSysEventQMsgHandler[AUDIO_STREAM_MGR_MAX_OP_CODE_EVENT_CMD_Q]=
{
		AudioStreamMgr_SysEventQUnSupportedHandler, // ELITE_CUSTOM_MSG
		AudioStreamMgr_SysEventQUnSupportedHandler,
		AudioStreamMgr_SysEventQUnSupportedHandler,
		AudioStreamMgr_SysEventQUnSupportedHandler,
		AudioStreamMgr_SysEventQUnSupportedHandler,
		AudioStreamMgr_SysEventQUnSupportedHandler,
		...
		AudioStreamMgr_SysEventQUnSupportedHandler,
		AudioStreamMgr_SysEventQUnSupportedHandler,
		AudioStreamMgr_SysEventQCustomEventHandler,    // ELITE_CUSTOM_EVENT
};
...
ADSPResult AudioStreamMgr_Create (uint32_t inputParam, void **handle)
{
   ...
   qurt_elite_mutex_init( &(pMe->session_lock) );
   // set up channel
...
void AudioStreamMgr_Destroy(AudioStreamMgr_t *pMe)
{
   ...
   qurt_elite_mutex_destroy( &(pMe->session_lock) );
...
static int AudioStreamMgr_WorkLoop(void* pInstance)
{
   ...
   uint32_t unCurrentBitfield = 0;

   unCurrentBitfield |= unSysCmdQMask ;
   unCurrentBitfield |= unSysEvtQMask ;
   ...
   {
	   unCurrentBitfield |= (1 << i);
   }
   qurt_elite_atomic_or(&pMe->unCurrentBitfield, unCurrentBitfield);
   ...
   for(;;)
   {
	 ...
	 unCurrentBitfield =  qurt_elite_atomic_get(&pMe->unCurrentBitfield);
     pMe->unChannelStatus = qurt_elite_channel_wait(&(pMe->channel), unCurrentBitfield);
     ...
     unCurrentBitfield =  qurt_elite_atomic_get(&pMe->unCurrentBitfield);
     pMe->unChannelStatus = qurt_elite_channel_poll(&pMe->channel, unCurrentBitfield);
...
static ADSPResult AudioStreamMgr_ProcessSysReserveQ(AudioStreamMgr_t *pMe, int32_t nChannelBit)
{
   ...
   if ( msg.unOpCode != ELITE_CUSTOM_EVENT) {
      return elite_msg_finish_msg( &msg, ADSP_EOK);
   }

   pMe->msg = msg;

   return pSysEventQMsgHandler[pMe->msg.unOpCode](pMe) ;
}
...
{
	ADSPResult result =  ADSP_EOK;
	MSG(MSG_SSID_QDSP6, DBG_HIGH_PRIO,"AudioStreamMgr: unsupported Event");
...
static ADSPResult  AudioStreamMgr_SysEventQCustomEventHandler(AudioStreamMgr_t *pMe)
{
	...
	ADSPResult result =  ADSP_EOK;
	elite_msg_custom_header_t *pPayload = (elite_msg_custom_header_t*) pMe->msg.pPayload;
	MSG_1(MSG_SSID_QDSP6, DBG_MED_PRIO,"AudioStreamMgr: Received custom event with opcode 0x%lx",pPayload->unSecOpCode);

	// Given the potential custom events.
	switch (pPayload->unSecOpCode)
	{
		case ELITE_EVENT_CUSTOM_FOR_STREAM_OPEN:
		{
			result = AudioStreamMgr_ProcessHelperThreadOpenResponseCustomEvent(pMe);
			break;
		}
		default:
		{
...
static ADSPResult AudioStreamMgr_DestroySvcHandler_WaitforHTJoin(AudioStreamMgr_t *pMe)
{
	 ADSPResult result = ADSP_EOK;
	 uint32_t helper_thread_count = 0;
	 uint32_t i = 0;

	 for( i = 0; i < ASM_MAX_SUPPORTED_SESSION_ID ; i++)
	 {
		 if(pMe->open_stream_obj[i]!=NULL)
		 {
			 helper_thread_count++;
		 }
	 }

	 MSG_1(MSG_SSID_QDSP6, DBG_MED_PRIO, "AudioStreamMgr: DestroySvc  Waiting for %lu helper threads to Join",helper_thread_count);

	 if(helper_thread_count > 0)
	 {
		 uint32_t ht_reponse_count = 0;
		 qurt_elite_queue_t *pEvtQue = (qurt_elite_queue_t *)pMe->asmEvtQBuf;

		 while(ht_reponse_count < helper_thread_count)
		 {
			 int32_t res = qurt_elite_queue_pop_front(pEvtQue, (uint64*)(&pMe->msg));
			 /* break out of loop if there are no more messages */
			 if (res == ADSP_ENEEDMORE)
			 {
				 continue;
			 }
			 elite_msg_custom_header_t *p_msg_payload = (elite_msg_custom_header_t*)pMe->msg.pPayload;
			 if(ELITE_CUSTOM_EVENT == pMe->msg.unOpCode)
			 {
				 if(ELITE_EVENT_CUSTOM_FOR_STREAM_OPEN == p_msg_payload->unSecOpCode)
				 {
					 result = AudioStreamMgr_ProcessHelperThreadOpenResponseCustomEvent(pMe);
					 ht_reponse_count++;
					 MSG_1(MSG_SSID_QDSP6, DBG_MED_PRIO, "AudioStreamMgr: DestroySvc - %lu helper threads are joined",ht_reponse_count);
				 }
			 }
			 else
			 {
...
static ADSPResult AudioStreamMgr_SysQDestroySvcHandler(AudioStreamMgr_t *pMe)
{
   ...
   AudioStreamMgr_DestroySvcHandler_WaitforHTJoin(pMe);
   AudioStreamMgr_Destroy(pMe);
