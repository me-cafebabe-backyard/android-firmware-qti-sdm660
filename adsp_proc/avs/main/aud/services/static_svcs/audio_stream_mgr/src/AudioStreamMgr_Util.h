...
$Header: //components/rel/avs.adsp/2.8/aud/services/static_svcs/audio_stream_mgr/src/AudioStreamMgr_Util.h#6 $
...
#define ASM_ALIGN_BY_4(x)  (ELITE_ALIGN_BY_4(x))
      ...
      qurt_elite_queue_t *queue, uint64_t *msg)
{
   ...
   qurt_elite_atomic_and(&pMe->unCurrentBitfield, ~(pStSession->unSessionCmdQMask));
   return ADSP_EOK;
}
...
typedef struct
{
    ...
    uint8_t  channelMap[ELITE_PCM_MAX_NUM_CH];
} PPOutputParams_t;
...
static inline bool_t ASM_IsPcmFormat(uint32_t dec_fmt_id)
{
   return ( (ASM_MEDIA_FMT_MULTI_CHANNEL_PCM_V2 == dec_fmt_id) ||
         ...
         (ASM_MEDIA_FMT_MULTI_CHANNEL_PCM_V5 == dec_fmt_id) );
}
