...
uint32_t ASM_GetNextDataLogId(AudioStreamMgr_t* pMe, uint16_t port)
{
	uint16_t seq_id =  (uint16_t) qurt_elite_atomic_get(&pMe->unDataLogSeqNum);
	qurt_elite_atomic_add(&pMe->unDataLogSeqNum, 1);
      ...
      uint8_t int_session_id, uint8_t stream_id)
{
   ...
   if (0 == memMapHandle) //in-band
   {
      ...
      memset((void *)&bufferMemNode, 0, sizeof(elite_mem_shared_memory_map_t));
   }
   else //out-of-band
   {
   ...
   {
      uint32_t unPayloadSize = sizeof( elite_msg_param_cal_v2_t );
      if ( ADSP_FAILED( result = elite_msg_create_msg( &msg, &unPayloadSize,
            ELITE_CMD_SET_PARAM,
            pStSession->pSessionRespQ,
            ulClientToken,
      ...
      elite_msg_param_cal_v2_t *pMsgPayload = (elite_msg_param_cal_v2_t*) msg.pPayload;
      pMsgPayload->unParamId  = unParamId;
      pMsgPayload->pnParamData   = paramBase;
      pMsgPayload->unSize        = paramPayloadSize;
      pMsgPayload->bufferMemNode = bufferMemNode;
      ...
      uint8_t int_session_id, uint8_t stream_id)
{
   ...
   if ( 0== memMapHandle ) //in-band
   {
      ...
      memset(&bufferMemNode, 0, sizeof(elite_mem_shared_memory_map_t));
      // store APR packet: thus in-band is identified by this packet which as opcode=ASM_STREAM_CMDRSP_GET_PP_PARAMS_V2
      pStSession->cmdMsg.pPayload = pAckAprPkt;
   ...
   else
   {
      ...
      pMsgPayload->unParamId  = calVersion;
      pMsgPayload->pnParamData = pnParamData;
      memscpy(&pMsgPayload->bufferMemNode,
    		  sizeof(elite_mem_shared_memory_map_t),
			  &bufferMemNode,
			  sizeof(elite_mem_shared_memory_map_t));
   }
