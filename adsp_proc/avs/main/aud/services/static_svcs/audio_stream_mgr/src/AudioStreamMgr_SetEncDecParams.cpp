...
/*========================================================================
$Header: //components/rel/avs.adsp/2.8/aud/services/static_svcs/audio_stream_mgr/src/AudioStreamMgr_SetEncDecParams.cpp#5 $
      ...
      void *pEncCfgBlk )
{
   ...
   if ((pStream->nodes[ASM_NODE_PP]).pSvcHandle)
   {
      ...
      if ( (ppParams.ulNumChannels > 32 ) ||
          ((ASM_MEDIA_FMT_MULTI_CHANNEL_PCM_V5 != enc_cfg_id) && (ppParams.ulNumChannels > 8 )))
      {
      ...
      memset(ppConfigMsg->params.outChannelMap, PCM_CHANNEL_INVALID, sizeof(ppConfigMsg->params.outChannelMap));
      ...
      else
      {
         ...
         memscpy(ppConfigMsg->params.outChannelMap, sizeof(ppConfigMsg->params.outChannelMap),
                    ppParams.channelMap, ppParams.ulNumChannels * sizeof(ppConfigMsg->params.outChannelMap[0]));
      }
      ...
      void *pEncCfgBlk )
{
   ...
   if((pStream->nodes[ASM_NODE_TX_MTX]).pSvcHandle)
   {
     ...
     {
        asm_multi_channel_pcm_enc_cfg_v5_t *pEncConfig = (asm_multi_channel_pcm_enc_cfg_v5_t *) pEncCfgBlk ;
        asm_multi_channel_pcm_fmt_ch_map_t *ch_map;
        config_ports_msg_ptr->num_channels = pEncConfig->num_channels;
        ch_map = (asm_multi_channel_pcm_fmt_ch_map_t *)(pEncConfig + 1);
        memset(config_ports_msg_ptr->channel_mapping, 0, sizeof(config_ports_msg_ptr->channel_mapping));
        memscpy(config_ports_msg_ptr->channel_mapping, sizeof(config_ports_msg_ptr->channel_mapping), ch_map, pEncConfig->num_channels * sizeof (asm_multi_channel_pcm_fmt_ch_map_t));
        config_ports_msg_ptr->bMtMxPassNativeNumChannels = (pEncConfig->num_channels == 0)? TRUE : FALSE;
        break;
     }
     default:
