...
ADSPResult AudioStreamMgr_OpenStreamCmdHandler(asm_stream_open_thread_obj   *open_stream_obj)
{
	char threadName[16]; // 4 spaces for the unique id and one for the NULL terminator.
	snprintf(threadName, 9, "ASM_HT%hhX", open_stream_obj->int_session_id);
	static const uint32_t HELPER_THREAD_STACK_SIZE = 16640;
	int32_t nPriority = elite_get_thrd_prio(ELITETHREAD_STAT_AUDIO_STREAM_SVC_PRIO_ID);
	ADSPResult result=ADSP_EOK;

	MSG_2(MSG_SSID_QDSP6, DBG_HIGH_PRIO, "AudioStreamMgr [%u,%u]:  "
			"launch ASM helper thread for the stream open",open_stream_obj->int_session_id, open_stream_obj->stream_id);

	result = qurt_elite_thread_launch(&open_stream_obj->threadId,
										threadName,
										NULL,
										HELPER_THREAD_STACK_SIZE,
										nPriority,
										AudioStreamMgr_OpenStreamCmdHelperThreadHandler,
										(void*)(open_stream_obj),
										QURT_ELITE_HEAP_DEFAULT);

	if (ADSP_FAILED(result))
	{
		MSG_2(MSG_SSID_QDSP6, DBG_FATAL_PRIO, "AudioStreamMgr [%u,%u]:  "
				"Failed to launch ASM helper thread!",open_stream_obj->int_session_id, open_stream_obj->stream_id);
		return result;
	}
	return result;
}
...
ADSPResult AudioStreamMgr_SessionCmdQAprPktHandler( AudioStreamMgr_t *pMe, uint8_t intSessionId)
{
   ...
   AudioStreamMgr_OpenStreamCmdHandler_f fn_ptr = NULL;
   ADSPResult result=ADSP_EOK;
   ...
   {
	   fn_ptr = AudioStreamMgr_OpenRxStreamCmdHandler;
	   break;
   }

   case ASM_STREAM_CMD_OPEN_READ_V3:
   {
	   fn_ptr = AudioStreamMgr_OpenTxStreamCmdHandler;
	   break;
   }

   case ASM_STREAM_CMD_OPEN_READWRITE_V2:
   {
	   fn_ptr = AudioStreamMgr_OpenRxTxStreamCmdHandler;
	   break;
   }
   ...
   case ASM_STREAM_CMD_OPEN_TRANSCODE_LOOPBACK:
   {
	   ...
	   fn_ptr = AudioStreamMgr_OpenLoopBackCmdHandler;
	   break;
   }

   case ASM_STREAM_CMD_OPEN_WRITE_COMPRESSED:
   {
	   fn_ptr = AudioStreamMgr_OpenWriteCompressedCmdHandler;
	   break;
   }

   case ASM_STREAM_CMD_OPEN_READ_COMPRESSED:
   {
	   fn_ptr = AudioStreamMgr_OpenReadCompressedCmdHandler;
	   break;
   }

   case ASM_STREAM_CMD_OPEN_PULL_MODE_WRITE:
   {
	   fn_ptr = AudioStreamMgr_OpenPullModeRxStreamCmdHandler;
	   break;
   }

   case ASM_STREAM_CMD_OPEN_PUSH_MODE_READ:
   {
	   fn_ptr = AudioStreamMgr_OpenPushModeTxStreamCmdHandler;
	   break;
   }
   ...
   /* if the Command corresponds to OPEN, then launch the Open handling in a separate helper thread */
   if(fn_ptr!=NULL)
   {
	   uint32_t unsession_cmd_respQ_mask =  pStSession->unSessionCmdQMask | (pStSession->unSessionCmdQMask <<1);
	   if(pMe->open_stream_obj[intSessionId]!=NULL)
	   {
		   MSG_2(MSG_SSID_QDSP6, DBG_HIGH_PRIO, "AudioStreamMgr [%u,%u]:"
				   " Open command sent while Processing an existing open command", intSessionId,  stream_id);
		   AudioStreamMgr_GenerateAck( pPkt, ADSP_EALREADY, NULL, 0, 0);
		   return ADSP_EUNEXPECTED;
	   }
	   else
	   {
		   /* disable the command Q for the session to stop processing any other commands for this session until open is completed */
		   qurt_elite_atomic_and(&pMe->unCurrentBitfield, ~(unsession_cmd_respQ_mask));
	   }

	   if(pMe->open_stream_obj[intSessionId]==NULL)
	   {
		   pMe->open_stream_obj[intSessionId] =
				   (asm_stream_open_thread_obj*) qurt_elite_memory_malloc(sizeof(asm_stream_open_thread_obj), QURT_ELITE_HEAP_DEFAULT);
	   }

	   if(pMe->open_stream_obj[intSessionId]!=NULL)
	   {
		   pMe->open_stream_obj[intSessionId]->pMe = pMe;
		   pMe->open_stream_obj[intSessionId]->int_session_id = intSessionId;
		   pMe->open_stream_obj[intSessionId]->stream_id = stream_id;
		   pMe->open_stream_obj[intSessionId]->fn_ptr = fn_ptr;
		   result = AudioStreamMgr_OpenStreamCmdHandler(pMe->open_stream_obj[intSessionId]);
		   if(result != ADSP_EOK)
		   {
			   qurt_elite_atomic_or(&pMe->unCurrentBitfield, unsession_cmd_respQ_mask);
			   if(pMe->open_stream_obj[intSessionId]!=NULL)
			   {
				   qurt_elite_memory_free(pMe->open_stream_obj[intSessionId]);
				   pMe->open_stream_obj[intSessionId] = NULL;
			   }
			   AudioStreamMgr_GenerateAck( pPkt, ADSP_ENOMEMORY, NULL, 0, 0);
			   return result;
		   }
	   }
	   else
	   {
		   ...
		   AudioStreamMgr_GenerateAck( pPkt, ADSP_ENOMEMORY, NULL, 0, 0);
		   MSG_2(MSG_SSID_QDSP6, DBG_HIGH_PRIO, "AudioStreamMgr [%u,%u]: "
				   "failed to allocate memory for helper thread", intSessionId,  stream_id);
