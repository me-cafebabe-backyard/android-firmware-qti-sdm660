...
$Header: //components/rel/avs.adsp/2.8/aud/services/static_svcs/utils/avsync_lib/lib/src/avsync_lib.cpp#7 $
...
struct avsync_param_t
{
   //Session clk for avsync requirements
    ...
    void*                   avt_drv_handle;      //Handle to timer
...
ADSPResult avsync_lib_open_hw_resources(void *inst_ptr)
{
   ...
   avsync_param_t *avsync_lib_ptr = (avsync_param_t *)inst_ptr;
   //Open timer Driver
   char* client_name = avsync_lib_ptr->instance_name;
   if(NULL != avsync_lib_ptr->avt_drv_handle)
   {
      return ADSP_EOK;
   }
   MSG_2(MSG_SSID_QDSP6, DBG_HIGH_PRIO, "AvSync Lib %s Opening timer Driver at %p",avsync_lib_ptr->instance_name,avsync_lib_ptr->avt_drv_handle);
   if (ADSP_EOK != (result = qurt_elite_timer_open(&(avsync_lib_ptr->avt_drv_handle), client_name)))
   {
      MSG_1(MSG_SSID_QDSP6, DBG_ERROR_PRIO, "Failed to Open timer Driver with result: %d",result);
      avsync_lib_ptr->avt_drv_handle = NULL;
   }
   MSG_2(MSG_SSID_QDSP6, DBG_HIGH_PRIO, "AvSync Lib pointer %p, timer pointer %p",avsync_lib_ptr,avsync_lib_ptr->avt_drv_handle);
...
ADSPResult avsync_lib_close_hw_resources(void *inst_ptr)
{
   ...
   avsync_param_t *avsync_lib_ptr = (avsync_param_t *)inst_ptr;
   MSG(MSG_SSID_QDSP6, DBG_HIGH_PRIO, "AvSync Lib Closing timer Driver");

   // For TX matrix this is not applicable and should pass right through (NULLness check).
   if (NULL != avsync_lib_ptr->avt_drv_handle)
   {
      if (ADSP_EOK != (result = qurt_elite_timer_close(avsync_lib_ptr->avt_drv_handle)))
      {
         MSG_1(MSG_SSID_QDSP6, DBG_ERROR_PRIO, "Failed to Close timer Driver with result: %d",result);
      }
      avsync_lib_ptr->avt_drv_handle = NULL;
   }
...
ADSPResult avsync_lib_update_stc_clock(void * inst_ptr)
{
   ...
   avsync_param_t *avsync_lib_ptr = (avsync_param_t *)inst_ptr;
   {
      if(NULL != avsync_lib_ptr->avt_drv_handle)
      {
         avsync_lib_ptr->stc_value_rendering = qurt_elite_fwk_get_time(avsync_lib_ptr->avt_drv_handle);
      }
   }
   return ADSP_EOK;
}
...
ADSPResult avsync_lib_set_internal_param(void *inst_ptr, uint32_t param_type, const void *param_payload,uint32_t param_size)
{
   ...
   case START_TIME:
      ...
      {
         uint64_t current_wall_time = (NULL != avsync_lib_ptr->avt_drv_handle) ? (qurt_elite_fwk_get_time(avsync_lib_ptr->avt_drv_handle)):(0);
...
ADSPResult avsync_lib_update_absolute_time(void *inst_ptr,int64_t adjustment, bool_t commit_stat)
{
   ...
   avsync_param_t *avsync_lib_ptr = (avsync_param_t *)inst_ptr;
   avsync_lib_ptr->absolute_ts = (NULL != avsync_lib_ptr->avt_drv_handle) ? (qurt_elite_fwk_get_time(avsync_lib_ptr->avt_drv_handle)):(0);
