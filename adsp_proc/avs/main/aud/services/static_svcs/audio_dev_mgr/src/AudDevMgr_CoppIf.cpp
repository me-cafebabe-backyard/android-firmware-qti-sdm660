...
$Header: //components/rel/avs.adsp/2.8/aud/services/static_svcs/audio_dev_mgr/src/AudDevMgr_CoppIf.cpp#11 $
...
ADSPResult Adm_SendCmdCoppSetParams(elite_svc_handle_t *phCopp,
                                    ...
                                    uint32_t calVersion,
                                    elite_mem_shared_memory_map_t *payload_shared_mem_info,
                                    bool_t isInlineWait)
{
  uint32_t                 unPayloadSize;
  ADSPResult               result;
  elite_msg_any_t          msg;
  elite_msg_param_cal_v2_t *pParamCalType;
  ...
  pParamCalType = (elite_msg_param_cal_v2_t*)msg.pPayload;

  pParamCalType->unParamId   = calVersion;
  pParamCalType->pnParamData = pSetParamsCmdBase;
  pParamCalType->unSize      = unDataPayloadSize;
  
  if(payload_shared_mem_info!=NULL)
  {
    memscpy((void *) &pParamCalType->bufferMemNode, sizeof(elite_mem_shared_memory_map_t),
              (void *)payload_shared_mem_info,sizeof(elite_mem_shared_memory_map_t) );
  }
  else
  {
    memset((void*)&pParamCalType->bufferMemNode, 0, sizeof(elite_mem_shared_memory_map_t));
  }
  ...
  /* This flag is set, when this function is called during Open command */
  if(isInlineWait)
  {
    if (ADSP_FAILED(result = elite_svc_wait_for_ack(&msg)))
    {
  ...
  else //This is not set for client Set Param
  {
    return result;
  }
...
void Adm_SendCmdCoppSetParamsDriftPtrSetup(AdmStatSvc_InfoType *pAdmStatSvc, uint16_t unCoppID, uint16_t unPrimaryDevice, qurt_elite_queue_t *pRspQ)
{
  ...
  //Call SetParam
  if (ADSP_FAILED(result= Adm_SendCmdCoppSetParams(pCoppHandle, pRspQ,
                                                   cmd, cmd, pSetParamsCmdBase,
                                                   coppDriftPtrParamData.data_payload_size,
                                                   ELITEMSG_PARAM_ID_CAL, NULL,
                                                   TRUE)))
  {
