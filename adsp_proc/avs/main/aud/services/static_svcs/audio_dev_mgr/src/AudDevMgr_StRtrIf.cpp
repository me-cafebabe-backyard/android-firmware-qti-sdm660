...
$Header: //components/rel/avs.adsp/2.8/aud/services/static_svcs/audio_dev_mgr/src/AudDevMgr_StRtrIf.cpp#14 $
...
ADSPResult Adm_MsgStRtrStreamConnect(AdmStatSvc_InfoType *pAudStatAdmSvc, EliteMsg_CustomAdmStreamConnectType *pAdmConnectMsg, adm_client_type_t eClient)
{
  ...
  if ( ADSP_FAILED( result = elite_msg_create_msg(&msg, &unSize, ELITE_CUSTOM_MSG,
                                                  pAudStatAdmSvc->admInlineRspQ, 0, 0)))
  {
...
ADSPResult Adm_MsgStRtrStreamDisconnect(AdmStatSvc_InfoType *pAudStatAdmSvc, EliteMsg_CustomAdmStreamConnectType *pAdmConnectMsg, adm_client_type_t eClient)
{
  ...
  if (ADM_RX_DIR == direction)
  {
    ...
    if (ADSP_FAILED(result = Adm_DetectAndUpdatePrimaryDeviceCompressed(pAudStatAdmSvc, pAudStatAdmSvc->admInlineRspQ)))
    {
                                           ...
                                           qurt_elite_queue_t *pRspQ)
{
  ...
  if (AUDPROC_TOPOLOGY_ID_COMPRESSED_NONE != topology_id)
  {
    ...
    if (ADSP_FAILED(result = AudPP_CreateSvc(&PPParams,
                                             (void **) &pComprCoppInfo->phDevCopp,
                                             (void**)&pDummyHandle)))
    {
  ...
  afe_connect_msg_ptr->afe_client.bytes_per_channel                = (bits_per_sample > 16) ? 4 : 2;
  ...
  if (AUDPROC_TOPOLOGY_ID_COMPRESSED_NONE != topology_id)
  {
    ...
    if (ADSP_FAILED(result = AdmSendCoppMsg(pComprCoppInfo->phDevCopp, pRspQ,
                                            temp_afe_svc_handle,
                                            ELITE_CMD_CONNECT)))
    {
  ...
  //now send the custom connect command to stream router
  {
    ...
    if ( ADSP_FAILED( result = elite_msg_create_msg(&strtr_msg, &unPayloadSize,
                                                    ELITE_CUSTOM_MSG,
                                                    pRspQ, 0, 0)))
    {
  ...
  if (AUDPROC_TOPOLOGY_ID_COMPRESSED_NONE != topology_id)
  {
    ...
    if (ADSP_FAILED(result = Adm_SendCmdRunCopp(pComprCoppInfo->phDevCopp, pRspQ)))
    {
    ...
    if (ADSP_FAILED(result = Adm_DataBaseRegisterPPCb(pComprCoppInfo->unCoppID,
                                                      pComprCoppInfo->structPPCallbackInfo,
                                                      TRUE)))
    {
  ...
  if(!is_device_open)
  {
    if (ADSP_FAILED(result = Adm_DetectAndUpdatePrimaryDeviceCompressed(pAdmStatSvc, pRspQ)))
    {
      MSG_1(MSG_SSID_QDSP6, DBG_ERROR_PRIO, "ADM: updating compressed primary device/port failed, result = %d", result);
      goto __bailoutCmdRxComprCoppOpen1;
    }
  ...
  if(NULL == pComprCoppInfo->pCmdQ)
  {
    result = Adm_HandleDeviceCmdQReq(pAdmStatSvc, pComprCoppInfo->unCoppID, TRUE, TRUE);
    if(ADSP_FAILED(result))
      {
        MSG_1(MSG_SSID_QDSP6, DBG_ERROR_PRIO, "ADM: Failed to setup devicecmdQ for COPP [%u]", pComprCoppInfo->unCoppID);
        goto __bailoutCmdRxComprCoppOpen0;
      }
  }
  ...
  __bailoutCmdRxComprCoppOpen0:
  if(AUDPROC_TOPOLOGY_ID_COMPRESSED_NONE != topology_id)
  {
    MSG(MSG_SSID_QDSP6, DBG_ERROR_PRIO, "ADM: DeRegistering PP clbk");
    uint32_t res;
    if (ADSP_FAILED(res = Adm_DataBaseDeRegisterPPCb(pComprCoppInfo->unCoppID,
                                                      pComprCoppInfo->structPPCallbackInfo,
                                                      TRUE)))
    {
      MSG_1(MSG_SSID_QDSP6, DBG_ERROR_PRIO, "ADM: Registering PP clbk failed, result = %d", result);
  ...
  if (AUDPROC_TOPOLOGY_ID_COMPRESSED_NONE != topology_id)
  {
    ...
    uint32_t res;
    if (ADSP_FAILED(res  = AdmSendCoppMsg(pComprCoppInfo->phDevCopp, pRspQ,
                                           ...
                                           qurt_elite_queue_t *pRspQ)
{
  ...
  if(NULL == pComprCoppInfo->pCmdQ)
  {
    result = Adm_HandleDeviceCmdQReq(pAdmStatSvc, device_id, TRUE, TRUE);
    if(ADSP_FAILED(result))
      {
        MSG_1(MSG_SSID_QDSP6, DBG_ERROR_PRIO, "ADM: Failed to setup devicecmdQ for COPP [%u]", pComprCoppInfo->unCoppID);
        goto __bailout;
...
======================================================================*/
ADSPResult Adm_RxStRtrAfeDisconnectCmdHandler(void *pAudStatAdmSvc, uint16_t afe_port_id, qurt_elite_queue_t *pRspQ)
{
  ADSPResult          result         = ADSP_EOK;
  AdmStatSvc_InfoType *pAdmStatSvc   = (AdmStatSvc_InfoType*)pAudStatAdmSvc;
  uint32_t            output_port_id = STRTR_INVALID_PORT_ID;
  ...
  if(ADSP_FAILED(result = Adm_MsgStRtrDisableOutputPort(pAdmStatSvc, RX_STRTR_ID, output_port_id, pRspQ)))
  {
  ...
  if (AUDPROC_TOPOLOGY_ID_COMPRESSED_NONE != pComprCoppInfo->unCoppTopologyID)
  {
    ...
    if (ADSP_FAILED(result = AdmSendCoppMsg(pComprCoppInfo->phDevCopp, pRspQ,
                                            pAdmStatSvc->rxStRtrParams.afe_port_svc_handle[output_port_id],
  ...
  if(NULL != pComprCoppInfo->pCmdQ)
  {
    result = Adm_HandleDeviceCmdQReq(pAdmStatSvc, unCoppID, TRUE, FALSE);
    MSG_2(MSG_SSID_QDSP6, DBG_HIGH_PRIO, "ADM: Removed CmdQ for device ID [%d] with result = 0x%x",unCoppID, result);
...
======================================================================*/
ADSPResult Adm_TxStRtrAfeDisconnectCmdHandler(void *pAudStatAdmSvc, uint16_t afe_port_id, qurt_elite_queue_t *pRspQ)
{
  ADSPResult result = ADSP_EOK;
  AdmStatSvc_InfoType *pAdmStatSvc = (AdmStatSvc_InfoType*)pAudStatAdmSvc;
  uint32_t input_port_id = STRTR_INVALID_PORT_ID;
...
======================================================================*/
ADSPResult Adm_MsgStRtrDisableOutputPort(AdmStatSvc_InfoType *pAdmStatSvc, uint32_t direction,
                                         uint32_t output_port_id, qurt_elite_queue_t *pRspQ)
{
...
ADSPResult Adm_StRtrStreamDeviceMapRoutingsHandler(AdmStatSvc_InfoType *pAdmStatSvc, adm_cmd_stream_device_map_routings_v5_t *map_routings_cmd_ptr, uint32_t opcode, adm_client_type_t eClient)
{
  ...
  if (ADM_COMPRESSED_AUDIO_OUT == map_routings_cmd_ptr->mode)
  {
    ...
    if (ADSP_FAILED(result = Adm_DetectAndUpdatePrimaryDeviceCompressed(pAdmStatSvc, pAdmStatSvc->admInlineRspQ)))
    {
      MSG_1(MSG_SSID_QDSP6, DBG_ERROR_PRIO, "ADM: updating compressed primary device/port failed, result = %d", result);
      elite_msg_release_msg(&strtr_msg);
      return result;
...
ADSPResult Adm_SetStrtrPrimaryOutputPort(AdmStatSvc_InfoType  *pAdmStatSvc, qurt_elite_queue_t *pRspQ)
{
  ADSPResult                  result = ADSP_EOK;
