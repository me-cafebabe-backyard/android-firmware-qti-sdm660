...
$Header: //components/rel/avs.adsp/2.8/aud/services/static_svcs/audio_dev_mgr/src/AudDevMgr_AprIf.cpp#13 $
...
int32_t AudDevMgr_AprCallBackFct(elite_apr_packet_t* packet, void* dispatch_data)
{
  ...
  AdmStatSvc_InfoType *pAdmStatSvc = (AdmStatSvc_InfoType*)dispatch_data;
  ...
  switch ( ulOpCode )
  {
    ...
    {
      Adm_GetDeviceCmdQ(pAdmStatSvc, (uint16_t)port, &pDestQ );
      if(NULL == pDestQ)
      {
        MSG(MSG_SSID_QDSP6, DBG_ERROR_PRIO, "ADM_AprCallBackFct: Failed to get device cmdQ!");
