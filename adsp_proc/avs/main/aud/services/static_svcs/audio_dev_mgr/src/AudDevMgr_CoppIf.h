...
$Header: //components/rel/avs.adsp/2.8/aud/services/static_svcs/audio_dev_mgr/src/AudDevMgr_CoppIf.h#6 $
...
ADSPResult Adm_SendCmdCoppSetParams(elite_svc_handle_t *phCopp, qurt_elite_queue_t *responseQ,
                                    uint32_t opcode, uint32_t token, int32_t *pSetParamsCmdBase,
                                    uint16_t unDataPayloadSize, uint32_t calVersion,
                                    elite_mem_shared_memory_map_t *payload_shared_mem_info,
																    bool_t isInlineWait);
void Adm_SendCmdCoppSetParamsDriftPtrSetup(AdmStatSvc_InfoType *pAdmStatSvc, uint16_t unCoppID,
                                           uint16_t unPrimaryDevice, qurt_elite_queue_t *pRspQ);
