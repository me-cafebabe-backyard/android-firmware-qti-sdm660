...
#define SINGLE_HELPER_THREAD_KIPS         10000

#define SINGLE_HELPER_THREAD_CPU_BW       1024
...
ADSPResult AudDevMgr_RequestHwResources(AdmStatSvc_InfoType *pAudStatAdmSvc)
{
   ...
   for (unCoppID = 0; unCoppID < ADM_MAX_COPPS; unCoppID++)
   {
	   ...
	   if(0 != phCalPort->helperThrdInfo.tId)
	   {
	     unTotalKipsReq += SINGLE_HELPER_THREAD_KIPS;
	     unAvgBusBWtReq += SINGLE_HELPER_THREAD_CPU_BW;
	   }
   }
   //Loop through Rx Compressed COPPS
   for(unCoppID = 0; unCoppID < STRTR_MAX_PORTS; unCoppID++)
   {
     ComprCoppInfo_t *pComprCopp = &pAudStatAdmSvc->rxStRtrParams.compr_copp_info[unCoppID];

     if(0 != pComprCopp->helperThrdInfo.tId)
     {
       unTotalKipsReq += SINGLE_HELPER_THREAD_KIPS;
       unAvgBusBWtReq += SINGLE_HELPER_THREAD_CPU_BW;
     }
   ...
   //Loop through Tx Compressed COPPS
   for(unCoppID = 0; unCoppID < STRTR_MAX_PORTS; unCoppID++)
   {
     ComprCoppInfo_t *pComprCopp = &pAudStatAdmSvc->txStRtrParams.compr_copp_info[unCoppID];
