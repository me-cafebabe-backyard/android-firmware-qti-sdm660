...
$Header: //components/rel/avs.adsp/2.8/aud/services/static_svcs/stream_router_svc/avsync/lib/stub_src/StreamRouterSvc_AvSync_stub.cpp#7 $
...
ADSPResult stream_router_init_input_avsync(stream_router_svc_t *pMe, uint32_t port_id)
{
	...
	MSG_1(MSG_SSID_QDSP6, DBG_HIGH_PRIO, "StrRtr, Opening timer Driver for i/p port %lu", port_id);
	if (ADSP_EOK != (result = qurt_elite_timer_open(&(pMe->in_port[port_id].avt_drv_handle), (char*)"StrRtrIp")))
	{
		MSG_2(MSG_SSID_QDSP6, DBG_ERROR_PRIO, "Failed to Open timer Driver for stream router i/p port %lu with result: %d", port_id, result);
...
ADSPResult stream_router_deinit_input_avsync(stream_router_svc_t *pMe, uint32_t port_id)
{
	...
	{
		MSG_1(MSG_SSID_QDSP6, DBG_HIGH_PRIO, "StrRtr, Releasing timer Driver for i/p port %lu", port_id);
		if (ADSP_EOK != (result = qurt_elite_timer_close(pMe->in_port[port_id].avt_drv_handle)))
		{
			MSG_2(MSG_SSID_QDSP6, DBG_ERROR_PRIO, "Failed to close timer Driver for stream router i/p port %lu with result: %d", port_id, result);
			pMe->in_port[port_id].avt_drv_handle = NULL;
			return result;
		}
		pMe->in_port[port_id].avt_drv_handle = NULL;
...
ADSPResult stream_router_svc_update_session_clock(stream_router_svc_t *pMe, uint32_t input_port_id, bool_t is_pausing)
{
	...
	in_port_ptr->absolute_time = (in_port_ptr->avt_drv_handle)?(qurt_elite_fwk_get_time(in_port_ptr->avt_drv_handle)):(0);
...
ADSPResult stream_router_svc_process_output_data_q_rx_mode(stream_router_svc_t *pMe, uint32_t output_port_id)
{
	...
	if(out_port_ptr->intermediate_buff.is_updated)
	{
		...
		if(0 == pause_duration)
		{
			...
			//query the av-timer
			in_port_ptr->absolute_time = (in_port_ptr->avt_drv_handle)?(qurt_elite_fwk_get_time(in_port_ptr->avt_drv_handle)+out_port_ptr->downstream_buffer_duration):(0);
		}
	}
	else
	{
