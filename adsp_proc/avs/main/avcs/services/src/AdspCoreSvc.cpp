...
/*========================================================================
$Header: //components/rel/avs.adsp/2.8/avcs/services/src/AdspCoreSvc.cpp#5 $
...
//AVCS handler for Remote Processor Event Handler (From AP Static Service)
static ADSPResult AdspCoreSvc_RemoteServiceEventHandler(AvsCoreSvc_t *pMe, elite_apr_packet_t* pAprPacket);
...
static bool_t IsProcIDValid(uint16_t proc_id);
...
static int AdspCoreSvcApr(void* pInstance, elite_msg_any_t* pMsg)
{
	...
	{
	  result = elite_mem_map_mdf_shared_memory_cmd_handler( avcsMemoryMapClient, pMe->aprHandle, pAprPacket );
	  break;
	}

	case AP_CMDRSP_SHARED_MEM_MAP_REGIONS:
	{
	  result = elite_mem_mdf_shared_memory_response_handler( pMe->aprHandle, pAprPacket, TRUE );
	  break;
	}

	case AP_EVENT_STATIC_SERVICE_UP:
	{
	  result = AdspCoreSvc_RemoteServiceEventHandler(pMe, pAprPacket);
	  break;
	}
	...
	case AVCS_AMDB_RESET:
	{
        ...
        {
          aprv2_ibasic_rsp_result_t* payload;
          elite_apr_if_get_payload( (void**) &payload, pAprPacket );

          switch ( payload->opcode )
          {
            case AP_CMD_SHARED_MEM_MAP_REGIONS:
              result = elite_mem_mdf_shared_memory_response_handler( pMe->aprHandle, pAprPacket, FALSE );
              break;

            default:
              elite_apr_if_free( pMe->aprHandle,pAprPacket );
              break;
          }
          break;
        }
...
static ADSPResult AVS_RemoteAVTimerVoteRequest(AvsCoreSvc_t *pMe, elite_apr_packet_t* pAprPacket)
{
   ...
   if (ADSP_FAILED(rc = qurt_elite_timer_open((void**)(&(pRspPayload->avtimer_handle)), (char*) &(pCmd->client_name[0]))))
   {
   ...
   MSG_2(MSG_SSID_QDSP6, DBG_ERROR_PRIO, "AdspCoreSvc: avtimer_handle = 0x%lx, name = %s!",
         (pRspPayload->avtimer_handle), pCmd->client_name);
...
static ADSPResult AVS_RemoteAVTimeReleaserRequest(AvsCoreSvc_t *pMe, elite_apr_packet_t* pAprPacket)
{
   ...
   if (ADSP_FAILED(result = qurt_elite_timer_close((void*)(pCmd->avtimer_handle))))
   {
...
static void AdspCoreSvc_UpdateRemoteServiceStatus(elite_apr_handle_t hAprHandle, elite_apr_packet_t *pPkt)
{
  ap_event_static_service_up_v1_t* pPayload = NULL;
  ...
  {
    MSG_1( MSG_SSID_QDSP6, DBG_ERROR_PRIO, "AdspCoreSvc: UpdateRemoteServiceStatus,"
  ...
  if(FALSE == IsProcIDValid((uint16_t)pPayload->proc_id))
  {
    return;
  }

  qurt_elite_globalstate.uApStatSvcUpStatus |= (1 << pPayload->proc_id);
}
...
static ADSPResult AdspCoreSvc_RemoteServiceEventHandler(AvsCoreSvc_t *pMe, elite_apr_packet_t* pAprPacket)
{
  ...
  AdspCoreSvc_UpdateRemoteServiceStatus(pMe->aprHandle, pAprPacket);

  result = elite_mem_remote_service_event_handler( pMe->aprHandle, pAprPacket );
  result = avs_pdr_ssr_remote_service_up_handler(pMe->aprHandle, pAprPacket );

  /* Always end the command with ADSP_EOK to indicate that the event has been processed by the ADSP */
  elite_apr_if_end_cmd(pMe->aprHandle, pAprPacket, ADSP_EOK);
...
static ADSPResult AdspCoreSvc_AprRegisterTopologiesHandler(AvsCoreSvc_t *pMe, elite_apr_packet_t* pAprPacket)
{
  ...
  uint32_t moduleInfoSize = 0;
	...
	while (unNumberTopologiesInPayload > 0)
	{
		...
		//1. Version check (version 0, 1 & 2 are supported)
    if (2 < pTopoDefBase->version)
		{
    ...
    switch(pTopoDefBase->version)
    {
      case 0 :
        moduleInfoSize = sizeof(avcs_module_info_t);
        break;

      case 1 :
        moduleInfoSize =  sizeof(avcs_module_instance_info_t);
        break;

      case 2 :
        moduleInfoSize = sizeof(avcs_module_instance_proc_info_t);
        break;

      default :
        break;
    }
    moduleInfoBufferSize = pTopoDefBase->num_modules * moduleInfoSize;
    ...
    switch(pTopoDefBase->version)
    {
      ...
      default :
		...
		//Advance to the next part of the payload. This will work even if num_modules is zero.
    pTemp += moduleInfoSize * pTopoDefBase->num_modules;
		unNumberTopologiesInPayload--;
	}
...
static bool_t IsModuleListUnique(avcs_topology_definition_t *topo_def_ptr, void *mod_def_ptr, uint32_t topo_version)
{
  ...
  else if(2 == topo_version)
  {
    avcs_module_instance_proc_info_t *module_info_ptr = (avcs_module_instance_proc_info_t*)mod_def_ptr;
    // Check for duplicate module ID - Instance ID pair
    for (uint32_t i = 0; i < topo_def_ptr->num_modules; i++)
    {
      if(FALSE == IsProcIDValid(module_info_ptr[i].client_proc_id))
      {
        MSG_4(MSG_SSID_QDSP6, DBG_ERROR_PRIO, "AVCS_CMD_REGISTER_TOPOLOGIES: Error: Module ID 0x%lx, Instance ID 0x%lx has unsupported"
              "     Proc ID 0x%lx in topology ID 0x%lx", module_info_ptr[i].module_id, module_info_ptr[i].instance_id,module_info_ptr[i].client_proc_id, topo_def_ptr->topology_id);
        return FALSE;
      }
