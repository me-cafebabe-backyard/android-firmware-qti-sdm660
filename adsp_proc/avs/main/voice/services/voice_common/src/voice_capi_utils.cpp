...
uint32_t voice_capi_get_static_prop(voice_capi_module_t* module_info_ptr)
{
   ...
   MSG_6(MSG_SSID_QDSP6, DBG_HIGH_PRIO,"VCP: module_id(0x%lx) received capi_size(%lu), inplace(%d), stack_size(%lu), buffering(%d). num_extensions(%ld)",
                                        module_info_ptr->module_id,
                                        module_info_ptr->mem_size_allocated,
                                        (int)inplace.is_inplace,
                                        stack_size.size_in_bytes,
                                        (int)needs_buffering.requires_data_buffering,
                                        num_of_extn.num_extensions);
