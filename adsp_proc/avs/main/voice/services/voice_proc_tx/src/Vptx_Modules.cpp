...
$Header: //components/rel/avs.adsp/2.8/voice/services/voice_proc_tx/src/Vptx_Modules.cpp#24 $
...
ADSPResult vptx_modules_set_param_int(vptx_t* pVptx, uint32_t mod_id, uint16_t instance_id, uint32_t param_id, void* payload_address, uint32_t param_size )
{
   ...
   else
   {
      ...
      {
         MSG_2(MSG_SSID_QDSP6, DBG_MED_PRIO,"VCP: set_param(): Unsupported set vptx module(%#lx), instance(%lx) - ignoring", mod_id,instance_id);
...
ADSPResult vptx_modules_get_param(vptx_t* pVptx, void *cal_param_payload, uint32_t cal_param_id_ver, uint16_t* param_size)
{
...
capi_v2_err_t vptx_capi_v2_cb_func(void *context_ptr, capi_v2_event_id_t id, capi_v2_event_info_t *event_info_ptr)
{
   ...
   MSG_3(MSG_SSID_QDSP6, DBG_HIGH_PRIO,"VCP: Vptx event callback invoked, event_id %d, module %lx instance %lx", (int)id, module_ptr->module_id,module_ptr->instance_id);
