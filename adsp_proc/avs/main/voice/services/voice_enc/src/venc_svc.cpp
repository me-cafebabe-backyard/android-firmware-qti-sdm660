...
/*========================================================================
  ...
  $Header: //components/rel/avs.adsp/2.8/voice/services/voice_enc/src/venc_svc.cpp#30 $
...
static ADSPResult venc_run_cmd(void* instance_ptr, elite_msg_any_t* msg_ptr)
{
   ...
   // subscribe to timer for some time stamp calculations
   if (ADSP_EOK != (result = qurt_elite_timer_open(&(venc_ptr->avt_drv), (char *)"VEC")))
   {
     MSG_1(MSG_SSID_QDSP6, DBG_ERROR_PRIO,"VCP: Failed to Open timer Driver for venc session(%x)",(int)venc_ptr->session_num);
   }
...
static ADSPResult venc_stop_cmd(void* instance_ptr, elite_msg_any_t* msg_ptr)
{
   ...
   qurt_elite_timer_close(venc_ptr->avt_drv);
   // OOB statistics
...
static ADSPResult venc_process(venc_t* venc_ptr)
{
         ...
         (venc_ptr->io.vfr_frame_cntr == 1) )
   {
      ...
      if ( venc_ptr->enc_tick_timestamp_us < qurt_elite_fwk_get_time( venc_ptr->avt_drv ))
      {
