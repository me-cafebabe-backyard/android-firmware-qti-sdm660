...
/*========================================================================
  ...
  $Header: //components/rel/avs.adsp/2.8/voice/services/voice_timer/src/VoiceTimer.cpp#9 $
...
#if defined(VTM_ONESHOT_DEBUG)
   if (ADSP_FAILED(result = qurt_elite_timer_create(&vtm_ptr->test_timer, QURT_ELITE_TIMER_ONESHOT_DURATION, QURT_ELITE_TIMER_USER, &vtm_ptr->test_signal)))
   {
...
static ADSPResult vtm_start_oneshot_timer(void *instance_ptr, elite_msg_any_t *data_ptr)
{
   ...
   if ( i != VOICE_MAX_TIMERS_PER_VFR_SOURCE )
   {
	   ...
	   uint64_t current_time = qurt_elite_fwk_get_time( vtm_ptr->avt_drv ); 
   ...
   if (ADSP_FAILED(result = qurt_elite_timer_oneshot_start_absolute(oneshot_timer_ptr[i], abs_time)))
   {
...
static ADSPResult vtm_soft_vfr_handler(void *instance_ptr)
{
   ...
   vtm_ptr->vfr_source_context[VFR_NONE].vfr_time_stamp = qurt_elite_fwk_get_time( vtm_ptr->avt_drv ); 
   ...
   for (int32_t i = 0; i < VOICE_MAX_TIMERS_PER_VFR_SOURCE; i++)
   {
      if(voice_tst_bit(mask, i))
      {
          if (VFR_NONE == client_subscription_ptr[i]->vfr_mode)
          {
            ...
            {
                if (ADSP_FAILED(result = qurt_elite_timer_oneshot_start_absolute(oneshot_timer_ptr[i], client_subscription_ptr[i]->avtimer_timestamp_us + (uint64_t)client_subscription_ptr[i]->offset)))
                {
...
static ADSPResult vtm_subscribe_msg(void *instance_ptr, elite_msg_any_t *data_ptr)
{
   ...
   if (0 == sum_mask)
   {
      if (ADSP_EOK != (result = qurt_elite_timer_open(&(vtm_ptr->avt_drv), (char *)"VTM")))
      {
         MSG(MSG_SSID_QDSP6, DBG_ERROR_PRIO,"VCP: Failed to Open timer Driver for VTM");
         return ADSP_EFAILED;
      }
   ...
   if ((VOICE_MAX_TIMERS_TOTAL > free_index_oneshot_timer) && (*client_cnt_ptr < VOICE_MAX_TIMERS_PER_VFR_SOURCE))
   {
      ...
      *oneshot_timer_ptr = &(vtm_ptr->oneshot_timer_struct[free_index_oneshot_timer]);
      ...
      if (ADSP_FAILED(result = qurt_elite_timer_create(*oneshot_timer_ptr,QURT_ELITE_TIMER_ONESHOT_ABSOLUTE,
                        QURT_ELITE_TIMER_USER, data_sub_ptr->signal_ptr)))
      {
      ...
      if (1 == *client_cnt_ptr)
      {
         if (VFR_NONE == data_sub_ptr->vfr_mode) //this takes care of VFR_NONE case in both versions of timing
         {
            ...
            if (ADSP_FAILED(result = qurt_elite_timer_create(&vtm_ptr->soft_vfr_timer,QURT_ELITE_TIMER_PERIODIC,
                        QURT_ELITE_TIMER_USER, &vtm_ptr->soft_vfr_signal)))
            {
            ...
            if (ADSP_FAILED(result = qurt_elite_timer_periodic_start(&vtm_ptr->soft_vfr_timer,data_sub_ptr->vfr_cycle)))
            {
      ...
      else
      {
          ...
          uint64_t client_subscription_time= qurt_elite_fwk_get_time( vtm_ptr->avt_drv ); 
          ...
          if (vfr_time)
          {
              ...
              if ((*client_subscription_ptr)->signal_enable)
              {
                  ...
                  {
                      if (ADSP_FAILED(result = qurt_elite_timer_oneshot_start_absolute(*oneshot_timer_ptr, absolute_offset_time)))
                      {
   ...
   {
      if( data_sub_ptr->signal_end_ptr)
      {
...
static ADSPResult vtm_unsubscribe_msg(void *instance_ptr, elite_msg_any_t *data_ptr)
{
   ...
   for (int32_t i = 0; i < VOICE_MAX_TIMERS_PER_VFR_SOURCE; i++)
   {
      ...
      {
         /* Send Ack */
         if(client_subscription_ptr[i]->signal_end_ptr)
         {
         ...
         *client_cnt_ptr -= 1; //decrement counter tracking number of clients
         ...
         if (ADSP_FAILED(result = qurt_elite_timer_delete(oneshot_timer_ptr[i])))
         {
         ...
         if (*client_cnt_ptr == 0)
         {
            if (VFR_NONE == data_unsub_ptr->vfr_mode)
            {
               ...
               if (ADSP_FAILED(result = qurt_elite_timer_delete(&vtm_ptr->soft_vfr_timer)))
               {
   ...
   if (0 == sum_mask)
   {
      ...
      qurt_elite_timer_close(vtm_ptr->avt_drv);
   }
...
static ADSPResult vtm_hard_vfr_msg(void *instance_ptr, elite_msg_any_t *data_ptr)
{
   ...
   // rearm test timer
   if (ADSP_FAILED(result =qurt_elite_timer_oneshot_start_duration(&vtm_ptr->test_timer, TEST_TIMER_DURATION)))
   {
      ...
      if(voice_tst_bit(mask, i))
      {
         if (client_subscription_ptr[i]->signal_enable)
         {
               ...
               )*/ //this check is not needed because index into context is derived using timing_ver and vfr_source
            {
               ...
               if (ADSP_FAILED(result = qurt_elite_timer_oneshot_start_absolute(oneshot_timer_ptr[i], corrected_absolute_offset)))
               {
                  MSG_1(MSG_SSID_QDSP6, DBG_ERROR_PRIO,"VCP: vtm_hard_vfr_msg: one shot timer re-arming failed, result(%d)",result);
               }
            }
   ...
   vtm_ptr->vfr_jitter_info.vfr_vtm_av_timestamp_us = qurt_elite_fwk_get_time( vtm_ptr->avt_drv ); 
