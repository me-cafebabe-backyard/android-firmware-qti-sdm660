/*
  ...
  $Header: //components/rel/avs.adsp/2.8/vsd/common/cvd/cvd_utils/src/cvd_devcfg_parser.c#11 $
  $Author: pwbldsvc $
*/
...
)
{
  ...
  MSG_2( MSG_SSID_DFLT, MSG_LEGACY_MED, "cvd_devcfg_parser: cycles per thousand instr packet %d, "
                                        "sleep latency in us %d",
                                 ret_clock_and_bus_preempt_margin_config->cycles_per_thousand_instr_packets,
  ...
  for ( clk_level_idx = 0; clk_level_idx < num_clk_levels; ++clk_level_idx )
  {
     ...
     for ( use_case_idx = 0; use_case_idx < num_supported_use_cases; ++use_case_idx )
     {
         ...
         )
       {
         ...
         MSG_1( MSG_SSID_DFLT, MSG_LEGACY_MED, "cvd_devcfg_parser: core_clock_hz %d",
