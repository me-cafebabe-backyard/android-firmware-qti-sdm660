...
$Header: //components/rel/avs.adsp/2.8/vsd/common/cvd/cvp/src/vpm_session_dyn_svc_cfg_utils.cpp#29 $
...
ADSPResult vpm_send_get_kpps_cmd_to_dyn_svc(vpm_session_object_t *session_obj)
{
   ...
   if (!session_obj->session_rsp_ctrl.num_cmd_issued)
   {
      ...
      if ( VPM_IS_PCIE_PORT(session_obj->active_set) )
      {
         result = ADSP_EOK;
      }
      else
...
ADSPResult vpm_disconnect_aferx_to_vprx( vpm_session_object_t *p_session )
{
   ADSPResult           result = ADSP_EOK;
   elite_msg_any_t      msg_pkt;
   uint32_t             payload_size;
   ...
   {
      MSG(MSG_SSID_DFLT, DBG_HIGH_PRIO,
          "vpm_disconnect_aferx_to_vprx(): FAILED to create EliteMsg");
      return result;
   }

   p_payload_connect_cmd = (elite_msg_cmd_connect_t *)msg_pkt.pPayload;
   p_payload_connect_cmd->pSvcHandle = p_dyn_svc->rx_afe_handle;

   if (ADSP_EOK != (result = qurt_elite_queue_push_back(p_dyn_svc->rx->cmdQ,
                                                        (uint64_t *)&msg_pkt)))
   {
      MSG(MSG_SSID_DFLT, DBG_HIGH_PRIO,
          "vpm_disconnect_aferx_to_vprx(): FAILED to push connect msg to vpRx");
   ...
   {
      MSG_1(MSG_SSID_DFLT, DBG_HIGH_PRIO,
            "vpm_disconnect_aferx_to_vprx(): success, sessionIndex(0x%lx)", session_id);
   }
   else
   {
      MSG_1(MSG_SSID_DFLT, DBG_ERROR_PRIO,
            "vpm_disconnect_aferx_to_vprx(): failed, sessionIndex(0x%lx)", session_id);
   }
...
ADSPResult vpm_action_disable_vocproc(vpm_session_object_t *session_obj)
{
   ...
   if ((VSS_IVOCPROC_PORT_ID_NONE != session_obj->active_set.ec_ref_port_id) &&
       ...
       (NULL != session_obj->dyn_svc_info.tx_far))
   {
      result |= vpm_disconnect_afe(session_obj,
   ...
   if (p_dyn_svc->rx)
   {
       ...
       *
       * Opening qurt timer handle is mandatory for sleep/other functionality.
       */
      void* avt_drv = NULL;
      if ( ADSP_EOK != qurt_elite_timer_open( &avt_drv, (char*)"VPM" ) )
      {
        MSG( MSG_SSID_DFLT, DBG_ERROR_PRIO,
             "vpm_action_disable_vocproc(): Failed to Open timer Driver for VPM_SLEEP");
      }
      else
      {
        ...
        if (ADSP_EOK != qurt_elite_timer_close(avt_drv) )
        {
           MSG(MSG_SSID_DFLT, DBG_ERROR_PRIO,
               "vpm_action_disable_vocproc(): Failed to close timer Driver for VPM_SLEEP");
        }
      }
