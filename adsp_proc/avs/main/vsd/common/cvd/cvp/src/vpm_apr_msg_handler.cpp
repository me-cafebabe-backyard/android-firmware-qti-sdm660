...
$Header: //components/rel/avs.adsp/2.8/vsd/common/cvd/cvp/src/vpm_apr_msg_handler.cpp#26 $
...
static ADSPResult vpm_attach_stream_cmd_ctrl(vpm_session_object_t *session_obj)
{
   ...
   for (;;)
   {
      ...
      if (FALSE == is_already_attached)
      {
         ...
         {
           rsp_attach_stream.tx_port_id = session_obj->target_set.tx_port_id;
           rsp_attach_stream.rx_port_id = session_obj->target_set.rx_port_id;
         }
      ...
      if (FALSE == session_obj->session_cmd_ctrl.is_prev_cmd_pending)
      {
         ...
         if (VPM_STATE_ENUM_RUN == session_obj->state_ctrl.state)
         {
            ...
            else if (TRUE == session_obj->dynamic_cal.is_registered)
            {
               ...
               /* If commands have been sent, wait for response. */
...
static ADSPResult vpm_volume_mute_v2_cmd_ctrl(vpm_session_object_t *session_obj)
{
   ADSPResult           result = ADSP_EOK;
   elite_apr_packet_t   *p_apr_pkt;
   apr_list_node_t      *pivot_node = NULL;
   apr_list_node_t      *cur_node = NULL;
   uint32_t             payload_size;
   voice_mute_path_t    mute_mode;
   ...
   for (;;)
   {
      ...
      {
	      for ( uint32_t strm_cntr = 0; strm_cntr < session_obj->attached_stream_list.size; strm_cntr++)
	      {
	       	 vpm_attached_stream_item_t* stream_item;
			 ...
			 stream_item = (vpm_attached_stream_item_t *)cur_node;
			 p_apr_pkt->dst_addr = stream_item->addr;
		     p_apr_pkt->dst_port = stream_item->port;
	       
		 	 elite_apr_if_cmd_fwd( session_obj->self_apr_handle, p_apr_pkt );			 	 		 	 
	      }      
		  return ADSP_EOK;
      }
      if (FALSE == session_obj->session_cmd_ctrl.is_prev_cmd_pending)
   ...
   for (;;)
   {
      switch (session_obj->session_cmd_ctrl.cmd_seq_action)
      {
         ...
         case VPM_ACTION_ENUM_HDVOICE_DISABLE:
         {
            ...
            if( session_obj->session_rsp_ctrl.is_prev_cmd_rsp_pending )
            {
              return ADSP_EOK;
            }
         }
   ...
   for (;;)
   {
      switch (p_cmd_ctrl->cmd_seq_action)
      {
         case VPM_ACTION_ENUM_CALIBRATE_DYNAMIC:
         {
            ...
            else if( session_obj->session_rsp_ctrl.is_prev_cmd_rsp_pending )
            {
