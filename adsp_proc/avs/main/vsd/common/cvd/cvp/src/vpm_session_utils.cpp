...
$Header: //components/rel/avs.adsp/2.8/vsd/common/cvd/cvp/src/vpm_session_utils.cpp#29 $
...
ADSPResult vpm_update_device_params(vpm_session_object_t *p_session)
{
   ADSPResult  result = ADSP_EOK;
   uint32_t    size = 0;
      ...
      
   {
      ...
      p_session->dyn_svc_info.ec_ref_num_channels = 1;
      p_session->dyn_svc_info.rx_num_channels = 1;
      /* Check if device channel info is registered. */
      if (  p_session->target_set.dev_chan_info.ec_ref.num_channels > 0 )
      {
         p_session->dyn_svc_info.ec_ref_num_channels = p_session->target_set.dev_chan_info.ec_ref.num_channels;
      }
      if ( p_session->target_set.dev_chan_info.tx.num_channels > 0 )
      {
         p_session->dyn_svc_info.tx_num_channels = p_session->target_set.dev_chan_info.tx.num_channels;
         return ADSP_EOK;
      }
      ...
      {
         if (p_session->target_set.tx_topology_id == topo_vptx[j].topology_id)
         {
            p_session->dyn_svc_info.tx_num_channels = topo_vptx[j].num_in_channels;

            MSG_2(MSG_SSID_DFLT, DBG_HIGH_PRIO,
                  "vpm_update_device_params(): Num channels for Tx topology(0x%lx): (%lu)",
                  p_session->target_set.tx_topology_id, p_session->dyn_svc_info.tx_num_channels);
      ...
      if (NULL == payload_address)
      {
         MSG_1(MSG_SSID_DFLT, DBG_ERROR_PRIO,
               "vpm_update_device_params(): Virtual address is null for custom topology(0x%lx)",
               p_session->target_set.tx_topology_id);

         return ADSP_EBADPARAM;
      }

      p_session->dyn_svc_info.tx_num_channels = 0;
         ...
         switch (additional_create_params->param_id)
         {
            case VSS_PARAM_NUM_DEV_CHANNELS:
            {
                     ...
                     (additional_create_params->param_size != sizeof(vss_param_num_dev_channels_t)))
               {
                  MSG_2(MSG_SSID_DFLT, DBG_ERROR_PRIO,
                        " vpm_update_device_params: Incorrect virtual address and/orsize. Required size (%ld) Received size (%ld)",
               ...
               p_session->dyn_svc_info.tx_num_channels = channel_info_ptr->tx_num_channels;

               MSG_2(MSG_SSID_DFLT, DBG_HIGH_PRIO,
                     "vpm_update_device_params(), Num channels for Custom Tx topology(0x%lx) = (%ld)",
                     p_session->target_set.tx_topology_id, p_session->dyn_svc_info.tx_num_channels);
            ...
            default:
            {
               MSG_1(MSG_SSID_DFLT, DBG_HIGH_PRIO,
                     "vpm_update_device_params(), Unrecognized param id (0x%lx)"
      ...
      if (0 == p_session->dyn_svc_info.tx_num_channels)
      {
         MSG_1(MSG_SSID_DFLT, DBG_ERROR_PRIO,
               "vpm_update_device_params(), num_channel is not received for custom topo(0x%lx)",
               p_session->target_set.tx_topology_id);
...
)
{
  ...
  if ( ( ( active_set->system_config.network_id !=  target_set->system_config.network_id ) 			 ||
         ...
         ( active_set->system_config.feature != target_set->system_config.feature ) )            &&
 	     (!VPM_IS_PCIE_PORT(session_obj->target_set) ) )	 
  {
    return TRUE;
  }
...
ADSPResult vpm_topology_commit_cmd_ctrl(vpm_session_object_t *session_obj)
{
   ...
   if (FALSE == session_obj->session_cmd_ctrl.is_prev_cmd_pending)
   {
      ...
      {
         if ( VPM_IS_PCIE_PORT(session_obj->target_set) )
         {
           vpm_destroy_session_dyn_svc( session_obj, TRUE, TRUE );
         }
         ...
         case VSS_MODULE_CVD_GENERIC:
         {
			*is_generic_params = TRUE;
            ...
            switch(voice_common_set_get_param_ptr->param_id)
            {
               ...
               {
                 /*removed the payload size check as HLOS sends channel mapping info for 32 channels instead of 8 channels, but CVD copies 
                   channel mapping info for just 8 channels. Hence, no change in functionality*/
