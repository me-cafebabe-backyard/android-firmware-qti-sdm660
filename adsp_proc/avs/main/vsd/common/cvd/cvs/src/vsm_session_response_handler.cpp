/*
  ...
  $Header: //components/rel/avs.adsp/2.8/vsd/common/cvd/cvs/src/vsm_session_response_handler.cpp#18 $
  $Author: pwbldsvc $
*/
...
)
{
  ...
  switch ( apr_pkt->opcode )
  {
    ...
    case VSS_IVOCPROC_RSP_ATTACH_STREAM:
    {
    	session_object->target_set.pcie_config.tx_port_id =  
        APRV2_PKT_GET_PAYLOAD( vss_ivocproc_rsp_attach_detach_stream_t, apr_pkt )->tx_port_id;
    	session_object->target_set.pcie_config.rx_port_id = 
    		APRV2_PKT_GET_PAYLOAD( vss_ivocproc_rsp_attach_detach_stream_t, apr_pkt )->rx_port_id;    	
    }
    /* Fall through. */
    case VSS_IVOCPROC_RSP_DETACH_STREAM:
    {
  ...
  if ( p_cvp_afe_tx_port_id_array && p_cvp_afe_rx_port_id_array && p_cvp_afe_ec_ref_port_id_array )
  {
    for (; i_index < VSM_MAX_ATTACHED_VOCPROC_CNT; i_index++)
    {
      //if (p_cvp_handle_array[i_index] == session_object->target_set.attach_detach_vocproc_handle)
      {
        ...
        if( payload->is_pcie_port )
        {
          if( !VSM_IS_PCIE_PORT( &session_object->target_set.pcie_config ) )
          {
            session_object->target_set.pcie_config.rx_port_id = payload->rx_port_id;
            session_object->target_set.pcie_config.tx_port_id = payload->tx_port_id;
