/*
  ...
  $Header: //components/rel/avs.adsp/2.8/vsd/common/cvd/cvs/src/vsm_actions.cpp#34 $
  $Author: pwbldsvc $
*/
...
ADSPResult vsm_send_disconnect_marker_to_afe(vsm_session_object_t *session_obj)
{
   ADSPResult        result = ADSP_EOK;
   ...
   session_id = session_obj->apr_info.self_port;

   afe_port_id = session_obj->active_set.pcie_config.rx_port_id;
   ...
   p_msg_payload->svc_handle_ptr   = session_obj->rx_afe_handle_ptr;

   /* Send the msg to device static service cmdQ */
   if (ADSP_FAILED(result = qurt_elite_queue_push_back(session_obj->vsm_afe_cmd_q_ptr,
                                                       (uint64_t *)&msg_pkt)))
   {
      MSG_1(MSG_SSID_DFLT, DBG_ERROR_PRIO,
            "vsm_send_disconnect_marker_to_afe(): Failed to push msg buf to AFE svc cmd Q, result: 0x%lx",
            result);
   ...
   if (ADSP_EOK != (result = p_msg_payload->response_result))
   {
      MSG_3(MSG_SSID_DFLT, DBG_ERROR_PRIO,
            "vsm_send_disconnect_marker_to_afe(): AFE Ack Failed"
...
ADSPResult vsm_action_pcie_disconnect_afe(
  vsm_session_object_t* session_object
)
{
  int32_t rc = ADSP_EOK;
  elite_msg_cmd_connect_t* p_payload_connect_cmd = NULL;
  ...
  if ( session_object->venc )        
  {
    ...
    afe_port = session_object->active_set.pcie_config.tx_port_id;
    ...
    elite_svc_wait_for_ack(&msg_tx_t);
    p_payload_connect_cmd = (elite_msg_cmd_connect_t *)msg_tx_t.pPayload;
    if (ADSP_EOK == (rc = p_payload_connect_cmd->unResponseResult))
    {
       MSG(MSG_SSID_DFLT, MSG_LEGACY_HIGH,
             "vsm_action_disconnect_afe_pcie(): success");
    }
    else
    {
       MSG(MSG_SSID_DFLT, MSG_LEGACY_ERROR,
             "vsm_action_disconnect_afe_pcie(): failed");
    }
  ...
  if( session_object->vdec )  
  {
    ...
    afe_port = session_object->active_set.pcie_config.rx_port_id;

    rc = vsm_send_disconnect_marker_to_afe(session_object);

    qurt_elite_timer_sleep(2000);
    ...
    elite_svc_wait_for_ack(&msg_rx_t);
    p_payload_connect_cmd = (elite_msg_cmd_connect_t *)msg_rx_t.pPayload;
    if (ADSP_EOK == (rc = p_payload_connect_cmd->unResponseResult))
    {
       MSG(MSG_SSID_DFLT, MSG_LEGACY_HIGH,
             "vsm_action_disconnect_afe_pcie(): success");
...
ADSPResult vsm_action_pcie_connect_afe(
  vsm_session_object_t* session_object
)
{
  int32_t rc = ADSP_EOK;
  uint16_t sampling_rate;
  afe_client_t afe_client_tx;	  
  afe_client_t afe_client_rx;	  


	if ( ( NULL == session_object ) ||
	 	 ( AFE_PORT_ID_INVALID == session_object->target_set.pcie_config.rx_port_id ) ||
	 	 ( AFE_PORT_ID_INVALID == session_object->target_set.pcie_config.tx_port_id ) )
	{
	  return ADSP_EBADPARAM;
	}
	session_object->cmd_control.issued_count = 0;
  ...
  if ( session_object->vdec ) 
  {
    ...
    memset(&afe_client_rx, 0, sizeof(afe_client_rx));
    afe_client_rx.sample_rate 	= sampling_rate;
    afe_client_rx.data_path 	= AFE_RX_VOC_MIXER_IN;
    afe_client_rx.channel 		= 1;
    afe_client_rx.bytes_per_channel = 2;
    afe_client_rx.bit_width = 16;
    afe_client_rx.buf_size		= ( sampling_rate / 1000 ) * 20;// 20ms worth of data.
    afe_client_rx.num_buffers 	 = 2;
    afe_client_rx.interleaved 	= FALSE;
    afe_client_rx.cust_proc.resampler_type = IIR_BASED_SRC;
    afe_client_rx.afe_client_handle 	= session_object->vdec; 
    afe_client_rx.cust_proc.subscribe_to_avt_drift = FALSE; // dont subscribe to AV Timer, Voice Timer will take care of this

    afe_port = session_object->target_set.pcie_config.rx_port_id;  
    ...
    rc = elite_msg_create_msg( &msg_rx_t,
	  ...
	  if( ADSP_FAILED( rc ) )
	  {
			MSG(MSG_SSID_DFLT, MSG_LEGACY_ERROR,"vsm_action_connect_afe_rx: FAILED to create EliteMsg!!\n" );
			return rc;
	  }
	  payload_ptr = ( elite_msg_custom_afe_connect_t* ) msg_rx_t.pPayload;
	  payload_ptr->sec_op_code = ELITEMSG_CUSTOM_AFECONNECT_REQ;
	  payload_ptr->afe_id = afe_port;
	  payload_ptr->afe_client = afe_client_rx;

	  rc = qurt_elite_queue_push_back( session_object->vsm_afe_cmd_q_ptr, ( uint64_t* )&msg_rx_t );
	  if ( ADSP_FAILED( rc ) )
	  {
			MSG_1(MSG_SSID_DFLT, MSG_LEGACY_ERROR,"vsm_action_connect_afe_rx: FAILED to push to afe cmdQ. error %d!!\n", rc );
			elite_msg_return_payload_buffer( &msg_rx_t );
			return rc;
	  }		
	  session_object->cmd_control.issued_count++;
	...
	{		
		...
		memset(&afe_client_tx, 0, sizeof(afe_client_tx));
		afe_client_tx.sample_rate			= sampling_rate;
		afe_client_tx.data_path			= AFE_PP_OUT;
		afe_client_tx.channel				= 1; 
		afe_client_tx.bytes_per_channel = 2;
		afe_client_tx.bit_width = 16;
		afe_client_tx.buf_size 				= ( sampling_rate / 1000 ) * 20; // 20ms worth of data.
		afe_client_tx.num_buffers			 = 2;
		afe_client_tx.interleaved			= FALSE;
		afe_client_tx.cust_proc.resampler_type = IIR_BASED_SRC;
		afe_client_tx.afe_client_handle				= session_object->venc; 
		afe_client_tx.cust_proc.subscribe_to_avt_drift = FALSE; // dont subscribe to AV Timer, Voice Timer will take care of this
	
		afe_port = session_object->target_set.pcie_config.tx_port_id;		
		...
		if( ADSP_FAILED( rc ) )
		{
			MSG(MSG_SSID_DFLT, MSG_LEGACY_ERROR,"vsm_action_connect_afe_tx: FAILED to create EliteMsg!!\n" );
			return rc;
		}
		payload_ptr = ( elite_msg_custom_afe_connect_t* )msg_tx_t.pPayload;
		payload_ptr->sec_op_code = ELITEMSG_CUSTOM_AFECONNECT_REQ;
		payload_ptr->afe_id = afe_port;
		payload_ptr->afe_client 	= afe_client_tx;

		rc = qurt_elite_queue_push_back( session_object->vsm_afe_cmd_q_ptr, ( uint64_t* )&msg_tx_t );
		if ( ADSP_FAILED( rc ) )
		{
			MSG_1(MSG_SSID_DFLT, MSG_LEGACY_ERROR,"vsm_action_connect_afe_tx: FAILED to push to afe cmdQ. error %d!!\n", rc );
			elite_msg_return_payload_buffer( &msg_tx_t );
			return rc;
		}
		session_object->cmd_control.issued_count++;
...
ADSPResult vsm_action_clear_voice_timing_to_afe(
  vsm_session_object_t* session_object
)
{
  ADSPResult rc = ADSP_EOK;
  elite_msg_any_t msg_afe_tx;
  elite_msg_any_t msg_afe_rx;
  elite_msg_custom_afe_data_rw_offsets* payload_ptr;
  uint32_t size = 0;
  ...
  while( VSM_IS_PCIE_PORT(&session_object->active_set.pcie_config) && 
         session_object->tx_afe_handle_ptr && 
         session_object->rx_afe_handle_ptr ) 
  {
    /* Sending the timing offset to AFE Tx. */  
    size = sizeof( elite_msg_custom_afe_data_rw_offsets );
    rc = elite_msg_create_msg( &msg_afe_tx, &size, ELITE_CUSTOM_MSG,
                                  NULL,
                                  VSM_ASYNC_CMD_TOKEN_TX, ADSP_EOK );
    if( ADSP_FAILED( rc ) )
    {
      MSG_1( MSG_SSID_DFLT, MSG_LEGACY_ERROR,
             "VSM: ACTION_CLEAR_VOICE_TIMING_TO_AFE: FAILED to create  EliteMsg  session_port0x%X )",
            ( int )session_object->apr_info.self_port );
      break;
    }

    payload_ptr = ( elite_msg_custom_afe_data_rw_offsets* ) msg_afe_tx.pPayload;
    payload_ptr->sec_op_code = ELITEMSG_CUSTOM_VFR_AFE_DATA_RW_OFFSETS;
    payload_ptr->ul_offset_1 = 0;
    payload_ptr->ul_offset_2 = 0;
    payload_ptr->dl_offset_1 = 0;
    payload_ptr->dl_offset_2 = 0;
    payload_ptr->afe_id = session_object->active_set.pcie_config.tx_port_id;
    payload_ptr->vsid = session_object->target_set.voice_timing.vsid;
    payload_ptr->vfr_mode = session_object->target_set.voice_timing.vfr_mode;
    payload_ptr->vfr_cycle_us = 0;
    if ( ADSP_FAILED( rc = qurt_elite_queue_push_back( session_object->tx_afe_handle_ptr->cmdQ ,( uint64_t * )&msg_afe_tx ) ) )
    {
      MSG_1( MSG_SSID_DFLT, MSG_LEGACY_ERROR,
             "VSM: ACTION_SET_VOICE_TIMING_TO_AFE: Unable to push to Afe tx cmdQ, "
             "error = %d!!\n",( int )rc );
      elite_msg_return_payload_buffer( &msg_afe_tx );
      break;
    }       

    /* Sending the timing offset to AFE Rx. */
    size = sizeof( elite_msg_custom_afe_data_rw_offsets );
    rc = elite_msg_create_msg( &msg_afe_rx, &size, ELITE_CUSTOM_MSG,
                                  NULL,
                                  VSM_ASYNC_CMD_TOKEN_RX, ADSP_EOK );
    if( ADSP_FAILED( rc ) )
    {
      MSG_1( MSG_SSID_DFLT, MSG_LEGACY_ERROR,
             "VSM: ACTION_SET_VOICE_TIMING_TO_AFE: FAILED to create  EliteMsg  session_port0x%X )",
            ( int )session_object->apr_info.self_port );
      break;
    }

    payload_ptr = ( elite_msg_custom_afe_data_rw_offsets* ) msg_afe_rx.pPayload;
    payload_ptr->sec_op_code = ELITEMSG_CUSTOM_VFR_AFE_DATA_RW_OFFSETS;
    payload_ptr->ul_offset_1 = 0;
    payload_ptr->ul_offset_2 = 0;
    payload_ptr->dl_offset_1 = 0;
    payload_ptr->dl_offset_2 = 0;
    payload_ptr->afe_id = session_object->active_set.pcie_config.rx_port_id;
    payload_ptr->vsid = session_object->target_set.voice_timing.vsid;
    payload_ptr->vfr_mode = session_object->target_set.voice_timing.vfr_mode;
    payload_ptr->vfr_cycle_us = 0;
    if ( ADSP_FAILED( rc = qurt_elite_queue_push_back( session_object->rx_afe_handle_ptr->cmdQ ,( uint64_t * )&msg_afe_rx ) ) )
    {
      MSG_1( MSG_SSID_DFLT, MSG_LEGACY_ERROR,
             "VSM: ACTION_SET_VOICE_TIMING_TO_AFE: Unable to push to Afe Rx cmdQ, "
             "error = %d!!\n",( int )rc );
      elite_msg_return_payload_buffer( &msg_afe_rx );
      break;
    }
    break;
  }
...
ADSPResult vsm_action_set_voice_timing_to_afe(
  vsm_session_object_t* session_object
)
{
  ADSPResult rc = ADSP_EOK;
  ...
  while( VSM_IS_PCIE_PORT(&session_object->target_set.pcie_config) ) 
  {
    /* Sending the timing offset to AFE Tx. */  
    size = sizeof( elite_msg_custom_afe_data_rw_offsets );
    rc = elite_msg_create_msg( &msg_afe_tx, &size, ELITE_CUSTOM_MSG,
    ...
    payload_ptr = ( elite_msg_custom_afe_data_rw_offsets* ) msg_afe_tx.pPayload;
    payload_ptr->sec_op_code = ELITEMSG_CUSTOM_VFR_AFE_DATA_RW_OFFSETS;
    payload_ptr->ul_offset_1 = session_object->target_set.voice_timing.vptx_proc_start_offset;
    payload_ptr->ul_offset_2 = session_object->target_set.voice_timing.vptx_buf_start_offset;
    payload_ptr->dl_offset_1 = session_object->target_set.voice_timing.vprx_delivery_offset;
    payload_ptr->dl_offset_2 = session_object->target_set.voice_timing.vprx_delivery_offset;
    payload_ptr->afe_id = session_object->target_set.pcie_config.tx_port_id;
    payload_ptr->vsid = session_object->target_set.voice_timing.vsid;
    payload_ptr->vfr_mode = session_object->target_set.voice_timing.vfr_mode;
    payload_ptr->vfr_cycle_us = session_object->target_set.voice_timing.vfr_cycle_duration_us;
    if ( ADSP_FAILED( rc = qurt_elite_queue_push_back( session_object->tx_afe_handle_ptr->cmdQ ,( uint64_t * )&msg_afe_tx ) ) )
    {
      MSG_1( MSG_SSID_DFLT, MSG_LEGACY_ERROR,
             "VSM: ACTION_SET_VOICE_TIMING_TO_AFE: Unable to push to Afe tx cmdQ, "
    ...
    payload_ptr->dl_offset_1 = session_object->target_set.voice_timing.vprx_delivery_offset;
    payload_ptr->dl_offset_2 = session_object->target_set.voice_timing.vprx_delivery_offset;
    payload_ptr->afe_id = session_object->target_set.pcie_config.rx_port_id;
    payload_ptr->vsid = session_object->target_set.voice_timing.vsid;
    payload_ptr->vfr_mode = session_object->target_set.voice_timing.vfr_mode;
    payload_ptr->vfr_cycle_us = session_object->target_set.voice_timing.vfr_cycle_duration_us;
    if ( ADSP_FAILED( rc = qurt_elite_queue_push_back( session_object->rx_afe_handle_ptr->cmdQ ,( uint64_t * )&msg_afe_rx ) ) )
...
)
{
  ...
  if( session_object->mixer_in_port_ptr != 0 )
  {
    ...
    session_object->mixer_in_port_ptr = NULL;
    session_object->cmd_control.issued_count++;
    session_object->cmd_control.is_resp_pending = TRUE;
  }
  ...
  if( session_object->mixer_out_port_ptr != NULL )
  {
    ...
    session_object->mixer_out_port_ptr = NULL;
    session_object->cmd_control.issued_count++;
    session_object->cmd_control.is_resp_pending = TRUE;
  }
...
ADSPResult vsm_action_disconnect_rx_downstream(
  vsm_session_object_t* session_object
)
{
  ADSPResult result = ADSP_EOK;
  ...
  if( ADSP_FAILED( result ) )
  {
    MSG( MSG_SSID_DFLT, MSG_LEGACY_HIGH, "vsm_action_disconnect_rx_downstream: FAILED to create EliteMsg!!\n" );
    return result;
  }

  elite_msg_cmd_connect_t* connect_payload_ptr = ( elite_msg_cmd_connect_t* )rx_connect_msg_t.pPayload;

  //TBD: Need to check why poiner is requried here.    	
  if ( session_object->mixer_in_port_ptr )
  {
  	connect_payload_ptr->pSvcHandle = &session_object->mixer_in_port_ptr->port_handle;  	
  	result = qurt_elite_queue_push_back( session_object->vdec->cmdQ,(uint64_t*)&rx_connect_msg_t );
  }
  ...
  if ( ADSP_FAILED( result ) )
  {
    MSG_1( MSG_SSID_DFLT, MSG_LEGACY_HIGH, "vsm_action_disconnect_rx_downstream: FAILED to push to vdec cmdQ. error %d!!\n", result );
    elite_msg_return_payload_buffer( &rx_connect_msg_t);
    return result;
  }
  session_object->cmd_control.issued_count++;
  ...
  MSG_2( MSG_SSID_DFLT, MSG_LEGACY_MED, "vsm_action_disconnect_rx_downstream: cmd issued count %d, resp count %d\n",
         session_object->cmd_control.issued_count, session_object->cmd_control.resp_count );

  return result;
}

ADSPResult vsm_action_connect_afe_rx_downstream(
  vsm_session_object_t* session_object
)
{
  ADSPResult result = ADSP_EOK;
  ...
  if( ADSP_FAILED( result ) )
  {
    MSG( MSG_SSID_DFLT, MSG_LEGACY_HIGH, "vsm_action_connect_afe_rx_downstream: FAILED to create EliteMsg !!\n ");
    return result;
  }
  elite_msg_cmd_connect_t* connect_payload_ptr = ( elite_msg_cmd_connect_t* )rx_connect_msg_t.pPayload;  
  connect_payload_ptr->pSvcHandle = session_object->rx_afe_handle_ptr;  
  result = qurt_elite_queue_push_back( session_object->vdec->cmdQ,(uint64_t*)&rx_connect_msg_t );
  if ( ADSP_FAILED( result ) )
  {
    MSG_1( MSG_SSID_DFLT, MSG_LEGACY_HIGH, "vsm_action_connect_afe_rx_downstream: FAILED to push to vdec cmdQ. error %d!!\n", result );
    elite_msg_return_payload_buffer( &rx_connect_msg_t);
    return result;
  }
  session_object->cmd_control.issued_count++;
  ...
  MSG_2( MSG_SSID_DFLT, MSG_LEGACY_MED, "vsm_action_connect_afe_rx_downstream: cmd issued count %d, resp count %d\n",
         session_object->cmd_control.issued_count, session_object->cmd_control.resp_count );

  return result;
}

ADSPResult vsm_action_disconnect_afe_rx_downstream(
  vsm_session_object_t* session_object
)
{
  ADSPResult result = ADSP_EOK;
  ...
  elite_msg_any_t rx_disconnect_msg_t;
  uint32_t size = sizeof( elite_msg_cmd_connect_t );
  result = elite_msg_create_msg( &rx_disconnect_msg_t,
        &size,
        ELITE_CMD_DISCONNECT,
        session_object->cmd_control.respQ,
        VSM_ASYNC_CMD_TOKEN_RX,
  ...
  if( ADSP_FAILED( result ) )
  {
    MSG( MSG_SSID_DFLT, MSG_LEGACY_HIGH, "vsm_action_disconnect_afe_rx_downstream: FAILED to create EliteMsg!!\n" );
    return result;
  }

  elite_msg_cmd_connect_t* connect_payload_ptr = ( elite_msg_cmd_connect_t* )rx_disconnect_msg_t.pPayload;  
  connect_payload_ptr->pSvcHandle = session_object->rx_afe_handle_ptr;  
  result = qurt_elite_queue_push_back( session_object->vdec->cmdQ,(uint64_t*)&rx_disconnect_msg_t );
  if ( ADSP_FAILED( result ) )
  {
    MSG_1( MSG_SSID_DFLT, MSG_LEGACY_HIGH, "vsm_action_disconnect_afe_rx_downstream: FAILED to push to vdec cmdQ. error %d!!\n", result );
    elite_msg_return_payload_buffer( &rx_disconnect_msg_t);
    return result;
  }
  session_object->cmd_control.issued_count++;
  session_object->cmd_control.is_resp_pending = TRUE;

  MSG_2( MSG_SSID_DFLT, MSG_LEGACY_MED, "vsm_action_disconnect_afe_rx_downstream: cmd issued count %d, resp count %d\n",
         session_object->cmd_control.issued_count, session_object->cmd_control.resp_count );
...
)
{
  ...
  if ( VSM_IS_PCIE_PORT(&session_object->target_set.pcie_config) )
  {
    return ADSP_EOK;
  }
...
)
{
  ...
  if (session_obj->vsm_timer_handle == NULL)
  {
    MSG(MSG_SSID_DFLT, MSG_LEGACY_ERROR,"VSM: RX_MAILBOX_PKTEXG: vsm_action_send_dtmf_gen_msg_to_afe:FAILED to send command to afe cmdQ due to NULL timer handle.");
    return ADSP_EFAILED;
  }
  ...
  for (i = 0; i < VSM_MAX_ATTACHED_VOCPROC_CNT;i++)
  {
    ...
    if (APR_NULL_V != afe_port)
    {
      ...
      time = qurt_elite_fwk_get_time(session_obj->vsm_timer_handle);
      ...
      time = time + rx_path_delay + (num_of_dec_pkts_per_vfr_cycle - mailbox_buffer_index) * 20000;
      ...
      payload_ptr->avtimer_timestamp_lsw = (uint32_t)time;
      payload_ptr->avtimer_timestamp_msw = (uint32_t)(time >>32);
