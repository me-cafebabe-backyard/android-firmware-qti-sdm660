/*
  ...
  $Header: //components/rel/avs.adsp/2.8/vsd/common/cvd/mvm/src/mvm_ccm.c#16 $
  $Author: pwbldsvc $
*/
...
static uint32_t mvm_ccm_get_cpu_clock_value( uint32_t option )
{
  ...
  {
    MSG( MSG_SSID_DFLT, MSG_LEGACY_ERROR, "mvm_ccm_get_cpu_clock_value(): Failed" );
    return 0;
  }
...
static int32_t mvm_ccm_get_clock_level_info( void )
{
  ...
  for ( i = 0; i < num_clk_perf_levels; i++)
  {
    ...
    MSG_2( MSG_SSID_DFLT, MSG_LEGACY_HIGH,
           ...
           "CLOCK_NPA_QUERY_PERF_LEVEL_KHZ[%d] = %d", i, clk_perf_level );
...
)
{
  ...
  if ( mvm_ccm_system_config.num_voice_sessions == 1 )
  {
    ...
    mvm_ccm_system_config.enc_kpps = 
      session_tracking_list_item->session_tracking_obj->enc_kpps;

    mvm_ccm_system_config.dec_kpps = 
      session_tracking_list_item->session_tracking_obj->dec_kpps;

    mvm_ccm_system_config.vp_tx_kpps = 
      session_tracking_list_item->session_tracking_obj->vp_tx_kpps;

    mvm_ccm_system_config.vp_rx_kpps = 
      session_tracking_list_item->session_tracking_obj->vp_rx_kpps;
...
bool_t mvm_csm_evaluate_sleep_possibility( void )
{
   ...
   for ( ;; ) 
   {
      timestamp_us = qurt_elite_fwk_get_time( mvm_timer_handle );

      /* Check if any of the dynamic threads are active. 
       */
      if ( ( mvm_csm_config.is_tx_active ) || ( mvm_csm_config.tx_wakeup < ( mvm_csm_config.threshold_us + timestamp_us ) ) )
      {
         rsp_status = FALSE;
         break;
      }
      if ( ( mvm_csm_config.is_rx_active ) || ( mvm_csm_config.rx_wakeup < ( mvm_csm_config.threshold_us + timestamp_us ) ) )
      {
         rsp_status = FALSE;
         break;
      }
...
bool_t mvm_csm_dl_sleep_manager()
{
  ...
  for ( ;; ) 
  {
    ...
    if ( mvm_csm_config.rx_wakeup < timestamp_us ) 
    {
      mvm_csm_config.rx_wakeup += mvm_csm_config.vfr_cycle_us; 
    }

    if ( mvm_csm_config.rx_wakeup > ( mvm_csm_config.threshold_us + timestamp_us ) )
    {  
      mvm_csm_config.is_rx_active = FALSE;
      ( void )apr_timer_start ( mvm_csm_rx_timer,
                              ( mvm_csm_config.rx_wakeup - timestamp_us - mvm_csm_config.safety_margin_us ) * 1000 );
      rc = FALSE;
      break;
    }
    break;
...
bool_t mvm_csm_ul_sleep_manager( void )
{
  ...
  for ( ;; ) 
  {
     ...
     else if ( ( mvm_csm_config.enc_pkt_count == ( mvm_csm_config.num_venc_instances - 1 ) ) && ( mvm_csm_config.vptx_frame_size == MVM_VOICE_FRAME_SIZE_US ) )
     {
        ...
        if ( mvm_csm_config.tx_wakeup > ( mvm_csm_config.threshold_us + timestamp_us ) )
        {
           mvm_csm_config.is_tx_active = FALSE;
           apr_timer_start ( mvm_csm_tx_timer,
                             ( mvm_csm_config.tx_wakeup - timestamp_us - mvm_csm_config.safety_margin_us ) * 1000 );
           rc = FALSE;
           break;
        }
     }
   ...
   if ( ( TRUE == enable ) && ( NULL != voice_timing ) && ( NULL != processing_times ) ) 
   {
      ...
      else
      {
        ...
        ( void ) apr_timer_start ( mvm_csm_enable_timer, 
                                   ( mvm_csm_config.tx_wakeup - timestamp_us ) * 1000 );
      }
   }
   mvm_csm_task_unlock_fn();
}
...
)
{
   ...
   for ( ;; ) 
   {
      ...
      switch ( req->client ) 
      {
         ...
         {
         	 /* Consider the devote from Vdec only if VpRx is not running. */
           if( ( req->vote == FALSE ) && ( 0 == mvm_ccm_system_config.vp_rx_kpps ) )
           {
