/*
  ...
  $Header: //components/rel/avs.adsp/2.8/vsd/common/cvd/mvm/src/mvm_module.c#36 $
  $Author: pwbldsvc $
*/
...
static uint32_t mvm_session_count = 0;
/****************************************************************************
...
)
{
  ...
  for ( ;; )
  {
    if ( session_obj->clock_and_concurrency_config.is_multi_session )
    {
      ...
      session_obj->voice_proc_delays.venc_proc = MVM_VOICE_FRAME_SIZE_US;
      session_obj->voice_proc_delays.vdec_proc = MVM_VOICE_FRAME_SIZE_US;
      session_obj->voice_proc_delays.vptx_proc = MVM_VOICE_FRAME_SIZE_US;
      session_obj->voice_proc_delays.vprx_proc = MVM_VOICE_FRAME_SIZE_US;
    }
    else
    {
      ...
      session_obj->voice_proc_delays.venc_proc =
        ( ( uint16_t ) ( ( session_obj->stream_vocproc_load.enc_kpps *
                           MVM_VOICE_FRAME_SIZE_US ) / total_core_kpps ) );

      session_obj->voice_proc_delays.vdec_proc =
        ( ( uint16_t ) ( ( session_obj->stream_vocproc_load.dec_kpps *
                           MVM_VOICE_FRAME_SIZE_US ) / total_core_kpps ) );

      session_obj->voice_proc_delays.vptx_proc =
        ( ( uint16_t ) ( ( session_obj->stream_vocproc_load.vp_tx_kpps *
                           MVM_VOICE_FRAME_SIZE_US ) / total_core_kpps ) );

      session_obj->voice_proc_delays.vprx_proc =
        ( ( uint16_t ) ( ( session_obj->stream_vocproc_load.vp_rx_kpps *
                           MVM_VOICE_FRAME_SIZE_US ) / total_core_kpps ) );

      MSG_5( MSG_SSID_DFLT, MSG_LEGACY_MED,
             "MVM: VOICE_PROCESSING_DELAYS: vptx_kpps: %d, enc_kpps:%d, "
             "dec_kpps:%d, vprx_kpps:%d, total_core_kpps:%d ",
             session_obj->stream_vocproc_load.vp_tx_kpps, session_obj->stream_vocproc_load.enc_kpps,
             session_obj->stream_vocproc_load.dec_kpps, session_obj->stream_vocproc_load.vp_rx_kpps,
             total_core_kpps );

      /* Additional safety margin in the VpTx, VpRx, Vdec and Venc processing times 
       * to take care of thread preemption delay and to compensate sample
       * slippling/stuffing. */
      if( session_obj->stream_vocproc_load.vp_tx_kpps > 0  )
      {
        session_obj->voice_proc_delays.vptx_proc += MVM_VOCPROC_PROCESSING_TIME_SAFETY_MARGIN_US;
      	session_obj->voice_proc_delays.vptx_proc += session_obj->voice_timing.vptx_preemption_safety_margin;
      }
      if( session_obj->stream_vocproc_load.vp_rx_kpps > 0  )
      {
        session_obj->voice_proc_delays.vprx_proc += MVM_VOCPROC_PROCESSING_TIME_SAFETY_MARGIN_US;
      	session_obj->voice_proc_delays.vprx_proc += session_obj->voice_timing.vprx_preemption_safety_margin;
      }
            	
      session_obj->voice_proc_delays.vdec_proc += session_obj->voice_timing.vdec_preemption_safety_margin;
      session_obj->voice_proc_delays.venc_proc += session_obj->voice_timing.venc_preemption_safety_margin;
      ...
      if(mvm_is_afe_port_fractional_rate(session_obj->stream_vocproc_load.tx_afe_port_sr))
      {
         session_obj->voice_proc_delays.vptx_proc  += MVM_FRACT_RESAMP_BUFFER_TIME_US;
      }

      if ( session_obj->voice_proc_delays.venc_proc > MVM_VOICE_FRAME_SIZE_US )
      {
        MSG_1( MSG_SSID_DFLT, MSG_LEGACY_ERROR,
               "mvm_calculate_processing_times(): enc processing time %d us "
               "exceeds 20ms", session_obj->voice_proc_delays.venc_proc );
        session_obj->voice_proc_delays.venc_proc = MVM_VOICE_FRAME_SIZE_US;
      }

      if ( session_obj->voice_proc_delays.vdec_proc > MVM_VOICE_FRAME_SIZE_US )
      {
        MSG_1( MSG_SSID_DFLT, MSG_LEGACY_ERROR,
               "mvm_calculate_processing_times(): dec processing time %d us "
               "exceeds 20ms", session_obj->voice_proc_delays.vdec_proc );
        session_obj->voice_proc_delays.vdec_proc = MVM_VOICE_FRAME_SIZE_US;
      }

      if ( session_obj->voice_proc_delays.vptx_proc > MVM_VOICE_FRAME_SIZE_US )
      {
        MSG_1( MSG_SSID_DFLT, MSG_LEGACY_ERROR,
               "mvm_calculate_processing_times(): vp_tx processing time %d us "
               "exceeds %d ms", session_obj->voice_proc_delays.vptx_proc );
        session_obj->voice_proc_delays.vptx_proc = MVM_VOICE_FRAME_SIZE_US;
      }

      if ( session_obj->voice_proc_delays.vprx_proc > MVM_VOICE_FRAME_SIZE_US )
      {
        MSG_1( MSG_SSID_DFLT, MSG_LEGACY_ERROR,
               "mvm_calculate_processing_times(): vp_rx processing time %d us "
               "exceeds 20ms", session_obj->voice_proc_delays.vprx_proc );
        session_obj->voice_proc_delays.vprx_proc = MVM_VOICE_FRAME_SIZE_US;
      }
    }
  ...
  MSG_4( MSG_SSID_DFLT, MSG_LEGACY_MED,
         "MVM: VOICE_PROCESSING_DELAYS: enc_proc_delay:%d us, dec_proc_delay:%d us, "
         "vptx_proc_delay:%d us, vprx_proc_delay:%d us",
         session_obj->voice_proc_delays.venc_proc, session_obj->voice_proc_delays.vdec_proc,
         session_obj->voice_proc_delays.vptx_proc, session_obj->voice_proc_delays.vprx_proc );
...
)
{
  ...
  if ( 0 == session_obj->stream_vocproc_load.vp_tx_kpps )
  {
    target_timing->vptx_proc_start_offset = 
    	( target_timing->vptx_buf_start_offset + MVM_VOICE_FRAME_SIZE_US ) % session_obj->vfr_info.vfr_cycle_duration_us;
  }   
  else
  {
...
)
{
  ...
  evt_voice_session_active.enc_kpps = 
    session_obj->stream_vocproc_load.enc_kpps;

  evt_voice_session_active.dec_kpps = 
    session_obj->stream_vocproc_load.dec_kpps;

  evt_voice_session_active.vp_tx_kpps = 
    session_obj->stream_vocproc_load.vp_tx_kpps;

  evt_voice_session_active.vp_rx_kpps = 
    session_obj->stream_vocproc_load.vp_rx_kpps;
...
)
{
  ...
  for ( ;; )
  {
    rc = mvm_calculate_processing_times( session_obj );
    if ( rc )
    {
...
#ifndef WINSIM
  time = qurt_elite_fwk_get_time( mvm_timer_handle );
  avtimer_timestamp.timestamp_us = time;
#endif
...
{
  int32_t rc = APR_EOK;
  uint64_t qtimer_timestamp_us, avtimer_timestamp_us;
  vss_iavtimer_rsp_get_offset_t avtimer_offset;
  uint16_t client_addr = packet->src_addr;

  for( ;; )
  {
    avtimer_timestamp_us = qurt_elite_fwk_get_time( mvm_timer_handle );
    if ( !avtimer_timestamp_us )
    {
      rc = __aprv2_cmd_end_command( mvm_apr_handle, packet, APR_EFAILED );
      MVM_COMM_ERROR( rc, client_addr);
      break;
    ...
    avtimer_offset.offset_us = (int64_t) (avtimer_timestamp_us - qtimer_timestamp_us);
    ...
    MSG_5( MSG_SSID_DFLT, MSG_LEGACY_MED,
           ...
           (uint32_t) (avtimer_timestamp_us >> 32), (uint32_t) avtimer_timestamp_us,
           (uint32_t) (qtimer_timestamp_us >> 32) , (uint32_t) qtimer_timestamp_us,
           (uint32_t) (avtimer_timestamp_us - qtimer_timestamp_us) );
    ...
    { /* Initialize the session object. */
      ...
      if ( is_full_ctrl )
      {
        if ( session_obj->master_addr == APRV2_PKT_INIT_ADDR_V )
        {
          ...
          if ( mvm_session_count == 0 ) 
          {
             /* Open the mvm_timer_handle for the very first request. */
             ( void ) qurt_elite_timer_open(&mvm_timer_handle, (char *)"MVMTIMER"); 
          }
          /* Increment mvm_session_count on subsequent open handle request. */
          mvm_session_count++;
        }
        else
  ...
  for( ;; )
  {
    if( ctrl->state == MVM_PENDING_CMD_STATE_ENUM_EXECUTE )
    {
                          ...
                          indirect_obj->access_bits ) )
      {
        session_obj->master_addr = APRV2_PKT_INIT_ADDR_V;
        session_obj->master_port = APR_NULL_V;
        if ( mvm_session_count == 1 ) 
        {
           /* Close the mvm_timer_handle when the session count goes to zero. */
           ( void ) qurt_elite_timer_close(mvm_timer_handle);
        }
        /* Decrement mvm_session_count on subsequent close handle request. */
        mvm_session_count--;
      }
