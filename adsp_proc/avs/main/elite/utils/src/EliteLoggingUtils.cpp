...
$Header: //components/rel/avs.adsp/2.8/elite/utils/src/EliteLoggingUtils.cpp#4 $
...
void * log_alloc_buffer(uint32_t buf_size, elite_qxdm_log_code log_code,
                        elite_log_data_format data_fmt, uint16_t pcm_log_version)
{
    ...
    if(ELITE_LOG_DATA_FMT_PCM == data_fmt)
    {
        ...
        if(pcm_log_version)
        {
          log_header_size = (sizeof(log_pkt_pcm_data_v2) - 1);
        }
        else
        {
...
ADSPResult elite_allocbuf_and_log_data_pkt(elite_log_info * log_info_ptr)
{
    ...
    log_pkt_pcm_data_v2            *pcm_data_pkt_ptr_v2;
    uint32_t                        total_log_buf_size;
    bool_t                      is_first_seg = TRUE;
    uint32_t                        num_channels = 0;
    uint8_t                     interleaved;
    uint16_t pcm_log_version = 0;
    uint32_t max_logging_size = ELITE_MAX_LOG_PKT_SIZE;
    ...
    if((ELITE_LOG_DATA_FMT_PCM == log_info_ptr->data_fmt) && (log_info_ptr->data_info.pcm_data_fmt.num_channels > ELITE_MAX_NUM_CH))
    {
      pcm_log_version = 1;
    }

	if(log_info_ptr->data_info.pcm_data_fmt.num_channels > 8 && (ELITE_LOG_DATA_FMT_PCM == log_info_ptr->data_fmt))
    {
      max_logging_size = max_logging_size/log_info_ptr->data_info.pcm_data_fmt.num_channels;
      max_logging_size = max_logging_size/(log_info_ptr->data_info.pcm_data_fmt.bits_per_sample >> 3);
      max_logging_size *= (log_info_ptr->data_info.pcm_data_fmt.bits_per_sample >> 3) * log_info_ptr->data_info.pcm_data_fmt.num_channels;
    }
    /*  If the buffer size to log is > max_logging_size, logging per iteraton
        is restricted to this size.
    */
    while(rem_log_buf_size > 0)
    {
        log_buf_size = s32_min_s32_s32(max_logging_size, rem_log_buf_size);

        log_pkt_ptr = log_alloc_buffer(log_buf_size, log_info_ptr->qxdm_log_code, log_info_ptr->data_fmt, pcm_log_version);
        ...
        if(pcm_log_version)
        {
          pcm_data_pkt_ptr_v2 = (log_pkt_pcm_data_v2 *)log_pkt_ptr;
          log_header_cmn = &(pcm_data_pkt_ptr_v2->log_header_pcm.cmn_struct);
        }
        else
        {
        ...
        */
        ...
        if( (max_logging_size == log_buf_size) &&
             (total_log_buf_size > max_logging_size) &&
             (TRUE == is_first_seg) &&
...
ADSPResult elite_log_buffer(void *log_pkt_ptr, elite_log_info  *log_info_ptr)
{
    ...
    audiolog_pcm_data_format_v2            *pcm_data_fmt_v2;
    ...
    if(log_info_ptr->data_info.pcm_data_fmt.num_channels > ELITE_MAX_NUM_CH)
    {
      pcm_data_pkt_ptr_v2 = (log_pkt_pcm_data_v2 *)log_pkt_ptr;
    ...
    if(ELITE_LOG_DATA_FMT_PCM == log_info_ptr->data_fmt)
    {
        if(log_info_ptr->data_info.pcm_data_fmt.num_channels > ELITE_MAX_NUM_CH)
        {
          log_header_cmn->size = sizeof(audiolog_header_pcm_data_v2);
        }
        else
        {
    ...
    case ELITE_LOG_DATA_FMT_PCM:
        {
          ...
          if(log_info_ptr->data_info.pcm_data_fmt.num_channels > ELITE_MAX_NUM_CH)
          {
            pcm_data_fmt_v2 = (audiolog_pcm_data_format_v2 *)(&(pcm_data_pkt_ptr_v2->log_header_pcm.pcm_data_fmt));

            pcm_data_info_ptr = (pcm_data_info *)(&log_info_ptr->data_info.pcm_data_fmt);

            pcm_data_fmt_v2->minor_version = 1;
            pcm_data_fmt_v2->tag = AUDIOLOG_CONTAINER_PCM_DATA_FORMAT;
            pcm_data_fmt_v2->size = sizeof(audiolog_pcm_data_format_v2);
            pcm_data_fmt_v2->log_tap_id = log_info_ptr->log_tap_id;
            pcm_data_fmt_v2->sampling_rate = pcm_data_info_ptr->sampling_rate;
            pcm_data_fmt_v2->num_channels = pcm_data_info_ptr->num_channels;
            pcm_data_fmt_v2->pcm_width = pcm_data_info_ptr->bits_per_sample;
            pcm_data_fmt_v2->interleaved = pcm_data_info_ptr->interleaved;

            if(NULL != pcm_data_info_ptr->channel_mapping)
            {
                for(i=0; i <  pcm_data_info_ptr->num_channels; i++)
                {
                  pcm_data_fmt_v2->channel_mapping[i] = pcm_data_info_ptr->channel_mapping[i];
                }
            }
            else /* Provide the default mapping */
            {
                for(i=0; i <  pcm_data_info_ptr->num_channels; i++)
                {
                  pcm_data_fmt_v2->channel_mapping[i] = i+1;
                }
            }

            /* Set the remaining channels elements as un-used */
            for(i =  pcm_data_info_ptr->num_channels; i < ELITE_MAX_NUM_CH_V2; i++)
            {
              pcm_data_fmt_v2->channel_mapping[i] = 0;
            }

            // MSG_6(MSG_SSID_QDSP6, DBG_HIGH_PRIO, "DEBUG : EliteLoggingUtil : channel map pointer by client : %d and channel mapping for 0th 7th 15th and 31st channel is %d  %d  %d  %d for QXDM log code 0x%X", (int)pcm_data_info_ptr->channel_mapping,
            //                     (int)pcm_data_fmt_v2->channel_mapping[0], (int)pcm_data_fmt_v2->channel_mapping[7] , (int)pcm_data_fmt_v2->channel_mapping[15] , (int)pcm_data_fmt_v2->channel_mapping[31],  log_info_ptr->qxdm_log_code);


            log_dst_ptr = ((log_pkt_pcm_data_v2 *)log_pkt_ptr)->payload;
          }
          else
          {
