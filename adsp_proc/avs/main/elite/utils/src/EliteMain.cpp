...
ADSPResult elite_framework_init(void)
{
   ...
   //This has to be called only after afe_main_init(), depends on avtimer_init()
   if (ADSP_FAILED(qurt_elite_timer_init()))
   {
      MSG(MSG_SSID_QDSP6, DBG_HIGH_PRIO, "FAILED to init qurt_elite_timer!");
      return ADSP_EFAILED;
   }
   MSG(MSG_SSID_QDSP6, DBG_HIGH_PRIO, "qurt_elite_timer is initialized");
