...
$Header: //components/rel/avs.adsp/2.8/elite/qurt_elite/src/qurt_elite_memory.cpp#4 $
...
/**
Allocated aligned pointer from heap manager for a given heap.

@param[in] unBytes      Size to be alloc'ed
@param[in] unAlignSize  Alignment size of the pointer
@param[in] heapId       Heap ID to alloc from
@return
    Pointer to the allocated memory
 */

void *qurt_elite_memory_heapmgr_aligned_malloc(uint32_t unBytes, uint32_t unAlignSize, QURT_ELITE_HEAP_ID heapId)
{
   // only allow alignment to 4 or more bytes.
   if (unAlignSize <= 4) unAlignSize = 4;
  ...
  //validate the heap ID with the range
  if ( (heapId < QURT_ELITE_HEAP_MGR_HEAP_INDEX_START) || (heapId > QURT_ELITE_HEAP_MGR_HEAP_INDEX_END))
  {
    MSG_1(MSG_SSID_QDSP6, DBG_ERROR_PRIO, "heap id %d out of range!", heapId);
    return NULL;
  }

  // check if there is an existing heap with given heap-id
  if ( qurt_elite_heap_table[heapId].used_flag != 1)
  {
    MSG_1(MSG_SSID_QDSP6, DBG_ERROR_PRIO, "trying to allocate memory from non-existing heap %d !!", heapId);
    return NULL;
  }

  /* Use memalign since the wrapper handles alignment of the headers for the current block
      being allocated and the trailing block as well   */
  return mem_memalign(&heap_state[heapId], unAlignSize, unBytes);

}

/**
Free aligned pointer from heap manager for a given heap.

@param[in] ptr          Pointer to be freed
@param[in] heapId       Heap ID to be freed from
@return
    None
 */

void qurt_elite_memory_heapmgr_aligned_free(void *ptr, QURT_ELITE_HEAP_ID heapId)
{
   if (NULL == ptr)
   {
      MSG_1(MSG_SSID_QDSP6, DBG_ERROR_PRIO, "Thread 0x%x trying to free null pointer!", qurt_thread_get_id());
  ...
  qurt_elite_memory_heapmgr_free(ptr, heapId);
}
...
ADSPResult qurt_elite_memory_heapmgr_create(QURT_ELITE_HEAP_ID *heap_id_ptr, void *heap_start_ptr, uint32_t heap_size)
{
   ...
   *heap_id_ptr = (QURT_ELITE_HEAP_ID)heap_idx;
   ...
   //To keep KW happy, explicitly assign the array element to the pointer of right type
   mem_heap_type *mem_heap_state_ptr = &(heap_state[heap_idx]); 
   ...
   mem_init_heap(mem_heap_state_ptr,
                 heap_start_ptr,
                 heap_size,
                 NULL);
...
ADSPResult qurt_elite_memory_heapmgr_delete(QURT_ELITE_HEAP_ID heap_id)
{
   ...
   //To keep KW happy, explicitly assign the array element to the pointer of right type
   uint32_t index = (uint32_t)heap_id;
   mem_heap_type *mem_heap_state_ptr = &(heap_state[index]); 

   //Deinit the heap
   mem_deinit_heap(mem_heap_state_ptr);
