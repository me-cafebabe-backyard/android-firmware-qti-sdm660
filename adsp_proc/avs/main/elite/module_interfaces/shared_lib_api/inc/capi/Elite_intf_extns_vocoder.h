...
/** ID of the parameter that identifies if BeAMR is to be enabled.
  ...
  @table{weak__voc__param__beamr__t} @newpage
 */
...
/** ID of the parameter used to reset the voice encoder or decoder.

    This parameter can be used to reset the vocoder whenever the client sends a
    homing sequence to the network. Because the homing sequence resets the
    far-end decoder, the client uses this parameter ID to reset the near-end
    encoder so the encoder and decoder are synchronized.

    @msgpayload{voc_param_voc_reset_t}
    @table{weak__voc__param__voc__reset__t}
 */
#define VOC_PARAM_VOC_RESET             (0x00010EB3)

typedef struct voc_param_voc_reset_t voc_param_voc_reset_t;
/** @weakgroup weak_voc_param_voc_reset_t
@{ */
struct voc_param_voc_reset_t
{
   uint16_t reset_encoder;
   /**< Specifies whether to reset the encoder.

        @values
        - 0 -- Do not reset
        - 1 -- Reset @tablebulletend */

   uint16_t reset_decoder;
   /**< Specifies whether to reset the decoder.

        @values
        - 0 -- Do not reset
        - 1 -- Reset @tablebulletend */
};
/** @} *//* end_weakgroup weak_voc_param_voc_reset_t */
