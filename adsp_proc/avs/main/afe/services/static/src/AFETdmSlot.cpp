		...
		int8_t *param_buffer_ptr, uint16_t param_size)
{
	...
	afe_param_id_slot_mapping_cfg_v2_t *cfg_params;
	...
	case AFE_PARAM_ID_PORT_SLOT_MAPPING_CONFIG:
	{
		...
		if(0x1 == slot_mapping_cfg_version)
		{
			...
			cfg_params     = (afe_param_id_slot_mapping_cfg_v2_t*)param_buffer_ptr;
			...
			for(i = 0; i < AFE_PORT_EIGHT_CHANNELS; i++)
			{
				MSG_2(MSG_SSID_QDSP6, DBG_HIGH_PRIO, "Array [Index:%d] slot slot mapping [slot_offset:0x%x]", i, cfg_params->offset[i]);
			}

			//Error check
			if((1 > cfg_params->num_channel) || (AFE_PORT_EIGHT_CHANNELS < cfg_params->num_channel))
			{
		...
		else if(slot_mapping_cfg_version >= 0x2)
 		{
			if (param_size < sizeof(afe_param_id_slot_mapping_cfg_v2_t))
			{
				MSG_2(MSG_SSID_QDSP6, DBG_ERROR_PRIO, "AFE TDM Slot Cfg fail: Invalid payload size: %d, port_id: 0x%x", param_size, pDevPort->intf);
				return ADSP_EBADPARAM;
			}
			...
			//Error check
			if((1 > cfg_params->num_channel) || (AFE_PORT_MAX_AUDIO_CHAN_CNT < cfg_params->num_channel))
			{
				MSG_2(MSG_SSID_QDSP6, DBG_ERROR_PRIO, "AFE TDM Slot Cfg fail: num of channel is not supported: port_id: 0x%x num_channel: %d", pDevPort->intf, cfg_params->num_channel);
				return ADSP_EBADPARAM;
			}
	...
	case AFE_PARAM_ID_CUSTOM_TDM_HEADER_CONFIG:
	{
		...
		if(0x1 == tdm_header_cfg_version)
		{
			...
			{
				if(NULL == (pDevPort->custom_header_info.p_header = (void *) qurt_elite_memory_aligned_malloc(cfg_params->header_width * cfg_params->num_frame_repeat, 8, QURT_ELITE_HEAP_DEFAULT)))
				{
			...
			if(pDevPort->p_group_device_state && (AFE_CUSTOM_TDM_HEADER_TYPE_DEFAULT == pDevPort->custom_header_info.header_type))
			{
				...
				if(FALSE == p_group_device->default_header_backup.is_cfg_received)
				{
					...
					if(NULL == (p_group_device->default_header_backup.p_header = (void *) qurt_elite_memory_aligned_malloc(cfg_params->header_width * cfg_params->num_frame_repeat, 8, QURT_ELITE_HEAP_DEFAULT)))
					{
