/*========================================================================
  ...
  Copyright (c) 2009 -2015, 2018 QUALCOMM Technologies, Inc. (QTI).  All Rights Reserved.
  QUALCOMM Proprietary.  Export of this technology or software is regulated
  by the U.S. Government, Diversion contrary to U.S. law prohibited.

  $Header: //components/rel/avs.adsp/2.8/afe/services/static/src/AFEPortHandlers.cpp#13 $
 ====================================================================== */
...
static void afe_port_aggregate_sample_slip_kpps(void *dev_ptr, void *client_info_ptr);
                                          ...
                                          int16_t *is_queue_init)
{
  ...
  *is_queue_init = TRUE;
  ...
  if(client_buf_size > scratch_buf_size)
  {
    ...
    if(AFE_PORT_STATE_RUN == pDevPort->port_state)
    {
      if(NULL == (scratch_buf_new = (int8_t *)qurt_elite_memory_aligned_malloc(scratch_buf_size, 8, QURT_ELITE_HEAP_DEFAULT)))
      {
...
void afe_port_update_aggregated_client_kpps_and_bw(void *dev_ptr, void *client_info_ptr, bool_t is_client_connect)
{
  ...
  if(TRUE == is_client_connect)
  {
    ...
    // Include sample slip kpps. Sample slip is active if rate_match_info_ptr exists. 
    if(psNewClient->rate_match_info_ptr != NULL) {
      afe_port_aggregate_sample_slip_kpps(pDevPort, psNewClient);
    }
  }
  else
  {
...
static void afe_port_aggregate_sample_slip_kpps(void *dev_ptr, void *client_info_ptr)
{
   ...
   afe_rate_match_info_t *rate_match_info = psNewClient->rate_match_info_ptr;
   afe_mmpm_info_t *mmpm_info_ptr = (afe_mmpm_info_t *)pDevPort->mmpm_info_ptr;

   if((NULL == mmpm_info_ptr) || (NULL == rate_match_info))
   {
      MSG_2(MSG_SSID_QDSP6, DBG_ERROR_PRIO,
            "mmpm_info_ptr/rate_match_info pointer is/are NULL, mmpm_info_ptr: 0x%x, rate_match_info ptr: 0x%x",
            mmpm_info_ptr, rate_match_info);
      return;
   }

   rate_match_ss_lib_info_t *ss_lib_info = &(rate_match_info->ss_lib_info);
   if(ss_lib_info->use_lib)
   {
      psNewClient->client_kpps += ss_lib_info->kpps;
      mmpm_info_ptr->dyn_vot_info.agr_client_kpps += ss_lib_info->kpps;
   }

   MSG_1(MSG_SSID_QDSP6, DBG_HIGH_PRIO, "AFE MPPS:: client_sample_slip_kpps: %lu",
         ss_lib_info->kpps);
}
...
ADSPResult afe_port_vfr_data_rw_offsets(afe_dev_port_t *p_dev_port, elite_msg_any_t *pMsg)
{
   ADSPResult result = ADSP_EOK;
   if( IS_PCIE_PORT_AFE_ID(p_dev_port->intf) )
   {
   	 result = afe_dev_pcie_set_timing_offset(p_dev_port,pMsg);
   }
   else
   {
   	 result = ADSP_EBADPARAM;
     MSG_1(MSG_SSID_QDSP6, DBG_ERROR_PRIO, "Improper port ID passed for VFR data driven offsets, port_id: 0x%lx",
         p_dev_port->intf);
   }
