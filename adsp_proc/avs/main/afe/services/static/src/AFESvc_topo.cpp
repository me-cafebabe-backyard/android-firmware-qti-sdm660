...
/*========================================================================
 $Header: //components/rel/avs.adsp/2.8/afe/services/static/src/AFESvc_topo.cpp#10 $
...
/* Codec register operation virtual table */
extern capi_v2_cdc_reg_op_vtbl_t cdc_reg_vtbl;
...
/*
  ...
  None.
*/
capi_v2_err_t afe_topo_event_callback(void *context_ptr,
                                      ...
                                      capi_v2_event_info_t *event_ptr)
{
   ...
   if (!event_ptr)
   {
      MSG_1(MSG_SSID_QDSP6, DBG_ERROR_PRIO,
            "AFE Svc topo evt: Null ptr event_ptr(0x%lx)", event_ptr);
      return CAPI_V2_EFAILED;
   }

   /* For modules involving BG thread, during topo destroy, topo
      struct is destroyed but BGT module is destroyed after it
      has finished any ongoing data frame processing. This implies
      that topo struct could be destroyed while BG thread is in process
      state.
      For BGT thread, this context pointer is set to NULL by default.
      Regardless of topo state, BG thread invokes this CB with
      a NULL context pointer. 
    
      The null pointer checks added below is for allowing the event callback
      handling processing to finish even with NULL context pointer so that
      BGT thread is not stuck. */

   /* If the context pointer is non-NULL */
   if (context_ptr)
   {
      topo_ptr = topo_event_get_struct_ptr(context_ptr);

      if (!topo_ptr)
      {
         MSG_1(MSG_SSID_QDSP6, DBG_ERROR_PRIO,
               "AFE Svc topo evt: Null topo_ptr(0x%lx)", topo_ptr);

         return CAPI_V2_EFAILED;

      }

      module_index = topo_event_get_module_index(context_ptr);

      if (module_index >= topo_ptr->num_modules)
      {
         MSG_1(MSG_SSID_QDSP6, DBG_ERROR_PRIO,
               "AFE Svc topo evt: Module with id 0x%lx array out of bound",
               module_index);
   ...
   switch (id)
   {
         ...
         if (event_ptr->payload.actual_data_len < sizeof(capi_v2_event_KPPS_t))
         {
            if (topo_ptr)
            {
               MSG_4(MSG_SSID_QDSP6, DBG_ERROR_PRIO,
                     "AFE Svc topo evt: Module with id 0x%lX provided insufficient size for event id %lu."
                     " Required: %d bytes, provided: %lu bytes.",
                     topo_ptr->module[module_index].module_id, (uint32_t)(id),
                     sizeof(capi_v2_event_KPPS_t), event_ptr->payload.actual_data_len);
      ...
      case CAPI_V2_EVENT_BANDWIDTH:
      {
         if (event_ptr->payload.actual_data_len < sizeof(capi_v2_event_bandwidth_t))
         {
            ...
            {
               MSG_4(MSG_SSID_QDSP6, DBG_ERROR_PRIO,
                     "AFE Svc topo evt: Module with id 0x%lX provided insufficient size"
                     " for event id %lu. Required: %d bytes, provided: %lu bytes.",
                     topo_ptr->module[module_index].module_id, (uint32_t)(id),
                     sizeof(capi_v2_event_bandwidth_t), event_ptr->payload.actual_data_len);
            }
            return CAPI_V2_ENEEDMORE;
         }
         ...
         if (topo_ptr)
         {
            ...
            // Currently assuming AFE will not have special handling for
            // keeping code in spl memory such as OCMEM
      ...
      case CAPI_V2_EVENT_ALGORITHMIC_DELAY:
      {
         if (event_ptr->payload.actual_data_len < sizeof(capi_v2_event_algorithmic_delay_t))
         {
            if (topo_ptr)
            {
               MSG_4(MSG_SSID_QDSP6, DBG_ERROR_PRIO,
                     ...
                     static_cast<uint32_t>(id), sizeof(capi_v2_event_algorithmic_delay_t),
         ...
         if (topo_ptr)
         {
            ...
            MSG_5(MSG_SSID_QDSP6, DBG_HIGH_PRIO,
                  "AFE Svc topo evt: Module with id 0x%lX changed delay from %lu us to %lu us."
                  " PP total delay changed from %lu us to %lu us.",
                  topo_ptr->module[module_index].module_id, old_delay,
                  new_delay, old_total_delay, new_total_delay);
         }
      ...
      case CAPI_V2_EVENT_DATA_TO_DSP_SERVICE:
      {
         ...
         capi_v2_event_data_to_dsp_service_t *data_ptr = (capi_v2_event_data_to_dsp_service_t *)\
            event_ptr->payload.data_ptr;
      ...
      default:
      {
         ...
         {
            MSG_2(MSG_SSID_QDSP6, DBG_ERROR_PRIO,
                  "AFE Svc topo evt: Event id %lu raised by module 0x%lx is not supported.",
...
ADSPResult afe_set_topo_id(void *dev_port_ptr, int8_t *cfg_ptr, uint32_t payload_size)
{
   ...
   else
   {
      ...
      uint32_t size_req = sizeof(elite_cmn_topo_db_entry_t) + (num_modules  * sizeof(avcs_module_instance_proc_info_t));
      ...
      case TOPO_CB_TYPE_PRE:
      {
         ...
         for (uint32_t num_extn = 0; num_extn < topo_node_ptr->num_extensions.num_extensions; num_extn++)
         {
            switch (topo_node_ptr->fwk_extn_ptr[num_extn].id)
            {
               ...
               case FWK_EXTN_CDC_REG_OP:
               {
                  /* Set codec register operation virtual table*/
                  capi_v2_cdc_reg_op_vtbl_t *cdc_reg_vtbl_ptr = &cdc_reg_vtbl;
                  cfg_buf.data_ptr = (int8_t *)&cdc_reg_vtbl_ptr;
                  cfg_buf.max_data_len = sizeof(capi_v2_cdc_reg_op_vtbl_t *);
                  cfg_buf.actual_data_len = sizeof(capi_v2_cdc_reg_op_vtbl_t *);
                  module_ptr->vtbl_ptr->set_param(module_ptr, CAPI_V2_PARAM_ID_CDC_REG_OP_VTBL, &port_info,
                                                  &cfg_buf);
