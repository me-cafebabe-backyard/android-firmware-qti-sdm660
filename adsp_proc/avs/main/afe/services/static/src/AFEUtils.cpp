/*========================================================================
  ...
  $Header: //components/rel/avs.adsp/2.8/afe/services/static/src/AFEUtils.cpp#11 $
 ====================================================================== */
...
ADSPResult afe_svc_get_port(uint16 unPortId, afe_dev_port_t **pDevPort)
{
    ...
    else if (IS_PCIE_PORT_AFE_ID(unPortId))
    {
        *pDevPort = afe_svc_global_ptr->ports_pcie_ptr[GET_PCIE_PORT_INDEX(unPortId)];
    }
	else
	{
		*pDevPort = NULL;
...
ADSPResult afe_port_get_frame_size(uint32_t latency_mode, uint32_t sample_rate, uint32_t *num_samples_ptr)
{
		...
		case AFE_PORT_SAMPLE_RATE_384K:
    {
      ...
      switch(latency_mode)
      {
        ...
        break;          
        case AFE_PORT_HIGH_LATENCY_MODE_20MS:
        {
          *num_samples_ptr = 20 * sample_rate/1000;          
        }  
        break;
        default:
          break;
