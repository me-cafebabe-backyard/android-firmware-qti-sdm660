/*========================================================================
  ...
  $Header: //components/rel/avs.adsp/2.8/afe/services/static/src/AFEAvSync.cpp#8 $
 ====================================================================== */
		...
		if (AFE_UNCOMPRESSED_DATA_PORT == pDevPort->port_data_type)
		{
			...
			for (loop_index = 0; loop_index < total_samples; loop_index++)
			{
				if(0 != buffer[loop_index])
				{
					...
					{
						av_stats_ptr->nonzero_timestamp = qurt_elite_fwk_get_time(pDevPort->avt_drv) - ((uint64_t)(pDevPort->int_samples_per_period * NUM_US_PER_SEC)/pDevPort->sample_rate) + ((uint64_t)((uint64_t)(pDevPort->int_samples_per_period * NUM_US_PER_SEC) * loop_index)/pDevPort->sample_rate)/total_samples;
					}
					else
					{
						av_stats_ptr->nonzero_timestamp = qurt_elite_fwk_get_time(pDevPort->avt_drv) + ((uint64_t)(pDevPort->int_samples_per_period * NUM_US_PER_SEC)/pDevPort->sample_rate) + ((uint64_t)((uint64_t)(pDevPort->int_samples_per_period * NUM_US_PER_SEC) * loop_index)/pDevPort->sample_rate)/total_samples;
			...
			while(loop_index < (pDevPort->int_samples_per_period * pDevPort->channels))
			{
				if((Pa == bufferPtr[loop_index]) && (Pb == bufferPtr[loop_index + 1]))
				{
					...
					if((0 != type) && (3 != type))
					{
						if (IS_IT_TX_PORT_AFE_ID(pDevPort->intf))
						{
							av_stats_ptr->nonzero_timestamp = qurt_elite_fwk_get_time(pDevPort->avt_drv)- ((uint64_t)(pDevPort->int_samples_per_period * NUM_US_PER_SEC)/pDevPort->sample_rate) + ((uint64_t)((uint64_t)(pDevPort->int_samples_per_period * NUM_US_PER_SEC) * loop_index)/pDevPort->sample_rate)/(pDevPort->int_samples_per_period * pDevPort->channels);
						}
						else
						{
							av_stats_ptr->nonzero_timestamp = qurt_elite_fwk_get_time(pDevPort->avt_drv)+ ((uint64_t)(pDevPort->int_samples_per_period * NUM_US_PER_SEC)/pDevPort->sample_rate) + ((uint64_t)((uint64_t)(pDevPort->int_samples_per_period * NUM_US_PER_SEC) * loop_index)/pDevPort->sample_rate)/(pDevPort->int_samples_per_period * pDevPort->channels);
...
void afe_port_calc_client_gp_delay_us(afe_client_info_t *client)
{
	...
	if(NULL != client->pop_suppressor_module_ptr)
	{
  	...
  	/**<If sample slip is present then update sample-slip algorithmic delay in client group delay.*/
  	if(NULL != client->rate_match_info_ptr && client->rate_match_info_ptr->ss_lib_info.use_lib)
  	{
  		client->client_gp_delay_us += client->rate_match_info_ptr->ss_lib_info.algo_delay_us;
  	}
...
ADSPResult afe_port_set_group_delay(afe_dev_port_t *pDevPort)
{
    ...
    {
	    uint64_t             data_path_delay_us = 0;
	    afe_driver_pcie_get_data_path_delay(pDevPort, &data_path_delay_us);
		group_delay_us = pDevPort->port_cmn_group_delay_us + data_path_delay_us;
	}
	else
	{
