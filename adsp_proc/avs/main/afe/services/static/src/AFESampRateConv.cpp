/*========================================================================
  ...
  $Header: //components/rel/avs.adsp/2.8/afe/services/static/src/AFESampRateConv.cpp#8 $
 ====================================================================== */
...
ADSPResult sample_rate_conv_init_iir(samp_rate_conv_t *samp_rate_conv, uint16_t num_channels,  uint32_t  bit_width, uint32_t in_freq, uint32_t out_freq)
{
  ...
  /* 1. CAPI_V2_INPUT_MEDIA_FORMAT supports only upto 16 channels.
     2. CAPI_V2_INPUT_MEDIA_FORMAT_V2 is introduced to support upto 32 channels.
     3. To support backward compatibility, first send CAPI_V2_INPUT_MEDIA_FORMAT_V2 to the module.
        If the module returns error and number of channels are less than 16 then send CAPI_V2_INPUT_MEDIA_FORMAT
        else return error.
     4. If CAPI_V2_INPUT_MEDIA_FORMAT also returns error then return error.
  */

  media_format_v2_t   media_formatv2;
  uint32_t actual_payload_len;
  media_formatv2.d.data_format = CAPI_V2_FIXED_POINT;
  media_formatv2.f.minor_version = CAPI_V2_MEDIA_FORMAT_MINOR_VERSION;
  media_formatv2.f.bits_per_sample = (bit_width > AFE_BITS_PER_SAMPLE_16)? AFE_BITS_PER_SAMPLE_32: AFE_BITS_PER_SAMPLE_16;
  media_formatv2.f.num_channels = num_channels;
  media_formatv2.f.bitstream_format = CAPI_V2_FIXED_POINT;
  media_formatv2.f.data_interleaving = CAPI_V2_DEINTERLEAVED_UNPACKED;
  media_formatv2.f.q_factor = elite_data_get_q_format_factor(bit_width);
  media_formatv2.f.data_is_signed = TRUE;
  media_formatv2.f.sampling_rate = in_freq;

  actual_payload_len = sizeof(media_formatv2.d) + sizeof(media_formatv2.f) + media_formatv2.f.num_channels * sizeof(media_formatv2.channel_type[0]);
  actual_payload_len = ELITE_ALIGN_BY_4(actual_payload_len);
  capi_v2_prop_t prop =
  { CAPI_V2_INPUT_MEDIA_FORMAT_V2, /* capi_v2_property_id_t */
      /* capi_v2_buf_t */
      { reinterpret_cast<int8_t*>(&media_formatv2),
          actual_payload_len,
          sizeof(media_formatv2) },
      /* capi_v2_port_info_t*/
      {
          TRUE,
          TRUE, 0,
  ...
  if(NULL != resampler_ptr->vtbl_ptr)
  {
    ...
    if(CAPI_V2_FAILED(result))
    {
      if(media_formatv2.f.num_channels <= CAPI_V2_MAX_CHANNELS)
      {
        media_format_v1_t *pMedia_format = (media_format_v1_t *)(&media_formatv2);

        pMedia_format->d.data_format = CAPI_V2_FIXED_POINT;
        pMedia_format->f.bits_per_sample = (bit_width > AFE_BITS_PER_SAMPLE_16)? AFE_BITS_PER_SAMPLE_32: AFE_BITS_PER_SAMPLE_16;
        pMedia_format->f.num_channels = num_channels;
        pMedia_format->f.bitstream_format = CAPI_V2_FIXED_POINT;
        pMedia_format->f.data_interleaving = CAPI_V2_DEINTERLEAVED_UNPACKED;
        pMedia_format->f.q_factor = elite_data_get_q_format_factor(bit_width);
        pMedia_format->f.data_is_signed = TRUE;
        pMedia_format->f.sampling_rate = in_freq;

        capi_v2_prop_t prop =
        { CAPI_V2_INPUT_MEDIA_FORMAT, /* capi_v2_property_id_t */
            /* capi_v2_buf_t */
            { reinterpret_cast<int8_t*>(pMedia_format),
                sizeof(*pMedia_format),
                sizeof(*pMedia_format) },
            /* capi_v2_port_info_t*/
            {
                TRUE,
        ...
        {
          pMedia_format->f.sampling_rate = out_freq;
    ...
    {
      media_formatv2.f.sampling_rate = out_freq;
                                 ...
                                 uint32_t in_freq, uint32_t out_freq, samp_rate_conv_type_t resamp_type)
{
  ...
  switch(resamp_type)
  {
    case FIR_BASED_RESAMPLER:
    {
      ...
      MSG_1(MSG_SSID_QDSP6, DBG_HIGH_PRIO, "AFE sample rate conv: FIR resampler sample rate %lu", media_formatv2.f.sampling_rate);
      capi_v2_prop_t prop =
      { CAPI_V2_INPUT_MEDIA_FORMAT_V2, /* capi_v2_property_id_t */
          /* capi_v2_buf_t */
          { reinterpret_cast<int8_t*>(&media_formatv2),
