/*========================================================================
  ...
  $Header: //components/rel/avs.adsp/2.8/afe/drivers/hw/devcfg/src/hwd_devcfg.c#5 $
 ====================================================================== */
...
typedef struct
{
   ...
   hw_adaptive_filter_prop_t adaptive_filter_prop;
...
ADSPResult HwdDevCfg_Init(void)
{
   ...
   else if (ADSP_EOK != (rc = HwdDevCfg_ReadAdaptiveFilterData(&(hwdDevCfgProp.adaptive_filter_prop))))
   {
      MSG(MSG_SSID_QDSP6, DBG_ERROR_PRIO, "DevCfg: HwdDevCfg_ReadAdaptiveFilterData failed");
   }
   else
   {
      devCfgInitFlag = TRUE;
   ...
   if (NULL != pDev)
   {
      if (devCfgInitFlag)
      {
         ...
         switch (moduleType)
         {
            ...
            case HWD_MODULE_TYPE_ADAPTIVE_FILTER:
            {
               memscpy(pDev, sizeof(hw_adaptive_filter_prop_t),
                       (void *)&(hwdDevCfgProp.adaptive_filter_prop), sizeof(hw_adaptive_filter_prop_t));
               break;
            }
            default:
            {
...
boolean HwdDevCfg_HWSupport(HwdModuleType moduleType)
{
   ...
   if (devCfgInitFlag)
   {
      switch (moduleType)
      {
         ...
         case HWD_MODULE_TYPE_ADAPTIVE_FILTER:
         {
            if (0 != hwdDevCfgProp.adaptive_filter_prop.hw_revision)
            {
               result = TRUE;
            }
            break;
...
static ADSPResult HwdDevCfg_ReadAdaptiveFilterData(hw_adaptive_filter_prop_t* pDev)
{
   DALSYS_PROPERTY_HANDLE_DECLARE(hProp);
   DALSYSPropertyVar PropVar;
   ADSPResult rc = ADSP_EOK;
   hw_adaptive_filter_prop_t *pProp;
   char *devId = "HWAF";
   ...
   if (ADSP_FAILED(rc = (ADSPResult)DALSYS_GetDALPropertyHandleStr(devId, hProp)))
   {
      // error handle
      MSG_1(MSG_SSID_QDSP6, DBG_ERROR_PRIO,
            "DevCfg: Error establishing Adaptive Filter dev handles returned %d", rc);
   }
   else
   {
      // read property ptr
      if (ADSP_FAILED(rc = (ADSPResult)DALSYS_GetPropertyValue(hProp, "HwAdaptiveFilterPropStructPtr", 0, &PropVar)))
      {
         MSG_1(MSG_SSID_QDSP6, DBG_ERROR_PRIO,
               "DevCfg: Error getting Adaptive Filter Property Struct Ptr returned %d", rc);
      }
      else
      {
         pProp = (hw_adaptive_filter_prop_t *)PropVar.Val.pStruct;

         pDev->base_reg_phy_addr = pProp->base_reg_phy_addr;
         pDev->reg_size = pProp->reg_size;
         pDev->int_irq_no = pProp->int_irq_no;
         pDev->hw_revision = pProp->hw_revision;
      }
   }
