/*==============================================================================
$Header: //components/rel/avs.adsp/2.8/afe/drivers/hw/devcfg/config/lpasshwio_devcfg_660.c#18 $
$DateTime: 2019/11/11 09:42:40 $
$Author: pwbldsvc $
$Change: 21330356 $
$Revision: #18 $

FILE: lpasshwio_devcfg_660.c

DESCRIPTION: SDM660 Device Config
...
{
   0x15080000,       /**< LPASS register base address */
   GPIO_TYPE_LPI,    /**< Codec Intr GPIO type: LPI */ 
   IRQ_TYPE_PMU,     /**< Codec Intr type: PMU gated */
   23,               /**< GPIO# for codec interrupt */
};
...
{
    0x15280000,  /* lpaif reg base addr LPASS_LPAIF_VERSION*/
    (256*1024),  /* lpaif reg page size*/
    {
        0x15281000,   /*i2s reg offset : LPASS_LPAIF_I2S_CTL0*/
        0x152A0000,   /* internal i2s reg offset : LPASS_LPAIF_INT_I2S_CTL0*/
        11,           /* inf_type_cnt*/
    ...
    {
        0x15281500, /*pcm reg offset LPASS_LPAIF_PCM_CTL0*/
        4,           /* num_interfaces */
        {
            PRIMARY_PCM,
            SECONDARY_PCM,
    ...
    {
        0x15281500,                    /*tdm uses the same pcm reg offset LPASS_LPAIF_PCM_CTL0*/
        4,                             /* num_interfaces */
        {
            PRIMARY_TDM,
            SECONDARY_TDM,
        ...
        {
            32,                      /* maximum # of channel for audio if PRIMARY_TDM.*/
            32,                      /* maximum # of channel for audio if SECONDARY_TDM.*/
            32,                      /* maximum # of channel for audio if TERTIARY_TDM.*/
            32                       /* maximum # of channel for audio if QUATERNARY_TDM.*/
        },
        {
    ...
    {
        0xC9A0000,  //MMSS_HDMI_CTRL
        (4*1024),  // hdmiAudHwRegSize
        SUPPORTED
    },
    {
        0x1529A000,    /* mux reg base LPASS_LPAIF_PCM_I2S_SEL0*/
    ...
    LPASS_HW_VER_4_0_3,  // hw revision
};
...
lpasshwio_prop_afe_dma_struct_t audioif_dma_prop =
{
    ...
    0x1528A000,  // dma offset LPASS_LPAIF_IRQ_EN0 : interrupt
    0x1528D000,  // dma offset LPASS_LPAIF_RDDMA_CTLa
    0x15293000,  // dma offset LPASS_LPAIF_WRDMA_CTLa
    0x1528D028,  // stc rddma offset   LPASS_LPAIF_RDDMA_STC_LSB0
    0x15293028,  // stc wrdma offset LPASS_LPAIF_WRDMA_STC_LSB0
    0x152A7000,  // dma offset LPASS_LPAIF_WRDMA_CTL5
    0x152A7028,   // stc wrdma offset LPASS_LPAIF_WRDMA_STC_LSB5
    6,   // sink DMA channel count
    6,   // source DMA channel count
    19,  // lpaif intr irq #
    2,   // interrupt line #
    LPASS_HW_VER_4_0_3   //  i2s hw revision
};

lpasshwio_prop_afe_dma_struct_t hdmiout_dma_prop = {
    0x150F8000,   // hdmi output reg base addr LPASS_HDMITX_VERSION
    ...
    LPASS_HW_VER_4_0_2   //  hdmiout hw revision
};
...
{
    0x150F7000,    /**< Reg base address: LPASS_AVTIMER_HW_VERSION */
    ...
    /* AV timer Hw instances information */
    {
        ...
        {
            0x150F7000,              /**< AVT reg base addr: LPASS_AVTIMER_HW_VERSION>*/
...
{
    0x15138000,   // baseRegAddr: base address of resampler - LPASS_RESAMPLER_HW_VERSION
...
{
    0x1508F000,      // VFR reg base address LPASS_VFR_IRQ_MUX_CTL
    0x9000,          // VFR register page size
    0x15092000,      // LPASS_VFR_IRQ_MUX_CTL_1
    0x15094000,      // LPASS_GP_IRQ_TS_MUX_CTL_0
    0x15095000,      // LPASS_GP_IRQ_TS_MUX_CTL_1
   ...
   IRQ_TYPE_PMU     // VFR Interrupt type
};
...
lpasshwio_prop_hdmi_output_struct_t hdmi_output_prop =
{
    0x150F8000,   // hdmi output reg base addr - LPASS_HDMITX_VERSION
    0x8000,       // hdmi output reg page size
    0x150C8000,   // hdmi output reset reg base addr - LPASS_HDMITX_CTL
    0x1000,       // hdmi output reset reg page size
    0xC9A0000,   // hdmi core output reg base addr - MMSS_HDMI_CTRL
    0x1000,       // hdmi core output reg page size
    0x40000002    // hdmi output hw version
};

/* LPM configuration for SD660 */
lpm_prop_struct_t lpm_prop =
{
    0x15120000, /**< Starting physical address, 64-bit */
...
{
    0x15080000,     // base address of tcsr reg  -- LPASS_LPASS_TCSR
    (120*1024),     // tcsr reg size
    LPASS_HW_VER_4_0_3,    // hw revision
    MMPM_CORE_ID_LPASS_CORE, /**< clk_mgr_coreid */
    31,            // Number of clk-id defined below
...
/** Hardware Adaptive Filter - dummy values */
hw_adaptive_filter_prop_t hw_adaptive_filter_prop = 
{
    0,     /**< Base address of HW Adaptive Filter registers */
    0,     /**< Size of HW Adaptive Filter registers */
    0,     /**< IRQ no assigned to HW Adaptive Fitler */
    0,     /**< HW version number */
};
