...
/**
Tells if avtimer hw mode is qtimer.

@return True if avtimer hw mode is QCOUNTER, false otherwise.
*/
ADSPResult avtimer_drv_is_hw_mode_qcounter(bool_t *is_qtimer_enabled)
{
   avtimer_drv_ctx_t *ptr_driver_ctx = &avtimer_driver_global_ctx;
   *is_qtimer_enabled                = 0;
   ...
   if (!ptr_driver_ctx->is_initialized)
   {
      MSG(MSG_SSID_QDSP6, DBG_ERROR_PRIO, "Avtimer has not been initialized yet!");
      return ADSP_EFAILED;
   }
   else
   {
      if (ptr_driver_ctx->avt_prop.avtimer_instance_info[0].mode == AVTIMER_HW_MODE_QCOUNTER)
      {
         *is_qtimer_enabled = 1;
      }
      MSG(MSG_SSID_QDSP6, DBG_HIGH_PRIO, "Yes, avtimer has been initialized");
      return ADSP_EOK;
   }
}
