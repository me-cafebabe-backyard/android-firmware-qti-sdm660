...
$Header: //components/rel/avs.adsp/2.8/afe/drivers/hw/avtimer/src/avtimer_server.c#4 $
...
ADSPResult avtimer_init(void)
{
  ...
  if(HwdDevCfg_HWSupport(HWD_MODULE_TYPE_AVTIMER))
  {
    ...
    switch(hw_revision)
    {
      ...
      {
        avtimer_server_global_inst_ptr->avtimer_server_funcs_table.avtimer_server_config_next_expiry_event_f = avtimer_server_config_next_expiry_event_v2;
        avtimer_server_global_inst_ptr->avtimer_server_funcs_table.avtimer_server_process_interrupt_f = avtimer_server_process_interrupt_v2 ;
      }
      break;
      default:
      {
