/*==============================================================================
$Header: //components/rel/avs.adsp/2.8/afe/drivers/afe/dsat/src/AFEDsatDriver.cpp#3 $
$DateTime: 2018/01/02 03:21:08 $
$Author: pwbldsvc $
$Change: 15151673 $
$Revision: #3 $

Copyright 2015, 2017 Qualcomm Technologies, Inc. (QTI).
All Rights Reserved.
...
ADSPResult afe_port_dangergen_enable(afe_dev_port_t *pDevPort)
{
  ...
  if(NULL != pDevPort->danger_info_ptr)
  {
    afe_danger_gen_info_t *danger_gen_ptr = (afe_danger_gen_info_t *)pDevPort->danger_info_ptr;
    dangergen_watchdog_params_type *watchdog_ptr = &danger_gen_ptr->watchdog_params;
    uint64_t current_timestamp = qurt_elite_fwk_get_time(pDevPort->avt_drv);
