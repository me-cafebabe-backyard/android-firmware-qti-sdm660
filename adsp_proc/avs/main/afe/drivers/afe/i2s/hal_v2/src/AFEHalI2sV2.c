...
$Header: //components/rel/avs.adsp/2.8/afe/drivers/afe/i2s/hal_v2/src/AFEHalI2sV2.c#2 $
...
/* This header contains macro definitions of INT_I2S_CTL registers, Address file took from nazgulQ*/
#include "LPASS_ADDRESS_FILE_LPAIF_4_0_3.h"
...
typedef struct hal_i2s_dump_v2
{
  uint32_t LPASS_LPAIF_I2S_CTLa_DUMP[MAX_I2S_TYPE_NUM][LPA_IF_SEQ_MAX];
} hal_i2s_dump_v2_t;

static hal_i2s_dump_v2_t afe_hal_i2s_dump_v2;

static void hal_i2s_dump_v2(uint32_t intf_index, uint32_t dir, uint32_t dump_seq);
#endif

void hal_i2s_init_v2(uint32_t version, uint32_t phy_base, uint32_t virt_base, uint32_t i2s_offset, uint32_t int_i2s_offset);
void hal_i2s_config_v2(uint32_t i2s_index, uint32_t dir, i2s_config_t *config_ptr);
void hal_i2s_enable_v2(uint32_t i2s_index, uint32_t dir);
void hal_i2s_disable_v2(uint32_t i2s_index, uint32_t dir);
uint32_t hal_i2s_get_intf_v2(uint32_t i2s_index, uint32_t dir);
...
void hal_i2s_get_func_table_v2(afe_i2s_hal_func_def_t *func_table)
{
  ...
  func_table->hal_i2s_init_fptr                 = hal_i2s_init_v2;
  func_table->hal_i2s_config_fptr               = hal_i2s_config_v2;
  func_table->hal_i2s_enable_fptr               = hal_i2s_enable_v2;
  func_table->hal_i2s_disable_fptr              = hal_i2s_disable_v2;
  func_table->hal_i2s_get_intf_fptr             = hal_i2s_get_intf_v2;
#ifdef AUDIOIF_REG_DUMP  
  func_table->hal_i2s_dump_fptr             = hal_i2s_dump_v2;
#endif
 ...
 */
uint32 hal_i2s_get_intf_v2(uint32_t i2s_index, uint32_t dir)
{   
  ...
  switch (i2s_index)
  {
    ...
    case I2S_SPEAKER_I2S:
      if(I2S_SINK == dir)
      {
        field_value = LPASS_LPAIF_RDDMA_CTLa__AUDIO_INTF__SPKR_I2S;
      }
      break;
...
static void hal_i2s_dump_v2(uint32_t intf_index, uint32_t dir, uint32_t dump_seq)
{
  ...
  {
    ReadRegister1(i2s_phy_base + I2S_REG_OFFSET(LPASS_LPAIF_I2S_CTLa(intf_index)),i2s_virt_offset, &afe_hal_i2s_dump_v2.LPASS_LPAIF_I2S_CTLa_DUMP[intf_index][dump_seq]);
    MSG_2(MSG_SSID_QDSP6, DBG_HIGH_PRIO, "==== LPASS_LPAIF_I2S_CTLa(addr: 0x%x) = 0x%x", LPASS_LPAIF_I2S_CTLa(intf_index), afe_hal_i2s_dump_v2.LPASS_LPAIF_I2S_CTLa_DUMP[intf_index][dump_seq]);
  }
}
#endif
