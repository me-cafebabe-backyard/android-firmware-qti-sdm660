/*==============================================================================
$Header: //components/rel/avs.adsp/2.8/afe/drivers/afe/common/src/AFEDeviceDriver.cpp#32 $
...
ADSPResult afe_device_driver_init()
{
   ...
   if (ADSP_EOK != (result = afe_pcie_driver_init()))
   {
      if (ADSP_EUNSUPPORTED == result)
      {
         MSG(MSG_SSID_QDSP6, DBG_HIGH_PRIO, "Warning: PCIE interface is not supported");
      }
      else
      {
         MSG(MSG_SSID_QDSP6, DBG_ERROR_PRIO, "AFE PCIE Driver Init failed");
      }
   }
...
ADSPResult afe_dev_stop(afe_dev_port_t *pDevPort)
{
   ...
   else if (IS_PCIE_PORT_AFE_ID(pDevPort->intf))
   {   
     afe_driver_pcie_get_header_size(pDevPort, &header_size);
     pDevPort->working_buf_1 -= header_size;  
     pDevPort->working_buf_2 -= header_size;  
     afe_pcie_close(pDevPort);
   }  
...
ADSPResult afe_dev_start(afe_dev_port_t *pDevPort)
{
   ...
   {
      if (ADSP_EOK !=  afe_pcie_open(pDevPort))
      {
         return ADSP_EFAILED;
      }
   }
...
ADSPResult afe_restore_dev_config_param(afe_dev_port_t *p_dev_port)
{
   ...
   else if (IS_PCIE_PORT_AFE_ID(p_dev_port->intf))
   {
      result = afe_restore_pcie_cfg(p_dev_port);
   }
...
void* afe_dev_lpa_aligned_malloc(uint32_t num_bytes, uint32_t align_size)
{
  ...
  if (align_size > (1 << 30)) return NULL;
   ...
   ptr = (char *)qurt_elite_memory_heapmgr_malloc(num_bytes + align_size + sizeof(int), elite_mem_lpm_heap_id);
   if(ptr == NULL)
   {
	   afe_lpm_client_deregister();
       return (NULL);
...
void afe_dev_lpa_aligned_free(void *ptr)
{
   ...
   qurt_elite_memory_heapmgr_free(temp_ptr, elite_mem_lpm_heap_id);
...
ADSPResult afe_dev_create_working_buffers(afe_dev_port_t *pDevPort)
{
   ...
   if ((AFE_UNCOMPRESSED_DATA_PORT == pDevPort->port_data_type) && (AFE_UNCOMPRESSED_DATA_PORT == pDevPort->dev_data_type))
   {
      ...
      if( IS_PCIE_PORT_AFE_ID(pDevPort->intf) )
      {
      	afe_driver_pcie_get_header_size( pDevPort, &header_size );              
       	working_buf_size += header_size;
      }
       ...
       */
      if (NULL == (pDevPort->working_buf_1 = (int8_t *)qurt_elite_memory_aligned_malloc(working_buf_size, 8, QURT_ELITE_HEAP_DEFAULT)))
      {
         MSG_1(MSG_SSID_QDSP6, DBG_ERROR_PRIO, "Malloc failure while allocating working buffer 1 for port 0x%x", pDevPort->intf);
         return ADSP_ENOMEMORY;
      }
			
      if (NULL == (pDevPort->working_buf_2 = (int8_t *)qurt_elite_memory_aligned_malloc(working_buf_size, 8, QURT_ELITE_HEAP_DEFAULT)))
      {
         MSG_1(MSG_SSID_QDSP6, DBG_ERROR_PRIO, "Malloc failure while allocating working buffer 2 for port 0x%x", pDevPort->intf);
         return ADSP_ENOMEMORY;
      }
      
	  pDevPort->working_buf_size -= header_size;  
	  pDevPort->working_buf_1 += header_size;  
	  pDevPort->working_buf_2 += header_size;  
	  working_buf_size -= header_size;  
          ...
          (NULL == pDevPort->p_group_device_state->p_group_io_buf))
      {
         ...
         if (NULL == (p_group_device->p_group_io_buf = (int8_t *)qurt_elite_memory_aligned_malloc(group_dev_working_buf_size, 8, QURT_ELITE_HEAP_DEFAULT)))
         {
...
void afe_drv_update_avt_drift(afe_dev_port_t *dev_port_ptr)
{
   ...
   else if (IS_PCIE_PORT_AFE_ID(port_id))
   {
      afe_update_avt_drift_for_pcie_port(dev_port_ptr);
   }
                                             ...
                                             int8_t *param_buffer_ptr, uint32_t param_size)
{
   ...
   switch (param_id)
   {
               ...
               if ((NULL != pDevPort->afe_drv_ftbl) && (NULL != pDevPort->afe_drv_ftbl->set_param) && (NULL != pDevPort->afe_drv_ftbl->get_properties))
               {
                  ...
                  else
                  {
                     ...
                     else
                     {
                        ...
                        if ((AFE_PARAM_ID_TDM_CONFIG == param_id) && (port_cfg_data.channels > AFE_PORT_MAX_CHANNEL_CNT) )
                        {
      ...
      case AFE_PARAM_ID_PCIE_AUDIO_CONFIG:
         result = afe_set_pcie_cfg(pDevPort, param_buffer_ptr, param_size, param_id);
         is_it_port_cfg_param = TRUE;
         break;
                                                ...
                                                int8_t *param_buffer_ptr, int32_t param_buf_len, uint16_t *params_buffer_len_req_ptr)
{
   ...
   switch (param_id)
   {
      ...
      case AFE_PARAM_ID_USB_AUDIO_DEV_LPCM_FMT:
         ...
         break;
      case AFE_PARAM_ID_PCIE_AUDIO_CONFIG:
         result = afe_get_pcie_cfg(pDevPort, param_buffer_ptr, param_buf_len, params_buffer_len_req_ptr, param_id);
         break;
      case AFE_PARAM_ID_INTERNAL_BT_FM_CONFIG:
                                           ...
                                           uint64_t duration)
{
   ...
   if (ADSP_SUCCEEDED(result = afe_time_srvr_timer_create(timer_ptr, timer_type, &(pDevPort->dma_signal))))
   {
      ...
      {
         pDevPort->init_port_clock = afe_time_srvr_timer_get_time((avtimer_drv_handle_t)pDevPort->avt_drv);
...
ADSPResult afe_svc_cmd_request_lpass_resources(aud_stat_afe_svc_t *pMe, elite_apr_packet_t *apr_pkt_ptr)
{
  ...
  switch (cmd_payload_ptr->resource_id) 
  {
    ...
    case AFE_LPM_RESOURCE_ID:
    {
      result = afe_request_lpm_resources_by_remote_client_handler((void *)apr_pkt_ptr);
      break;
    }
    default:
    {
  ...
  {
    MSG_1(MSG_SSID_QDSP6,DBG_ERROR_PRIO, "Resource allocation failed for resource id:0x%x",cmd_payload_ptr->resource_id);
  }
  return result;
...
ADSPResult afe_svc_cmd_release_lpass_resources(aud_stat_afe_svc_t *pMe, elite_apr_packet_t *apr_pkt_ptr)
{
  ...
  switch (cmd_payload_ptr->resource_id) 
  {
    ...
    case AFE_LPM_RESOURCE_ID:
    {
      result = afe_release_lpm_resources_by_remote_client_handler((void *)apr_pkt_ptr);
      break;
    }
    default:
    {
  ...
  if(ADSP_FAILED(result))
  {
    MSG_1(MSG_SSID_QDSP6,DBG_ERROR_PRIO, "Resource release failed for resource id:0x%x",cmd_payload_ptr->resource_id);
  }
  return result;
...
ADSPResult afe_dev_pcie_set_timing_offset(afe_dev_port_t *p_dev_port, elite_msg_any_t *pMsg)
{
   ADSPResult result = ADSP_EOK;
   result = afe_pcie_voice_set_rw_offsets(p_dev_port,pMsg);
   MSG_1(MSG_SSID_QDSP6,DBG_ERROR_PRIO, "afe_dev_pcie_set_timing_offset result %d",result);
   return result;
}
