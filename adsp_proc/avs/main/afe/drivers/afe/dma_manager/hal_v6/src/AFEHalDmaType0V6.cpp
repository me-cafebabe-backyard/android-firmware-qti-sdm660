/*==============================================================================
$Header: //components/rel/avs.adsp/2.8/afe/drivers/afe/dma_manager/hal_v6/src/AFEHalDmaType0V6.cpp#2 $
$DateTime: 2017/07/07 09:13:53 $
$Author: pwbldsvc $
$Change: 13774098 $
$Revision: #2 $

FILE:     AFEHalDmaType0V6.cpp

DESCRIPTION: DMA Manager Type6 HAL layer
...
--------    ---     -------------------------------------------------------
09/07/2016  SJ      Created
============================================================================*/


#include "LPASS_ADDRESS_FILE_LPAIF_v6.h"
#include "common.h"
#include <assert.h>
...
typedef struct hal_audioif_dma_dump_v6
{
...
} hal_audioif_dma_dump_v6_t;

static hal_audioif_dma_dump_v6_t afe_hal_audioif_dma_dump_v6;

static void hal_dma_dump_type0_v6(uint32_t dma_dir, uint32_t dma_idx, uint32_t dump_seq);
#endif

/*==========================================================================
  Function Definitions
========================================================================== */
static void hal_dma_enable_dma_channel_type0_v6(uint32_t dma_dir, uint32_t dma_idx);
static void hal_dma_disable_dma_channel_type0_v6(uint32_t dma_dir, uint32_t dma_idx);
static void hal_dma_config_dma_channel_type0_v6(uint32_t dma_dir, uint32_t dma_idx, hal_dma_config_t *dma_config_ptr);
static void hal_dma_get_dma_curr_addr_type0_v6(uint32_t dma_dir, uint32_t dma_idx, uint32_t *curr_addr);
static void hal_dma_clear_dma_interrupt_type0_v6(uint32_t dma_dir, uint32_t dma_idx);
static void hal_dma_disable_dma_interrupt_type0_v6(uint32_t dma_dir, uint32_t dma_idx);
static void hal_dma_enable_dma_interrupt_type0_v6(uint32_t dma_dir, uint32_t dma_idx);
static void hal_dma_get_dma_int_stc_type0_v6(uint32_t dma_dir, uint32_t dma_idx, uint64_t *p_stc);
static void hal_dma_get_dma_fifo_count_type0_v6(uint32_t dma_dir, uint32_t dma_idx, uint32_t *fifo_cnt_ptr);

static uint32_t hal_dma_check_dma_interrupt_type0_v6(uint32_t dma_dir, uint32_t dma_idx, uint32_t int_status, uint32 int_status2);
static void hal_dma_read_interrupt_status_type0_v6(uint32 *int_status_ptr, uint32 *int_status2_ptr);
static void hal_dma_clear_interrupt_type0_v6(uint32_t int_status, uint32_t int_status2);


static void hal_dma_config_dma_channel_type0_sub_sink_v6(uint32_t dma_idx, hal_dma_config_t *dma_config_ptr);
static void hal_dma_config_dma_channel_type0_sub_source_v6(uint32_t dma_idx, hal_dma_config_t *dma_config_ptr);

static void hal_dma_reset_type0_v6(uint32_t dma_dir, uint32_t dma_idx);

void hal_dma_init_type0_v6(uint32_t intr_index, afe_hal_dma_func_def_t *func_table, uint32_t virt_base, lpasshwio_prop_afe_dma_struct_t *dma_hw_info_ptr)
{
  MSG(MSG_SSID_QDSP6, DBG_HIGH_PRIO, "hal_dma_init_type0_v6 is called");
  if(NULL == func_table)
  {
  ...
  func_table->afe_hal_dma_enable_dma_channel_fptr = hal_dma_enable_dma_channel_type0_v6;
  func_table->afe_hal_dma_disable_dma_channel_fptr = hal_dma_disable_dma_channel_type0_v6;
  func_table->afe_hal_dma_config_dma_channel_fptr = hal_dma_config_dma_channel_type0_v6;
  func_table->afe_hal_dma_get_dma_curr_addr_fptr = hal_dma_get_dma_curr_addr_type0_v6;
  func_table->afe_hal_dma_disable_dma_interrupt_fptr = hal_dma_disable_dma_interrupt_type0_v6;
  func_table->afe_hal_dma_enable_dma_interrupt_fptr = hal_dma_enable_dma_interrupt_type0_v6;
  func_table->afe_hal_dma_clear_dma_interrupt_fptr = hal_dma_clear_dma_interrupt_type0_v6;
  func_table->afe_hal_dma_get_dma_interrupt_stc_fptr   = hal_dma_get_dma_int_stc_type0_v6;
  func_table->afe_hal_dma_get_dma_fifo_count_fptr = hal_dma_get_dma_fifo_count_type0_v6;

  // interrupt handling
  func_table->afe_hal_dma_check_dma_interrupt_fptr = hal_dma_check_dma_interrupt_type0_v6;
  func_table->afe_hal_dma_read_interrupt_status_fptr = hal_dma_read_interrupt_status_type0_v6;
  func_table->afe_hal_dma_clear_interrupt_fptr = hal_dma_clear_interrupt_type0_v6;

  //ssr handling
  func_table->afe_hal_dma_reset_fptr  = hal_dma_reset_type0_v6;

#ifdef AUDIOIF_REG_DUMP              
  func_table->afe_hal_dma_dump_fptr = hal_dma_dump_type0_v6;
#endif
...
static void hal_dma_disable_dma_channel_type0_v6(uint32_t dma_dir, uint32_t dma_idx)
{
  ...
  else
  {
    ...
    mask = LPASS_LPAIF_WRDMA_CTLa__ENABLE___M | LPASS_LPAIF_WRDMA_CTL0__AUDIO_INTF___M;
...
static void hal_dma_config_dma_channel_type0_v6(uint32_t dma_dir, uint32_t dma_idx, hal_dma_config_t *dma_config_ptr)
{
  ...
  if(LPASS_HW_DMA_SINK == dma_dir)
  {
    hal_dma_config_dma_channel_type0_sub_sink_v6(dma_idx, dma_config_ptr);
  }
  else
  {
    hal_dma_config_dma_channel_type0_sub_source_v6(dma_idx, dma_config_ptr);
  }
}
...
static void hal_dma_config_dma_channel_type0_sub_source_v6(uint32_t dma_idx, hal_dma_config_t *dma_config_ptr)
{
  ...
  mask = LPASS_LPAIF_WRDMA_CTLa__BURST_EN___M |
      LPASS_LPAIF_WRDMA_CTLa__AUDIO_INTF___M |
...
static void hal_dma_reset_type0_v6(uint32_t dma_dir, uint32_t dma_idx)
{
  ...
  if(LPASS_HW_DMA_SINK == dma_dir)
  {
    ...
    field_value =  	LPASS_LPAIF_RDDMA_CTLa___POR;
    UpdateRegister1(reg_base_type0 + hal_rddma_register_offset_type0(LPASS_LPAIF_RDDMA_CTLa(dma_idx)),
                    rddma_reg_virtoffset_type0, mask,field_value,0);
  ...
  else
  {
    ...
    field_value = LPASS_LPAIF_WRDMA_CTLa___POR;
    UpdateRegister1(reg_base_type0 + hal_wrdma_register_offset_type0(LPASS_LPAIF_WRDMA_CTLa(dma_idx)),
                        wrdma_reg_virtoffset_type0, mask,field_value,0);
  }
}
...
static void hal_dma_dump_type0_v6(uint32_t dma_dir, uint32_t dma_idx, uint32_t dump_seq)
{
  ...
  {  //READ DMA
    ...
    {
      ReadRegister1(reg_base_type0 + hal_rddma_register_offset_type0(LPASS_LPAIF_RDDMA_CTLa(dma_idx)), rddma_reg_virtoffset_type0, &afe_hal_audioif_dma_dump_v6.LPASS_LPAIF_RDDMA_CTLa_DUMP[dma_idx][dump_seq]);
      ReadRegister1(reg_base_type0 + hal_rddma_register_offset_type0(LPASS_LPAIF_RDDMA_BASEa(dma_idx)),rddma_reg_virtoffset_type0, &afe_hal_audioif_dma_dump_v6.LPASS_LPAIF_RDDMA_BASEa_DUMP[dma_idx][dump_seq]);
      ReadRegister1(reg_base_type0 + hal_rddma_register_offset_type0(LPASS_LPAIF_RDDMA_BUFF_LENa(dma_idx)), rddma_reg_virtoffset_type0, &afe_hal_audioif_dma_dump_v6.LPASS_LPAIF_RDDMA_BUFF_LENa_DUMP[dma_idx][dump_seq]);
      ReadRegister1(reg_base_type0 + hal_rddma_register_offset_type0(LPASS_LPAIF_RDDMA_PER_LENa(dma_idx)), rddma_reg_virtoffset_type0, &afe_hal_audioif_dma_dump_v6.LPASS_LPAIF_RDDMA_PER_LENa_DUMP[dma_idx][dump_seq]);
      MSG_2(MSG_SSID_QDSP6, DBG_HIGH_PRIO, "==== LPASS_LPAIF_RDDMA_CTLa(%d) = 0x%x", dma_idx, afe_hal_audioif_dma_dump_v6.LPASS_LPAIF_RDDMA_CTLa_DUMP[dma_idx][dump_seq]);
      MSG_2(MSG_SSID_QDSP6, DBG_HIGH_PRIO, "==== LPASS_LPAIF_RDDMA_BASEa(%d) = 0x%x", dma_idx, afe_hal_audioif_dma_dump_v6.LPASS_LPAIF_RDDMA_BASEa_DUMP[dma_idx][dump_seq]);
      MSG_2(MSG_SSID_QDSP6, DBG_HIGH_PRIO, "==== LPASS_LPAIF_RDDMA_BUFF_LENa(%d) = 0x%x", dma_idx, afe_hal_audioif_dma_dump_v6.LPASS_LPAIF_RDDMA_BUFF_LENa_DUMP[dma_idx][dump_seq]);
      MSG_2(MSG_SSID_QDSP6, DBG_HIGH_PRIO, "==== LPASS_LPAIF_RDDMA_PER_LENa(%d) = 0x%x", dma_idx, afe_hal_audioif_dma_dump_v6.LPASS_LPAIF_RDDMA_PER_LENa_DUMP[dma_idx][dump_seq]);
    }
  }
  else
  {  //WRITE DMA
    ...
    {
      ReadRegister1(reg_base_type0 + hal_wrdma_register_offset_type0(LPASS_LPAIF_WRDMA_CTLa(dma_idx)), wrdma_reg_virtoffset_type0, &afe_hal_audioif_dma_dump_v6.LPASS_LPAIF_WRDMA_CTLa_DUMP[dma_idx][dump_seq]);
      ReadRegister1(reg_base_type0 + hal_wrdma_register_offset_type0(LPASS_LPAIF_WRDMA_BASEa(dma_idx)), wrdma_reg_virtoffset_type0, &afe_hal_audioif_dma_dump_v6.LPASS_LPAIF_WRDMA_BASEa_DUMP[dma_idx][dump_seq]);
      ReadRegister1(reg_base_type0 + hal_wrdma_register_offset_type0(LPASS_LPAIF_WRDMA_BUFF_LENa(dma_idx)), wrdma_reg_virtoffset_type0, &afe_hal_audioif_dma_dump_v6.LPASS_LPAIF_WRDMA_BUFF_LENa_DUMP[dma_idx][dump_seq]);
      ReadRegister1(reg_base_type0 + hal_wrdma_register_offset_type0(LPASS_LPAIF_WRDMA_PER_LENa(dma_idx)), wrdma_reg_virtoffset_type0, &afe_hal_audioif_dma_dump_v6.LPASS_LPAIF_WRDMA_PER_LENa_DUMP[dma_idx][dump_seq]);
      MSG_2(MSG_SSID_QDSP6, DBG_HIGH_PRIO, "==== LPASS_LPAIF_WRDMA_CTLa(%d) = 0x%x", dma_idx, afe_hal_audioif_dma_dump_v6.LPASS_LPAIF_WRDMA_CTLa_DUMP[dma_idx][dump_seq]);
      MSG_2(MSG_SSID_QDSP6, DBG_HIGH_PRIO, "==== LPASS_LPAIF_WRDMA_BASEa(%d) = 0x%x", dma_idx, afe_hal_audioif_dma_dump_v6.LPASS_LPAIF_WRDMA_BASEa_DUMP[dma_idx][dump_seq]);
      MSG_2(MSG_SSID_QDSP6, DBG_HIGH_PRIO, "==== LPASS_LPAIF_WRDMA_BUFF_LENa(%d) = 0x%x", dma_idx, afe_hal_audioif_dma_dump_v6.LPASS_LPAIF_WRDMA_BUFF_LENa_DUMP[dma_idx][dump_seq]);
      MSG_2(MSG_SSID_QDSP6, DBG_HIGH_PRIO, "==== LPASS_LPAIF_WRDMA_PER_LENa(%d) = 0x%x", dma_idx, afe_hal_audioif_dma_dump_v6.LPASS_LPAIF_WRDMA_PER_LENa_DUMP[dma_idx][dump_seq]);
    }
  }
}
