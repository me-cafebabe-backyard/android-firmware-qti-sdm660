...
ADSPResult afe_set_rt_proxy_cfg (afe_dev_port_t* dev_port, int8_t *p_cfg, uint16_t payload_size)
{
	...
	case RTPROXY_CFG_V1:
	{
		...
		if ((config->num_channels < 1) || (config->num_channels > AFE_DRIVER_RT_PROXY_MAX_CHANNEL_CNT))
		{
...
ADSPResult afe_rt_proxy_port_open(afe_dev_port_t *pDevPort)
{
   ...
   is needed.
   2. To cover for any drifts between HLOS timing and afe internal timing (timer), we need to allocate buffer on ping and pong portion. 
...
ADSPResult afe_get_time_for_rt_proxy_port(afe_dev_port_t *dev_port,uint64_t *curr_time)
{
	...
	*curr_time = qurt_elite_fwk_get_time((avtimer_drv_handle_t)dev_port->avt_drv);
