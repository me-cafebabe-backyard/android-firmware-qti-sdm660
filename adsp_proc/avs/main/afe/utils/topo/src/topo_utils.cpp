...
/*========================================================================
 $Header: //components/rel/avs.adsp/2.8/afe/utils/topo/src/topo_utils.cpp#9 $
                                   ...
                                   adsp_amdb_module_handle_info_t *handle_ptr)
{
  ...
  topo_node_ptr->module_ptr = NULL;
  topo_node_ptr->heap_id = QURT_ELITE_HEAP_DEFAULT;
                                 ...
                                 topo_callback_info_t *cb_info_ptr)
{
  ...
  avcs_module_instance_proc_info_t  *module_list_ptr = NULL;
  ...
  //Find all modules
  {
    ...
    module_list_ptr = (avcs_module_instance_proc_info_t *)qurt_elite_memory_malloc(num_modules * sizeof(avcs_module_instance_proc_info_t), QURT_ELITE_HEAP_DEFAULT);
    ...
    *topo_ptr_ptr = topo_ptr;
    ...
    for(mod_idx = 0; mod_idx < topo_ptr->num_modules; mod_idx++)
    {
      ...
      result = topo_init_module(topo_ptr, mod_idx, event_cb_f, module_handle_info_ptr_tmp);
      if(ADSP_FAILED(result))
      {
                      ...
                      topo_callback_info_t *cb_info_ptr)
{
  ...
  for(mod_idx = 0; mod_idx < topo_ptr->num_modules; mod_idx++)
  {
    ...
    topo_node_ptr->module_output_buf_size = topo_ptr->topo_input_buf_size;
    ...
    if(mod_idx>0)
    {
      ...
      topo_node_ptr->module_output_buf_size = topo_node_ptr->module_input_buf_size;
      if(TRUE == topo_prev_node_ptr->is_output_media_event_raised)
      {
        if(!topo_is_equal_media_format(&(topo_prev_node_ptr->output_media_fmt), (topo_media_fmt_t *)prop.payload.data_ptr))
        {
          ...
          /* update output buf size as well like doing above*/
          topo_node_ptr->module_output_buf_size = temp_buf_size;
          /* Previous module's output buffer size is same as current module's input buffer size*/
          topo_prev_node_ptr->module_output_buf_size = temp_buf_size;
                        ...
                        bool_t is_timestamp_valid)
{
  ...
  /* Topologie's Output buffer size per channel is going to same as last module's module_output_buf_size*/
  topo_node_ptr = &topo_ptr->module[topo_ptr->num_modules-1];
  out_buf_size_per_channel = topo_node_ptr->module_output_buf_size;
  ...
  for(mod_idx = 0; mod_idx < topo_ptr->num_modules; mod_idx++)
  {
    ...
    for(i = 0; i<topo_ptr->num_channels; i++)
    {
      ...
      pProcessBuffers[out_buf_index]->buf_ptr[i].max_data_len = topo_node_ptr->module_output_buf_size;
    }
...
ADSPResult topo_send_capiv2_output_media_format (topo_handle_t handle, const topo_send_output_media_fmt_t *pParams)
{
  ...
  if(pParams->num_channels == 0 || pParams->num_channels > SIZE_OF_ARRAY(pParams->channel_mapping)
     || pParams->num_channels > SIZE_OF_ARRAY(proposed_media_format.channel_map))
  {
