...
/*========================================================================
 $Header: //components/rel/avs.adsp/2.8/afe/utils/misc/src/afe_main.cpp#2 $
...
ADSPResult afe_main_init()
{
  ...
  if (ADSP_FAILED(result = hw_af_init()))
  {
     MSG_1(MSG_SSID_QDSP6, DBG_FATAL_PRIO, "FAILED to init hw_af with status %d", result);
     return ADSP_EFAILED;
  }
  MSG(MSG_SSID_QDSP6, DBG_HIGH_PRIO, "hw_af is initialized");
...
ADSPResult afe_main_deinit()
{
  ...
  if (ADSP_FAILED(result = hw_af_deinit()))
  {
     MSG_1(MSG_SSID_QDSP6, DBG_FATAL_PRIO, "FAILED to deinit hw_af with status %d", result);
  }
  else
  {
    MSG(MSG_SSID_QDSP6, DBG_HIGH_PRIO, "hw_af is successfully deinitialized");
  }
