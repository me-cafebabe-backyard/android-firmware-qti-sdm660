...
# build_files_sha1 structure, <File Name>, <Hash>, <Manditory Existance>
build_files_sha1=[["buildit.py", "426b864c0a3b1b5c2dc07cb9ab2f2efb52314fd6", True]]
...
def build_scripts_check(evaluation_dir,
  ...
  for current_entry in build_files_sha1:
    ...
    # Check if file exists, if it doesn't and manditory existance field 
    # is true then error
    if (not os.path.isfile(os.path.join(base_directory,
                                        current_entry[0]))):
      if (current_entry[2]):
        raise NameError("ERROR: watchit::build_scripts_check: " + \
                        "File does not exist but marked manditory: " + \
