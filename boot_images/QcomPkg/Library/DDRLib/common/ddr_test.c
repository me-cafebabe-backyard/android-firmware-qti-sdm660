...
)
{
  ...
  if (level == 0)
  {
	...
	/* Test address with offset 1024 to see the problems in channel 1 during interleaving */
    else if((ddr_physical_info.interleaved_memory & SDRAM_CS0) && 
                                !ddr_test_data_lines(base+1024, limit-1024)) 
    {
      test_error_type = DDR_TEST_ERR_DATA_LINES;
    }
...
{
	volatile uint32 index1, index2;
	ddr_test_error_type test_error_type = DDR_TEST_ERR_NONE;
	
	DDR_TEST_LOG_MESSAGE("Testing DDR Clock Switching: Enter DDR Clock Switch."); 
		
	for(index1=0;index1<ddrsns_share_data->misc.ddr_num_clock_levels;index1++)
	{	 	 
		Clock_SetBIMCSpeed(ddrsns_share_data->misc.clock_plan[index1].clk_freq_in_khz);
		if (!ddr_test_own_addr(base, limit))
		{
			test_error_type = DDR_TEST_ERR_OWN_ADDR;
		}
		for (index2 = 0; index2 <ddrsns_share_data->misc.ddr_num_clock_levels; index2++) 
		{
			Clock_SetBIMCSpeed(ddrsns_share_data->misc.clock_plan[index2].clk_freq_in_khz);
			if (!ddr_test_own_addr(base, limit))
			{
				test_error_type = DDR_TEST_ERR_OWN_ADDR;
			}
...
)
{
  ...
  /* if memory interleave is enabled, check only channel 0 
     as it will interleave across to channel 1 as well
  */
