...
/*=============================================================================
 ...
 --------   ---     -----------------------------------------------------------
 10/24/17   sv      Increase FG MEM RT status poll max time to 250ms.
...
PmicFgSram_ImaReadSingleOffset(uint32 PmicDeviceIndex, uint16 ReadAddress, uint8 *Data, uint8 Offset)
{
  ...
  else
  {
    ...
    *Data = (uint8) (ReadData & (0xFF << Offset));
    //PMIC_DEBUG(( EFI_D_WARN, "PmicDxe:: %a Single Ima ReadData = 0x%x Offset Data = 0x%x Offset = %d \n\r", __FUNCTION__, ReadData, *Data, Offset));
  }
...
PmicFgSram_ImaRead(uint32 PmicDeviceIndex, uint16 ReadAddress, uint32 *fg_memif_data)
{
  ...
  do{
    ...
    else
    {
      ...
      //PMIC_DEBUG(( EFI_D_WARN, "PmicDxe:: %a Single Ima Read FG Beat Count did not match Retry IMA read = %d \n\r", __FUNCTION__, NoOfImaRetries));
  ...
  {
    //PMIC_DEBUG(( EFI_D_WARN, "PmicDxe:: %a Single Ima Read FG Beat Count did not match Read Failure = %d \n\r", __FUNCTION__));
  ...
  else
  {
    ...
    *fg_memif_data = ReadData;
    //PMIC_DEBUG(( EFI_D_WARN, "PmicDxe:: %a Single Ima ReadData = %d NoOfImaRetries = %d \n\r", __FUNCTION__, ReadData, NoOfImaRetries));
  }
...
PmicFgSram_HandleIfImaException(uint32  PmicDeviceIndex, boolean *bExceptionOccured)
{
  ...
  *bExceptionOccured = FALSE;
  ...
  if((TRUE == bExceptionSts) || (TRUE == bMemExceptionRtSts))
  {
    ...
    PMIC_DEBUG(( EFI_D_WARN, "PmicDxe:: %a iacs_err = %d , xct_type_err = %d , be_burstwr_warn = %d , data_rd_err = %d , data_wr_err = %d , addr_burst_wrap = %d , addr_stable_err = %d \n\r",
...
/** 
 pm_fg_sram_healthy_check() 
   
 @brief 
 Verify if SRAM content are health  
 */ 
 pm_err_flag_type pm_fg_sram_healthy_check(uint32 pmic_index) 
 { 
   boolean     unexpected_match = FALSE;
   pm_err_flag_type  err_flag = PM_ERR_FLAG__SUCCESS;  
   
   err_flag = pm_fg_sram_content_verify(pmic_index, &unexpected_match);
   
   if((PM_ERR_FLAG__SUCCESS == err_flag ) && (TRUE == unexpected_match)) 
   { 
     /* Issue Dvdd Reset */  
     /* PM: PS-HOLD DVDD_HARD_RST   
        PMI: GP1_DVDD_SHDN */  
     err_flag |= pm_app_pon_pshold_cfg(PM_APP_PON_CFG_DVDD_HARD_RESET);     
	 pm_log_message("ERROR in FG SRAM: Hard resetting the device!"); 
	 
     pm_utils_drop_pshold();
   }  
 ...
 */ 
 ...
 { 
   pm_err_flag_type  err_flag = PM_ERR_FLAG__SUCCESS; 
   uint32      fg_trim_data   = 0, fg_unused_data = 0; 
   ...
   *unexpected_match = FALSE; 
   ...
   /* Read given address */ 
   err_flag = PmicFgSram_ImaRead(pmic_index, FG_SRAM_SP_TRIM_ADDR, &fg_trim_data); 
   if(err_flag != PM_ERR_FLAG__SUCCESS) 
   { 
     return err_flag; 
   } 
   ...
   if((FG_SRAM_SP_TRIM_ZEROED == fg_trim_data) && (fg_unused_data != FG_SRAM_SP_UN_USED_ZERO)) 
   { 
