...
*  \n
*  \n &copy; Copyright 2013-2016,2017 Qualcomm Technologies Inc, All Rights Reserved
*/
/* =======================================================================
Edit History
...
pm_sbl_chg_init (void)
{
  ...
  if((pmic_model == PMIC_IS_UNKNOWN) || (pmic_model == PMIC_IS_INVALID))
  {
      return err_flag = PM_ERR_FLAG__SUCCESS; //Temp Fix:  PMICs are Not currently supported in this RUMI
  }
     ...
     ( (pmic_model == PMIC_IS_PM660)) )
      {
		...
		//WA: [SDM660]WA for Erroneous USBIN auto-power-on(CR2005474)
	  	err_flag |= pm_sbl_auto_power_on_check(device_index);
		if(err_flag != PM_ERR_FLAG__SUCCESS )
        {
           return err_flag;
        }
        ...
        if((err_flag != PM_ERR_FLAG__SUCCESS) && (err_flag != PM_ERR_FLAG__FEATURE_NOT_SUPPORTED))
        {
           return err_flag;
        }
  ...
  {  //Log total heap mem used by PMIC XBL loader driver
     pm_log_message("Heap MEM: %d", pm_memory_allocated());
  }
  pm_log_num_spmi_transaction(initial_num_spmi_trans, verbose_log);
