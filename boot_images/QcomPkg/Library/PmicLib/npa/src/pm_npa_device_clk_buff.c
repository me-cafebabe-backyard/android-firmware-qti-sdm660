...
pm_npa_clk_buff_kvps_update_fcn(npa_resource *resource, npa_client_handle client)
{
  ...
  npa_client_type req_type = client->type;
    ...
    DALSYS_memscpy(client_req, sizeof(pm_npa_clk_buff_kvps), NPA_PENDING_REQUEST(client).pointer.vector, sizeof(pm_npa_clk_buff_kvps));
    ...
    DALSYS_memscpy(kvps, sizeof(pm_npa_clk_buff_kvps), client_req, sizeof(pm_npa_clk_buff_kvps));

  /* Reqd request Only*/
  reqd_kvps = (pm_npa_clk_buff_kvps *)resource->required_state_vector;
  if (req_type == NPA_CLIENT_VECTOR)
  {
    DALSYS_memscpy(reqd_kvps, sizeof(pm_npa_clk_buff_kvps), client_req, sizeof(pm_npa_clk_buff_kvps));
  }
