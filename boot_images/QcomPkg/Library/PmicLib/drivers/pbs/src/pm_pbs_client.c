...
--------   ---     ---------------------------------------------------------- 
05/23/17   sv      Update hw trigger API to support trigger edge.
04/12/17   ks      Added pm_pbs_oledb_sw_trigger api(CR-2004206)
...
pm_pbs_client_hw_trigger_enable(uint8 pmic_chip, pm_pbs_client_type pbs_client_index, pm_pbs_client_trigger hw_trigger, pm_pbs_client_trigger_edge_type trigger_edge, boolean enable)
{
    ...
    pm_pbs_client_data_type *pbs_client_ptr        = pm_pbs_client_get_data(pmic_chip);
    if ((pbs_client_ptr == NULL) || (enable == TRUE)) /*currently not supporting feature to enable trigger*/
    {
      return PM_ERR_FLAG__FEATURE_NOT_SUPPORTED;
    }
  ...
  //Disabling HW trigger for PBS client 0 is not allowed as it used for PON, also failing if SW trigger is given as argument
    ...
    hw_trigger_data = 0x01 << hw_trigger;
    hw_trig_re_fe_en_mask = hw_trigger_data;
    ...
    err_flag |= pm_comm_write_byte_mask(pbs_client_ptr->comm_ptr->slave_id, PMIO_PBS_CORE_MEM_INTF_CFG_ADDR, PMIO_PBS_CORE_MEM_INTF_CFG_RMSK, 0x80, 0);
    err_flag |= pm_comm_write_byte_mask(pbs_client_ptr->comm_ptr->slave_id, PMIO_PBS_CORE_MEM_INTF_CFG_ADDR, PMIO_PBS_CORE_MEM_INTF_CFG_RMSK, 0x00, 0);
