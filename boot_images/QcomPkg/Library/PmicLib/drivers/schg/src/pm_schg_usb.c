...
02/23/17    mr      Added API to get USB Connector/Mode type
12/12/16    mr      Updated _get_typec_status API to get USB Mode and uUSB status
...
#define PM_SCHG_100_MS_WAIT 			100000 //100 milli Second wait
#define PM_SCHG_10_MS_WAIT			10000 //10 milli Second wait
/*===========================================================================
...
pm_err_flag_type pm_schg_usb_get_usb_connector_type(uint32 device_index, pm_schg_usb_connector_type *usb_connector)
{
  ...
  else
  {
    ...
    *usb_connector = PM_SCHG_USB_CONNECTOR_INVALID;
...
pm_err_flag_type pm_schg_usb_set_usb_connector_type(uint32 device_index, pm_schg_usb_connector_type usb_connector)
{
  pm_err_flag_type       err_flag = PM_ERR_FLAG__SUCCESS;
  pm_register_data_type  data = usb_connector;
  uint8                  mask = 0x01;
  pm_schg_data_type      *schg_ptr = pm_schg_get_data(device_index);
  ...
  else if(usb_connector == PM_SCHG_USB_CONNECTOR_MICRO_USB)
  {
    /* Hard Reset can cause to write to 0x1358[0] changing the mode from uUSB to Type C mode 
    violating timing requirement for changing modes properly. SW Workaround is below: */  

	/* Disable TypeC */
    pm_register_address_type address = ADDRESS(type_c_intrpt_enb_software_ctrl);	
    err_flag = pm_comm_write_byte_mask(SLAVE_ID, address, 0x01, 0x01, 0);

	/* USB Sink enable cmd */
    err_flag |= pm_comm_write_byte_mask(SLAVE_ID, address, 0x04, 0x0, 0);
    gBS->Stall(PM_SCHG_200_MS_WAIT);

	/*Set Type C connector mode */
    address = ADDRESS(type_c_cfg); 
    err_flag = pm_comm_write_byte_mask(SLAVE_ID, address, 0x1, 0x0, 0);
    gBS->Stall(PM_SCHG_10_MS_WAIT);

	/* Enable TypeC */
    address = ADDRESS(type_c_intrpt_enb_software_ctrl);	
    err_flag = pm_comm_write_byte_mask(SLAVE_ID, address, 0x01, 0x0, 0);
    gBS->Stall(PM_SCHG_100_MS_WAIT);
	...
	/*Set uUSBconnector mode */
    address = ADDRESS(type_c_cfg); 
    err_flag = pm_comm_write_byte_mask(SLAVE_ID, address, 0x1, 0x1, 0);
    gBS->Stall(PM_SCHG_10_MS_WAIT);
  ...
  else  /* Type C */
  {
	  ...
	  err_flag = pm_comm_write_byte_mask(SLAVE_ID, address, mask, data, 0);
