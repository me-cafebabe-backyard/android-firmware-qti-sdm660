...
--------   ---    	----------------------------------------------------------
05/02/17    sv      added API to get sysok reason status.
...
#define BITGET(data, bitpos) ((data >> bitpos) & 1)
/*===========================================================================
...
pm_err_flag_type pm_schg_misc_get_temp_range_sts(uint32 device_index, pm_schg_misc_temp_range_status *misc_temp_range_sts)
{
  ...
  else
  {
    ...
    {
      misc_temp_range_sts->thermreg_disabled    = BITGET(data, 0);
      misc_temp_range_sts->temp_below_range     = BITGET(data, 1);
      misc_temp_range_sts->temp_within_range    = BITGET(data, 2);
      misc_temp_range_sts->temp_above_range     = BITGET(data, 3);
      misc_temp_range_sts->alert_level          = BITGET(data, 4);
      misc_temp_range_sts->tlim                 = BITGET(data, 5);
      misc_temp_range_sts->therm_range_active   = BITGET(data, 6);
      misc_temp_range_sts->buck_oc              = BITGET(data, 7);
    }
  }
...
pm_err_flag_type pm_schg_misc_get_aicl_sts(uint32 device_index, pm_schg_misc_aicl_status *misc_aicl_sts)
{
  ...
  else
  {
    ...
    if (PM_ERR_FLAG__SUCCESS == err_flag)
    {
      misc_aicl_sts->aicl_done                = BITGET(data, 0);
      misc_aicl_sts->aicl_fail                = BITGET(data, 1);
      misc_aicl_sts->icl_imin                 = BITGET(data, 2);
      misc_aicl_sts->dcin_ch_collapse         = BITGET(data, 3);
      misc_aicl_sts->usbin_ch_collapse        = BITGET(data, 4);
      misc_aicl_sts->highest_dc               = BITGET(data, 5);
      misc_aicl_sts->soft_limit               = BITGET(data, 6);
      misc_aicl_sts->icl_override_latch       = BITGET(data, 7);
    }
  }
...
/*This API returns DCIN/USBIN status*/
pm_err_flag_type pm_schg_misc_get_misc_sysok_reason_status(uint32 device_index, pm_schg_misc_sysok_reason_status *misc_sysok_rsn_sts)
{
  pm_err_flag_type      err_flag    = PM_ERR_FLAG__SUCCESS;
  pm_register_data_type data = 0;
  ...
  else if (NULL == misc_sysok_rsn_sts)
  {
    err_flag = PM_ERR_FLAG__PAR2_OUT_OF_RANGE;
  }
  else
  {
    pm_register_address_type misc_sysok_reason_status_reg = ADDRESS(sysok_reason_status);
    err_flag = pm_comm_read_byte(SLAVE_ID, misc_sysok_reason_status_reg, &data, 0);
    
    misc_sysok_rsn_sts->sysok_reason_status_7 = BITGET(data, 7);
    misc_sysok_rsn_sts->sysok_reason_status_6 = BITGET(data, 6);
    misc_sysok_rsn_sts->sysok_reason_status_5 = BITGET(data, 5);
    misc_sysok_rsn_sts->sysok_reason_status_4 = BITGET(data, 4);
    misc_sysok_rsn_sts->sysok_reason_status_2 = BITGET(data, 3);
    misc_sysok_rsn_sts->sysok_reason_status_3 = BITGET(data, 2);
    misc_sysok_rsn_sts->sysok_reason_dcin     = BITGET(data, 1);    /* DCIN plugin generated SYSOK */
    misc_sysok_rsn_sts->sysok_reason_usbin    = BITGET(data, 0);    /* USBIN plugin generated SYSOK */
  }
