...
static pm_schg_usb_connector_type usb_connector;
...
pm_err_flag_type pm_sbl_chg_check_weak_battery_status(uint32 device_index)
{
   ...
   uint32  led_pmic_device_index = 1; // RGB or TRILED is on second PMIC - PM660L or PMI8998
   ...
   //Set SDP ICL to 500mA for no type-c and do APSD rerun
   err_flag |= pm_schg_usb_get_apsd_result_status(device_index, &chgr_src_detected);
   err_flag |= pm_schg_usb_get_usb_connector_type(device_index,  &usb_connector);
   if ( err_flag != PM_ERR_FLAG__SUCCESS )  
   { 
       return err_flag;
   }

   if(usb_connector != PM_SCHG_USB_CONNECTOR_TYPEC)
   {             
       err_flag |= pm_schg_usb_configure_usbin_icl_options(device_index, PM_SCHG_USB_ICL_OPTIONS_USB51_MODE, TRUE);
       err_flag |= pm_schg_usb_configure_usbin_icl_options(device_index, PM_SCHG_USB_ICL_OPTIONS_USBIN_MODE_CHG, FALSE);

       //set ICL to 1500mA since in uUSB mode the ICL max is set by ICL_CFG not AICL.
         err_flag |= pm_schg_usb_set_usbin_current_limit_cfg(device_index, 1500);
         pm_log_message("Manually set USB500 mode for SDP and 1500 mA ICL for DCP");
	   if ( err_flag != PM_ERR_FLAG__SUCCESS )  
       { 
           return err_flag;
       }
   ...
   {	   
	  //set ICL for type-c
	  if(icl_flag == FALSE && usb_connector == PM_SCHG_USB_CONNECTOR_TYPEC)
	  {
      ...
      err_flag |= pm_sbl_schg_config_red_led(led_pmic_device_index, toggle_led, TRUE);

   }//while

   toggle_led = FALSE;
   err_flag |= pm_sbl_schg_config_red_led(led_pmic_device_index, toggle_led, FALSE); //Ensure LED is turned of at the end of SBL charging
...
//WA: [8998/SDM660]WA for Erroneous USBIN auto-power-on(CR2005474) 
pm_err_flag_type 
pm_sbl_auto_power_on_check(uint32 device_index)
{
   pm_err_flag_type err_flag = PM_ERR_FLAG__SUCCESS;
   pm_pon_reason_type pon_reason;
   pm_pon_on_state_reason_type on_reason;
   boolean usbin_status = FALSE;
   pm_schg_misc_sysok_reason_status misc_sysok_rsn_sts;
  
   //If the SYS_OK reason is wi-power(DC-IN), then skip this WA
   err_flag = pm_schg_misc_get_misc_sysok_reason_status(device_index, &misc_sysok_rsn_sts);
   if((PM_ERR_FLAG__SUCCESS == err_flag) && (TRUE  == misc_sysok_rsn_sts.sysok_reason_dcin))
   {
   ...
   if(((*((uint8*)&pon_reason))==0x10) &&(on_reason.warm_seq == FALSE)) //Triggered from USB charger(USB_CHG)
   {
       ...
       {
           pm_log_message("Auto power on, shudown!");
           boot_hw_powerdown();
       }
   }
...
pm_err_flag_type pm_sbl_schg_config_red_led(uint32 device_index, boolean toggle_led, boolean delay)
{
   ...
   err_flag = pm_rgb_led_config(device_index,
                                PM_RGB_1,
                                PM_RGB_SEGMENT_R,
...
pm_sbl_schg_info_init(uint32 device_index)
{
   ...
   //Check FG sram healthy. 
   err_flag |= pm_fg_sram_healthy_check(device_index);
