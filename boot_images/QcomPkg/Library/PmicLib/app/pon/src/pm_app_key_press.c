...
pm_app_pwrkey_long_press_check(pm_pon_pwrkey_dbnc_chk_at_type  dbnc_chk_at)
{
  ...
  if(((FALSE == chk_condtions) && (PM_ERR_FLAG__SUCCESS == err_flag)) 
     || (PM_ERR_FLAG__SUCCESS != err_flag ))
  {
      return err_flag;
  }
...
pm_app_pwrkey_check_press_contions(boolean* chk_condtions)
{
  ...
  *chk_condtions = FALSE;
  ...
  if((on_reason.pon_seq != TRUE) || (err_flag != PM_ERR_FLAG__SUCCESS))
  {
    return err_flag;
  }
...
pm_app_pwrkey_monitor_press(uint32 dbnc_time_msec, boolean* valid_keypress)
{
  ...
  *valid_keypress = TRUE;
  ...
  while((elapsed_tick < wait_tick) && (err_flag == PM_ERR_FLAG__SUCCESS))
  {
