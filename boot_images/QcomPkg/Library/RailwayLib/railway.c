...
static void railway_transition_individual_rail(int rail_num)
{
    ...
    // The PMIC API doesn't work for WLAN. WLAN voltage is programmed by PMIC driver in pm_config_ssc_active_and_retention_level().
    // We just maintain corner voltage so that PMIC driver can query corner voltage for WLAN when it programs.
    if(rail_num == wlan_rail_id)
    {
...
static void railway_do_transition_rails(void)
{
	...
	// As of now, Commenting out wlan and ebi rails for Starlord. Enable if required
#if 0
