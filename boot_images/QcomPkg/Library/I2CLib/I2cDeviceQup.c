...
07/01/15 UR     Add reference counting for Init-DeInit
04/02/14 UR     64 Bit support and Removed QUP_PerformInputOutput call from BAM CB.
02/07/14 SB     Restore QUP_PerformInputOutput call to BAM CB.
...
* ----------------------------------------------------------------------*/
/*
 *  When we start reading data, the core inserts a tag every 255 bytes.
 *  At the 512 byte boundary, we found that the core stops giving the
 *  INPUT_BLOCK_REQ interrupt. Setting the segment size to 254 fixes
 *  this issue.
 *
 * */
#define MAX_DATA_SEGMENT_LENGTH                      254
...
)
{
  ...
  I2C_CALLTRACE_LEVEL2(pDev->pDevLog, 2,
                        "DEV:0x%p: I2CDEVQUP_ResetQupCore: Reseting Platform pDev=%p",
                         pDev->uPlatDevId, pDev);
  platRes = I2CPLATDEV_ResetTarget(pDev->hDevTgt);
  
  I2C_CALLTRACE_LEVEL2(pDev->pDevLog, 2,
                        "DEV:0x%p: I2CDEVQUP_ResetQupCore: Reseting QUP pDev=%p",
                         pDev->uPlatDevId, pDev);
...
)
{
   ...
   if (!bSplitTransfer) {
      ...
      if (bSplitTransfer) {
         if (!QUP_WaitForI2cBusFree(pDev)) {
            I2C_ERROR_LOG_LEVEL0(pDev->pDevLog, 2, "[DEV:%p:] QUP_Completion(), After FORCED LOW Wait for IDLE failed", pDev->uPlatDevId);     
         }
      }
   }
}
...
)
{
   ...
   I2C_CALLDATA_LEVEL2(pDev->pDevLog, 4,
                       "[DEV:%p:]: I2CDEVQUP_ProcesBamEvts: bam_evt=0x%x, eOutputState=0x%x, eInSeqState=0x%x",
...
)
{
   ...
   I2C_CALLTRACE_LEVEL2(pDev->pDevLog, 2,
                        "[DEV:%p:]: QUP_PerformInputOutput pDev=%p",
                         pDev->uPlatDevId, pDev);
   do {
   ...
   I2C_CALLDATA_LEVEL2(pDev->pDevLog, 5,
                        "[DEV:%p:]: QUP_PerformInputOutput eOutputState=0x%x eInSeqState=0x%x"
...
{
   I2CDEVQUP_Device *pDev = (I2CDEVQUP_Device *) pdata;

   if ( NULL == pdata ) {
      return I2CDEVQUP_SEQSTATE_INVALID_PARAMETERS;
   }

   QUP_PerformInputOutput(pDev);

   return (int32) (&pDev->seqInfo)->eSeqState;
}
...
)
{
   ...
   I2C_CALLDATA_LEVEL2(pDev->pDevLog, 3,
                       "[DEV:%p:]: QUP_BamCallback: raw bam_evt=0x%x: eBamEvt=0x%x",
                       pDev->uPlatDevId, bamPlatRes.bamEvt, pQupInfo->eBamEvt);
   /* 
      Removing again since it breaks BAM mode in HLOS and no feasible workaround
      exists.

      QUP_PerformInputOutput calls the callback passed in Async transfer functions.
      This callback will clear the request handle. After the handle is cleared we do
      not have a way to complete the request at the HLOS layer.

      In non-BAM cases, PerformInputOutput is called from IST in HLOS. This DPC
      function maintains a request handle as a lcoal variable and even if
      PerformInputOutput calls HLOS callback (clears request handle), the
      request can be completed in HLOS layer.

      At the BAM layer, we do not have much control over the request because we
      may get multiple EOTs based on how many requests are batched together.
...
)
{
   ...
   {
      I2C_CALLDATA_LEVEL2(pDev->pDevLog, 4,
                        "[DEV:%p:]: QUP_ServiceQUP:Exit not servicable uQupErrors=0x%x uOperational=0x%x"
   ...
   I2C_CALLDATA_LEVEL2(pDev->pDevLog, 4,
                        "[DEV:%p:]: QUP_ServiceQUP:Entry uQupErrors=0x%x uOperational=0x%x"
                        " i2cStatus.uRawStatus=0x%x",
                         pDev->uPlatDevId, uQupErrors, uOperational,
                         i2cStatus.uRawStatus);
   ...
   I2C_CALLDATA_LEVEL2(pDev->pDevLog, 5,
                        "[DEV:%p:]: QUP_ServiceQUP:Exit uQupErrors=0x%x uOperational=0x%x",
                         pDev->uPlatDevId,
...
)
{
   ...
   I2C_CALLTRACE_LEVEL2(pDev->pDevLog, 2,
                        "[DEV:%p:]: QUP_WaitForCompletion pDev=%p",
                         pDev->uPlatDevId, pDev);
   ...
   I2C_CALLTRACE_LEVEL2(pDev->pDevLog, 2,
                        "[DEV:%p:]: QUP_WaitForCompletion()=0x%x",
...
)
{
   ...
   I2cTransfer                    *pTransfer;
   boolean                         bSplitTransfer = FALSE;
   boolean                         bConfigureDuringRun = FALSE;
   ...
   res = HAL_qupe_SetI2cConfig(pDev->devProps.virtBlockAddr,
                               ...
                               pDev->clntCfg.uBusSDANR,
                               pDev->clntCfg.uBusSCLNR);
   if( I2C_RES_SUCCESS != res )
   {
      I2CDEVQUP_ResetQupCore(pDev);
      HAL_qupe_Configure(pDev->devProps.virtBlockAddr);
...
)
{
   ...
   I2C_CALLTRACE_LEVEL1(pDev->pDevLog, 5,
                        "[DEV:%p:]: I2CDEV_Read hdev=%p pTransfer=%p pClntCfg=%p puNumCompleted=%p",
                        pDev->uPlatDevId, hDev, pTransfer, pClntCfg, puNumCompleted);
   ...
   if ( I2C_RES_SUCCESS != res ) {
     pDev->seqInfo.ioRes.nOperationResult = res;
     I2C_ERROR_LOG_LEVEL0(pDev->pDevLog,2,"[DEV:%p:] I2CDEV_Read() Error =%d",
                          pDev->uPlatDevId,pDev->seqInfo.ioRes.nOperationResult);
   }

   I2C_CALLTRACE_LEVEL1(pDev->pDevLog,2,"[DEV:%p:] I2CDEV_Read()=%d",
                        pDev->uPlatDevId,pDev->seqInfo.ioRes.nOperationResult);
...
)
{
   ...
   do {
      ...
      I2cLog_AssociateBlockAddress(pDev->pDevLog, pDev->devProps.virtBlockAddr);
...
)
{
   ...
   I2C_CALLTRACE_LEVEL1(pDev->pDevLog,5,"[DEV:%p:] I2CDEV_Write(hdev=%p, pTransfer=%p,"
                        "pClntCfg=%p, puNumCompleted=%p)",
                         pDev->uPlatDevId, hDev, pTransfer, pClntCfg, puNumCompleted);
   I2CLOG_LogTransferData(pDev->pDevLog, pTransfer);
   ...
   if ( I2C_RES_SUCCESS != res ) {
     pDev->seqInfo.ioRes.nOperationResult = res;
     I2C_ERROR_LOG_LEVEL0(pDev->pDevLog, 2, "[DEV:%p:] I2CDEV_Write() Error =%d",
                          pDev->uPlatDevId,pDev->seqInfo.ioRes.nOperationResult);
   }

   I2C_CALLTRACE_LEVEL1(pDev->pDevLog, 2, "[DEV:%p:] I2CDEV_Write()=%d",
                        pDev->uPlatDevId, pDev->seqInfo.ioRes.nOperationResult);
...
)
{
   ...
   I2C_CALLTRACE_LEVEL1(pDev->pDevLog, 5, "[DEV:%p:] I2CDEV_BatchTransfer(hdev=%p, pSequence=%p,"
                        "pClntCfg=%p, pIoRes=%p)",
                        pDev->uPlatDevId, hDev, pSequence, pClntCfg, pIoRes);
   ...
   if ( I2C_RES_SUCCESS != res ) {
     pDev->seqInfo.ioRes.nOperationResult = res;
     I2C_ERROR_LOG_LEVEL0(pDev->pDevLog, 2, "[DEV:%p:] I2CDEV_BatchTransfer() Error =%d",
                          pDev->uPlatDevId, pDev->seqInfo.ioRes.nOperationResult);
   }

   I2C_CALLTRACE_LEVEL1(pDev->pDevLog, 2, "[DEV:%p:] I2CDEV_BatchTransfer()=%d",
                        pDev->uPlatDevId, pDev->seqInfo.ioRes.nOperationResult);
...
)
{
   ...
   I2C_CALLTRACE_LEVEL1(pDev->pDevLog, 7, "[DEV:%p:] I2CDEV_AsyncRead(hdev=%p, pTransfer=%p,"
                        "pClntCfg=%p, pIoRes=%p, pArg=%p)",
                        pDev->uPlatDevId, hDev, pTransfer, pClntCfg, pIoRes, pArg);
   I2CLOG_LogTransferData(pDev->pDevLog, pTransfer);
   ...
   if ( I2C_RES_SUCCESS != res ) {
      I2C_ERROR_LOG_LEVEL0(pDev->pDevLog, 2, "[DEV:%p:] I2CDEV_AsyncRead(), Error =%d",
                        pDev->uPlatDevId, pIoRes->nOperationResult);     
   }
   I2C_CALLTRACE_LEVEL1(pDev->pDevLog, 2, "[DEV:%p:] I2CDEV_AsyncRead()=%d",
                        pDev->uPlatDevId, pIoRes->nOperationResult);
   return res;
}
...
)
{
   ...
   I2C_CALLTRACE_LEVEL1(pDev->pDevLog, 7, "[DEV:%p:] I2CDEV_AsyncWrite(hdev=%p, pTransfer=%p,"
                        "pClntCfg=%p, pIoRes=%p, pArg=%p)",
                         pDev->uPlatDevId, hDev, pTransfer, pClntCfg, pIoRes, pArg);
   I2CLOG_LogTransferData(pDev->pDevLog, pTransfer);
   do {
   ...
   if ( I2C_RES_SUCCESS != res ) {
      I2C_ERROR_LOG_LEVEL0(pDev->pDevLog, 2,
                           "[DEV:%p:] I2CDEV_AsyncWrite(), Error =%d",
                           pDev->uPlatDevId, pIoRes->nOperationResult);     
   }
   I2C_CALLTRACE_LEVEL1(pDev->pDevLog, 2, "[DEV:%p:] I2CDEV_AsyncWrite()=%d",
                        pDev->uPlatDevId, pIoRes->nOperationResult);
   return res;
}
...
)
{
   ...
   I2C_CALLTRACE_LEVEL1(pDev->pDevLog, 7, "[DEV:%p:] I2CDEV_AsyncBatchTransfer(hdev=%p, pSequence=%p,"
                        "pClntCfg=%p, pIoRes=%p, pArg=%p)",
                        pDev->uPlatDevId, hDev, pSequence, pClntCfg, pIoRes, pArg);
   I2CLOG_LogSeqData(pDev->pDevLog, pSequence);
   ...
   if ( I2C_RES_SUCCESS != res ) {
      I2C_ERROR_LOG_LEVEL0(pDev->pDevLog, 2,
                           "[DEV:%p:] I2CDEV_AsyncBatchTransfer() Error =%d",
                           pDev->uPlatDevId, pIoRes->nOperationResult);    
   }

   I2C_CALLTRACE_LEVEL1(pDev->pDevLog, 2, "[DEV:%p:] I2CDEV_AsyncBatchTransfer()=%d",
                        pDev->uPlatDevId, pIoRes->nOperationResult);
   return res;
}
...
)
{
   ...
   I2C_CALLTRACE_LEVEL1(pDev->pDevLog, 2, "[DEV:%p:] I2CDEV_CancelTransfer(hdev=%p)", pDev->uPlatDevId, hDev);
   ...
   *asyncCall.pIoRes = pDev->seqInfo.ioRes;
   ...
   I2C_CALLTRACE_LEVEL1(pDev->pDevLog, 2, "[DEV:%p:] I2CDEV_CancelTransfer()=", pDev->uPlatDevId);

   return I2CDEVQUP_SEQSTATE_COMPLETE;
}
...
)
{
   ...
   I2C_CALLTRACE_LEVEL1(pDev->pDevLog, 2, "[DEV:%p:] I2CDEV_AbortTransfer(pDev=%p)",
                        pDev->uPlatDevId, pDev);
       ...
       (pSeqInfo->eSeqState != I2CDEVQUP_SEQSTATE_COMPLETE))
   {
       ...
       * wsa configured */
      if (!QUP_WaitForI2cBusClkForcedLow(pDev)) {
         I2C_ERROR_LOG_LEVEL0(pDev->pDevLog, 2, "[DEV:%p:] I2CDEV_AbortTransfer(), Wait for FORCED CLK LOW failed", pDev->uPlatDevId);     
      }
      ...
      if (!QUP_WaitForI2cBusFree(pDev)) {
         I2C_ERROR_LOG_LEVEL0(pDev->pDevLog, 2, "[DEV:%p:] I2CDEV_AbortTransfer(), After FORCED LOW Wait for IDLE failed", pDev->uPlatDevId);     
      }
          ...
          (bSkipBusClear == FALSE))
      {
         ...
         if (!QUP_WaitForI2cBusFree(pDev)) {
            I2C_ERROR_LOG_LEVEL0(pDev->pDevLog, 2, "[DEV:%p:] I2CDEV_AbortTransfer(), Wait for BUS FREE failed", pDev->uPlatDevId);     
         }     
      }
   }
   else
   ...
   I2C_CALLTRACE_LEVEL1(pDev->pDevLog, 2, "[DEV:%p:] I2CDEV_AbortTransfer()=", pDev->uPlatDevId);
   return I2C_RES_SUCCESS;
}
...
)
{
   ...
   I2C_CALLTRACE_LEVEL1(pDev->pDevLog, 3, "[DEV:%p:] I2CDEV_SetPowerState(hdev=%p, ePowerState=%d)",
                        pDev->uPlatDevId, hDev, ePowerState);
   do
   {
   ...
   I2C_CALLTRACE_LEVEL1(pDev->pDevLog, 2, "[DEV:%p:] I2CDEV_SetPowerState()=%d", pDev->uPlatDevId, res);
...
)
{
   ...
   I2C_CALLTRACE_LEVEL1(pDev->pDevLog, 3, "[DEV:%p:] I2CDEV_GetPowerState(hdev=%p, pePowerState=%p)",
                        pDev->uPlatDevId, hDev, pePowerState);
   *pePowerState = pDev->devPowerState;
   
   I2C_CALLTRACE_LEVEL1(pDev->pDevLog, 2, "[DEV:%p:] I2CDEV_GetPowerState()=%d", pDev->uPlatDevId, res);
   return res;
}
...
)
{
   ...
   I2C_CALLTRACE_LEVEL1(pDev->pDevLog, 3, "[DEV:%p:] I2CDEV_SendCmd(hdev=0x%p, devCmd=%d)",
                         pDev->uPlatDevId, hDev, devCmd);     
   switch ( devCmd ) {
