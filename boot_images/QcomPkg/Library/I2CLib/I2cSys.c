...
)
{
   ...
   if ( DAL_ERROR_TIMEOUT == dalRes ) {  //Timeout event.
      return I2CSYS_RESULT_ERROR_EVT_WAIT_TIMEOUT;
   }
	
   else if ( (DAL_SUCCESS != dalRes) || (uEvtIdx > 1) ) {
      return I2CSYS_ERROR_EVT_WAIT_FAILED;
   }
   else if ( 0 == uEvtIdx ) {  //Timeout event.
      return I2CSYS_RESULT_ERROR_EVT_WAIT_TIMEOUT;
   }
...
{
   /* TODO: Replace with DALSYS_memset once DALSYS is fixed */
   // --------------------------
   uint32 i = 0;
