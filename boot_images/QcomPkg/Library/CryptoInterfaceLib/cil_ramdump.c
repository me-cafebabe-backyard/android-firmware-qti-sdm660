...
static uint8             dload_aes_and_hmac_key_and_version[XBL_SW_AES_PLUS_HMAC_KEY_PLUS_VERSION_SIZE];
...
/* for testing, may dump unencrypted AES key
#define MD_UNENCR_MEMORY_REGION1 {(uint32)dload_aes_key, XBL_SW_AES_KEY_SIZE, "AES Key", "md_key_aes.BIN"}
*/
#define MD_UNENCR_MEMORY_REGION2 {(uint32)dload_rsa_encr_key, XBL_RSA_KEY_LEN, "Encrypted AES Key", "md_encr_key_aes.BIN"}
#define MD_UNENCR_MEMORY_REGION3 {(uint32)dload_aes_key_iv, (NUM_REGIONS*XBL_RSA_KEY_LEN), "IV key region", "md_encr_key_iv.BIN"}
#define MD_UNENCR_MEMORY_REGION4 {(uint32)dload_aes_key_mac, (NUM_REGIONS*XBL_RSA_KEY_LEN), "MAC region", "md_encr_key_mac.BIN"}


//#define MD_UNENCR_MEMORY_REGION_TABLE MD_UNENCR_MEMORY_REGION1,MD_UNENCR_MEMORY_REGION2,MD_UNENCR_MEMORY_REGION3,MD_UNENCR_MEMORY_REGION4,MEMORY_REGION_LAST
...
{
  boot_PrngML_getdata_lite(dload_aes_key_iv[i],XBL_SW_AES_IV_SIZE);
}
...
static void generate_encr_aeskey(void)
{
  ...
  uint32  offset = 0;
  boot_PrngML_getdata_lite(dload_aes_key, XBL_SW_AES_KEY_SIZE);
  boot_PrngML_getdata_lite(dload_hmac_key, XBL_SW_HMAC_KEY_SIZE);
