...
#include <api/systemdrivers/pmic/pm_version.h>
...

{
  ...
  if ((PM_ERR_FLAG__SUCCESS == pm_version_protocol_init()) && pmic_device_info_ptr)
  {
...
pm_model_type pm_get_pmic_model(uint8 pmic_index)
{
  ...
  if (PM_ERR_FLAG__SUCCESS == pm_get_pmic_info(pmic_index, &pmic_device_info))
  {
