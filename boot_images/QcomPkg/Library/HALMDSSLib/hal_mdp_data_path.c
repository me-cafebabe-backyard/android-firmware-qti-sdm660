...
//   uRegOffset = HAL_MDP_Get_DataPath_BaseOffset(eDataPathId);
   ...
   uRegValue = in_dword(HWIO_MMSS_ROT_WB_DST_OP_MODE_ADDR + uRegOffset);
...
//   uRegValue = HWIO_OUT_FLD(uRegValue, MDP_WB_0_DST_OP_MODE, BLOCK_SIZE, HAL_MDP_ROTATOR_BLOCK_SIZE_128x128_TILE);   // Always force to 128x128 
   ...
   if(HAL_MDP_COMPRESSION_MODE_NONE != psRotateConfig->eCompressionMode)
   {
      /* Note: same field for Elessar for BWC */
      uRegValue = HWIO_OUT_FLD(uRegValue, ROT_WB_DST_OP_MODE, UBWC_ENC_EN, 1);
      /* QP is not necessary since there's only lossless compression */
   }
   else 
   {
      // reset to 0
      uRegValue = HWIO_OUT_FLD(uRegValue, ROT_WB_DST_OP_MODE, UBWC_ENC_EN, 0);
   }   
   out_dword(HWIO_MMSS_ROT_WB_DST_OP_MODE_ADDR + uRegOffset, uRegValue);
}
  ...
  switch(psColorEncInfo->eColorEncodeType)
  {
   ...
   if(HAL_MDSS_STATUS_SUCCESS == eStatus)
   {
         ...
         (NULL != pCSCPostBias))
     {
       ...
       // Matrix Coefficients
       out_dword(HWIO_MMSS_MDP_WB_2_CSC_MATRIX_COEFF_0_ADDR + uRegOffset, *(pCSCMatrixCoeff + 0x00000000));
       out_dword(HWIO_MMSS_MDP_WB_2_CSC_MATRIX_COEFF_1_ADDR + uRegOffset, *(pCSCMatrixCoeff + 0x00000001));
       out_dword(HWIO_MMSS_MDP_WB_2_CSC_MATRIX_COEFF_2_ADDR + uRegOffset, *(pCSCMatrixCoeff + 0x00000002));
       out_dword(HWIO_MMSS_MDP_WB_2_CSC_MATRIX_COEFF_3_ADDR + uRegOffset, *(pCSCMatrixCoeff + 0x00000003));
       out_dword(HWIO_MMSS_MDP_WB_2_CSC_MATRIX_COEFF_4_ADDR + uRegOffset, *(pCSCMatrixCoeff + 0x00000004));

       // Preclamp
       out_dword(HWIO_MMSS_MDP_WB_2_CSC_COMP0_PRECLAMP_ADDR + uRegOffset, *(pCSCPreClamp + 0x00000000));
       out_dword(HWIO_MMSS_MDP_WB_2_CSC_COMP1_PRECLAMP_ADDR + uRegOffset, *(pCSCPreClamp + 0x00000001));
       out_dword(HWIO_MMSS_MDP_WB_2_CSC_COMP2_PRECLAMP_ADDR + uRegOffset, *(pCSCPreClamp + 0x00000002));

       // Postclamp
       out_dword(HWIO_MMSS_MDP_WB_2_CSC_COMP0_POSTCLAMP_ADDR + uRegOffset, *(pCSCPostClamp + 0x00000000));
       out_dword(HWIO_MMSS_MDP_WB_2_CSC_COMP1_POSTCLAMP_ADDR + uRegOffset, *(pCSCPostClamp + 0x00000001));
       out_dword(HWIO_MMSS_MDP_WB_2_CSC_COMP2_POSTCLAMP_ADDR + uRegOffset, *(pCSCPostClamp + 0x00000002));

       // Prebias
       out_dword(HWIO_MMSS_MDP_WB_2_CSC_COMP0_PREBIAS_ADDR + uRegOffset, *(pCSCPreBias + 0x00000000));
       out_dword(HWIO_MMSS_MDP_WB_2_CSC_COMP1_PREBIAS_ADDR + uRegOffset, *(pCSCPreBias + 0x00000001));
       out_dword(HWIO_MMSS_MDP_WB_2_CSC_COMP2_PREBIAS_ADDR + uRegOffset, *(pCSCPreBias + 0x00000002));

       // Postbias
       out_dword(HWIO_MMSS_MDP_WB_2_CSC_COMP0_POSTBIAS_ADDR + uRegOffset, *(pCSCPostBias + 0x00000000));
       out_dword(HWIO_MMSS_MDP_WB_2_CSC_COMP1_POSTBIAS_ADDR + uRegOffset, *(pCSCPostBias + 0x00000001));
       out_dword(HWIO_MMSS_MDP_WB_2_CSC_COMP2_POSTBIAS_ADDR + uRegOffset, *(pCSCPostBias + 0x00000002));
    }
  }
}
                                                    ...
                                                    uint32                          uFlags)
{
  ...
  uDestAddrFetchConfig            = in_dword(HWIO_MMSS_MDP_WB_2_DST_WRITE_CONFIG_ADDR + uRegOffset);
  ...
  if(uCurrentGpuHighestMemoryBankBit != uGpuHighestMemoryBankBit)
  {
    ...
    out_dword(HWIO_MMSS_MDP_WB_2_DST_WRITE_CONFIG_ADDR + uRegOffset, uDestAddrFetchConfig);
  }
                                                  ...
                                                  uint32                      uFlags)
{
   ...
   uint32                     uMaxBurstSize           = HAL_MDP_MAX_BURST_SIZE_8;      // Default burst beat size
   ...
   uDstOpMode = in_dword(HWIO_MMSS_MDP_WB_2_DST_OP_MODE_ADDR + uRegOffset);

   // Keep the DS_ALPHA_X & DST_DITHER_EN configuration from being overwritten
   uDstFormatFields = in_dword(HWIO_MMSS_MDP_WB_2_DST_FORMAT_ADDR + uRegOffset) & 
                      (HWIO_OUT_FLD(0, MDP_WB_2_DST_FORMAT, DST_ALPHA_X, 1) |
                       HWIO_OUT_FLD(0, MDP_WB_2_DST_FORMAT, DST_DITHER_EN, 1));
   ...
   if(HAL_MDSS_STATUS_SUCCESS == eStatus)
   {
     ...
     else
     {
      ...
      if(TRUE == HAL_MDP_IS_MDP_PIXEL_YUV_FORMAT(psSurfaceConfig->ePixelFormat))
      {
        uDstFormatInfo |= HWIO_MMSS_MDP_WB_2_DST_FORMAT_DST_COLOR_SPACE_BMSK;
      }
      else
      {
        uDstFormatInfo &=~(HWIO_MMSS_MDP_WB_2_DST_FORMAT_DST_COLOR_SPACE_BMSK);
      ...
      if(HAL_MDP_COMPRESSION_UBWC == pgsMdpHwInfo->eBWC)
      {
        // For UBWC encoder MAX_BURST_SIZE is required to be 8 for BGR565/RGB565, for all other formats it should be 16
        uDstWriteConfig   = in_dword(HWIO_MMSS_MDP_WB_2_DST_WRITE_CONFIG_ADDR + uRegOffset);
        if(  0x00 != (HAL_MDP_FLAGS_SURFACE_COMPRESSED & psSurfaceConfig->uFlags) &&
              ( (HAL_MDP_PIXEL_FORMAT_RGB_565_16BPP == psSurfaceConfig->ePixelFormat) 
           ||   (HAL_MDP_PIXEL_FORMAT_BGR_565_16BPP == psSurfaceConfig->ePixelFormat)  ))
        {
          uMaxBurstSize = HAL_MDP_MAX_BURST_SIZE_8;
        }
        else
        {
          uMaxBurstSize = HAL_MDP_MAX_BURST_SIZE_16;
        }
        uDstWriteConfig   = HWIO_OUT_FLD(uDstWriteConfig, MDP_WB_2_DST_WRITE_CONFIG, MAX_BURST_SIZE, uMaxBurstSize);
      }
      ...
      out_dword(HWIO_MMSS_MDP_WB_2_DST0_ADDR_ADDR + uRegOffset, uPlane0Address);           // Plane-0 address
      out_dword(HWIO_MMSS_MDP_WB_2_DST1_ADDR_ADDR + uRegOffset, uPlane1Address);           // Plane-1 address
      out_dword(HWIO_MMSS_MDP_WB_2_DST2_ADDR_ADDR + uRegOffset, uPlane2Address);           // Plane-2 address
      out_dword(HWIO_MMSS_MDP_WB_2_DST3_ADDR_ADDR + uRegOffset, uPlane3Address);           // Plane-3 address
      out_dword(HWIO_MMSS_MDP_WB_2_DST_YSTRIDE0_ADDR + uRegOffset, uPlane01StrideInBytes); // Plane-0 and 1 stride
      out_dword(HWIO_MMSS_MDP_WB_2_DST_YSTRIDE1_ADDR + uRegOffset, uPlane23StrideInBytes); // Plane-2 and 3 stride
      out_dword(HWIO_MMSS_MDP_WB_2_DST_FORMAT_ADDR + uRegOffset, uDstFormatInfo);          // Dst format information
      out_dword(HWIO_MMSS_MDP_WB_2_DST_PACK_PATTERN_ADDR + uRegOffset, uDstPackInfo);      // Dst unpack information
      out_dword(HWIO_MMSS_MDP_WB_2_DST_OP_MODE_ADDR + uRegOffset, uDstOpMode);             // Op-mode
      out_dword(HWIO_MMSS_MDP_WB_2_DST_WRITE_CONFIG_ADDR + uRegOffset, uDstWriteConfig);   // Write config

      out_dword(HWIO_MMSS_MDP_WB_2_OUT_IMG_SIZE_ADDR + uRegOffset, uImageSize);

      if(HAL_MDP_DATA_PATH_LINEAR_MODE != eDataPathMode)
      {
         out_dword(HWIO_MMSS_MDP_WB_2_DST_DITHER_BITDEPTH_ADDR + uRegOffset, uDitherDepth);   // Dither bit-depth
      }
   }
}
                                               ...
                                               uint32                         uFlags)
{
   ...
   uDstFormatInfo = in_dword(HWIO_MMSS_MDP_WB_2_DST_FORMAT_ADDR + uRegOffset);          // Dst format information

   if (psConstAlphaConfig->bConstAlpha)
   {
       uDstFormatInfo = HWIO_OUT_FLD(uDstFormatInfo, MDP_WB_2_DST_FORMAT, DST_ALPHA_X, 1);

       out_dword(HWIO_MMSS_MDP_WB_2_DST_ALPHA_X_VALUE_ADDR + uRegOffset, psConstAlphaConfig->uConstAlphaValue);
   }
   else
   {
       // Disable const alpha
                                            ...
                                            uint32                      uFlags)
{
   ...
   out_dword(HWIO_MMSS_MDP_WB_2_OUT_SIZE_ADDR + uRegOffset, uOutSize);
   out_dword(HWIO_MMSS_MDP_WB_2_OUT_XY_ADDR + uRegOffset, uOutXY);
}
                                                         ...
                                                         uint32                    uFlags)
{
   ...
   if(HAL_MDSS_STATUS_SUCCESS == eStatusRet)
   {
     ...
     out_dword(HWIO_MMSS_MDP_WB_2_ROTATOR_PIPE_DOWNSCALER_ADDR + uRegOffset, uDownscalerRegValue);
   }
   return eStatusRet;
}
