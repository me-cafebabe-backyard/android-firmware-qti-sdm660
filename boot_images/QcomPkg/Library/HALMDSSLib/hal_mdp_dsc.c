...
static void HAL_MDP_DSC_ENC_PrepareRegInfo(HAL_MDP_DSC_Info *pDSCInfo, HAL_MDP_DSC_EncoderConfigType *pDSCEncConfig)
{
   ...
   uint32                     uCompressionRatio       = 0x00000000;
   ...
   // General Parameters
   pDSCInfo->uValInputBPP  = 24;       // Input to DSC Encoder is always RGB888 - 24BPP
   pDSCInfo->uValInputBPC  = 8;        // Bits-Per-Color-Components in input pixel format is always 8-bits
   pDSCInfo->bRgbInput     = TRUE;     // Find the input color format   - Always RGB color format

   // Find the target (output) bpp based on the input bpp and the compression ratio
   uCompressionRatio       = (uint32)(pDSCInfo->eCompressionRatio);
   fTargetBPP              = (float)((float)pDSCInfo->uValInputBPP/(float)uCompressionRatio);
   pDSCInfo->uTargetBPP    = (uint32)fTargetBPP;
   ...
   {
      pEncParams->uBitsPerPixel        = (24/uCompressionRatio);
      pEncParams->uBitsPerComponent    = 8;
                                          ...
                                          HAL_MDP_DSC_EncoderConfigType *pDSCEncConfig)
{
   ...
   out_dword(HWIO_MMSS_MDP_DSC_0_COMMON_MODE_ADDR + uBlkRegOffset, uRegValue);
   ...
   out_dword(HWIO_MMSS_MDP_DSC_0_ENC_ADDR + uBlkRegOffset, uRegValue);
   ...
   out_dword(HWIO_MMSS_MDP_DSC_0_PICTURE_ADDR + uBlkRegOffset, uRegValue);
   ...
   out_dword(HWIO_MMSS_MDP_DSC_0_SLICE_ADDR + uBlkRegOffset, uRegValue);

   // DSC_X_CHUNK_SIZE
   uRegValue   = 0x00;
   uRegValue   = HWIO_OUT_FLD(uRegValue, MDP_DSC_0_CHUNK_SIZE, VALUE, pEncParams->uChunkSize);
   out_dword(HWIO_MMSS_MDP_DSC_0_CHUNK_SIZE_ADDR + uBlkRegOffset, uRegValue);
   ...
   out_dword(HWIO_MMSS_MDP_DSC_0_DELAY_ADDR + uBlkRegOffset, uRegValue);

   // DSC_X_SCALE
   out_dword(HWIO_MMSS_MDP_DSC_0_SCALE_INITIAL_ADDR + uBlkRegOffset, pEncParams->uInitialScaleValue);

   // DSC_X_SCALE_DECREMENT_INTERVAL
   out_dword(HWIO_MMSS_MDP_DSC_0_SCALE_DECREMENT_INTERVAL_ADDR + uBlkRegOffset, pEncParams->uScaleDecInterval);

   // DSC_X_SCALE_INCREMENT_INTERVAL
   out_dword(HWIO_MMSS_MDP_DSC_0_SCALE_INCREMENT_INTERVAL_ADDR + uBlkRegOffset, pEncParams->uScaleIncInterval);

   // DSC_X_FIRST_LINE_BPG_OFFSET
   out_dword(HWIO_MMSS_MDP_DSC_0_FIRST_LINE_BPG_OFFSET_ADDR + uBlkRegOffset, pEncParams->uFirstLineBpgOffset);
   ...
   out_dword(HWIO_MMSS_MDP_DSC_0_BPG_OFFSET_ADDR + uBlkRegOffset, uRegValue);
   ...
   out_dword(HWIO_MMSS_MDP_DSC_0_DSC_OFFSET_ADDR + uBlkRegOffset, uRegValue);
   ...
   out_dword(HWIO_MMSS_MDP_DSC_0_FLATNESS_ADDR + uBlkRegOffset, uRegValue);

   // DSC_X_RC_MODEL_SIZE
   out_dword(HWIO_MMSS_MDP_DSC_0_RC_MODEL_SIZE_ADDR + uBlkRegOffset, pEncParams->uRcModelSize);
   ...
   out_dword(HWIO_MMSS_MDP_DSC_0_RC_ADDR + uBlkRegOffset, uRegValue);

   // DSC_X_RANGE_YYY
   for(uIndex = 0; uIndex < HAL_MDP_DSC_ENC_RC_RANGE_PARAMS_TABLE_SIZE; uIndex++)
   {
      out_dword((HWIO_MMSS_MDP_DSC_0_RANGE_MIN_QP_n_ADDR(uIndex) + uBlkRegOffset), pEncParams->uRangeMinQp[uIndex]);
      out_dword((HWIO_MMSS_MDP_DSC_0_RANGE_MAX_QP_n_ADDR(uIndex) + uBlkRegOffset), pEncParams->uRangeMaxQp[uIndex]);
      out_dword((HWIO_MMSS_MDP_DSC_0_RANGE_BPG_OFFSET_n_ADDR(uIndex) + uBlkRegOffset), pEncParams->iRangeBpgOffset[uIndex]);
   }

   // DSC_X_RC_BUF_THRESH
   for(uIndex = 0; uIndex < HAL_MDP_DSC_ENC_RC_BUF_THRESHOLD_TABLE_SIZE; uIndex++)
   {
      out_dword((HWIO_MMSS_MDP_DSC_0_RC_BUF_THRESH_n_ADDR(uIndex) + uBlkRegOffset), pEncParams->uRcBufThresh[uIndex]);
   }
}
...
static void HAL_MDP_DSC_ENC_DCESelCfg(HAL_MDP_DSC_EncoderConfigType *pDSCEncConfig, HAL_MDP_DSC_Info *pDSCInfo)
{
    ...
    out_dword(HWIO_MMSS_MDP_DCE_SEL_ADDR, uRegValue);
}
...
static void HAL_MDP_DSC_ENC_ContextInit(HAL_MDP_DSC_EncoderConfigType *pDSCEncConfig, uint32 uFlags)
{
   ...
   // Compression Ratio
   if(HAL_MDP_COMPRESSION_RATIO_3_TO_1 == pDSCEncConfig->eCompressionRatio)
   {
      sDSCInfo.eCompressionRatio = HAL_MDP_DSC_ENC_3TO1_COMPRESSION;
   }
   else  // HAL_MDP_COMPRESSION_RATIO_2_TO_1
   {
      sDSCInfo.eCompressionRatio = HAL_MDP_DSC_ENC_2TO1_COMPRESSION;
   }
}
