/** @file FwPayloadEntry.h
  ...
  Copyright (c) 2012-2013,2015-2016 Qualcomm Technologies, Inc.  All Rights 
...
)
{
  ...
  *bEntryGood = FALSE;
  ...
  if (boot_from_ufs())
  {
	  ...
	  if (EFI_ERROR(Status))
	  {
		  DEBUG_ERR_LABEL("Incompatible Flash Type: FW Entry vs Device (UFS)", FwEntryFlashTypeCheckingExit);
	  }
  }
  else if (boot_from_emmc())
  {
	  ...
	  if (EFI_ERROR(Status))
	  {
		  DEBUG_ERR_LABEL("Incompatible Flash Type: FW Entry vs Device (eMMC)", FwEntryFlashTypeCheckingExit);
	  }
  }
  else
  {
	  Status = EFI_INCOMPATIBLE_VERSION;
	  DEBUG_ERR_LABEL("Could not determine if device is UFS or eMMC", FwEntryFlashTypeCheckingExit);
  }
