...
$Header: //components/rel/boot.xf/1.4/QcomPkg/Library/SmemLib/src/smem_boot.c#1 $
...
static void smem_boot_toc_setup(void)
{
  ...
  uint32 toc_version, num_partitions, i;
  ...
  if (smem_boot_info.smem_version == SMEM_LEGACY_VERSION_ID)
  {
    ...
    /* Only one addtional legacy partition along with partitions 
  ...
  else
  {
    ...
    /* Two addtional partitions (legacy and common) along with partitions 
     * mentioned in thee TOC config */
    additional_partn = 2;
  }
  ...
  smem_boot_toc_entry_setup(&toc_header->entry[0],
  ...
  for (i = 1; i < num_partitions; i++)
  {
    ...
    smem_boot_toc_entry_setup(&toc_header->entry[i],
                              free_offset + heap_remaining,
