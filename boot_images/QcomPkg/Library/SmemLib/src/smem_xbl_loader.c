...
$Header: //components/rel/boot.xf/1.4/QcomPkg/Library/SmemLib/src/smem_xbl_loader.c#3 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
02/08/17   vr      Enable reading target specific partition table
...
void smem_boot_targ_init(void)
{
  ...
  /* Read target specific partition table */
  if (DalChipInfo_ChipFamily() == DALCHIPINFO_FAMILY_SDM630)
  {
    result = DALSYS_GetPropertyValue(smem_prop_handle, "smem_partitions_sdm630", 0, &prop);
    if (result != DAL_SUCCESS)
    {
