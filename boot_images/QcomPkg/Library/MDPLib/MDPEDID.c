...
/*
 * Below table maps ABL override topology config number to DSC profile numbers used in gDscProfileModes
 */
static const uint32 gABLConfigToDsc[DISP_TOPOLOGY_CONFIG_MAX] =
{
     7,        // DISP_TOPOLOGY_CONFIG_NONE,      
     4,        // DISP_TOPOLOGY_CONFIG_0,        
     5,        // DISP_TOPOLOGY_CONFIG_1,    
     7,        // DISP_TOPOLOGY_CONFIG_2, 
};
...
MDP_Status MDPDetectPanel(MDP_Display_IDType eDisplayId, MDP_Panel_AttrType *pDisplayInfo)
{
  ...
  uint32               uPanelDetectionStartTime = 0;
  ...
  { 
    uPanelDetectionStartTime = MDP_GetTimerCountMS();
  }
  ...
  else
  {
    ...
    else if (XML_PARSER_STATUS_OK == XML_Parser(sPlatformParams.sPlatformPanel.pPanelXMLConfig, sPlatformParams.sPlatformPanel.uConfigSize, sXmlTagsList, XML_TAGSLIST_LENGTH))
    {
        ...
        case MDP_DISPLAY_CONNECT_SECONDARY_DSI_CMD:
        {
            ...
            if (TRUE == pPanelInfo->uAttrs.sDsi.bDSCEnable)
            {
                  ...
                  (1 > pPanelInfo->uAttrs.sDsi.uDSCMinor))
              {
                ...
                MDP_Log_Message(MDP_LOGLEVEL_ERROR, "MDPDetectPanel: DSC major = %d Minor = %d is not Supported\n", pPanelInfo->uAttrs.sDsi.uDSCMajor, pPanelInfo->uAttrs.sDsi.uDSCMinor);
                pPanelInfo->uAttrs.sDsi.uDSCMajor = 1;
                pPanelInfo->uAttrs.sDsi.uDSCMinor = 1;
                pPanelInfo->uAttrs.sDsi.uDSCScr   = 0;
              }

              /* Check if there is a topology override */
              Display_ABLParamsType *pDisplayOverride = Display_ABL_GetParams();
              if ((NULL                      != pDisplayOverride) &&
                  (DISP_TOPOLOGY_CONFIG_NONE != pDisplayOverride->sPrimary.eTopologyCfg))
              {                      
                pPanelInfo->uAttrs.sDsi.uDSCProfileID = gABLConfigToDsc[pDisplayOverride->sPrimary.eTopologyCfg];
              }
            }
        ...
        if (PRODMODE_DISABLED)
        { 
            MDP_Log_Message(MDP_LOGLEVEL_WARN, "DisplayDxe: Panel ID:0x%08x (detection time: %dms) [%a] \n", sPlatformParams.sPlatformPanel.uPanelId,
                                                                                            (MDP_GetTimerCountMS() - uPanelDetectionStartTime),
