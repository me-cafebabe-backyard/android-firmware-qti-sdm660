...
/* MMU resources data, terminated by all zeros
*/
const HWIOResourceFieldType gMMU_resource_regs[] = {
  //Power control
  {
    HWIO_GCC_HLOS2_VOTE_ALL_MMSS_SMMU_GDS_ADDR,
    HWIO_GCC_HLOS2_VOTE_ALL_MMSS_SMMU_GDS_SW_COLLAPSE_BMSK,
    0,
    1,
    HWIO_GCC_HLOS2_VOTE_ALL_MMSS_SMMU_GDS_ADDR,
    HWIO_GCC_HLOS2_VOTE_ALL_MMSS_SMMU_GDS_PWR_ON_BMSK,
    HWIO_GCC_HLOS2_VOTE_ALL_MMSS_SMMU_GDS_PWR_ON_BMSK,
    0
  },
  //Clock control
  {
    HWIO_GCC_HLOS2_VOTE_ALL_MMSS_SMMU_CLK_ADDR,
    HWIO_GCC_HLOS2_VOTE_ALL_MMSS_SMMU_CLK_CLK_ENABLE_BMSK,
    1,
    0,
    HWIO_GCC_HLOS2_VOTE_ALL_MMSS_SMMU_CLK_ADDR,
    HWIO_GCC_HLOS2_VOTE_ALL_MMSS_SMMU_CLK_CLK_OFF_BMSK,
    0,
    HWIO_GCC_HLOS2_VOTE_ALL_MMSS_SMMU_CLK_CLK_OFF_BMSK
  }
};
...
*/
const HWIOFieldType gMMU_regs[] = {
  { HWIO_MMSS_BIMC_SMMU_SMMU_SMRn_ADDR(0),         0,                                               0x80000000 }, // Add stream id 0 to stream mapping table search 
  { HWIO_MMSS_BIMC_SMMU_SMMU_S2CRn_ADDR(0),        0,                                               0x0        }, // Use all attributes as presented with transaction
  { HWIO_MMSS_BIMC_SMMU_SMMU_CBARn_ADDR(0),        0,                                               0x10003    }, // Virtual Machine Identifier to be associated with the translation context bank
  { HWIO_MMSS_BIMC_SMMU_SMMU_CBA2Rn_ADDR(0),       HWIO_MMSS_BIMC_SMMU_SMMU_CBA2Rn_RW64_BMSK,       0x1        }, // enable access to addresses > 4GB
  { HWIO_MMSS_BIMC_SMMU_A_CB0_SMMU_CBN_SCTLR_ADDR, HWIO_MMSS_BIMC_SMMU_A_CB0_SMMU_CBN_SCTLR_M_BMSK, 0x0        }  // Value 0 indicates that MMU for this context bank is disabled. ie. bypass mode.
};
...
static bool32 GetUpdatedCmdLine(CHAR8* pOrig, uint32 uOrigSize, CHAR8 **ppUpdatedStr, int *piUpdatedSize)
{
  ...
  Display_ABL_GetDisplayCommandLine(&pCmdLine, &uLen);
...
static void UpdateSplashNode(void *pFdt)
{
  ...
  else 
  {
    ...
    Display_ABL_GetContinuousSplashInfo(&uAddr, &uSize);
...
{
  uint32                       uTimeoutUs = MMU_SID_WAIT_TIMEOUT_US;
  MDP_Status                   eStatus    = MDP_STATUS_OK;
  const HWIOResourceFieldType *pResources = gMMU_resource_regs;
  const HWIOFieldType         *pCmds      = gMMU_regs;
  uint32                       uResCount  = sizeof(gMMU_resource_regs)/sizeof(HWIOResourceFieldType);
  uint32                       uCmdCount  = sizeof(gMMU_regs)/sizeof(HWIOFieldType);

  //Enable resources
  if (FALSE == EnableResources(TRUE, pResources, uResCount, uTimeoutUs))
  {
    DEBUG((EFI_D_ERROR, "Failed to enable MMU domain. Display MMU SIDs are not programmed\n"));
    eStatus = MDP_STATUS_TIMEOUT;
  }
  else
  {
    uint32 i           = 0;
    ...
    //Disable resources
    EnableResources(FALSE, pResources, uResCount, uTimeoutUs);
  }
...
MDP_Status MDP_Display_SetVariable (CHAR16 *pVariableName, UINT8 *pVariableValue, UINTN uDataLength, UINTN uFlags)
{
  ...
  if (EFI_SUCCESS != Display_ABL_SetVariable(pVariableName, pVariableValue, uDataLength, uFlags))
  {
    eStatus = MDP_STATUS_FAILED;
  }
...
MDP_Status MDP_Display_GetVariable (CHAR16 *pVariableName, UINT8 *pVariableValue, UINTN *pDataLength, UINTN uFlags)
{
  MDP_Status  eStatus = MDP_STATUS_OK;
    ...
    DEBUG((EFI_D_ERROR, "MDP_Display_GetVariable\n"));
  if (EFI_SUCCESS != Display_ABL_GetVariable(pVariableName, pVariableValue, pDataLength, uFlags))
  {
    eStatus = MDP_STATUS_FAILED;
  }
...
MDP_Status MDP_Display_SetVariable_Integer (CHAR16 *pVariableName, UINT32 uVariableValue)
{
  ...
  if (EFI_SUCCESS  != Display_ABL_SetVariable(pVariableName, (UINT8 *)VarValue, AsciiStrLen (VarValue), 0))
  {
    eStatus = MDP_STATUS_FAILED;
  }
...
MDP_Status MDP_Display_GetVariable_Integer (CHAR16 *pVariableName, UINT32 *pVariableValue)
{
  ...
  if ((EFI_SUCCESS  == Display_ABL_GetVariable(pVariableName, (UINT8 *)VarValue, &uVarLength, 0)) &&
      (uVarLength  > 0))
  {
