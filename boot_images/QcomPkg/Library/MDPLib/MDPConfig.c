...
{
  /* uEncoderNum     bLMSplitEnable   bBlockPredEnable      uCompressionRatio*/  
  {  1,              FALSE,           1,                    2               },                  /*ProfileID = 0x00*/
  {  1,              TRUE,            1,                    2               },                  /*ProfileID = 0x01*/
  {  2,              FALSE,           1,                    2               },                  /*ProfileID = 0x02*/
  {  2,              TRUE,            1,                    2               },                  /*ProfileID = 0x03*/
  {  1,              FALSE,           1,                    3               },                  /*ProfileID = 0x04*/
  {  1,              TRUE,            1,                    3               },                  /*ProfileID = 0x05*/
  {  2,              FALSE,           1,                    3               },                  /*ProfileID = 0x06*/
  {  2,              TRUE,            1,                    3               },                  /*ProfileID = 0x07*/
};
                                   ...
                                   uint32                     uPipeIndex)
{
  ...
  MDP_PresentationMode_Type         eMode               = MDP_PRESENTATIONMODE_LETTERBOX;
...
static MDP_Status SetupInterface(MDP_Panel_AttrType *pPanelConfig)
{
  ...
  // Fill in the Programmable Fetch Start config
...
static void displayCommandModeVsyncConfig(MDP_Panel_AttrType *pPanelAttr, HAL_MDP_PingPong_VsyncConfigType *pVsyncConfig, uint32 uPathIndex)
{
    ...
    pVsyncConfig->uVsyncRdPtrIrqLine = (0 == pPanelAttr->uAttrs.sDsi.sTE.vSyncRdPtrIrqLine) ? (pVsyncConfig->uVsyncInitValue + 1) : pPanelAttr->uAttrs.sDsi.sTE.vSyncRdPtrIrqLine;
...
static MDP_Status  SetupAutoRefresh(MDP_Panel_AttrType *pPanelConfig, uint32 uPipeIndex)
{
  ...
  /*
   *  For PP split case, we will be using the smart panel free running mode which will mean 
   *  Auto refresh is to enabled on both interfaces
   */
  if ((MDP_MASTER_PATH_INDEX == uPipeIndex) ||
      (MDP_PANEL_FLAG_ENABLE_PINGPONG_SPLIT & pPanelConfig->eDisplayFlags))
  {
