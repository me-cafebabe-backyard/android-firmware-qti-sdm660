/*=============================================================================
  ...
  Copyright (c) 2013-2016, 2018-2019 Qualcomm Technologies, Inc.  All Rights Reserved.
...
static const sMDSS_BaseAddressMappings asMDSS_BaseAddressMappings[MDSS_BASEADDRESSMAPINGS_MAX] = {  
    ...
    {EFICHIPINFO_FAMILY_SDM660,  0x0C900000},  //for 660(Starlord) family
    {EFICHIPINFO_FAMILY_SDM630,  0x0C900000},  //for 630(Starhawk) family
};
...
MDP_Status  MDPInit(MDP_InitParamsType *pMDPInitParams, uint32 uFlags)
{
    ...
    else
    {
        ...
        if (FALSE == bSWRender)
        {
          ...
          else if (TRUE == Display_ABL_CheckPanelSkip())
          {
...
MDP_Status  MDPExitBoot(uint32 uFlags)
{
  ...
  {
    // Turn off the displays clocks before exiting
    MDPDeInit(MDP_DISPLAY_PRIMARY, 0x0);
    MDPDeInit(MDP_DISPLAY_EXTERNAL, 0x0);
    // Turn off the core clocks and footswitches as well
    MDPTerm(0x0);
  }
...
void MDPSetupDSCProperty(MDP_Panel_AttrType *pPanelInfo)
{
  ...
  sDSCConfig.pDSCEncoderCfg->pEncoderParams->bBlockPredEnable = pPanelInfo->uAttrs.sDsi.bDSCBlockPred;
