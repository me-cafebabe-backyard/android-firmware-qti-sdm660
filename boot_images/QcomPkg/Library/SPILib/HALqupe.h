...
/** Operation state.                                                         */
typedef enum HAL_qupe_I2cStateFlagsType
{
   ...
   HAL_QUPE_I2C_OP_STATUS_DATA_STATE          = 0x1C00,
...
typedef enum HAL_qupe_ResultType{
   ...
   HAL_QUPE_RESULT_INVALID_HS_DIVIDER, /**< ERROR HS DIVIDER TOO SMALL.  */
   HAL_QUPE_RESULT_INVALID_I2C_FREQ, /**< ERROR FS DIVIDER TOO SMALL.  */
...
/** @brief Checks if bus clk is in forced low state
    @param[in]  pQupAddr qup block address.
    @return Nothing.
  */
boolean
...
/** @brief Gets the state of the I2C.
    @param[in]  pQupAddr qup block address.
    @param[out] uErrMask errors HAL_qupe_I2cStateFlagsType.
    @return     Nothing.
  */
void
 ...
 * 
 * @param[in]  base         - QUP base address
 * @param[in]  chip_select  - chip select line (0 - 3)
 * @param[in]  force_assert - TRUE to force assertion of the chip select
 * @param[in]  active_low   - TRUE if the chip select is active low
 */
...
(
   uint8                 *pQupAddr,
   HAL_qupe_RunStateType  ceNextRunState
);
