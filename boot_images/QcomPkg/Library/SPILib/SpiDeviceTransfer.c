...
)
{
   ...
   do
   {
      ...
      {
         if (!lData->uWriteVectorAvail) { break; }
      ...
      if (!(lData->curWritePtr) || !(lData->uCurWriteAvail) || !(buf)) { break; }
   ...
   if ((buf) && (lData->curWritePtr))
   {
      ...
      else
      {
         *buf = 0;  /*Get the available data, rest is padded to zero*/
         ...
         {
            res = transfer->getNextByte(transfer, bytePtr++);
            bytesGet = (uint32)(bytePtr - (uint8 *)buf);
...
)
{
   ...
   do
   {
      ...
      {
         if (!lData->uReadVectorAvail) { break; }
      ...
      if (!(lData->curReadPtr) || !(lData->uCurReadAvail) || !(buf)) { break; }
...
)
{
   ...
   if ((buf) && (lData->curReadPtr))
   {
      ...
      else
      {
         ...
         {
            res = transfer->setNextByte(transfer, bytePtr++);
            bytesSet = (uint32)(bytePtr - (uint8 *)buf);
...
)
{
   ...
   if (!transfer || !(readBuf || writeBuf)) { return; }
...
)
{
   ...
   if (!transfer || !(readVector || writeVector)) { return; }
