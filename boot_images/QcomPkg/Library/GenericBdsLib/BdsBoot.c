  ...
  )
{
  ...
  if ((BootString == NULL) || (CurBBSEntry == NULL))
    return;
  ...
  )
{
  ...
  *ExitData     = NULL;
  ...
  if (IsBootOptionValidNVVarialbe (Option)) {
    ...
    if (EFI_ERROR (Status)){
      DEBUG ((DEBUG_INFO | DEBUG_LOAD | DEBUG_ERROR, "SetVariable of BootCurrent failed! \n"));
    }
  }
...
/* QCOM: Removed for now */
  ...
  if (ImageHandle == NULL) {
    ...
    /* If preload fails, do not attempt to load and start boot option */
    Status = PlatformBdsPreLoadBootOption(Option);
    ...
    if (EFI_ERROR (Status)) {
      ...
      if (Handle == NULL) {
        DEBUG ((DEBUG_ERROR, "Could not find bootable handle\r\n"));
        goto Done;
      }
      //
      // Load the default boot file \EFI\BOOT\boot{machinename}.EFI from removable Media
  ...
  )
{
  ...
  if (EFI_ERROR (Status)){
      DEBUG ((DEBUG_INFO | DEBUG_LOAD | DEBUG_ERROR, "SetVariable of BootOrder failed! \n"));
    }
...
{
  EFI_STATUS Status;
  UINT16 *BootOrderData = NULL;
  UINTN  BootOrderDataSize = 0;
  UINT16 BootOptionNumber = 0; /* Boot#### number */
  UINT32 BootOrderIndex = 0; /* Put as first entry in BootOrder list i.e. index into that array */
  EFI_DEVICE_PATH_PROTOCOL  *BootOrderDevPath = NULL;
  ...
  if (Status != EFI_SUCCESS)
  {
    DEBUG ((DEBUG_ERROR, "Could not retrieve BootOrder list\n"));
    return Status;
  }

  /* Get device path from handle */
  BootOrderDevPath = DevicePathFromHandle (Handle);
  if (BootOrderDevPath == NULL)
  {
    DEBUG ((DEBUG_WARN, "Could not retrieve device path for boot option\n"));
    return Status;
  }

  /* Get Boot#### number from device path */
  if (TestIfInBootOrder(BootOrderDevPath, &BootOptionNumber, BootOrderData, BootOrderDataSize))
  {
    /* Prepend boot option - move from current position to position 0 */
    BootOrderIndex = 0;
    Status = ReorderBootOrderIndex(BootOrderData, BootOrderDataSize, BootOptionNumber, BootOrderIndex);

    if (Status != EFI_SUCCESS)
    {
      DEBUG ((DEBUG_WARN, "Unable to update BootOrder data: 0x%08x\n", Status));
      return Status;
    }
  ...
  )
{
  ...
  if (BootNext != NULL) {
    ...
    if (EFI_ERROR (Status)){
          DEBUG ((DEBUG_INFO | DEBUG_LOAD | DEBUG_ERROR, "Remove Variable of BootNext failed! \n"));
    }
