...
{
 /* set apcs_ipc register address here which may be used during sending
  * interrupt */
 if((PcdGet32(PcdSmemInformation) & 0xff) == 3/*honeybadger 8996*/)
 {
   glink_apcs_ipc_reg = (uint32*)0x09820010;
 } else if((PcdGet32(PcdSmemInformation) & 0xff) == 5/*nazgul 8998*/){
   glink_apcs_ipc_reg = (uint32*)0x17911008;
 } else {
   ERR_FATAL("glink_os_register_isr: Unsupported Chip family, PCD:0x%x",
...
void glink_os_send_interrupt( os_ipc_intr_type *intr )
{
  ...
  if(glink_apcs_ipc_reg) 
  {
    *glink_apcs_ipc_reg = *intr;
  }
}
