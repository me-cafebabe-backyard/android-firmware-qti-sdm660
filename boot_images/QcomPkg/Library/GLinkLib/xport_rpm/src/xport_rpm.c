...
--------   ---     ----------------------------------------------------------
02/10/15   an      Add xport_rpm_wait_link_down
...
/*===========================================================================
FUNCTION      xport_rpm_get_context
===========================================================================*/
/**

  Helper to get proper context since rpm isr doesn't allow to register 
  callback data to interrupt

  @return rpm context pointer

  @dependencies None.
*/
/*=========================================================================*/
static xport_rpm_ctx_type *xport_rpm_get_context( void )
{
  uint32 irq_in = interrupt_current_isr();
  uint32 i = 0;
  uint32 num_contexts = xport_rpm_config_num;
  
  for( i = 0; i < num_contexts; ++i )
  {
    if( xport_rpm_ctx[i].pcfg->irq_in == irq_in )
    {
      return &xport_rpm_ctx[i];
    }
  }
  
  OS_LOG_ERROR_FATAL(2, "xport_rpm_get_context: no ctx for irq in[%d]", irq_in);
  return NULL;
}
#endif
