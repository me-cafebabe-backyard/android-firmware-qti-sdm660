 ...
 *
 * $DateTime: 2016/12/05 03:32:45 $
 * $Header: //components/rel/boot.xf/1.4/QcomPkg/Library/CPRLib/image/xbl/src/cpr_image.c#5 $
 * $Change: 11912421 $
 */
#include <stdlib.h>
#include "cpr_image.h"
...
void cpr_image_open_remote_cfg(void** cfg, uint32* size)
{
    *cfg = smem_get_addr( SMEM_CPR_CONFIG, size );
    ...
    if(!*cfg)
    {
        *size = 0x600;     //Reserve 1.5KB for now
...
//As of now, we are not using VDDA rail for starlord. Uncomment, if needed in future.
//  cpr_smem_serialize_config( cpr_utils_get_rail_state( CPR_RAIL_VDDA ), true );
