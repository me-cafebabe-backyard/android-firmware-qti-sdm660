...
UpdateOfflineCrashDumpConfigTable( VOID )
{
 ...
 if (!IsAbnormalResetOccurredValid(AbnormalResetOccurredVal))
 {
  ...
  if (((Status == EFI_SUCCESS) && (Data == OFFLINE_MEMORY_DUMP_USE_CAPABILITY_GPT_BMSK)) ||
  ...
  /* At this point, it means offline_memory_dump_use_capability is DISABLED, RAMDUMP NOT ALLOWED: 
     1). Clear memory capture mode
     2). Disable SDI WDOG DEBUG
     3). Clear DLOAD cookie
   */   
  ...
  /* Clear Dload Cookie to prevent entering DLOAD mode */
  Status = ClearDLOADCookie();
  if (Status != EFI_SUCCESS)
  {
    DEBUG((EFI_D_ERROR, "Failed to clear DLOAD cookie to disable offline crash dump\r\n"));
    goto ExitOCD;
  }
