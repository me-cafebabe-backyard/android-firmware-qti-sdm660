...
----------   ---   ---------------------------------------------------------
2015-07-08   wek   Return failure when blockdev_open fails.
2014-10-14   wek   Change the argument type/size of erase.
2014-08-26   vm    Integrate string library for XBL.
...
hotplug_close_device (struct hotplug_device *hdev)
{
  ...
  if (hotplug_is_valid_hdev (hdev) != 0 ||
        hdev->is_user_holding_internal_handle)
  {
    HOTPLUG_UNLOCK ();
    return -1;
  }
...
---------------------------------------------------------------------------*/
int
hotplug_dev_format (struct hotplug_device *hdev)
{
  if (hotplug_is_valid_hdev_for_read_write (hdev) != 0)
    return -1;
  ...
  return blockdev_device_format (hdev->phy_hdev->bdev, hdev->bdev_handle);
}
                             ...
                             struct hotplug_device_info *dev_info)
{
  ...
  // To support UNICODE, memscpy is used instead of ASCII string operations
  memscpy (dev_info->product_name, sizeof (dev_info->product_name),
           bdev_info.product_name, sizeof (bdev_info.product_name) );
...
/*---------------------------------------------------------------------------
  hotplug_set_bootable_partition
---------------------------------------------------------------------------*/
int hotplug_set_bootable_partition (struct hotplug_device *hdev)
{
  ...
  return blockdev_set_bootable_partition(hdev->phy_hdev->bdev,
                hdev->bdev_handle, hdev->phy_hdev->bdev_handle);
}

/*---------------------------------------------------------------------------
  hotplug_configure_device
---------------------------------------------------------------------------*/
int hotplug_configure_device (struct hotplug_device *hdev, void *config_data)
{
  if (hotplug_is_valid_hdev_for_read_write (hdev) != 0 || config_data == NULL)
    return -1;

  hotplug_reset_device_idle_counter (hdev->phy_hdev);

  return blockdev_configure_device(hdev->phy_hdev->bdev, hdev->bdev_handle,
                hdev->phy_hdev->bdev_handle, config_data);
}

/*---------------------------------------------------------------------------
  hotplug_get_internal_handle
---------------------------------------------------------------------------*/
int hotplug_get_internal_handle(struct hotplug_device *hdev, void **handle)
{
    ...
    if (blockdev_get_internal_handle(hdev->phy_hdev->bdev,
                hdev->bdev_handle, handle) == 0)
    {
        hdev->is_user_holding_internal_handle = 1;
        return 0;
    }
    else
    {
...
/*---------------------------------------------------------------------------
  hotplug_release_internal_handle
---------------------------------------------------------------------------*/
                                        ...
                                        void **handle)
{
    ...
    *handle = NULL;
    hdev->is_user_holding_internal_handle = 0;
    return 0;
}

/*---------------------------------------------------------------------------
  hotplug_get_device_type
---------------------------------------------------------------------------*/
