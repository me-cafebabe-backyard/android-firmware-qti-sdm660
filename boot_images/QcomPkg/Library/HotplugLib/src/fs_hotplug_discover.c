...
2013-11-06   plc   Change int hard_pnum to long int, for ptr-casting
...
hotplug_add_entire_device (struct hotplug_physical_device *phy_hdev)
{
  ...
  if (parti != NULL)
  {
    ...
    {
      FS_ERR_FATAL ("Name size too long %d > %d %d", copied_size, name_size, 0);
    }
                            ...
                            uint8 hard_partition_num)
{
  ...
  if (result >= FS_HOTPLUG_NAME_MAX || result < 0)
  {
    FS_ERR ("Too long string %d h%d %lu", result, hard_partition_num,
            (long) strlen (parent_parti->canonical_name));
                           ...
                           struct hotplug_partition *parent_parti)
{
  ...
  result = snprintf (parti->canonical_name, FS_HOTPLUG_NAME_MAX, "%sp%lu",
                     parent_parti->canonical_name, (long) partition_num);
  /* Check if we copied everything successfully. */
  if (result >= FS_HOTPLUG_NAME_MAX || result < 0)
  {
    FS_ERR ("Too long string %d p%lu %lu", result, (long) partition_num,
            (long) strlen (parent_parti->canonical_name));
    hotplug_partition_unref (parti);
    return NULL;
  }
                           ...
                           struct hotplug_partition *parent_parti)
{
  ...
  result = snprintf (parti->canonical_name, FS_HOTPLUG_NAME_MAX, "%s%c%lu",
                     parent_parti->canonical_name, ptype, (long) partition_num);
  /* Check if we copied everything successfully. */
  if (result >= FS_HOTPLUG_NAME_MAX || result < 0)
  {
    FS_ERR ("Too long string %d p%lu %lu", result, (long) partition_num,
...
static uint32 hotplug_lun_numbers[FS_HOTPLUG_MAX_PHY_PARTITIONS];

int
hotplug_discover_device (struct hotplug_physical_device *phy_hdev)
{
  int is_embedded_device, result;
  uint32 i, hard_pnum, lun_count, actual_lun_count;
  struct hotplug_partition *entire_parti;
  void *bdev_handle2 = NULL;
  ...
  memset (hotplug_lun_numbers, 0, sizeof (hotplug_lun_numbers));
  lun_count = FS_HOTPLUG_MAX_PHY_PARTITIONS;
  actual_lun_count = 0;

  result = blockdev_get_lun_numbers (phy_hdev->bdev, &bdev_handle2,
                lun_count, hotplug_lun_numbers, &actual_lun_count);
  if (result != 0)
    return -1;
  ASSERT (actual_lun_count <= lun_count);

  /* Try to open each hard partition. */
  for (i = 0; i < actual_lun_count; i++)
  {
    ...
    hard_pnum = hotplug_lun_numbers[i];
