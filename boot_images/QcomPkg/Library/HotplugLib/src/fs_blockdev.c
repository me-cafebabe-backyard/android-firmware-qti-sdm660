...
int
blockdev_set_bootable_partition (struct block_device *bdev, void *handle,
                    void *whole_device_handle)
{
  if (IS_INVALID_BDEV (bdev))
    return -1;

  return bdev->ops->set_bootable_partition (bdev, handle, whole_device_handle);
}

int
blockdev_configure_device (struct block_device *bdev, void *handle,
                void *whole_device_handle, void *config_data)
{
  if (IS_INVALID_BDEV (bdev))
    return -1;

  return bdev->ops->configure_device (bdev, handle, whole_device_handle,
                                        config_data);
}

int
blockdev_get_lun_numbers (struct block_device *bdev, void *handle,
                          ...
                          uint32 *actual_count)
{
  ...
  return bdev->ops->get_lun_numbers (bdev, handle, count, lun_numbers,
                                     actual_count);
}

int
blockdev_get_internal_handle(struct block_device *bdev, void *handle,
                                void **out_handle) {
    if (IS_INVALID_BDEV (bdev) || handle == NULL || out_handle == NULL)
        return -1;
