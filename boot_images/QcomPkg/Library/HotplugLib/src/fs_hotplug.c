...
struct hotplug_device hotplug_devices[FS_HOTPLUG_MAX_HDEVS];
  ...
  #define HOTPLUG_NUM_PHYSICAL_SPI_NOR 1
#else
  #define HOTPLUG_NUM_PHYSICAL_SPI_NOR 0
#endif
...
#define FS_HOTPLUG_MAX_PHYSICAL_DEVICES  \
  (HOTPLUG_NUM_PHYSICAL_SD + HOTPLUG_NUM_PHYSICAL_SD_CPRM + \
   ...
   HOTPLUG_NUM_PHYSICAL_UFS + HOTPLUG_NUM_PHYSICAL_SPI_NOR + \
   HOTPLUG_NUM_PHYSICAL_DEVNULL)
...
blockdev_to_hotplug_type (block_device_type type)
{
  ...
  switch (type)
  {
    ...
    case BLOCKDEV_TYPE_UFS:
      ...
      break;
    case BLOCKDEV_TYPE_SPI_NOR:
      btype = HOTPLUG_TYPE_SPI_NOR;
      break;
