...
$Header: //components/rel/boot.xf/1.4/QcomPkg/Library/UfsCommonLib/ufs_api.c#3 $
$DateTime: 2017/02/13 06:28:46 $ $Author: pwbldsvc $
...
int32 ufs_close (struct ufs_handle *handle)
{
   ...
   // Initialize the device upon closing of the WLUN_BOOT
   if (handle->lun == UFS_WLUN_BOOT) {
      rc = ufs_set_fDeviceInit (handle);
      if (rc != EOK) {
         return -EINVAL;
      }
   }
