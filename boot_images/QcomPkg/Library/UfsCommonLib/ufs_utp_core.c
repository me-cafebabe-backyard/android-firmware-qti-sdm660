...
/*======================================================================
  ...
  $Header: //components/rel/boot.xf/1.4/QcomPkg/Library/UfsCommonLib/ufs_utp_core.c#4 $
  $DateTime: 2018/10/16 09:22:30 $ $Author: pwbldsvc $

when         who     what, where, why
----------   ---     --------------------------------------------------- 
2018-10-12   sb      Add reset api support
2016-12-21   sb      Extend UTRD timeout to 30s for unmap  
...
#define UFS_DEFAULT_GEAR         0x02        // GEAR-1
...
{
   int32_t rc = EOK;
   struct ufs_dal_config_item config = {UFS_USES_HS_GEAR, UFS_DEFAULT_GEAR, 
                                        UFS_DEFAULT_LANE};
   uint32_t val;
   uint32_t tout;
   uint32_t lanes;
...
static int32_t ufs_preinit_mphy_configure (ufs_host_t *hba)
{
   ...
   for (i = 0; i < size; i++) {
      ...
      out_dword((reg + hba->ufs_hci_addr), val);
   }
