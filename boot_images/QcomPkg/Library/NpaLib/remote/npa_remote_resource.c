...
/* Forward declare, so that the code looks as linear as it is */
static void 
 ...
 * @return None
 */
void npa_remote_define_resource_cb( npa_remote_resource_definition *definition, 
                               ...
                               unsigned int  data_size )
{
  ...
  if (handle)//KW Fix
  {
    ...
    if ( protocol ) // KW fix
    {
        ...
        remote_node_continuation->node->resources[0].query_fcn = protocol->definition->query_fcn;
                             ...
                             unsigned int  data_size )
{
  ...
  remote_node_continuation->node->resources[0].attributes &= ~NPA_RESOURCE_REMOTE_NO_INIT;
