...
#define QUSB_DCI_PMI660_INDEX                                (0)

// Total timeout for USB cable to get connected after entering RAMDUMP
// delay_us_per_loop * loop_count.   Currently 90s from entering RAMDUMP
#define QUSB_DCI_VBUS_HIGH_TIMEOUT_DELAY_US_PER_LOOP          (100)
#define QUSB_DCI_VBUS_HIGH_TIMEOUT_LOOP_COUNT                 (900000)
...
boolean qusb_dci_enable_usb30_clocks(void)
{
  ...
  /** TODO: Enable HS PHY reference clock, 
   ** USB BOOT Clock Replace API turns it off **/
...
void qusb_dci_typec_lane_detect(void)
{
  ...
  {
    if (PM_ERR_FLAG__SUCCESS != pm_schg_usb_get_typec_status(QUSB_DCI_PMI660_INDEX, &typec_status))
    {
    ...
    {
	  if(PM_SCHG_USB_CONNECTOR_MICRO_USB == typec_status.usb_connector)
	  {
		return;
	  }
	  else if(PM_SCHG_USB_CONNECTOR_INVALID == typec_status.usb_connector)
	  {
		qusb_uart_log("pm_usb_invalid",0);
	  }
      break;
    }
