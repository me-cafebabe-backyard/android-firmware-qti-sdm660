...
static boolean qusb_fedl_is_chgr_sdp(void)
{
  ...
  if (PM_ERR_FLAG__SUCCESS != pm_get_pmic_info(QUSB_DCI_PMI660_INDEX, &pmic_device_info))
  {    
  ...
  while (--charger_det_retry_cnt)
  {
    ...
    pm_err = pm_schg_usb_get_typec_status(QUSB_DCI_PMI660_INDEX, &typec_status);
    ...
    pm_err = pm_schg_usb_get_apsd_result_status(QUSB_DCI_PMI660_INDEX, &usb_charger_type);
    if (PM_ERR_FLAG__SUCCESS != pm_err)
    {
...
boolean qusb_ldr_utils_fedl_check(void)
{
  ...
  if(DALPLATFORMINFO_TYPE_RUMI == DalPlatformInfo_Platform() ||
     DALPLATFORMINFO_TYPE_VIRTIO == DalPlatformInfo_Platform()) 
  {
    return FALSE;
  }
