...
/*=============================================================================
 ...
 --------   ---      ----------------------------------------------------------
 01/17/17   pm       Fix KW issues
 01/05/17   pm       Remove unneeded wait time in GetChargerPortType and call the func in UFP mode only
 12/01/16   ak       Porting from Boot.xf.1.2
 09/16/16   ck       Add correct return code
...
/**
  ...
  @param [in] ChargerPortType    Charger port type
  ...
  )
{
  ...
  switch (CoreType)
  {
    ...
    default:
      ...
      *CoreNum = (QCOM_USB_CORE_NUM) USB_CORE_MAX_NUM_SDM_660;
    break;
  }
  ...
  )
{
  ...
  if (CoreNum >= USB_CORE_MAX_NUM_SDM_660) {
    Status = EFI_INVALID_PARAMETER;
  ...
  )
{
  ...
  if (gUsbConfigDevice[USB_CORE_MAX_NUM_SDM_660].PeripheralHandle != NULL)
  {
  ...
  gUsbConfigDevice[USB_CORE_MAX_NUM_SDM_660].UsbConfig = gUsbConfigTemplate;
  gUsbConfigDevice[USB_CORE_MAX_NUM_SDM_660].UsbConfig.coreNum = (QCOM_USB_CORE_NUM)USB_CORE_MAX_NUM_SDM_660;
  gUsbConfigDevice[USB_CORE_MAX_NUM_SDM_660].UsbConfig.modeType = USB_INVALID_MODE;
  gUsbConfigDevice[USB_CORE_MAX_NUM_SDM_660].HostHandle = NULL;
  gUsbConfigDevice[USB_CORE_MAX_NUM_SDM_660].PeripheralHandle = NULL;
  // Install the protocol on PeripheralHandle, this is used as an indicator that
  // the UsbConfigInit has been called.
  Status = gBS->InstallMultipleProtocolInterfaces(&gUsbConfigDevice[USB_CORE_MAX_NUM_SDM_660].PeripheralHandle,
    &gQcomUsbConfigProtocolGuid, &gUsbConfigDevice[USB_CORE_MAX_NUM_SDM_660].UsbConfig,
    NULL);
  ...
  for (Index = 0; Index < USB_CORE_MAX_NUM_SDM_660; Index++) {
  ...
  WRN_CHK("UsbCSetPortRole failed: %r", Status);

  // Check if host mode is supported in UEFI.
  if (!FeaturePcdGet(HostModeSupported)) {
    gUsbConfigDevice[USB_CORE_0_SDM_660].EnableStaticCfg.Enabled = TRUE;
    gUsbConfigDevice[USB_CORE_0_SDM_660].EnableStaticCfg.UsbMode = USB_DEVICE_MODE_SS;
  }
  else {
    Status = gPmicUsbProtocol->UsbCSetPortRole(EFI_PM_USB_TYPEC_ROLE_DRP);
    ...
    Status = UsbPortGetRtConnectionMode(gUsbConfigDevice[USB_CORE_0_SDM_660].UsbPortHandle, &PortMode);
    ERR_CHK(" Cannot Get Connection Mode for Core %d: %r", USB_CORE_0_SDM_660, Status);
    if (PortMode == USB_TYPEC_PORT_MODE_DFP) {
      DBG(EFI_D_WARN, "Enable Host Mode");
      gUsbConfigDevice[USB_CORE_0_SDM_660].EnableStaticCfg.Enabled = TRUE;
      gUsbConfigDevice[USB_CORE_0_SDM_660].EnableStaticCfg.UsbMode = USB_HOST_MODE_XHCI;
      gUsbConfigDevice[USB_CORE_0_SDM_660].UsbCoreInterface->ModeType = USB_HOST_MODE;
    }
    else {
  ...
  )
{
  ...
  UsbCoreIfc = gUsbConfigDevice[USB_CORE_0_SDM_660].UsbCoreInterface;
  ...
  if (!FeaturePcdGet(HostModeSupported)) {
	...
	ERR_CHK("NOT EXPECTED TO REACH HERE IN STARLORD: %r", Status);
    if (gUsbConfigDevice[USB_CORE_0_SDM_660].EnableStaticCfg.Enabled == TRUE &&
        gUsbConfigDevice[USB_CORE_0_SDM_660].EnableStaticCfg.UsbMode == USB_HOST_MODE_XHCI) {
  ...
  )
{
  ...
  if ((CoreNum > USB_CORE_0) || (ModeType >= USB_INVALID_MODE))
  {
    Status = EFI_INVALID_PARAMETER;
  ...
  if ((ModeType == USB_DEVICE_MODE_HS) || (ModeType == USB_DEVICE_MODE_SS)) {
  ...
  )
{
  ...
  // For Sdm660, only switch from USB_DEVICE_MODE_SS to USB_HOST_MODE_XHCI
  switch (CurrModeType) {
...
{
  // for Sdm660, the function is not used.
  ...
  )
{
  ...
  if (CoreNum > USB_CORE_0 || !ModeType) {
    Status = EFI_INVALID_PARAMETER;
    ERR_CHK("invalid parameter");
  }
  ...
  IN QCOM_USB_CORE_NUM    CoreNum)
{
  ...
  DBG (EFI_D_WARN,"LaneNum = %d", LaneNum);
  ...
  if (Message == UsbCfgPortMsgPartnerAttach) { //Attach
    ...
    if (FeaturePcdGet(InitUsbControllerOnBoot)) {
      ...
      if (gUsbConfigDevice[CoreNum].EnableStaticCfg.Enabled == TRUE &&
        ...
        // static cfg for the host mode is not set => the hardware will be
        // configured in the device mode. Start the controller if the port is
        // UFP and the charger type is SDP or CDP
        if (IS_PORT_UFP(PortMode) && IS_SDP_CDP(ChargerType)) {
          StCtrler = TRUE;
        }
      }
