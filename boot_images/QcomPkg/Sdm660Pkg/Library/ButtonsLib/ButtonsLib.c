...
--------   ---      ----------------------------------------------------------
12/07/16   sv       Added support for SDM660
09/13/16   ma       Support QRD8998
...
//KeyMap for CDP
KEY_TYPE KeyMap_Sdm660_CDP[ NUMBER_OF_KEYS] =
{
   PWR, VOL_UP, VOL_DOWN, HOME, NONE
};

//KeyMap for MTP and FLUID
KEY_TYPE KeyMap_Sdm660_MTP_FLUID[ NUMBER_OF_KEYS] =
{
   PWR, VOL_UP, VOL_DOWN, NONE, NONE
};

//KeyMap for LIQUID
KEY_TYPE KeyMap_Sdm660_LIQUID[ NUMBER_OF_KEYS] =
{
   PWR, VOL_UP, VOL_DOWN, HOME, NONE
};

//KeyMap for QRD
KEY_TYPE KeyMap_Sdm660_QRD[ NUMBER_OF_KEYS] =
{
   PWR, VOL_UP, VOL_DOWN, NONE, NONE
};
   ...
   )
{
   ...
   case EFI_PLATFORMINFO_TYPE_CDP:
      CopyMem( pKeyMap, KeyMap_Sdm660_CDP, NUMBER_OF_KEYS*sizeof(KEY_TYPE) );
      break;
   case EFI_PLATFORMINFO_TYPE_MTP:
   case EFI_PLATFORMINFO_TYPE_FLUID:
      CopyMem( pKeyMap, KeyMap_Sdm660_MTP_FLUID, NUMBER_OF_KEYS*sizeof(KEY_TYPE) );
      break;
   case EFI_PLATFORMINFO_TYPE_LIQUID:
      CopyMem( pKeyMap, KeyMap_Sdm660_LIQUID, NUMBER_OF_KEYS*sizeof(KEY_TYPE) );
      break;
   case EFI_PLATFORMINFO_TYPE_QRD:
      CopyMem( pKeyMap, KeyMap_Sdm660_QRD, NUMBER_OF_KEYS*sizeof(KEY_TYPE) );
      break;
  ...
  *(pButtonArray + 0) = ButtonPressed;
   ...
   *(pButtonArray + 2) = ButtonPressed;
       ...
       (PlatformType == EFI_PLATFORMINFO_TYPE_CDP))
   {
      //for camera snapshot
      ButtonPressed = FALSE;
      Status = ReadGpioStatus(PM_DEVICE_1, CAMERA_SNAPSHOT_BUTTON_GPIO, &ButtonPressed );
      if ( EFI_ERROR (Status) )
      {
