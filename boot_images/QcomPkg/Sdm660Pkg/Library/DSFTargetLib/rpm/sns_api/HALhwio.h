...
$Header: //components/rel/boot.xf/1.4/QcomPkg/Sdm660Pkg/Library/DSFTargetLib/rpm/sns_api/HALhwio.h#4 $
 ...
 */
  ...
  #define __inpw(port)        bus_read(port)
  #define __inpdw(port)       bus_read(port)

  #define __outp(port, val)   printf("__outp\n");   bus_write (port, val)
  #define __outpw(port, val)  printf("__outpw\n");  bus_write (port, val)
  #define __outpdw(port, val) printf("__outpdw\n"); bus_write (port, val)
#else
  #define __inp(port)         (*((volatile uint8 *) (port)))
  #define __inpw(port)        (*((volatile uint16 *) (port)))
  #define __inpdw(port)       (*((volatile uint32 *) (uintnt)(port)))
  #define __outp(port, val)   (*((volatile uint8 *) (port)) = ((uint8) (val)))
  #define __outpw(port, val)  (*((volatile uint16 *) (port)) = ((uint16) (val)))
  #define __outpdw(port, val) (*((volatile uint32 *) (uintnt)(port)) = ((uint32) (val)))
#endif
