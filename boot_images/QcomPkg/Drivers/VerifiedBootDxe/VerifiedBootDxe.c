...
/*=============================================================================
 ...
 --------   ---      ----------------------------------------------------------
 06/14/17   AC      Add bootstate with ROT
  ...
  */
  ...
  case WRITE_CONFIG:
    if (secure_device_nocheck_rpmb) {
      ...
      status = QseeComProtocol->QseecomSendCmd(
        ...
        */
      if ((is_secure_device(&secure_device)) != EFI_SUCCESS) {
          DEBUG((EFI_D_ERROR, "VB: RWDeviceState: Cannot read security state!\n"));
          goto exit;
        }

      if (secure_device && (status != EFI_SUCCESS || wread_rsp.status != 0)) {
          DEBUG((EFI_D_ERROR, "VB: RWDeviceState: WRITE_CONFIG: Can't read security state! \
                               status: %d read status: %d \n", status, wread_rsp.status));
          status = EFI_FAILURE;
          goto exit;
      }
  ...
  case READ_CONFIG:
    if (secure_device_nocheck_rpmb) {
      ...
      if (secure_device && (status != EFI_SUCCESS || read_rsp.status != 0)) {
          DEBUG((EFI_D_ERROR, "VB: RWDeviceState: READ_CONFIG: Can't read security state! \
                         ...
                         OUT BOOLEAN *State)
{
  ...
  if ((status = is_secure_device_nocheck_rpmb (&secure_device_nocheck_rpmb)) != EFI_SUCCESS) {
    DEBUG((EFI_D_ERROR, "VB: Is_Device_Secure: Cannot read security state with no rpmb check!\n"));
    goto exit;
  }
  
  if (!secure_device && !secure_device_nocheck_rpmb)
     *State = 0;
  else
     *State = 1;
