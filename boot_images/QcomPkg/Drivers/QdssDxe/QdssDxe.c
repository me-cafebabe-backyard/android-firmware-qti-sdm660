...
Notice that changes are listed in reverse chronological order.
 ...
 $Header: //components/rel/boot.xf/1.4/QcomPkg/Drivers/QdssDxe/QdssDxe.c#5 $
 $DateTime: 2017/04/25 03:06:40 $
 $Author: pwbldsvc $

 When     Who    What, where, why
 -------- ---    -----------------------------------------------------------
 15/07/16 vk     Log only when set
  ...
  )
{
  ...
  UINTN enable_qdss_size=sizeof(UINT8);
    ...
    /*QDSS Timestamp Work around*/
  HAL_qdss_ssc_csr_HalConfigInit();
  ...
  if ((EFI_SUCCESS == status) && (0 != enable_qdss_data)) {
     ...
     DEBUG(( EFI_D_INFO, "EnableQDSSOnBoot not set status=%x, data=%d\n",
