...
/*=============================================================================
 ...
 --------   ---      ----------------------------------------------------------
 07/27/16   ma       Move creation of CallbackTimer event to KeysDxeRegisterKeyNotify
 07/13/15   al       Changing TPL_NOTIFY to TPL_CALLBACK
 04/17/14   ma       Fix KW error
 11/18/13   ma       Add pwr key to button array
 08/26/13   ma       Reduce polling period to 50ms
 06/17/13   vk       Remove UefiRunTime lib
 04/11/13   ma       Clear timer buffer when timer is off
 04/09/13   ma       Send only one notification when keys are pressed
 02/08/13   ma       Temporarily disable periodic timer
 01/29/13   ma       Temporarily increase polling period to 300ms
 11/07/12   ma       Adding keyshiftstate,keytogglestate for logo key
 10/22/12   ma       Updating copyrights , enabling polling for 100ms
 09/10/12   ma       Updating license info
 08/11/12   ma       Temporarily disabling polling
 07/30/12   ma       Port to 8974 branch
 05/11/12   sr       Changes for Reset based solution to avoid flooding issue.
 05/08/12   ma       CR:345658 fix for FFU Hotkey issue
 02/07/12   ma       Porting Changes from KeypadDxe to ButtonsDxe.
 01/11/12   hkm      Code review fixes
...
#define QCOM_KEYPAD_DEVICE_GUID \
...
EFI_INPUT_KEY  keyBuffer[MAX_KEYS_TO_BUFFER];
EFI_INPUT_KEY* keyBufferRead;
EFI_INPUT_KEY* keyBufferWrite;

EFI_INPUT_KEY  TimerEfiKeyBuffer[MAX_KEYS_TO_BUFFER];
