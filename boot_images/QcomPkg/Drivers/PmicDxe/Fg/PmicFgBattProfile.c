...
--------   ---     ----------------------------------------------------------
02/23/16   mr      Adding profile rev to battery profile info
...
**/
EFI_STATUS PmicFgBattProfileInit(EFI_PM_FG_CFGDATA_TYPE *pFgConfigData, EFI_PM_BATTID_CFGDATA_TYPE *pFgBattIdCfgData)
{
  UINT32 Count = 0;

  SetMem (&gFgBattIdCfgData,  sizeof(EFI_PM_BATTID_CFGDATA_TYPE), 0x00);
    /* Assign global Config Data */
  CopyMemS(&gFgBattIdCfgData, sizeof(EFI_PM_BATTID_CFGDATA_TYPE), pFgBattIdCfgData, sizeof(EFI_PM_BATTID_CFGDATA_TYPE));
...
/*
  ...
  if ((EFI_SUCCESS == Status) && (NULL != FgProfile.Pd[0].addrData) && (PM_PARSER_STATUS_PARSE_ERROR != Parserstatus))
  {
    ...
    Battery_Id.BatteryIdTolerance = gFgBattIdCfgData.BatteryIdTolerance;
  ...
  else
  {
    ...
    Status |= PmicFgSram_Init(PmicDeviceIndex, FG_SRAM_STATUS_AVAILABLE, &gFgConfigData, &gFgBattIdCfgData);
  }
