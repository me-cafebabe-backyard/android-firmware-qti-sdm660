...
/*=============================================================================
 ...
 --------   ---     -----------------------------------------------------------
 10/26/17   pbitra    Fixing Array bound for SRAM data
 07/24/17   pbitra    Do One time configuration on all boot, dvdd reset only on
                      20th addr data non zero
 05/09/17   pbitra    Fixed fail to detecting profile loaded when profile loaded
                      by HLOS and FG reset in uefi case
 04/12/17   pbitra    Detect the bad conditions of FG SRAM (trim registers) 
					  and issue VDD hard reset on PM/PMi to recover
 03/30/17   va        Fall back to IMA signal read if IMA burst fails
 03/27/17   pbitra    Decoupling sram one time configs from profile load
 03/01/17   va        CC_SOC saturation auto-clear
 02/23/17   mr        Updating review comments for updating profile integrity byte
 02/16/17   ll        Updated ESR qualification current threshold (sp.esrIThrsh) to 110 mA
 01/31/17   cs        need to lower the ESR qualification threshold to 110 mA
 01/31/17   va        Adding Rconn negative configuration
 01/10/17   va        Adding Rconn configuration for considering compensation
 12/09/16   va        Updating review comments for updating profile integrity byte
 ...
 09/28/16   sv        Updated to support SDM660.
...
STATIC EFI_PM_BATTID_CFGDATA_TYPE  gFgBattIdCfgData;
...
*/
EFI_STATUS PmicFgSram_Init(
    UINT32 PmicDeviceIndex, FgSramState FgSramState,
    EFI_PM_FG_CFGDATA_TYPE *pFgConfigData,
...
)
{
  ...
  CopyMemS(&gFgBattIdCfgData, sizeof(EFI_PM_BATTID_CFGDATA_TYPE), pBattIdCfgDa, sizeof(EFI_PM_BATTID_CFGDATA_TYPE));
...
EFI_STATUS PmicFgSram_PollLatchedIrq(UINT32 PmicDeviceIndex, pm_fg_batt_soc_irq_type irq_type, BOOLEAN *bIrqUpdated)
{
  ...
  *bIrqUpdated = FALSE;
  ...
  do{
      ...
      if(err_flg == PM_ERR_FLAG__SUCCESS && TRUE == bIrqUpdateStatus)
      {
...
EFI_STATUS PmicFgSram_PollRisingEdgeIrq(UINT32 PmicDeviceIndex, pm_fg_batt_soc_irq_type irq_type, BOOLEAN *bIrqUpdated)
{
  ...
  *bIrqUpdated = FALSE;
  ...
  do{
      ...
      err_flg |=  pm_fg_batt_soc_irq_status(PmicDeviceIndex, irq_type, PM_IRQ_STATUS_LATCHED, &bIrqUpdateStatus);
      if(err_flg != PM_ERR_FLAG__SUCCESS)
      {
...
EFI_STATUS PmicFgSram_EnableCharging(UINT32 PmicDeviceIndex, BOOLEAN bSet)
{
  ...
  if(EFI_SUCCESS == Status )
  {
    ...
    Status |= PmicSchgProtocol->EnableCharger(PmicDeviceIndex, bSet);
  }
...
EFI_STATUS PmicFgSram_ImaReadSingleOffset(UINT32 PmicDeviceIndex, UINT16 ReadAddress,
                                               UINT8 *Data, UINT8 Offset)
{
  EFI_STATUS Status   = PM_ERR_FLAG__SUCCESS;
...
EFI_STATUS PmicFgSram_SramHealthy(UINT32 PmicDeviceIndex)
{
  ...
  if((EFI_SUCCESS == Status ) && (TRUE == UnExpectedMatch))
  {
    ...
    DEBUG(( EFI_D_WARN, "PmicDxe:: %a SRAM contents are corrupted, issuing dvdd reset \r\n", __FUNCTION__));
...
EFI_STATUS PmicFgSram_VerifySramContent(UINT32 PmicDeviceIndex, BOOLEAN *UnExpectedMatch)
{
  ...
  *UnExpectedMatch = FALSE;
  ...
  /* If read and unexpected value match then device needs to do dvdd reset to re cover SRAM trim and other contents 
     ...
     verifying address 20 is non zero */
...
EFI_STATUS PmicFgSram_SetOneTimeSramCfgs(UINT32 PmicDeviceIndex)
{
  ...
  Status = PmicFgSram_ImaWriteSingleOffset(PmicDeviceIndex, FG_SRAM_SP_ESRITRSH_ADDR,
    ...
    if((gFgConfigData.VBtEmpty == 0 )|| (gFgConfigData.VBtEmpty > FG_SRAM_ENUM_VBAT_EMPTY_MAX))
    {
      VBtEmptyValue = FG_SRAM_VEMPTY_REG_VALUE ;
    }
    else
    {
      VBtEmptyValue = ((gFgConfigData.VBtEmpty - FG_SRAM_ENUM_VBAT_EMPTY_OFFSET) * FG_SRAM_VEMPTY_VOLTAGE_MAX_ENCODING_VALUE ) / FG_SRAM_VEMPTY_VOLTAGE_MAX;/* Data in milliVolt */
    }
...
)
{
 ...
 // UINT8    VBtEmptyValue          = 0;
  ...
  PMIC_FILE_DEBUG(( EFI_D_WARN, "PmicDxe: Battery Profile Name flashed = %a \r\n", Profile->Pd[ProfileIndex].Pp.BatteryName));
...
pm_err_flag_type PmicFgSram_ResetIma(uint32 PmicDeviceIndex)
{
  ...
  do
  {
    ...
    if(nPauseDelay > FG_SRAM_PAUSE_DELAY_MAX_TIME)
    {
      ...
      err_flg = PM_ERR_FLAG__FG_SRAM_DEVICE_ACCESS;
      break;
    }
  ...
  }while((err_flg == PM_ERR_FLAG__SUCCESS ) && ((bExceptionSts == TRUE ) || (fg_memif_data != 0x00)));
...
PmicFgSram_ClearDmaLog(uint32 PmicDeviceIndex)
{
   ...
   if (dma_err_sts)
   {
      ...
      for (poll_count=0; poll_count < PM_FG_NUM_OF_DMA_ERR_STS_POLL; poll_count++) //Poll for DMA err status
      {
         ...
         if ((dma_err_sts == FALSE) || (err_flag != PM_ERR_FLAG__SUCCESS))
         {
            break;
         }
      }

      if ((dma_err_sts == TRUE)|| (err_flag != PM_ERR_FLAG__SUCCESS))
      {
         PMIC_DEBUG(( EFI_D_WARN, "PmicDxe: %a DMA Clear Log failed \r\n", __FUNCTION__));
         return err_flag = PM_ERR_FLAG__FG_SRAM_DEVICE_ACCESS;
      }
      else
      {
                                                ...
                                                UINT32 fg_memif_data[BATTARY_PROFILE_MAX_SIZE])
{
  ...
  if(nExcpCnt)
  {
    ...
    PMIC_DEBUG(( EFI_D_WARN, "PmicDxe:: %a IMA Exception occured during burst read = %d \r\n", __FUNCTION__, nExcpCnt));
    PMIC_DEBUG(( EFI_D_WARN, "PmicDxe:: %a Falling back to IMA read single \r\n", __FUNCTION__));

    nExcpCnt = 0;
    do{

      Status = PmicFgSram_ImaReadSingle(PmicDeviceIndex, StartAddress, &ReadData, &bBeatCountMatch);
      if(Status != EFI_SUCCESS)
      {
        PMIC_DEBUG(( EFI_D_WARN, "PmicDxe:: %a Single Ima Read Failed = %r  Addr = %d, bBeatCountMatch = %d\r\n", __FUNCTION__, Status, StartAddress, bBeatCountMatch));
        break;
      }
      //Update Read Profile array 
      fg_memif_data[ReadCount] = ReadData;
      ...
      StartAddress += FG_SRAM_ADDR_INCREMENT;
      ReadCount++;
     }while((StartAddress < (BATTARY_PROFILE_MAX_SIZE))
           && (ReadCount < BATTARY_PROFILE_MAX_SIZE) 
           && (ReadCount <= (noOFWordToRead)));
  }
  else
  {
    /* Read last byte */
...
EFI_STATUS PmicFgSram_ImaReadCust(UINT32 PmicDeviceIndex, FgSramAddrData *SramAddrData, UINT32 AddrDataCount)
{
  ...
  if(TRUE == bMemAvailable)
  {
    ...
    }while((ReadCount < AddrDataCount) && (PM_ERR_FLAG__SUCCESS == err_flg));
  }
  else
  {
    PMIC_DEBUG(( EFI_D_WARN, "PmicDxe:: %a IMA Polling Failed updating read address \r\n", __FUNCTION__));
                                   ...
                                   UINT8 noOFWordToRead)
{
  ...
  if(FALSE == gFgConfigData.FgSramDbgCfgs.DumpSram)
  {
                                                ...
                                                FgBattAddrData fg_memif_data[BATTARY_PROFILE_MAX_SIZE])
{
  ...
  if((StartAddress > FG_SRAM_END_ADDR) || (noOFWordToWrite > FG_SRAM_END_ADDR) || (noOFWordToWrite > BATTARY_PROFILE_MAX_SIZE))
  {
    return EFI_INVALID_PARAMETER;
  }
  ...
  if((nExcpCnt) && (WriteCount < BATTARY_PROFILE_MAX_SIZE))
  {
    PMIC_DEBUG(( EFI_D_WARN, "PmicDxe:: %a IMA Exception occured during burst write = %d \r\n", __FUNCTION__, nExcpCnt));
    PMIC_DEBUG(( EFI_D_WARN, "PmicDxe:: %a Falling back to IMA write single \r\n", __FUNCTION__));

    nExcpCnt = 0;

    /* Attempt burst write with signle as fall back */
    do{

      /* Write Single Now */
      Status |= PmicFgSram_ImaWriteSingle(PmicDeviceIndex, StartWriteAddress, fg_memif_data[WriteCount].Data);
      if(Status != EFI_SUCCESS)
      {
        PMIC_DEBUG(( EFI_D_WARN, "PmicDxe:: %a Error @ IMA Addr = %d Write Data = 0x%x Success \r\n", __FUNCTION__, StartWriteAddress,fg_memif_data[WriteCount].Data ));
      }
...
)
{
  ...
  /* Do burst write */
  PMIC_DEBUG(( EFI_D_WARN, "PmicDxe:: %a Loading Profile to FG SRAM, noOFWordToWrite = %d \r\n", __FUNCTION__,noOFWordToWrite));

  Status = PmicFgSram_ImaWriteBurst(PmicDeviceIndex, CurrSramProfileAddress, noOFWordToWrite, profileArray);
