...
{
  pm_err_flag_type  errFlag = PM_ERR_FLAG__SUCCESS;
  STATIC boolean XvddStatus = FALSE;
  STATIC boolean ReturnSavedXvdd = FALSE;
  ...
  case EFI_PM_PWRON_PON_PBL_STATUS_XVDD_RB_OCCURRED:
    if(ReturnSavedXvdd == FALSE)
    {
      errFlag = pm_pon_pbl_get_status(PmicDeviceIndex, PM_PON_PBL_XVDD_RB_OCCURRED, &XvddStatus);
      
      if(XvddStatus == TRUE && PM_ERR_FLAG__SUCCESS == errFlag)
      {
        /*if xvdd then write 1 to xvdd_rb register to track status in next boot*/
        errFlag |= pm_pon_set_xvdd_rb(PmicDeviceIndex, TRUE);
      }

      if(errFlag == PM_ERR_FLAG__SUCCESS)
      {
        ReturnSavedXvdd = TRUE;
      }
    }
    *Status = XvddStatus;
    break;
