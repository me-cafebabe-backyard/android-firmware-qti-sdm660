...
--------   ---     ----------------------------------------------------------
04/04/17   pbitra  For Type-C, initialize ICL for all charger as 500mA and 
				   increase it to 1500mA in case charger is not SDP/Float charger
04/03/17   pbitra  Add re run APSD for uUSB
02/20/17   mr      Fixed Battery missing not detected after battery missing shutdown and reboot
01/19/17   cs      battery missing is true either latched or RT STATUS is TRUE.
01/10/17   cs      add config to enable/disable INOV
12/05/16   cs      fixed battery missing not detected after battery missing shutdown
11/28/16   mr      Added API to get SCHG Charger PmicIndex
...
STATIC EFI_SCHG_DEBUG_CFGDATA_TYPE  gSchgDbgCfgData = {0};
...
EFI_STATUS
EFIAPI
EFI_PmicSchgDumpPeripheral(IN UINT32 PmicDeviceIndex)
{
...
(
  ...
  IN EFI_SCHG_DEBUG_CFGDATA_TYPE SchgDbgCfgData
)
{
  EFI_STATUS Status = EFI_SUCCESS;
  pm_err_flag_type errFlag = PM_ERR_FLAG__SUCCESS;
  ...
  DALSYS_memscpy(&gSchgDbgCfgData, sizeof(EFI_SCHG_DEBUG_CFGDATA_TYPE), &SchgDbgCfgData, sizeof (EFI_SCHG_DEBUG_CFGDATA_TYPE));

  /* Perform Charger initial Configuration for Type-C*/
 ...
 errFlag |= PmicSchgInitialChargerCheckup(PmicDeviceIndex);
  ...
  if(gSchgDbgCfgData.EnableChargerFgDump)
  {
    EFI_PmicSchgDumpPeripheral(PmicDeviceIndex);
  }
...
)
{
  ...
  /* Perform Charger initial Configuration for Type-C before exit UEFI */
  errFlag |= PmicSchgSetChargerDefaultSetting(PmicDeviceIndex);
  ...
  *PortType = EFI_PM_SCHG_CHG_PORT_INVALID;
  ...
  do
  {
    ...
    else
    {
      ...
      PMIC_UART_DEBUG(( EFI_D_WARN, "PmicDxe:: %a APSD not done, stalling for 200mS before re-checking \n\r", __FUNCTION__));
...
{
  if(PM_ERR_FLAG__SUCCESS != pm_schg_otg_enable(PmicDeviceIndex, Enable))
  {
    return EFI_DEVICE_ERROR;
      }
  return EFI_SUCCESS;
...
)
{
    ...
    err_flag |= pm_schg_otg_irq_status(PmicDeviceIndex, PM_SCHG_OTG_IRQ_OTG_OVERCURRENT, PM_IRQ_STATUS_RT, (boolean *)&OverCurrent);

    *pOtgStatus = (OverCurrent)? EFI_PM_SCHG_OTG_ERROR : (EFI_PM_SCHG_OTG_STATUS_TYPE)TempOtgStatus ;
...
)
{
  ...
  errFlag  = pm_schg_usb_set_usbin_option1_cfg(PM_DEVICE_0, PM_SCHG_USB_HVDCP_AUTH_ALG_EN_CFG, pSchgHvdcpDataType->bEnHvdcp);
  errFlag |= pm_schg_usb_set_usbin_option1_cfg(PM_DEVICE_0, PM_SCHG_USB_HVDCP_AUTONOMOUS_MODE_EN_CFG, pSchgHvdcpDataType->bEnHvdcp);
  if (PM_ERR_FLAG__SUCCESS != errFlag)
  {
    return EFI_DEVICE_ERROR;
  }
...
EFI_STATUS PmicSchgSetChargerDefaultSetting(UINT32 PmicDeviceIndex)
{
  ...
  /* If we fail to get connector type for any reason, we assume it is Type-C */
  if((PM_ERR_FLAG__SUCCESS != errFlag ) || (UsbConnectorType == PM_SCHG_USB_CONNECTOR_TYPEC)) 
  {
...
EFI_STATUS PmicSchgInitialChargerCheckup(UINT32 PmicDeviceIndex)
{
  ...
  if((PM_ERR_FLAG__SUCCESS != errFlag ) || (UsbConnectorType == PM_SCHG_USB_CONNECTOR_TYPEC)) 
  {
		  ...
		  PortType != PM_SCHG_USB_APSD_RESULT_STATUS_INVALID)
	  {
		...
		PMIC_DEBUG((EFI_D_WARN, "PmicDxe:: %a Setting ICL = 1500 mA (port type = %d)\n\r", __FUNCTION__, PortType));      
	  }
  }
  return EFI_SUCCESS;
}
