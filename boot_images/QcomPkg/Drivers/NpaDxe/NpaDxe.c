/** @file NpaDxe.c
  ...
  Copyright (c) 2013-2016, Qualcomm Techologies Inc. All rights 
  reserved. 
...
/*=============================================================================
 ...
 --------   ---     -----------------------------------------------------------
 08/11/16   akl      Add Exit Boot Service support.
...
STATIC EFI_HANDLE  gRpmProtocolHandle = NULL;
static EFI_STATUS EFI_npa_deinit(void);
static EFI_STATUS EFI_install_rpm_protocol(VOID);
static EFI_STATUS EFI_uninstall_rpm_protocol(VOID);
...
{
    npa_remote_define_resource_cb( resource, 
                                   initial_state,
                                   define_callback,
                                   define_context );
  return EFI_SUCCESS;
...
{
    *ret_state = npa_remote_resource_local_aggregation_driver_fcn( resource, client, state );
...
{
    *ret_state = npa_remote_resource_local_aggregation_no_initial_request_driver_fcn (resource, client, state);
  return EFI_SUCCESS;
}
...
{
    *ret_state = npa_remote_resource_remote_aggregation_driver_fcn (resource, client, state );
  return EFI_SUCCESS;
}
...
{
    npa_remote_resource_available( context );
  return EFI_SUCCESS;
}
...
/**
 * <!-- EFI_ddr_health_monitor -->
 *
 * @brief DDR Health monitor request 
 *
 * @return       EFI_SUCCESS
 */
static EFI_STATUS EFI_ddr_health_monitor
(
    UINT64 address,
    UINT32 size
)
{
  UINT32 key;
  UINT32 msg_id;
  ddr_loc_size kvp_loc_size;

  /*Create msg for kvp*/
  kvp_t* msg = kvp_create(0);

  kvp_clear(msg);

  kvp_loc_size.address = address;
  kvp_loc_size.size    = size;

  key = RPM_MISC_REQ_DDR_HEALTH;

  kvp_put(msg, key, 16, (VOID *)&kvp_loc_size);
  msg_id = rpm_post_request(RPM_ACTIVE_SET, RPM_MISC_REQ_TYPE, 0, msg);
 ...
 * @return       EFI_SUCCESS
 */
static EFI_STATUS EFI_kvp_destroy
(
    kvp_t *self
)
{
...
{
  kvp_put(self, type, length, value);
  return EFI_SUCCESS;  
}
 ...
 * @return       EFI_SUCCESS
 */
static EFI_STATUS EFI_kvp_clear
(
    kvp_t *self
)
{
 ...
 *
 */
static EFI_STATUS EFI_kvp_create
(
  unsigned buffer_size,
  kvp_t **retval
)
{
    *retval = kvp_create( buffer_size );
 ...
 *
 */
static EFI_STATUS EFI_rpm_post_request
(
  rpm_set_type set,
 ...
 {
  unsigned msg_id = rpm_post_request(set, resource, id, kvps);
  if (pMsg_id != NULL)
  {
    *pMsg_id = msg_id;
  }
 ...
 *
 */
static EFI_STATUS EFI_rpm_churn_queue
(
  unsigned message_id
)
{
  rpm_churn_queue(message_id);
...
static EFI_RPMSERVICES_PROTOCOL RpmServicesProtocol =
{
  EFI_RPMSERVICES_PROTOCOL_REVISION,
  EFI_ddr_health_monitor,
  EFI_install_rpm_protocol,
  EFI_uninstall_rpm_protocol,
};

static EFI_RPM_PROTOCOL RpmProtocol =
{
  EFI_RPM_PROTOCOL_REVISION,
  EFI_kvp_destroy,
  EFI_kvp_put,
  EFI_kvp_clear,
  EFI_kvp_create,
...
static EFI_STATUS EFI_npa_deinit(void)
{
    ...
    Status = gBS->UninstallMultipleProtocolInterfaces (
                    ...
                    &gEfiRpmServicesProtocolGuid,(void **)&RpmServicesProtocol,
                    NULL );
  return Status;
}
 ...
 *
 * @brief Call NPA destructor
 *
 * @return       VOID
 */
VOID
EFIAPI
NpaPlatformBDSExitBSCallback (
  IN EFI_EVENT        Event,
  IN VOID             *Context
  )
{
...
**/
static VOID NpaRegisterEBSCallback(VOID)
{
...
static EFI_STATUS EFI_uninstall_rpm_protocol( VOID )
{
  EFI_STATUS  status = gBS->UninstallMultipleProtocolInterfaces (
                    gRpmProtocolHandle,
                    &gEfiRpmProtocolGuid,(void **)&RpmProtocol, NULL );
