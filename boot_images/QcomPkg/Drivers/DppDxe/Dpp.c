/** @file Dpp.c
  ...
  Copyright (c) 2012-2016 Qualcomm Technologies, Inc. All rights 
  reserved. 
...
--------   ---     ----------------------------------------------------------- 
08/26/16   ltg	   Added mounting of FAT partition for DPP 
  ...
  )
{
  ...
  Status = MountFatPartition(DPP_NAME);
  if (EFI_ERROR(Status))
  {
	Status = EFI_UNSUPPORTED;
	DEBUG_ERR_LABEL("\nWarning: Failed to mount DPP FAT partition \n", InitializeDppProtocolExit);
  }
