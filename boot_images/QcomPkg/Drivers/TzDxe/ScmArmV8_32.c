...
extern UINT32             ListenerTableCurrentIndex;
...
// GLOBALs
static SMC_ARG_LIST_32   *IndirectParameters = NULL;
...
ScmArmV8Initialize (VOID)
{
  ...
  IndirectParameters = (SMC_ARG_LIST_32*)UncachedAllocateAlignedPool( sizeof(SMC_ARG_LIST_32), TZ_MEM_ALIGNMENT_SIZE);
...
)
{
  ...
  {
    if( ListenerTableCurrentIndex < MAX_LISTENER_NUM )
    {
      ListenerTable[ListenerTableCurrentIndex].ListenerId      = ListenerId;
      ListenerTable[ListenerTableCurrentIndex].CallbackPtr     = CallbackPtr;
      ListenerTable[ListenerTableCurrentIndex].CallbackHandle  = CallbackHandle;
      ListenerTable[ListenerTableCurrentIndex].SharedBufferPtr = SharedBufferPtr;
      ListenerTable[ListenerTableCurrentIndex].SharedBufferLen = SharedBufferLen;
      ListenerTableCurrentIndex++;
    }
    else
    {
...
)
{
  ...
  if( ListenerTableCurrentIndex == 0 )
  {
    Status = EFI_INVALID_PARAMETER;
    goto ErrorExit;
  }
  ...
  for( i = 0; i < ListenerTableCurrentIndex; i++ )
  {
    AppDeregisterListenerSyscall->listener_id = ListenerTable[i].ListenerId;
