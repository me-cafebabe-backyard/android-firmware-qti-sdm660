/** @file Encryption.c
  ...
  Copyright (c) 2012 - 2015 Copyright Qualcomm Technologies, Inc.  All Rights Reserved.
...
// Clock Protocol pointer
EFI_CLOCK_PROTOCOL     *pClockProtocol = NULL;

/**
*  EnableClocks ()
*  @brief
*  This function turns ON the required Crypto Clocks
*
*  @param NONE
*  @return
*  EFI_STATUS
*/
EFI_STATUS EnableClocks()
{
  ...
  UINTN       ClockId;
  UINT32      Freq;
  CHAR8       AhbClkName[] = AHB_CLK_NAME;
  CHAR8       AxiClkName[] = AXI_CLK_NAME;
  CHAR8       ClkName[] = CLK_NAME;

  /* pClockProtocol is pre-initialized in EncryptionProtocolInit */
  if (pClockProtocol == NULL)
  {
    DEBUG((EFI_D_WARN,"Clock Protocol not initialized\n\r"));
    Status = EFI_INVALID_PARAMETER;
    goto Exit;
  }

  Status = pClockProtocol->GetClockID(pClockProtocol,AhbClkName,&AhbClockId);
  if (EFI_ERROR(Status))
  {
    DEBUG((EFI_D_WARN, "GetClockID failed for AHB Clock, Status = (0x%x)\r\n", Status));
    goto Exit;
  }

  Status = pClockProtocol->GetClockID(pClockProtocol,AxiClkName,&AxiClockId);
  if (EFI_ERROR(Status))
  {
    DEBUG((EFI_D_WARN, "GetClockID failed for AXI Clock, Status = (0x%x)\r\n", Status));
    goto Exit;
  }

  Status = pClockProtocol->GetClockID(pClockProtocol,ClkName,&ClockId);
  if (EFI_ERROR(Status))
  {
    DEBUG((EFI_D_WARN, "GetClockID failed for Clock, Status = (0x%x)\r\n", Status));
    goto Exit;
  }

  Status = pClockProtocol->SetClockFreqHz(pClockProtocol,
                                          ClockId,
                                          CLK_FREQ,
                                          EFI_CLOCK_FREQUENCY_HZ_AT_LEAST,
                                          &Freq);
  if (EFI_ERROR(Status))
  {
    DEBUG((EFI_D_WARN, "SetClockFreq failed for Clock, Status = (0x%x)\r\n", Status));
    goto Exit;
  }

  Status = pClockProtocol->EnableClock(pClockProtocol, AhbClockId);
  if (EFI_ERROR(Status))
  {
    DEBUG((EFI_D_WARN, "EnableClock failed for AHB Clock, Status = (0x%x)\r\n", Status));
    goto Exit;
  }

  Status = pClockProtocol->EnableClock(pClockProtocol, AxiClockId);
  if (EFI_ERROR(Status))
  {
    DEBUG((EFI_D_WARN, "EnableClock failed for AXI Clock, Status = (0x%x)\r\n", Status));
    goto Exit;
  }
  ...
  if (EFI_ERROR(Status))
  {
    DEBUG((EFI_D_WARN, "EnableClock failed for Clock, Status = (0x%x)\r\n", Status));
    goto Exit;
  }
...
*  DisableClocks ()
*  @brief
*  This function turns OFF the required Crypto Clocks
*
*  @param NONE
*  @return
*  EFI_STATUS
*/
EFI_STATUS DisableClocks()
{
  ...
  if (EFI_ERROR(Status))
  {
    ...
    Status = pClockProtocol->DisableClock(pClockProtocol, AhbClockId);
  if (EFI_ERROR(Status))
  {
    DEBUG((EFI_D_WARN, "DisableClock failed for AHB Clock, Status = (0x%x)\r\n", Status));
    goto Exit;
  }

  Status = pClockProtocol->DisableClock(pClockProtocol, AxiClockId);
  if (EFI_ERROR(Status))
  {
    DEBUG((EFI_D_WARN, "DisableClock failed for AXI Clock, Status = (0x%x)\r\n", Status));
    goto Exit;
  }
  ...
  if (EFI_ERROR(Status))
  {
    DEBUG((EFI_D_WARN, "DisableClock failed for Clock, Status = (0x%x)\r\n", Status));
    goto Exit;
  }
   ...
   )
{
  ...
  //Turn ON crypto clocks
  Status = EnableClocks();
  if (EFI_ERROR(Status))
  {
    DEBUG((EFI_D_WARN, "EnableClocks() failed, Status = (0x%x)\r\n", Status));
    goto Exit_tzt_key_provision_service_syscall;
  }
   ...
   )
{
  ...
  //Locate EFI_CLOCK PROTOCOL
  Status = gBS->LocateProtocol (&gEfiClockProtocolGuid, NULL, (VOID**) &pClockProtocol);
  if (EFI_ERROR(Status) || (pClockProtocol == NULL))
  {
    DEBUG((EFI_D_WARN, " Locate EFI_CLOCK Protocol failed, Status =  (0x%x)\r\n", Status));
    goto Exit;
  }
