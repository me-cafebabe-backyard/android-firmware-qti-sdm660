  ...
  )
{
  ...
  if (AsyncFlag == FALSE) {
    if (HubDev->Speed == EFI_USB_SPEED_SUPER) {
      ...
      //
      Status = UsbHubCtrlGetHubDesc (HubDev, HubDesc, 2, AsyncFlag, AsyncContext);
  ...
  )
{
  ...
  // Inhereit only if we are performing async control transfers
