  ...
  )
{
  ...
  USB_ASSERT_GOTO(DescBuf != NULL, ON_ERROR);
  ...
  )
{
  ...
  USB_ASSERT_RETURN (((UsbDev != NULL) && (UsbDev->Bus != NULL)), EFI_INVALID_PARAMETER);
  ...
  if (AsyncFlag == TRUE)
  {
      DEBUG (( EFI_D_INFO, "UsbCtrlRequest: Perform Async Control Transfer \n"));

      USB_ASSERT_RETURN ((AsyncConext!=NULL), EFI_INVALID_PARAMETER);

      HubIfCont = (USB_INTERFACE_CONTEXT *) AsyncConext;

      Status = UsbHcAsyncControlTransfer (
               UsbDev->Bus,
               UsbDev->Address,
               UsbDev->Speed,
               UsbDev->MaxPacket0,
               ...
               Buf,
               &Len,
               CONTROL_IN_ASYNC_POLL_TIME,
               &UsbDev->Translator,
               HubIfCont->usb_contxt->CallBackFunction,
               AsyncConext
               );
  }
  else
  {
      DEBUG (( EFI_D_INFO, "UsbCtrlRequest: Perform Sync Control Transfer \n"));
  ...
  )
{
  ...
  for (Index = 0; Index < 3; Index++) {
    Status = UsbCtrlGetDesc (UsbDev, USB_DESC_TYPE_DEVICE, 0, 0, &DevDesc, 8, FALSE, NULL );
    DEBUG ((EFI_D_INFO, "UsbGetMaxPacketSize0: index %d, status = 0x%x, DevDesc.MaxPacketSize0= 0x%x \n", Index,Status, DevDesc.MaxPacketSize0 ));

    if (!EFI_ERROR (Status)) {
      //
      // All SuperSpeed devices have a 512 byte max packet size for endpoint 0,
      // Since this doesn't fit in a byte value, bMaxPacketSize0 is defined to be 9.
      //
      // See USB 3.0 spec, 9.6.1
      //
  ...
  )
{
  ...
  //
  Status = UsbCtrlGetDesc (UsbDev, USB_DESC_TYPE_STRING, Index, LangId, &Desc, 2, AsyncFlag, AsyncConext);
  ...
  )
{
  ...
  //
  Desc = UsbGetOneString (UsbDev, 0, 0, AsyncFlag, AsyncConext);
  ...
  )
{
  ...
  //
  Status = UsbCtrlGetDesc (UsbDev, USB_DESC_TYPE_CONFIG, Index, 0, &Desc, 8, AsyncFlag, AsyncConext);
  ...
  Status = UsbCtrlGetDesc (UsbDev, USB_DESC_TYPE_CONFIG, Index, 0, Buf, Desc.TotalLength, AsyncFlag, AsyncConext);
  ...
  )
{
  ...
  //
  Status = UsbGetDevDesc (UsbDev, AsyncFlag, AsyncConext);
  ...
  for (Index = 0; Index < NumConfig; Index++) {
    Config = UsbGetOneConfig (UsbDev, Index, AsyncFlag, AsyncConext);
  ...
  //
  Status = UsbBuildLangTable (UsbDev, AsyncFlag, AsyncConext);
