  ...
  IN  VOID                                         *Context OPTIONAL  )
{
  ...
  if (UsbBus->Usb2Hc != NULL) {
    Status = UsbBus->Usb2Hc->AsyncControlTransfer (
                               UsbBus->Usb2Hc,
                               DevAddr,
                               DevSpeed,
                               MaxPacket,
...
/*
    IsSlowDevice = (BOOLEAN)(EFI_USB_SPEED_LOW == DevSpeed);
    Status = UsbBus->UsbHc->AsyncControlTransfer (
                              UsbBus->UsbHc,
                              DevAddr,
                              IsSlowDevice,
                              (UINT8) MaxPacket,
  ...
  )
{
  ...
  if (UsbBus->Usb2Hc != NULL) {
    Status = UsbBus->Usb2Hc->AsyncBulkTransfer (
                               UsbBus->Usb2Hc,
                               DevAddr,
                               EpAddr,
                               DevSpeed,
  ...
  )
{
  ...
  USB_ASSERT_RETURN ((UsbDevicePathPtr != NULL), NULL);
  ...
  )
{
  ...
  while (ListIndex != UsbIoDPList){
    ...
    //
    USB_ASSERT_RETURN ((ListItem->DevicePath != NULL), FALSE);
  ...
  )
{
  ...
  USB_ASSERT_RETURN ((ListItem != NULL), EFI_OUT_OF_RESOURCES);
  ...
  )
{
  ...
  USB_ASSERT_RETURN ((UsbIf != NULL), FALSE);
  IfDesc       = UsbIf->IfDesc;
  USB_ASSERT_RETURN ((IfDesc->ActiveIndex < USB_MAX_INTERFACE_SETTING), FALSE);
  ActIfDesc    = &(IfDesc->Settings[IfDesc->ActiveIndex]->Desc);
  DevDesc      = &(UsbIf->Device->DevDesc->Desc);
  ...
  for (Index = 0; Index < UsbIf->Device->TotalLangId; Index++) {
    StrDesc = UsbGetOneString (UsbIf->Device, DevDesc->StrSerialNumber, UsbIf->Device->LangId[Index], FALSE, NULL);
    if (StrDesc == NULL) {
      continue;
    }
  ...
  )
{
  ...
  (void)Status; // Make compiler happy
  ...
  USB_ASSERT_RETURN ((DevicePathPtr != NULL), EFI_DEVICE_ERROR);
  ...
  )
{
  ...
  USB_ASSERT_RETURN ((DevicePathPtr != NULL), FALSE);
  ...
  while (WantedListIndex != WantedUsbIoDPListPtr){
    ...
    USB_ASSERT_RETURN ((WantedListItem->DevicePath->Type == MESSAGING_DEVICE_PATH), FALSE);
