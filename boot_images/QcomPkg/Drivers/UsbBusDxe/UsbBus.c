...
/*=============================================================================
 ...
 --------   ---      ----------------------------------------------------------
 11/25/16   pm       Port EDK2 changes from BOOT.XF.1.2 
 04/30/14   ar       Cleanup for NDEBUG build  
 02/05/14   amitg    Async USB Host Stack
 01/20/14   ck       Port EDK2 host stack
 12/20/11   wufan    Install UsbPortTest protocol when USB bus is started.
  ...
  )
{
  ...
  if (EFI_ERROR (Status) || (*UsbStatus != EFI_USB_NOERROR)) {
    ...
    //
    USB_ASSERT_GOTO_SETSTS((Dev->Translator.TranslatorHubAddress < Dev->Bus->MaxDevices),
			   ON_EXIT, EFI_DEVICE_ERROR, Status);
  ...
  if ((Request->Request == USB_REQ_SET_CONFIG) &&
    ...
    if (Request->Value != 0) {
      // (AG) Added async arguments
      Status = UsbSelectConfig (Dev, (UINT8) Request->Value, FALSE, NULL);
    }
  ...
  if ((Request->Request     == USB_REQ_SET_INTERFACE) &&
    ...
    if (!EFI_ERROR (Status)) {
      USB_ASSERT_GOTO_SETSTS ((UsbIf->IfDesc->ActiveIndex < USB_MAX_INTERFACE_SETTING),
			      ON_EXIT, EFI_DEVICE_ERROR, Status);
...
/**
  Execute an async bulk transfer to the device endpoint.

  @param  Data                   The data received/transferred succesfully
  @param  DataLength             The length of the data transferred succesfully
  @param  Context                The context of this callback function
  @param  Toggle                 Data Toggle value after the data transferred
  @param  Status                 The result of USB transfer.
  ...
  )
{
  ...
  Info = (CallbackInfo *) Context;
  UsbIf = Info->UsbIf;
  Dev     = UsbIf->Device;

  EpDesc  = UsbGetEndpointDesc (UsbIf, Info->Endpoint);

  if(EpDesc == NULL) {
    ASSERT(FALSE);
    return EFI_ABORTED; // For KW
  }
  ...
  if (Info->Callback != NULL) {
    ...
    //
    gBS->RestoreTPL (OldTpl);
    (Info->Callback) (Data, DataLength, Info->ClientContext, Status);
    OldTpl = gBS->RaiseTPL (USB_BUS_TPL);
  }
  ...
  if (Status != EFI_USB_NOERROR) {
     ...
     // Clear the TRANSLATOR TT buffer, not parent's buffer
     //
     USB_ASSERT_GOTO ((Dev->Translator.TranslatorHubAddress < USB_MAX_DEVICES), ON_EXIT);
     if (Dev->Translator.TranslatorHubAddress != 0) {
       UsbHubCtrlClearTTBuffer (
           Dev->Bus->Devices[Dev->Translator.TranslatorHubAddress],
           Dev->Translator.TranslatorPortNumber,
...
/**
  ...
  @param  Callback               The function to call periodicaly when transfer is
                                 ready.
  @param  Context                The context to the client callback.
...
// Callback context storing the client callback info
  Info = AllocatePool(sizeof(CallbackInfo));
  USB_ASSERT_GOTO_SETSTS (Info, ON_EXIT, EFI_OUT_OF_RESOURCES, Status);
  Info->ClientContext = Context;
  Info->UsbIf = UsbIf;
  Info->Callback = Callback;
  Info->Endpoint = Endpoint;
  Info->Request = NULL; // Required only for Async Control Transfer

  BufNum  = 1;
  Toggle  = EpDesc->Toggle;
  Status  = UsbHcAsyncBulkTransfer (
      Dev->Bus,
      Dev->Address,
      Endpoint,
      Dev->Speed,
...
/**
  Execute an async interrupt transfer to the device endpoint.
  ...
  )
{
  ...
  for (Index = 0; Index < Dev->TotalLangId; Index++) {
    USB_ASSERT_GOTO ((Index < USB_MAX_LANG_ID), ON_EXIT);
  ...
  //
  // (AG) Added async arguments
  StrDesc = UsbGetOneString (Dev, StringIndex, LangID, FALSE, NULL);
  ...
  )
{
  ...
  HubIf  = Dev->ParentIf;
  // (AG) - added async arguments
  Status = HubIf->HubApi->ResetPort (HubIf, Dev->ParentPort,FALSE, NULL);
  ...
  HubIf->HubApi->ClearPortChange (HubIf, Dev->ParentPort, FALSE, NULL);
  ...
  Dev->Address = 0;
  Status  = UsbSetAddress (Dev, Dev->Address, FALSE, NULL);
  ...
  if (Dev->ActiveConfig != NULL) {
    // (AG) - added async arguments
    Status = UsbSetConfig (Dev, Dev->ActiveConfig->Desc.ConfigurationValue, FALSE, NULL);
    if (EFI_ERROR (Status)) {
  ...
  )
{
  ...
  if (!EFI_ERROR (Status)) {
    ...
    //
    USB_ASSERT_GOTO ((UsbBus->Usb2Hc != NULL),CLOSE_HC);
  ...
  //
  // Create and initialize an instance of the PortTestHelper protocol for this bus
  //
  Status = UsbPortTestHelperBuildProtocol(UsbBus, &PortTestHelper);

  // If we fail to build the PortTestHelper Protocol, don't fail everything
  if (EFI_ERROR(Status)) {
    DEBUG ((EFI_D_ERROR, "UsbBusStart: Failed to build PortTestHelper, only installing UsbBus"));
    //
    ...
    //
    // Install an EFI_USB_BUS_PROTOCOL to host controller to identify it.
    // Install a QCOM_USB_PORT_TEST_HELPER_PROTOCOL
    //
    Status = gBS->InstallMultipleProtocolInterfaces (
                    &Controller,
                    &gEfiCallerIdGuid,
    ...
    if (EFI_ERROR (Status)) {
      DEBUG ((EFI_D_ERROR, "UsbBusStart: Failed to install bus/port test helper protocol %r\n", Status));
      goto CLOSE_HC;
    }
  }

  //
  // Initialize the wanted child device path list, and add first RemainingDevicePath
  //
  InitializeListHead (&UsbBus->WantedUsbIoDPList);
  Status = UsbBusAddWantedUsbIoDP (&UsbBus->BusId, RemainingDevicePath);
  USB_ASSERT_EFI_ERROR_GOTO (Status, UNINSTALL_USBBUS);
  ...
  //(AG)
  // No control transfer operation in root hub init so can set the async flag to False
  Status                  = mUsbRootHubApi.Init (RootIf, FALSE, NULL);
...
UNINSTALL_USBBUS:
  ...
  if (NULL != PortTestHelper) {
    gBS->UninstallProtocolInterface (Controller, &gQcomUsbPortTestHelperProtocolGuid, &PortTestHelper);
  }
  ...
  )
{
  ...
  USB_ASSERT_EFI_ERROR_RETURN (Status, EFI_PROTOCOL_ERROR);
  ...
  )
{
  ...
  //
  // Locate PortTestHelper for the current host controller
  //
  Status = gBS->OpenProtocol (
                  Controller,
                  &gQcomUsbPortTestHelperProtocolGuid,
  ...
  // error here just means we failed to install protocol in the first place
  if (EFI_ERROR(Status)) {
    PortTestHelper = NULL;
  }
  ...
  USB_ASSERT_RETURN ((Bus->MaxDevices <= 256), EFI_DEVICE_ERROR);
  ...
  if (!EFI_ERROR (ReturnStatus)) {
    ...
    //
    // Uninstall the helper protocol and free it
    //
    if (NULL != PortTestHelper) {
      gBS->UninstallProtocolInterface (Controller, &gQcomUsbPortTestHelperProtocolGuid, PortTestHelper);
      FreePool(PORT_TEST_HELPER_FROM_THIS(PortTestHelper));
    }
