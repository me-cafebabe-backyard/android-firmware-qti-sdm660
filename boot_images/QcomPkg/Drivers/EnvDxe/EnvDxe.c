...
/*=============================================================================
 ...
 09/20/16    bd       Register 32bit mode switch protocol
...
extern EFI_GUID gDisplaySetModeSignalGuid;


/* FBPT PayloadMemBase */
STATIC UINTN FBPTPayLoadMemBase = 0;
...
VOID DisablePeriodicSerialDrain(IN EFI_EVENT Event, IN VOID *Context)
{
...
)
{
  ...
  Status = gBS->SetTimer(DrainSerialTimerEvent, TimerPeriodic, 1000000); 
  if (EFI_ERROR(Status)) {
  ...
  )
{
  ...
  DEBUG((EFI_D_ERROR, "EnvDxe Render Callback\n"));
}
...
)
{
  ...
  if(Status != EFI_SUCCESS)
  {
    DEBUG((EFI_D_ERROR, " SCM call failed to launch 32 bit image, Status =  (0x%x)\r\n", Status));
  }
...
/**
  This function will install the runtime protocol to launch a 32bit image
...
**/ 
EFI_STATUS
RegisterLaunch32BitImageProtocol (VOID)
{
  EFI_STATUS            Status = EFI_SUCCESS;
  ...
  if(Status != EFI_SUCCESS)
  {
    DEBUG((EFI_D_ERROR, " 32-bit image launch protocol installation failed, Status =  (0x%x)\r\n", Status));
  }                
  ...
  )
{
  ...
  Status = RegisterLaunch32BitImageProtocol();
  if (EFI_ERROR(Status))
