...
/*=============================================================================
 ...
 --------     ---     -----------------------------------------------------------
 12/09/16     va      Config file feature using LogFS instead of EFIESP
...
EFI_STATUS ChargerPlatformFile_ReadBattProfileFile( QCOM_CHARGER_PLATFORM_BATT_PROFILE_DATA *pBattProfileInfo)
{
  ...
  {
    CHARGER_FILE_UART_DEBUG((EFI_D_WARN, "QcomChargerDxe:: %a Could not open File = %r \r\n",__FUNCTION__, Status));
    return EFI_LOAD_ERROR;
  }
...
EFI_STATUS ChargerPlatformFile_DebugReadFile(CHAR16 *pFilePath, UINT8 **pFileBuffer, UINTN *pDataSize)
{
  ...
  {
     CHARGER_FILE_UART_DEBUG(( EFI_D_WARN, "QcomChargerDxe:: %a ChargerApp Config overwriting from Logfs Success %r Size = %d \r\n", __FUNCTION__, DebugStatus, *pDataSize));
  }
  else
  {
     CHARGER_DEBUG(( EFI_D_WARN, "QcomChargerDxe:: %a ChargerApp Config overwriting Failed %r \r\n", __FUNCTION__, DebugStatus));
  }

   CHARGER_FILE_UART_DEBUG((EFI_D_WARN, "QcomChargerDxe:: %a ChargerApp Config Read Success = %r, DataSize = %d \r\n",__FUNCTION__, DebugStatus, *pDataSize));
