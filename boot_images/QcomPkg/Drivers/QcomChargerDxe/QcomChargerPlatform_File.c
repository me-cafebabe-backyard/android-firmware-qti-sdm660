...
/*=============================================================================
 ...
 --------     ---     -----------------------------------------------------------
 12/27/16     sv      Added support for section based configuration.
...
EFI_STATUS ChargerPlatformFile_FileLogInit( QCOM_CHARGER_PLATFORM_CFGDATA_TYPE ChargerPlatformCfgData )
{
  ...
  if (TRUE == ChargerPlatformCfgData.DbgCfgData.PrintChargerAppDbgMsgToFile)
  {
    if (!RETAIL) 
    {
      /* Mounting Log fs */
    ...
    Status = ChargerLib_InitFileLog(ChargerPlatformCfgData.DbgCfgData.PrintChargerAppDbgMsgToFile);
  }
  else
  {
    CHARGER_DEBUG((EFI_D_WARN, "QcomChargerDxe:: %a File Log Disabled  = %d \r\n", __FUNCTION__, ChargerPlatformCfgData.DbgCfgData.PrintChargerAppDbgMsgToFile));
  }
