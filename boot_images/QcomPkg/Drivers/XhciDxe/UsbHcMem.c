...
Portions Copyright (c) 2013 - 2015, Intel Corporation. All rights reserved.<BR>
  ...
  )
{
  ...
  //
  USB_ASSERT_RETURN ((USBHC_MEM_UNIT * 8 <= EFI_PAGE_SIZE), NULL);
  ...
  )
{
  ...
  USB_ASSERT (((Pool != NULL) && (Block != NULL)));
  ...
  )
{
  ...
  USB_ASSERT_RETURN ((((Block != 0) && (Units != 0))), NULL);
  ...
  for (Count = 0; Count < Units; Count++) {
    USB_ASSERT_RETURN ((!USB_HC_BIT_IS_SET (Block->Bits[Byte], Bit)), NULL);
  ...
  )
{
  ...
  USB_ASSERT_RETURN (((Block != NULL)), (EFI_PHYSICAL_ADDRESS)0);
...
{
  USB_ASSERT ((Head != NULL) && (Block != NULL));
  ...
  )
{
  ...
  USB_ASSERT ((Head != NULL) && (BlockToUnlink != NULL));
  ...
  )
{
  ...
  USB_ASSERT_RETURN ((Pool->Head != NULL), EFI_INVALID_PARAMETER);
  ...
  )
{
  ...
  AllocSize = USBHC_MEM_ROUND (Size);
  Head      = Pool->Head;
  USB_ASSERT_RETURN ((Head != NULL), NULL);
  ...
  for (Block = Head; Block != NULL; Block = Block->Next) {
    ...
    if ((Block->BufHost <= ToFree) && ((ToFree + AllocSize) <= (Block->BufHost + Block->BufLen))) {
      ...
      for (Count = 0; Count < (AllocSize / USBHC_MEM_UNIT); Count++) {
        USB_ASSERT (USB_HC_BIT_IS_SET (Block->Bits[Byte], Bit));
  ...
  )
{
  ...
  //
  USB_ASSERT_RETURN(((Alignment & (Alignment - 1)) == 0), EFI_INVALID_PARAMETER);
  ...
  if (Alignment > EFI_PAGE_SIZE) {
    ...
    //
    USB_ASSERT_RETURN((RealPages > Pages), EFI_INVALID_PARAMETER);
    ...
    if (UnalignedPages > 0) {
      ...
      USB_ASSERT_EFI_ERROR_RETURN (Status, EFI_INVALID_PARAMETER);
    }
