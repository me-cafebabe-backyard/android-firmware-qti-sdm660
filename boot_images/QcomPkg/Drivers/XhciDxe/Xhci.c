...
Portions Copyright (c) 2011 - 2016, Intel Corporation. All rights reserved.<BR>
...
/*=============================================================================
 ...
 11/25/16   pm       Port from Boot.XF.1.2, update EDK2 host stack from upstream
 07/15/16   ck       Set the upper limit to re-enable the VBUS
 06/20/16   ck       Switch to PMIC USB Protocol
...
//
// Write Values for Port Link State
//
#define PLSW_U0_TRANS_VAL    0
#define PLSW_U2_TRANS_VAL    2
#define PLSW_U3_TRANS_VAL    3
#define PLSW_RxDet_TRANS_VAL 5
#define PLSW_USB2_RESUME_VAL 15

//
// PMIC secondary device index
//
#define PMIC_DEV_INDEX 1
#define MAX_EN_MSG_CNT 3
...
//
// Declaration for async bulk transfer
//
EFI_STATUS
EFIAPI
XhcAsyncBulkTransfer (
  ...
  )
{
  ...
  case EFI_USB_HC_RESET_HOST_CONTROLLER:
    ...
    USB_ASSERT_GOTO_SETSTS ((!(XHC_REG_BIT_IS_SET (Xhc, XHC_USBSTS_OFFSET, XHC_USBSTS_CNR))), ON_EXIT, EFI_NOT_READY, Status);
    if (EFI_ERROR (Status)) {
      goto ON_EXIT;
    }
    //
  ...
  )
{
  ...
  // Make sure VBUS is on
  if (Xhc->PmicUsbProtocol) {
    EFI_PM_USB_OTG_STATUS_TYPE OtgSts;
    static UINT8 OtgEnMsgCnt = 0;
    Status = Xhc->PmicUsbProtocol->GetOtgStatus(&OtgSts);
    if (EFI_ERROR(Status)) {
      DEBUG ((EFI_D_ERROR, "PmicUsbProtocol->GetOtgStatus %r\n", Status));
    } else  {
      if (OtgSts == EFI_PM_USB_OTG_ENABLED){
        // reset OTG enablement count to zero
        OtgEnMsgCnt = 0;
      }
      else 
      {
        // Re-enable VBUS if it is not enabled
        Xhc->PmicUsbProtocol->EnableOtg(TRUE);
        // Only print the message up to MAX_EN_MSG_CNT times
        if (OtgEnMsgCnt < MAX_EN_MSG_CNT) {
          DEBUG ((EFI_D_WARN, "XhcGetRootHubPortStatus: Re-enable VBUS\n"));
          OtgEnMsgCnt++;
        }
      }
    }
    // Regardless the result of calling OtgSts, set the Status back to
    // EFI_SUCCESS.
    Status = EFI_SUCCESS;
  }
  ...
  )
{
  ...
  *TransferResult = EFI_USB_ERR_SYSTEM;
  ...
  DEBUG ((EFI_D_INFO, "XhcControlTransfer: RSLT from XhcBusDevAddrToSlotId - SlotId %d, DeviceAddress = %d, RequestType 0x%x, Request 0x%x, Value 0x%x, Index 0x%x, Length 0x%x\n",
        SlotId,
        DeviceAddress,
        Request->RequestType,
        Request->Request,
  ...
  if ((Request->Request     == USB_REQ_SET_ADDRESS) &&
    ...
    DEBUG ((EFI_D_INFO, "Mapping SlotId %d, DeviceAddress %d (passed in), and BusDevAddr (recorded) %d\n",
      SlotId,
      DeviceAddress,
      (UINT8)Request->Value));
    Status = EFI_SUCCESS;
  ...
  *DataLength     = Urb->Completed;
  ...
  if (Urb->DataMap != NULL) {
    ...
    USB_ASSERT_EFI_ERROR_GOTO (Status, FREE_URB);
    if (EFI_ERROR (Status)) {
      Status = EFI_DEVICE_ERROR;
      goto FREE_URB;
    }  
  ...
  if (Xhc->UsbPortTest.QtdIndex == USB_PORT_TEST_VAR_UNDEFINED) {
    if ((Request->Request     == USB_REQ_GET_DESCRIPTOR) &&
      ...
      if ((DescriptorType == USB_DESC_TYPE_DEVICE) && ((*DataLength == sizeof (EFI_USB_DEVICE_DESCRIPTOR)) || ((DeviceSpeed == EFI_USB_SPEED_FULL) && (*DataLength == 8)))) {
          if (NULL == Data) {
             USB_ASSERT_GOTO_SETSTS (0, FREE_URB, EFI_DEVICE_ERROR, Status);
          }
          //
        ...
        if (*DataLength == ((UINT16 *)Data)[1]) {
          ...
          USB_ASSERT_GOTO_SETSTS ((Index < Xhc->UsbDevContext[SlotId].DevDesc.NumConfigurations), FREE_URB, EFI_INVALID_PARAMETER, Status);
        ...
        USB_ASSERT_GOTO_SETSTS ((HubDesc->NumPorts <= 15), FREE_URB, EFI_DEVICE_ERROR, Status);
        //
  ...
  )
{
  ...
  *TransferResult = EFI_USB_ERR_SYSTEM;
  ...
  WriteBackInvalidateDataCacheRange(Data[0], *DataLength);
  Status = XhcExecTransfer (Xhc, FALSE, Urb, Timeout);
  WriteBackInvalidateDataCacheRange(Data[0], *DataLength);
...
/**
  Sets the timer for checking the completion of Async Control transfer URB

  @param  Xhc                   The XHCI device.
  @param  PollTimer           The interval for poling for transfer completion

  @retval EFI_SUCCESS           Host controller capability were retrieved successfully.
  @retval EFI_INVALID_PARAMETER Either of the three capability pointer is NULL.
**/
static inline EFI_STATUS
XhcAsyncControlSetTimer (
  IN USB_XHCI_INSTANCE                        *Xhc
  )
{
    EFI_STATUS        Status;

    Status = gBS->SetTimer (Xhc->ControlPollTimer, TimerRelative, Xhc->ControlPollInterval * XHC_1_MILLISECOND_100NS);

    return Status;
}

/**
  Submits control transfer to a target USB device asynchronously
...
XhcAsyncControlTransfer (
  ...
  IN  EFI_USB2_HC_TRANSACTION_TRANSLATOR  *Translator,
  IN  EFI_ASYNC_USB_TRANSFER_CALLBACK_INT_BULK CallBackFunction,
  IN  VOID                                *Context OPTIONAL
  )
{
  USB_XHCI_INSTANCE       *Xhc;
  ...
  // Validate parameters
  //
  if ((Request == NULL) || (DataLength == NULL)) {
    return EFI_INVALID_PARAMETER;
  }
  ...
  if (XhcIsHalt (Xhc) || XhcIsSysError (Xhc)) {
    DEBUG ((EFI_D_ERROR, "XhcAsyncControlTransfer: HC halted at entrance\n"));
    goto ON_EXIT;
  }
  ...
  if (Urb == NULL) {
    DEBUG ((EFI_D_ERROR, "XhcAsyncControlTransfer: failed to create URB"));
    Status = EFI_OUT_OF_RESOURCES;
    goto ON_EXIT;
  }
  ...
  InsertHeadList (&Xhc->AsyncControlTransfers, &Urb->UrbList);

  // To share the Async Control In Polling interval
  // with the polling function XhcCheckAsyncControlInResult()
  // which checks if transfer completed successfully

  //gAsyncControlPollInterval = PollingInterval;
  Xhc->ControlPollInterval = PollingInterval;


  // Set the timer to check urb result
  //
  Status = XhcAsyncControlSetTimer(Xhc);
  if (EFI_ERROR (Status)) {
    DEBUG ((EFI_D_ERROR, "XhcAsyncControlTransfer: failed to set bulk timer\n"));
    goto ON_EXIT;
  }

   //
   // Ring the doorbell (despite name there's nothing interrupt related about it)
   //
   Status = RingIntTransferDoorBell (Xhc, Urb);
   if (EFI_ERROR (Status)) {
     DEBUG ((EFI_D_ERROR, "XhcAsyncBulkTransfer: failed to ring doorbell\n"));
   }
...
ON_EXIT:
  ...
  if (EFI_ERROR (Status)) {
    DEBUG ((EFI_D_ERROR, "XhcAsyncControlTransfer: error - %r\n", Status));
    if (Urb != NULL) {
...
/**
  Set the timer to check asynchronous bulk transfers.

  @param  Xhc                   Pointer to USB_XHCI_INSTANCE.

  @retval EFI_SUCCESS           The event has been set to be signaled at the requested time.
  @retval EFI_INVALID_PARAMETER Xhc's BulkPollTimer is invalid.

**/
STATIC EFI_STATUS
XhcSetAsyncBulkTimer (
    IN  USB_XHCI_INSTANCE   *Xhc
    )
{
  EFI_STATUS  Status;

  Status = gBS->SetTimer (Xhc->BulkPollTimer, TimerRelative, Xhc->BulkPollInterval * XHC_1_MILLISECOND_100NS);
...
/**
  Submits async bulk transfer to a bulk endpoint of a USB device.
  ...
  )
{
  ...
  if (XhcIsHalt (Xhc) || XhcIsSysError (Xhc)) {
    DEBUG ((EFI_D_ERROR, "XhcAsyncBulkTransfer: HC is halted\n"));
    goto ON_EXIT;
  }
  ...
  //
  // Set poll interval and insert urb in async bulk list
  //
  if ((PollingInterval > 0) && (PollingInterval < 255)) {
    Xhc->BulkPollInterval = PollingInterval;
  }
  InsertHeadList (&Xhc->AsyncBulkTransfers, &Urb->UrbList);

  //
  // Set the timer to check urb result
  //
  Status = XhcSetAsyncBulkTimer(Xhc);
  if (EFI_ERROR (Status)) {
    DEBUG ((EFI_D_ERROR, "XhcAsyncBulkTransfer: failed to set bulk timer\n"));
    goto ON_EXIT;
  }
...
ON_EXIT:
  ...
  if (EFI_ERROR (Status)) {
    DEBUG ((EFI_D_ERROR, "XhcAsyncBulkTransfer: error - %r\n", Status));
    if (Urb != NULL) {
      RemoveEntryList (&Urb->UrbList);
      XhcFreeUrb (Xhc, Urb);
    }
  ...
  )
{
  ...
  Data = UncachedAllocateZeroPool (DataLength);
  ...
  )
{
  ...
  CopyMem (&Xhc->UsbPortTest, &gXhciUsbPortTestTemplate, sizeof(QCOM_USB_PORT_TEST_PROTOCOL));

  //
  // Populate PortTestIdentity string
  //
  Status = Xhc->PciIo->GetLocation(Xhc->PciIo, NULL, NULL, &CoreNum, NULL);
  if (EFI_ERROR(Status)) {
    CoreNum = (UINTN)-1;
  }
  (VOID) UnicodeSPrint(
      Xhc->UsbPortTestIdentity,
      sizeof(Xhc->UsbPortTestIdentity),
      L"Xhci PortTest protocol on core %d",
      CoreNum
      );

  InitializeListHead (&Xhc->AsyncIntTransfers);
  InitializeListHead (&Xhc->AsyncBulkTransfers);
  InitializeListHead (&Xhc->AsyncControlTransfers);
  InitializeListHead (&Xhc->SavedAsyncTransfers);
  ...
  //
  // Create Polling Timer for asynchronous bulk transfers
  //
  Status = gBS->CreateEvent (
                  EVT_TIMER | EVT_NOTIFY_SIGNAL,
                  TPL_CALLBACK,
  ...
  if (EFI_ERROR (Status)) {
    DEBUG ((EFI_D_ERROR, "XhcCreateUsbHc: BulkPollTimer creation failed with status %r\n", Status));
    goto ON_ERROR;
  }

  //
    // Create Polling Timer for asynchronous control transfers
    //
    Status = gBS->CreateEvent (
                    EVT_TIMER | EVT_NOTIFY_SIGNAL,
                    TPL_CALLBACK,
...

{
  ...
  if (Xhc->BulkPollTimer != NULL) {
    gBS->SetTimer (Xhc->BulkPollTimer, TimerCancel, 0);
  }
  if (Xhc->ControlPollTimer != NULL) {
    gBS->SetTimer (Xhc->ControlPollTimer, TimerCancel, 0);
  }
  ...
  if (Xhc->BulkPollTimer != NULL) {
    gBS->CloseEvent (Xhc->BulkPollTimer);
  }

  if (Xhc->ControlPollTimer != NULL) {
    gBS->CloseEvent (Xhc->ControlPollTimer);
  }
  ...
  )
{
  ...
  //
  USB_ASSERT_GOTO_SETSTS ((!(XHC_REG_BIT_IS_SET (Xhc, XHC_USBSTS_OFFSET, XHC_USBSTS_CNR))), FREE_POOL, EFI_NOT_READY, Status);
  ...
  Xhc->PmicUsbProtocol = NULL;
  Status = gBS->LocateProtocol(
      &gQcomPmicUsbProtocolGuid,
      NULL,
      (VOID**)&Xhc->PmicUsbProtocol);

  if (EFI_ERROR(Status)) {
    Xhc->PmicUsbProtocol = NULL;
    DEBUG((EFI_D_WARN, "XhcDriverBindingStart: unable to locate PmicUsbProtocol %r\n", Status));
  }
  ...
  )
{
  ...
  if (EFI_ERROR (Status)) {
    DEBUG ((EFI_D_ERROR, "XhcDriverBindingStop: failed to uninstall QcomUsbPortTestProtocol\n"));
    return Status;
  }

  //
  // Stop AsyncRequest Polling timers then stop the XHCI driver
  // and uninstall the XHCI protocl.
  //
  gBS->SetTimer (Xhc->PollTimer, TimerCancel, 0);
  gBS->SetTimer (Xhc->BulkPollTimer, TimerCancel, 0);
  ...
  XhcClearBiosOwnership (Xhc);
  XhciDelAllAsyncIntTransfers (Xhc);
  XhciDelAllAsyncBulkTransfers (Xhc);
  XhciDelAllAsyncControlTransfers (Xhc);
  XhcFreeSched (Xhc);
