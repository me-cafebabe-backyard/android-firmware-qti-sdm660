...
/*
  ...
  $Header: //components/rel/boot.xf/1.4/QcomPkg/Drivers/ClockDxe/ClockSources.c#2 $
...
)
{
  ...
  {
    HAL_clk_ConfigSource(eSource, HAL_CLK_CONFIG_PLL_EVEN_OUTPUT_ENABLE);
  }

  if (nConfigMask & CLOCK_CONFIG_PLL_ODD_OUTPUT_ENABLE)
  {
    HAL_clk_ConfigSource(eSource, HAL_CLK_CONFIG_PLL_ODD_OUTPUT_ENABLE);
  }
...
)
{
  ...
  if (nConfigMask & CLOCK_CONFIG_PLL_EVEN_OUTPUT_ENABLE)
  {
    HAL_clk_ConfigSource(eSource, HAL_CLK_CONFIG_PLL_EVEN_OUTPUT_DISABLE);
  }

  if (nConfigMask & CLOCK_CONFIG_PLL_ODD_OUTPUT_ENABLE)
  {
    HAL_clk_ConfigSource(eSource, HAL_CLK_CONFIG_PLL_ODD_OUTPUT_DISABLE);
  }
...
)
{
  ...
  if (eResult != DAL_SUCCESS)
  {
    DALSYS_LogEvent (
      DALDEVICEID_CLOCK, DALSYS_LOGEVENT_FATAL_ERROR,
      "Unable to allocate %lu bytes for %lu sources.",
  ...
  Clock_InitFlags(pDrvCtxt, CLOCK_FLAG_NODE_TYPE_SOURCE);
  ...
  for (nSourceIndex = 0; nSourceIndex < pDrvCtxt->nNumSources; nSourceIndex++)
  {
    ...
    if ((eSubSource == HAL_CLK_SOURCE_NULL) ||
        (pSource->nFlags & CLOCK_FLAG_SOURCE_NOT_CONFIGURABLE))
    {
      pSource->pActiveFreqConfig = pSource->pBSPConfig->pSourceFreqConfig;
      bDetectSource = FALSE;
    }

    if( bDetectSource == TRUE)
    {
      /*
      ...
      /*
       * Make sure to preserve previously enabled PLL Voting FSM Mode.
       * This mode is FALSE by default. Force it to TRUE only if already enabled.
       * Otherwise, continue with the selection in the BSP Data.
       * Do NOT update BSP data for CPU Source PLLs.
       */
      if (DetectedConfig.bFsmMode == TRUE && (pSource->nFlags & CLOCK_FLAG_CPU_SOURCE_PLL) == 0)
      {
        pSource->pBSPConfig->nConfigMask |= CLOCK_CONFIG_PLL_FSM_MODE_ENABLE;
      }
    ...
    #else
    QDSS_SWEVENT(CLOCK_EVENT_SOURCE_CONFIG, pSource->szName);
    #endif
  }
...
)
{
  ...
  if (pSource->nFlags & CLOCK_FLAG_SOURCE_NOT_CONFIGURABLE)
  {
    return;
  }
    ...
    else
    {
      ...
      if (bStub != TRUE)
      {
        bResult =
          HAL_clk_EnableSource(
            pSource->eSource,
            pSource->pBSPConfig->eDisableMode,
            pnCalibration);
        if (bResult != TRUE)
        {
          DALSYS_LogEvent(
  ...
  {
    PEP_CLK_TRACE_SOURCE_STATUS(pSource->eSource, 1);
  }
  #else
  QDSS_SWEVENT(CLOCK_EVENT_SOURCE_STATUS,pSource->eSource);
  #endif
...
)
{
  ...
  else
  {
    ...
    if (bStub != TRUE)
    {
      HAL_clk_DisableSource(pSource->eSource, eDisableMode, pnCalibration);
    }
  }
  ...
  if (pDrvCtxt->bQdssEnabled)
  {
    PEP_CLK_TRACE_SOURCE_STATUS(pSource->eSource, 0);
  }
  #else
  QDSS_SWEVENT(CLOCK_EVENT_SOURCE_STATUS,pSource->eSource);
  #endif
...
)
{
  ...
  /*-----------------------------------------------------------------------*/
  /* Recurse to parent if this source is forbidden to be configured        */
  /*-----------------------------------------------------------------------*/

  if (pSource->nFlags & CLOCK_FLAG_SOURCE_NOT_CONFIGURABLE )
  {
    /* Try configuring parent source (for PLL_EVEN) */
    return Clock_ConfigSource( pDrvCtxt, pSource->pSource, pFreqConfig );
  }
...
)
{
  ...
  /* 1. If a frequency was specified explicitly,                           */
  /*    then find the corresponding configuration.                         */
  /* 2. Else search for the correct configuration based on the chip        */
  /*    version number.                                                    */
  /* 3. Default: Use the first config in the BSP.                          */
  /*-----------------------------------------------------------------------*/

  if (nFreqHz == 0)
  {
    eResult =
      Clock_FindSourceConfigByVersion(
