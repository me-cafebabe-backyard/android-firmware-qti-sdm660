...
static DALResult DALClock_SetExternalSourceDiv( DalDeviceHandle * h,  const char * szClock,  uint32  nDivider)
  { return Clock_SetExternalSourceDiv(HANDLE2CTXT(h), szClock, nDivider); }
...
static DALResult DALClock_SelectExternalSource( DalDeviceHandle * h,  ClockIdType  nClock,  uint32 nFreqHz,  uint32 nSource, uint32  nDivider,  uint32  nM,  uint32  nN,  uint32  n2D)
  { return Clock_SelectExternalSource(HANDLE2CTXT(h), nClock, nFreqHz, nSource, nDivider, nM, nN, n2D); }
...
static DALResult DALClock_SetUARTBaudClock( DalDeviceHandle * h,  ClockIdType  nClock, uint32 nDI, uint32 nFI, uint32 *pnResultFreqHz)
  { return Clock_SetUARTBaudClock(HANDLE2CTXT(h), nClock, nDI, nFI, pnResultFreqHz); }

static DALResult DALClock_GetCpuFrequency( DalDeviceHandle * h,  uint32 * pnFrequencyHz)
  { return Clock_GetCpuFrequencyHz(HANDLE2CTXT(h), pnFrequencyHz); }
...
)
{
   static const DalClock vtbl =
   {
      ...
      DALClock_EnterLowPowerMode,
      DALClock_ExitLowPowerMode,
      DALClock_CalcClockFrequency,
      DALClock_ConfigClock,
      DALClock_SetUARTBaudClock,
      DALClock_GetCpuFrequency,
      DALClock_SetExternalSourceDiv
   };
