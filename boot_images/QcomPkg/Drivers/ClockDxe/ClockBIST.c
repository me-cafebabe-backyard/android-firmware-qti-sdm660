  ...
  if (bBISTPassed == TRUE)
  {
        ...
        pClock->pDomain->pBSPConfig == NULL)
    {
      ...
      if (bIsClockSupportedCalcFreq)
      {
        ...
        {
          ULOG_RT_PRINTF_1 (pDrvCtxt->hClockBISTLog, "   CalcClockFrequency: %lu Hz", nFreqHz);
        }
      }
      else
      {
    ...
    else
    {
       ...
       */
        ...
        {
          ULOG_RT_PRINTF_1 (pDrvCtxt->hClockBISTLog, "   SetClockFrequency:  %lu Hz", nFreqHz);
        }
        ...
        if (pBSPConfig->nFreqHz != nFreqHz)
        {
          ...
          ULOG_RT_PRINTF_2 (pDrvCtxt->hClockBISTLog,
                            "   Frequency mismatch -- wanted: %lu Hz vs got: %lu Hz",
        ...
        if (bIsClockSupportedCalcFreq)
        {
          ...
          if (nFreqError > CLOCK_BIST_ERROR_MINIMUM_TOLERANCE)
          {
            ...
            if (nFreqError > nFreqErrorMargin)
            {
              bBISTPassed = FALSE;
              ULOG_RT_PRINTF_2 (pDrvCtxt->hClockBISTLog,
                                "   Frequency error %lu Hz exceeds error margin of %lu Hz",
                                nFreqError, nFreqErrorMargin);
            }
            else
            {
              ULOG_RT_PRINTF_2 (pDrvCtxt->hClockBISTLog,
                                "   Frequency error %lu Hz is within error margin of %lu Hz",
                                nFreqError, nFreqErrorMargin);
            }
          }
          else
          {
            ULOG_RT_PRINTF_2 (pDrvCtxt->hClockBISTLog,
                              "   Frequency error %lu Hz is within error tolerance of %d Hz",
...
)
{
  ...
  for (i = 0; i < pDrvCtxt->nNumPowerDomains; i++)
  {
    ...
    /*
     * Skip VDD_OXILI  and VDD_GPU_GX right now, because it doesn't turn on.
     */
    if (!strcmp(pPowerDomain->szName, "VDD_OXILI") || !strcmp(pPowerDomain->szName, "VDD_GPU_GX"))
    {
      continue;
    }
  ...
  for (nClockIndex = 0; nClockIndex < pDrvCtxt->nNumClocks; nClockIndex++)
  {
    ...
    if (pDrvCtxt->pBISTExceptions != NULL)
    {
      if (Clock_CheckBistExceptions(pDrvCtxt, pDrvCtxt->aClocks[nClockIndex].szName) == TRUE)
      {
        CLOCK_FLAG_SET(&pDrvCtxt->aClocks[nClockIndex], BIST_FAILURE_ANTICIPATED);
      }
    }
  ...
  {
    ULOG_RT_PRINTF_2 (pDrvCtxt->hClockBISTLog,
                      "Clock BIST Failed %lu out of %lu tests!",
