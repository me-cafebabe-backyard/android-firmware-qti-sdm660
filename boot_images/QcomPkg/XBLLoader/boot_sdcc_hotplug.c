...
--------   ---     ------------------------------------------------------------
06/01/18   c_vkuppa Adjust size of end_sector_bytes in dev_sdcc_read_bytes to 64-bit
03/07/17   bd      Read offset to be 64-bit
...
)
{
  ...
  {
    dev_loc = HOTPLUG_ITER_EMBEDDED_DEVICES_ONLY;
    dev_type = HOTPLUG_TYPE_SPI_NOR;
  }

  /* Poll the hotplug device if it is not already done */
  if (poll_done == FALSE)
  {
    boot_hotplug_poll_device(dev_type, dev_loc);
    poll_done = TRUE;
  }
  
  /* Get the Hotplug ID, used to index into image_partition_id[] */
  hotplug_id = path_id[(uint16)image_id];
  BL_VERIFY( hotplug_id != INVALID_FILE_HANDLE, BL_ERR_IMG_NOT_FOUND|BL_ERROR_GROUP_BOOT );
  BL_VERIFY( hotplug_id < IMAGE_NUMBERS, BL_ERR_IMG_NOT_FOUND|BL_ERROR_GROUP_BOOT );
  ...
  if (sbl_partitions[hotplug_id].hdev_ptr != NULL)
  {
    ...
    BL_VERIFY( (status == HOTPLUG_SUCCESS), BL_ERR_HOTPLUG|BL_ERROR_GROUP_BOOT );

    /* Check for the device block count & block size */
    BL_VERIFY( (sbl_partitions[hotplug_id].blkcnt || 
          sbl_partitions[hotplug_id].blksz), BL_ERR_SDC_INVALID_SIZE|BL_ERROR_GROUP_BOOT);
  }  
  else
  {
