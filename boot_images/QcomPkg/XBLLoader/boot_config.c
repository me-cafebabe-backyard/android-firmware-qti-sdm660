                          ...
                          boot_configuration_table_entry * boot_config_entry)
{
  ...
  sec_img_auth_error_type populate_sbl_info,error_status;
  ...
  /*------------------------------------------------------------------
   Whitelist table should be set for the image implicitly.  This is
   because a previous image loading could have set it.  If it is not
   reset or cleared then current image will be evaluated against previous
   image's whitelist table.
  ------------------------------------------------------------------*/
    ...
    switch(boot_config_entry->target_img_type)
    {
      ...
      case CONFIG_IMG_ELF:
        ...
        if(boot_config_entry->auth == TRUE)
        {
          ...
          BL_VERIFY( error_status == BL_ERR_NONE, (uint16)error_status|BL_ERROR_GROUP_SECURITY );
      ...
      case CONFIG_IMG_ELF_SEG:
        ...
        */
        ...
        /* Load the ELF segments and verify segments hash */ 
        boot_load_elf_image(boot_config_entry->whitelist_table,
                            (uint32)boot_config_entry->target_img_sec_type,
                            boot_config_entry->auth
                            );
...
)
{
  ...
  BL_VERIFY(  bl_shared_data != NULL && boot_config_table != NULL, BL_ERR_NULL_PTR_PASSED|BL_ERROR_GROUP_BOOT);

  /* For every entry in the boot configuration table */
  for(curr_entry = boot_config_table; curr_entry->host_img_id != NONE_IMG; curr_entry++)
  {
    /* Process entries sequentially only for the specific host_img, but only perform
       Pre and Post-procedure function arrays to avoid compiling in unnecessary elf 
       loading logic */
    if(curr_entry->host_img_id == host_img)
    {
      /*------------------------------------------------------------------
