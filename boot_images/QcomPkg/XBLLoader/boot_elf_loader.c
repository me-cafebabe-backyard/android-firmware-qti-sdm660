...
static Elf32_Phdr phdr_array_32[MI_PBT_MAX_SEGMENTS] SECTION(".bss.BOOT_INTERNAL_HEAP");
static Elf64_Phdr phdr_array[MI_PBT_MAX_SEGMENTS] SECTION(".bss.BOOT_INTERNAL_HEAP");

/* ELF Header */
static Elf32_Ehdr ehdr_32 SECTION(".bss.BOOT_INTERNAL_HEAP");
static Elf64_Ehdr ehdr SECTION(".bss.BOOT_INTERNAL_HEAP");

/* Output digest buffer for comparing hashes */
static uint8 verify_digest_buf[PBL_SHA_HASH_MAX_LEN] SECTION(".bss.BOOT_INTERNAL_HEAP");
extern hash_alg_t sbl_hash_algorithm SECTION(".bss.BOOT_INTERNAL_HEAP");
extern uint32 sbl_hash_len SECTION(".bss.BOOT_INTERNAL_HEAP");

/* ELF format (object class), default to 64bit */
static uint8 elf_format SECTION(".bss.BOOT_INTERNAL_HEAP");

static void *ehdr_ptr, *phdr_array_ptr SECTION(".bss.BOOT_INTERNAL_HEAP");
static uint32 ehdr_size, phdr_size SECTION(".bss.BOOT_INTERNAL_HEAP");
static uint16 phdr_num SECTION(".bss.BOOT_INTERNAL_HEAP");
                                ...
                                whitelist_region_type * config_entry_whitelist_table )
{
  ...
  {  
    ...
    {
      while((0x0 !=config_entry_whitelist_table->start_addr) &&
...
*/
static void
boot_elf_load_and_verify_image(whitelist_region_type * elf_segment_whitelist,
                               uint32 image_id, 
                               boot_boolean auth)
{
  uint32 index = 0;
  void *phdr_entry_ptr;
  sec_img_auth_error_type status;
  ...
  if(auth == TRUE )
  {
    /* Compute hash over full data segment */
     status= (sec_img_auth_error_type) boot_auth_compute_verify_hash(image_id);
...
* 
*/
static void boot_elf_image_cache_flush(whitelist_region_type * elf_segment_whitelist)
{
  uint32 index;
  void *phdr, *dest_addr;
  uintnt phdr_memsz;
...
{
  boolean success = FALSE;
  sec_img_auth_error_type sec_img_success = SEC_IMG_AUTH_FAILURE; 
...
void boot_elf_load_init_hash_segment(uint8 * partition_id, mi_boot_image_header_type *header_ptr)
{
 ...
 boot_elf_open_and_parse_elf_header(partition_id,
  ...
  BL_VERIFY( !sec_img_success, (uint16)sec_img_success|BL_ERROR_GROUP_SECURITY );
...
* 
*/
void boot_load_elf_image(whitelist_region_type * elf_segment_whitelist,
                         uint32 image_id, 
                         boot_boolean auth )
{
  ...
  */
  ...
  /* Flush the cache for image being loaded */  
  boot_elf_image_cache_flush(elf_segment_whitelist);
