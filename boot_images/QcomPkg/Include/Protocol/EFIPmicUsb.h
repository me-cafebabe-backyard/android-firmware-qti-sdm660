...
04/04/17   pbitra  Added API efi_pmic_usb_get_usb_connector_type
12/21/16   mr      Updated UsbCGetPortState API to get USB Mode and uUSB status
...
/**@ingroup efi_pmic_usb_get_usb_connector_type
  @par Summary
  This API returns USB connector type

  @param[out]  pUSBConnectorType  
...
*/
typedef
EFI_STATUS (EFIAPI *EFI_PMIC_USB_GET_USB_CONNECTOR_TYPE) (
  OUT EFI_PM_USB_CONNECTOR_TYPE  *pUSBConnectorType
);

/**@ingroup efi_pmic_usb_set_usb_connector_type
  @par Summary
  This API sets the USB connector type

  @param[in]  pUSBConnectorType  
...
*/
typedef
EFI_STATUS (EFIAPI *EFI_PMIC_USB_SET_USB_CONNECTOR_TYPE) (
...
struct _EFI_QCOM_PMIC_USB_PROTOCOL {
  ...
  EFI_PMIC_USB_GET_USB_CONNECTOR_TYPE		GetUSBConnectorType;	// Returns if it is a Type-C or a uUSB connector
  EFI_PMIC_USB_SET_USB_CONNECTOR_TYPE           SetUSBConnectorType;    // Sets the connector type to Type-C or uUSB 
};
