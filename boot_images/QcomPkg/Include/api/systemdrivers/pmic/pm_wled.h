 ...
 * @param[in]  pmic_device_index:
 *			        0 for primary pmic
 * @param[in]  avdd_soft_start_namp:
 *				       soft start in nano amp
 * @param[in] ramp_delay_usec:
...
(
  uint32 pmic_chip, uint32 avdd_soft_start_na, 
  uint32 ramp_delay_usec
);
 ...
 *             Valid values are 60000 to 397500 micro volt
 *  
 ...
 * 
 * @param[in] device_index      : PMIC device index. 0 for primary and 1 for secondary
 * @param[in] short_ckt_config  : Short circuit configuration. Refer #pm_wled_short_ckt_config_type for more info
 * 
 * @return  pm_err_flag_type 
 *          PM_ERR_FLAG__FEATURE_NOT_SUPPORTED = Feature not available on this
 *          version of the PMIC.
 *          PM_ERR_FLAG__SUCCESS               = SUCCESS.
 */
pm_err_flag_type pm_wled_config_short_ckt_protect(uint32 device_index, pm_wled_short_ckt_config_type *short_ckt_config);
 ...
 * 
 * @param[in] device_index : PMIC device index. 0 for primary and 1 for secondary
 * @param[in] sh_type      : Different type SH and soft start type. Refer #pm_wled_sh_config_type for more info
 ...
 *          PM_ERR_FLAG__SUCCESS               = SUCCESS.
 */
pm_err_flag_type pm_wled_config_sh_soft_start(uint32 device_index, pm_wled_sh_config_type sh_type, boolean enable);
