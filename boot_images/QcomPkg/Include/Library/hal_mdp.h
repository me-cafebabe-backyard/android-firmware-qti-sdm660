...
#define HAL_MDP_DSC_DATAPATH_MAX                2
#define HAL_MDP_DSC_OUTPUT_INTF_MAX             2
...
#define HAL_MDP_WDGM_MAX_COLOR_COMPONENTS                3     /* MAX Wide GM color compoenents. Color-0, Color-1, Color-2  */
#define HAL_MDP_WDGM_MAX_COMPONENT_TABLES                4     /* Total no.of.tables for each color component to hold 17x17x17 or
                                                                  5x5x5 table entried                                       */
#define HAL_MDP_WDGM_MAX_NON_UNIFORM_MAP_TABLE_ENTRIES   16    /* Non-Uniform LUT table entries for each color compoenent   */

#define HAL_MDP_WDGM_TABLE_0_ENTRIES                     1229
#define HAL_MDP_WDGM_TABLE_1_ENTRIES                     1228
#define HAL_MDP_WDGM_TABLE_2_ENTRIES                     1228
#define HAL_MDP_WDGM_TABLE_3_ENTRIES                     1228  /** Total entries are 4931 (17x17x17 matrix)                  */
#define HAL_MDP_WDGM_COARSE_MODE_TABLE_0_ENTRIES         32
#define HAL_MDP_WDGM_COARSE_MODE_TABLE_1_ENTRIES         31
#define HAL_MDP_WDGM_COARSE_MODE_TABLE_2_ENTRIES         31
#define HAL_MDP_WDGM_COARSE_MODE_TABLE_3_ENTRIES         31   /** Total entries are 125 (5x5x5 matrix)                       */

// Maximum of 4 SMP buffers are required for worst case
#define HAL_MDP_MAX_SMP_BUF                     4
/* 
...
#define HAL_MDP_FLAGS_SURFACE_ATTR_MACRO_TILE                  0x00000008


/* Flags to be used with uFeatureFlags in HAL_MDP_HwInfo() */
#define HAL_MDP_FLAG_FEATURE_PROGRAMMABLE_FETCH_START          0x00000001
...
/* This flag is used to notify pixel extension SW block if decimation supported by HW */
#define HAL_MDP_FLAGS_DECIMATION_SUPPORTED                     0x00000001
...
{
  HAL_MDP_VBIF_CLIENT_VIG0 = 0x00000000,   // xin0  (RT Port)
  HAL_MDP_VBIF_CLIENT_DMA0,                // xin1  (RT Port) 
  HAL_MDP_VBIF_CLIENT_CURSOR0,             // xin2  (RT Port)
  HAL_MDP_VBIF_CLIENT_NONE0,               // xin3  (RT Port)
  HAL_MDP_VBIF_CLIENT_VIG1,                // xin4  (RT Port)
  HAL_MDP_VBIF_CLIENT_DMA1,                // xin5   (RT Port)
  HAL_MDP_VBIF_CLIENT_WB2,                 // xin6   (RT Port)
  HAL_MDP_VBIF_CLIENT_DSIDMA,              // xin7   (RT Port)
  HAL_MDP_VBIF_CLIENT_VIG2,                // xin8   (RT Port)
  HAL_MDP_VBIF_CLIENT_DMA2,                // xin9   (RT Port)
  HAL_MDP_VBIF_CLIENT_CURSOR1,             // xin10  (RT Port)
  HAL_MDP_VBIF_CLIENT_NONE1,               // xin11  (RT Port)  
  HAL_MDP_VBIF_CLIENT_VIG3,                // xin12  (RT Port)
  HAL_MDP_VBIF_CLIENT_DMA3,                // xin13  (RT Port)
  HAL_MDP_VBIF_CLIENT_ROT_RD,              // xin0  (NRT Port)
  HAL_MDP_VBIF_CLIENT_ROT_WR,              // xin1  (NRT Port)
...
typedef enum _HAL_MDP_ScaleFilterMode
{
   ...
   HAL_MDP_SCALE_FILTER_PCMN,                               /**< PCMN filter (Reserved)               */
...
typedef enum _HAL_MDP_InterfaceStandardType
{
   ...
   HAL_MDP_INTERFACE_STANDARD_DSI,                             /**< Display Serial Interface                      */
   HAL_MDP_INTERFACE_STANDARD_HDMI,                            /**< High Definition Multimedia Interface          */
   HAL_MDP_INTERFACE_STANDARD_LCDC,                            /**< Generic LCDC Interface                        */
   HAL_MDP_INTERFACE_STANDARD_eDP,                             /**< DEPRECATED - Embedded Display Port                         */
   HAL_MDP_INTERFACE_STANDARD_LVDS,                            /**< LVDS                                          */
   HAL_MDP_INTERFACE_STANDARD_DP,                              /**< Display Port                                  */
...
typedef enum _HAL_MDP_InterfaceConnectionType
{
   ...
   HAL_MDP_INTERFACE_CONNECTION_eDP,                                      /**< DEPRECATED: Embedded Display Port                         */
...
typedef struct _HAL_MDP_HwInfo
{
   ...
   HAL_MDP_ScalingCapsType sRotatorScaleCaps;                           /**< Rotator scaling capabilities                     */
   uint32                  uNumOfChromaDownBlocks;                      /**< Number of Chroma-down blocks                     */
   uint32                  uFeatureFlags;                               /**< Chip specific supported features                 */
} HAL_MDP_HwInfo;
...
typedef struct _HAL_MDP_InterruptStatusType
{
   ...
   uint32                        uInterruptStatusRegVal; /**< Reserved                                               */

} HAL_MDP_InterruptStatusType;

/*!
 * \struct HAL_MDP_SMP_ConfigType
 *
 * The \b HAL_MDP_SMP_ConfigType provides the parameters to configure the SMP Memory Macro Blocks
 *
 */
typedef struct _HAL_MDP_SMP_ConfigType
{
   HAL_MDP_SMP_MMBClientId       eClientId;        /**< Client ID for uMacroBlockNum   */
   uint32                        uMacroBlockNum;   /**< SMPs Memory Macro Block Number */

} HAL_MDP_SMP_ConfigType;

/*!
 * HAL_MDP_TrfCtrlParamsType
 *
 * Traffic control parameters for read and write clients.
 *
 */
typedef struct 
{
   ...
   uint32                             uTotalSMPBytes;      /* Total number of bytes in SMP allocated for the client                             */
   bool32                             bEnableTrfShaper;    /* FALSE: traffic shaper disabled (pass through), TRUE: traffic shaper in control    */
   uint32                             uBwLimitInMBs;       /* client's bandwidth limit in number of MB/sec                                      */
                                                           /* ALERT: setting uBwLimitInMBs to 0 blocks the traffic when traffic shaper enabled  */
} HAL_MDP_TrfCtrlParamsType;
...
/*!
 * \struct _HAL_MDP_Interface_DualInferfaceType
 *
 * The \b _HAL_MDP_Interface_DualInferfaceType specify the dual interface (split display) configuration.
 */
typedef struct _HAL_MDP_Interface_DualInferfaceType
{
...
} HAL_MDP_Interface_DualInferfaceType;
...
typedef struct _HAL_MDP_InterfaceConfigType
{
   ...
   HAL_MDP_Interface_DualInferfaceType        *pDualInterfaceConfig;     /**< dual interface (Split display) configuration     */
...
typedef struct _HAL_MDP_Linear_GammaCorrectConfigType
{
   ...
   bool32      bSwap;               /**< SWAP the LUT table used by software and hardware                               */
   bool32      bConfigLUT;          /**< Program the LUT table                                                          */
   uint16     *puRedLUTData;        /**< Gamma LUT data for R. Should point to a buffer with HAL_MDP_GAMMA_LUT_ENTRIES  */
   uint16     *puGreenLUTData;      /**< Gamma LUT data for G. Should point to a buffer with HAL_MDP_GAMMA_LUT_ENTRIES  */
   uint16     *puBlueLUTData;       /**< Gamma LUT data for B. Should point to a buffer with HAL_MDP_GAMMA_LUT_ENTRIES  */
} HAL_MDP_Linear_GammaCorrectConfigType; 
...
typedef struct _HAL_MDP_InterfaceInfoParamsType
{
   ...
   bool32                                 *pbActive;           /**< Extract the curent state (active/idle) of interface        */
...
typedef struct _HAL_MDP_WideGamutMappingConfigType
{
   ...
   uint16  *pWdGammutTableEntries[HAL_MDP_WDGM_MAX_COLOR_COMPONENTS][HAL_MDP_WDGM_MAX_COMPONENT_TABLES];
   uint32  *pWdNonUniformMapTableEntries[HAL_MDP_WDGM_MAX_COLOR_COMPONENTS][HAL_MDP_WDGM_MAX_NON_UNIFORM_MAP_TABLE_ENTRIES];
} HAL_MDP_WideGamutMappingConfigType;

/*
 * HAL_MDP_DitherType
 *
 * Defines blending set up for foreground or background layer
 */
typedef struct _HAL_MDP_DitherType
{
   bool32   bDitherEn;
 ...
 *
 * Write-back doesn't have scalar. 
 *       In line-mode, the output size of LM = the active rectangle information of the surface
 *       In Block-mode, the destination rectangle of the input layer (DMAx) = the active rectangle information of the surface
 */
...
typedef struct _HAL_MDP_InterfaceConfigObjType
{
   ...
   HAL_MDP_Interface_DualInferfaceType   sIntrDualDispConfig;
...
/*!
 * \struct HAL_MDP_Interface_DualInferfaceConfigType
 *
 * The \b HAL_MDP_Interface_DualInferfaceConfigType specify the dual interface (split display) configuration.
 */
