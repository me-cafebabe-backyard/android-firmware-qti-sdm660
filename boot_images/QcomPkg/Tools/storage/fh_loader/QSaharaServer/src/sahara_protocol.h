 ...
 * ==========================================================================================
 *   $Header: //components/rel/boot.xf/1.4/QcomPkg/Tools/storage/fh_loader/QSaharaServer/src/sahara_protocol.h#3 $
 ...
 *  YYYY-MM-DD		who		why
 *  -----------------------------------------------------------------------------
 *  2016-10-23       sj      Added support to getting multiple command.
...
#define SAHARA_VERSION_SUPPORTED 5
...
} sahara_packet_memory_read_64bit;
...
{
  sahara_packet_header header;
  uint32_t client_command;          // command ID for target Sahara client to
                                        // execute
...
typedef struct
{
  ...
  uint32_t client_command;          // command ID for target Sahara client to
                                        // execute
  uint32_t resp_length;             // length of response returned from command
                                        // execution
...
typedef struct {
    ...
    boot_sahara_mode mode;
    boot_sahara_mode prev_mode;

	unsigned int command;
	bool ram_dump_64bit;

    // Send a Sahara Reset State Machine before sending the first hello.
    bool clear_state_machine;

	bool switch_command ;

    //after collecting do not switch to any mode, just exit.
    bool no_mode_reset;
...
typedef struct
{
  ...
  uint32_t reserved4;               // reserved field
  uint32_t reserved5;               // reserved field
} sahara_packet_reset_state_machine;

extern sahara_data_t sahara_data;
bool sahara_main(bool is_efs_sync);

#define MAX_SAHARA_MAPPINGS 64
#define MAX_IMAGE_ID_STRING_LENGTH 8
#define MAX_SAHARA_COMMAND   SAHARA_EXEC_CMD_LAST
#define CMD_FILE_NAME  "commandop%02d.bin"
#define COMMAND_ID_8 8
