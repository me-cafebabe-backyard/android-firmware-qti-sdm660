 ...
 *
 * Release Version: 16.06.09.12.36 (June 09 2016 @ 12:36:50)
 *
...
=============================================================================
 ...
 $Header: //components/rel/boot.xf/1.4/QcomPkg/Tools/storage/fh_loader/fh_loader/fh_loader.c#2 $
...
char XMLStringTable[MAX_XML_FILES][MAX_XML_SIZE];
SIZE_T NumXMLFilesInTable = 1;                      // start at 1 since <configure> will go at 0

char MaxFilesToTrack[MAX_XML_FILES][MAX_PATH_SIZE];    // --search_path=command1.xml,command2.xml,etc
SIZE_T FileToTrackCount = 0;

#define MAX_FILES_IN_OPTIONS 128
char XMLFileTable[MAX_FILES_IN_OPTIONS][MAX_PATH_SIZE];    // --sendxml=command1.xml,command2.xml,etc
char search_path[MAX_FILES_IN_OPTIONS][MAX_PATH_SIZE];     // --search_path=command1.xml,command2.xml,etc
char filter_files[MAX_FILES_IN_OPTIONS][MAX_PATH_SIZE];    // --files=sbl1.mbn,tz.mbn
char filter_not_files[MAX_FILES_IN_OPTIONS][MAX_PATH_SIZE];// --notfiles=sbl1.mbn,tz.mbn
...
void MyLog (int log_level, const char *func_name, int line_number, const char *format, ...)
{
    ...
    case LOG_ERROR:
        ThereWereErrors = 1;
        snprintf (fileloglevel, sizeof(fileloglevel),
                  "\n\n\n"
                 "\t _____                    \n"
                 "\t|  ___|                   \n"
        ...
        snprintf (screenloglevel, sizeof(screenloglevel),
                 "\n\n\n"
                 "\t _____                    \n"
                 "\t|  ___|                   \n"
                 "\t| |__ _ __ _ __ ___  _ __ \n"
    ...
    case LOG_INFO:
      snprintf (fileloglevel, sizeof(fileloglevel), "%s INFO: ", timelog);
      snprintf (screenloglevel, sizeof(screenloglevel), "%s INFO: ", timelog);
        break;
    ...
    case LOG_ONLY:
      snprintf (fileloglevel, sizeof(fileloglevel), "%s DEBUG: %s:%d ", timelog, func_name, line_number);
      snprintf (screenloglevel, sizeof(screenloglevel), "%s DEBUG: ", timelog);
        break;

    case LOG_DEBUG:
      //snprintf(fileloglevel, sizeof(fileloglevel),"%s DEBUG: %s:%d ", timelog, func_name, line_number);
      snprintf (fileloglevel, sizeof(fileloglevel), "%s DEBUG: ", timelog, func_name, line_number);
      snprintf (screenloglevel, sizeof(screenloglevel), "%s DEBUG: ", timelog);
        break;
...
void sparse_close (struct sparse_handle_t * sparse_file_handle)
{
    ...
    free (sparse_file_handle->group_file_offset);
    sparse_file_handle->start_sector = NULL;
    sparse_file_handle->bytes_remaining = NULL;
    sparse_file_handle->group_file_offset = NULL;
...
void GenerateSparseXMLTags (FILE * fd, char * filename, SIZE_T start_sector, SIZE_T SECTOR_SIZE_IN_BYTES, SIZE_T physical_partition)
{
    ...
    for (i = 0; i < sparse_file_handle->total_groups; i++)
    {
        ...
        snprintf (temp_buffer, sizeof(temp_buffer), "%s ", "<program "); //the program tag
        AppendToBuffer (tx_buffer, temp_buffer, FIREHOSE_TX_BUFFER_SIZE);

        snprintf (temp_buffer, sizeof(temp_buffer), "filename=\"%s\" ", filename);

        AppendToBuffer (tx_buffer, temp_buffer, FIREHOSE_TX_BUFFER_SIZE);

        snprintf (temp_buffer, sizeof(temp_buffer), "SECTOR_SIZE_IN_BYTES=\"%"SIZE_T_FORMAT"\" ", SECTOR_SIZE_IN_BYTES);
        AppendToBuffer (tx_buffer, temp_buffer, FIREHOSE_TX_BUFFER_SIZE);

        snprintf (temp_buffer, sizeof(temp_buffer), "num_partition_sectors=\"%"SIZE_T_FORMAT"\" ", FileSizeNumSectors); //sparse_file_handle->
        AppendToBuffer (tx_buffer, temp_buffer, FIREHOSE_TX_BUFFER_SIZE);

        snprintf (temp_buffer, sizeof(temp_buffer), "physical_partition_number=\"%"SIZE_T_FORMAT"\" ", physical_partition);
        AppendToBuffer (tx_buffer, temp_buffer, FIREHOSE_TX_BUFFER_SIZE);

        snprintf (temp_buffer, sizeof(temp_buffer), "start_sector=\"%"SIZE_T_FORMAT"\" ", sparse_file_handle->start_sector[i]);
        AppendToBuffer (tx_buffer, temp_buffer, FIREHOSE_TX_BUFFER_SIZE);

        snprintf (temp_buffer, sizeof(temp_buffer), "file_sector_offset=\"%"SIZE_T_FORMAT"\" ", sparse_file_handle->group_file_offset[i]);
        AppendToBuffer (tx_buffer, temp_buffer, FIREHOSE_TX_BUFFER_SIZE);

        snprintf (temp_buffer, sizeof(temp_buffer), "sparse=\"%s\" ", "true");
        AppendToBuffer (tx_buffer, temp_buffer, FIREHOSE_TX_BUFFER_SIZE);

        snprintf (temp_buffer, sizeof(temp_buffer), "UNSPARSE_FILE_SIZE=\"%"SIZE_T_FORMAT"\" ", sparse_file_handle->bytes_remaining[i]);
        AppendToBuffer (tx_buffer, temp_buffer, FIREHOSE_TX_BUFFER_SIZE);
...
void CreateFFUXMLTag(struct FFUImage_t * FFU, uint64_t FileOffset, int64_t StartSector, uint64_t NumSectors)
{
    ...
    snprintf(temp_buffer, sizeof(temp_buffer), "filename=\"%s\" ", FFU->filename);

    AppendToBuffer(tx_buffer, temp_buffer, FIREHOSE_TX_BUFFER_SIZE);

    snprintf(temp_buffer, sizeof(temp_buffer), "SECTOR_SIZE_IN_BYTES=\"%"SIZE_T_FORMAT"\" ", FFU->sector_size_in_bytes); 
    AppendToBuffer(tx_buffer, temp_buffer, FIREHOSE_TX_BUFFER_SIZE);

    snprintf(temp_buffer, sizeof(temp_buffer), "num_partition_sectors=\"%"SIZE_T_FORMAT"\" ", NumSectors);
    AppendToBuffer(tx_buffer, temp_buffer, FIREHOSE_TX_BUFFER_SIZE);

    snprintf(temp_buffer, sizeof(temp_buffer), "physical_partition_number=\"%"SIZE_T_FORMAT"\" ", FFU->physical_partition_number);
    AppendToBuffer(tx_buffer, temp_buffer, FIREHOSE_TX_BUFFER_SIZE);
    
    if (StartSector < 0)
      snprintf(temp_buffer, sizeof(temp_buffer), "start_sector=\"NUM_DISK_SECTORS%"SIZE_T_FORMAT"\" ", StartSector);
    else
      snprintf(temp_buffer, sizeof(temp_buffer), "start_sector=\"%"SIZE_T_FORMAT"\" ", StartSector);
    AppendToBuffer(tx_buffer, temp_buffer, FIREHOSE_TX_BUFFER_SIZE);

    snprintf(temp_buffer, sizeof(temp_buffer), "file_sector_offset=\"%"SIZE_T_FORMAT"\" ", FileOffset);
    AppendToBuffer(tx_buffer, temp_buffer, FIREHOSE_TX_BUFFER_SIZE);
...
int main (int argc, char *argv[])
{
    ...
    printf ("\nBase Version: 16.06.09.12.36");
    printf ("\nBinary build date: %s @ %s", __DATE__, __TIME__);
    printf ("\nIncremental Build version: %s\n", fh_loader_version);
    if(0) /* Enable to test SHA, the output should be c1 cd 70 1f 05 36 7f c6 fd ce 55 c0 6c bc 0a f8 2e 58 88 ca cd 55 02 ab ca 22 0a bd d5 56 a5 0b */
    {
      uint8* inputPtr;
      uint32 inputLen;
      uint8* outputPtr;
      uint8 temp_hash_value[32] = {0};
      char example[]= "<?xml version=\"1.0\" encoding=\"UTF-8\" ?>\n<data>\n<configure MemoryName=\"emmc\" Verbose=\"0\" AlwaysValidate=\"0\" MaxDigestTableSizeInBytes=\"8192\" MaxPayloadSizeToTargetInBytes=\"1048576\" ZlpAwareHost=\"1\" SkipStorageInit=\"0\" TargetName=\"8960\" />\n</data>\n";
      inputLen = strlen(example);
      inputPtr = (uint8 *)example;
      outputPtr = temp_hash_value;
      PerformSHA256(inputPtr, inputLen, outputPtr);
      printf("\nInput data:  ");

      for (i = 0; i < inputLen; i++)
      {
        printf("%02x ", inputPtr[i]);
      }
      for (i = 0; i < inputLen; i++)
      {
        printf("%c", inputPtr[i]);
    ...
    */
    ...
    for (i = 1; i < (SIZE_T) argc; i++) // skip 0 since argv[0] is fh_loader.exe
    {
        ...
        {
          if (CopyString(MainOutputDir, MyArg, 0, 0, strlen(MyArg), sizeof(MainOutputDir), strlen(MyArg)) == 0)
          {
            dbg (LOG_ERROR, "Failed to copy '%s' of length %"SIZE_T_FORMAT" bytes into MainOutputDir", MyArg, strlen (MyArg));
            ExitAndShowLog (1);
          }
    ...
    szIndex += snprintf (&szTemp[szIndex], sizeof(szTemp) - szIndex, "FH_LOADER WAS CALLED EXACTLY LIKE THIS\n************************************************\n");

    for (i = 0; i < (SIZE_T) argc; i++) // skip 0 since argv[0] is fh_loader.exe
      szIndex += snprintf (&szTemp[szIndex], sizeof(szTemp) - szIndex, "%s ", argv[i]);

    szIndex += snprintf (&szTemp[szIndex], sizeof(szTemp) - szIndex, "\n************************************************\n");
    dbg (LOG_INFO, szTemp);
    szIndex = 0;
    ...
    for (i = 1; i < (SIZE_T) argc; i++) // skip 0 since argv[0] is fh_loader.exe
    {
        ...
        else if (strncmp (MyOpt, "erase=", MAX (strlen (MyOpt), strlen ("erase=") ) ) == 0)
        {
            ...
            snprintf (temp_buffer, sizeof(temp_buffer), "SECTOR_SIZE_IN_BYTES=\"%"SIZE_T_FORMAT"\" ", SectorSizeInBytes);
            AppendToBuffer (tx_buffer, temp_buffer, FIREHOSE_TX_BUFFER_SIZE);

            snprintf (temp_buffer, sizeof(temp_buffer), "physical_partition_number=\"%d\" ", atoi (MyArg) );
            AppendToBuffer (tx_buffer, temp_buffer, FIREHOSE_TX_BUFFER_SIZE);

            AppendToBuffer (tx_buffer, "/>\n</data>", FIREHOSE_TX_BUFFER_SIZE);

            CopyString (XMLStringTable[NumXMLFilesInTable], tx_buffer, 0, 0, strlen (tx_buffer), MAX_XML_SIZE, sizeof (tx_buffer));
            NumXMLFilesInTable++;
    ...
    for (i = 1; i < (SIZE_T) argc; i++) // skip 0 since argv[0] is fh_loader.exe
    {
        ...
        {
            CopyString (ContentsXMLFilename, MyArg, 0, 0, strlen (MyArg), MAX_STRING_SIZE, MAX_STRING_SIZE);
        }
            ...
            if ( ContensXML[i].FileType == 'r') // && ContensXML[i].StorageType==fh.attrs.MemoryName[0])
            {
                ...
                if (fh.attrs.MemoryName[0] == ContensXML[i].StorageType || ContensXML[i].StorageType == 0 )
                {
                  ...
                  if (forcecontentsxmlpaths)
                  {
                    ...
                    if ( CopyString (XMLFileTable[num_xml_files_to_send], ContensXML[i].Filename, strlen (ContensXML[i].Path), 0, strlen (ContensXML[i].Filename), MAX_PATH_SIZE, MAX_PATH_SIZE) == 0)
                    {
                      dbg (LOG_ERROR, "Failed to copy string '%s' of length %i into XMLFileTable[%i]", ContensXML[i].Filename, strlen (ContensXML[i].Filename), num_xml_files_to_send);
                      return 1;
                    }
                  } // end if (forcecontentsxmlpaths)
                  else
                  {
                    if ( CopyString (XMLFileTable[num_xml_files_to_send], ContensXML[i].Filename, 0, 0, strlen (ContensXML[i].Filename), MAX_PATH_SIZE, MAX_PATH_SIZE) == 0)
                    {
                      dbg (LOG_ERROR, "Failed to copy string '%s' of length %i into XMLFileTable[%i]", ContensXML[i].Filename, strlen (ContensXML[i].Filename), num_xml_files_to_send);
                      return 1;
                    }
    ...
    for (i = 1; i < (SIZE_T) argc; i++) // skip 0 since argv[0] is fh_loader.exe
    {
        ...
        else if ( strncmp (MyOpt, "setactivepartition=", MAX (strlen (MyOpt), strlen ("setactivepartition=") ) ) == 0 )
        {
            ...
            snprintf (temp_buffer, sizeof(temp_buffer), "value=\"%i\" ", atoi (MyArg) );
            AppendToBuffer (tx_buffer, temp_buffer, FIREHOSE_TX_BUFFER_SIZE);
        ...
        else if ( strncmp (MyOpt, "getstorageinfo=", MAX (strlen (MyOpt), strlen ("getstorageinfo=") ) ) == 0 )
        {
            ...
            snprintf (temp_buffer, sizeof(temp_buffer), "physical_partition_number=\"%i\" ", atoi (MyArg) );
            AppendToBuffer (tx_buffer, temp_buffer, FIREHOSE_TX_BUFFER_SIZE);
            AppendToBuffer (tx_buffer, "/>\n</data>\n", FIREHOSE_TX_BUFFER_SIZE); // HACK for Zeno, remove the \n
    ...
    for (i = 1; i < (SIZE_T) argc; i++) // skip 0 since argv[0] is fh_loader.exe
    {
        ...
        if ( strncmp (MyOpt, "getgptmainbackup=", MAX (strlen (MyOpt), strlen ("getgptmainbackup=") ) ) == 0 )
        {
            ...
            snprintf (temp_buffer, sizeof(temp_buffer), "SECTOR_SIZE_IN_BYTES=\"%"SIZE_T_FORMAT"\" ", fh.attrs.SECTOR_SIZE_IN_BYTES);
            AppendToBuffer (tx_buffer, temp_buffer, FIREHOSE_TX_BUFFER_SIZE);

            if (fh.attrs.SECTOR_SIZE_IN_BYTES == 4096)
              snprintf (temp_buffer, sizeof(temp_buffer), "num_partition_sectors=\"6\" ");
            else
              snprintf (temp_buffer, sizeof(temp_buffer), "num_partition_sectors=\"34\" "); // 512 byte sector size

            AppendToBuffer (tx_buffer, temp_buffer, FIREHOSE_TX_BUFFER_SIZE);

            snprintf (temp_buffer, sizeof(temp_buffer), "filename=\"gpt_main%d.bin\" ", atoi (MyArg) );
            AppendToBuffer (tx_buffer, temp_buffer, FIREHOSE_TX_BUFFER_SIZE);

            snprintf (temp_buffer, sizeof(temp_buffer), "physical_partition_number=\"%i\" ", atoi (MyArg) );
            AppendToBuffer (tx_buffer, temp_buffer, FIREHOSE_TX_BUFFER_SIZE);

            snprintf (temp_buffer, sizeof(temp_buffer), "start_sector=\"0\" ");
            AppendToBuffer (tx_buffer, temp_buffer, FIREHOSE_TX_BUFFER_SIZE);
            ...
            if (fh.attrs.SECTOR_SIZE_IN_BYTES == 4096)
              snprintf (temp_buffer, sizeof(temp_buffer), "num_partition_sectors=\"5\" ");
            else
              snprintf (temp_buffer, sizeof(temp_buffer), "num_partition_sectors=\"33\" "); // 512 byte sector size

            AppendToBuffer (tx_buffer, temp_buffer, FIREHOSE_TX_BUFFER_SIZE);

            snprintf (temp_buffer, sizeof(temp_buffer), "filename=\"gpt_backup%d.bin\" ", atoi (MyArg) );
            AppendToBuffer (tx_buffer, temp_buffer, FIREHOSE_TX_BUFFER_SIZE);
            ...
            if (fh.attrs.SECTOR_SIZE_IN_BYTES == 4096)
              snprintf (temp_buffer, sizeof(temp_buffer), "start_sector=\"NUM_DISK_SECTORS-5.\" ");
            else
              snprintf (temp_buffer, sizeof(temp_buffer), "start_sector=\"NUM_DISK_SECTORS-33.\" ");
        ...
        else if (strncmp (MyOpt, "benchmarkddr", MAX (strlen (MyOpt), strlen ("benchmarkddr") ) ) == 0)
        {
            ...
            snprintf (temp_buffer, sizeof(temp_buffer), "trials=\"%lld\" ", MyTrials);
            AppendToBuffer (tx_buffer, temp_buffer, FIREHOSE_TX_BUFFER_SIZE);
        ...
        else if (strncmp (MyOpt, "sendimage=", MAX (strlen (MyOpt), strlen ("sendimage=") ) ) == 0)
        {
            ...
            if (wipefirst)
            {
                ...
                snprintf (temp_buffer, sizeof(temp_buffer), "num_partition_sectors=\"%"SIZE_T_FORMAT"\" ", ( (16 * 1024) / SectorSizeInBytes) );
                AppendToBuffer (tx_buffer, temp_buffer, FIREHOSE_TX_BUFFER_SIZE);

                snprintf (temp_buffer, sizeof(temp_buffer), "filename=\"%s\" ", WipeFirstFileName);
                AppendToBuffer (tx_buffer, temp_buffer, FIREHOSE_TX_BUFFER_SIZE);

                snprintf (temp_buffer, sizeof(temp_buffer), "physical_partition_number=\"%"SIZE_T_FORMAT"\" ", fh.attrs.physical_partition_number);
                AppendToBuffer (tx_buffer, temp_buffer, FIREHOSE_TX_BUFFER_SIZE);

                snprintf (temp_buffer, sizeof(temp_buffer), "start_sector=\"%s\" ", fh.attrs.start_sector);
                AppendToBuffer (tx_buffer, temp_buffer, FIREHOSE_TX_BUFFER_SIZE);
            ...
            if (erasefirst)
            {
                ...
                snprintf (temp_buffer, sizeof(temp_buffer), "num_partition_sectors=\"%"SIZE_T_FORMAT"\" ", FileSizeNumSectors );
                AppendToBuffer (tx_buffer, temp_buffer, FIREHOSE_TX_BUFFER_SIZE);
            ...
            else
            {
                ...
                snprintf (temp_buffer, sizeof(temp_buffer), "filename=\"%s\" ", MyArg);
                AppendToBuffer (tx_buffer, temp_buffer, FIREHOSE_TX_BUFFER_SIZE);

                //snprintf(temp_buffer, sizeof(temp_buffer),"physical_partition_number=\"%"SIZE_T_FORMAT"\" ",fh.attrs.physical_partition_number);
                snprintf (temp_buffer, sizeof(temp_buffer), "physical_partition_number=\"%"SIZE_T_FORMAT"\" ", MyLun);
                AppendToBuffer (tx_buffer, temp_buffer, FIREHOSE_TX_BUFFER_SIZE);

                //snprintf(temp_buffer, sizeof(temp_buffer),"start_sector=\"%s\" ",fh.attrs.start_sector);
                if (MyStartSector >= 0)
                  snprintf (temp_buffer, sizeof(temp_buffer), "start_sector=\"%"SIZE_T_FORMAT"\" Version=\"1\" ", MyStartSector);
                else
                  snprintf (temp_buffer, sizeof(temp_buffer), "start_sector=\"NUM_DISK_SECTORS%"SIZE_T_FORMAT"\" ", MyStartSector);
                AppendToBuffer (tx_buffer, temp_buffer, FIREHOSE_TX_BUFFER_SIZE);
        ...
        else if (strncmp (MyOpt, "stresstest", MAX (strlen (MyOpt), strlen ("stresstest") ) ) == 0)
        {
            ...
            snprintf (temp_buffer, sizeof(temp_buffer), "filename=\"%s\" ", StressTestFileName);
            AppendToBuffer (tx_buffer, temp_buffer, FIREHOSE_TX_BUFFER_SIZE);
            ...
            if (MyStartSector >= 0)
              snprintf (temp_buffer, sizeof(temp_buffer), "start_sector=\"%"SIZE_T_FORMAT"\" ", MyStartSector);
            else
              snprintf (temp_buffer, sizeof(temp_buffer), "start_sector=\"NUM_DISK_SECTORS%"SIZE_T_FORMAT"\" ", MyStartSector);
...
#ifdef _MSC_VER // i.e. if compiling under Windows
    timeouts.ReadIntervalTimeout        = 20; // maximum number of milliseconds that can elapse between two characters without a timeout occurring
...
void PrettyPrintHexValueIntoTempBuffer (uint8 *temp_hash_val, int length, int offset, int MaxLength)
{
    ...
    for (i = 0; i < length; i++)
    {
      snprintf (&temp_buffer[offset + i * 2], sizeof(temp_buffer) - (offset + i * 2), "%02X", temp_hash_val[i]);
    }
...
void printBuffer (unsigned char *buffer, unsigned int length, unsigned int MaxLength, const char *func_name, int line_number)
{
    ...
    while (1)
    {
        ...
        for (j = 0; j < 16; j++)
        {
            ...
            snprintf (&szBuffer[j * 3], sizeof(szBuffer) - (j * 3), "%02X ", (CurrentChar & 0xFF) );
        } // end j
        ...
        while (16 - j > 0)
        {
            ...
            snprintf (&szBuffer[k], sizeof(szBuffer) - k, "   ");
        ...
        for (j = 0; j < 16; j++)
        {
            ...
            if (CurrentChar >= 0x20 && CurrentChar <= 0x7E)
            {
                if (CurrentChar == '%')
                {
                  snprintf (&szBuffer[k + j], sizeof(szBuffer) - (k + j), "%%%%");
                    k++;
                }
                else
                  snprintf (&szBuffer[k + j], sizeof(szBuffer) - (k + j), "%c", CurrentChar); // this puts a null
...
SIZE_T  DetermineTag (char *Packet, SIZE_T  CurrentPacketLoc, SIZE_T MaxPacketSize)
{
    ...
    if (PacketLength > MaxPacketSize)
    {
        dbg (LOG_ERROR, "PacketLength>MaxPacketSize (%d > %d)", PacketLength, MaxPacketSize);
    ...
    do
    {
        ...
        if (Simulate)
        {
            ...
            if (fh.attrs.MaxPayloadSizeToTargetInBytes > fh.attrs.MaxPayloadSizeToTargetInBytesSupported)
            {
                ...
                snprintf (temp_buffer, sizeof(temp_buffer), "MaxPayloadSizeToTargetInBytes=\"%"SIZE_T_FORMAT"\" ", fh.attrs.MaxPayloadSizeToTargetInBytesSupported);
                AppendToBuffer (ReadBuffer, temp_buffer, FIREHOSE_TX_BUFFER_SIZE);
                snprintf (temp_buffer, sizeof(temp_buffer), "MaxPayloadSizeToTargetInBytesSupported=\"%"SIZE_T_FORMAT"\" ", fh.attrs.MaxPayloadSizeToTargetInBytesSupported);
                AppendToBuffer (ReadBuffer, temp_buffer, FIREHOSE_TX_BUFFER_SIZE);
            }
            else
            {
                AppendToBuffer (ReadBuffer, "<response value=\"ACK\" ", MAX_READ_BUFFER_SIZE);
                snprintf (temp_buffer, sizeof(temp_buffer), "MaxPayloadSizeToTargetInBytes=\"%"SIZE_T_FORMAT"\" ", fh.attrs.MaxPayloadSizeToTargetInBytes);
                AppendToBuffer (ReadBuffer, temp_buffer, FIREHOSE_TX_BUFFER_SIZE);
                snprintf (temp_buffer, sizeof(temp_buffer), "MaxPayloadSizeToTargetInBytesSupported=\"%"SIZE_T_FORMAT"\" ", fh.attrs.MaxPayloadSizeToTargetInBytesSupported);
                AppendToBuffer (ReadBuffer, temp_buffer, FIREHOSE_TX_BUFFER_SIZE);
            }
            ...
            CharsInBuffer = strlen (ReadBuffer);
            //snprintf(temp_buffer, sizeof(temp_buffer),"value=\"%s\" ",fh.attrs.value);
...
firehose_error_t GetNextPacket (void)
{
    ...
    while (TimeToBreak == 0)
    {
        ...
        if (ParseAttributes)
        {
            ...
            PacketLoc = GetStringFromXML ( (char *) &ReadBuffer[PacketLoc], PacketLoc, MAX_STRING_SIZE);
...
void LoadConfigureIntoTXBuffer (void)
{
    ...
    snprintf (temp_buffer, sizeof(temp_buffer), "MemoryName=\"%s\" ", fh.attrs.MemoryName);
    AppendToBuffer (tx_buffer, temp_buffer, FIREHOSE_TX_BUFFER_SIZE);
    ...
    snprintf (temp_buffer, sizeof(temp_buffer), "MaxDigestTableSizeInBytes=\"%"SIZE_T_FORMAT"\" ", fh.attrs.MaxDigestTableSizeInBytes);
    AppendToBuffer (tx_buffer, temp_buffer, FIREHOSE_TX_BUFFER_SIZE);
    ...
    {
      snprintf (temp_buffer, sizeof(temp_buffer), "SkipWrite=\"%"SIZE_T_FORMAT"\" ", fh.attrs.SkipWrite);
        AppendToBuffer (tx_buffer, temp_buffer, FIREHOSE_TX_BUFFER_SIZE);
    } // HACK, I removed else to match zeno

//  if(fh.attrs.SkipStorageInit)
//  {
    snprintf (temp_buffer, sizeof(temp_buffer), "SkipStorageInit=\"%"SIZE_T_FORMAT"\" ", fh.attrs.SkipStorageInit);
    AppendToBuffer (tx_buffer, temp_buffer, FIREHOSE_TX_BUFFER_SIZE);
//  }
...
void LoadConfigureIntoStringTable (void)
{
    ...
    CopyString (XMLStringTable[0], tx_buffer, 0, 0, strlen (tx_buffer), MAX_XML_SIZE, sizeof(tx_buffer));
...
void StoreXMLFile (char *FileWithPath)
{
    ...
    xml_buffer = calloc(xml_buffer_size, 1);
    if (xml_buffer == NULL)
    {
      dbg (LOG_ERROR, "Falled to allocate a buffer of %zu bytes to store read the file %s, reduce the file size or free some memory and try again.", xml_buffer_size, FileWithPath);
      ExitAndShowLog (1);
    }

    fread (xml_buffer, XMLFileSize, 1, fd);
    fclose (fd);
    fd = NULL;
    ...
    */
    ...
    if (pch != NULL)
    {
        ...
        if (pch != NULL)
        {
            parseConfigXML (XMLFileSize, xml_buffer, xml_buffer_size); // works on xml_buffer
            CleanseSearchPaths();
            goto cleanup;
        }
    }
    ...
    while (1)
    {
        ...
        ShowXMLFileInLog = 0;
        PacketLoc = DetermineTag ( (char *) xml_buffer, PacketLoc, xml_buffer_size); // This sets CurrentHandlerFunction()
        ...
        // This chomps up empty space till we hit a tag
        while ( IsEmptySpace ( xml_buffer[PacketLoc] ) )
        {
            PacketLoc++;
        ...
        //XMLFileSize = strlen((char *)xml_buffer); // since DetermineTag() can replace final '\n' with '\0'
        ...
        {
            // To be this far fh.xml_buffer[PacketLoc] is pointing to the first char of the first attribute
            PacketLoc = DetermineAttributes ( (char *) xml_buffer, PacketLoc, xml_buffer_size);
        ...
        if ( strncmp (NewTagName, "program", MAX (strlen (NewTagName), strlen ("program") ) ) == 0 )
        {
            ...
            if (erasefirst)
            {
                ...
                snprintf (temp_buffer, sizeof(temp_buffer), "num_partition_sectors=\"%"SIZE_T_FORMAT"\" ", fh.attrs.num_partition_sectors );
                AppendToBuffer (tx_buffer, temp_buffer, FIREHOSE_TX_BUFFER_SIZE);
        ...
        memcpy (single_xml_buffer, xml_buffer + PacketLoc, pch - &xml_buffer[PacketLoc]);
        ...
        else
        {
            ...
            snprintf (temp_buffer, sizeof(temp_buffer), "%s ", NewTagName);
            AppendToBuffer (tx_buffer, temp_buffer, FIREHOSE_TX_BUFFER_SIZE);

            CopyString (tx_buffer, xml_buffer, strlen (tx_buffer), PacketLoc, pch - &xml_buffer[PacketLoc], FIREHOSE_TX_BUFFER_SIZE, xml_buffer_size);
            AppendToBuffer (tx_buffer, " />\n</data>", FIREHOSE_TX_BUFFER_SIZE);
        ...
        PacketLoc += pch - &xml_buffer[PacketLoc] + 2; // +2 gets past the />
        ...
        if (xml_buffer[PacketLoc] == 0)
            break;  // we're out of packet, all is well

        //if(strlen( &xml_buffer[PacketLoc] ) == 0)
        //  break;
...
FILE * ReturnFileHandle (char *filename, long MaxLength, char *AccessMode)
{
    ...
    if ((strlen (MainOutputDir) > 0) && (w_pch != NULL) && (dir_pch == NULL) &&(dir2_pch == NULL)) // user specified this at command line
    {
        ...
        CopyString (FilePlusOutputDir, "/", strlen(FilePlusOutputDir), 0, 1, MAX_PATH_SIZE, 1);
...
static firehose_error_t handleProgram()
{
    ...
    snprintf (&temp_buffer[TempBufferSize], sizeof(temp_buffer) - TempBufferSize, "num_partition_sectors=\"%"SIZE_T_FORMAT"\"", FileSizeNumSectors); // no space at end here!!
    ...
    else if (verify_programming)
    {
        ...
        //snprintf(temp_buffer, sizeof(temp_buffer),"filename=\"%s\" ",FileToReadBackWithPath);
        // There can be no path on this name
        snprintf (temp_buffer, sizeof(temp_buffer), "filename=\"%s\" ", fh.attrs.filename);
...
// -----------------------------------------------------------------------------------------------------------------------------------------
    ...
    while (RawMode > 0)
    {
        ...
        if (MaxBytesToReadFromUSB < fh.attrs.SECTOR_SIZE_IN_BYTES)
            MaxBytesToReadFromUSB = fh.attrs.SECTOR_SIZE_IN_BYTES;
...
void time_throughput_calculate (struct timeval *start_time, struct timeval *end_time, size_t size_bytes, double NetworkElapsed, enum LOG_TYPES ThisLogType)
{
    ...
    if (NetworkSlow < 100000000.0)
    {
        ...
        snprintf (SizeString3, sizeof(SizeString3), "FILE ACCESS SLOW!! %10s in %6.3f seconds (%10sps) --- Overall to target", SizeString2, NetworkElapsed, SizeString1);
    }
    else
      snprintf (SizeString3, sizeof(SizeString3), "Overall to target");
...
void parseConfigXML (SIZE_T XMLFileSize, char *xml_buffer, size_t xml_buffer_size)
{
    ...
    while (1)
    {
        ...
        if (ParseAttributes)
        {
            ...
            PacketLoc = GetStringFromXML ( (char *) xml_buffer, PacketLoc, xml_buffer_size);
    ...
    // xml_buffer holds the entire XML file
...
void FindPartitionByLabel (SIZE_T LUN, char *LabelToMatch, char *Filename)
{
    ...
    for (i = 0; i < 128; i++)
    {
        ...
        {
            CurrentUTF16Char = tx_buffer[offset + k]; // don't forget, moving by 2's since UTF16, converting to UTF8
            snprintf (SmallString, sizeof(SmallString), "%.2X", CurrentUTF16Char & 0xFF);
...
void ReturnSizeString (unsigned long long size_bytes, char *sz, unsigned long long Length)
{
    ...
    if (size_bytes >= (1024 * 1024 * 1024) )
      snprintf (sz, Length, "%.2f GB", (size_bytes * 1.0) / (1024.0 * 1024.0 * 1024.0) );
    else if (size_bytes >= (1024 * 1024) )
      snprintf (sz, Length, "%.2f MB", (size_bytes * 1.0) / (1024.0 * 1024.0) );
    else if (size_bytes >= (1024) )
      snprintf (sz, Length, "%.2f KB", (size_bytes * 1.0) / (1024.0) );
    else
      snprintf (sz, Length, "%.2f B", (size_bytes * 1.0) );
...
void ParseContentsXML (char *FileAndPath)
{
    ...
    if (xml_buffer == NULL)
    {
      dbg (LOG_ERROR, "Falled to allocate a buffer of %zu bytes to store contents.xml you provided (%s), free some memory and try again.", xml_buffer_size, FileAndPath);
      ExitAndShowLog (1);
    }

    BytesRead = fread (xml_buffer, 1, FileSize, fx);
    fclose (fx);
    fx = NULL;
    ...
    while (1)
    {
        ...
        pch = strstr (&xml_buffer[CurrentPacketLoc], "<windows_root_path");         // Find a space after the TAG name
        ...
        if ( CopyString (ContensXMLPath[NumContentsXMLPath].Path, xml_buffer, 0, CurrentPacketLoc, pch - &xml_buffer[CurrentPacketLoc], sizeof (ContensXMLPath[NumContentsXMLPath].Path), xml_buffer_size) == 0)
        {
            dbg (LOG_ERROR, "Failed to copy '%s' of length %"SIZE_T_FORMAT" bytes into fh.attrs.start_sector", "0", strlen ("0") );
            free(xml_buffer);
            ExitAndShowLog (1);
        ...
        if (ContensXMLPath[NumContentsXMLPath].Path[0] == '.')
        {
            ...
            {
                if ( CopyString (ContensXMLPath[NumContentsXMLPath].Path, FileAndPath, 0, 0, i + 1, sizeof (ContensXMLPath[NumContentsXMLPath].Path), xml_buffer_size) == 0)
                {
                    dbg (LOG_ERROR, "Failed to copy '%s' of length %"SIZE_T_FORMAT" bytes into fh.attrs.start_sector", "0", strlen ("0") );
                    free(xml_buffer);
                    ExitAndShowLog (1);
    ...
    while (1)
    {
        ...
        pch = strstr (&xml_buffer[CurrentPacketLoc], "<file_name"); // this gets us to either <filename> OR <file_name flavor="asic_8916">
        if (pch == NULL)
        {
            // We're done, no more files
            break;
        ...
        pch_ref = strstr (&xml_buffer[CurrentPacketLoc], "<file_ref"); // this gets us to either <file_ref
        ...
        else
        {
            ...
            pch = strstr (&xml_buffer[CurrentPacketLoc], "flavor=\""); // this gets us to either <filename> OR <file_name flavor="asic_8916">

            if (pch == NULL)
            {
                dbg (LOG_ERROR, "Looks like fh_loader cannot parse this contents.xml file. Confused by what looked like <file_name flavor=, shown below\n\n%s", &xml_buffer[CurrentPacketLoc]);
                free(xml_buffer);
                ExitAndShowLog (1);
            }

            pch += strlen ("flavor=\"");
            CurrentPacketLoc = pch - xml_buffer; // &xml_buffer[CurrentPacketLoc] = rawprogram_unsparse.xml</file_name>
            ...
            if ( CopyString (ContensXML[NumContensXML].Flavor, xml_buffer, 0, CurrentPacketLoc, i - CurrentPacketLoc, sizeof (ContensXML[NumContensXML].Flavor), xml_buffer_size) == 0)
            {
        ...
        // At this point &xml_buffer[CurrentPacketLoc] is pointing at <file_name>rawprogram_unsparse.xml</file_name>
        ...
        pch = strstr (&xml_buffer[i], "storage_type=");         // Find a space after the TAG name
        ...
        if (pch != NULL) //'\0')
        {
            ...
            if (pch < &xml_buffer[CurrentPacketLoc])
            {
                ...
                else if ( pch[0] == 's' && pch[1] == 'p' && pch[2] == 'i' && pch[3] == 'n' && pch[4] == 'o' && pch[5] == 'r')
                    ContensXML[NumContensXML].StorageType = 's';  // 'e'=emmc,'u'=ufs,'n'=nand, 's'=spinor
        ...
        pch = strstr (&xml_buffer[CurrentPacketLoc], "</file_name>");         // Find a space after the TAG name
        ...
        if ( CopyString (ContensXML[NumContensXML].Filename, xml_buffer, 0, CurrentPacketLoc, pch - &xml_buffer[CurrentPacketLoc], sizeof (ContensXML[NumContensXML].Filename), xml_buffer_size) == 0)
        {
            dbg (LOG_ERROR, "Failed to copy '%s' of length %"SIZE_T_FORMAT" bytes into fh.attrs.start_sector", "0", strlen ("0") );
            free(xml_buffer);
            ExitAndShowLog (1);
        ...
        if (pch[0] == ' ')
        {
            ...
            pchMAX = strstr(&xml_buffer[CurrentPacketLoc], ">"); // End of <file_path.....>

            while (1)
            {
                pchOld = pch; // backup since the next search can blow way past the actual file

                pch = strstr (&xml_buffer[CurrentPacketLoc], "flavor="); // <file_path flavor="asic"> --> flavor="asic">
            ...
            pch = strstr (&xml_buffer[CurrentPacketLoc], ">"); // <file_path flavor="asic"> -->   >
        ...
        // temp buffer
        if ( CopyString (temp_buffer, xml_buffer, 0, CurrentPacketLoc, pch - &xml_buffer[CurrentPacketLoc], sizeof (temp_buffer), xml_buffer_size ) == 0)
        {
            dbg (LOG_ERROR, "Failed to copy '%s' of length %"SIZE_T_FORMAT" bytes into fh.attrs.start_sector", "0", strlen ("0") );
            free(xml_buffer);
            ExitAndShowLog (1);
...
void SortMyXmlFiles (void)
{
    ...
    SIZE_T k = 0, j = 0;
    char *pch, *pch2;
    static char XMLStringTableTemp[MAX_XML_FILES][MAX_XML_SIZE];
    ...
    while (XMLStringTableTemp[k][0] != '\0')
    {
        ...
        if (pch != NULL)
        {
            CopyString (XMLStringTable[j], XMLStringTableTemp[k], 0, 0, strlen (XMLStringTableTemp[k]), MAX_XML_SIZE, MAX_XML_SIZE);
...
void ModifyTags (void)
{
    ...
    while (XMLStringTable[k][0] != '\0')
    {
        ...
        memscpy (XMLStringTable[k], MAX_XML_SIZE, temp_buffer, strlen (temp_buffer) ); // memcpy
...
static firehose_error_t handleBenchmark()
{
    ...
    if (pch != NULL)
    {
        ...
        for (k = 0; k < 2 * fh.attrs.trials; k++) // 2 times since 2 channels
        {
            ...
            sechsharm_sha256_final (&context, context.leftover, & (context.leftover_size), last_hash_value);
            PrettyPrintHexValueIntoTempBuffer (last_hash_value, 32, 0, 32); // from, size, offset, maxlength
            dbg (LOG_INFO, "%"SIZE_T_FORMAT" of %"SIZE_T_FORMAT" verify_programming SHA256 (%7"SIZE_T_FORMAT" bytes) %s\n", k + 1, 2 * fh.attrs.trials, SizeOfDataFedToHashRoutine, temp_buffer);
            memscpy (verify_hash_value, sizeof (verify_hash_value), last_hash_value, sizeof (last_hash_value) ); // memcpy
