...
class AsnObject(object):
    ...
    def unpack(self, data):
        ...
        if short_length & self.MASK_LONG_LEN:
            ...
            long_length = int(''.join(data[self.OFFSET_LONG_LEN:self.OFFSET_LONG_LEN + long_length_bytes]), 16)
            real_length = long_length
            offset_value += long_length_bytes

        # Get the value
        value = data[offset_value:offset_value + real_length]
    ...
    def _dict_repr_(self):
        retval = {'type': hex(self.type),
                  'short_length': hex(self.short_length),
                  'long_length': hex(self.long_length) if self.long_length is not None else None,
                  'value': [v._dict_repr_() if isinstance(v, AsnObject) else str(v) for v in self.value]}
        return retval
