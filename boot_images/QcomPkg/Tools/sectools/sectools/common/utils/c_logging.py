...
lock_handlers = threading.RLock()
lock_file_log = threading.RLock()
...
class CoreLogger(object):
    ...
    def __init__(self):
        ...
        self._fileLoggerHandlersCntr = 0
        self._fileLogger.setLevel(self.DEBUG3)
        self._consoleLogger.setLevel(self.DEBUG3)

        # Specify if logging to GUI
        self.logToGui = False
        self.allowRaw = True
        self.allowFileLogging = True
    ...
    def verbosity(self, level):
        ...
        if level in [self.CRITICAL, self.ERROR, self.WARN, self.NOTE, self.INFO,
                     self.DEBUG, self.DEBUG2, self.DEBUG3]:
    ...
    def add_file_logger(self, filePath, verbosity, mode='w'):
        ...
        fl = _FileLogger(filePath, verbosity, mode, self._fileLogger)
        flid = self._fileLoggerHandlersCntr
        self._fileLoggerHandlers[flid] = fl
        self._fileLoggerHandlersCntr += 1
        return flid

    def removeFileLogger(self, flids):
        """ Remove the given file logger """
        if not isinstance(flids, list):
            flids = [flids]
        for fileLoggerID in flids:
            fl = self._fileLoggerHandlers.pop(fileLoggerID, None)
            if fl is None:
    ...
    def getFileLoggerPath(self, fileLoggerID):
        ...
        fl = self._fileLoggerHandlers.get(fileLoggerID, None)
        if fl is None:
            raise ValueError('Invalid logger ID: "{0}".'.format(fileLoggerID))
        return fl.filePath

    def _msg(self, msg, level, raw=False, color=None):
        """
        ...
        if self.logToGui:
            ...
            if color is not None:
                console_msg = color + msg + colorama.Fore.RESET

            if level >= self.verbosity:
                if raw and self.allowRaw:
                    self.outputStream.write(console_msg)
                    self.outputStream.flush()
                else:
                    self._consoleLogger.log(level, console_msg)

        if self._fileLoggerHandlers and self.allowFileLogging:
            with lock_file_log:
                if raw:
                    for fileLogger in self._fileLoggerHandlers.values():
    ...
    def info(self, msg, raw=False, color=None):
        """
        Log an info message. If raw is True, the message is logged as is
        without any extra formatting.
        """
        self._msg(msg, self.INFO, raw, color)
    ...
    def warning(self, msg, raw=False):
        ...
        without any extra formatting.
        """
        self._msg(('' if raw else 'WARNING: ') + str(msg), self.WARN, raw, self.YELLOW)
    ...
    def askListOptions(self, askMessage, optionsList):
        ...
        if self.defaultPromptList is not None:
            ...
            if ((returnValue is not None) and (isinstance(returnValue, (int, long))) and
...
"""
    ***** WARNING: {0} *****
    ...
    def log_to_file(self, toolname, log_dir):
        try:
            return self.enable_file_logging(toolname, 1, log_dir)
        except Exception as e:
            raise RuntimeError('Unable to create directory for logging: ' + log_dir + '\n'
                               '    ' + 'Error: ' + str(e))
        ...
        if log_dir:
            ...
            if not retval:
                ...
                if self.verbosity < self.INFO:
                    flids.append(self.add_file_logger(log_file_debug, self.verbosity, 'w'))
