...
class IntegerHandler(BaseHandler):
    ...
    @property
    def _detailer(self):
        """Returns the :class:`IntegerDetailer` object for the data type."""
...
class IntegerNormalizer(BaseNormalizer):
    ...
    def normalize(self, i_value):
        ...
        try: assert isinstance(o_value, (int, long))
...
class IntegerValidator(BaseValidator):
    ...
    def validate(self, i_value, i_format):
        ...
        assert isinstance(i_format, (int, long))
...
class IntegerTransferMgr(BaseTransferMgr):
    ...
    def transfer(self, i_value_from, i_value_to, i_structure, i_structure_parent):
        ...
        assert isinstance(i_structure, (int, long))
        o_value = i_value_from
        if o_value is not None:
            assert isinstance(o_value, (int, long))
        return o_value
...
class IntegerCodeGenerator(BaseCodeGenerator):
    ...
    def generate_code(self, attr_key, attr_value):
        ...
        assert isinstance(attr_value, (int, long))
        return attr_key + ' = ' + str(attr_value)


class IntegerDetailer(BaseDetailer):
    """Detailer for Integer data.
    ...
    def detail(self, i_format):
        ...
        assert isinstance(i_format, (int, long))
        return DataDetail(DataType.Int, self.data_handler, i_format)
