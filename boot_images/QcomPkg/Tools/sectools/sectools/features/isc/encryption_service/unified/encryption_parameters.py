...
class EncryptionParameters(BaseEncryptionParameters):
    ...
    def __init__(self, encryption_params_blob=None, private_key=None, config=None, debug_dir=None, l3_key_blob=None, validating=False):
        self.image_id = int(config.signing_attributes.sw_id[EncryptionParameters.SOFTWARE_TYPE_FLD_OFFSET:], 16)
    ...
    def _generate_new_blob(self, config, debug_dir):
        ...
        eps_hdr = EncryptionParamsSectionHdr(None, None, 1 if config.signing_attributes.UIE_root_key_type is None else config.signing_attributes.UIE_root_key_type, 0)
        eps_body = EncryptionParamsSectionBody(self.major_version,
                                               self.minor_version,
                                               self.image_id,
                                               l1_key=self.l1_key,
                                               l2_key=self.l2_key,
    ...
    def major_version(self):
        return EncryptionParameters.MAJOR_VERSION

    @property
    def minor_version(self):
        return EncryptionParameters.MINOR_VERSION


class EncryptionParameters_1_1(EncryptionParameters):

    MINOR_VERSION = 1

    @property
    def minor_version(self):
        return EncryptionParameters_1_1.MINOR_VERSION
