...
class EncryptionParameters(BaseEncryptionParameters):
    ...
    def __init__(self, encryption_params_blob=None, private_key=None, config=None, debug_dir=None, validating=False):
    ...
    def _decode_encryption_parameters_blob(self, encryption_params_blob, private_key):
        ...
        tmp_config_file_path = c_path.store_data_to_temp_file(encryption_params_blob)
        ...
        if self.iek_enc_algo==IEK_ENC_ALGO_RSA_2048:
            private_key_pem=crypto.rsa.get_key_in_format(private_key, crypto.utils.FORMAT_PEM)
            image_encryption_key=crypto.rsa.decrypt(encrypted_image_encryption_key, private_key_pem,
            ...
            image_encryption_key=crypto.aes_cbc.decrypt(encrypted_image_encryption_key, binascii.hexlify(private_key), image_encryption_iv_hex)
