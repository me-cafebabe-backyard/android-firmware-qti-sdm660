...
from sectools.features.isc.cfgparser.defines import JTAGID_SIGN_IDS, SECBOOT_2_0_DOUBLE_SIGN_CHIPSETS, MRC_2_0_CHIPSETS, SECBOOT_1_IMAGE_TYPES
...
class ImageInfo(object):
    ...
    def _sanitize_general_properties(self):
        ...
        if self.signing_attributes.secboot_version is None or (self.image_type.id in SECBOOT_1_IMAGE_TYPES and self.signing_attributes.secboot_version != defines.SECBOOT_VERSION_1_0):
            self.signing_attributes.secboot_version = defines.SECBOOT_VERSION_1_0
    ...
    def _validate_properties(self):
        ...
        if not self.signing_attributes.qti_sign and not self.signing_attributes.oem_sign:
            raise RuntimeError('Cannot perform image operations. No permissions enabled.')
        ...
        if self.signing_attributes.secboot_version == defines.SECBOOT_VERSION_1_0:
            ...
            if self.signing_attributes.qti_sign:
                ...
                raise RuntimeError('Cannot perform qti_exclusive operations on a secboot version ' + str(defines.SECBOOT_VERSION_1_0) + ' image.')

        # Raise warnings for Secboot v2.1 chipsets
        if self.chipset in SECBOOT_2_0_DOUBLE_SIGN_CHIPSETS:
