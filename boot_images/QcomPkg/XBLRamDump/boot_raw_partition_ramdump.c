...
--------   ---         --------------------------------------------------
05/005/17  bd          Logs only in debug mode
03/07/17   bd          Add support for multiple dumps
06/28/16   na          CR#989683: fix rawdump base address to 64-bit
09/10/15   kedara      remove redundant mmu_flush_cache
...
{
  DBG_MSG("boot_raw_ram_dump_error_handler \n");
...
{  
  if (dev_sdcc_read_bytes(buf_addr, offset, sizeof(struct boot_raw_parition_dump_header),
      get_hotplug_dev_handle(GEN_IMG)) != TRUE)
  {
    boot_raw_ram_dump_error_handler();
  }
}
...
{ 
  boot_ram_dump_hotplug_write(&first_dump_header, 
                              0, 
                              DUMP_HEADER_SIZE);
}
...
void boot_update_section_header_table_by_index(uint32 i)
{
   ...
   char dbg_info[100];
    ...
    snprintf(dbg_info, 100, "section %d offset %lx ", i, curr_partition_offset);
    DBG_MSG(dbg_info);    
...
static boot_boolean boot_process_raw_ram_dump_sections()
{
 ...
 /* Iterate through mem_debug_info table and process each section*/
  for(; i < ram_dump_sections_num ; i++)
  {
    ...
    {
      boot_ram_dump_hotplug_write((void*)(uintnt)curr_sec_base_addr, 
...
static void boot_process_raw_ram_dump()
{
  ...
  DBG_MSG("first_dump_header.total_dump_size_required=");
  snprintf(dbg_info, 40, "0x%lx \n", first_dump_header.total_dump_size_required);  
  DBG_MSG(dbg_info);
  
  if(first_dump_header.total_dump_size_required >= partition_size)
  {
    DBG_MSG("Rollback to offset-0");
  ...
  // TBD : below print required ??
  snprintf(dbg_info, 40, "partition_size=0x%lx \n", partition_size);  
  DBG_MSG(dbg_info);
...
void boot_ram_dump_header_init()
{
  ...
  {
    DBG_MSG("Valid dump at offset-0");
  ...
  else
  {
    ...
    DBG_MSG("No valid header at offset-0");
...
void boot_ram_dump_to_raw_parition()
{
     ...
     dload_mem_debug_supported())
  {
    ...
    if(trans_if != NULL)
    {
      ...
      {
		DBG_MSG("Taking dumps to internal memory");
        /* Process the dump */
        boot_process_raw_ram_dump();
