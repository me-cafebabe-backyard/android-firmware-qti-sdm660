/** @file AppCmds.c
  ...
  Copyright 2012 - 2016 Qualcomm Technologies, Inc.  All Rights Reserved.
...
/*
 * Handler to Toggle Qualcomm Charger App
 */
...
DisableChgApp (CHAR8* Arg)
{
  ...
  VarSize = sizeof(Flag);
  Status = gRT->GetVariable (L"BattChgApp", &gQcomTokenSpaceGuid,
                             NULL, &VarSize, &Flag);

  if(Status != EFI_SUCCESS)
    Flag = 0;

  if (QueryToggleFlag ("Charger power-on trigger", &Flag, INVERTED_MEANING) != EFI_SUCCESS)
    return EFI_SUCCESS;

  VarSize = sizeof(Flag);
  Status = gRT->SetVariable (L"BattChgApp", &gQcomTokenSpaceGuid,
                             EFI_VARIABLE_BOOTSERVICE_ACCESS | EFI_VARIABLE_NON_VOLATILE,
                             VarSize, &Flag);
...
ConfigRsense (CHAR8* Arg)
{
  ...
  VarSize = sizeof(Flag);
  Status = gRT->GetVariable (L"ConfigRsense", &gQcomTokenSpaceGuid,
                             NULL, &VarSize, &Flag);

  if(Status != EFI_SUCCESS)
    Flag = 0;

  if (QueryToggleFlag ("Config External Rsense (Enable: External, Disable: Internal) ", &Flag, NON_INVERTED_MEANING) != EFI_SUCCESS)
    return EFI_SUCCESS;

  VarSize = sizeof(Flag);
  Status = gRT->SetVariable (L"ConfigRsense", &gQcomTokenSpaceGuid,
                             EFI_VARIABLE_BOOTSERVICE_ACCESS | EFI_VARIABLE_NON_VOLATILE,
                             VarSize, &Flag);
...
ToggleOfflineMemoryDumpUseCapability (CHAR8* Arg)
{
  ...
  if (QueryToggleFlag ("Toggle OfflineMemoryDumpUseCapability", &Flag, FALSE) != EFI_SUCCESS)
    return EFI_SUCCESS;
...
RunQcomChgrApp (CHAR8* Arg)
{
  ...
  Status = gRT->GetVariable (L"RunQcomChgrApp", &gQcomTokenSpaceGuid,
             NULL, &VarSize, &Flag);

  if (Status == EFI_NOT_FOUND)
    Flag = 0;

  if (QueryToggleFlag ("Toggle Qcom Charging App", &Flag, NON_INVERTED_MEANING) != EFI_SUCCESS)
  return EFI_SUCCESS;

  VarSize = sizeof(Flag);

  Status = gRT->SetVariable (L"RunQcomChgrApp", &gQcomTokenSpaceGuid,
               EFI_VARIABLE_BOOTSERVICE_ACCESS | EFI_VARIABLE_NON_VOLATILE,
             VarSize, &Flag);

  if(Status == EFI_SUCCESS){
    AsciiPrint("Qcom Charging App "); //.\r\n");
    if(Flag == 0){
      AsciiPrint("disabled. \r\n");
    } else {
      AsciiPrint("enabled. \r\n");
...
EnableFactoryMode(CHAR8* Arg)
{
  ...
  VarSize = sizeof(Flag);
  Status = gRT->GetVariable (L"StickyCharging", &gStickyGuid,
                             NULL, &VarSize, &Flag);

  if(Status != EFI_SUCCESS)
    Flag = 0;

  if (QueryToggleFlag ("Enable/Disable Factory Mode for charging", &Flag, NON_INVERTED_MEANING) != EFI_SUCCESS)
    return EFI_SUCCESS;

  VarSize = sizeof(Flag);
  Status = gRT->SetVariable (L"StickyCharging", &gStickyGuid,
                             EFI_VARIABLE_BOOTSERVICE_ACCESS | EFI_VARIABLE_RUNTIME_ACCESS | EFI_VARIABLE_NON_VOLATILE,
                             VarSize, &Flag);
