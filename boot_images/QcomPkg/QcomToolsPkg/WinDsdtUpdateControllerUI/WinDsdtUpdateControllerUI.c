...
featureaction (CONST CHAR8* FeatureString , UINTN Feature)
{
    EFI_STATUS                              Status;
    EFI_INPUT_KEY                           Key;
    static EFI_WIN_DSDT_UPDATE_PROTOCOL     *gWinDsdt = NULL;
    BOOLEAN                                 configuration = FALSE;  

	// Locate Win DSDT update protocol
    ...
    if (EFI_ERROR (Status)) 
    {
        DEBUG((EFI_D_ERROR, " Failed to Locate WinDsdtUpdate protocol \r\n"));
        return EFI_LOAD_ERROR;
    }

	//  Get the feature configuration
    Status = gWinDsdt->GetFeatureVariable(
                        gWinDsdt,
                        Feature,
                        &configuration
                        );
    ...
    if (Key.ScanCode == SCAN_UP)
    {
        AsciiPrint("\r\nEnabling: \"%a\"\r\n\r\n", FeatureString);
        configuration = TRUE;
        Status = gWinDsdt->SetFeatureVariable(
                                    gWinDsdt,
                                    Feature,
                                    configuration
                                    );
        ...
        Status = gWinDsdt->CommitFeatureVariable(gWinDsdt);
        if (EFI_ERROR (Status)) 
        {
