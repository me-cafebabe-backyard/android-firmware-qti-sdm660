  ...
  )
{
  ...
  else if (0==AsciiStriCmp("disabledisplay", Param))
  {
      ...
      DisplaySetEnvVar((CHAR16*)&EnvVarList[DISPLAY_VAR_DISABLEDISPLAY].sVarName, uValue);
  }
  else if (0==AsciiStriCmp("amoled", Param))
  {
      ...
      // Configure or display the environment variable
      DisplaySetEnvVar((CHAR16*)&EnvVarList[DISPLAY_VAR_DISPLAYFLAGS].sVarName, uFlag);
  }
  else if (0==AsciiStriCmp("panelid", Param))
  {
      ...
      // Configure or display the environment variable
      DisplaySetEnvVar((CHAR16*)&EnvVarList[DISPLAY_VAR_DISPLAYID].sVarName, uValue);
  }
  else
  {
...
static EFI_STATUS DisplaySetEnvVar(CHAR16 *pVarname, UINT32 uValue)
{
  ...
  EFI_QCOM_DISPLAY_ABL_PROTOCOL       *pDisplayABLProtocol  = NULL;

  AsciiSPrint (VarValue, sizeof(VarValue), "%x", uValue);
  eStatus = gBS->LocateProtocol(&gQcomDisplayABLProtocolGuid, NULL, (void**)&pDisplayABLProtocol);
  if ((EFI_SUCCESS !=  eStatus) ||
      (NULL        ==  pDisplayABLProtocol))
  {
    DEBUG((EFI_D_WARN, "Locate display ABL protocol failed with status: %r\n", eStatus));
  }
  else
  {
    // Set variable value
    eStatus = pDisplayABLProtocol->DisplayABLSetVariable(pVarname, (UINT8 *)VarValue, AsciiStrLen (VarValue), 0);
  }
...
static EFI_STATUS DisplayGetEnvVar(CHAR16 *pVarname, UINT32 *pValue)
{
  ...
  {
    // Get variable value
    eStatus = pDisplayABLProtocol->DisplayABLGetVariable(pVarname, (UINT8 *)VarValue, &uStorageSize, 0);
  }
