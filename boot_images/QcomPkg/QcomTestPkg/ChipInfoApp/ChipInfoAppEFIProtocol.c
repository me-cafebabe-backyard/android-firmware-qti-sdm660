...
EFI_STATUS ChipInfoTestUsingEFIProtocol()
{
  ...
  eResult = mChipInfoProtocol->GetProcessorNameString( mChipInfoProtocol, 
                                                (CHAR8*)&szNameString,  
                                                EFICHIPINFO_MAX_NAME_LENGTH);
  if (eResult  == EFI_SUCCESS)
  {
    AsciiPrint ("ChipInfo:  ProcessorNameString :%a \n", szNameString);
  }
  else
  {
    AsciiPrint ("ChipInfo:  GetProcessorNameString failed \n");
  }
