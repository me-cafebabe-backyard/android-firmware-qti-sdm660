  ...
  )
{
  ...
  //
#if !defined (QCOM_EDK2_PATCH)
  // But we are diverging from UEFI spec, in that we are supporting lower resolution
  // modes include 480x640 and 480x800.  So we do not need to assert this requirement
...
#if defined(QCOM_EDK2_PATCH)
  // Explicitly set the delta to be 0 to allow text to be in upper left corner
  ...
  )
{
  ...
  //DEBUG_CODE_BEGIN ();
    ...
    Status = GraphicsConsoleConOutSetMode (&Private->SimpleTextOutput, MaxMode - 1);
#else
...
#if defined (QCOM_EDK2_PATCH)
  // Disable cursor
  Status = GraphicsConsoleConOutEnableCursor(&Private->SimpleTextOutput, FALSE);
#endif
...
#if defined (QCOM_EDK2_PATCH)  
  // Always set mode to be maximium
  ModeNumber = This->Mode->MaxMode - 1;
#endif
