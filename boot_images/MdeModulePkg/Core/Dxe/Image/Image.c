...
#if defined (QCOM_EDK2_PATCH)
  if (gSecurity != NULL || gSecurity2 != NULL)
    SecurityStatus = EFI_SECURITY_VIOLATION;
  else
#endif
...
#if defined (QCOM_EDK2_PATCH)
  CoreNotifySignalList (&gEfiEventFBPTOsLoaderLoadImageStartGuid);
#endif
  ...
  )
{
  ...
  // Parse the input filename to find the portion to copy over
  if (PdbFileName == NULL) {
    ...
    for (Index = StartIndex; Index < EndIndex; Index++) {
      ...
      if (Index1 == DXE_PERFORMANCE_STRING_SIZE - 1) {
	break;
      }
    }
  ...
  )
{
...
#if defined (QCOM_EDK2_PATCH)
  GetNameFromHandle(ImageHandle, Module);
#endif
  ...
  if (Image->JumpBuffer == NULL) {
    ...
    //
#if defined (QCOM_EDK2_PATCH)
    PERF_START (NULL, "StartImage:", Module, Tick);
    PERF_END (NULL, "StartImage:", Module, 0);
#else
...
#if defined (QCOM_EDK2_PATCH)
    //
    //Signal to write LoadTime to FPDT
    //
    if (Image->Type == EFI_IMAGE_SUBSYSTEM_EFI_APPLICATION)
      CoreNotifySignalList (&gEfiEventFBPTOsLoaderStartImageStartGuid);
#endif
    //
  ...
  //
#if defined (QCOM_EDK2_PATCH)
  PERF_START (Handle, "StartImage:", Module, Tick);
  PERF_END (Handle, "StartImage:", Module, 0);
#else
