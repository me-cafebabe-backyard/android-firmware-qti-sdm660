...
#if defined (QCOM_EDK2_PATCH)
	    // Get the remaining length to make this address aligned
	    LengthToAlign = ((PhysicalBase + TT_DESCRIPTOR_SECTION_SIZE) & ~(TT_DESCRIPTOR_SECTION_SIZE-1)) - PhysicalBase;
		
	    if (RemainLength > LengthToAlign) {
		  // Map that in the L2 page table
		  PopulateLevel2PageTable (SectionEntry++, PhysicalBase, LengthToAlign, MemoryRegion->Attributes);
		  // Align the physical address
		  PhysicalBase = (PhysicalBase + TT_DESCRIPTOR_SECTION_SIZE) & ~(TT_DESCRIPTOR_SECTION_SIZE-1);
		  // Update RemainLength
		  RemainLength -= LengthToAlign;
	    } else {
#endif
