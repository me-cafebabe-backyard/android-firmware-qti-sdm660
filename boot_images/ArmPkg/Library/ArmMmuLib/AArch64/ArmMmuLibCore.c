  ...
  )
{
  ...
  case ARM_MEMORY_REGION_ATTRIBUTE_WRITE_BACK_XN:
    return TT_ATTR_INDX_MEMORY_WRITE_BACK|TT_UXN_MASK|TT_PXN_MASK|TT_SH_INNER_SHAREABLE;
  case ARM_MEMORY_REGION_ATTRIBUTE_WRITE_THROUGH_XN:
    return TT_ATTR_INDX_MEMORY_WRITE_THROUGH|TT_UXN_MASK|TT_PXN_MASK|TT_SH_INNER_SHAREABLE;
#endif
...
#if defined(QCOM_EDK2_PATCH)
    return TT_ATTR_INDX_MEMORY_NON_CACHEABLE| TT_UXN_MASK | TT_PXN_MASK;
#else
    return TT_ATTR_INDX_MEMORY_NON_CACHEABLE;
#endif

#if defined (QCOM_EDK2_PATCH)
  case ARM_MEMORY_REGION_ATTRIBUTE_UNCACHED_UNBUFFERED_XN:
    return TT_ATTR_INDX_MEMORY_NON_CACHEABLE| TT_UXN_MASK | TT_PXN_MASK;
  case ARM_MEMORY_REGION_ATTRIBUTE_NONSECURE_UNCACHED_UNBUFFERED_XN:
    return TT_ATTR_INDX_MEMORY_NON_CACHEABLE| TT_UXN_MASK | TT_PXN_MASK;
#endif
...
#if defined(QCOM_EDK2_PATCH)
  // QC Edit:  Calculate LastBlockEntry from T0SZ - this is the last block entry of the root Translation table
  ...
  *LastBlockEntry = TT_LAST_BLOCK_ADDRESS(RootTable, RootTableEntryCount);
  ...
  for (IndexLevel = RootTableLevel; IndexLevel <= PageLevel; IndexLevel++) {
    ...
    if ((IndexLevel != 3) && ((*BlockEntry & TT_TYPE_MASK) == TT_TYPE_TABLE_ENTRY)) {
	  ...
	  // #if defined(QCOM_EDK2_PATCH) -- If we are at the last level then update the output
...
#if defined(QCOM_EDK2_PATCH)
  //Need to do cache operations before disabling caches
...
#if defined(QCOM_EDK2_PATCH)
      (TranslationTableAttribute == ARM_MEMORY_REGION_ATTRIBUTE_UNCACHED_UNBUFFERED_XN) || 
      (TranslationTableAttribute == ARM_MEMORY_REGION_ATTRIBUTE_NONSECURE_UNCACHED_UNBUFFERED_XN) ||
#endif      
...
#if defined(QCOM_EDK2_PATCH)
      (TranslationTableAttribute == ARM_MEMORY_REGION_ATTRIBUTE_WRITE_BACK_XN) ||
#endif
...
#if defined(QCOM_EDK2_PATCH)
      (TranslationTableAttribute == ARM_MEMORY_REGION_ATTRIBUTE_WRITE_THROUGH_XN) || 
#endif
