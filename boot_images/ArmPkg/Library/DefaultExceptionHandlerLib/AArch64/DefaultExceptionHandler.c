  ...
  )
{
...
#if defined(QCOM_EDK2_PATCH) && defined(ENABLE_RAMDUMP)
  /* Save CPU ctxt to UefiInfoBlock */
  SaveUefiContext (SystemContext);

  /* Enable synchronous serial port to flush all logs */
  SerialPortDrain();
  EnableSynchronousSerialPortIO();

  /* Write one byte, now that it is set to synchronous
   * We will loop here till the logs are flushed */
  ...
  DoWatchDogReset();
#else
  /* Do not assert in exception path or ASSERT() will overwrite crash context */
  ASSERT (FALSE);
#endif
