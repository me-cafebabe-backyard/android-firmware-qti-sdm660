  ...
  )
{
  ...
  if (TT_DESCRIPTOR_SECTION_TYPE_IS_PAGE_TABLE(FirstLevelTable[0]))
    NextSectionAttributes = 0; // this should get automatically handled when pages being handled in SyncCacheConfigPage
  else
#endif    
  ...
  )
{
  ...
  switch (Attributes) {
    ...
    case EFI_MEMORY_XP:
#if defined (QCOM_EDK2_PATCH)
      EntryMask |= TT_DESCRIPTOR_PAGE_XN_MASK;
      EntryValue |= TT_DESCRIPTOR_PAGE_XN;
      break;
  ...
  )
{
  ...
  switch(Attributes) {
    ...
    case EFI_MEMORY_XP:
#if defined (QCOM_EDK2_PATCH)    
      EntryMask |= TT_DESCRIPTOR_SECTION_XN_MASK;
      EntryValue |= TT_DESCRIPTOR_SECTION_XN;
      break;
...
#if defined (QCOM_EDK2_PATCH)	  
      Mva = (VOID *)(UINTN)(((UINTN)CurrentDescriptor) >> TT_DESCRIPTOR_SECTION_BASE_SHIFT);
      Mva = (VOID *)(UINTN)(((UINTN)Mva) << TT_DESCRIPTOR_SECTION_BASE_SHIFT);
