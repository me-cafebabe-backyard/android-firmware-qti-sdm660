...
// to be present.
//
static inline DALResult DALSYS_EventCtrlEx(DALSYSEventHandle hEvent, uint32 dwCtrl, uint32 dwParam, void *pPayload, uint32 dwPayloadSize)
{
  assert(false);
  return DAL_ERROR;
}
...
//      
static inline void DALSYS_LogEvent(DALDEVICEID DeviceId, uint32 dwLogEventType, const char * pszFmt, ...) {}

static inline DALResult DAL_DeviceDetach(DalDeviceHandle *hDevice)
{
  return DAL_SUCCESS;
}

static inline DALResult DAL_DeviceAttachEx(const char *pszArg, DALDEVICEID DeviceId,
...
static inline DALResult DAL_DeviceAttach(DALDEVICEID DeviceId, DalDeviceHandle **phDevice)      
{
