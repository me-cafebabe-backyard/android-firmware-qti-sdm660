...
============================================================================*/
/* $Header: //components/rel/rpm.bf/1.8/core/api/power/npa_resource.h#1 $ */
...
{
  struct npa_client   *next;          /* Linked list */
...
{
  struct npa_event          *next;         /* Linked list */
  ...
  struct npa_resource       *resource;     /* Resource element is
                                            * linked to */
  npa_event_trigger_type     trigger_type; /* Trigger Type */
  union 
  {
...
/**
  @brief npa_enable_node - Enable a node

  Enable/re-enable the given node.  
...
/**
  @brief npa_disable_node - Disable a node
  ...
  @param node : definition of node that is to be disabled
*/
void npa_disable_node( npa_node_definition *node ); 

/**
  @brief npa_resource_lock - Lock the resource.
...
/**
  @brief npa_resource_unlock - Unlock the resource.

  Unlock the resource (and by extension, the node) that was previously
