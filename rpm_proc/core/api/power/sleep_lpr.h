...
$Header: //components/rel/rpm.bf/1.8/core/api/power/sleep_lpr.h#1 $
...
/**
 * @brief SLEEP_POW_FUNC_CONST
 *
 * Use this macro to fill power related fields of sleep_lprm if the power
 * saved by mode is constant.
 *
 * @param value_ptr: Constant value representing power savings of this mode.
 */
#define SLEEP_POW_FUNC_CONST(value_ptr) POW_FUNC_CONSTANT, NULL, \
                                        (void *)value_ptr

/**
 * @brief SLEEP_POW_FUNC_POLY_DURATION
 *
 * Use this macro to fill power fields of sleep_lprm if the power saved
 * by this mode is a polynomial function of the duration.
 *
 * @param coeffs: Coefficients (of type sleep_coeffs, defined in global type
 *                definition section) for the polynomial.
 */
#define SLEEP_POW_FUNC_POLY_DURATION(coeffs) POW_FUNC_POLY_DURATION, coeffs, \
                                             ( void * )sleep_power_poly_duration

/**
 * @brief SLEEP_POW_FUNC_CUSTOM_DURATION
 *
 * Use this macro to fill power fields of sleep_lprm if this mode has a
 * custom power saving function.
 *
 * @param coeffs: Coefficients (of type sleep_coeffs) for the power function.
 * @param pow_func: Pointer to power function.
 */
#define SLEEP_POW_FUNC_CUSTOM_DURATION(coeffs, pow_func) \
                            POW_FUNC_CUSTOM_DURATION, coeffs, (void *)pow_func
...
/**
 * @brief SLEEP_LAT_FUNC_CONST
 *
 * Use this macro to fill latency fields of sleep_lprm if the latency
 * (enter/exit) of the mode is constant.
 *
 * @param value_ptr: Constant value of the latency.
 */
#define SLEEP_LAT_FUNC_CONST(value_ptr) LAT_FUNC_CONSTANT, NULL, \
                                        (void *)(value_ptr)

/**
 * @brief SLEEP_LAT_FUNC_POLY
 *
 * Use this macro to fill latency fields of sleep_lprm if the latency
 * function is of polynomial form.
 *
 * @param coeffs: Coefficients (of type sleep_coeffs) for the polynomial.
 */
#define SLEEP_LAT_FUNC_POLY(coeffs) LAT_FUNC_POLY, coeffs, sleep_latency_poly

/**
 * @brief SLEEP_LAT_FUNC_CUSTOM
 *
 * Use this macro to fill latency fields if the it is defined by a custom
 * function.
 *
 * @param coeffs: Coefficients (of type sleep_coeffs) for the latency function.
 * @param pow_func: Pointer to latency function.
 */
#define SLEEP_LAT_FUNC_CUSTOM(coeffs,lat_func) LAT_FUNC_CUSTOM, coeffs, lat_func
 ...
 */
#define SLEEP_BACKOFF_NONE LAT_FUNC_NONE, 0, 0

/**
 * @brief SLEEP_BACKOFF_CONST
 *
 * Use this macro to fill backoff fields if back off time is constant.
 *
 * @param value_ptr: Constant value of the mode back off time.
 */
#define SLEEP_BACKOFF_CONST(value_ptr) SLEEP_LAT_FUNC_CONST(value_ptr)

/**
 * @brief SLEEP_BACKOFF_POLY
 *
 * Indicates that LPRM's backoff time is polynomial in nature.
 *
 * @param coeffs: Coefficients (of type sleep_coeffs) for the polynomial.
 */
#define SLEEP_BACKOFF_POLY(coeffs) SLEEP_LAT_FUNC_POLY(coeffs)
 ...
 *
 * @param coeffs: Coefficients (of type sleep_coeffs) for the back off function.
 ...
 *
 * The structure below stores the coefficients of power and latency functions
...
/** 
 *  @brief The prototype for a function that returns the power that would be
 *         saved for entering the mode.
 *  
 *  Each low power resource mode is required to define a function of this type.
 *  Its purpose is to give the solver a concept of how much power the mode will
 *  save, so that it can figure out if the mode is worth entering, based on the
 *  current latency budget.
 *  
 *  The units of the estimate returned by this function should be (at battery)
 *  microampere-tick (uA*tick).  The estimate should be as accurate as
 *  possible, taking into consideration not only the savings acquired once in
 *  the mode but also the power consumed in order to enter and leave the mode
 *  as well (i.e.  the current consumed while preparing hardware or negotiating
 *  the mode with another processor).
 * 
 *  It is upto the user who defines this function to clip the return value if
 *  required. Sometime it is possible to have a very large power saving which
 *  cannot be accomodated in 32 bit signed integer and hence possibliy resulting
 *  in negative value. 
 * 
 *  When multiple power modes clip to max it is up to the solver's
 *  implementation to determine which modes are selected.
 *
 *  @param coeffs: Coefficients for power equation. For a polynomial power
 *                 function, it represents coeffs[0] + coeffs[1]*duration_tick +
 *                 coeffs[2]*duration_tick^2 + ... + coeffs[n]*duration_tick^n.
 *                 However, for polynomial use the sleep_power_poly_duration
 *                 as declared below in this file.
 *  @param duration_tick: Time in ticks.
 *
 *  @return Total power that will be saved when this mode is entered.
 */
typedef int64_t (*sleep_power_func_type1)(sleep_coeffs *coeffs,
                                        uint64_t duration);
 ...
 */
typedef uint64_t (*sleep_latency_func_type1)(sleep_coeffs *coeffs,
                                             uint64_t x_value);
 ...
 */
typedef void (*sleep_enter_func)(uint64_t wakeup_tick);
...
/**
 * @brief Enum for the tag of type of power function. 
 *
 * The power_func pointer will be cast based on this tag.
 */
typedef enum power_func_tag
{
  POW_FUNC_CONSTANT,              // Constant power
  POW_FUNC_POLY_DURATION,         // Polynomial implementation
  POW_FUNC_CUSTOM_DURATION,       // tag for user's power function
}sleep_power_tag;
...
typedef enum latency_func_tag
{
  ...
  LAT_FUNC_CONSTANT,              // Constant latency
  LAT_FUNC_POLY,                  // Polynomial implementation
...
struct sleep_lprm
{
  ...
  /* 
   * The dependencies of this mode.  See <insert link> for the page which
   * describes the sleep dependency language. 
   */
  const char *dependencies;

  /* 
   * Order dependency string. Describes what order this mode has relative
   * to other modes for transaction.
   */
  const char *mode_order;
  ...
  /* Attributes for power function */
  sleep_power_tag p_tag;
  sleep_coeffs *power_coeffs;
  void* power_func;

  /* Attributes for entering latency function */
  sleep_latency_tag l_enter_tag;
  sleep_coeffs *enter_coeffs;
  void* latency_enter_func;

  /* Attributes for exiting latency function */
  sleep_latency_tag l_exit_tag;
  sleep_coeffs *exit_coeffs;
  void* latency_exit_func;
  ...
  sleep_latency_tag l_backoff_tag;
  sleep_coeffs *backoff_coeffs;
  void* backoff_func;
  ...
  /* The following two fields are for sleep statistics. Sleep will 
  ...
  /* Total number of ticks we were in this low power mode */
  uint64_t in_mode_time;

  /* Number of times this mode is entered */
  uint32 mode_frequency;
...
typedef enum
{
  SLEEP_NO_DURATION,
  SLEEP_INTR_PENDING,
  SLEEP_INTR_PENDING_BEFORE_HALT
} sleep_bail_reason;
 ...
 *         duration.
 */
int64_t sleep_power_poly_duration(sleep_coeffs *coeffs, uint64_t duration);
 ...
 */
uint64_t sleep_latency_poly(sleep_coeffs *coeffs, uint64_t x_value);
 ...
 */
void sleep_define_lpr(sleep_lpr *resource, npa_client_handle client_handle);
