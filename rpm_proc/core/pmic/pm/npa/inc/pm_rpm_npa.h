...
$Header: //components/rel/rpm.bf/1.8/core/pmic/pm/npa/inc/pm_rpm_npa.h#1 $ 
...
typedef struct
{
   unsigned sw_en:1; // Software Enable
   unsigned mode:3; // [FORCED_BYPASS (default), AUTO_BOOST] - max aggregation (left to right)
   unsigned reserved:28; //32-bit boundary
   // 32 bit boundary
   unsigned uvol  :32; // requested voltage from the client
   // 32 bit boundary
   unsigned pin1_uvol :32; // pin1 voltage
   //32-bit boundary
   unsigned pin2_uvol :32; // pin2 voltage
   //32-bit boundary
   unsigned pin3_uvol :32; // pin3 voltage
   //32-bit boundary
}pm_npa_bob_int_rep;
...
/* The number of times we multiply the estimated settling time to
   calculate the maximum allowed settling time on a resource if the
   settling abort is disabled */
#define PM_RPM_MIN_SETTLING_TIME_MULTIPLIER   2

/* The minimum settling time resource needs to be given to settle
   while polling for VREG_READY */
#define PM_RPM_MIN_SETTLING_TIME_POLL_THR     300 //uS
...
typedef enum
{
   ...
   PM_RPM_NPA_INT_CLIENT_BOB_DEPENDENT, // Internal client for BOB sources
