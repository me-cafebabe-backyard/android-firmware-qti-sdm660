...
$Header: //components/rel/rpm.bf/1.8/core/pmic/pm/npa/src/pm_rpm_smps_trans_apply.c#2 $  

when       who     what, where, why
--------   ---     ---------------------------------------------------------- 
08/10/16   al      Adding pin control mode support
...
void pm_rpm_smps_apply(rpm_application_info *info)
{
    ...
    if(!isSawCtrl)
    {
        ...
        else // this is an old request to check if the rail has settled
        {
            ...
            {
               settle_start_time = smps_data->settlingInfo[internal_resource_index].settle_start_time;
               settle_end_time = smps_data->settlingInfo[internal_resource_index].settle_end_time;
               ...
               if(settling_time_elapsed_us >= min_settling_time_us)
               {
                ...
                pm_pwr_is_vreg_ready_alg(&smps_ptr->pm_pwr_data, smps_ptr->comm_ptr,
                  ...
                  if( vreg_status == FALSE )
                  {
                     ...
                     if(settle_start_time == 0)
                     {
                        ...
                        smps_data->settlingInfo[internal_resource_index].settle_start_time = current_time;

                        // calculate the settle end time using the settling time threshold
                        smps_data->settlingInfo[internal_resource_index].settle_end_time = 
                  ...
                  else // settling is done since VREG_OK status is TRUE
                  {
                     ...
                     smps_data->settlingInfo[internal_resource_index].settle_end_time = 0;
                     smps_data->settlingInfo[internal_resource_index].settle_start_time = 0;
                  }
               }
            }
            ...
            if(info->settling_time == 0)
            {
              ...
              if((smps_ptr->pm_pwr_data.pwr_specific_info[internal_resource_index].periph_type == PM_HW_MODULE_HF_BUCK) && 
                 (smps_ptr->pm_pwr_data.pwr_specific_info[internal_resource_index].is_periph_unified_reg == FALSE))
              {
...
int pm_rpm_smps_aggregate(rpm_application_info *info)
{
    ...
    //If access on SMPS is disallowed , only allow child dependent vote to be aggregated
    if(((info->client_type == 0) || (info->client_type == PM_RPM_NPA_CLIENT_SMPS_REQ)) && 
             (smps_data->railInfo[pmic_index][info->id -1].AccessAllowed == 0))
    {
            ...
            if(smpsClientRequest != NULL)
            {
                ...
                else
                {
                    ...
                    if(aggregatedIntRep.freq_reason == PM_NPA_FREQ_REASON_NONE)
                    {
                        ...
                        if((smpsClientRequest->freq != 0) && (smpsClientRequest->freq < PM_SWITCHING_FREQ_INVALID))
                        {
...
void pm_rpm_smps_execute_driver(rpm_application_info *info, pm_npa_smps_int_rep* previous_aggregation)
{
    ...
    uint8 pmic_index = GET_PMIC_INDEX_SMPS(smps_data->resourceType);
    boolean continue_enable_flag = TRUE;
    ...
    if(smps_ptr != NULL)
    {
        ...
        {
            pm_smps_quiet_mode_mask(pmic_index, internal_resource_index, (pm_quiet_mode_type)smps_int_rep->quiet_mode);
        }
        ...
        if((smps_int_rep->uvol != previous_aggregation->uvol) && (smps_int_rep->uvol != 0))
        {
            ...
            stepper_rate = smps_data->settlingInfo[internal_resource_index].stepper_rate;

            // voltage changed, adding settling time.
            if(smps_int_rep->uvol > previous_aggregation->uvol)
            {
                //calculate the estimated settling time
                estimated_settling_time_us = ((smps_int_rep->uvol - previous_aggregation->uvol)/stepper_rate);
            }
            else
            {
               //calculate the estimated settling time
               estimated_settling_time_us = ((previous_aggregation->uvol - smps_int_rep->uvol)/stepper_rate);
            }
        }
        ...
        {
            // Target specific function called to do SMPS pre-enable activity if any
            pm_rpm_target_execute_smps_pre_enable(pmic_index, internal_resource_index, 
                                                  (pm_on_off_type)smps_int_rep->sw_en, 
                                                  &continue_enable_flag);

            // Flag which indicates whether the common sw enable logic needs to be executed
            // or not after the pre_enable function call. This flag is TRUE by default.
            if(continue_enable_flag == TRUE)
            {
               // HF OCP issue work-around - Disable OCP while enabling SMPS HF rail and enable back OCP after the 
               // rail is enabled and settled
               if(smps_int_rep->sw_en > previous_aggregation->sw_en)
               ...
               if(smps_int_rep->sw_en != 0)
               {
                  pm_pwr_ocp_latched_status_clear_alg(&(smps_ptr->pm_pwr_data), smps_ptr->comm_ptr, internal_resource_index, &ocp_occured);
               }
               ...
               if(smps_int_rep->sw_en > previous_aggregation->sw_en)
               {
                  ...
                  min_settling_time_us = smps_data->railInfo[pmic_index][internal_resource_index].MinSettlingTime;
                  // calculate the estimated settling time
                  estimated_settling_time_us = MAX(smps_data->settlingInfo[internal_resource_index].vreg_ok_us, min_settling_time_us);
               }
            }
        }
        ...
        prev_settle_start_time = smps_data->settlingInfo[internal_resource_index].settle_start_time;
           ...
           (smps_int_rep->sw_en != 0) && (smps_data->railInfo[pmic_index][internal_resource_index].SettlingEnabled == PM_SETTLING_EN))
        {
           ...
           if(prev_settle_start_time == 0)
           {
              ...
              smps_data->settlingInfo[internal_resource_index].settle_start_time = curr_settle_start_time;
           ...
           else // rail is already settling
           {
              ...
              prev_settle_end_time = smps_data->settlingInfo[internal_resource_index].settle_end_time;
           ...
           smps_data->settlingInfo[internal_resource_index].settle_end_time = curr_settle_end_time;
              ...
              (info->client_type == PM_RPM_NPA_INT_CLIENT_VS_DEPENDENT))
           {
              ...
              if((smps_data->railInfo[pmic_index][internal_resource_index].StepDownDelay)
                 && (smps_int_rep->uvol < previous_aggregation->uvol))
              {
                DALSYS_BusyWait(smps_data->railInfo[pmic_index][internal_resource_index].StepDownDelay);
              }
...
void pm_rpm_smps_register_resources(rpm_resource_type resourceType, uint32 num_npa_resources , uint8 pmic_index)
{
    ...
    if(num_npa_resources > 0)
    {
        ...
        if(smps_data == NULL)
        {
            ...
            vreg_ok_time = (uint32*)pm_target_information_get_common_info(PM_PROP_SMPS_VREG_OK_TIME);
            ...
            ult_stepper_rate = (uint32*)pm_target_information_get_common_info(PM_PROP_ULT_SMPS_STEPPER_RATE);
            CORE_VERIFY_PTR(ult_stepper_rate);

            stepper_rate = (uint32*)pm_target_information_get_common_info(PM_PROP_SMPS_STEPPER_RATE);
            CORE_VERIFY_PTR(stepper_rate);

            pm_malloc(sizeof(pm_pwr_settling_info_type)*num_npa_resources, (void**)&(smps_data->settlingInfo));
            ...
            { 
                 ...
                 smps_data->settlingInfo[smps_index].vreg_ok_us = vreg_ok_time[0];

                 if(smps_ptr->pm_pwr_data.pwr_specific_info[smps_index].is_periph_unified_reg == TRUE)
                 {
                    err_flag = pm_pwr_volt_get_vstep(&smps_ptr->pm_pwr_data, smps_ptr->comm_ptr, smps_index, &vstep);
                    ...
                    if(smps_ptr->pm_pwr_data.pwr_specific_info[smps_index].periph_type == PM_HW_MODULE_FTS)
                    {
                      smps_data->settlingInfo[smps_index].stepper_rate = (unified_stepper_rate[vstep])/2;
                    }
                    else
                    {
                      smps_data->settlingInfo[smps_index].stepper_rate = unified_stepper_rate[vstep];
                    }
                 }
                 else if(smps_ptr->pm_pwr_data.pwr_specific_info[smps_index].periph_type == PM_HW_MODULE_ULT_BUCK)
                 {
                    smps_data->settlingInfo[smps_index].stepper_rate = ult_stepper_rate[0];
                 }
                 else
                 {
                    smps_data->settlingInfo[smps_index].stepper_rate = stepper_rate[0];
                 }
            }
    ...
    if(num_npa_resources > 0)
    {
        ...
        for(count = 1; count < num_npa_resources + 1; count++)
        {
            ...
            if(current_agg != NULL)
            {
                ...
                //skip over invalid peripherals
                if(smps_ptr->pm_pwr_data.pwr_specific_info[count-1].periph_type == 0)
                {
                    continue;
                }
...
void pm_rpm_smps_settle(rpm_application_info *info)
{
    pm_npa_smps_data_type *smps_data = (pm_npa_smps_data_type *)info->cb_data;
    uint8  internal_resource_index = (uint8)(info->id - 1);
    uint64 settle_start_time = smps_data->settlingInfo[internal_resource_index].settle_start_time;
    uint64 settle_end_time = smps_data->settlingInfo[internal_resource_index].settle_end_time;
    uint32 estimated_settling_time_us = 0;
    pm_smps_data_type* smps_ptr = NULL;
    uint8 pmic_index = GET_PMIC_INDEX_SMPS(smps_data->resourceType);
    ...
    if(num_npa_resources > 0)
    {
        ...
        for(smps_index = 0; smps_index < num_npa_resources; smps_index++)
        {
              ...
              (smps_ptr->pm_pwr_data.pwr_specific_info[smps_index].is_periph_unified_reg == TRUE))
           {
              ...
              // Enable the droop detection VREG_FAULT interrupt 
