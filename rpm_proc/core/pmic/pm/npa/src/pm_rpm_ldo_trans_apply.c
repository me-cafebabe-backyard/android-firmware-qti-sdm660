...
$Header: //components/rel/rpm.bf/1.8/core/pmic/pm/npa/src/pm_rpm_ldo_trans_apply.c#2 $  
...
void pm_rpm_ldo_apply(rpm_application_info *info)
{
    ...
    if(0 == info->settling_time) //  this is a new request
    {
        ...
        pm_rpm_target_pre_dependency_execute(info);
    ...
    else // this is an old request to check if the rail has settled
    {
        ...
        {
           settle_start_time = ldo_data->settlingInfo[internal_resource_index].settle_start_time;
           settle_end_time = ldo_data->settlingInfo[internal_resource_index].settle_end_time;
           ...
           if (settling_time_elapsed_us >= min_settling_time_us)
           {
              ...
              pm_pwr_is_vreg_ready_alg(&ldo_ptr->pm_pwr_data, ldo_ptr->comm_ptr, internal_resource_index, &vreg_status);
              ...
              if(vreg_status == FALSE)
              {
                 ...
                 if(settle_start_time == 0)
                 {
                    ...
                    ldo_data->settlingInfo[internal_resource_index].settle_start_time = current_time;

                    // calculate the settle end time using the settling time threshold
                    ldo_data->settlingInfo[internal_resource_index].settle_end_time = 
              ...
              else // settling is done since VREG_OK status is TRUE
              {
                 ...
                 periph_type = (&ldo_ptr->pm_pwr_data)->pwr_specific_info[internal_resource_index].periph_type;
                 ...
                 ldo_data->settlingInfo[internal_resource_index].settle_end_time = 0;
                 ldo_data->settlingInfo[internal_resource_index].settle_start_time = 0;
              }
           }
        }
...
int pm_rpm_ldo_aggregate(rpm_application_info *info)
{
    ...
    pm_npa_ldo_int_rep *ldo_prev_agg =(pm_npa_ldo_int_rep *)info->current_aggregation;
    ...
    if(ldo_data->railInfo[pmic_index][info->id - 1].AccessAllowed == 0)
    {
        ...
        aggregatedIntRep.input_uvol  = aggregatedIntRep.output_uvol + 
                                     (MAX(safety_hr, aggregatedIntRep.noise_hr) * 1000);
        ...
        if(aggregatedIntRep.output_uvol == 0 && aggregatedIntRep.sw_en  == 1)
        {
            aggregatedIntRep.output_uvol = ldo_prev_agg->output_uvol;
            aggregatedIntRep.input_uvol  = aggregatedIntRep.output_uvol + 
            	                           (MAX(safety_hr, aggregatedIntRep.noise_hr) * 1000);
        }
...
void pm_rpm_ldo_execute_driver(rpm_application_info *info, pm_npa_ldo_int_rep *previous_aggregation)
{
    ...
    uint32 min_settling_time_us = 0;
    uint32 stepper_rate = 0;
    boolean ocp_occured = 0;
    ...
    if(ldo_ptr != NULL)
    {
        ...
        if((ldo_int_rep->output_uvol != previous_aggregation->output_uvol) && (ldo_int_rep->output_uvol != 0))
        {
            ...
            stepper_rate = ldo_data->settlingInfo[internal_resource_index].stepper_rate;

            // voltage changed, adding settling time.
            if(ldo_int_rep->output_uvol > previous_aggregation->output_uvol)
            {
               if(ldo_ptr->pm_pwr_data.pwr_specific_info[internal_resource_index].is_periph_stepper == TRUE)
               {
                  //calculate the estimated settling time
                  estimated_settling_time_us = ((ldo_int_rep->output_uvol - previous_aggregation->output_uvol)/stepper_rate);
               }
               else
               {
                  // giving 50 micro seconds delay for non-stepper ldo's 
                  // only for increasing voltage as recommended by systems
                  estimated_settling_time_us = PM_LDO_NON_STEPPER_VOLT_SETTLING_TIME;
               }
            }
            else
            {
               ...
               {
                  //calculate the estimated settling time
                  estimated_settling_time_us = ((previous_aggregation->output_uvol - ldo_int_rep->output_uvol)/stepper_rate);
               }
            }
        }
        ...
        if(ldo_int_rep->sw_en != previous_aggregation->sw_en)
        {
            ...
            //Before enabling check for OCP and clear OCP status if high
            if(ldo_int_rep->sw_en != 0)
            {   
                pm_pwr_ocp_latched_status_clear_alg(&ldo_ptr->pm_pwr_data, ldo_ptr->comm_ptr, internal_resource_index, &ocp_occured);
            }
            ...
            if(ldo_int_rep->sw_en > previous_aggregation->sw_en)
            {
               ...
               // calculate the estimated settling time
               estimated_settling_time_us = MAX(ldo_data->settlingInfo[internal_resource_index].vreg_ok_us, min_settling_time_us);
            }
        }
        ...
        prev_settle_start_time = ldo_data->settlingInfo[internal_resource_index].settle_start_time;
           ...
           (ldo_int_rep->sw_en != 0) && (ldo_data->railInfo[pmic_index][internal_resource_index].SettlingEnabled == PM_SETTLING_EN))
        {
           ...
           if(prev_settle_start_time == 0)
           {
              ...
              ldo_data->settlingInfo[internal_resource_index].settle_start_time = curr_settle_start_time;
           ...
           else // rail is already settling
           {
              ...
              prev_settle_end_time = ldo_data->settlingInfo[internal_resource_index].settle_end_time;
           ...
           ldo_data->settlingInfo[internal_resource_index].settle_end_time = curr_settle_end_time;
              ...
              (info->client_type == PM_RPM_NPA_CLIENT_LDO_REQ))
           {
              ...
              /*add flat busy wait for stepping down case so that total wait = VREG_OK+ StepDownDelay*/
              if((ldo_data->railInfo[pmic_index][internal_resource_index].StepDownDelay)
                 && (ldo_int_rep->output_uvol < previous_aggregation->output_uvol))
              {
                DALSYS_BusyWait(ldo_data->railInfo[pmic_index][internal_resource_index].StepDownDelay);
              }
           }
        }
...
void pm_rpm_ldo_bypass_cb(uint32 pmic_index, uint32 resource_id, pm_on_off_type on_off)
{
        ...
        (((pm_npa_ldo_int_rep *)ldo_int_rep)->sw_en > 0))/* the LDO currently is ON */
    {
        if((pm_on_off_type)on_off == PM_ON)
        {
            ...
            if(ldo_data->settlingInfo[resource_id-1].settle_start_time == 0)
            {
...
void pm_rpm_ldo_register_resources(rpm_resource_type resourceType, uint32 num_npa_resources , uint8 pmic_index)
{
    ...
    uint32 *unified_stepper_rate = NULL;
    uint32 *ult_stepper_rate = NULL;
    ...
    if(num_npa_resources > 0)
    {
        ...
        if (ldo_data == NULL)
        {
            ...
            vreg_ok_time = (uint32*)pm_target_information_get_common_info(PM_PROP_LDO_VREG_OK_TIME);
            CORE_VERIFY_PTR(vreg_ok_time);

            unified_stepper_rate = (uint32*)pm_target_information_get_common_info(PM_PROP_UNIFIED_RAILS_STEPPER_RATE);
            CORE_VERIFY_PTR(unified_stepper_rate);

            ult_stepper_rate = (uint32*)pm_target_information_get_common_info(PM_PROP_ULT_LDO_STEPPER_RATE);
            CORE_VERIFY_PTR(ult_stepper_rate);

            stepper_rate = (uint32*)pm_target_information_get_common_info(PM_PROP_LDO_STEPPER_RATE);
            CORE_VERIFY_PTR(stepper_rate);

            pm_malloc(sizeof(pm_pwr_settling_info_type)*num_npa_resources, (void**)&(ldo_data->settlingInfo));
            ...
            for( ldo_index = 0; ldo_index < num_npa_resources ; ldo_index++)
            {
                 ...
                 ldo_data->settlingInfo[ldo_index].vreg_ok_us = vreg_ok_time[0];

                 if(ldo_ptr->pm_pwr_data.pwr_specific_info[ldo_index].is_periph_stepper == TRUE)
                 {
                    if(ldo_ptr->pm_pwr_data.pwr_specific_info[ldo_index].is_periph_unified_reg == TRUE)
                    {
                       err_flag = pm_pwr_volt_get_vstep(&ldo_ptr->pm_pwr_data, ldo_ptr->comm_ptr, ldo_index, &vstep);
                       CORE_VERIFY (err_flag == PM_ERR_FLAG__SUCCESS);

                       vstep = (vstep & 0x3); // step_delay mask is 0x3 (bits 1:0)
                       ldo_data->settlingInfo[ldo_index].stepper_rate = unified_stepper_rate[vstep];
                    }
                    else if(ldo_ptr->pm_pwr_data.pwr_specific_info[ldo_index].periph_type == PM_HW_MODULE_ULT_LDO)
                    {
                       ldo_data->settlingInfo[ldo_index].stepper_rate = ult_stepper_rate[0];
                    }
                    else
                    {
                       ldo_data->settlingInfo[ldo_index].stepper_rate = stepper_rate[0];
                    }
                 }
            }
...
void pm_rpm_ldo_register_resource_dependencies(rpm_resource_type resourceType, uint32 num_npa_resources, uint8 pmic_index)
{
    ...
    pm_sw_mode_type sw_mode = PM_SW_MODE_INVALID;
    pm_pwr_resource_dependency_info *depInfo = NULL;
    pm_npa_ldo_data_type *ldo_data = NULL;
    ...
    if(num_npa_resources > 0)
    {
        ...
        for(count = 1; count < num_npa_resources + 1; count++)
        {
            ...
            if(current_agg != NULL)
            {
                ...
                //skip over invalid peripherals
                if(ldo_ptr->pm_pwr_data.pwr_specific_info[count-1].periph_type == 0)
                {
                    continue; 
                }
                ...
                pm_pwr_sw_mode_status_alg(&ldo_ptr->pm_pwr_data, ldo_ptr->comm_ptr, count-1, &sw_mode);
                if (sw_mode == PM_SW_MODE_BYPASS)
                {
                  ldo_ptr->in_bypass[count-1] = TRUE;
                  ldo_data->inBypass[count-1] = PM_ON; 
                }
...
void pm_rpm_ldo_settle(rpm_application_info *info)
{
    pm_npa_ldo_data_type *ldo_data = (pm_npa_ldo_data_type *)info->cb_data;
    uint8  internal_resource_index = (uint8)(info->id - 1);
    uint64 settle_start_time = ldo_data->settlingInfo[internal_resource_index].settle_start_time;
    uint64 settle_end_time = ldo_data->settlingInfo[internal_resource_index].settle_end_time;
    uint32 estimated_settling_time_us = 0;
    pm_ldo_data_type* ldo_ptr = NULL;
    uint8 pmic_index = GET_PMIC_INDEX_LDO(ldo_data->resourceType);
    ...
    if(settle_start_time != 0)
    {
        ...
        // poll VREG
        pm_rpm_check_vreg_settle_status(settle_start_time, estimated_settling_time_us, &ldo_ptr->pm_pwr_data,
