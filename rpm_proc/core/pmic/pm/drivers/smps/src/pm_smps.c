...
$Header: //components/rel/rpm.bf/1.8/core/pmic/pm/drivers/smps/src/pm_smps.c#1 $
...
)
{
    ...
    {
        err_flag = pm_pwr_volt_get_vstep(&(smps_ptr->pm_pwr_data), smps_ptr->comm_ptr, periph_index, vstep_ptr);
    }
    return err_flag;
...
pm_err_flag_type pm_smps_ocp_enable(uint8 pmic_chip, uint8 smps_peripheral_index, boolean enable)
{
    ...
    {
        pm_register_address_type    sec_reg = 0;
        pm_register_address_type    ocp_reg = 0;
        pm_register_address_type    base_addr = 0;
        pm_register_data_type       data = 0;
        pm_register_mask_type       mask = 0;
        pm_pwr_data_type            *pwr_data = &(smps_ptr->pm_pwr_data);
        ...
        /* No use case for OCP on the new subtypes so better not to provide access to user */
        if (pwr_data->pwr_specific_info[smps_peripheral_index].is_periph_unified_reg == TRUE)
        {
            err_flag = PM_ERR_FLAG__FEATURE_NOT_SUPPORTED;
        }
        else 
        {
            ...
            {
                ocp_reg = base_addr + pwr_data->pwr_reg_table->OCP;
                mask = 0x80;
            }
            else
            {
        ...
        if (err_flag == PM_ERR_FLAG__SUCCESS)
        {
            ...
            err_flag |= pm_comm_write_byte_mask(smps_ptr->comm_ptr->slave_id, ocp_reg, mask, data, 0);
        }
    }
