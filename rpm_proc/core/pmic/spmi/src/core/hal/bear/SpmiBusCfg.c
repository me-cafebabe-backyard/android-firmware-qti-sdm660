 ...
 * $Header: //components/rel/rpm.bf/1.8/core/pmic/spmi/src/core/hal/bear/SpmiBusCfg.c#1 $
 * $Change: 11690628 $ 
 * 
 *                              Edit History
 * Date     Description
...
Spmi_Result SpmiBusCfg_SetPortPriorities(uint8 busId, uint8* ports, uint32 numPorts)
{
    ...
    for(i = 0; i < numPorts; i++) 
    {
       ...
       SPMI_HWIO_OUT_FIELD( HWIO_PMIC_ARB_PRIORITIESn_ADDR( base_addr, i ), 
                            HWIO_PMIC_ARB_PRIORITIESn_PORT,
                            ports[i] );
    }
