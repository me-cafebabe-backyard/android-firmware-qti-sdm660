...
================================================================================
  $Header: //components/rel/rpm.bf/1.8/core/debugtrace/lite/src/QDSS.c#2 $
==============================================================================*/
...
static const char qdss_npa_client_name[] = "qdssrpm";
...
#define QDSS_TO_MPM_RATIO_9    2       //QDSS TS counter at 9.6 MHz (check and div)
...
#define QDSS_TS_FREQ_9     9600000

#define QDSS_MPM_COUNTER_FREQ 19200000
...
static void QDSSRestoreTimestamp(void)
{
   ...
   if(qdss.ts_freq >= QDSS_TS_FREQ_19)
   {
   ...
   qdss.qdss_ts_counter_postmin = qdss_ts_counter_postmin;
...
void QDSSInit()
{
   ...
   qdss.qdss_to_mpm_ratio =  QDSS_TO_MPM_RATIO_9; //default

   qdss.npa_client = npa_create_sync_client("/clk/qdss", qdss_npa_client_name, NPA_CLIENT_REQUIRED);
   if (NULL== qdss.npa_client) {
      return;
   }
   npa_issue_required_request(qdss.npa_client,CLOCK_QDSS_LEVEL_DEBUG);
   STMCfgPreInit(qdss.ts_freq);
   TPDAPreInit(qdss.ts_freq);
...
{
   /*If the same frequency is being set do nothing */
   if (dwFreq == qdss.ts_freq) {
      return;
   }
   ...
   case QDSS_TS_FREQ_150:
      qdss.qdss_to_mpm_ratio = (uint32)QDSS_TO_MPM_RATIO_150;
      break;
   case QDSS_TS_FREQ_300:
      qdss.qdss_to_mpm_ratio = (uint32)QDSS_TO_MPM_RATIO_300;
      break;
   case QDSS_TS_FREQ_600:
      qdss.qdss_to_mpm_ratio = (uint32)QDSS_TO_MPM_RATIO_600;
      break;
   case QDSS_TS_FREQ_19:
      qdss.qdss_to_mpm_ratio = (uint32)QDSS_TO_MPM_RATIO_19;
      break;
   case QDSS_TS_FREQ_9:
      qdss.qdss_to_mpm_ratio = (uint32)QDSS_TO_MPM_RATIO_9;
      break;
   default:
      qdss.qdss_to_mpm_ratio = (dwFreq/QDSS_MPM_COUNTER_FREQ);
   }
