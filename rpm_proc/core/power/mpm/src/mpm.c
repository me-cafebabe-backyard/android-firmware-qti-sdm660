...
$Header: //components/rel/rpm.bf/1.8/core/power/mpm/src/mpm.c#2 $
...
DESCRIPTION
  ...
  the core voltage rail tos sleep.
...
{
	/* Ignore absolute voltage check for active and sleep voltages in sleep path.
		Because, we have absolute mode check for active and sleep modes in vdd_min_enter.
	*/
...
)
{
  ...
  SWEVENT(SLEEP_MPM_VOLTAGE, mx_off_uv, mx_on_uv, cx_off_uv, cx_on_uv);
}
