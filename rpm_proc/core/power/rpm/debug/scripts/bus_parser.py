...
class ICBMasterLatencyXlate:
  __metaclass__ = Parser
  id = 72
  def parse(self, data):
    master = get_icb_master_name(data[0])
    latency = data[1]
    return 'icb_master_latency_translate_cb: (master: %s) (latency %d)' % (master, latency)

class ICBMasterLatencyApply:
  __metaclass__ = Parser
  id = 73
  def parse(self, data):
    master = get_icb_master_name(data[0])
    latency = data[1]
    return 'icb_master_latency_apply_cb: (master: %s) (latency %d)' % (master, latency)
