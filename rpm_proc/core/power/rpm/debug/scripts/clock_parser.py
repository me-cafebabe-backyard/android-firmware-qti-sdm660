...
class CLOCKStatus:
  ...
  def parse(self, data):
    if data[1] == 1:
      return 'Clock: %-30s  Requested State = Enable.  Reference Count = %d' % (get_clock(data[0]), data[2])
    else:
      return 'Clock: %-30s  Requested State = Disable. Reference Count = %d' % (get_clock(data[0]), data[2])
