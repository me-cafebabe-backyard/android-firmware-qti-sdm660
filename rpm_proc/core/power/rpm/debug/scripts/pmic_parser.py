...
class PMIC_MSG_x219:
	...
	def parse_v2(self, data):
		return 'PMIC %s Vreg Stepper Settling error' % (get_pmic_rsrc_name(data[0], data[1], data[2]))
	def parse(self, data):
		if get_branch('major') == 1:
			if get_branch('minor') <= 3:
				return self.parse_v1(data)
...
class PMIC_MSG_x21B:
	...
	def parse_v2(self, data):
		pmic_index = ((data[0]>>16) & 0xFF)
		periph_type = ((data[0]>>8) & 0xFF)
		periph_index = (data[0] & 0xFF)
		return 'PMIC %s Settling time estimated = %d, actual = %d' % (get_pmic_rsrc_name(pmic_index, periph_type, periph_index), data[1], data[2])
	def parse(self, data):
		if get_branch('major') == 1:
			if get_branch('minor') <= 3:
				return self.parse_v1(data)
