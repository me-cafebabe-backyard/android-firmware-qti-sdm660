...
{
    uint64_t suppressible_length = 0;
    ...
    while(sched_task)
    {
       ...
       if(!task_start)
       {
         suppressible_length += sched_task->get_length();
         sched_task = sched_task->next_task;
       }
       else
       {
         return (task_start - suppressible_length);
       }
    }
...
void Sched::cpu_speed_change(unsigned int cpu_freq_khz)
{
    if(cpu_freq_khz == cpuFreq_)
      return;
...
{
  theSchedule().cpu_speed_change(cpu_freq_khz);
}
