...
//---------------------------------------------------------------------------
static cpu_mode_id recording_speed;
...
// processor speeds.
static uint64_t duration[CPU_NUM_MODES][ROB_NUM_DURATIONS] = {0};
...
static duration_data_type duration_data[ROB_NUM_DURATIONS] =
  {
    ...
    {
      .start = ROB_EVENT_RPM_CPR_UPDATE_START,
      .end = ROB_EVENT_RPM_CPR_UPDATE_END,
      .max = 29300
    }
  };
...
{
  uint64_t ret = 0;
  CORE_VERIFY(d < ROB_NUM_DURATIONS);
...
===========================================================================*/
uint64_t
rob_get_duration (rob_duration_type d, cpu_mode_id speed)
{
  CORE_VERIFY(speed < CPU_NUM_MODES);	
  CORE_VERIFY(d < ROB_NUM_DURATIONS);	
  return duration[speed][d];
}

void rob_set_recording_speed(cpu_mode_id speed)
{
