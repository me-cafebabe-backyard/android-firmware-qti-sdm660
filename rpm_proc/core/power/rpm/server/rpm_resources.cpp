...
#define RESOURCE_REQUEST_LOG_SIZE          16 /* power of 2 */
#define RESOURCE_REQUEST_IDX_MASK          (RESOURCE_REQUEST_LOG_SIZE - 1)
#define GET_RESOURCE_REQUEST_LOG_PTR(stats) (stats.requests + (RESOURCE_REQUEST_IDX_MASK & stats.request_count))
...
{
    uint8_t ee_id;
    uint8_t msg_id;         /* store only LSB of msg id to save memory */
    uint8_t resource_idx;
    bool    bypassed;
} resource_request_log_t;
...
static struct
{
    uint32 request_count;   /* used as an index of requests[] */
    uint32 bypass_count;
    resource_request_log_t requests[RESOURCE_REQUEST_LOG_SIZE];
} resource_request_stats;
...
static inline void translate_request(ResourceData *r, unsigned ee, kvp_t *data, void *source_buffer, void *dest_buffer)
{
    ...
    if(!source_buffer)
        ...
        memscpy(dest_buffer, rc->inrep_size, source_buffer, rc->inrep_size);
...
bool resource_ee_has_apply_work(const ResourceData *r, const void* request0, const void* request1)
{
    return (!request0 || !request1 || memcmp(request0, request1, rpm->classes[r->class_idx].inrep_size));
}
...
static uint64 resource_ee_apply(ResourceData *r, unsigned ee, void* old_request, void* new_request, uint64 settling_time)
{
    ...
    memscpy(old_state, rc->inrep_size, r->current_state, rc->inrep_size);

    SWEVENT(RPM_APPLY_REQUEST, rc->type, ainfo.id, 0);
...
bool resource_ee_request(ResourceData *r, unsigned ee, rpm_set_type set, kvp_t *data, uint64 &settling_time)
{
    ...
    else if(has_apply_step)
    {
        ...
        memscpy(old_request, inrep_size, client->request[selected], inrep_size);
    }
    ...
    //
    resource_request_stats.request_count++;

    resource_request_log_t *log_ptr = GET_RESOURCE_REQUEST_LOG_PTR(resource_request_stats);
    log_ptr->ee_id  = (uint8_t)ee;

    // Proxy vote on behalf of a master can be sent before rpmserver init.
    if(rpm->ees[ee].handler)
    {
        log_ptr->msg_id = (uint8_t)((Handler*)rpm->ees[ee].handler)->getMsgID();
    }

    if(r >= rpm->resources && r < rpm->resources + rpm->supported_resources)
    {
        log_ptr->resource_idx = (uint8_t)(r - rpm->resources);
    }

    if(resource_ee_has_apply_work(r, old_request, new_request))
    {
        log_ptr->bypassed = false;

        settling_time = resource_ee_apply(r, ee, old_request, new_request, 0);
    }
    else
    {
        log_ptr->bypassed = true;
        resource_request_stats.bypass_count++;

        SWEVENT(RPM_APPLY_REQUEST, rc->type, r->id, resource_request_stats.bypass_count);
...
uint64 resource_ee_transition(ResourceData *r, unsigned ee, rpm_transition_type transition)
{
    ...
    if(resource_ee_has_apply_work(r, client->request[selected], client->request[off_idx]))
    {
        settling_time = resource_ee_apply(r, ee, client->request[selected], client->request[off_idx], 0);
    }
    else
    {
        SWEVENT(RPM_APPLY_REQUEST, rpm->classes[r->class_idx].type, r->id, 1);
    }
...
uint64 resource_client_request(ResourceData *r, unsigned client, unsigned size, void *data, uint64 settling_time)
{
    ...
    memscpy(old_request, size, ic->request, size);
    memscpy(ic->request, size, data, size);
