...
boolean ram_is_swevent_enabled(tracer_event_id_t id) 
{
    uint64 msg_id = 0;
    uint64 shifter = 1;
    msg_id = (shifter << (id%64));
    ...
    return (ram_enabled_bmsk[id/64] & msg_id);
}
    ...
    if ((type < SWEVENT_TYPE_MAX) && (setting < SWEVENT_SETTING_UNDEFINED)) 
    {
        switch(setting)
        {
            ...
            case SWEVENT_SETTING_QDSS:
                ...
                break;
			case SWEVENT_SETTING_ETM: break;
            case SWEVENT_SETTING_UNDEFINED:
                break;
        }
    }
...
__attribute__((section("swevent_dram_reclaim_pool")))
void swevent_init(void)
{
    ...
    for (i = 0; i<SWEVENT_TYPE_MAX; i++)
    {
        ...
        {
        case 3:
          //disable master_state swevent
          ram_enabled_bmsk[3] = 0xFFFFFFEFFFFFFFFF;
          break;
