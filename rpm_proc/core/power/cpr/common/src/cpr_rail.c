 ...
 *
 * $DateTime: 2017/02/20 02:24:59 $
 * $Header: //components/rel/rpm.bf/1.8/core/power/cpr/common/src/cpr_rail.c#3 $
 * $Change: 12484920 $
 */
#include <stdbool.h>
#include <string.h>
...
void cpr_rail_isr(void* ctx)
{
    ...
    {
        cpr_hal_get_results( &rail->hal, &rslts, RESULTS_TIMEOUT_20MS );
...
static void cpr_rail_init_rail_data(cpr_rail* rail)
{
    ...
    for(int modeIdx = 0; modeIdx < railState->modeSettingsCount; modeIdx++)
    {
        ...
        for(int submodeIdx = 0; submodeIdx < modeSettings->subModesCount; submodeIdx++)
        {
            ...
            SWEVENT( CPR_EVT_RAIL_INIT, ((rail->id << 16) | modeSettings->mode), submode->ceiling, submode->floor);
        }
    }
}
...
boolean cpr_rail_process_results(cpr_rail* rail, cpr_results* rslts)
{
    ...
    cpr_isr_data_t        *log             =  cpr_image_log_get_entry(rail);
    ...
    SWEVENT( CPR_EVT_RAIL_ISR, ((rail->id << 16) | currentMode->mode), recommendation, ((rslts->up << 16) | rslts->down) );
    ...
    CPR_LOG_TRACE( " Results ack: %u", log->ackResult );
    ...
    cpr_hal_ack_result( &rail->hal, log->ackResult );
...
static void cpr_rail_adjust_fused_voltages(cpr_rail* rail)
{
    ...
    for(int modeIdx = 0; modeIdx < vp->modesCount; modeIdx++)
    {
        ...
        for(int submodeIdx = 0; submodeIdx < modeSettings->subModesCount; submodeIdx++)
        {
            ...
            SWEVENT( CPR_EVT_RAIL_INIT_OPEN_LOOP, ((rail->id << 16) | modeSettings->mode), submode->ceiling, submode->floor);
        }
    }
}
...
void cpr_rail_set_initial_voltages(cpr_rail* rail, boolean useGlobalCeiling, boolean useGlobalFloor)
{
    ...
    /*
     * Next, calcualte fused voltage for each mode without TARG_VOLT fuse by Open Loop Interpolation.
     */
...
static void cpr_rail_update_lower_modes(cpr_rail* rail, const disablement_params_t *params)
{
    ...
    for(int modeIdx = 0; modeIdx < rail->vp->modesCount-1; modeIdx++)
    {
        ...
        SWEVENT( CPR_EVT_RAIL_DISABLE_MODE, ((rail->id << 16) | mode), restricted_openloop_mode, restricted_closedloop_mode );
    }
}
...
void cpr_rail_enable(cpr_rail* rail, cpr_mode_settings* modeState, cpr_submode_settings* submodeState, boolean changeOveride)
{
    ...
    SWEVENT( CPR_EVT_RAIL_ENABLE, ((rail->id << 16) | modeState->mode), current, ((submodeState->debug.up << 16) | submodeState->debug.down) );
