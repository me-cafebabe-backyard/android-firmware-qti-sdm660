 ...
 * $Header: //components/rel/rpm.bf/1.8/core/power/cpr/common/src/cpr_smem.c#1 $
 * $Change: 11690628 $
 */
#include "cpr_smem.h"
#include "cpr_image.h"
...
static void cpr_smem_read_rail_state(cpr_smem_hdr* hdr, cpr_smem_rail_hdr* railHdr, uint32 *currOffset, cpr_rail* rail, cpr_rail_state* state)
{
    for(int i = 0; i < railHdr->modeStateCount; i++)
    {
        ...
        {
            CORE_VERIFY(modeState->submodesCount);
        ...
        SWEVENT( CPR_EVT_RAIL_INIT_SMEM, ((rail->id << 16) | modeSetting->mode), modeSetting->subModes[0].ceiling, modeSetting->subModes[0].floor, modeSetting->totalMarginMV );
    }
}
...
void cpr_smem_deserialize_config(cpr_rail* rail, cpr_rail_state* state)
{
    ...
    switch(hdr->version)
    {
        case 0: /* Don't abort to support smem.bin without CPR config for pre-silicon env */
            break;
