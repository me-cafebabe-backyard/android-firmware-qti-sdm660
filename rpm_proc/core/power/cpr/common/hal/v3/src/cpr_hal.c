 ...
 * $Header: //components/rel/rpm.bf/1.8/core/power/cpr/common/hal/v3/src/cpr_hal.c#5 $
 * $Change: 12599917 $
 *
 */
#include "cpr_hal.h"
...
static void cpr_hal_read_results( cpr_hal_handle* hdl, cpr_results* rslts)
{
    ...
    if(!hdl->thread->results)
    {
        hdl->thread->results = (cpr_hal_results*)cpr_image_malloc(sizeof(cpr_hal_results));
    }

    results = hdl->thread->results;

    /*
     * CR 1041073 - IRQ status register must be used for interrupt handling as per HW team
     */
    status = CPR_HWIO_IN( HWIO_CPR_IRQ_STATUS_t_ADDR( base, tid ) );

    rslts->up    = !!CPR_HWIO_GET_FIELD_VALUE( status, HWIO_CPR_IRQ_STATUS_t_UP_FLAG );
    rslts->down  = !!CPR_HWIO_GET_FIELD_VALUE( status, HWIO_CPR_IRQ_STATUS_t_DOWN_FLAG );

    *(uint32*)&results->result0 = status = CPR_HWIO_IN( HWIO_CPR_RESULT0_t_ADDR( base, tid ) );
    ...
    *(uint32*)&results->result1 = status = CPR_HWIO_IN( HWIO_CPR_RESULT1_t_ADDR( base, tid ) );
    ...
    *(uint32*)&results->result2 = CPR_HWIO_IN( HWIO_CPR_RESULT2_t_ADDR( base, tid ) );
...
boolean cpr_hal_get_results(cpr_hal_handle* hdl, cpr_results* rslts, uint32 timeout_ticks)
{
    ...
    do {
        ...
        if(rslts->busy) {
             ...
             * That means each loop actually took ~4.9 usec, and 6667 * 4.9 usec = 32668 usec > 32ms:
             *
             *      32ms / (6667 - 164) loops = ~4.9 usec per loop
             *
             * Changed to use QTimer instead of counter to make it more robust.
             */
            now = time_service_now();
