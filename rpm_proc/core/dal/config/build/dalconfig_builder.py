...
#-------------------------------------------------------------------------------
#
#  $Header: //components/rel/rpm.bf/1.8/core/dal/config/build/dalconfig_builder.py#1 $
...
def DALModDir_src_builder(target, source, env):
   ...
   preprocess_xml = str(source[2]) # added by emitter
   ...
   #python $(DALROOT)/tools/cleanup_xml.py -p $(DALROOT)/config/dal_mod_modem_pre.xml -o $(DALROOT)/config/dal_mod_modem_out.xml -c $(DALROOT)/config/DALConfig_modem.c
   ...
   cmds = [
      ...
      propgen_root_cmd + ' --XmlFile=' + clean_xml + ' --DirName=' + target_path + ' --ModDirFile=' + str(target[0]) + ' --RemoveDups=True' + ' --MasterCFile=' + cfile
   ]
...
def DALConfig_src_builder(target, source, env):
   ...
   cmds = [
      ...
      propgen_root_cmd + ' --XmlFile=' + clean_xml + ' --DirName=' + target_path + ' --ConfigFile=' + target_config_full + ' --DevcfgDataFile=' + target_devcfgdata_full + ' --ConfigType=' + env['CONFIG'] + ' --MasterCFile=' + cfile
   ]
