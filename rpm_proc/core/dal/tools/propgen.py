...
def PrintConfig(ModName, CONFIG, devcfgdatafilename, ConfigType, MasterCFileName):
   ...
   CONFIG.write ("#include \"DevConfig.h\"\n")
   CONFIG.write ("#include \"Chipinfo.h\"\n")
   ...
   CONFIG.write("extern char Image$$" + ConfigType + "_DEVCONFIG_DATA$$Base[];\n")
   CONFIG.write("extern char Image$$" + ConfigType + "_DEVCONFIG_DATA$$ZI$$Limit[];\n\n")

   CONFIG.write("const DALPROP_Target_Data DALPROP_Target_Data_" + ConfigType + " = {\n")
   CONFIG.write("    " + dalprop_structptrs_str + ",\n")
   CONFIG.write("    sizeof(" + dalprop_structptrs_str + "),\n")
   CONFIG.write("    " + dalprop_propbin_str + ",\n")
   CONFIG.write("    sizeof(" + dalprop_propbin_str + "),\n")
   if len(ConfigStructList)!=0:
      CONFIG.write("    " + driver_list_str + ",\n")
      CONFIG.write("    sizeof(" + driver_list_str + ")/sizeof(const StringDevice),\n")
   else:
      CONFIG.write("    NULL,\n")
      CONFIG.write("    0,\n")
   
   CONFIG.write("    DALCONFIG_CHIPINFO_FAMILY_MSM" + ConfigType + ",\n")
   CONFIG.write("    Image$$" + ConfigType + "_DEVCONFIG_DATA$$Base,\n")
   CONFIG.write("    Image$$" + ConfigType + "_DEVCONFIG_DATA$$ZI$$Limit,\n};\n\n")
...
def GenerateArray(DriverName, node, GlobalDef, GlobalOffset, remove_dup_names):
   ...
   for Props in node.getElementsByTagName("props"):
      ...
      DevSection.extend([type1, nameoffset, value])
   # 0xFF00FF00 is the end marker for properties 
