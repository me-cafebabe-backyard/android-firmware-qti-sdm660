...
$Header: //components/rel/rpm.bf/1.8/core/dal/framework/sys/src/rpm/DALProps.c#2 $
...
extern const DALProps DALPROP_PropsInfo;
...
DALResult
DAL_StringDeviceObjectLookup(const char *pszDevName, const StringDevice **ppStringDeviceObj, const DALProps *pDALPtr )
{
   uint32 dwDriverHash;
   int dwIndex = 0;
   int dwSize;
...
uint32 GetNameOffsetOrIdVal(uint32 dwAttr, byte *pbCurPropsPos,uint32 *pdwId) 
{
   ...
   memcpy(pdwId,pbCurPropsPos,sizeof(uint32));
                             ...
                             byte *pbPropsBuf)
{
   ...
   memcpy(&dwOffset, pbPropsBuf, sizeof(uint32));
...
void _PopulateUint32Property(DALSYSPropertyVar *pPropVar,
                             byte *pbPropsBuf)
{
   memcpy(&(pPropVar->Val.dwVal), pbPropsBuf, sizeof(uint32));
                                   ...
                                   byte *pbPropsBuf)
{
   ...
   if(pStruct)
   {
      memcpy(&dwIdx,pbPropsBuf,sizeof(uint32));
                        ...
                        uint32 dwId, DALSYSPropertyVar *pDALPropVar)
{
   ...
   {
      //Get the Type
      memcpy(&pDALPropVar->dwType,pbDevicePropsPos,sizeof(uint32));
