...
$Header: //components/rel/rpm.bf/1.8/core/systemdrivers/icb/src/common/icb_rpm_master_latency.c#1 $

when         who     what, where, why
----------   ---     ------------------------------------------------------
2015/06/18   sds     Restructure table search to be smaller.
...
static const char icb_latency_npa_client_name[] = "ICB Latency Driver";
/* -----------------------------------------------------------------------
** Functions
** ----------------------------------------------------------------------- */
...
)
{
  ...
  if( latency != 0 )
  {
    ...
    * and use the lowest latency we support. */
     ...
     * Don't match any zero bw entries in the table.
     * No need to check the top entry, since we default to using that if
     * we match none of the others. */
    const uint16_t *bw_table = latency_table->aBWs[lat_idx];
    uint16_t        bw_MBps  = CEILING_DIV(bw,1000000ULL);
    uint32_t        freq_idx;
    for( freq_idx = 0;
        freq_idx < latency_table->uNumFreqs - 1;
        freq_idx++ )
    {
      if( 0 != bw_table[freq_idx] && bw_MBps <= bw_table[freq_idx] )
      {
        break;
      }
    }
...
void icb_master_latency_init( void )
{
  ...
  for( uint32_t idx = 0; idx < sys_info->num_latency_masters; idx++ )
  {
    ...
    CORE_VERIFY_PTR( master->handle =
      npa_create_sync_client(master->clk, icb_latency_npa_client_name, NPA_CLIENT_REQUIRED) );
  }
