...
$Header: //components/rel/rpm.bf/1.8/core/systemdrivers/icb/src/common/icb_rpmi.h#1 $

when         who     what, where, why
----------   ---     ------------------------------------------------------
2015/06/18   sds     Move bw->clock freq calculation to common code.
                     Add SCHEME_1 support.
2015/03/10   sds     Change clock div macros to support new clock div mode
...
/**
@brief Calculate required clock value based on node/bus parameters.
*/
uint32_t icb_calc_clock_request( uint64_t request, const icb_bus_def_type *pBus,
                                 uint32_t width, uint32_t num_agg_ports );
