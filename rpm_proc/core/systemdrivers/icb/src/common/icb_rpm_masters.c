...
$Header: //components/rel/rpm.bf/1.8/core/systemdrivers/icb/src/common/icb_rpm_masters.c#1 $

when         who     what, where, why
----------   ---     ------------------------------------------------------
2015/10/07   sds     Remove use of memcpy for structure copy.
2015/06/18   sds     Move bw->clock freq calculation to common code.
2015/03/12   sds     Add support for different width modes for clock freq calculation.
...
)
{
  if( current_request != next_request )
  {
    ...
    uint32_t clk_req = icb_calc_clock_request( next_request,
                                               master->soc_data.pBus,
                                               master->master_width,
