...
$Header: //components/rel/rpm.bf/1.8/core/systemdrivers/hal/clk/inc/HALclk.h#4 $
...
typedef enum
{
  ...
  HAL_CLK_SOURCE_GPLL2_EVEN,
  HAL_CLK_SOURCE_MMPLL0_EVEN,
  HAL_CLK_SOURCE_MMPLL1_EVEN,
  HAL_CLK_SOURCE_MMPLL4_EVEN,
  HAL_CLK_SOURCE_MMPLL7_EVEN,
  HAL_CLK_SOURCE_MMPLL10_EVEN,

  HAL_CLK_SOURCE_PLLTEST,
  HAL_CLK_SOURCE_MMPLL6,
 ...
 *  pUserDate     - Pointer to user data for setting in HAL driver
 */
typedef struct
{
 ...
 *
 * Prius and Tesla PLLs could be dynamically configured.
 * We use HAL_CLK_SOURCE_CONFIG_MODE_FULL for SR PLLs. 
 */
typedef enum
{
  HAL_CLK_SOURCE_CONFIG_MODE_FULL,
  HAL_CLK_SOURCE_CONFIG_MODE_SLEW,
  HAL_CLK_SOURCE_CONFIG_MODE_SLEEP,
...
void    HAL_clk_ConfigPLL               ( HAL_clk_SourceType eSource, const HAL_clk_PLLConfigType *pmConfig, HAL_clk_SourceConfigMode eMode );
...
void    HAL_clk_ValidateRegSetTimeout   ( uint32 nAddr, uint32 nMask, uint32 nTimeoutUS );
void    HAL_clk_ValidateRegClearTimeout ( uint32 nAddr, uint32 nMask, uint32 nTimeoutUS );
...
void    Hal_clk_OCMEM_POWER_UP          ( void );
void    Hal_clk_OCMEM_POWER_DOWN        ( void );
uint16 HAL_clk_GetDeviceID              ( void );
void    HAL_clk_TimerHwCntrl            (boolean enable);
void    HAL_clk_SetUserData             ( void* pData);
void*   HAL_clk_GetUserData             ( void );

HAL_clk_SourceType HAL_clk_getDDRSources(const HAL_clk_ClockMuxConfigType *pmConfig);
HAL_clk_SourceType HAL_clk_getBIMCSources(const HAL_clk_ClockMuxConfigType *pmConfig);
HAL_clk_SourceType HAL_clk_getHMSSSources(const HAL_clk_ClockMuxConfigType *pmConfig);
HAL_clk_SourceType HAL_clk_getGFXSources(const HAL_clk_ClockMuxConfigType *pmConfig);
HAL_clk_SourceType HAL_clk_getMPSSSources(const HAL_clk_ClockMuxConfigType *pmConfig);
HAL_clk_SourceType HAL_clk_get_CDSPSources(const HAL_clk_ClockMuxConfigType *pmConfig);
HAL_clk_SourceType HAL_clk_getMDSSSources(const HAL_clk_ClockMuxConfigType *pmConfig);
HAL_clk_SourceType HAL_clk_getLPASSSources(const HAL_clk_ClockMuxConfigType *pmConfig);
HAL_clk_SourceType HAL_clk_getAPSSTCUSSources(const HAL_clk_ClockMuxConfigType *pmConfig);
...
Description:
  Hit abort() if the register field did not get set within the timeout.
...
Description:
  Hit abort() if the register field did not get cleared within the timeout.
