...
$Header: //components/rel/rpm.bf/1.8/core/systemdrivers/hal/clk/src/HALclk.c#1 $
...
void HAL_clk_ValidateRegSetTimeout
(
  uint32 nAddr,
  uint32 nMask,
  uint32 nTimeoutUS
)
{
  ...
  // Hit abort since it hits timeout
  CORE_VERIFY( FALSE );
...
void HAL_clk_ValidateRegClearTimeout
(
  uint32 nAddr,
  uint32 nMask,
  uint32 nTimeoutUS
...
void HAL_clk_SetUserData
(
  void* pData
)
{
  HAL_clk.mHALClkCtxt.pUserData = pData;
}
...
void* HAL_clk_GetUserData
(
  void
)
{
  return HAL_clk.mHALClkCtxt.pUserData;
}
