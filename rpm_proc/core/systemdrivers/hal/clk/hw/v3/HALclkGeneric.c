...
$Header: //components/rel/rpm.bf/1.8/core/systemdrivers/hal/clk/hw/v3/HALclkGeneric.c#2 $
...
typedef enum{
CLOCK_CFG_ENTER = 0,
AFTER_CMD_CFG_ROOT_ENABLE,
AFTER_CMD_CFG_UPDATE_DONE,
CLOCK_CFG_EXIT,
CLOCK_CFG_INVALID = 0x7FFFFFF
}CLOCK_CFG_MUX_DBG;
volatile CLOCK_CFG_MUX_DBG clk_cfg_mux_dbg = CLOCK_CFG_INVALID;
volatile uint32 nSaveCmdCGRAddr;

void HAL_clk_GenericConfigMuxRootEnable
(
...
)
{
  ...
  nSaveCmdCGRAddr = 0x7FFFFFF;
  debug_marker(clk_cfg_mux_dbg, CLOCK_CFG_ENTER);
   ...
   */
  nSaveCmdCGRAddr = pmClockDomainDesc->nCGRAddr;
  ...
  if(bRootEnable)
  {
	...
	debug_marker(clk_cfg_mux_dbg, AFTER_CMD_CFG_ROOT_ENABLE);
  }
  ...
  debug_marker(clk_cfg_mux_dbg, AFTER_CMD_CFG_UPDATE_DONE);
  if(bRootEnable)
  {
    /*
  ...
  debug_marker(clk_cfg_mux_dbg, CLOCK_CFG_EXIT);
} /* HAL_clk_GenericConfigMux */

void HAL_clk_GenericNoRootConfigMux
(
  HAL_clk_ClockDomainDescType      *pmClockDomainDesc,
  const HAL_clk_ClockMuxConfigType *pmConfig
)
{
  HAL_clk_GenericConfigMuxRootEnable( pmClockDomainDesc, pmConfig, FALSE );
}
...
{
  HAL_clk_GenericConfigMuxRootEnable( pmClockDomainDesc, pmConfig, TRUE );
}
