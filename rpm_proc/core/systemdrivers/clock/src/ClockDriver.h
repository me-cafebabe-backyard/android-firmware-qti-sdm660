...
/*
  ...
  $Header: //components/rel/rpm.bf/1.8/core/systemdrivers/clock/src/ClockDriver.h#1 $
...
#define CLOCK_FLAG_DOMAIN_SUPPRESSIBLE        0x00000200
...
/*
 * Suspend state for clock driver voltage requests.
 */
typedef enum
{
  CLOCK_VOLT_REQ_OPEN,          // allow all voltage votes, transition railways
  CLOCK_VOLT_REQ_SUSPENDED,     // abort if voltage vote requires a railway transition
  CLOCK_VOLT_REQ_SUSPENDED_INIT // allow all voltage votes, don't transition railways
} ClockVoltReqSuspStateType;
...
typedef struct ClockVRegConfigType
{
  ...
  ClockVRegLevelType  eRailVoteLevel;                       /** Current level that vote to railway */
  ClockVRegLevelType  eSuspRailVoteLevel;                   /** Current "suspend mode" vote to railway */
  int32               nRailID;
...
struct ClockSourceNodeType
{
  ...
  uint32                         nReferenceCount;     /**< Number of active domains */
...
typedef struct
{
  ...
  uint32                         nReferenceCountInSuppress;     /**< Number of active insuppressible clocks */
  ClockVRegLevelType             eVRegLevel;                    /**< Voltage request */
  ...
  uint32                         nReferenceCountPreSleep;       /**< Last ref-count before deep sleep entry */
  ClockMuxConfigType            *pActiveMuxConfigPreSleep;      /**< Last config before deep sleep entry */
} ClockDomainNodeType;
...
typedef struct
{
  ...
  npa_client_handle              XOHandle;            /**<  XO NPA client handle */
  uint32                         nNumInsupDomain;     /**< Number of insuppressible clock domain */
...
** =========================================================================*/
/**
  This function prepares the voltage vote for VDDA_EBI rail

  @param 
    eInitLevel - Initial voltage level
...
/**
  ...
  This function is used internally for a clock domain to terminate a previous
  voltage regulator level request.
...
static inline DALResult Clock_VoltageTerminateRequest
(
  ClockDrvCtxt        *pDrvCtxt,
  ClockVRegLevelType  *peCurrentLevel,
  boolean             bGFXVReg
...
{
  /*
   * Clock driver's local railway aggregator doesn't track client counts for
   * the lowest railway level, 'CLOCK_VREG_LEVEL_LOW_MINUS', so this is
   * equivalent to terminating the client's current request.
   */
  return Clock_VoltageRequest(pDrvCtxt,
                              peCurrentLevel,
...
** =========================================================================*/
/**
  Sets new clock driver voltage request suspend state and updates railway
  iff requested. 

  @param *pDrvCtxt               [in] -- Handle to the DAL driver context.
  @param eNewVoltReqSuspendState [in] -- New voltage request suspend state.
  @param bUpdateRailway          [in] -- Railway update request.
