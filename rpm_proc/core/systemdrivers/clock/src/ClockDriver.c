...
/*
  ...
  $Header: //components/rel/rpm.bf/1.8/core/systemdrivers/clock/src/ClockDriver.c#1 $
...
/**
  ...
  @dependencies
    None.
*/
static void Clock_SetupXONode(ClockDrvCtxt* pDrvCtxt, unsigned int unused2, const char **resources, unsigned int num_resources)
{
  pDrvCtxt->XOHandle = npa_create_sync_client("/xo/cxo", "ClockRPM", NPA_CLIENT_REQUIRED );
  CORE_VERIFY_PTR(pDrvCtxt->XOHandle);
  
  /* Vote for disable xo shutdown if insuppressible clocks are on */
  npa_issue_required_request(pDrvCtxt->XOHandle, pDrvCtxt->nNumInsupDomain);  
}
...
)
{
  ...
  /*--------------------------------------------------------------------------------*/
  /* Create XO handle for preventing XO shutdown if clock domain is insuppressible  */
  /*--------------------------------------------------------------------------------*/

  npa_resource_available_cb( "/xo/cxo", (npa_callback)Clock_SetupXONode, pDrvCtxt);
...
)
{
  ...
  {
    /* In RPM, mark all clocks as DOMAIN_SUPPRESSIBLE, so they can be suppress for XO shutdown. */
    CLOCK_FLAG_SET( &(pDrvCtxt->aClockDomains[nClockDomainIndex]), DOMAIN_SUPPRESSIBLE);		
     ...
     */
    if ((ClockMuxConfig.eSource < HAL_CLK_NUM_OF_SOURCES) &&
...
/* =========================================================================
**  Function : Clock_SetInsupClock
** =========================================================================*/
/**
  Increment Insuppressible Clock Domain

  This function is used to keep track on number of insuppressible clock domains.
  Set vote to prevent XO shutdown if number of insuppressible clock domains is 
  one or more, and remove the vote if the number of insuppressible clock domain
  is zero.

  @param bIncrement [in] - TRUE  - Increase number of clock domain.
                           FALSe - Decrease number of clock domain.
...
static void Clock_SetInsupClock( boolean bIncrement )
{
  // Increase number of insuppressible clock by 1
  if ( bIncrement )
  {
    // Vote a the first time only
    if ( (pDrvCtxt->XOHandle != NULL) && (pDrvCtxt->nNumInsupDomain == 0) )
    {
      npa_issue_required_request(pDrvCtxt->XOHandle, 1);  
    }

    pDrvCtxt->nNumInsupDomain++;
  }
  // Descrease number of insuppressible clock by 1
  else
  {
    if ( (pDrvCtxt->XOHandle != NULL) && (pDrvCtxt->nNumInsupDomain == 1) )
    {
      npa_issue_required_request(pDrvCtxt->XOHandle, 0);  
    }
    
    if ( pDrvCtxt->nNumInsupDomain > 0 )
    {
      pDrvCtxt->nNumInsupDomain--;
    }
  }
...
)
{
  ...
  if ( (pClockDomain->nReferenceCount == 0) && (pClockDomain->nReferenceCountInSuppress == 0) )
  {
    /* If this domain has a controllable source, enable it first */
    if (pClockDomain->pSource != NULL)
    {
      eResult = Clock_EnableSource( pDrvCtxt, pClockDomain->pSource);
  ...
  if ( CLOCK_FLAG_IS_SET(pClockDomain, DOMAIN_SUPPRESSIBLE) )
  {
    pClockDomain->nReferenceCount++;
  }
  else
  {
    // Increase number of insuppressible clocks
    pClockDomain->nReferenceCountInSuppress++;

    if ( pClockDomain->nReferenceCountInSuppress == 1 )
    {
      Clock_SetInsupClock( TRUE );
    }
  }
...
)
{
  ...
  if ( (pClockDomain->nReferenceCount == 1) ||
       (pClockDomain->nReferenceCountInSuppress == 1) )
  {
    /* If this domain has a controllable source, disable it */
    if (pClockDomain->pSource != NULL)
    {
      eResult = Clock_DisableSource(pDrvCtxt, pClockDomain->pSource);
    ...
    if (!CLOCK_FLAG_IS_SET(pClockDomain, DOMAIN_HOLD_VREG_REQUEST))
    {
      ...
      {
        Clock_VoltageRequest( pDrvCtxt, 
                              &pClockDomain->eVRegLevel, CLOCK_VREG_LEVEL_LOW_MINUS, bGFXVReg);
      }
    }
  }

  /* Check if the domain are suppressible */
  bSuppressibleRequest = CLOCK_FLAG_IS_SET(pClockDomain, DOMAIN_SUPPRESSIBLE);

  /* Decrement the reference count */
  if ( (bSuppressibleRequest) && (pClockDomain->nReferenceCount > 0) )
  {
    pClockDomain->nReferenceCount--;
  }
  else if ( (!bSuppressibleRequest) && (pClockDomain->nReferenceCountInSuppress > 0) )
  {
    // Reduce number of insuppressible clocks
    pClockDomain->nReferenceCountInSuppress--;

    if ( pClockDomain->nReferenceCountInSuppress == 0 )
    {
      Clock_SetInsupClock( FALSE );
    }
  }
...
)
{
   ...
   */
  bVoltageRequest =
    ((pDomain->nReferenceCount > 0) || (pDomain->nReferenceCountInSuppress > 0)) &&
   ...
   */
  if ( ((pDomain->nReferenceCount > 0) || (pDomain->nReferenceCountInSuppress > 0)) &&
...
)
{
  ...
  {
    eResult = Clock_SetClockConfig(pClock, pNewConfig);
...
)
{
  ...
  *pnFrequencyHz = 0;
  ...
  else
  {
    ...
    if ( HALConfig.eSource < HAL_CLK_NUM_OF_SOURCES )
    {
