...
#endif
#define SHARED_IMEM_BASE 0x666BF000
...
static void DataRAMConfig_InternalDataRAM(MPUPermissionRange* range)
{
    ...
    range->end_addr   = (unsigned) rpm_image_section_array[RPM_DATA_RAM].section_end;
}
...
static void DataRAMConfig_SharedDataRAM(MPUPermissionRange* range)
{
    ...
    range->end_addr   = (unsigned) rpm_image_section_array[RPM_LOG].section_end;
}
...
static const DataRAMConfig dram_cfg =
{
    ...
    .perms =
    {
        ...
        {   // Shared data RAM (must be after Internal data RAM config to overwrite access permission)
...
__attribute__((section("xpu_dram_reclaim_pool")))
