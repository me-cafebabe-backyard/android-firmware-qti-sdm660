...
def dump(debug_data): #dump_path, memory, debug_info, target_family):
    global ee_names
    global dump_error
    dump_error = 0
    ...
    target_family = debug_data['family']
    
    debug_data['printer'].p("Dumping RPM server state...")
        ...
        for ee in range(rpm.num_ees):
            ...
            try:
                if ee_data.stats.address != 0 :
                    stats = ee_data.stats
                    num_shutdowns    = stats.num_shutdowns
                    shutdown_req     = stats.shutdown_req
                    wakeup_ind       = stats.wakeup_ind
                    bringup_req      = stats.bringup_req
                    bringup_ack      = stats.bringup_ack
                    wakeup_reason    = "scheduled" if stats.wakeup_reason else "rude"
                    sleep_duration   = stats.last_sleep_transition_duration
                    wake_duration    = stats.last_wake_transition_duration
                    print('\t\tnum_shutdowns                  : %d' % num_shutdowns, file=ee_status_file)
                    print('\t\tshutdown_req                   : 0x%016X' % (shutdown_req), file=ee_status_file)
                    print('\t\twakeup_ind                     : 0x%016X' % (wakeup_ind), file=ee_status_file)
                    print('\t\tbringup_req                    : 0x%016X' % (bringup_req), file=ee_status_file)
                    print('\t\tbringup_ack                    : 0x%016X' % (bringup_ack), file=ee_status_file)
                    print('\t\twakeup_reason                  : %s' % wakeup_reason, file=ee_status_file)
                    print('\t\tlast_sleep_transition_duration : 0x%08X (%u usec)' % (sleep_duration, sleep_duration/19.2), file=ee_status_file)
                    print('\t\tlast_wake_transition_duration  : 0x%08X (%u usec)' % (wake_duration, wake_duration/19.2), file=ee_status_file)
                    print('\t\txo_count                       : %d' % stats.xo_count, file=ee_status_file)
                    print('\t\txo_last_entered_at             : 0x%016X' % stats.xo_last_entered_at, file=ee_status_file)
                    print('\t\txo_last_exited_at              : 0x%016X' % stats.xo_last_exited_at, file=ee_status_file)
                    print('\t\txo_accumulated_duration        : 0x%016X (%u usec)' % (stats.xo_accumulated_duration, stats.xo_accumulated_duration/19.2), file=ee_status_file)
                else :
            ...
            if check_member("smdlPort_", handler):
              try:
            ...
            try:
                ...
                if check_member("smdlPort_", handler):
                  ...
                  print('\t\t\tSMDL info: NONE', file=ee_status_file)
...
def dump_struct(address, memory, debug_info, struct_type, raw_size):
    ...
    for member_name, member_value in obj.members():
        if max_len_name < len(member_name):
            max_len_name = len(member_name)
        if max_len_val < len(str(member_value)):
            max_len_val = len(str(member_value))
    fmt = '\t\t{name:%d} : {val:%s%d}' % (max_len_name, '>' if str(member_value).isdigit() else '<', max_len_val)
    for member_name, member_value in obj.members():
        out_lines.append(fmt.format(name=member_name, val=member_value))
    out_lines.append('\t}')
    return '\n'.join(out_lines)
...
def format_request(address, rcd, memory, debug_info):
    ...
    xlate_funcs_to_req_type = {
        ...
        'pm_rpm_boost_translation'      : 'pm_npa_boost_int_rep',
        'pm_rpm_boost_byp_translation'  : 'pm_npa_boost_byp_int_rep',
        'pm_rpm_bob_translation'        : 'pm_npa_bob_int_rep',
...
def dump_ee_requests(dump_path, rpm, memory, debug_info):
    ...
    for ee in range(rpm.num_ees):
        ...
        with open(ee_reqs_file_name, 'w') as ee_reqs_file:
            ...
            print('\nRPM Estimator Clock Frequencies', file=ee_reqs_file)
            print('\tcpu_kHz : [%d, %d]' % (cpu_kHz[0], cpu_kHz[1]), file=ee_reqs_file)
                ...
                if (True == check_member('ee_id',resource.ees)):
                    ...
                    try:
                      ...
                      ee_reqs.append({
                          ...
                          'spare_type'       : 'NO_REQUEST',
                          'spare_request'    : 'n/a',
                          'message'          : 'ERROR PROCESSING CLIENTS',
                      })
                      dump_error += 1
                      continue
                else:
            ...
            for ee in range(rpm.num_ees):
                ...
                if (True == check_member('ee_id',resource.ees)):
                    ...
                    try:
                      ...
                      print('\n*** %s ***' % ee_names[ee], file=f)
                      print('\tERROR PROCESSING CLIENTS', file=f)
                      dump_error += 1
                      continue
                else:
                    #Old EEClient structure
