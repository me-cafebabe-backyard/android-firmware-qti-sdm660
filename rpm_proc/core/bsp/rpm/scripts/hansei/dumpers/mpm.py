...
def dump(debug_data): #dump_path, memory, debug_info, fill_char):
    ...
    debug_data['printer'].p("Dumping mpm registers...")
...
def dump_mpm(dump_path, rpm, memory, debug_info, fill_char):
    ...
    with open(mpm_file_name, 'w') as mpm_file:
        ...
        # This now works with the fix for array parsing in the dwarf tools
        mpm_regs = decode_object('rpm_mpm_registers', mpm_address, None, memory, debug_info, die=mpm_type)

        reg_num = 0
        for reg in mpm_regs:
            print(("\n\tREGISTER[%02.0u] = 0x%0.8x" % (reg_num, reg)).expandtabs(tab_size), file=mpm_file)
            
            try:
                mpm_reg = mpm_info[reg_num]
                print(("\t\tRegister Name: %s"%(mpm_reg['name'])).expandtabs(tab_size), file=mpm_file)
                if mpm_reg['name'] != mpm_reg['type'] :
                    bitfield = mpm_common_types[mpm_reg['type']]
                else:
                    bitfield = mpm_reg['bitfield']
                print("\t\tRegister Decode:".expandtabs(tab_size), file=mpm_file)
                index = 0
                while index < 32 :
                    region = bitfield[index]
                    data = reg #mpm_array[reg_num]
                    data = data >> index
                    mask = 1
                    ...
                    hex_out = ("{{0:{0}>8s}}".format(fill_char)).format(("{{0:0>{0}X}}".format(hex_digits)).format(data))
                    out_str = "\t\t\t[{0:2d}:{1:2d}] {2:>25s} = 0x{3:>8s}".format(index, region['end'], region['name'], hex_out )
                    print(out_str.expandtabs(tab_size), file=mpm_file)
                    
                    
                    index = region['end']+1
            except:
                print("\t\t!!! No decode available for this mpm register. !!!\n".expandtabs(tab_size), file=mpm_file)
            reg_num+=1
