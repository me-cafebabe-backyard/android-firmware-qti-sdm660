...
def dump(debug_data): #dump_path, memory, debug_info, ocimem_load, pmic_rtc_load):
    global dump_error
    dump_error = 0
    memory = debug_data['rpm_memory']
    debug_info = debug_data['debug_info']
    dump_path = debug_data['dump_path']
    ocimem = debug_data.get('ocimem_memory', None)
    pmic_mem = debug_data.get('pmic_memory', None)
    
    debug_data['printer'].p("Dumping watchdog events...")
    ...
    dump_wdog(dump_path, rpm, memory, debug_info, ocimem, pmic_mem)
...
def dump_wdog(dump_path, rpm, memory, debug_info, ocimem, pmic_mem):
    ...
    with open(wdog_file_name, 'w') as wdog_file:
        ...
        for index_num in range(4):
            ...
            print("\t\t\t\t-> dog event source: 0x{0:0>8x}".format(esource.address), file=wdog_file)
        ...
        if ocimem:
          try:
              SDI_PMIC_RTC = unpack('<I',ocimem.read(0x5774, 4))[0]
          except:
        ...
        if pmic_mem:
          try:
              SDI_PMIC_RTC_v2 = unpack('<I',pmic_mem.read(0x0, 4))[0]
          except:
              if ocimem_load == 0:
