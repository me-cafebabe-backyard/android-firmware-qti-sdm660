...
def dump_resource_clients(dump_file, resource, reclaim):
    ...
    while client.address != 0:
        ...
        if max_len_name < len(client_name):
          max_len_name = len(client_name)

        client_type = client.type
        if max_len_type < len(client_type):
          max_len_type = len(client_type)
        ...
        if max_len_req < len('{0}'.format(request)):
          max_len_req = len('{0}'.format(request))
    ...
    while client.address != 0:
        ...
        try:
          ...
          #fmt = '        npa_client (name: %s) (handle: 0x%x) (resource: 0x%x) (type: %s) (request: %d)'
          #dat = (client_name, client.address, resource.address, client_type, request)
          #print(fmt % dat, file=dump_file)
          
          fmt = '        npa_client (name: {name:%d}) (handle: {hdl}) (resource: {rsrc}) (type: {type:%d}) (request: {req:>%d})' % (max_len_name, max_len_type, max_len_req)
          dat = {
            'name': client_name,
            'hdl' : hex(client.address),
            'rsrc': hex(resource.address),
            'type': client_type,
            'req' : request
          }
          print(fmt.format(**dat), file=dump_file)
          #print(fmt.format(client_name, hex(client.address), hex(resource.address), client.type, request), file=dump_file)
          client = client.next
        except:
