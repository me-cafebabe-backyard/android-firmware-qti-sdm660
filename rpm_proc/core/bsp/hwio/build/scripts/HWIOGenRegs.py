...
#  ===========================================================================
#
#  $Header: //components/rel/rpm.bf/1.8/core/bsp/hwio/build/scripts/HWIOGenRegs.py#1 $
...
def ProcessField (field, regfile):
  ...
  value  = '%#10x%s' % (field.Mask(), 'uLL' if Is64Bit(field.Mask()) else '')
  ...
  if regfile['output-fvals']:
    for fval in field.values:
      ...
      value  = '%#10x%s' % (fval.value, 'uLL' if Is64Bit(fval.value) else '')
      output += form % (define, value)
...
def ProcessRegister (register, regfile):
  ...
  value  = '%#10x%s' % (register.mask, 'uLL' if Is64Bit(register.mask) else '')
  output += form % (define, value)

  if regfile['output-resets'] and register.reset_value != None:
    define = 'HWIO_%s_POR' % (register.Symbol())
    if Is64Bit(register.mask):
      value  = '%#018xuLL' % (register.reset_value)
    else:
      value  = '%#010x' % (register.reset_value)
    output += form % (define, value)

    define = 'HWIO_%s_POR_RMSK' % (register.Symbol())
    if Is64Bit(register.mask):
      value  = '%#018xuLL' % (register.reset_mask)
    else:
      value  = '%#010x' % (register.reset_mask)
    output += form % (define, value)
  ...
  # Set the access macro to use dword (32 bits) or qword (64 bits).
  if Is64Bit(register.mask):
    access_macro = 'qword'
    int_type = 'uint64'
  ...
  if IsRegisterShadowed(register, regfile):
    if regfile['explicit-addressing']:
      ...
      output += '        out_' + access_macro + '_masked(HWIO_%s_ADDR(x),%s,v,HWIO_%s_shadow)\n' % (register.Symbol(), full_mask, register.Symbol())
      output += '#define HWIO_%s_OUTM(x,m,v) \\\n' % register.Symbol()
      output += '        out_' + access_macro + '_masked(HWIO_%s_ADDR(x),m,v,HWIO_%s_shadow)\n' % (register.Symbol(), register.Symbol())
    else:
      ...
      output += '        out_' + access_macro + '_masked(HWIO_%s_ADDR,%s,v,HWIO_%s_shadow)\n' % (register.Symbol(), full_mask, register.Symbol())
      output += '#define HWIO_%s_OUTM(m,v) \\\n' % register.Symbol()
      output += '        out_' + access_macro + '_masked(HWIO_%s_ADDR,m,v,HWIO_%s_shadow)\n' % (register.Symbol(), register.Symbol())
    ...
    if register.Readable():
      if regfile['explicit-addressing']:
        output += '#define HWIO_%s_IN(x)      \\\n' % register.Symbol()
        output += '        in_' + access_macro + '_masked(HWIO_%s_ADDR(x), HWIO_%s_RMSK)\n' % (register.Symbol(), register.Symbol())
        output += '#define HWIO_%s_INM(x, m)      \\\n' % register.Symbol()
        output += '        in_' + access_macro + '_masked(HWIO_%s_ADDR(x), m)\n' % register.Symbol()
      else:
        output += '#define HWIO_%s_IN          \\\n' % register.Symbol()
        output += '        in_' + access_macro + '_masked(HWIO_%s_ADDR, HWIO_%s_RMSK)\n' % (register.Symbol(), register.Symbol())
        output += '#define HWIO_%s_INM(m)      \\\n' % register.Symbol()
        output += '        in_' + access_macro + '_masked(HWIO_%s_ADDR, m)\n' % register.Symbol()

    if register.Writeable():
      if regfile['explicit-addressing']:
        output += '#define HWIO_%s_OUT(x, v)      \\\n' % register.Symbol()
        output += '        out_' + access_macro + '(HWIO_%s_ADDR(x),v)\n' % register.Symbol()
        if register.Readable():
          output += '#define HWIO_%s_OUTM(x,m,v) \\\n' % register.Symbol()
          output += '        out_' + access_macro + '_masked_ns(HWIO_%s_ADDR(x),m,v,HWIO_%s_IN(x))\n' % (register.Symbol(), register.Symbol())
      else:
        output += '#define HWIO_%s_OUT(v)      \\\n' % register.Symbol()
        output += '        out_' + access_macro + '(HWIO_%s_ADDR,v)\n' % register.Symbol()
        if register.Readable():
          output += '#define HWIO_%s_OUTM(m,v) \\\n' % register.Symbol()
          output += '        out_' + access_macro + '_masked_ns(HWIO_%s_ADDR,m,v,HWIO_%s_IN)\n' % (register.Symbol(), register.Symbol())
...
def ProcessRegisterArray (register, regfile):
  ...
  if IsRegisterShadowed(register, regfile):
    ...
    output += '        out_' + access_macro + '_masked(%s,%s,val,HWIO_%s_shadow[%s])\n' % (hwio_addr, full_mask, register.Symbol(), register.ArgList(']['))
    output += '#define HWIO_%s_OUTMI%s(%s,mask,val) \\\n' % (register.Symbol(), index, arg_list)
    output += '        out_' + access_macro + '_masked(%s,mask,val,HWIO_%s_shadow[%s])\n' % (hwio_addr, register.Symbol(), register.ArgList(']['))
    ...
    if 'shadow-file' in regfile:
      if register.dimensions == 1:
        ...
        regfile['shadow-file'].write(
          ...
          '  ' + int_type + '  HWIO_%s[%d][%d];\n\n' % (register.Symbol(), register.max_index[0] + 1, register.max_index[1] + 1))
  else:
    if register.Readable():
      output += '#define HWIO_%s_INI%s(%s)        \\\n' % (register.Symbol(), index, arg_list)
      output += '        in_' + access_macro + '_masked(%s, HWIO_%s_RMSK)\n' % (hwio_addr, register.Symbol())
      output += '#define HWIO_%s_INMI%s(%s,mask)    \\\n' % (register.Symbol(), index, arg_list)
      output += '        in_' + access_macro + '_masked(%s, mask)\n' % hwio_addr
    if register.Writeable():
      output += '#define HWIO_%s_OUTI%s(%s,val)    \\\n' % (register.Symbol(), index, arg_list)
      output += '        out_' + access_macro + '(%s,val)\n' % hwio_addr
      if register.Readable():
        output += '#define HWIO_%s_OUTMI%s(%s,mask,val) \\\n' % (register.Symbol(), index, arg_list)
        output += '        out_' + access_macro + '_masked_ns(%s,mask,val,HWIO_%s_INI%s(%s))\n' % (hwio_addr, register.Symbol(), index, arg_list)
...
/*
  ...
  Copyright (c) %d Qualcomm Technologies Incorporated.''' % date.today().year + '''
  All Rights Reserved.
