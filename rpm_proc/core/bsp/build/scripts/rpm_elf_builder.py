        ...
        for seg in range (elf_header.e_phnum):
            ...
            if (curr_phdr.p_type == self.LOAD_TYPE) and \
                ...
                for attr in self.attribute_list:
                  ...
                  print "segment:", seg, "orig:", hex (old_val), "new:", hex (val)
...
def rpm_elf_builder (target, source, env):
   ...
   elif env['MSM_ID'] in ['8998', '660', '405', 'nicobar']:
      re = RpmElfBuilder (env, source_str, target_str, [('segment.paddr', '+0x00200000', '0,1,2,3'), ('segment.vaddr', '+0x00200000', '0,1,2,3'), ('entry_point', '+0x00200000', '')])
   else:
      re = RpmElfBuilder (env, source_str, target_str, ('segment.paddr', '+0xfc000000', '0,1,2,3'))
   re.generate ()
