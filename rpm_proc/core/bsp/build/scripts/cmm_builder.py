...
#-------------------------------------------------------------------------------
#
#  $Header: //components/rel/rpm.bf/1.8/core/bsp/build/scripts/cmm_builder.py#1 $
...
valid_images = {'rpm_proc': 'RPM', 'rpm' : 'RPM', 'adsp_proc':'ADSP', 'mba':'MPSS','modem_proc':'MPSS', 'wcnss_proc':'WCNSS','core':'MPSS','apps_proc':'APPS'}
# Tech teams
...
def cmm_menubuilder(env):
	...
	if mainenv[CMMBUILDER_LIST]:
		for i in range(len(mainenv[CMMBUILDER_LIST])):
			...
			scripts_sorted = sorted(scripts.iteritems(), key=get_display_name)
			for scriptname, properties in scripts_sorted:
