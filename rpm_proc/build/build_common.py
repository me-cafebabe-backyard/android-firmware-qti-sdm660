...
def find_compiler_llvm():
    print '\n\n\nAuto-detect your build environment...'
    
    for compiler in compiler_llvm_types:
...
def find_compiler_rvct():
    print '\n\n\nAuto-detect your build environment...'
    
    for compiler in compiler_rvct_types:
        paths = list(itertools.chain(*[glob.glob(path) for path in compiler['paths']]))
        if not paths:
            print 'Did not find any installs of', compiler['name']
	    continue
...
def create_target_specific_build_artifacts():
  ...
  know_build_variants = ['AAAAANAAR', 'AAAAANAZR', 'AAAAANADR']
  ...
  target_artifact_dir = '../core/bsp/rpm/build/'+dest_msm_id+'/'
  target_artifact = target_artifact_dir + 'RPM_' + build_variant
  source_artifact_dir = '../core/bsp/rpm/build/'
  source_artifact = source_artifact_dir + 'RPM_' + build_variant
...
#                [target_artifact + '.htm',                  source_artifact + '.htm'],
                ...
                [target_artifact + '_rpm.elf',              source_artifact + '_rpm.elf'],
...
#                '.htm',
  ...
  installed_mbn_path = '../build/rpm/bf/build'

  if os.path.exists('../core/bsp/rpm/build/'+'/RPM_'+build_variant+'.mbn'):
    ...
    for variant in know_build_variants:                 # Remove other build variants' artifacts
      if variant != build_variant:
        if os.path.exists('../core/bsp/rpm/build/RPM_'+variant):
          shutil.rmtree('../core/bsp/rpm/build/RPM_'+variant)
        if os.path.exists('../build/ms/bin/'+variant):
          shutil.rmtree('../build/ms/bin/'+variant)
        if os.path.exists('../core/bsp/rpm/build/'+dest_msm_id+'/'):
          shutil.rmtree('../core/bsp/rpm/build/'+dest_msm_id+'/')
        for S_artifact_ext in artifacts_ext:
          if os.path.exists(source_artifact_dir + 'RPM_' + variant + S_artifact_ext):
            os.remove(source_artifact_dir + 'RPM_' + variant + S_artifact_ext)
    ...
    if os.path.exists('../build/rpm/bf/build'):
    ...
    if os.path.exists('../core/bsp/rpm/build/RPM_'+build_variant):
      shutil.rmtree('../core/bsp/rpm/build/RPM_'+build_variant)
...
def start_build():
  ...
  # pass 'rpm' as COMMAND_LINE_TARGETS if no args passed in order to not generate/delete HWIO files.
  # SCons is set to use 'all' as the default BUILD TARGETS, if COMMAND_LINE_TARGETS is not specified.
  if not any(target in passed_args for target in ['rpm', 'hwio', 'all']):
      passed_args += 'rpm'
...
if __name__ == '__main__':
  ...
  DEFAULT_BUILD_ID = 'AAAAANAZR'
  
  os.environ['BUILD_ID']  = 'AAAAANAZR'
  ...
  if( os.environ.get('ARMPATH', 'none') == 'none' ):
    if os.environ['COMPILER_TYPE'] == 'llvm_compiler':
	    if find_compiler_llvm():
             start_build()
    else:
	    if find_compiler_rvct():
		    start_build()
